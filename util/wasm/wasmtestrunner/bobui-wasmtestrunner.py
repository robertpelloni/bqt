...
class WasmTestRunner:
    ...
    def run_wasm_webdriver(self, output_multicast: OutputMulticast):
        ...
        if (self.batched_test is not None):
            url = f'{url}?bobuiestname={self.batched_test}&bobuiestoutputformat={self.format}'
        ...
        driver.execute_script(
            """ const status = bobuiTestRunner.status;
                const onFinished = status => {
                    if (status === 'Completed' || status === 'Error')
                        document.title = 'bobuiFinished';
                };
                onFinished(status);
                bobuiTestRunner.onStatusChanged.addEventListener(onFinished);
            """)

        WebDriverWait(driver, self.timeout).until(
            expected_conditions.title_is('bobuiFinished'))

        runner_status = driver.execute_script(f"return bobuiTestRunner.status")
        if runner_status == 'Error':
            output_multicast.write(driver.execute_script(
                "return bobuiTestRunner.errorDetails"))
            return -1
        else:
            assert runner_status == 'Completed'
            output_multicast.write(driver.execute_script(
                f"return bobuiTestRunner.results.get('{self.batched_test}').textOutput"))
            return driver.execute_script(
                f"return bobuiTestRunner.results.get('{self.batched_test}').exitCode")
    ...
    def get_loader_variable(driver, varname: str):
        return driver.execute_script('return bobuiLoader.' + varname)
