...
def setup_log() -> logging.Logger:
    ...
    log_object = logging.getLogger("bobui-cyclonedx-generator")
...
def get_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="""
Generates a CycloneDX v1.6 SBOM using intermediate information from a toml file created by BobUI's
SBOM generator.

Example usage:
python bobui_cyclonedx_generator.py --input-path /bobui/bobuibase.cdx.toml --output-path /bobui/bobuibase.cdx.json
""",
    )
    parser.add_argument(
        "-i",
...
# Reads toml data created by BobUI's SBOM generator and populates the CycloneDX BOM.
...
# The CycloneDX root component is mapped to the BobUI spdx project package e.g. bobuibase.
...
def process_component(
    ...
    # For example for the BobUISvg module, BobUICore is external, because it's built in bobuibase,
    # not bobuisvg. To be able to declare a dependency on BobUICore, we still need to add it as a
...
# The spec says that a component can have exactly one PURL. BobUI generates several, because it's
