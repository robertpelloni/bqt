...
#
#       bobuibase/tests/auto/util/testrunner/tst_bobui_testrunner.py -v [--debug]
#
# ======== bobui-testrunner ========
#
# This script wraps the execution of a BobUI test executable, for example
...
# TESTRUNNER="bobui-testrunner.py --" to stop it from parsing further args.
#
# NOTE: this script is crafted specifically for use with BobUI tests and for
#       using it in BobUI's CI. For example it detects and acts specially if test
#       executable is "tst_selftests" or "androidtestrunner".  It also detects
#       env var "COIN_CTEST_RESULTSDIR" and uses it as log-dir.
#
# NOTE_2: Why is bobui-testrunner considering exit code outside [0,127] as CRASH?
...
#         BobUI-testrunner wants to catch both of these cases as CRASH.
#         On Windows, a crash is usually accompanied by exitcode >= 0xC0000000.
#         Finally, BOBUIest is limiting itself to exit codes in [0,127]
...
VERBOSE_ENV = {
    "BOBUI_LOGGING_RULES": "*=true",
    "BOBUI_MESSAGE_PATTERN": "[%{time process} %{if-debug}D%{endif}%{if-warning}W%{endif}%{if-critical}C%{endif}%{if-fatal}F%{endif}] %{category} %{file}:%{line} %{function}()  -  %{message}",
}
# The following special function names can not re-run individually.
NO_RERUN_FUNCTIONS = {
    "initTestCase", "init", "cleanup", "cleanupTestCase"
}
# The following tests do not write XML log files properly. bobui-testrunner will
...
def parse_args():
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
        description="""
Wrap BobUI test execution. This is intended to be invoked via the TESTRUNNER
...
Default flags: --max-repeats 5 --passes-needed 1
    ...
    # then append "--" and append our BOBUIest-specific arguments.
    #
    ...
    if args.test_basename == "androidtestrunner":
        if "--" in args.testargs:
            L.critical("bobui-testrunner can't handle pre-existing '--' argument to androidtestrunner")
            sys.exit(1)
        args.specific_extra_args = [ "--" ]
        apk_arg = False
        aab_arg = False
...
def run_test(arg_list: List[str], **kwargs):
    if (os.environ.get("BOBUI_TESTRUNNER_TESTING", "0") == "1"
        and os.name == "nt"
        and arg_list[0].endswith(".py")
    ):
        # For executing bobui_mock_test.py under the same Python interpreter when testing.
...
def unique_filename(test_basename: str) -> str:
    ...
    if os.environ.get("BOBUI_TESTRUNNER_DEBUG_NO_UNIQUE_OUTPUT_FILENAME"):
...
def run_full_test(test_basename, testargs: List[str], output_dir: str,
    ...
    # Append arguments to write log to bobuiestlib XML file,
    # and text to stdout.
