...
{
    using type = typename BobUIPrivate::ReduceResultType<std::decay_t<Callable>>::ResultType;
};
...
{
    using type = std::decay_t<typename BobUIPrivate::ArgResolver<Callable>::First>;
};
...
struct MapSequenceResultType
{
    static_assert(std::is_same_v<typename InputSequence::value_type,
                                 BobUIPrivate::MapResultType<InputSequence, MapFunctor>>,
...
{
    typedef InputSequence<BobUIPrivate::MapResultType<InputSequence<T...>, MapFunctor>> ResultType;
};

#endif // BOBUI_NO_TEMPLATE_TEMPLATE_PARAMETER
