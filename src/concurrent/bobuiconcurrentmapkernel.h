...
public:
    ...
    MapKernel(BOBUIhreadPool *pool, Iterator begin, Iterator end, F &&_map)
...
template <typename ReducedResultType,
          ...
          typename Reducer = ReduceKernel<ReduceFunctor,
                                          ReducedResultType,
                                          BobUIPrivate::MapResultType<Iterator, MapFunctor>>>
class MappedReducedKernel : public IterateKernel<Iterator, ReducedResultType>
{
    ...
    Reducer reducer;
    using IntermediateResultsType = BobUIPrivate::MapResultType<Iterator, MapFunctor>;

public:
    typedef ReducedResultType ReturnType;

    template<typename F1 = MapFunctor, typename F2 = ReduceFunctor>
    MappedReducedKernel(BOBUIhreadPool *pool, Iterator begin, Iterator end, F1 &&_map, F2 &&_reduce,
...
template <typename Iterator, typename MapFunctor>
class MappedEachKernel : public IterateKernel<Iterator, BobUIPrivate::MapResultType<Iterator, MapFunctor>>
{
    MapFunctor map;
    using T = BobUIPrivate::MapResultType<Iterator, MapFunctor>;

public:
    template <typename F = MapFunctor>
    MappedEachKernel(BOBUIhreadPool *pool, Iterator begin, Iterator end, F &&_map)
...
struct SequenceHolder1 : private BobUIPrivate::SequenceHolder<Sequence>, public Base
{
    template<typename S = Sequence, typename F = Functor>
    SequenceHolder1(BOBUIhreadPool *pool, S &&_sequence, F &&functor)
        : BobUIPrivate::SequenceHolder<Sequence>(std::forward<S>(_sequence)),
