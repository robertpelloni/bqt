...
ThreadEngineStarter<void> filterInternal(BOBUIhreadPool *pool, Sequence &sequence,
...
QFuture<void> filter(BOBUIhreadPool *pool, Sequence &sequence, KeepFunctor &&keep)
{
    return filterInternal(pool, sequence, std::forward<KeepFunctor>(keep),
                          BobUIPrivate::PushBackWrapper());
}

template <typename Sequence, typename KeepFunctor>
QFuture<void> filter(Sequence &sequence, KeepFunctor &&keep)
{
    return filterInternal(BOBUIhreadPool::globalInstance(),
                          sequence, std::forward<KeepFunctor>(keep), BobUIPrivate::PushBackWrapper());
}

// filteredReduced() on sequences
template <typename ResultType, typename Sequence, typename KeepFunctor, typename ReduceFunctor>
QFuture<ResultType> filteredReduced(BOBUIhreadPool *pool,
                                    Sequence &&sequence,
                                    KeepFunctor &&keep,
...
#ifndef Q_QDOC
template <typename Sequence, typename KeepFunctor, typename ReduceFunctor,
          std::enable_if_t<BobUIPrivate::isInvocable<KeepFunctor, Sequence>::value, int> = 0,
...
QFuture<ResultType> filteredReduced(Iterator begin,
                                    ...
                                    ReduceOptions options = ReduceOptions(UnorderedReduce
                                                                          | SequentialReduce))
{
    return startFilteredReduced<ResultType>(BOBUIhreadPool::globalInstance(), begin, end,
                                            std::forward<KeepFunctor>(keep),
                                            std::forward<ReduceFunctor>(reduce), options);
}
...
{
    return startFilteredReduced<ResultType>(
            BOBUIhreadPool::globalInstance(), begin, end, std::forward<KeepFunctor>(keep),
            std::forward<ReduceFunctor>(reduce),
            ResultType(std::forward<InitialValueType>(initialValue)), options);
}
...
template <typename Sequence, typename KeepFunctor>
QFuture<typename std::decay_t<Sequence>::value_type> filtered(BOBUIhreadPool *pool,Sequence &&sequence,
                                                              KeepFunctor &&keep)
{
...
QFuture<typename std::decay_t<Sequence>::value_type> filtered(Sequence &&sequence,
                                                              KeepFunctor &&keep)
{
    return startFiltered(BOBUIhreadPool::globalInstance(), std::forward<Sequence>(sequence),
                         std::forward<KeepFunctor>(keep));
}

// filtered() on iterators
template <typename Iterator, typename KeepFunctor>
QFuture<typename qValueType<Iterator>::value_type> filtered(BOBUIhreadPool *pool,
                                                            Iterator begin,
                                                            Iterator end,
                                                            KeepFunctor &&keep)
{
...
QFuture<typename qValueType<Iterator>::value_type> filtered(Iterator begin,
                                                            ...
                                                            KeepFunctor &&keep)
{
    return startFiltered(BOBUIhreadPool::globalInstance(), begin, end,
                         std::forward<KeepFunctor>(keep));
}

// blocking filter() on sequences
template <typename Sequence, typename KeepFunctor>
void blockingFilter(BOBUIhreadPool *pool, Sequence &sequence, KeepFunctor &&keep)
{
...
template <typename ResultType, typename Sequence, typename KeepFunctor, typename ReduceFunctor>
ResultType blockingFilteredReduced(BOBUIhreadPool *pool,
                                   Sequence &&sequence,
                                   KeepFunctor &&keep,
                                   ReduceFunctor &&reduce,
                                   ReduceOptions options = ReduceOptions(UnorderedReduce
...
template <typename Sequence, typename KeepFunctor>
std::decay_t<Sequence> blockingFiltered(BOBUIhreadPool *pool, Sequence &&sequence, KeepFunctor &&keep)
{
...
std::decay_t<Sequence> blockingFiltered(Sequence &&sequence, KeepFunctor &&keep)
{
    return blockingFilteredReduced<std::decay_t<Sequence>>(
            BOBUIhreadPool::globalInstance(), std::forward<Sequence>(sequence),
            std::forward<KeepFunctor>(keep), BobUIPrivate::PushBackWrapper(), OrderedReduce);
}

// blocking filtered() on iterators
template <typename OutputSequence, typename Iterator, typename KeepFunctor>
OutputSequence blockingFiltered(BOBUIhreadPool *pool, Iterator begin, Iterator end, KeepFunctor &&keep)
{
...
{
    return blockingFilteredReduced<OutputSequence>(BOBUIhreadPool::globalInstance(), begin, end,
                                                   std::forward<KeepFunctor>(keep),
                                                   BobUIPrivate::PushBackWrapper(), OrderedReduce);
}
