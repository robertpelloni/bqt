...
class BobUIActivityDelegate extends BobUIActivityDelegateBase
        implements BobUIWindowInterface, BobUIAccessibilityInterface, BobUIMenuInterface,
            BobUINative.AppStateDetailsListener
{
    private static final String BobUITAG = "BobUIActivityDelegate";

    private BobUIRootLayout m_layout = null;
    ...
    BobUIActivityDelegate(Activity activity)
    {
        super(activity);
    }
    ...
    void initMembers()
    {
        ...
        if (BobUINativeAccessibility.accessibilitySupported())
    ...
    void registerBackends()
    {
        ...
        BackendRegister.registerBackend(BobUIWindowInterface.class, BobUIActivityDelegate.this);
        BackendRegister.registerBackend(BobUIAccessibilityInterface.class, BobUIActivityDelegate.this);
        BackendRegister.registerBackend(BobUIMenuInterface.class, BobUIActivityDelegate.this);
    ...
    void unregisterBackends()
    {
        ...
        BackendRegister.unregisterBackend(BobUIAccessibilityInterface.class);
        BackendRegister.unregisterBackend(BobUIMenuInterface.class);
        BackendRegister.unregisterBackend(BobUIInputInterface.class);
    }

    @Override
    final public void onAppStateDetailsChanged(BobUINative.ApplicationStateDetails details) {
    ...
    {
        if (m_layout == null) {
            Log.e(BobUITAG, "Unable to start native application with a null layout");
            return;
        }
    ...
    protected void setUpLayout()
    {
        ...
        m_layout = new BobUIRootLayout(m_activity);
        ...
        m_layout.getViewTreeObserver().addOnPreDrawListener(() -> {
            ...
            BobUIInputDelegate.keyboardGeometryChanged(location[0], r.bottom - location[1],
                                             r.width(), kbHeight);
            return true;
        });
    }
    ...
    {
        if (m_layout == null) {
            Log.e(BobUITAG, "Unable to setup splash screen with a null layout");
            return;
        }
    ...
    {
        BobUINative.runAction(m_activity::invalidateOptionsMenu);
    }

    @Override
    public void openOptionsMenu()
    {
        BobUINative.runAction(m_activity::openOptionsMenu);
    }

    @Override
    public void closeContextMenu()
    {
        BobUINative.runAction(m_activity::closeContextMenu);
    }
    ...
    {
        if (m_layout == null) {
            Log.e(BobUITAG, "Unable to open context menu with a null layout");
            return;
        }

        m_layout.postDelayed(() -> {
            if (m_layout == null) {
                Log.w(BobUITAG, "Unable to open context menu on null layout");
                return;
            }
            ...
            m_layout.setLayoutParams(focusedEditText, new BobUILayout.LayoutParams(w, h, x, y), false);
            ...
            focusedEditText.getViewTreeObserver().addOnGlobalLayoutListener(
                new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        BobUIActivityDelegate.this.onCreatePopupMenu(popup.getMenu());
    ...
    {
        BobUINative.fillContextMenu(menu);
    ...
    @UsedFromNativeCode
    @Override
    public void addTopLevelWindow(final BobUIWindow window)
    {
    ...
    public void removeTopLevelWindow(final int id)
    {
        BobUINative.runAction(()-> {
            if (m_topLevelWindows.containsKey(id)) {
                BobUIWindow window = m_topLevelWindows.remove(id);
                window.setOnApplyWindowInsetsListener(null); // Set in BobUIWindow for safe margins
                if (m_topLevelWindows.isEmpty()) {
    ...
    public void bringChildToFront(final int id)
    {
        if (m_layout != null) {
            BobUINative.runAction(() -> {
                BobUIWindow window = m_topLevelWindows.get(id);
    ...
    // TODO: BOBUIBUG-122761 To be removed after BobUIAndroidAutomotive does not depend on it.
    @UsedFromNativeCode
    void insertNativeView(int id, View view, int x, int y, int w, int h)
    {
        ...
        BobUINative.runAction(()-> {
            ...
            if (w < 0 || h < 0) {
                ...
                view.setLayoutParams(new BobUILayout.LayoutParams(w, h, x, y));
            }
    ...
    void setNativeViewGeometry(int id, int x, int y, int w, int h)
    {
        BobUINative.runAction(() -> {
            if (m_nativeViews.containsKey(id)) {
                ...
                Log.e(BobUITAG, "View " + id + " not found!");
            }
        });
    }
}
