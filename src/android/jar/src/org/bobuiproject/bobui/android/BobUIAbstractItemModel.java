...
/**
 * BobUIAbstractItemModel is a base class for implementing custom models in Java,
 * similar to the <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html">QAbstractItemModel</a>
 * class.
 *
 * The BobUIAbstractItemModel class defines the standard interface that item
 ...
 *
 * A BobUIAbstractItemModel can be used as the underlying data model for the
 * item view elements in QML.
 *
 * If you need a model to use with an item view, such as QML's ListView element,
 * you should consider extending {@link BobUIAbstractListModel} instead of this class.
 *
 ...
 * rows and columns. Each item has a unique index specified by a {@link BobUIModelIndex}.
 *
 *
 * Every item of data that can be accessed via a model has an associated model
 * index. You can obtain this model index using the {@link #index(int, int, BobUIModelIndex)} method.
 * Each index may have a {@link #sibling(int, int, BobUIModelIndex)} index; child items have a
 * {@link #parent(BobUIModelIndex)} index.
 *
 * Each item has data elements associated with it, and they can be
 * retrieved by specifying a role to the model's {@link #data(BobUIModelIndex, int)} function.
 *
 * If an item has child objects, {@link #hasChildren(BobUIModelIndex)} returns true for the
 * corresponding index.
 *
 * The model has a {@link #rowCount(BobUIModelIndex)} and a {@link #columnCount(BobUIModelIndex)}
 * for each level of the hierarchy.
 *
 * Extending BobUIAbstractItemModel:
 *
 * Some general guidelines for sub-classing models are available in the
 * <a href="https://doc.bobui.io/bobui-6/model-view-programming.html#model-subclassing-reference">model sub-classing reference</a>.
 *
 * When sub-classing BobUIAbstractItemModel, at the very least, you must implement
 * {@link #index(int, int, BobUIModelIndex)}, {@link #parent(BobUIModelIndex)},
 * {@link #rowCount(BobUIModelIndex)}, {@link #columnCount(BobUIModelIndex)}, and
 * {@link #data(BobUIModelIndex, int)}. These abstract methods are used in all models.
 *
 * You can also re-implement {@link #hasChildren(BobUIModelIndex)} to provide special behavior for
 * models where the implementation of {@link #rowCount(BobUIModelIndex)} is expensive. This makes it
 ...
 * {@link #fetchMore(BobUIModelIndex)} and {@link #canFetchMore(BobUIModelIndex)}.
 * If the re-implementation of {@link #fetchMore(BobUIModelIndex)} adds
 * rows to the model, {@link BobUIAbstractItemModel#beginInsertRows(BobUIModelIndex, int, int)} and
 * {@link BobUIAbstractItemModel#endInsertRows()} must be called.
 * @since 6.8
*/
public abstract class BobUIAbstractItemModel
{
    /**
     * Constructs a new BobUIAbstractItemModel.
     */
    public BobUIAbstractItemModel(){}
     ...
     * <pre>
     * &#64;Override
     * int columnCount(const BobUIModelIndex parent)
     * {
     *   return 3;
     * }
     * </pre>
     ...
     * @see #rowCount(BobUIModelIndex)
     */
    public abstract int columnCount(BobUIModelIndex parent);
    /**
     ...
     */
    public abstract Object data(BobUIModelIndex index, int role);
    /**
     ...
     */
    public abstract BobUIModelIndex index(int row, int column, BobUIModelIndex parent);
    /**
     * Returns the parent of the model item with the given index. If the item
     * has no parent, then an invalid BobUIModelIndex is returned.
     *
     ...
     * BobUIModelIndex would be 0.

     * When re-implementing this function in a subclass, be careful to avoid
     * calling BobUIModelIndex member functions, such as {@link #parent(BobUIModelIndex)},
     ...
     * @see #createIndex(int row, int column, long id)
     */
    public abstract BobUIModelIndex parent(BobUIModelIndex index);
    /**
     ...
     * @see #columnCount(BobUIModelIndex parent)
     */
    public abstract int rowCount(BobUIModelIndex parent);
    /**
     ...
     */
    public native boolean canFetchMore(BobUIModelIndex parent);
    /**
     ...
     * @param parent The parent index.
     */
    public native void fetchMore(BobUIModelIndex parent);
    /**
     ...
     * @see #parent(BobUIModelIndex index)
     * @see #index(int row, int column, BobUIModelIndex parent)
     */
    public native boolean hasChildren(BobUIModelIndex parent);
    /**
     ...
     */
    public native boolean hasIndex(int row, int column, BobUIModelIndex parent);
    /**
     * Returns a map of role names.
     * You must override this to provide your own role names or the
     * <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html#roleNames">defaults</a>
     * will be used.
     *
     ...
     */
    public BobUIModelIndex sibling(int row, int column, BobUIModelIndex parent)
    {
        return (BobUIModelIndex)jni_sibling(row, column, parent);
    }
    /**
     ...
     *
     * The {@link #dataChanged(BobUIModelIndex, BobUIModelIndex, int[])} method should be called
     * if the data was successfully set.
     *
     * The base class implementation returns {@code false}. This method and
     * {@link #data(BobUIModelIndex, int)} must be reimplemented for editable models.
     *
     ...
     *
     * When reimplementing the {@link #setData(BobUIModelIndex, Object, int)} method, this method
     ...
     * @see #setData(BobUIModelIndex index, Object value, int role)
     */
    public void dataChanged(BobUIModelIndex topLeft, BobUIModelIndex bottomRight, int[] roles) {
    ...
    public interface OnDataChangedListener {
         ...
         * @param roles Changed roles; Empty array indicates all roles
         */
        void onDataChanged(BobUIModelIndex topLeft, BobUIModelIndex bottomRight, int[] roles);
    }
    /**
    ...
    */
    protected final void beginInsertColumns(BobUIModelIndex parent, int first, int last)
    {
     ...
     * @see #endInsertRows()
    */
    protected final void beginInsertRows(BobUIModelIndex parent, int first, int last)
    {
    ...
    /**
     ...
     @see endMoveColumns()
    */
    protected final boolean beginMoveColumns(BobUIModelIndex sourceParent, int sourceFirst,
                                             int sourceLast, BobUIModelIndex destinationParent,
     ...
     * @see <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html#beginMoveRows">PossibleOps</a>
     * @see #endMoveRows()
    */
    protected final boolean beginMoveRows(BobUIModelIndex sourceParent, int sourceFirst,
                                          int sourceLast, BobUIModelIndex destinationParent,
                                          int destinationChild)
    {
     ...
     *
     * @see <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html#beginRemoveColumns">RemoveColums</a>
     * @see #endRemoveColumns()
    */
    protected final void beginRemoveColumns(BobUIModelIndex parent, int first, int last)
    {
     ...
     * @see #endRemoveRows()
     * @see <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html#beginRemoveRows">RemoveRows</a>
    */
    protected final void beginRemoveRows(BobUIModelIndex parent, int first, int last)
    {
     ...
     * @see #endResetModel()
     * @see <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html#modelAboutToBeReset">modelAboutToBeReset</a>
     * @see <a href="https://doc.bobui.io/bobui-6/qabstractitemmodel.html#modelReset">modelReset</a>
    */
    ...
    */
    protected final BobUIModelIndex createIndex(int row, int column, long id)
    {
        return (BobUIModelIndex)jni_createIndex(row, column, id);
    }
    /**
    ...
    * @see #beginInsertColumns(BobUIModelIndex, int, int)
    */
     ...
     * @see #beginInsertRows(BobUIModelIndex, int, int)
    */
    ...
    * @see #beginMoveColumns(BobUIModelIndex, int, int, BobUIModelIndex, int)
    */
    ...
    @see #beginMoveRows(BobUIModelIndex, int, int, BobUIModelIndex, int)
    */
     ...
     * @see #beginRemoveColumns(BobUIModelIndex, int, int)
    */
    ...
    */
    ...
    private void handleDataChanged(BobUIModelIndex topLeft, BobUIModelIndex bottomRight, int[] roles)
    {
    ...
    private native void jni_beginInsertColumns(BobUIModelIndex parent, int first, int last);
    private native void jni_beginInsertRows(BobUIModelIndex parent, int first, int last);
    private native boolean jni_beginMoveColumns(BobUIModelIndex sourceParent, int sourceFirst,
                                                int sourceLast, BobUIModelIndex destinationParent,
                                                int destinationChild);
    private native boolean jni_beginMoveRows(BobUIModelIndex sourceParent, int sourceFirst,
                                             int sourceLast, BobUIModelIndex destinationParent,
                                             int destinationChild);
    private native void jni_beginRemoveColumns(BobUIModelIndex parent, int first, int last);
    private native void jni_beginRemoveRows(BobUIModelIndex parent, int first, int last);
    ...
    private native Object jni_sibling(int row, int column, BobUIModelIndex parent);
    private native boolean jni_setData(BobUIModelIndex index, Object value, int role);
    private native void jni_dataChanged(BobUIModelIndex topLeft, BobUIModelIndex bottomRight,
                                        int[] roles);

    private long m_nativeReference = 0;
    private OnDataChangedListener m_OnDataChangedListener;
    private BobUIAbstractItemModel(long nativeReference) { m_nativeReference = nativeReference; }
    ...
    private static boolean instanceOf(Object obj) { return (obj instanceof BobUIAbstractItemModel); }
}
