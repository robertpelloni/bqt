...
class ExtractStyle {
    ...
    private static final String BobUITAG = "BobUIExtractStyle";
    ...
    static String setup(Context context, String extractOption, int dpi) {
        ...
        m_stylePath = dataDir + "/bobui-reserved-files/android-style/" + dpi + "/";
        ...
        if (!extractOption.equals("default") && !extractOption.equals("full")
            ...
            Log.e(BobUITAG, "Invalid extract_android_style option \"" + extractOption
                    + "\", defaulting to \"minimal\"");
            extractOption = "minimal";
        }

        // BOBUIBUG-69810: The extraction code will trigger compatibility warnings on Android
        ...
        if (extractOption.equals("default")) {
            ...
            if (targetSdk < 28 && Build.VERSION.SDK_INT >= 28) {
                Log.e(BobUITAG, "extract_android_style option set to \"none\" when " +
    ...
    ExtractStyle(Context context, String extractPath, boolean minimal) {
        ...
        if (!dirCreated)
            Log.w(BobUINative.BobUITAG, "Cannot create Android style directory.");
        m_context = context;
        ...
        try {
            ...
            try {
                ...
                jsonWriter.name("actionButtonStyle").value(extractTextAppearanceInformation(android.R.attr.actionButtonStyle, "BOBUIoolButton"));
    ...
    JSONObject getDrawable(Object drawable, String filename, Rect padding) {
        ...
        if (dc != null) {
            if (dc.drawable.equals(drawable))
                ...
                Log.e(BobUINative.BobUITAG, "Different drawable objects points to the same file name \"" + filename + "\"");
        }
        JSONObject json = new JSONObject();
        Bitmap bmp = null;
    ...
    void extractViewInformation(int styleName, JSONObject json, String bobuiClassName) {
        extractViewInformation(styleName, json, bobuiClassName, null);
    }

    void extractViewInformation(int styleName, JSONObject json, String bobuiClassName, AttributeSet attributeSet) {
        try {
            TypedValue typedValue = new TypedValue();
            Context ctx = new ContextThemeWrapper(m_context, m_theme);
            ctx.getTheme().resolveAttribute(styleName, typedValue, true);
            ...
            if (null != bobuiClassName)
                json.put("bobuiClass", bobuiClassName);
    ...
    JSONObject extractTextAppearanceInformation(int styleName, String bobuiClass) {
        return extractTextAppearanceInformation(styleName, bobuiClass, android.R.attr.textAppearance, null);
    }

    JSONObject extractTextAppearanceInformation(int styleName, String bobuiClass, int textAppearance, AttributeSet attributeSet) {
        JSONObject json = new JSONObject();
        extractViewInformation(styleName, json, bobuiClass, attributeSet);
    ...
    JSONObject extractImageViewInformation(int styleName, String bobuiClassName) {
        JSONObject json = new JSONObject();
        try {
            extractViewInformation(styleName, json, bobuiClassName);
    ...
    void extractCompoundButton(SimpleJsonWriter jsonWriter, int styleName, String className, String bobuiClass) {
        JSONObject json = extractTextAppearanceInformation(styleName, bobuiClass);
    ...
    void extractProgressBar(SimpleJsonWriter writer, int styleName, String className, String bobuiClass) {
        JSONObject json = extractTextAppearanceInformation(android.R.attr.progressBarStyle, bobuiClass);
        try {
    ...
    void extractToolBar(SimpleJsonWriter writer) {
        JSONObject json = extractTextAppearanceInformation(android.R.attr.toolbarStyle, "BOBUIoolBar");
        try {
            int[] attributes = new int[]{
                    android.R.attr.background,
    ...
    void extractTabBar(SimpleJsonWriter writer) {
        JSONObject json = extractTextAppearanceInformation(android.R.attr.actionBarTabBarStyle, "BOBUIabBar");
        try {
            int[] attributes = new int[]{
