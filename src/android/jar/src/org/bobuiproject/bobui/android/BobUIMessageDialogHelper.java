...
{
    ButtonStruct(BobUIMessageDialogHelper dialog, int id, String text)
    {
        m_dialog = dialog;
        m_id = id;
        m_text = Html.fromHtml(text, Html.FROM_HTML_MODE_LEGACY);
    }
    final BobUIMessageDialogHelper m_dialog;
    private final int m_id;
    final Spanned m_text;

    @Override
    public void onClick(View view) {
        BobUINativeDialogHelper.dialogResult(m_dialog.handler(), m_id);
    }
}

class BobUIMessageDialogHelper
{
    BobUIMessageDialogHelper(Activity activity)
    {
        m_activity = activity;
    }
    ...
    void show(long handler)
    {
        ...
        m_activity.runOnUiThread(() -> {
            ...
            if (window != null)
                ...
                Log.w(BobUITAG, "show(): cannot set theme from null window!");

            if (m_title != null)
                m_dialog.setTitle(m_title);
            m_dialog.setOnCancelListener(dialogInterface -> BobUINativeDialogHelper.dialogResult(handler(), -1));
    ...
    private static final String BobUITAG = "BobUIMessageDialogHelper";
