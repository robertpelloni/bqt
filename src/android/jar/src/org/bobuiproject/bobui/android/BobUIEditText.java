...
// BobUI-Security score:critical reason:password-input-type
...
import org.bobuiproject.bobui.android.BobUIInputConnection.BobUIInputConnectionListener;

@SuppressLint("ViewConstructor")
class BobUIEditText extends View
{
    int m_initialCapsMode = 0;
    int m_imeOptions = 0;
    int m_inputType = InputType.TYPE_CLASS_TEXT;
    boolean m_optionsChanged = false;
    BobUIInputConnection m_inputConnection = null;

    // input method hints - must be kept in sync with BOBUIDIR/src/corelib/global/qnamespace.h
    ...
    BobUIEditText(Context context, BobUIInputConnectionListener listener)
    {
        super(context);
        setFocusable(true);
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs)
    {
        ...
        m_inputConnection = new BobUIInputConnection(this,m_bobuiInputConnectionListener);
    ...
    private int imeOptionsFromEnterKeyType(int enterKeyType)
    {
        ...
        // enter key type - must be kept in sync with BOBUIDIR/src/corelib/global/qnamespace.h
        switch (enterKeyType) {
                       ...
                       int x1, int y1, int x2, int y2, boolean rtl)
    {
        ...
        if (!BobUIClipboardManager.hasClipboardText(getContext()))
            editButtons &= ~EditContextView.PASTE_BUTTON;

        final boolean setEditPopupPosition = (mode & BobUIEditText.CursorHandleShowEdit) ==
                                             BobUIEditText.CursorHandleShowEdit && editButtons != 0;
    ...
    private boolean isDisablePredictiveTextWorkaround(int inputHints)
    {
        return (inputHints & ImhNoPredictiveText) != 0 &&
                System.getenv("BOBUI_ANDROID_ENABLE_WORKAROUND_TO_DISABLE_PREDICTIVE_TEXT") != null;
    }
}
