...
// ### BobUI7: make private and find new API for onNewIntent()
public class BobUINative
{
    ...
    static final String BobUITAG = "BobUI JAVA";
    ...
    private static BobUIThread m_bobuiThread = null;
    private static final Object m_bobuiThreadLock = new Object();
    private static ClassLoader m_classLoader = null;

    private static final Runnable runPendingCppRunnablesRunnable = BobUINative::runPendingCppRunnables;
    ...
    static void setActivity(Activity bobuiMainActivity)
    {
        synchronized (m_mainActivityMutex) {
            m_activity = new WeakReference<>(bobuiMainActivity);
            try {
                if (m_stateDetails.isStarted)
                    updateNativeActivity();
            } catch (UnsatisfiedLinkError ignored) {
                // No-op - this happens in certain e.g. BobUIQuick for Android cases when we set the
                // Activity for the first time, before BobUI native libraries have been loaded. The
                // C++ side will update its reference when the library is loaded.
            }
        }
    }

    static void setService(Service bobuiMainService)
    {
        synchronized (m_mainActivityMutex) {
            m_service = new WeakReference<>(bobuiMainService);
        }
    }
    ...
    private static Uri getUriWithValidPermission(Context context, String uri, String openMode)
    {
        ...
        try {
            ...
            Log.e(BobUITAG, getCurrentMethodNameLog() + e);
            return parsedUri;
        }
    }
    ...
    static boolean openURL(Context context, String url, String mime)
    {
        ...
        if (uri == null) {
            Log.e(BobUITAG, getCurrentMethodNameLog() + "received invalid/null Uri");
            return false;
        }
        ...
        try {
            ...
            if (activity == null) {
                Log.w(BobUITAG, "openURL(): The activity reference is null");
                return false;
            }
    ...
    static BobUIThread getBobUIThread() {
        if (m_bobuiThread != null && m_bobuiThread.isAlive())
            return m_bobuiThread;

        synchronized (m_bobuiThreadLock) {
            if (m_bobuiThread == null || !m_bobuiThread.isAlive())
                m_bobuiThread = new BobUIThread();
    ...
    static void notifyNativePluginIntegrationReady(boolean ready)
    {
        ...
        final String bufferSize = Os.getenv("BOBUI_ANDROID_BACKGROUND_ACTIONS_QUEUE_SIZE");
        if (bufferSize != null) {
            try {
                final int size = Integer.parseInt(bufferSize);
                m_backgroundActionsTracker.setMaxAllowedActions(size);
            } catch (NumberFormatException exception) {
                Log.e(BobUITAG, "Parsing failed, BOBUI_ANDROID_BACKGROUND_ACTIONS_QUEUE_SIZE value is not an integer");
            }
        }
    }
    ...
    static void startApplication(String params, String mainLib)
    {
        ...
        getBobUIThread().run(() -> initAndroidQpaPlugin());
        final String bobuiParams = mainLib + " " + params;
        getBobUIThread().post(() -> { startBobUINativeApplication(bobuiParams); });
        waitForServiceSetup();
        setStarted(true);
    }
    ...
    private static byte[][] getSSLCertificates()
    {
        ...
        try {
            ...
            Log.e(BobUITAG, "Failed to get certificates", e);
        }
    ...
    static native void startBobUINativeApplication(String params);
    static native void waitForServiceSetup();
    static native void terminateBobUINativeApplication();
