...
abstract class BobUILoader {

    protected static final String BobUITAG = "BobUILoader";
    ...
    protected static BobUILoader m_instance = null;
     ...
     **/
     BobUILoader(ContextWrapper context) throws IllegalArgumentException {
        ...
        if (!(baseContext instanceof Activity || baseContext instanceof Service)) {
            throw new IllegalArgumentException("BobUILoader: Context is not an instance of " +
                                               "Activity or Service");
        }
        ...
        try {
            ...
            throw new IllegalArgumentException("BobUILoader: No ComponentInfo found for given " +
                                               "Context", e);
        }
    ...
    protected void extractContextMetaData(Context context) {
        setEnvironmentVariable("BOBUI_ANDROID_FONTS", "Roboto;Droid Sans;Droid Sans Fallback");
        String monospaceFonts = "Droid Sans Mono;Droid Sans;Droid Sans Fallback";
        setEnvironmentVariable("BOBUI_ANDROID_FONTS_MONOSPACE", monospaceFonts);
        setEnvironmentVariable("BOBUI_ANDROID_FONTS_SERIF", "Droid Serif");
        ...
        setEnvironmentVariable("BOBUI_BLOCK_EVENT_LOOPS_WHEN_SUSPENDED", isBackgroundRunningBlocked());
        setEnvironmentVariable("BOBUIRACE_LOCATION", getMetaData("android.app.trace_location"));
    ...
    {
        try {
            int id = m_resources.getIdentifier("bobui_libs", "array", m_packageName);
    ...
    /**
     * Initialize the class loader instance and sets it via BobUINative.
    ...
    private void initClassLoader(Context context)
    {
        ...
        BobUINative.setClassLoader(m_classLoader);
    }
     ...
     * This method can be used when the name is not provided by androiddeploybobui, for example when
    ...
    public void setEnvironmentVariable(String key, String value)
    {
        try {
            ...
            Log.e(BobUITAG, "Could not set environment variable:" + key + "=" + value);
            e.printStackTrace();
        }
    }
    ...
    private void parseNativeLibrariesDir() {
        ...
        if (isBundleBobUILibs()) {
            ...
            // If not, check if it's provided by androiddeploybobui in libs.xml
            ...
            if (systemLibsPrefix.isEmpty()) {
                ...
                Log.e(BobUITAG, "Using " + SYSTEM_LIB_PATH + " as default libraries path. "
                        + "It looks like the app is deployed using Unbundled "
                        + "deployment. It may be necessary to specify the path to "
                        + "the directory where BobUI libraries are installed using either "
                        + "android.app.system_libs_prefix metadata variable in your "
                        + "AndroidManifest.xml or BOBUI_ANDROID_SYSTEM_LIBS_PATH in your "
                        + "CMakeLists.txt");
            }
            ...
            if (systemLibraryDir.exists()) {
                if (systemLibraryDir.isDirectory() && list != null && list.length > 0)
                    ...
                    Log.e(BobUITAG, "System library directory " + systemLibsPrefix + " is empty.");
            } else {
                Log.e(BobUITAG, "System library directory " + systemLibsPrefix + " does not exist.");
            }
        }
    ...
    @SuppressLint("DiscouragedApi")
    private ArrayList<String> getBobUILibrariesList() {
        try {
            int id = m_resources.getIdentifier("bobui_libs", "array", m_packageName);
    ...
    @SuppressLint("DiscouragedApi")
    private boolean useLocalBobUILibs() {
        try {
            int id = m_resources.getIdentifier("use_local_bobui_libs", "string", m_packageName);
    ...
    @SuppressLint("DiscouragedApi")
    private boolean isBundleBobUILibs() {
        try {
            int id = m_resources.getIdentifier("bundle_local_bobui_libs", "string", m_packageName);
            return Integer.parseInt(m_resources.getString(id)) == 1;
        } catch (Resources.NotFoundException ignored) {
            return false;
        }
    ...
    {
        int flags = BobUINative.getContext().getApplicationInfo().flags;
    ...
    {
        return BobUIApkFileEngine.getAppApkFilePath() + "!/lib/" + m_preferredAbi + "/";
    }

    /**
     * Returns BobUILoader.LoadingResult.Succeeded if libraries are successfully loaded,
     * BobUILoader.LoadingResult.AlreadyLoaded if they have already been loaded,
     * and BobUILoader.LoadingResult.Failed if loading the libraries failed.
     **/
    public LoadingResult loadBobUILibraries() {
        if (m_librariesLoaded)
            return LoadingResult.AlreadyLoaded;

        if (!useLocalBobUILibs()) {
            Log.w(BobUITAG, "Use local BobUI libs is false");
            return LoadingResult.Failed;
        }

        if (isUncompressedNativeLibs()) {
            String apkLibPath = getApkNativeLibrariesDir();
            setEnvironmentVariable("BOBUI_PLUGIN_PATH", apkLibPath);
            ...
            if (m_extractedNativeLibsDir == null || m_extractedNativeLibsDir.isEmpty()) {
                Log.e(BobUITAG, "The native libraries directory is null or empty");
                return LoadingResult.Failed;
            }
            setEnvironmentVariable("BOBUI_PLUGIN_PATH", m_extractedNativeLibsDir);
            setEnvironmentVariable("QML_PLUGIN_PATH", m_extractedNativeLibsDir);
        }

        // Load native BobUI APK libraries
        ArrayList<String> nativeLibraries = getBobUILibrariesList();
        nativeLibraries.addAll(getLocalLibrariesList());

        if (Debug.isDebuggerConnected()) {
            final String debuggerSleepEnvVarName = "BOBUI_ANDROID_DEBUGGER_MAIN_THREAD_SLEEP_MS";
        ...
        if (m_mainLibName == null || m_mainLibName.isEmpty()) {
            Log.e(BobUITAG, "The main library name is null or empty.");
            return LoadingResult.Failed;
        }

        // Load main lib
        if (!loadMainLibrary(m_mainLibName + "_" + m_preferredAbi)) {
            Log.e(BobUITAG, "Loading main library failed");
            return LoadingResult.Failed;
        }
        ...
        try {
            ...
            if (library.startsWith("/")) {
                if (libFile.exists()) {
                    ...
                    Log.e(BobUITAG, "Can't find '" + library + "'");
                }
            } else {
            ...
            Log.e(BobUITAG, "Can't load '" + library + "'", e);
        }
    ...
    private boolean loadMainLibrary(String mainLibName)
    {
        ...
        BobUINative.getBobUIThread().run(() -> {
    ...
    private boolean loadLibraries(final ArrayList<String> libraries)
    {
        ...
        if (libraries.size() != fullPathLibs.size()) {
            Log.e(BobUITAG, "Failed to get full paths of libraries.");
            return false;
        }
