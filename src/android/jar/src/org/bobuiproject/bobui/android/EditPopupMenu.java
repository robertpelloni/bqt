        ...
        EditContextView.OnClickListener
{
    ...
    EditPopupMenu(BobUIEditText editText)
    {
    ...
    void setPosition(final int x, final int y, final int buttons)
    {
        ...
        // Since BobUIEditText doesn't match the BobUIWindow size, we should use its parent for
        // EditPopupMenu positioning. However, there may be cases where the parent is
        // not set. In such cases, we need to use BobUIEditText instead.
        ...
        if (y2 <= 0) {
            try {
                BobUILayout parentLayout = (BobUILayout) m_editText.getParent();
                parentLayout.requestLayout();
            } catch (ClassCastException e) {
                Log.w(BobUINative.BobUITAG, "BobUIEditText " + m_editText + " parent is not a BobUILayout, " +
                                      "requestLayout() skipped");
            } catch (NullPointerException e) {
                Log.w(BobUINative.BobUITAG, "BobUIEditText " + m_editText + " does not have a parent, " +
                                      "requestLayout() skipped");
            }
        }
    ...
    public void contextButtonClicked(int buttonId) {
        ...
        case android.R.string.cut:
            BobUINativeInputConnection.cut();
            break;
        case android.R.string.copy:
            BobUINativeInputConnection.copy();
            break;
        case android.R.string.paste:
            BobUINativeInputConnection.paste();
            break;
        case android.R.string.selectAll:
            BobUINativeInputConnection.selectAll();
            break;
        }
        hide();
    }
