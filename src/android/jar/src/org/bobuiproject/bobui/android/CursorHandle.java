...
{
    private static final String BobUITag = "BobUICursorHandle";
    ...
    private void initOverlay(){
        ...
        if (drawable != null) {
            ...
            Log.w(BobUITag, "initOverlay(): cannot get width/height for popup " +
    ...
    void setPosition(final int x, final int y){
        ...
        // m_layout is BobUIEditText. Since it doesn't match the BobUIWindow size, we should use its
    ...
    void updatePosition(int x, int y) {
        ...
        if (Math.abs(m_lastX - x) > tolerance || Math.abs(m_lastY - y) > tolerance) {
            BobUIInputDelegate.handleLocationChanged(m_id, x + m_posX, y + m_posY);
            m_lastX = x;
            m_lastY = y;
        }
    }
