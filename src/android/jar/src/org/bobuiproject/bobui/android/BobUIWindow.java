...
class BobUIWindow extends BobUILayout implements BobUISurfaceInterface {
    private View m_surfaceContainer;
    private View m_nativeView;
    private final HashMap<Integer, BobUIWindow> m_childWindows = new HashMap<>();
    private BobUIWindow m_parentWindow;
    private GestureDetector m_gestureDetector;
    private final BobUIEditText m_editText;
    private boolean m_editTextFocusInitialized = false;
    private final BobUIInputConnection.BobUIInputConnectionListener m_inputConnectionListener;
    ...
    BobUIWindow(Context context, boolean isForeignWindow, BobUIWindow parentWindow,
                    BobUIInputConnection.BobUIInputConnectionListener listener)
    {
        super(context);
        ...
        if (!isForeignWindow && context instanceof Activity) {
            // TODO BOBUIBUG-122552 - Service keyboard input not implemented
            m_editText = new BobUIEditText(context, listener);
            ...
            BobUINative.runAction(() -> addView(m_editText, layoutParams));
        } else {
            m_editText = null;
        }
            ...
            m_gestureDetector =
                new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
                    ...
                    public void onLongPress(MotionEvent event) {
                        BobUIInputDelegate.longPress(getId(), (int) event.getX(), (int) event.getY());
                    }
                });
    ...
    {
        if (!(getContext() instanceof BobUIActivityBase))
            return;
    ...
    void setVisible(boolean visible) {
        BobUINative.runAction(() -> setVisibility(visible ? View.VISIBLE : View.INVISIBLE));
    }
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        // early BobUIInputConnection callbacks from blocking the UI thread.
        ...
        BobUIInputDelegate.sendTouchEvent(event, getId());
    ...
    {
        BobUIInputDelegate.sendTrackballEvent(event, getId());
        return true;
    }
    ...
    {
        return BobUIInputDelegate.sendGenericMotionEvent(event, getId());
    }
                              ...
                              final int surfaceContainerType) // TODO constant for type
    {
        BobUINative.runAction(()-> {
            ...
            if (surfaceContainerType == 0) {
                m_surfaceContainer = new BobUISurface(getContext(), BobUIWindow.this,
                                                   onTop, imageDepth);
            } else {
                m_surfaceContainer = new BobUITextureView(getContext(), BobUIWindow.this, isOpaque);
            }
             m_surfaceContainer.setLayoutParams(new BobUILayout.LayoutParams(
    ...
    void setGeometry(final int x, final int y, final int w, final int h)
    {
        BobUINative.runAction(()-> {
            if (getContext() instanceof BobUIActivityBase)
                setLayoutParams(new BobUILayout.LayoutParams(w, h, x, y));
        });
    }

    void addChildWindow(BobUIWindow window)
    {
        BobUINative.runAction(()-> {
    ...
    void setNativeView(final View view)
    {
        BobUINative.runAction(()-> {
            ...
            m_nativeView = view;
            m_nativeView.setLayoutParams(new BobUILayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                                                                   ViewGroup.LayoutParams.MATCH_PARENT));
    ...
    void setParent(BobUIWindow parentWindow)
    {
