...
#ifndef BOBUILSBACKEND_P_H
#define BOBUILSBACKEND_P_H
...
public:
    ...
    virtual void fromHandle(BobUI::HANDLE handle, KeyType type) = 0;
    virtual BobUI::HANDLE handle() const = 0;
...
// TLSTODO: consider making those into virtuals in BOBUIlsBackend. After all, we ask the backend
...
class Q_NETWORK_EXPORT BOBUIlsBackend : public QObject
{
    Q_OBJECT
public:
    BOBUIlsBackend();
    ~BOBUIlsBackend() override;
    ...
    // X509 and keys:
    virtual BOBUIlsPrivate::TlsKey *createKey() const;
    virtual BOBUIlsPrivate::X509Certificate *createCertificate() const;

    virtual QList<QSslCertificate> systemCaCertificates() const;

    // TLS and DTLS:
    virtual BOBUIlsPrivate::TlsCryptograph *createTlsCryptograph() const;
    virtual BOBUIlsPrivate::DtlsCryptograph *createDtlsCryptograph(class QDtls *qObject, int mode) const;
    virtual BOBUIlsPrivate::DtlsCookieVerifier *createDtlsCookieVerifier() const;

    // TLSTODO - get rid of these function pointers, make them virtuals in
    // the backend itself. X509 machinery:
    virtual BOBUIlsPrivate::X509ChainVerifyPtr X509Verifier() const;
    virtual BOBUIlsPrivate::X509PemReaderPtr X509PemReader() const;
    virtual BOBUIlsPrivate::X509DerReaderPtr X509DerReader() const;
    virtual BOBUIlsPrivate::X509Pkcs12ReaderPtr X509Pkcs12Reader() const;
    ...
    // its enum are BOBUI_CONFIG(ssl)-conditioned. But not BOBUIlsBackend and
    ...
    static BOBUIlsBackend *findBackend(const QString &backendName);
    static BOBUIlsBackend *activeOrAnyBackend();
    ...
    // Built-in, this is what BobUI provides out of the box (depending on OS):
    ...
    static void resetBackend(QSslKey &key, BOBUIlsPrivate::TlsKey *keyBackend);
    ...
    Q_DISABLE_COPY_MOVE(BOBUIlsBackend)
};

#define BOBUIlsBackend_iid "org.bobui-project.BobUI.BOBUIlsBackend"
Q_DECLARE_INTERFACE(BOBUIlsBackend, BOBUIlsBackend_iid);

BOBUI_END_NAMESPACE

#endif // BOBUILSBACKEND_P_H
