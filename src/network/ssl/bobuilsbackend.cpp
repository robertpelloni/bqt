...
Q_APPLICATION_STATIC(QFactoryLoader, bobuilsbLoader, BOBUIlsBackend_iid,
                     QStringLiteral("/tls"))
...
{
public:
    void addBackend(BOBUIlsBackend *backend)
    {
        Q_ASSERT(backend);
    ...
    void removeBackend(BOBUIlsBackend *backend)
    {
        Q_ASSERT(backend);
        const QMutexLocker locker(&collectionMutex);
    ...
    bool tryPopulateCollection()
    {
        ...
        bobuilsbLoader->update();
#endif
        int index = 0;
        while (bobuilsbLoader->instance(index))
            ++index;
    ...
    BOBUIlsBackend *backend(const QString &name)
    {
        if (!tryPopulateCollection())
            return nullptr;
...
private:
    std::vector<BOBUIlsBackend *> backends;
...
/*!
    ...
    \brief BOBUIlsBackend is a factory class, providing implementations
    for the QSsl classes.

    The purpose of BOBUIlsBackend is to enable and simplify the addition
    of new TLS backends to be used by QSslSocket and related classes.
    Starting from BobUI 6.1, these backends have plugin-based design (and
    thus can co-exist simultaneously, unlike pre 6.1 times), although
    any given run of a program can only use one of them.

    Inheriting from BOBUIlsBackend and creating an object of such
    ...
    The base abstract class - BOBUIlsBackend - provides, where possible,
    ...
    BOBUIlsBackend also provides some auxiliary functions that a derived
...
/*!
    \fn QString BOBUIlsBackend::backendName() const
    \internal
...
const QString BOBUIlsBackend::builtinBackendNames[] = {
...
/*!
    ...
    \sa ~BOBUIlsBackend(), availableBackendNames(), QSslSocket::availableBackends()
*/
BOBUIlsBackend::BOBUIlsBackend()
{
    if (backends())
...
bool BOBUIlsBackend::isValid() const
{
    return true;
}
...
*/
long BOBUIlsBackend::tlsLibraryVersionNumber() const
{
    return 0;
}
...
QString BOBUIlsBackend::tlsLibraryVersionString() const
{
    return {};
}
...
long BOBUIlsBackend::tlsLibraryBuildVersionNumber() const
{
    return 0;
}
...
*/
QString BOBUIlsBackend::tlsLibraryBuildVersionString() const
{
    return {};
}
...
*/
void BOBUIlsBackend::ensureInitialized() const
{
}
...
/*!
    ...
    of an implementation-specific type, inheriting from the class BOBUIlsPrivate::TlsKey.
    The default implementation of this function returns \nullptr.

    \sa QSslKey, implementedClasses(), BOBUIlsPrivate::TlsKey
*/
BOBUIlsPrivate::TlsKey *BOBUIlsBackend::createKey() const
{
...
/*!
    ...
    implementation-specific type, inheriting from the class BOBUIlsPrivate::X509Certificate.
    The default implementation of this function returns \nullptr.

    \sa QSslCertificate, BOBUIlsPrivate::X509Certificate, implementedClasses()
*/
BOBUIlsPrivate::X509Certificate *BOBUIlsBackend::createCertificate() const
{
...
*/
QList<QSslCertificate> BOBUIlsBackend::systemCaCertificates() const
{
...
/*!
    ...
    the backend must reimplement this method to return a dynamically-allocated instance of an
    implementation-specific type, inheriting from the class BOBUIlsPrivate::TlsCryptograph.
    The default implementation of this function returns \nullptr.

    \sa QSslSocket, BOBUIlsPrivate::TlsCryptograph, implementedClasses()
*/
BOBUIlsPrivate::TlsCryptograph *BOBUIlsBackend::createTlsCryptograph() const
{
...
/*!
    ...
    the backend must reimplement this method to return a dynamically-allocated instance of an
    implementation-specific type, inheriting from the class BOBUIlsPrivate::DtlsCryptograph.
    The default implementation of this function returns \nullptr.

    \sa QDtls, BOBUIlsPrivate::DtlsCryptograph, implementedClasses()
*/
BOBUIlsPrivate::DtlsCryptograph *BOBUIlsBackend::createDtlsCryptograph(QDtls *qObject, int mode) const
{
    Q_UNUSED(qObject);
    Q_UNUSED(mode);
...
/*!
    ...
    the backend must reimplement this method to return a dynamically-allocated instance of an
    implementation-specific type, inheriting from the class BOBUIlsPrivate::DtlsCookieVerifier. The
    default implementation returns \nullptr.

    \sa QDtlsClientVerifier, BOBUIlsPrivate::DtlsCookieVerifier, implementedClasses()
*/
BOBUIlsPrivate::DtlsCookieVerifier *BOBUIlsBackend::createDtlsCookieVerifier() const
{
...
BOBUIlsPrivate::X509ChainVerifyPtr BOBUIlsBackend::X509Verifier() const
{
...
/*!
    ...
    \sa QSslCertificate
*/
BOBUIlsPrivate::X509PemReaderPtr BOBUIlsBackend::X509PemReader() const
{
...
/*!
    ...
    \sa QSslCertificate
*/
BOBUIlsPrivate::X509DerReaderPtr BOBUIlsBackend::X509DerReader() const
{
...
/*!
    ...
    \sa QSslCertificate
*/
BOBUIlsPrivate::X509Pkcs12ReaderPtr BOBUIlsBackend::X509Pkcs12Reader() const
{
...
*/
QList<int> BOBUIlsBackend::ellipticCurvesIds() const
{
...
*/
int BOBUIlsBackend::curveIdFromShortName(const QString &name) const
{
    Q_UNUSED(name);
    REPORT_MISSING_SUPPORT("does not support QSslEllipticCurve");
    return 0;
...
*/
int BOBUIlsBackend::curveIdFromLongName(const QString &name) const
{
    Q_UNUSED(name);
    REPORT_MISSING_SUPPORT("does not support QSslEllipticCurve");
    return 0;
...
*/
QString BOBUIlsBackend::shortNameForId(int cid) const
{
    Q_UNUSED(cid);
    REPORT_MISSING_SUPPORT("does not support QSslEllipticCurve");
    return {};
...
/*!
    ...
    \sa ellipticCurvesIds(), QSslEllipticCurve::shortName()
*/
QString BOBUIlsBackend::longNameForId(int cid) const
{
    Q_UNUSED(cid);
    REPORT_MISSING_SUPPORT("does not support QSslEllipticCurve");
    return {};
...
*/
bool BOBUIlsBackend::isTlsNamedCurve(int cid) const
{
    Q_UNUSED(cid);
    REPORT_MISSING_SUPPORT("does not support QSslEllipticCurve");
    return false;
...
*/
int BOBUIlsBackend::dhParametersFromDer(const QByteArray &derData, QByteArray *data) const
{
    Q_UNUSED(derData);
    Q_UNUSED(data);
...
/*!
    ...
    \sa QSslDiffieHellmanParameters, implementedClasses()
*/
int BOBUIlsBackend::dhParametersFromPem(const QByteArray &pemData, QByteArray *data) const
{
    Q_UNUSED(pemData);
    Q_UNUSED(data);
...
/*!
    ...
    \sa BOBUIlsBackend(), isValid()
*/
QList<QString> BOBUIlsBackend::availableBackendNames()
{
    if (!backends())
        return {};
...
*/
QString BOBUIlsBackend::defaultBackendName()
{
...
*/
BOBUIlsBackend *BOBUIlsBackend::findBackend(const QString &backendName)
{
    if (!backends())
        return {};
...
/*!
    \internal
    Returns the backend that QSslSocket is using. If BobUI was built without TLS support,
    this function returns a minimal backend that only supports QSslCertificate.

    \sa defaultBackend()
*/
BOBUIlsBackend *BOBUIlsBackend::activeOrAnyBackend()
{
#if BOBUI_CONFIG(ssl)
...
/*!
    ...
    \sa QSsl::SslProtocol
*/
QList<QSsl::SslProtocol> BOBUIlsBackend::supportedProtocols(const QString &backendName)
{
    if (!backends())
        return {};
...
*/
QList<QSsl::SupportedFeature> BOBUIlsBackend::supportedFeatures(const QString &backendName)
{
    if (!backends())
        return {};
...
*/
QList<QSsl::ImplementedClass> BOBUIlsBackend::implementedClasses(const QString &backendName)
{
    if (!backends())
        return {};
...
*/
void BOBUIlsBackend::resetBackend(QSslKey &key, BOBUIlsPrivate::TlsKey *keyBackend)
{
#if BOBUI_CONFIG(ssl)
...
*/
void BOBUIlsBackend::setupClientPskAuth(QSslPreSharedKeyAuthenticator *auth, const char *hint,
...
/*!
    ...
    \a maxPskLen.
*/
void BOBUIlsBackend::setupServerPskAuth(QSslPreSharedKeyAuthenticator *auth, const char *identity,
...
*/
QSslCipher BOBUIlsBackend::createCiphersuite(const QString &descriptionOneLine, int bits, int supportedBits)
{
    QSslCipher ciph;

    const auto descriptionList = QStringView{descriptionOneLine}.split(u' ', BobUI::SkipEmptyParts);
...
/*!
    ...
    \endcode
*/
QSslCipher BOBUIlsBackend::createCiphersuite(const QString &suiteName, QSsl::SslProtocol protocol,
...
/*!
    ...
    \endcode
*/
QSslCipher BOBUIlsBackend::createCiphersuite(const QString &name, const QString &keyExchangeMethod,
...
*/
QList<QSslCipher> BOBUIlsBackend::defaultCiphers()
{
...
*/
QList<QSslCipher> BOBUIlsBackend::defaultDtlsCiphers()
{
...
/*!
    ...
    \sa defaultCiphers()
*/
void BOBUIlsBackend::setDefaultCiphers(const QList<QSslCipher> &ciphers)
{
...
*/
void BOBUIlsBackend::setDefaultDtlsCiphers(const QList<QSslCipher> &ciphers)
{
...
*/
void BOBUIlsBackend::setDefaultSupportedCiphers(const QList<QSslCipher> &ciphers)
{
...
*/
void BOBUIlsBackend::resetDefaultEllipticCurves()
{
...
*/
void BOBUIlsBackend::setDefaultCaCertificates(const QList<QSslCertificate> &certs)
{
...
*/
bool BOBUIlsBackend::rootLoadingOnDemandAllowed(const QSslConfiguration &configuration)
{
...
*/
void BOBUIlsBackend::storePeerCertificate(QSslConfiguration &configuration,
...
*/
void BOBUIlsBackend::storePeerCertificateChain(QSslConfiguration &configuration,
...
*/
void BOBUIlsBackend::clearPeerCertificates(QSslConfiguration &configuration)
{
...
*/
void BOBUIlsBackend::clearPeerCertificates(QSslSocketPrivate *d)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::setPeerSessionShared(QSslSocketPrivate *d, bool shared)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::setSessionAsn1(QSslSocketPrivate *d, const QByteArray &asn1)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::setSessionLifetimeHint(QSslSocketPrivate *d, int hint)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::setAlpnStatus(QSslSocketPrivate *d, AlpnNegotiationStatus st)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::setNegotiatedProtocol(QSslSocketPrivate *d, const QByteArray &protocol)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::storePeerCertificate(QSslSocketPrivate *d, const QSslCertificate &peerCert)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::storePeerCertificateChain(QSslSocketPrivate *d,
                                            const QList<QSslCertificate> &peerChain)
{
    Q_ASSERT(d);
...
/*!
    ...
    \note In BobUI 6.1 it's only used on Windows, during so called 'CA fetch'.
*/
void BOBUIlsBackend::addTustedRoot(QSslSocketPrivate *d, const QSslCertificate &rootCert)
{
    Q_ASSERT(d);
...
*/
void BOBUIlsBackend::setEphemeralKey(QSslSocketPrivate *d, const QSslKey &key)
{
    Q_ASSERT(d);
...
/*!
    ...
    Implementation-specific. Sets the security level suitable for BobUI's
    auto-tests.
*/
void BOBUIlsBackend::forceAutotestSecurityLevel()
{
}
...
/*!
    \internal (Network-private)
    \namespace BOBUIlsPrivate
    \brief Namespace containing onternal types that TLS backends implement.

    This namespace is private to BobUI and the backends that implement its TLS support.
*/
...
/*!
    ...
    \sa BOBUIlsBackend::createKey(), BOBUIlsBackend::implementedClasses(), QSslKey
*/
...
/*!
    ...
    \note This function is very implementation-specific. As of now (BobUI 6.1), it is only required by
    BobUI's own non-OpenSSL backends, that internally use DER and implement QSslKey::toPem()
    via pemFromDer().
*/

/*!
    \fn void TlsKey::fromHandle(BobUI::HANDLE handle, KeyType type)
    \internal
...
/*!
    ...
    This function is internally used only by BobUI's own TLS plugins and affects
    the way PEM file is generated by TlsKey. It's sufficient to override it
    and return \c false in case a new TLS backend is not using BobUI's plugin
    as a base.
*/
...
/*!
    ...
    \sa BOBUIlsBackend::createCertificate(), BOBUIlsBackend::X509PemReader(), BOBUIlsBackend::X509DerReader()
*/
...
/*!
    \fn BobUI::HANDLE X509Certificate::handle() const
    \internal
...
/*!
    ...
    QSslSocketPrivate provides access to its read/write buffers, BOBUIcpSocket it
    ...
    in the BobUINetwork module. This documentation is mostly conceptual and only describes what those
    functions are expected to do, but not how they must be implemented.

    \sa BOBUIlsBackend::createTlsCryptograph()
*/
...
/*!
    ...
    Windows and OpenSSL-specific, only used internally by BobUI's OpenSSL TLS backend.
...
/*!
    ...
    Windows and Schannel-specific, only used by BobUI's Schannel TLS backend, in
...
/*!
    ...
    in the BobUINetwork module. This documentation is mostly conceptual and only describes
...
/*!
    ...
    by BOBUIlsBackend and cannot change.

    \sa BOBUIlsBackend::createDtlsCryptograph()
*/
...
/*!
    \internal
*/
Q_NETWORK_EXPORT void bobui_ForceTlsSecurityLevel()
{
...
#include "moc_bobuilsbackend_p.cpp"
