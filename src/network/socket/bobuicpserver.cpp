...
//#define BOBUICPSERVER_DEBUG

/*! \class BOBUIcpServer

    \brief The BOBUIcpServer class provides a TCP-based server.
    ...
    You can specify the port or have BOBUIcpServer pick one
    ...
    a connected BOBUIcpSocket. The function returns a pointer to a
    BOBUIcpSocket in QAbstractSocket::ConnectedState that you can use for
    ...
    Calling close() makes BOBUIcpServer stop listening for incoming
    connections.

    Although BOBUIcpServer is mostly designed for use with an event
    ...
    \sa BOBUIcpSocket, {Fortune Server}, {Threaded Fortune Server},
        {Torrent Example}
*/

/*! \fn void BOBUIcpServer::newConnection()
...
/*! \fn void BOBUIcpServer::pendingConnectionAvailable()
...
/*! \fn void BOBUIcpServer::acceptError(QAbstractSocket::SocketError socketError)
    \since 5.0
...
/*! \internal
*/
BOBUIcpServerPrivate::BOBUIcpServerPrivate()
 : port(0)
...
*/
QNetworkProxy BOBUIcpServerPrivate::resolveProxy(const QHostAddress &address, quint16 port)
{
...
/*! \internal
*/
void BOBUIcpServerPrivate::configureCreatedSocket()
{
...
/*! \internal
*/
void BOBUIcpServerPrivate::readNotification()
{
    Q_Q(BOBUIcpServer);
    for (;;) {
        if (totalPendingConnections() >= maxConnections) {
#if defined (BOBUICPSERVER_DEBUG)
            qDebug("BOBUIcpServerPrivate::_q_processIncomingConnection() too many connections");
#endif
...
#if defined (BOBUICPSERVER_DEBUG)
        qDebug("BOBUIcpServerPrivate::_q_processIncomingConnection() accepted socket %i", descriptor);
#endif
        QPointer<BOBUIcpServer> that = q;
...
/*!
    ...
    what it did before.
*/
int BOBUIcpServerPrivate::totalPendingConnections() const
{
    return int(pendingConnections.size());
}

/*!
    Constructs a BOBUIcpServer object.

    \a parent is passed to the QObject constructor.

    \sa listen(), setSocketDescriptor()
*/
BOBUIcpServer::BOBUIcpServer(QObject *parent)
    : QObject(*new BOBUIcpServerPrivate, parent)
{
    Q_D(BOBUIcpServer);
#if defined(BOBUICPSERVER_DEBUG)
    qDebug("BOBUIcpServer::BOBUIcpServer(%p)", parent);
#endif
    d->socketType = QAbstractSocket::TcpSocket;
}

/*!
    Destroys the BOBUIcpServer object. If the server is listening for
    connections, the socket is automatically closed.

    Any client \l{BOBUIcpSocket}s that are still connected must either
    disconnect or be reparented before the server is deleted.

    \sa close()
*/
BOBUIcpServer::~BOBUIcpServer()
{
#if defined(BOBUICPSERVER_DEBUG)
    qDebug("BOBUIcpServer::~BOBUIcpServer()");
#endif
    close();
}
...
*/
BOBUIcpServer::BOBUIcpServer(QAbstractSocket::SocketType socketType, BOBUIcpServerPrivate &dd,
                       QObject *parent) : QObject(dd, parent)
{
    Q_D(BOBUIcpServer);
#if defined(BOBUICPSERVER_DEBUG)
    qDebug("BOBUIcpServer::BOBUIcpServer(%sSocket, BOBUIcpServerPrivate == %p, parent == %p)",
...
/*!
    ...
    \sa isListening()
*/
bool BOBUIcpServer::listen(const QHostAddress &address, quint16 port)
{
    Q_D(BOBUIcpServer);
    if (d->state == QAbstractSocket::ListeningState) {
        qWarning("BOBUIcpServer::listen() called when already listening");
        return false;
    }
...
#if defined (BOBUICPSERVER_DEBUG)
    qDebug("BOBUIcpServer::listen(%i, \"%s\") == true (listening on port %i)", port,
...
/*!
    ...
    \sa listen()
*/
bool BOBUIcpServer::isListening() const
{
    Q_D(const BOBUIcpServer);
...
void BOBUIcpServer::close()
{
    ...
    if (d->socketEngine) {
        ...
        BOBUI_TRY {
            ...
            // will be deleted in ~BOBUIcpServer (it's a child-object of this)
        }
...
*/
qintptr BOBUIcpServer::socketDescriptor() const
{
    Q_D(const BOBUIcpServer);
...
*/
bool BOBUIcpServer::setSocketDescriptor(qintptr socketDescriptor)
{
    Q_D(BOBUIcpServer);
    if (isListening()) {
        qWarning("BOBUIcpServer::setSocketDescriptor() called when already listening");
        return false;
    }
    ...
    if (!d->socketEngine->initialize(socketDescriptor, QAbstractSocket::ListeningState)) {
        ...
        d->serverSocketErrorString = d->socketEngine->errorString();
#if defined (BOBUICPSERVER_DEBUG)
        qDebug("BOBUIcpServer::setSocketDescriptor(%i) failed (%s)", socketDescriptor,
...
#if defined (BOBUICPSERVER_DEBUG)
    qDebug("BOBUIcpServer::setSocketDescriptor(%i) succeeded.", socketDescriptor);
#endif
    return true;
}
...
*/
quint16 BOBUIcpServer::serverPort() const
{
    Q_D(const BOBUIcpServer);
...
*/
QHostAddress BOBUIcpServer::serverAddress() const
{
    Q_D(const BOBUIcpServer);
...
/*!
    ...
    \sa hasPendingConnections(), nextPendingConnection()
*/
bool BOBUIcpServer::waitForNewConnection(int msec, bool *timedOut)
{
    Q_D(BOBUIcpServer);
    if (d->state != QAbstractSocket::ListeningState)
        return false;
...
*/
bool BOBUIcpServer::hasPendingConnections() const
{
    return !d_func()->pendingConnections.isEmpty();
}

/*!
    Returns the next pending connection as a connected BOBUIcpSocket
    object.

    The socket is created as a child of the server, which means that
    it is automatically deleted when the BOBUIcpServer object is
    ...
    \note The returned BOBUIcpSocket object cannot be used from another
...
*/
BOBUIcpSocket *BOBUIcpServer::nextPendingConnection()
{
    Q_D(BOBUIcpServer);
    ...
    if (!d->socketEngine) {
        qWarning("BOBUIcpServer::nextPendingConnection() called while not listening");
...
/*!
    This virtual function is called by BOBUIcpServer when a new
    connection is available. The \a socketDescriptor argument is the
    native socket descriptor for the accepted connection.

    The base implementation creates a BOBUIcpSocket, sets the socket
    descriptor and then stores the BOBUIcpSocket in an internal list of
    ...
    used with BOBUIcpSocket::setSocketDescriptor().
    ...
    \note If you want to handle an incoming connection as a new BOBUIcpSocket
    object in another thread you have to pass the socketDescriptor
    to the other thread and create the BOBUIcpSocket object there and
    use its setSocketDescriptor() method.

    \sa newConnection(), nextPendingConnection(), addPendingConnection()
*/
void BOBUIcpServer::incomingConnection(qintptr socketDescriptor)
{
#if defined (BOBUICPSERVER_DEBUG)
    qDebug("BOBUIcpServer::incomingConnection(%i)", socketDescriptor);
#endif

    BOBUIcpSocket *socket = new BOBUIcpSocket(this);
    socket->setSocketDescriptor(socketDescriptor);
    addPendingConnection(socket);
}

/*!
    This function is called by BOBUIcpServer::incomingConnection()
    ...
    \since 4.7
*/
void BOBUIcpServer::addPendingConnection(BOBUIcpSocket* socket)
{
...
/*!
    ...
    numConnections. BOBUIcpServer will accept no more than \a
    ...
    its maximum number of pending connections (i.e., BOBUIcpSocket can
    still emit the connected() signal). BOBUIcpServer will stop
    accepting the new connections, but the operating system may
    still keep them in queue.

    \sa maxPendingConnections(), hasPendingConnections()
*/
void BOBUIcpServer::setMaxPendingConnections(int numConnections)
{
...
*/
int BOBUIcpServer::maxPendingConnections() const
{
...
*/
void BOBUIcpServer::setListenBacklogSize(int size)
{
...
*/
int BOBUIcpServer::listenBacklogSize() const
{
...
/*!
    ...
    \sa errorString()
*/
QAbstractSocket::SocketError BOBUIcpServer::serverError() const
{
...
/*!
    ...
    \sa serverError()
*/
QString BOBUIcpServer::errorString() const
{
...
/*!
    ...
    \sa resumeAccepting()
*/
void BOBUIcpServer::pauseAccepting()
{
...
/*!
    ...
    \sa pauseAccepting()
*/
void BOBUIcpServer::resumeAccepting()
{
...
/*!
    ...
    \snippet code/src_network_socket_bobuicpserver.cpp 0

    \sa proxy(), QNetworkProxy
*/
void BOBUIcpServer::setProxy(const QNetworkProxy &networkProxy)
{
    Q_D(BOBUIcpServer);
...
*/
QNetworkProxy BOBUIcpServer::proxy() const
{
    Q_D(const BOBUIcpServer);
    return d->proxy;
}
