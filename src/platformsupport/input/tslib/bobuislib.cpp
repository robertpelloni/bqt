...
Q_STATIC_LOGGING_CATEGORY(qLcTsLib, "bobui.qpa.input")

BOBUIsLibMouseHandler::BOBUIsLibMouseHandler(const QString &key,
                                       const QString &specification,
                                       QObject *parent)
    : QObject(parent),
    m_rawMode(!key.compare("TslibRaw"_L1, BobUI::CaseInsensitive))
{
    ...
    connect(m_notify, &QSocketNotifier::activated, this, &BOBUIsLibMouseHandler::readMouseData);
}

BOBUIsLibMouseHandler::~BOBUIsLibMouseHandler()
{
    if (m_dev)
        ts_close(m_dev);
}
...
void BOBUIsLibMouseHandler::readMouseData()
{
    ts_sample sample;
    ...
    while (get_sample(m_dev, &sample, m_rawMode)) {
        ...
        BobUI::MouseButton button = pressed ^ m_pressed ? BobUI::LeftButton : BobUI::NoButton;
        BobUI::MouseButtons state = pressed ? BobUI::LeftButton : BobUI::NoButton;
