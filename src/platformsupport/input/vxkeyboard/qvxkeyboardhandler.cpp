...
Q_LOGGING_CATEGORY(qLcVxKey, "bobui.qpa.input")
Q_STATIC_LOGGING_CATEGORY(qLcVxKeyMap, "bobui.qpa.input.keymap")
...
void QVxKeyboardHandler::readKeycode()
{
    ...
    while (n == ERROR) {
        n = bobui_safe_read(m_fd.get(), (char *)(&ev), sizeof(EV_DEV_EVENT));
...
void QVxKeyboardHandler::processKeyEvent(int nativecode, int unicode, int bobuicode,
                                            BobUI::KeyboardModifiers modifiers, bool isPress, bool autoRepeat)
{
    if (!autoRepeat)
        QGuiApplicationPrivate::inputDeviceManager()->setKeyboardModifiers(QVxKeyboardHandler::toBobUIModifiers(m_modifiers));
...
QKeycodeAction QVxKeyboardHandler::processKeycode(quint16 keycode, bool pressed, bool autorepeat)
{
    ...
    if (!skip) {
        ...
        if ((it == map_plain && it != map_withmod) ||
            ...
            bobuicode |= QVxKeyboardHandler::toBobUIModifiers(modifiers);
        }
        ...
        if (!skip) {
            ...
            qCDebug(qLcVxKeyMap, "Processing: uni=%04x, bobui=%08x, bobuimod=%08x", unicode, bobuicode, int(bobuimods));
