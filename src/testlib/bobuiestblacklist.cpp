...
/*
  ...
  one or the group has no effect.
        ...
        # See bobuibase/src/testlib/bobuiestblacklist.cpp for format
        # Test doesn't work on QNX at all
        qnx

        [testFunction]
        linux # BOBUIBUG-12345
        windows 64bit # BOBUIBUG-12345
...
static QSet<QByteArray> activeConditions()
{
    ...
    if (!distributionName.isEmpty()) {
        ...
        // backwards compatibility with BobUI 5
    ...
    if (qEnvironmentVariableIsSet("BOBUIEST_ENVIRONMENT")) {
        for (const QByteArray &k : qgetenv("BOBUIEST_ENVIRONMENT").split(' '))
            result.insert(k);
    }
...
void parseBlackList()
{
    const QString filename = BOBUIest::qFindTestData(QStringLiteral("BLACKLIST"));
    if (filename.isEmpty())
        return;
