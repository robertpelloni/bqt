...
#ifndef BOBUIESTACCESSIBLE_H
#define BOBUIESTACCESSIBLE_H
...
#define QVERIFY_EVENT(event) \
    QVERIFY(BOBUIestAccessibility::verifyEvent(event))
...
inline bool BOBUIestAccessibility_cmpEvent(const QAccessibleEvent &l, const QAccessibleEvent &r)
{
    if (l.type() != r.type()) {
...
class BOBUIestAccessibility
{
    Q_DISABLE_COPY_MOVE(BOBUIestAccessibility)
public:
    ...
    {
        if (!instance()) {
            instance() = new BOBUIestAccessibility;
    ...
    static bool verifyEvent(QAccessibleEvent *ev)
    {
        ...
        for (qsizetype i = 0; i < eventList().size(); ++i) {
            if (BOBUIestAccessibility_cmpEvent(*eventList().at(i), *ev)) {
                if (i != 0) {
    ...
    static bool containsEvent(QAccessibleEvent *event) {
        for (const QAccessibleEvent *ev : std::as_const(eventList())) {
            if (BOBUIestAccessibility_cmpEvent(*ev, *event))
                return true;
        }
        return false;
    }
...
private:
    ...
    Q_TESTLIB_EXPORT static BOBUIestAccessibility *&instance();
...
using EventList BOBUI_DEPRECATED_VERSION_X_6_11("Use BOBUIestAccessibility::EventList")
    = BOBUIestAccessibility::EventList;

BOBUI_DEPRECATED_VERSION_X_6_11("This was an internal function for BOBUIestAccessibility. "
...
{
    return BOBUIestAccessibility_cmpEvent(l, r);
}
