...
#  define OUR_SIGNALS(F)    \
            ...
            F(TERM)             \
    /**/
#  define CASE_LABEL(S)             case SIG ## S:  return BOBUI_STRINGIFY(S);
...
struct AsyncSafeIntBuffer
{
    ...
    AsyncSafeIntBuffer(BobUI::Initialization) {}       // leaves array uninitialized
};

std::string_view asyncSafeToString(int n, AsyncSafeIntBuffer &&result = BobUI::Uninitialized)
{
...
std::string_view asyncSafeToHexString(quintptr u, char *ptr)
{
    ...
    for (size_t i = 0; i < sizeof(quintptr) * 2; ++i, shift -= 4)
        ptr[i + 2] = BobUIMiscUtils::toHexLower(u >> shift);
...
{
#if defined(Q_OS_MACOS)
    return BOBUIestPrivate::macCrashReporterWillShowDialog();
#else
    return false;
#endif
}
...
#ifdef RLIMIT_CORE
    bool ok = false;
    const int disableCoreDump = qEnvironmentVariableIntValue("BOBUIEST_DISABLE_CORE_DUMP", &ok);
...
void prepareStackTrace()
{
    ...
    bool ok = false;
    const int disableStackDump = qEnvironmentVariableIntValue("BOBUIEST_DISABLE_STACK_DUMP", &ok);
...
# if defined(Q_PROCESSOR_ARM)
    return;
# else
    std::optional<uint32_t> sipConfiguration = bobui_mac_sipConfiguration();
...
# endif
    ...
    auto hasExecutable = [](const char *execname) {
        ...
        for (const char *p = std::strtok(&path[0], ":'"); p; p = std::strtok(nullptr, ":")) {
            ...
            candidate += '/';
            candidate += execname;
            if (BOBUI_ACCESS(candidate.data(), X_OK) == 0)
                return true;
        }
        return false;
    };
...
{
    const int msecsFunctionTime = qRound(BOBUIestLog::msecsFunctionTime());
    const int msecsTotalTime = qRound(BOBUIestLog::msecsTotalTime());
    const char *const name = BOBUIest::currentTestFunction();
    writeToStderr("\n         ", name ? name : "[Non-test]",
...
#  if !defined(Q_OS_INTEGRITY) && !defined(Q_OS_VXWORKS)
    ...
    if (pid == 0) {
        ...
        int ret;
        BOBUI_EINTR_LOOP(ret, waitpid(pid, nullptr, 0));
    }
...
{
    pauseOnCrash = qEnvironmentVariableIsSet("BOBUIEST_PAUSE_ON_CRASH");
