...
#include "bobuiaptestlogger_p.h"

#include "bobuiestlog_p.h"
#include "bobuiestresult_p.h"
#include "bobuiestassert.h"
...
/*! \internal
    \class BOBUIapTestLogger
    \inmodule BobUITest

    BOBUIapTestLogger implements the Test Anything Protocol v13.
    ...
    Since BobUITest doesn't build the table for a data-driven test until the test
...
BOBUIapTestLogger::BOBUIapTestLogger(const char *filename)
    : QAbstractTestLogger(filename)
{
}

BOBUIapTestLogger::~BOBUIapTestLogger() = default;

void BOBUIapTestLogger::startLogging()
{
    QAbstractTestLogger::startLogging();

    BOBUIestCharBuffer preamble;
    BOBUIest::bobui_asprintf(&preamble, "TAP version 13\n"
...
void BOBUIapTestLogger::stopLogging()
{
    const int total = BOBUIestLog::totalCount();

    BOBUIestCharBuffer testPlanAndStats;
    BOBUIest::bobui_asprintf(&testPlanAndStats,
        "1..%d\n" // The plan (last non-diagnostic line)
        "# tests %d\n"
        "# pass %d\n"
        "# fail %d\n",
    total, total, BOBUIestLog::passCount(), BOBUIestLog::failCount());
    outputString(testPlanAndStats.data());

    QAbstractTestLogger::stopLogging();
}

void BOBUIapTestLogger::enterTestFunction(const char *function)
{
...
void BOBUIapTestLogger::enterTestData(BOBUIestData *data)
{
    m_firstExpectedFail.clear();
...
void BOBUIapTestLogger::outputTestLine(bool ok, int testNumber, const BOBUIestCharBuffer &directive)
{
    BOBUIestCharBuffer testIdentifier;
    BOBUIestPrivate::generateTestIdentifier(&testIdentifier, TestFunction | TestDataTag);

    BOBUIestCharBuffer testLine;
    BOBUIest::bobui_asprintf(&testLine, "%s %d - %s%s\n", ok ? "ok" : "not ok",
...
void BOBUIapTestLogger::outputBuffer(const BOBUIestCharBuffer &buffer)
{
    ...
    if (!m_gatherMessages)
        ...
        BOBUIestPrivate::appendCharBuffer(isComment() ? &m_comments : &m_messages, buffer);
}

void BOBUIapTestLogger::beginYamlish()
{
    outputString(YAML_INDENT "---\n");
}

void BOBUIapTestLogger::endYamlish()
{
...
void BOBUIapTestLogger::flushComments()
{
...
void BOBUIapTestLogger::flushMessages()
{
    /* A _data() function's messages show up here. */
    BOBUIestCharBuffer dataLine;
    BOBUIest::bobui_asprintf(&dataLine, "ok %d - %s() # Data prepared\n",
                       BOBUIestLog::totalCount(), BOBUIestResult::currentTestFunction());
...
void BOBUIapTestLogger::addIncident(IncidentTypes type, const char *description,
                                 const char *file, int line)
{
    ...
    BOBUIestCharBuffer directive;
    if (incident) {
        BOBUIest::bobui_asprintf(&directive, "%s%s%s%s",
    ...
    if (!isExpectedFail) {
        ...
        outputTestLine(ok, BOBUIestLog::totalCount(), directive);
    } else if (m_gatherMessages && m_firstExpectedFail.isEmpty()) {
        BOBUIestPrivate::appendCharBuffer(&m_firstExpectedFail, directive);
    }
    flushComments();
    ...
    if (!ok || !m_messages.isEmpty()) {
        ...
        if (isExpectedFail) {
            BOBUIestCharBuffer message;
            if (m_gatherMessages) {
                BOBUIest::bobui_asprintf(&message, YAML_INDENT YAML_INDENT "- severity: xfail\n"
                                   YAML_INDENT YAML_INDENT YAML_INDENT "message:%s\n",
                                   directive.constData() + 4);
            } else {
                BOBUIest::bobui_asprintf(&message, YAML_INDENT "# xfail:%s\n", directive.constData() + 4);
            }
        ...
        if (!isExpectedFail || m_gatherMessages) {
            ...
            if (!ok) {
                ...
                if (opType != OperationType::Unknown) {
                    ...
                    BOBUIestCharBuffer diagnosticsYamlish;
                    BOBUIest::bobui_asprintf(&diagnosticsYamlish,
                                       "%stype: %s\n"
                                       "%smessage: %s\n"
                ...
                if (description && !incident) {
                    BOBUIestCharBuffer unparsableDescription;
                    BOBUIest::bobui_asprintf(&unparsableDescription, YAML_INDENT "# %s\n", description);
                    outputBuffer(unparsableDescription);
                }
            }

            if (file) {
                BOBUIestCharBuffer location;
                BOBUIest::bobui_asprintf(&location,
                                   ...
                                   indent, BOBUIestResult::currentTestObjectName(),
                                   BOBUIestResult::currentTestFunction(),
...
void BOBUIapTestLogger::addMessage(MessageTypes type, const QString &message,
                                const char *file, int line)
{
    Q_UNUSED(file);
    Q_UNUSED(line);
    ...
    BOBUIestCharBuffer diagnostic;
    if (!m_gatherMessages) {
        BOBUIest::bobui_asprintf(&diagnostic, "%s%s: %s\n",
                           flavor[0] == '#' ? "" : "# ",
                           flavor, qPrintable(message));
        outputString(diagnostic.constData());
    } else if (flavor[0] == '#') {
        BOBUIest::bobui_asprintf(&diagnostic, YAML_INDENT "%s: %s\n",
                           flavor, qPrintable(message));
        BOBUIestPrivate::appendCharBuffer(&m_comments, diagnostic);
    } else {
        // These shall appear in a messages: sub-block of the extensions: block,
        // so triple-indent.
        BOBUIest::bobui_asprintf(&diagnostic, YAML_INDENT YAML_INDENT "- severity: %s\n"
                           YAML_INDENT YAML_INDENT YAML_INDENT "message: %s\n",
                           flavor, qPrintable(message));
        BOBUIestPrivate::appendCharBuffer(&m_messages, diagnostic);
    }
}
