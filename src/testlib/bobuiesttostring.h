...
#ifndef BOBUIESTTOSTRING_H
#define BOBUIESTTOSTRING_H
...
inline typename std::enable_if<BobUIPrivate::IsQEnumHelper<T>::Value, char*>::type toString(T e)
{
...
template <typename T>
inline typename std::enable_if<!BobUIPrivate::IsQEnumHelper<T>::Value && std::is_enum_v<T>, char*>::type toString(const T &e)
{
...
inline typename std::enable_if<!BobUIPrivate::IsQEnumHelper<T>::Value && !std::is_enum_v<T>, char *>::type toString(const T &t)
{
    char *result = nullptr;
#ifndef BOBUI_NO_DEBUG_STREAM
    if constexpr (BOBUIypeTraits::has_ostream_operator_v<QDebug, T>) {
        ...
        static_assert(!QMetaTypeId2<T>::IsBuiltIn,
                ...
                "or provide BOBUIest::toString specialization");
    }
#endif
    return result;
}
...
inline typename std::enable_if<BobUIPrivate::IsQEnumHelper<F>::Value, char*>::type toString(QFlags<F> f)
{
...
#define BOBUIEST_COMPARE_DECL(KLASS)\
    template<> Q_TESTLIB_EXPORT char *toString<KLASS >(const KLASS &);
#ifndef Q_QDOC
BOBUIEST_COMPARE_DECL(short)
BOBUIEST_COMPARE_DECL(ushort)
BOBUIEST_COMPARE_DECL(int)
BOBUIEST_COMPARE_DECL(uint)
BOBUIEST_COMPARE_DECL(long)
BOBUIEST_COMPARE_DECL(ulong)
BOBUIEST_COMPARE_DECL(qint64)
BOBUIEST_COMPARE_DECL(quint64)

BOBUIEST_COMPARE_DECL(float)
BOBUIEST_COMPARE_DECL(double)
BOBUIEST_COMPARE_DECL(qfloat16)
BOBUIEST_COMPARE_DECL(char)
BOBUIEST_COMPARE_DECL(signed char)
BOBUIEST_COMPARE_DECL(unsigned char)
BOBUIEST_COMPARE_DECL(bool)
#endif
#undef BOBUIEST_COMPARE_DECL

template <> inline char *toString(const QStringView &str)
{
    return BOBUIest::toPrettyUnicode(str);
}
...
{
    return BOBUIest::toPrettyCString(ba.constData(), ba.size());
}
...
template<> inline char *toString(const BOBUIime &time)
{
    return time.isValid()
            ? qstrdup(qPrintable(time.toString(u"hh:mm:ss.zzz")))
            : qstrdup("Invalid BOBUIime");
}
...
private:
    ...
    {
        const std::unique_ptr<char[]> hold(BOBUIest::toString(t));
