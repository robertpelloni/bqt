...
function(bobui6_add_dbus_interface _sources _interface _relativename)
    ...
    add_custom_command(OUTPUT "${_impl}" "${_header}"
        COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbusxml2cpp ${_params} -p ${_relativename} ${_infile}
        DEPENDS ${_infile} ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbuscpp2xml
        VERBATIM
    )
    ...
    bobui6_generate_moc("${_header}" "${_moc}")
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    ...
    function(bobui_add_dbus_interface sources interface relativename)
        if(ARGC GREATER 3)
            message(FATAL_ERROR "Unexpected arguments: ${ARGN}")
        endif()
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_add_dbus_interface("${sources}" "${interface}" "${relativename}")
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_add_dbus_interface("${sources}" "${interface}" "${relativename}")
        endif()
...
function(bobui6_add_dbus_interfaces _sources)
    foreach(_current_FILE ${ARGN})
        ...
        bobui6_add_dbus_interface(${_sources} ${_infile} ${_basename}interface)
    endforeach()
    set(${_sources} ${${_sources}} PARENT_SCOPE)
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_dbus_interfaces sources)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_add_dbus_interfaces("${sources}" ${ARGN})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_add_dbus_interfaces("${sources}" ${ARGN})
        endif()
        set("${sources}" "${${sources}}" PARENT_SCOPE)
    endfunction()
endif()
...
function(bobui6_generate_dbus_interface _header) # _customName OPTIONS -some -options )
    set(options)
    set(oneValueArgs)
    ...
    add_custom_command(OUTPUT ${_target}
        COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbuscpp2xml ${_DBUS_INTERFACE_OPTIONS} ${_in_file} -o ${_target}
        DEPENDS ${_in_file} ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbuscpp2xml
        VERBATIM
    )
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_generate_dbus_interface)
        ...
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_generate_dbus_interface(${ARGV})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_generate_dbus_interface(${ARGV})
        endif()
    endfunction()
endif()


function(bobui6_add_dbus_adaptor _sources _xml_file _include) # _optionalParentClass _optionalRelativename _optionalClassName)
    ...
    if(_optionalClassName)
        add_custom_command(OUTPUT "${_impl}" "${_header}"
          COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbusxml2cpp -m -a ${_relativename} -c ${_optionalClassName} -i ${_include} ${_parentClassOption} ${_parentClass} ${_infile}
          DEPENDS ${_infile} ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbuscpp2xml
          VERBATIM
        )
    else()
        add_custom_command(OUTPUT "${_impl}" "${_header}"
          COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbusxml2cpp -m -a ${_relativename} -i ${_include} ${_parentClassOption} ${_parentClass} ${_infile}
          DEPENDS ${_infile} ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qdbuscpp2xml
          VERBATIM
        )
    endif()
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_dbus_adaptor sources dbus_spec header)
        ...
        # Also note that the parent_class argument is required for bobui5, but is
        # optional for bobui6.
        if(ARGC LESS 4)
            set(parent_class "")
        else()
