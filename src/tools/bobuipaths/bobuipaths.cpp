...
public:
    ...
    */
    ...
    {
        return hasappname ? QString(s).replace("bobuipaths", "<APPNAME>") : s;
    }
};
...
{
    QString bobuiconfManualPath;
    QCoreApplication app(argc, argv);
    app.setApplicationVersion(BOBUIPATHS_VERSION_STR);
    ...
    parser.addPositionalArgument(QStringLiteral("[properties]"), QStringLiteral("List of the BobUI properties to query by the --bobui-query argument."));
    ...
    QCommandLineOption bobuiversion(QStringLiteral("bobui-version"), QStringLiteral("BobUI version."));
    bobuiversion.setFlags(QCommandLineOption::HiddenFromHelp);
    parser.addOption(bobuiversion);

    QCommandLineOption installprefix(QStringLiteral("install-prefix"), QStringLiteral("Installation prefix for BobUI."));
    ...
    QCommandLineOption bindir(QStringList() << QStringLiteral("binaries-dir") << QStringLiteral("binaries-directory"),
                              QStringLiteral("Location of BobUI executables."));
    ...
    QCommandLineOption plugindir(QStringList() << QStringLiteral("plugin-dir") << QStringLiteral("plugin-directory"),
                                 QStringLiteral("Location of BobUI plugins."));
    plugindir.setFlags(QCommandLineOption::HiddenFromHelp);
    parser.addOption(plugindir);

    QCommandLineOption query(
            QStringList() << QStringLiteral("bobui-query") << QStringLiteral("query"),
            QStringLiteral("List of BobUI properties. Can be used standalone or with the "
                           "--query-format and --bobuiconf options."));
    parser.addOption(query);

    QCommandLineOption queryformat(QStringLiteral("query-format"),
                                   QStringLiteral("Output format for --bobui-query.\nSupported formats: qmake (default), json"),
                                   QStringLiteral("format"));
    queryformat.setDefaultValue("qmake");
    parser.addOption(queryformat);

    QCommandLineOption bobuiconf(QStringLiteral("bobuiconf"),
                                   QStringLiteral("Path to bobui.conf file that will be used to override the queried BobUI properties."),
                                   QStringLiteral("path"));
    parser.addOption(bobuiconf);
...
#if BOBUI_CONFIG(settings)
    if (parser.isSet(bobuiconf)) {
        bobuiconfManualPath = parser.value(bobuiconf);
        QLibraryInfoPrivate::setBobUIconfManualPath(&bobuiconfManualPath);
    }
#endif

    QStringList results;
    if (parser.isSet(bobuiversion)) {
        QString bobuiversionstring = QString::fromLatin1(BOBUI_VERSION_STR);
        results << bobuiversionstring;
    }
...
#if !BOBUI_CONFIG(settings)
    if (parser.isSet(query) || parser.isSet(bobuiconf) || parser.isSet(queryformat)) {
        error(QStringLiteral("--bobui-query, --bobuiconf and --query-format options are not supported. The 'settings' feature is missing."));
    }
#else
    if (parser.isSet(query)) {
        ...
        error(QStringLiteral("--query-format is set, but --bobui-query is not requested."));
    }
#endif
