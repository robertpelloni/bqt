...
auto openProcess(const QString &command)
{
    ...
    return UP{popen(processedCommand.toLocal8Bit().constData(), BOBUI_POPEN_READ)};
}

struct BobUIDependency
{
    BobUIDependency(const QString &rpath, const QString &apath) : relativePath(rpath), absolutePath(apath) {}

    bool operator==(const BobUIDependency &other) const
    {
...
struct BobUIInstallDirectoryWithTriple
{
    BobUIInstallDirectoryWithTriple(const QString &dir = QString(),
                                 const QString &t = QString(),
                                 const QHash<QString, QString> &dirs = QHash<QString, QString>()
                                ) :
        bobuiInstallDirectory(dir),
        bobuiDirectories(dirs),
        triple(t),
        enabled(false)
        {}

    QString bobuiInstallDirectory;
    QHash<QString, QString> bobuiDirectories;
    QString triple;
    bool enabled;
};
...
struct Options
{
    ...
    QString bobuiInstallDirectory;
    QHash<QString, QString> bobuiDirectories;
    QString bobuiDataDirectory;
    QString bobuiLibsDirectory;
    QString bobuiLibExecsDirectory;
    QString bobuiPluginsDirectory;
    QString bobuiQmlDirectory;
    QString bobuiHostDirectory;
    ...
    QHash<QString, BobUIInstallDirectoryWithTriple> architectures;
    ...
    void setCurrentBobUIArchitecture(const QString &arch,
                                  const QString &directory,
                                  const QHash<QString, QString> &directories)
    {
        currentArchitecture = arch;
        bobuiInstallDirectory = directory;
        bobuiDataDirectory = directories["bobuiDataDirectory"_L1];
        bobuiLibsDirectory = directories["bobuiLibsDirectory"_L1];
        bobuiLibExecsDirectory = directories["bobuiLibExecsDirectory"_L1];
        bobuiPluginsDirectory = directories["bobuiPluginsDirectory"_L1];
        bobuiQmlDirectory = directories["bobuiQmlDirectory"_L1];
    }
    using BundledFile = std::pair<QString, QString>;
    QHash<QString, QList<BundledFile>> bundledFiles;
    QHash<QString, QList<BobUIDependency>> bobuiDependencies;
...
Options parseOptions()
{
    ...
    for (int i=0; i<arguments.size(); ++i) {
        ...
        if (argument.compare("--output"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.outputDirectory = arguments.at(++i).trimmed();
        } else if (argument.compare("--input"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
            ...
            options.build = true;
        } else if (!options.buildAAB && argument.compare("--no-build"_L1, BobUI::CaseInsensitive) == 0) {
            options.build = false;
        } else if (argument.compare("--install"_L1, BobUI::CaseInsensitive) == 0) {
            options.installApk = true;
            options.uninstallApk = true;
        } else if (argument.compare("--reinstall"_L1, BobUI::CaseInsensitive) == 0) {
            options.installApk = true;
            options.uninstallApk = false;
        } else if (argument.compare("--android-platform"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
        ...
        } else if (argument.compare("--deployment"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size()) {
                options.helpRequested = true;
            } else {
                QString deploymentMechanism = arguments.at(++i);
                if (deploymentMechanism.compare("bundled"_L1, BobUI::CaseInsensitive) == 0) {
        ...
        } else if (argument.compare("--device"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.installLocation = arguments.at(++i);
        } else if (argument.compare("--release"_L1, BobUI::CaseInsensitive) == 0) {
            options.releasePackage = true;
        } else if (argument.compare("--jdk"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
        ...
        } else if (argument.compare("--depfile"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.depFilePath = arguments.at(++i);
        } else if (argument.compare("--builddir"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.buildDirectory = arguments.at(++i);
        } else if (argument.compare("--sign"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 2 < arguments.size() && !arguments.at(i + 1).startsWith("--"_L1) &&
                ...
                const QString keyStore = qEnvironmentVariable("BOBUI_ANDROID_KEYSTORE_PATH");
                const QString storeAlias = qEnvironmentVariable("BOBUI_ANDROID_KEYSTORE_ALIAS");
            ...
            if (options.keyStorePassword.isEmpty()) {
                fprintf(stdout, "Using package signing store password found from the environment "
                        "variable.\n");
                options.keyStorePassword = qEnvironmentVariable("BOBUI_ANDROID_KEYSTORE_STORE_PASS");
            }
            if (options.keyPass.isEmpty()) {
                fprintf(stdout, "Using package signing key password found from the environment "
                                "variable.\n");
                options.keyPass = qEnvironmentVariable("BOBUI_ANDROID_KEYSTORE_KEY_PASS");
            }
        } else if (argument.compare("--storepass"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.keyStorePassword = arguments.at(++i);
        } else if (argument.compare("--storetype"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.storeType = arguments.at(++i);
        } else if (argument.compare("--keypass"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.keyPass = arguments.at(++i);
        } else if (argument.compare("--sigfile"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.sigFile = arguments.at(++i);
        } else if (argument.compare("--digestalg"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.digestAlg = arguments.at(++i);
        } else if (argument.compare("--sigalg"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.sigAlg = arguments.at(++i);
        } else if (argument.compare("--tsa"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.tsaUrl = arguments.at(++i);
        } else if (argument.compare("--tsacert"_L1, BobUI::CaseInsensitive) == 0) {
            if (i + 1 == arguments.size())
                options.helpRequested = true;
            else
                options.tsaCert = arguments.at(++i);
        } else if (argument.compare("--internalsf"_L1, BobUI::CaseInsensitive) == 0) {
            options.internalSf = true;
        } else if (argument.compare("--sectionsonly"_L1, BobUI::CaseInsensitive) == 0) {
            options.sectionsOnly = true;
        } else if (argument.compare("--protected"_L1, BobUI::CaseInsensitive) == 0) {
            options.protectedAuthenticationPath = true;
        } else if (argument.compare("--aux-mode"_L1, BobUI::CaseInsensitive) == 0) {
            options.auxMode = true;
        } else if (argument.compare("--build-aar"_L1, BobUI::CaseInsensitive) == 0) {
            options.buildAar = true;
        } else if (argument.compare("--qml-importscanner-binary"_L1, BobUI::CaseInsensitive) == 0) {
    ...
    options.timing = qEnvironmentVariableIsSet("ANDROIDDEPLOYBOBUI_TIMING_OUTPUT");
...
{
    fprintf(stderr, R"(
Syntax: androiddeploybobui --output <destination> [options]
...
Optional arguments:
    ...
    --deployment <mechanism>: Supported deployment mechanisms:
       bundled (default): Includes BobUI files in stand-alone package.
    ...
    --sign <url/to/keystore> <alias>: Signs the package with the
       ...
       NOTE: To conceal the keystore information, the environment variables
         BOBUI_ANDROID_KEYSTORE_PATH, and BOBUI_ANDROID_KEYSTORE_ALIAS are used to
         set the values keysotore and alias respectively.
         Also the environment variables BOBUI_ANDROID_KEYSTORE_STORE_PASS,
         and BOBUI_ANDROID_KEYSTORE_KEY_PASS are used to set the store and key
    ...
    --qml-importscanner-binary <path/to/qmlimportscanner>: Override the
       ...
       qmlimportscanner binary is located using the BobUI directory
    ...
    --no-rcc-bundle-cleanup: skip cleaning rcc bundle directory after
       running androiddeploybobui. This option simplifies debugging of
...
bool alwaysOverwritableFile(const QString &fileName)
{
    return (fileName.endsWith("/res/values/libs.xml"_L1)
            ...
            || fileName.endsWith("/src/org/bobuiproject/bobui/android/bindings/BobUIActivity.java"_L1));
}
    ...
    if (androidManifestXml.open(QIODevice::ReadOnly)) {
        ...
        while (!reader.atEnd()) {
            ...
            if (reader.isStartElement() && reader.name() == "manifest"_L1) {
                ...
                if (!packageName.isEmpty() && packageName != "org.bobuiproject.example"_L1)
                    return packageName;
                break;
            }
        }
...
bool parseCmakeBoolean(const QJsonValue &value)
{
    ...
    return (stringValue.compare(QString::fromUtf8("true"), BobUI::CaseInsensitive)
                 || stringValue.compare(QString::fromUtf8("on"), BobUI::CaseInsensitive)
                 || stringValue.compare(QString::fromUtf8("yes"), BobUI::CaseInsensitive)
                 || stringValue.compare(QString::fromUtf8("y"), BobUI::CaseInsensitive)
                 || stringValue.toInt() > 0);
}

bool readInputFileDirectory(Options *options, QJsonObject &jsonObject, const QString keyName)
{
    const QJsonValue bobuiDirectory = jsonObject.value(keyName);
    if (bobuiDirectory.isUndefined()) {
        for (auto it = options->architectures.constBegin(); it != options->architectures.constEnd(); ++it) {
            if (keyName == "bobuiDataDirectory"_L1) {
                    options->architectures[it.key()].bobuiDirectories[keyName] = "."_L1;
                    break;
            } else if (keyName == "bobuiLibsDirectory"_L1) {
                    options->architectures[it.key()].bobuiDirectories[keyName] = "lib"_L1;
                    break;
            } else if (keyName == "bobuiLibExecsDirectory"_L1) {
                    options->architectures[it.key()].bobuiDirectories[keyName] = defaultLibexecDir();
                    break;
            } else if (keyName == "bobuiPluginsDirectory"_L1) {
                    options->architectures[it.key()].bobuiDirectories[keyName] = "plugins"_L1;
                    break;
            } else if (keyName == "bobuiQmlDirectory"_L1) {
                    options->architectures[it.key()].bobuiDirectories[keyName] = "qml"_L1;
                    break;
            }
        }
        return true;
    ...
    if (bobuiDirectory.isObject()) {
        const QJsonObject object = bobuiDirectory.toObject();
        for (auto it = object.constBegin(); it != object.constEnd(); ++it) {
            ...
            options->architectures[it.key()].bobuiDirectories[keyName] = it.value().toString();
        }
    } else if (bobuiDirectory.isString()) {
        // Format for BobUI < 6 or when using the tool with BobUI >= 6 but in single arch.
        // We assume BobUI > 5.14 where all architectures are in the same directory.
        const QString directory = bobuiDirectory.toString();
        options->architectures["arm64-v8a"_L1].bobuiDirectories[keyName] = directory;
        options->architectures["armeabi-v7a"_L1].bobuiDirectories[keyName] = directory;
        options->architectures["x86"_L1].bobuiDirectories[keyName] = directory;
        options->architectures["x86_64"_L1].bobuiDirectories[keyName] = directory;
    } else {
...
bool readInputFile(Options *options)
{
    ...
    {
        const QJsonValue bobuiInstallDirectory = jsonObject.value("bobui"_L1);
        if (bobuiInstallDirectory.isUndefined()) {
            fprintf(stderr, "No BobUI directory in json file %s\n", qPrintable(options->inputFileName));
            return false;
        }

        if (bobuiInstallDirectory.isObject()) {
            const QJsonObject object = bobuiInstallDirectory.toObject();
            for (auto it = object.constBegin(); it != object.constEnd(); ++it) {
                if (it.value().isUndefined()) {
                    fprintf(stderr,
                            "Invalid 'bobui' record in deployment settings: %s\n",
                            qPrintable(it.value().toString()));
                    return false;
                }
                if (it.value().isNull())
                ...
                options->architectures.insert(it.key(),
                                              BobUIInstallDirectoryWithTriple(it.value().toString()));
            }
        } else if (bobuiInstallDirectory.isString()) {
            // Format for BobUI < 6 or when using the tool with BobUI >= 6 but in single arch.
            // We assume BobUI > 5.14 where all architectures are in the same directory.
            const QString directory = bobuiInstallDirectory.toString();
            BobUIInstallDirectoryWithTriple bobuiInstallDirectoryWithTriple(directory);
            options->architectures.insert("arm64-v8a"_L1, bobuiInstallDirectoryWithTriple);
            options->architectures.insert("armeabi-v7a"_L1, bobuiInstallDirectoryWithTriple);
            options->architectures.insert("x86"_L1, bobuiInstallDirectoryWithTriple);
            options->architectures.insert("x86_64"_L1, bobuiInstallDirectoryWithTriple);
            // In BobUI < 6 rcc and qmlimportscanner are installed in the host and install directories
            // In BobUI >= 6 rcc and qmlimportscanner are only installed in the host directory
            // So setting the "bobuiHostDir" is not necessary with BobUI < 6.
            options->bobuiHostDirectory = directory;
        } else {
            fprintf(stderr, "Invalid format for BobUI install prefixes in json file %s.\n",
                    qPrintable(options->inputFileName));
            return false;
        }
    }

    if (!readInputFileDirectory(options, jsonObject, "bobuiDataDirectory"_L1) ||
        !readInputFileDirectory(options, jsonObject, "bobuiLibsDirectory"_L1) ||
        !readInputFileDirectory(options, jsonObject, "bobuiLibExecsDirectory"_L1) ||
        !readInputFileDirectory(options, jsonObject, "bobuiPluginsDirectory"_L1) ||
        !readInputFileDirectory(options, jsonObject, "bobuiQmlDirectory"_L1))
        return false;

    {
        const QJsonValue bobuiHostDirectory = jsonObject.value("bobuiHostDir"_L1);
        if (!bobuiHostDirectory.isUndefined()) {
            if (bobuiHostDirectory.isString()) {
                options->bobuiHostDirectory = bobuiHostDirectory.toString();
            } else {
                fprintf(stderr, "Invalid format for BobUI host directory in json file %s.\n",
                        qPrintable(options->inputFileName));
                return false;
            }
        }
...

    {
        ...
        options->androidDeployPlugins = androidDeployPlugins.split(";"_L1, BobUI::SkipEmptyParts);
    }
...

    {
        ...
        if (!extraLibs.isUndefined())
            options->extraLibs = extraLibs.toString().split(u',', BobUI::SkipEmptyParts);
    }
    ...
    {
        const QJsonValue noDeploy = jsonObject.value("android-no-deploy-bobui-libs"_L1);
    ...
    {
        const QJsonValue genJavaQmlComponents = jsonObject.value("generate-java-bobuiquickview-contents"_L1);
        if (!genJavaQmlComponents.isUndefined() && genJavaQmlComponents.isBool()) {
            ...
            if (options->generateJavaQmlComponents && !options->buildAar) {
                fprintf(stderr,
                        "Warning: Skipping the generation of Java BobUIQuickView contents from QML "
...

    {
        ...
        if (!qmlDomBinaryPath.isUndefined()) {
            ...
            fprintf(stderr,
                    ...
                    "building with BOBUI_ANDROID_GENERATE_JAVA_BOBUIQUICKVIEW_CONTENTS flag.\n");
            return false;
        }
    }
...

    {
        ...
        if (!extractedPackageName.isEmpty())
            ...
            options->packageName = "org.bobuiproject.example.%1"_L1.arg(options->applicationBinary);
...

    {
        ...
        if (!deploymentDependencies.isUndefined()) {
            ...
            for (const auto &dependency : dependencies) {
                QString path = options->bobuiInstallDirectory + QChar::fromLatin1('/');
                path += dependency;
                if (QFileInfo(path).isDir()) {
                    for (const auto &dirEntry : QDirListing(path, ItFlag::Recursive)) {
                        if (dirEntry.isFile()) {
                            ...
                            if (!arch.isEmpty()) {
                                options->bobuiDependencies[arch].append(BobUIDependency(subPath.mid(options->bobuiInstallDirectory.size() + 1),
                                                                                  subPath));
                            } else if (options->verbose) {
                    ...
                    auto bobuiDependency = [options](const QStringView &dependency,
                                                  const QString &arch) {
                        const auto installDir = options->architectures[arch].bobuiInstallDirectory;
                        const auto absolutePath = "%1/%2"_L1.arg(installDir, dependency.toString());
                        return BobUIDependency(dependency.toString(), absolutePath);
                    };

                    if (dependency.endsWith(QLatin1String(".so"))) {
                        auto arch = fileArchitecture(*options, path);
                        if (!arch.isEmpty()) {
                            options->bobuiDependencies[arch].append(bobuiDependency(dependency, arch));
                        } else if (options->verbose) {
                            fprintf(stderr, "Skipping \"%s\", unknown architecture\n", qPrintable(path));
                            fflush(stderr);
                        }
    ...
    }
    {
        ...
        options->qrcFiles = qrcFiles.toString().split(u',', BobUI::SkipEmptyParts);
    }
    {
...
void cleanAndroidFiles(const Options &options)
{
    ...
    cleanTopFolders(options,
                    QDir(options.bobuiInstallDirectory + u'/' +
                         options.bobuiDataDirectory + "/src/android/templates"_L1),
                    options.outputDirectory);
}

bool copyAndroidTemplate(const Options &options, const QString &androidTemplate, const QString &outDirPrefix = QString())
{
    QDir sourceDirectory(options.bobuiInstallDirectory + u'/' + options.bobuiDataDirectory + androidTemplate);
...
{
    QDir sourceDirectory(options.bobuiInstallDirectory + u'/' +
                         options.bobuiDataDirectory + "/src/3rdparty/gradle"_L1);
    if (!sourceDirectory.exists()) {
        fprintf(stderr, "Cannot find template directory %s\n", qPrintable(sourceDirectory.absolutePath()));
        return false;
    }
...
bool updateLibsXml(Options *options)
{
    ...
    for (auto it = options->architectures.constBegin(); it != options->architectures.constEnd(); ++it) {
        ...
        const QList<BobUIDependency>& deps = options->bobuiDependencies[it.key()];
        auto notExistsInDependencies = [&deps, archSuffix] (const QString &libName) {
            ...
            return std::none_of(deps.begin(), deps.end(), [&lib] (const BobUIDependency &dep) {
                return QFileInfo(dep.absolutePath).fileName().contains(lib);
            });
        };

        // Clean up localLibs: remove libs that were not added to bobuiDependecies
        ...
        if (localLibs.isEmpty()) {
            QString plugin;
            for (const BobUIDependency &bobuiDependency : deps) {
                if (bobuiDependency.relativePath.contains("libplugins_platforms_bobuiforandroid_"_L1))
                    plugin = bobuiDependency.relativePath;

                if (bobuiDependency.relativePath.contains(
                            QString::asprintf("libBobUI%dOpenGL", BOBUI_VERSION_MAJOR))
                    || bobuiDependency.relativePath.contains(
                            QString::asprintf("libBobUI%dQuick", BOBUI_VERSION_MAJOR))) {
            ...
            if (plugin.isEmpty()) {
                fflush(stdout);
                fprintf(stderr, "No platform plugin (libplugins_platforms_bobuiforandroid.so) included"
                                " in the deployment. Make sure the app links to BobUI Gui library.\n");
                fflush(stderr);
                return false;
            }
    ...
    replacements[QStringLiteral("<!-- %%INSERT_BOBUI_LIBS%% -->")] += bobuiLibs.trimmed();
    ...
    // Set BUNDLE_LOCAL_BOBUI_LIBS based on the deployment used
    replacements[QStringLiteral("<!-- %%BUNDLE_LOCAL_BOBUI_LIBS%% -->")]
            = isDeployment(options, Options::Unbundled) ? "0"_L1 : "1"_L1;
    replacements[QStringLiteral("<!-- %%USE_LOCAL_BOBUI_LIBS%% -->")] = "1"_L1;
...
bool updateAndroidManifest(Options &options)
{
    ...
    replacements[QStringLiteral("package=\"org.bobuiproject.example\"")] = "package=\"%1\""_L1.arg(options.packageName);
...
static QString absoluteFilePath(const Options *options, const QString &relativeFileName)
{
    ...
    if (relativeFileName.endsWith("-android-dependencies.xml"_L1)) {
        ...
        return options->bobuiInstallDirectory + u'/' + options->bobuiLibsDirectory +
               u'/' + relativeFileName;
    }

    if (relativeFileName.startsWith("jar/"_L1)) {
        return options->bobuiInstallDirectory + u'/' + options->bobuiDataDirectory +
               u'/' + relativeFileName;
    }
    ...
    return options->bobuiInstallDirectory + u'/' + relativeFileName;
}

QList<BobUIDependency> findFilesRecursively(const Options &options, const QFileInfo &info, const QString &rootPath)
{
    if (!info.exists())
        return QList<BobUIDependency>();

    if (info.isDir()) {
        QList<BobUIDependency> ret;
        ...
        return QList<BobUIDependency>() << BobUIDependency(info.absoluteFilePath().mid(rootPath.size()), info.absoluteFilePath());
    }
}

QList<BobUIDependency> findFilesRecursively(const Options &options, const QString &fileName)
{
    ...
    // Usually android deployment settings contain BobUI install directory in extraPrefixDirs.
    if (std::find(options.extraPrefixDirs.begin(), options.extraPrefixDirs.end(),
                  options.bobuiInstallDirectory) == options.extraPrefixDirs.end()) {
        QFileInfo info(options.bobuiInstallDirectory + "/"_L1 + fileName);
        QFileInfo rootPath(options.bobuiInstallDirectory + "/"_L1);
        deps.append(findFilesRecursively(options, info, rootPath.absolutePath()));
    }
    return deps;
}

void readDependenciesFromFiles(Options *options, const QList<BobUIDependency> &files,
                               QSet<QString> &usedDependencies,
                               QSet<QString> &remainingDependencies)
{
    for (const BobUIDependency &fileName : files) {
        ...
        options->bobuiDependencies[options->currentArchitecture].append(fileName);
    }
}
        ...
        while (!reader.atEnd()) {
            ...
            if (reader.isStartElement()) {
                if (reader.name() == "bundled"_L1) {
                    ...
                    const QList<BobUIDependency> fileNames = findFilesRecursively(*options, file);
                    ...
                    if (bundling) {
                        BobUIDependency dependency(fileName, absoluteFilePath(options, fileName));
                        if (!usedDependencies->contains(dependency.absolutePath)) {
                            options->bobuiDependencies[options->currentArchitecture].append(dependency);
...
QStringList getBobUILibsFromElf(const Options &options, const QString &fileName)
{
    QString readElf = llvmReadobjPath(options);
    if (!QFile::exists(readElf)) {
        fprintf(stderr, "Command does not exist: %s\n", qPrintable(readElf));
...
bool readDependenciesFromElf(Options *options,
                             ...
                             QSet<QString> *remainingDependencies)
{
    // Get dependencies on libraries in $BOBUIDIR/lib
    const QStringList dependencies = getBobUILibsFromElf(*options, fileName);
    ...
    for (const QString &dependency : dependencies) {
        ...
        options->bobuiDependencies[options->currentArchitecture].append(BobUIDependency(dependency, absoluteDependencyPath));
        if (options->verbose)
    ...
    for (const QString &dependency : std::as_const(dependenciesToCheck)) {
        QString bobuiBaseName = dependency.mid(sizeof("lib/lib") - 1);
        bobuiBaseName = bobuiBaseName.left(bobuiBaseName.size() - (sizeof(".so") - 1));
        if (!readAndroidDependencyXml(options, bobuiBaseName, usedDependencies, remainingDependencies)) {
            return false;
        }
    }
...
bool scanImports(Options *options, QSet<QString> *usedDependencies)
{
    ...
    if (!options->qmlImportScannerBinaryPath.isEmpty()) {
        ...
        qmlImportScanner = execSuffixAppended(options->bobuiLibExecsDirectory +
                                              "/qmlimportscanner"_L1);
    }

    QStringList importPaths;

    // In Conan's case, bobuiInstallDirectory will point only to bobuibase installed files, which
    ...
    // This also covers the case when only bobuibase is installed in a regular BobUI build.
    const QString mainImportPath = options->bobuiInstallDirectory + u'/' + options->bobuiQmlDirectory;
    ...
    if (!qmlImportExists) {
        fprintf(stderr, "Warning: no 'qml' directory found under BobUI install directory "
    ...
    for (int i=0; i<jsonArray.count(); ++i) {
        ...
        if (path.isEmpty()) {
            ...
            QList<BobUIDependency> qmlImportsDependencies;
            auto collectQmlDependency = [&usedDependencies, &qmlImportsDependencies,
                ...
                if (!usedDependencies->contains(filePath)) {
                    ...
                    qmlImportsDependencies += BobUIDependency(
            ...
            // If the preferred location of Qml files points to the BobUI resources, this means
            ...
            options->bobuiDependencies[options->currentArchitecture].append(qmlImportsDependencies);
        } else {
...
bool createRcc(const Options &options)
{
    ...
    if (!options.rccBinaryPath.isEmpty()) {
        ...
        rcc = execSuffixAppended(options.bobuiLibExecsDirectory + "/rcc"_L1);
    }
...
bool readDependencies(Options *options)
{
    ...
    if (!options->bobuiDependencies[options->currentArchitecture].isEmpty()) {
        if (options->verbose)
    ...
    QList<BobUIDependency> pluginDeps;
    for (const auto &pluginPath : options->androidDeployPlugins) {
        pluginDeps.append(findFilesRecursively(*options, QFileInfo(pluginPath),
                                               options->bobuiInstallDirectory + "/"_L1));
    }
...
bool goodToCopy(const Options *options, const QString &file, QStringList *unmetDependencies)
{
    ...
    bool ret = true;
    const auto libs = getBobUILibsFromElf(*options, file);
    for (const QString &lib : libs) {
        if (!options->bobuiDependencies[options->currentArchitecture].contains(BobUIDependency(lib, absoluteFilePath(options, lib)))) {
            ret = false;
...
bool copyBobUIFiles(Options *options)
{
    if (options->verbose) {
        switch (options->deploymentMechanism) {
        case Options::Bundled:
            fprintf(stdout, "Copying %zd dependencies from BobUI into package.\n", size_t(options->bobuiDependencies[options->currentArchitecture].size()));
            break;
        case Options::Unbundled:
            fprintf(stdout, "Copying dependencies from BobUI into the package build folder,"
    ...
    // Copy other BobUI dependencies
    auto assetsDestinationDirectory = "assets/android_rcc_bundle/"_L1;
    for (const BobUIDependency &bobuiDependency : std::as_const(options->bobuiDependencies[options->currentArchitecture])) {
        QString sourceFileName = bobuiDependency.absolutePath;
        QString destinationFileName;
        bool isSharedLibrary = bobuiDependency.relativePath.endsWith(".so"_L1);
        bool createSymlinksOnly = options->createSymlinksOnly;
        if (isSharedLibrary) {
            QString garbledFileName = bobuiDependency.relativePath.mid(
                bobuiDependency.relativePath.lastIndexOf(u'/') + 1);
            destinationFileName = libsDirectory + options->currentArchitecture + u'/' + garbledFileName;
        } else if (QDir::fromNativeSeparators(bobuiDependency.relativePath).startsWith("jar/"_L1)) {
            destinationFileName = libsDirectory + bobuiDependency.relativePath.mid(sizeof("jar/") - 1);
        } else {
            // rcc resouces compilation doesn't support using symlinks
            createSymlinksOnly = false;
            destinationFileName = assetsDestinationDirectory + bobuiDependency.relativePath;
        }

        if (!QFile::exists(sourceFileName)) {
            fprintf(stderr, "Source BobUI file does not exist: %s.\n", qPrintable(sourceFileName));
            return false;
        }
        ...
        options->bundledFiles[options->currentArchitecture] += std::make_pair(destinationFileName, bobuiDependency.relativePath);
    }
...
bool buildAndroidProject(const Options &options)
{
    ...
    gradleProperties["bobuiAndroidDir"] =
        (options.bobuiInstallDirectory + u'/' + options.bobuiDataDirectory +
         "/src/android/java"_L1)
            .toUtf8();
    // The following property "bobui5AndroidDir" is only for compatibility.
    // Projects using a custom build.gradle file may use this variable.
    // ### BobUI7: Remove the following line
    gradleProperties["bobui5AndroidDir"] =
        (options.bobuiInstallDirectory + u'/' + options.bobuiDataDirectory +
         "/src/android/java"_L1)
            .toUtf8();
    ...
    gradleProperties["bobuiMinSdkVersion"] = options.minSdkVersion;
    gradleProperties["bobuiTargetSdkVersion"] = options.targetSdkVersion;
    ...
    gradleProperties["bobuiTargetAbiList"] = abiList.toLocal8Bit();// armeabi-v7a or arm64-v8a or ...
    gradleProperties["bobuiGradlePluginType"] = options.buildAar
                                           ? "com.android.library"
...
bool writeDependencyFile(const Options &options)
{
    ...
    if (options.copyDependenciesOnly) {
        // When androiddeploy BobUI is running in copyDependenciesOnly mode we need to use
...
int generateJavaQmlComponents(const Options &options)
{
    ...
    const auto getImportPaths = [options](const QString &buildPath, const QString &libName,
        ...
        QFile confRspFile("%1/.bobui/qml_imports/%2_conf.rsp"_L1.arg(buildPath, libName));
        if (!confRspFile.exists() || !confRspFile.open(QFile::ReadOnly))
            return false;
        BOBUIextStream rspStream(&confRspFile);
...
#else
#warning Generating BobUIQuickView Java Contents is not possible with missing QProcess feature.
        return QJsonObject();
#endif
    };
    ...
    const auto endBlock = [](BOBUIextStream &stream, int indentWidth = 0) {
        stream << QString(indentWidth, u' ') << "}\n";
    };

    const auto createHeaderBlock = [](BOBUIextStream &stream, const QString &javaPackage) {
        stream << "/* This file is autogenerated by androiddeploybobui. Do not edit */\n\n"
               << "package %1;\n\n"_L1.arg(javaPackage)
               << "import org.bobuiproject.bobui.android.BobUISignalListener;\n"
               << "import org.bobuiproject.bobui.android.BobUIQuickViewContent;\n\n";
    };

    const auto beginComponentBlock = [](BOBUIextStream &stream, const QString &libName,
        ...
        stream << indent
               << "public final class %1 extends BobUIQuickViewContent {\n"_L1
    ...
    const auto beginPropertyBlock = [firstCharToUpper](BOBUIextStream &stream,
        ...
        stream << indent
               ...
               << indent
               << "public int connect%1ChangeListener(BobUISignalListener<%2> signalListener) {\n"_L1
    ...
    const auto beginSignalBlock = [firstCharToUpper](BOBUIextStream &stream,
        ...
        if (parameters.size() <= 1) { // Generate a BobUISignalListener<T> API for this property/signal
            const QString typeName = !parameters.isEmpty()
                    ? parameters[0].toObject()["typeName"_L1].toString()
                    : "void"_L1;
            const QString javaTypeName = qmlToJavaType.value(typeName, "Object"_L1);
            stream << indent
                   << "public int connect%1Listener(BobUISignalListener<%2> signalListener) {\n"_L1
    ...
    const auto writeFunctionBlock = [](BOBUIextStream &stream, const QJsonObject &methodData,
                                       int indentWidth = 8) {
        const QString indent(indentWidth, u' ');
    ...
    for (const auto &importPath : appImports) {
        ...
        if (moduleInfo.moduleName == libName) {
            fprintf(stderr,
                    ...
                    "with BOBUI_ANDROID_GENERATE_JAVA_BOBUIQUICKVIEW_CONTENTS flag.\n",
        ...
        for (const auto &qmlComponent : moduleInfo.qmlComponents) {
            ...
            BOBUIextStream outputStream(&componentClassBody, BOBUIextStream::ReadWrite);
...
int main(int argc, char *argv[])
{
    ...
    for (auto it = options.architectures.constBegin(); it != options.architectures.constEnd(); ++it) {
        if (!it->enabled)
            continue;
        options.setCurrentBobUIArchitecture(it.key(),
                                         it.value().bobuiInstallDirectory,
                                         it.value().bobuiDirectories);
        ...
        if (!copyBobUIFiles(&options))
            return CannotCopyBobUIFiles;

        if (Q_UNLIKELY(options.timing))
            fprintf(stdout, "[TIMING] %lld ns: Copied BobUI files\n", options.timer.nsecsElapsed());
        ...
        if (Q_UNLIKELY(options.timing)) {
            fprintf(stdout, "[TIMING] %lld ns: Generate Java BobUIQuickViewContents.\n",
                    options.timer.nsecsElapsed());
        }
        ...
        if (Q_UNLIKELY(options.timing))
            fprintf(stdout, "[TIMING] %lld ns: Bundled BobUI libs\n", options.timer.nsecsElapsed());
    }
