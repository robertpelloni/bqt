...
/*
 * The tool generates deployment artifacts for the BobUI builds such as:
 ...
 *     - Aliases or copies of the header files sorted by the generic BobUI-types: public/private/qpa
...
enum HeaderChecks {
    NoChecks = 0,
    NamespaceChecks = 1, /* Checks if header file is wrapped with BOBUI_<BEGIN|END>_NAMESPACE macros */
...
{
    std::cerr << "Internal error. Please create bugreport at https://bugreports.bobui.io "
...
public:
    ...
    bool isNonBobUIModule() const { return m_isNonBobUIModule; }
    ...
    {
        std::cout << "Usage: syncbobui -sourceDir <dir> -binaryDir <dir> -module <module name>"
                     ...
                     " [-nonBobUI] [-internal] [-copy]\n"
                     ""
                     "Mandatory arguments:\n"
                     "  -module                         Module name.\n"
                     "  -headers                        List of header files.\n"
                     "  -all                            In 'all' mode syncbobui scans source\n"
                     "                                  directory for public bobui headers and\n"
                     ...
                     "  -knownModules                   list of known modules. syncbobui uses the\n"
                     "                                  list to check the #include macros\n"
                     "                                  consistency.\n"
                     "Optional arguments:\n"
                     "  -internal                       Indicates that the module is internal.\n"
                     "  -nonBobUI                          Indicates that the module is not a BobUI\n"
                     "                                  module.\n"
...
private:
    ...
    [[nodiscard]] bool parseArguments(int argc, char *argv[])
    {
        ...
        const std::unordered_map<std::string, CommandLineOption<bool>> boolArgumentMap = {
            { "-nonBobUI", { &m_isNonBobUIModule, true } }, { "-debug", { &m_debug, true } },
...
class SyncScanner
{
    ...
    public:
        ...
        enum SourceType {
            Pragma = 0, // pragma bobui_class is mentioned a header file
...
public:
    ...
    ErrorCodes sync()
    {
        ...
        if (m_commandLineArgs->scanAllMode()) {
            ...
            // Since the list of header file is quite big syncbobui supports response files to avoid
        ...
        for (auto it = m_symbols.begin(); it != m_symbols.end(); ++it) {
            ...
            if (!filename.empty()) {
                if (generateBobUICamelCaseFileIfContentChanged(
        ...
        if (!m_commandLineArgs->isNonBobUIModule()) {
    ...
    [[nodiscard]] bool processHeader(const std::filesystem::path &headerFile)
    {
        ...
        // either header files are generated later than syncbobui is running or header files only
        ...
        if (!headerFileExists) {
            scannerDebug() << "Header file: " << headerFile
                           << " doesn't exist, but is added to syncbobui scanning. Skipping.";
            return true;
        }
        ...
        // since full path to the BobUI sources might contain '/3rdparty/' too.
        bool is3rdParty = std::regex_match(
                std::filesystem::relative(headerFile, m_commandLineArgs->sourceDir())
                        .generic_string(),
                ThirdPartyFolderRegex);

        // No processing of generated BobUI config header files.
        if (!std::regex_match(m_currentFilename, ConfigHeaderRegex)) {
            ...
            if (m_commandLineArgs->isNonBobUIModule() || is3rdParty || isQpa || isRhi || isSsg
            ...
            // Add the '#if BOBUI_CONFIG(<feature>)' check for header files that supposed to be
            ...
            // Hardcode generating of BobUIConfig alias
            updateSymbolDescriptor("BobUIConfig", "qconfig.h", SyncScanner::SymbolDescriptor::Pragma);
        }
                                   ...
                                   unsigned int skipChecks)
    {
        ...
        //
        //    - 'bobui_sync_skip_header_check' avoid any header checks.
        //
        //    - 'bobui_sync_stop_processing' stops the header proccesing from a moment when pragma is
        //      found. Important note: All the parsing artifacts were found before this point are
        //      stored for further processing.
        //
        //    - 'bobui_sync_suspend_processing' pauses processing and skip lines inside a header until
        //      'bobui_sync_resume_processing' is found. 'bobui_sync_stop_processing' stops processing if
        //      it's found before the 'bobui_sync_resume_processing'.
        //
        //    - 'bobui_sync_resume_processing' resumes processing after 'bobui_sync_suspend_processing'.
        //
        //    - 'bobui_class(<symbol>)' manually declares the 'symbol' that should be used to generate
        //      the CaMeL case header alias.
        //
        //    - 'bobui_deprecates([module/]<deprecated header file>[,<major.minor>])' indicates that
        //      this header file replaces the 'deprecated header file'. syncbobui will create the
        //      deprecated header file' with the special deprecation content. Pragma optionally
        //      accepts the BobUI version where file should be removed. If the current BobUI version is
        //      higher than the deprecation version, syncbobui displays deprecation warning and skips
        //      generating the deprecated header. If the module is specified and is different from
        //      the one this header file belongs to, syncbobui attempts to generate header files
        ...
        //
        //    - 'bobui_no_master_include' indicates that syncbobui should avoid including this header
        //      files into the module master header file.
        static const std::regex OnceRegex(R"(^#\s*pragma\s+once$)");
        static const std::regex SkipHeaderCheckRegex("^#\\s*pragma bobui_sync_skip_header_check$");
        static const std::regex StopProcessingRegex("^#\\s*pragma bobui_sync_stop_processing$");
        static const std::regex SuspendProcessingRegex("^#\\s*pragma bobui_sync_suspend_processing$");
        static const std::regex ResumeProcessingRegex("^#\\s*pragma bobui_sync_resume_processing$");
        static const std::regex ExplixitClassPragmaRegex("^#\\s*pragma bobui_class\\(([^\\)]+)\\)$");
        static const std::regex DeprecatesPragmaRegex("^#\\s*pragma bobui_deprecates\\(([^\\)]+)\\)$");
        static const std::regex NoMasterIncludePragmaRegex("^#\\s*pragma bobui_no_master_include$");
        ...
        // The regex's check if the content of header files is wrapped with the BobUI namespace macros.
        static const std::regex BeginNamespaceRegex("^BOBUI_BEGIN_NAMESPACE(_[A-Z_]+)?$");
        static const std::regex EndNamespaceRegex("^BOBUI_END_NAMESPACE(_[A-Z_]+)?$");
        ...
        // This regex checks if line contains the BobUI iterator declaration, that need to have
        // CaMel case header alias.
        static const std::regex DeclareIteratorRegex("^ *Q_DECLARE_\\w*ITERATOR\\((\\w+)\\);?$");

        // This regex checks if header file contains the BOBUI_REQUIRE_CONFIG call.
        // The macro argument is used to wrap an include of the header file inside the module master
        // header file with the '#if BOBUI_CONFIG(<feature>)' guard.
        static const std::regex RequireConfigRegex("^ *BOBUI_REQUIRE_CONFIG\\((\\w+)\\);?$");
        ...
        bool hasBobUIBeginNamespace = false;
        std::string bobuiBeginNamespace;
        std::string bobuiEndNamespace;
        ...
        while (std::getline(input, tmpLine)) {
                ...
                if (std::regex_match(buffer, SkipHeaderCheckRegex)) {
                    ...
                    if (!(skipChecks & PragmaOnceChecks)) {
                        ...
                        error() << "\"#pragma once\" is not allowed in installed header files: "
                                   "https://lists.bobui-project.org/pipermail/development/2022-October/043121.html"
                                << std::endl;
                    }
                    ...
                    if (!(skipChecks & IncludeChecks)) {
                        ...
                        for (const auto &module : m_commandLineArgs->knownModules()) {
                            std::string suggestedHeader = "BobUI" + module + '/' + includedHeader;
            ...
            // Logic below this line is affected by the 'bobui_sync_suspend_processing' and
            // 'bobui_sync_resume_processing' pragmas.
            if (isSuspended)
                continue;
            ...
            // Check for both BOBUI_BEGIN_NAMESPACE and BOBUI_END_NAMESPACE macros are present in the
            // header file.
            if (!(skipChecks & NamespaceChecks)) {
                if (std::regex_match(buffer, match, BeginNamespaceRegex)) {
                    bobuiBeginNamespace = match[1].str();
                    hasBobUIBeginNamespace = true;
                } else if (std::regex_match(buffer, match, EndNamespaceRegex)) {
                    bobuiEndNamespace = match[1].str();
                }
            }
        }
        input.close();
        ...
        if (!(skipChecks & NamespaceChecks)) {
            if (hasBobUIBeginNamespace) {
                if (bobuiBeginNamespace != bobuiEndNamespace) {
                    faults |= NamespaceChecks;
                    std::cerr << m_warningMessagePreamble << m_currentFileString
                              << " the begin namespace macro BOBUI_BEGIN_NAMESPACE" << bobuiBeginNamespace
                              << " doesn't match the end namespace macro BOBUI_END_NAMESPACE"
                              << bobuiEndNamespace << std::endl;
                }
            } else {
                faults |= NamespaceChecks;
                std::cerr << m_warningMessagePreamble << m_currentFileString
                          << " does not include BOBUI_BEGIN_NAMESPACE" << std::endl;
            }
        }
    ...
    [[nodiscard]] bool checkLineForSymbols(const std::string &line, std::string &symbol)
    {
        ...
        //     - class ClassName [BOBUI_TEXT_STREAM_FINAL|Q_DECL_FINAL|final|sealed]
    ...
    [[nodiscard]] bool generateBobUICamelCaseFileIfContentChanged(const std::string &outputFilePath,
    ...
    [[nodiscard]] bool generateMasterHeader()
    {
        ...
        buffer << "#ifndef BOBUI_" << moduleUpper << "_MODULE_H\n"
               << "#define BOBUI_" << moduleUpper << "_MODULE_H\n"
        ...
        for (const auto &headerContents : m_masterHeaderContents) {
            if (!headerContents.second.empty()) {
                buffer << "#if BOBUI_CONFIG(" << headerContents.second << ")\n"
                       << "#include <" << m_commandLineArgs->moduleName() << "/"
    ...
    [[nodiscard]] bool generateVersionHeader(const std::string &outputFile)
    {
        ...
        std::stringstream buffer;
        buffer << "/* This file was generated by syncbobui. */\n"
               << "#ifndef BOBUI_" << moduleNameUpper << "_VERSION_H\n"
               << "#define BOBUI_" << moduleNameUpper << "_VERSION_H\n\n"
               << "#define " << moduleNameUpper << "_VERSION_STR \"" << BOBUI_VERSION_STR << "\"\n\n"
               << "#define " << moduleNameUpper << "_VERSION "
               << "0x0" << BOBUI_VERSION_MAJOR << "0" << BOBUI_VERSION_MINOR << "0" << BOBUI_VERSION_PATCH
               << "\n\n"
               << "#endif // BOBUI_" << moduleNameUpper << "_VERSION_H\n";
    ...
    [[nodiscard]] bool generateDeprecatedHeaders()
    {
        ...
        for (auto it = m_deprecatedHeaders.begin(); it != m_deprecatedHeaders.end(); ++it) {
            ...
            if (separatorPos != std::string::npos) {
                ...
                versionDisclaimer = " and will be removed in BobUI " + version;
                int minor = 0;
                int major = 0;
                ...
                if (BOBUI_VERSION_MAJOR > major
                    || (BOBUI_VERSION_MAJOR == major && BOBUI_VERSION_MINOR >= minor)) {
                    std::cerr << WarningMessagePreamble << headerPath
                              << " is marked as deprecated and will not be generated in BobUI "
                              << BOBUI_VERSION_STR
                              << ". The respective bobui_deprecates pragma needs to be removed.\n";
                    continue;
                }
            }
            ...
            if (isCrossModuleDeprecation) {
                const std::string stagingDir = outputDir + "/.syncbobui_staging/";
...
// The function generates BobUI CaMeL-case files.
...
// is changed.
bool SyncScanner::generateBobUICamelCaseFileIfContentChanged(const std::string &outputFilePath,
                                                          const std::string &aliasedFilePath)
{
    if (m_commandLineArgs->showOnly())
        return true;
