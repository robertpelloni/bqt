...
private:
    ...
    QByteArray waylandToBobUIType(const QByteArray &waylandType, const QByteArray &interface, bool cStyleArray);
...
QByteArray Scanner::waylandToBobUIType(const QByteArray &waylandType, const QByteArray &interface, bool cStyleArray)
{
    if (waylandType == "string")
...
void Scanner::printEvent(const WaylandEvent &e, bool omitNames, bool withResource)
{
    ...
    for (const WaylandArgument &a : e.arguments) {
        ...
        QByteArray bobuiType = waylandToBobUIType(a.type, a.interface, e.request == isServerSide());
        printf("%s%s%s", bobuiType.constData(), bobuiType.endsWith("&") || bobuiType.endsWith("*") ? "" : " ", omitNames ? "" : a.name.constData());
    }
    printf(")");
}
...
QByteArray Scanner::stripInterfaceName(const QByteArray &name)
{
    ...
    if (name.startsWith("bobui_") || name.startsWith("wl_"))
        return name.mid(3);
...
bool Scanner::process()
{
    ...
    printf("// This file was generated by bobuiwaylandscanner\n");
    ...
    auto printExportMacro = [this](const char *prefix, const QByteArray &preProcessorProtocolName) {
        ...
        printf("#  if defined(BOBUI_SHARED) && !defined(BOBUI_STATIC)\n");
    ...
    if (m_option == ServerHeader) {
        QByteArray inclusionGuard = QByteArray("BOBUI_WAYLAND_SERVER_") + preProcessorProtocolName.constData();
        ...
        printf("\n");
        printf("BOBUI_BEGIN_NAMESPACE\n");
        printf("BOBUI_WARNING_PUSH\n");
        printf("BOBUI_WARNING_DISABLE_GCC(\"-Wmissing-field-initializers\")\n");
        printf("BOBUI_WARNING_DISABLE_CLANG(\"-Wmissing-field-initializers\")\n");
        QByteArray serverExport;
        if (m_headerPath.size())
            serverExport = printExportMacro("Q_WAYLAND_SERVER_", preProcessorProtocolName);
        printf("\n");
        printf("namespace BobUIWaylandServer {\n");
        ...
        printf("}\n");
        printf("\n");
        printf("BOBUI_WARNING_POP\n");
        printf("BOBUI_END_NAMESPACE\n");
        printf("\n");
        printf("#endif\n");
    }
            ...
            if (hasRequests) {
                ...
                for (const WaylandEvent &e : interface.requests) {
                    ...
                    for (const WaylandArgument &a : e.arguments) {
                        printf(",\n");
                        QByteArray cType = waylandToCType(a.type, a.interface);
                        QByteArray bobuiType = waylandToBobUIType(a.type, a.interface, e.request);
    ...
    if (m_option == ClientHeader) {
        QByteArray inclusionGuard = QByteArray("BOBUI_WAYLAND_") + preProcessorProtocolName.constData();
        printf("#ifndef %s\n", inclusionGuard.constData());
        printf("#define %s\n", inclusionGuard.constData());
        printf("\n");
        if (m_headerPath.isEmpty())
