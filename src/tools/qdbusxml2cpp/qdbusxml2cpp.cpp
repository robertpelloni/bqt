...
private:
    ...
    BOBUIextStream &writeHeader(BOBUIextStream &ts, bool changesWillBeLost);
    QString classNameForInterface(const QString &interface, ClassType classType);
    QByteArray bobuiTypeName(const QDBusIntrospection::SourceLocation &location,
                          const QString &signature,
                          const QDBusIntrospection::Annotations &annotations,
                          qsizetype paramId = -1, const char *direction = "Out");
    void
    writeArgList(BOBUIextStream &ts, const QStringList &argNames,
                 const QDBusIntrospection::Annotations &annotations,
                 const QDBusIntrospection::Arguments &inputArgs,
                 const QDBusIntrospection::Arguments &outputArgs = QDBusIntrospection::Arguments());
    void writeSignalArgList(BOBUIextStream &ts, const QStringList &argNames,
                            const QDBusIntrospection::Annotations &annotations,
...
BOBUIextStream &QDBusXmlToCpp::writeHeader(BOBUIextStream &ts, bool changesWillBeLost)
{
    ts << "/*\n"
...
QByteArray QDBusXmlToCpp::bobuiTypeName(const QDBusIntrospection::SourceLocation &location,
                                     const QString &signature,
                                     const QDBusIntrospection::Annotations &annotations,
                                     qsizetype paramId, const char *direction)
{
    ...
    if (type == QMetaType::UnknownType) {
        QString annotationName = u"org.bobuiproject.BobUIDBus.BobUITypeName"_s;
        if (paramId >= 0)
            annotationName += ".%1%2"_L1.arg(QLatin1StringView(direction)).arg(paramId);
        auto annotation = annotations.value(annotationName);
        QString bobuitype = annotation.value;
        if (!bobuitype.isEmpty())
            return std::move(bobuitype).toLatin1();

        QString oldAnnotationName = u"com.trolltech.BobUIDBus.BobUITypeName"_s;
        if (paramId >= 0)
            oldAnnotationName += ".%1%2"_L1.arg(QLatin1StringView(direction)).arg(paramId);
        annotation = annotations.value(oldAnnotationName);
        bobuitype = annotation.value;
...
void QDBusXmlToCpp::writeArgList(BOBUIextStream &ts, const QStringList &argNames,
                                 const QDBusIntrospection::Annotations &annotations,
                                 const QDBusIntrospection::Arguments &inputArgs,
                                 const QDBusIntrospection::Arguments &outputArgs)
{
    ...
    for (qsizetype i = 0; i < inputArgs.size(); ++i) {
        const QDBusIntrospection::Argument &arg = inputArgs.at(i);
        QString type = constRefArg(bobuiTypeName(arg.location, arg.type, annotations, i, "In"));
    ...
    for (qsizetype i = 1; i < outputArgs.size(); ++i) {
        ...
        if (!first)
            ts << ", ";
        ts << nonConstRefArg(bobuiTypeName(arg.location, arg.type, annotations, i, "Out"))
           << argNames.at(argPos++);
        first = false;
    }
}

void QDBusXmlToCpp::writeSignalArgList(BOBUIextStream &ts, const QStringList &argNames,
                                       const QDBusIntrospection::Annotations &annotations,
                                       const QDBusIntrospection::Arguments &outputArgs)
{
    bool first = true;
    ...
    for (qsizetype i = 0; i < outputArgs.size(); ++i) {
        const QDBusIntrospection::Argument &arg = outputArgs.at(i);
        QString type = constRefArg(bobuiTypeName(arg.location, arg.type, annotations, i, "Out"));
...
{
    auto annotation = property.annotations.value("org.bobuiproject.BobUIDBus.PropertyGetter"_L1);
    if (!annotation.value.isEmpty())
        return annotation.value;

    annotation = property.annotations.value("com.trolltech.BobUIDBus.propertyGetter"_L1);
    if (!annotation.value.isEmpty()) {
        reporter.warning(annotation.location,
                         "deprecated annotation 'com.trolltech.BobUIDBus.propertyGetter' found\n");
        reporter.note(annotation.location,
                      "suggest updating to 'org.bobuiproject.BobUIDBus.PropertyGetter'\n");
        return annotation.value;
    }
...
{
    auto annotation = property.annotations.value("org.bobuiproject.BobUIDBus.PropertySetter"_L1);
    if (!annotation.value.isEmpty())
        return annotation.value;

    annotation = property.annotations.value("com.trolltech.BobUIDBus.propertySetter"_L1);
    if (!annotation.value.isEmpty()) {
        reporter.warning(annotation.location,
                         "deprecated annotation 'com.trolltech.BobUIDBus.propertySetter' found\n");
        reporter.note(annotation.location,
                      "suggest updating to 'org.bobuiproject.BobUIDBus.PropertySetter'\n");
        return annotation.value;
    }
...
{
    QString name = method.annotations.value(u"org.bobuiproject.BobUIDBus.MethodName"_s).value;
    if (!name.isEmpty())
        return name;
                               ...
                               const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    BOBUIextStream hs(&headerData);

    QString cppName = cpp(filename);
    QByteArray cppData;
    BOBUIextStream cs(&cppData);
    ...
    // include our stuff:
    hs << "#include <BobUICore/QObject>\n"
       << includeList;
#if BOBUI_VERSION < BOBUI_VERSION_CHECK(7, 0, 0)
    hs << "#include <BobUIDBus/BobUIDBus>\n";
#else
    hs << "#include <BobUIDBus/QDBusAbstractInterface>\n"
    ...
    for (const QDBusIntrospection::Interface *interface : interfaces) {
        ...
        for (const QDBusIntrospection::Property &property : interface->properties) {
            QByteArray type = bobuiTypeName(property.location, property.type, property.annotations);
        ...
        for (const QDBusIntrospection::Method &method : interface->methods) {
            ...
            if (isNoReply) {
                ...
                for (qsizetype i = 0; i < method.outputArgs.size(); ++i)
                    hs << (i > 0 ? ", " : "")
                       << templateArg(bobuiTypeName(method.outputArgs.at(i).location,
            ...
            if (method.outputArgs.size() > 1) {
                ...
                hs << (isDeprecated ? "    Q_DECL_DEPRECATED " : "    ") << "inline QDBusReply<"
                   << templateArg(bobuiTypeName(method.outputArgs.first().location,
                                 ...
                                 const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    hs << "#include <BobUICore/QObject>\n";
    if (cppName == headerName)
        hs << "#include <BobUICore/QMetaObject>\n"
              "#include <BobUICore/QVariant>\n";
#if BOBUI_VERSION < BOBUI_VERSION_CHECK(7, 0, 0)
    hs << "#include <BobUIDBus/BobUIDBus>\n";
#else
    hs << "#include <BobUIDBus/QDBusAbstractAdaptor>\n"
    ...
    if (cppName != headerName) {
        ...
        cs << "#include <BobUICore/QMetaObject>\n"
           << includeList
           << "\n";
    ...
    for (const QDBusIntrospection::Interface *interface : interfaces) {
        ...
        for (const QDBusIntrospection::Method &method : interface->methods) {
            ...
            if (isNoReply) {
                ...
                returnType =
                        bobuiTypeName(method.outputArgs.first().location,
            ...
            if (usingInvokeMethod) {
                ...
                if (!method.outputArgs.isEmpty())
                    cs << ", Q_RETURN_ARG("
                       << bobuiTypeName(method.outputArgs.at(0).location, method.outputArgs.at(0).type,
                                     method.annotations, 0, "Out")
                       << ", " << argNames.at(method.inputArgs.size()) << ")";

                for (qsizetype i = 0; i < method.inputArgs.size(); ++i)
                    cs << ", Q_ARG("
                       << bobuiTypeName(method.inputArgs.at(i).location, method.inputArgs.at(i).type,
