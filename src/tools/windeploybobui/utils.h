...
QMap<QString, QString> queryBobUIPaths(const QString &qmakeBinary, QString *errorMessage);
                ...
                QString *errorMessage)
{
    ...
    if (!(flags & SkipUpdateFile)) {
        ...
        if (!(file.permissions() & QFile::WriteUser)) { // BOBUIBUG-40152, clear inherited read-only attribute
            QFile targetFile(targetFileName);
