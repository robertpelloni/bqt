...
void PluginInformation::generateAvailablePlugins(const QMap<QString, QString> &bobuiPathsVariables,
                                                 const Platform &platform)
{
    const QDir pluginTypesDir(bobuiPathsVariables.value(QLatin1String("BOBUI_INSTALL_PLUGINS")));
    const QDir platformPluginDir(pluginTypesDir.absolutePath() + QStringLiteral("/platforms"));
    const QString infix(bobuiPathsVariables.value(QLatin1String(qmakeInfixKey)));
