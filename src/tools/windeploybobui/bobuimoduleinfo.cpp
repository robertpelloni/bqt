...
static BobUIModule moduleFromJsonFile(const QString &filePath, QString *errorString)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        *errorString = QLatin1String("Cannot open ") + file.fileName();
    ...
    BobUIModule module;
    module.name = "BobUI6"_L1 + obj[QLatin1String("name")].toString();
...
static void dump(const BobUIModule &module)
{
...
bool BobUIModuleInfoStore::populate(const QString &modulesDir, const QString &translationsDir,
                                 bool verbose, QString *errorString)
{
    ...
    for (const auto &dirEntry : QDirListing(modulesDir, {u"*.json"_s}, F::FilesOnly)) {
        BobUIModule module = moduleFromJsonFile(dirEntry.filePath(), errorString);
        if (!errorString->isEmpty())
            return false;
        if (module.internal && module.name.endsWith(QStringLiteral("Private")))
            module.name.chop(7);
        module.id = modules.size();
        if (module.id == BobUIModule::InvalidId) {
...
const BobUIModule &BobUIModuleInfoStore::moduleById(size_t id) const
{
    return modules.at(id);
}

size_t BobUIModuleInfoStore::moduleIdForPluginType(const QString &pluginType) const
{
    auto moduleHasPluginType = [&pluginType] (const BobUIModule &module) {
