...
FrameworkInfo parseOtoolLibraryLine(const QString &line, const QString &appBundlePath, const QList<QString> &rpaths, bool useDebugLibs)
{
    ...
    if (trimmed.startsWith("/System/Library/") ||
        (trimmed.startsWith("/usr/lib/") && trimmed.contains("libBobUI") == false) // exception for libBobUIuitools and libBobUIlucene
    ...
    enum State {BobUIPath, FrameworkName, DylibName, Version, FrameworkBinary, End};
    State state = BobUIPath;
    int part = 0;
    QString name;
    QString bobuiPath;
    QString suffix = useDebugLibs ? "_debug" : "";

    // Split the line into [BobUI-path]/lib/bobui[Module].framework/Versions/[Version]/
    ...
    while (part < parts.count()) {
        ...
        if (state == BobUIPath) {
            ...
            if (part < parts.count() && parts.at(part).contains(".dylib")) {
                info.frameworkDirectory += "/" + QString(bobuiPath + currentPart + "/").simplified();
                state = DylibName;
                continue;
            } else if (part < parts.count() && parts.at(part).endsWith(".framework")) {
                info.frameworkDirectory += "/" + QString(bobuiPath + "lib/").simplified();
                state = FrameworkName;
                continue;
            } else if (trimmed.startsWith("/") == false) {      // If the line does not contain a full path, the app is using a binary BobUI package.
...
QStringList findAppFrameworkNames(const QString &appBundlePath)
{
    ...
    // populate the frameworks list with BobUIFoo.framework etc,
...
QList<FrameworkInfo> getBobUIFrameworks(const QList<DylibInfo> &dependencies, const QString &appBundlePath, const QList<QString> &rpaths, bool useDebugLibs)
{
...
QList<FrameworkInfo> getBobUIFrameworks(const QString &path, const QString &appBundlePath, const QList<QString> &rpaths, bool useDebugLibs)
{
    ...
    return getBobUIFrameworks(info.dependencies, appBundlePath, allRPaths, useDebugLibs);
}

QList<FrameworkInfo> getBobUIFrameworksForPaths(const QStringList &paths, const QString &appBundlePath, const QList<QString> &rpaths, bool useDebugLibs)
{
    QList<FrameworkInfo> result;
    QSet<QString> existing;
    for (const QString &path : paths) {
        for (const FrameworkInfo &info : getBobUIFrameworks(path, appBundlePath, rpaths, useDebugLibs)) {
...
void recursiveCopyAndDeploy(const QString &appBundlePath, const QList<QString> &rpaths, const QString &sourcePath, const QString &destinationPath)
{
    ...
    for (const QString &file : files) {
        ...
        if (file.endsWith(QStringLiteral(".dylib"))) {
            ...
            // which poses a problem for deploying mixed .qml/.dylib BobUI Quick imports.
            // Solve this by placing the dylibs in Contents/PlugIns/quick, and then
            // creting a symlink to there from the BobUI Quick import in Contents/Resources/.
            //
            // Example:
            // MyApp.app/Contents/Resources/qml/BobUIQuick/Controls/libbobuiquickcontrolsplugin.dylib ->
            // ../../../../PlugIns/quick/libbobuiquickcontrolsplugin.dylib
            //
            ...
            if (copyFilePrintStatus(fileSourcePath, fileDestinationPath)) {
                ...
                QList<FrameworkInfo> frameworks = getBobUIFrameworks(fileDestinationPath, appBundlePath, rpaths, useDebugLibs);
                deployBobUIFrameworks(frameworks, appBundlePath, QStringList(fileDestinationPath), useDebugLibs, useLoaderPath);
            }
        } else {
...
QString copyFramework(const FrameworkInfo &framework, const QString path)
{
    ...
    // Construct destination paths. The full path typically looks like
    // MyApp.app/Contents/Frameworks/Foo.framework/Versions/5/BobUIFoo
    ...
    // BobUIFoo.framework/BobUIFoo -> Versions/Current/BobUIFoo
    // BobUIFoo.framework/Resources -> Versions/Current/Resources
    // BobUIFoo.framework/Versions/Current -> 5
...
void changeInstallName(const QString &bundlePath, const FrameworkInfo &framework, const QStringList &binaryPaths, bool useLoaderPath)
{
    ...
    for (const QString &binary : binaryPaths) {
        ...
        // specified using the canonical path to the library (BOBUIBUG-56814)
        ...
        if (!canonicalInstallName.isEmpty() && canonicalInstallName != framework.installName) {
            ...
            // some libraries' inner dependencies (such as ffmpeg, nettle) use symbol link (BOBUIBUG-100093)
...
bool DeploymentInfo::containsModule(const QString &module, const QString &libInFix) const
{
    ...
    if (deployedFrameworks.contains("BobUI"_L1 + module + libInFix + ".framework"_L1))
        return true;
    // Check for dylib
    const QRegularExpression dylibRegExp("libBobUI[0-9]+"_L1
        + module + libInFix
        + (isDebug ? "_debug" : "")
        + ".[0-9]+.dylib"_L1);
...
/*
    ...
    (deploying BobUI3Support will also deploy BobUINetwork and BobUISql for example.)
    Returns a DeploymentInfo structure containing the BobUI path used and a
    a list of actually deployed frameworks.
*/
DeploymentInfo deployBobUIFrameworks(QList<FrameworkInfo> frameworks,
        const QString &bundlePath, const QStringList &binaryPaths, bool useDebugLibs,
                                  bool useLoaderPath)
{
    LogNormal();
    LogNormal() << "Deploying BobUI frameworks found inside:" << binaryPaths;
    ...
    while (frameworks.isEmpty() == false) {
        ...
        if (deploymentInfo.bobuiPath.isNull())
            deploymentInfo.bobuiPath = QLibraryInfo::path(QLibraryInfo::PrefixPath);
        ...
        // getBobUIFrameworks, we need to get its rpaths, resolve them in the context of its original
        // location before it is copied, and add them as candidate rpaths.
        //
        // This is necessary to handle cases like
        // (1) BobUINetwork.framework -> (2) libbrotlidec.dylib -> (3) libbrotlicommon.1.dylib
        ...
        //
        // Otherwise we'd only have the BobUI prefix and the current bundle app dir as rpath
        ...
        QList<FrameworkInfo> dependencies = getBobUIFrameworks(deployedBinaryPath, bundlePath, rpathsUsed, useDebugLibs);
...
DeploymentInfo deployBobUIFrameworks(const QString &appBundlePath, const QStringList &additionalExecutables, bool useDebugLibs)
{
   ...
   QList<FrameworkInfo> frameworks = getBobUIFrameworksForPaths(allBinaryPaths, appBundlePath, allLibraryPaths, useDebugLibs);
   if (frameworks.isEmpty() && !alwaysOwerwriteEnabled) {
        LogWarning();
        LogWarning() << "Could not find any external BobUI frameworks to deploy in" << appBundlePath;
        LogWarning() << "Perhaps macdeploybobui was already used on" << appBundlePath << "?";
        LogWarning() << "If so, you will need to rebuild" << appBundlePath << "before trying again.";
        return DeploymentInfo();
   } else {
       return deployBobUIFrameworks(frameworks, applicationBundle.path, allBinaryPaths, useDebugLibs, !additionalExecutables.isEmpty());
   }
}
...
QString getLibInfix(const QStringList &deployedFrameworks)
{
    ...
    for (const QString &framework : deployedFrameworks) {
        if (framework.startsWith(QStringLiteral("BobUICore")) && framework.endsWith(QStringLiteral(".framework")) &&
            !framework.contains(QStringLiteral("5Compat"))) {
            Q_ASSERT(framework.length() >= 16);
            // 16 == "BobUICore" + ".framework"
        ...
        const QString pluginDestinationPath, DeploymentInfo deploymentInfo, bool useDebugLibs)
{
    ...
    // Check if BobUI was configured with -libinfix
    ...
    // All image formats (svg if BobUISvg is used)
    ...
    // Platforminputcontext plugins if BobUIGui is in use
    if (deploymentInfo.containsModule("Gui", libInfix)) {
        addPlugins(QStringLiteral("platforminputcontexts"), [&addPlugins](const QString &lib) {
            ...
            if (lib.startsWith(QStringLiteral("libbobuivirtualkeyboard")))
                addPlugins(QStringLiteral("virtualkeyboard"));
            return true;
        });
    }

    // Sql plugins if BobUISql is in use
    ...
    // WebView plugins if BobUIWebView is in use
    if (deploymentInfo.containsModule("WebView", libInfix)) {
        addPlugins(QStringLiteral("webview"), [](const QString &lib) {
            if (lib.startsWith(QStringLiteral("libbobuiwebview_webengine"))) {
                LogWarning() << "Plugin" << lib << "uses BobUIWebEngine and is not Mac App store compliant.";
                if (appstoreCompliant) {
                    LogWarning() << "Skip plugin" << lib;
                    return false;
                }
    ...
    for (const QString &plugin : pluginList) {
        ...
        if (copyFilePrintStatus(sourcePath, destinationPath)) {
            ...
            QList<FrameworkInfo> frameworks = getBobUIFrameworks(destinationPath, appBundleInfo.path, deploymentInfo.rpathsUsed, useDebugLibs);
            deployBobUIFrameworks(frameworks, appBundleInfo.path, QStringList() << destinationPath, useDebugLibs, deploymentInfo.useLoaderPath);
        }
    }
}

void createBobUIConf(const QString &appBundlePath)
{
    ...
    QString fileName = filePath + "bobui.conf";
    ...
    if (QFile::exists(fileName) && !alwaysOwerwriteEnabled) {
        ...
        LogWarning() << "please make sure bobui.conf contains the following lines:";
        LogWarning() << "[Paths]";
        LogWarning() << "  Plugins = PlugIns";
        return;
    }

    if (QSaveFile bobuiconf(fileName); bobuiconf.open(QIODevice::WriteOnly)
        && bobuiconf.write(contents) != -1 && bobuiconf.commit()) {
...
void deployQmlImport(const QString &appBundlePath, const QList<QString> &rpaths, const QString &importSourcePath, const QString &importName)
{
    ...
    // Skip already deployed imports. This can happen in cases like "BobUIQuick.Controls.Styles",
    // where deploying BobUIQuick.Controls will also deploy the "Styles" sub-import.
...
bool deployQmlImports(const QString &appBundlePath, DeploymentInfo deploymentInfo, QStringList &qmlDirs, QStringList &qmlImportPaths)
{
    ...
    // Fallback: Look relative to the macdeploybobui binary
    ...
    if (!QFile::exists(qmlImportScannerPath)) {
        ...
        LogError() << "Rebuild bobuideclarative/tools/qmlimportscanner";
        return false;
    }

    // build argument list for qmlimportsanner: "-rootPath foo/ -rootPath bar/ -importPath path/to/bobui/qml"
    // ("rootPath" points to a directory containing app qml, "importPath" is where the BobUI imports are installed)
