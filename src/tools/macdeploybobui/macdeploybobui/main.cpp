...
int main(int argc, char **argv)
{
    ...
    if (argc < 2 || appBundlePath.startsWith("-")) {
        qDebug() << "Usage: macdeploybobui app-bundle [options]";
        qDebug() << "";
        qDebug() << "Options:";
        ...
        qDebug() << "";
        qDebug() << "macdeploybobui takes an application bundle as input and makes it";
        qDebug() << "self-contained by copying in the BobUI frameworks and plugins that";
        ...
        qDebug() << "";
        qDebug() << "BobUI plugins may use private API and will cause the app to be";
        qDebug() << "rejected from the Mac App store. macdeploybobui will print a warning";
        ...
        qDebug() << "";
        qDebug() << "See the \"BobUI for macOS - Deployment\" topic in the";
    ...
    DeploymentInfo deploymentInfo = deployBobUIFrameworks(appBundlePath, additionalExecutables, useDebugLibs);
    ...
    if (plugins) {
        ...
        if (deploymentInfo.pluginPath.isEmpty()) {
            LogError() << "Missing BobUI plugins path\n";
            return 1;
        }
