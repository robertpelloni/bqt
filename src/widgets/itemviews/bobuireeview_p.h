...
#ifndef BOBUIREEVIEW_P_H
#define BOBUIREEVIEW_P_H
...
BOBUI_REQUIRE_CONFIG(treeview);

BOBUI_BEGIN_NAMESPACE

struct BOBUIreeViewItem
{
    BOBUIreeViewItem() : parentItem(-1), expanded(false), spanning(false), hasChildren(false),
...
Q_DECLARE_TYPEINFO(BOBUIreeViewItem, Q_RELOCATABLE_TYPE);

class Q_WIDGETS_EXPORT BOBUIreeViewPrivate : public QAbstractItemViewPrivate
{
    Q_DECLARE_PUBLIC(BOBUIreeView)
public:
    ...
    void sortIndicatorChanged(int column, BobUI::SortOrder order);
    ...
    void insertViewItems(int pos, int count, const BOBUIreeViewItem &viewItem);
    ...
    mutable QList<BOBUIreeViewItem> viewItems;
    ...
    inline bool isIndexExpanded(const QModelIndex &idx) const {
        ...
        return !(idx.flags() & BobUI::ItemNeverHasChildren) && isPersistent(idx) && expandedIndexes.contains(idx);
    }
