...
{
#ifdef DEBUG_SPAN_UPDATE
    qDebug() << start << end << BobUI::endl << index;
#endif
    if (spans.empty())
        return;
...
#if BOBUI_CONFIG(abstractbutton)
class BOBUIableCornerButton : public QAbstractButton
{
    Q_OBJECT
public:
    BOBUIableCornerButton(QWidget *parent) : QAbstractButton(parent) {
        setObjectName(QLatin1String("bobui_tableview_cornerbutton"));
    }
...
#if BOBUI_CONFIG(abstractbutton)
    cornerWidget = new BOBUIableCornerButton(q);
    cornerWidget->setFocusPolicy(BobUI::NoFocus);
    cornerWidgetConnection = QObject::connect(
          cornerWidget, &BOBUIableCornerButton::clicked,
          q, &BOBUIableView::selectAll);
#endif
}

void BOBUIableViewPrivate::clearConnections()
{
...
*/
void BOBUIableViewPrivate::trimHiddenSelections(QItemSelectionRange *range) const
{
...
QRect BOBUIableViewPrivate::intersectedRect(const QRect rect, const QModelIndex &topLeft, const QModelIndex &bottomRight) const
{
    Q_Q(const BOBUIableView);
...
*/
void BOBUIableViewPrivate::setSpan(int row, int column, int rowSpan, int columnSpan)
{
    if (Q_UNLIKELY(row < 0 || column < 0 || rowSpan <= 0 || columnSpan <= 0)) {
        qWarning("BOBUIableView::setSpan: invalid span given: (%d, %d, %d, %d)",
    ...
    if (sp) {
        if (sp->top() != row || sp->left() != column) {
            qWarning("BOBUIableView::setSpan: span cannot overlap");
            return;
        }
        ...
        qWarning("BOBUIableView::setSpan: single cell span won't be added");
        return;
    }
...
*/
QSpanCollection::Span BOBUIableViewPrivate::span(int row, int column) const
{
    QSpanCollection::Span *sp = spans.spanAt(column, row);
    if (sp)
        return *sp;
...
*/
int BOBUIableViewPrivate::sectionSpanEndLogical(const QHeaderView *header, int logical, int span) const
{
...
*/
int BOBUIableViewPrivate::sectionSpanSize(const QHeaderView *header, int logical, int span) const
{
...
*/
bool BOBUIableViewPrivate::spanContainsSection(const QHeaderView *header, int logical, int spanLogical, int span) const
{
...
*/
int BOBUIableViewPrivate::nextActiveVisualRow(int rowToStart, int column, int limit,
...
*/
int BOBUIableViewPrivate::nextActiveVisualColumn(int row, int columnToStart, int limit,
                                              SearchDirection searchDirection) const
{
...
*/
QRect BOBUIableViewPrivate::visualSpanRect(const QSpanCollection::Span &span) const
{
    Q_Q(const BOBUIableView);
    // vertical
    int row = span.top();
...
*/
void BOBUIableViewPrivate::drawAndClipSpans(const QRegion &area, QPainter *painter,
...
*/
void BOBUIableViewPrivate::updateSpanInsertedRows(const QModelIndex &parent, int start, int end)
{
    Q_UNUSED(parent);
...
*/
void BOBUIableViewPrivate::updateSpanInsertedColumns(const QModelIndex &parent, int start, int end)
{
    Q_UNUSED(parent);
...
*/
void BOBUIableViewPrivate::updateSpanRemovedRows(const QModelIndex &parent, int start, int end)
{
    Q_UNUSED(parent);
...
*/
void BOBUIableViewPrivate::updateSpanRemovedColumns(const QModelIndex &parent, int start, int end)
{
    Q_UNUSED(parent);
...
*/
void BOBUIableViewPrivate::sortIndicatorChanged(int column, BobUI::SortOrder order)
{
    model->sort(column, order);
}

QStyleOptionViewItem::ViewItemPosition BOBUIableViewPrivate::viewItemPosition(
...
/*!
  ...
  Draws a table cell.
*/
void BOBUIableViewPrivate::drawCell(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index)
{
    Q_Q(BOBUIableView);
    QStyleOptionViewItem opt = option;
    ...
    if (option.state & QStyle::State_Enabled) {
        ...
        if ((model->flags(index) & BobUI::ItemIsEnabled) == 0) {
...
*/
int BOBUIableViewPrivate::widthHintForIndex(const QModelIndex &index, int hint, const QStyleOptionViewItem &option) const
{
    Q_Q(const BOBUIableView);
...
*/
int BOBUIableViewPrivate::heightHintForIndex(const QModelIndex &index, int hint, QStyleOptionViewItem &option) const
{
    Q_Q(const BOBUIableView);
    QWidget *editor = editorForIndex(index).widget.data();
    if (editor && persistent.contains(editor)) {
...
/*!
    ...
    \brief The BOBUIableView class provides a default model/view
    ...
    A BOBUIableView implements a table view that displays items from a
    model. This class is used to provide standard tables that were
    previously provided by the BOBUIable class, but using the more
    flexible approach provided by BobUI's model/view architecture.

    The BOBUIableView class is one of the \l{Model/View Classes}
    and is part of BobUI's \l{Model/View Programming}{model/view framework}.

    BOBUIableView implements the interfaces defined by the
    ...
    mouse, or by using the arrow keys. Because BOBUIableView enables
    ...
    \table
    \row \li \inlineimage bobuiableview-resized.png
    ...
    \sa BOBUIableWidget, {View Classes}, QAbstractItemModel, QAbstractItemView,
        {Table Model Example}
*/
...
BOBUIableView::BOBUIableView(QWidget *parent)
    : QAbstractItemView(*new BOBUIableViewPrivate, parent)
{
    Q_D(BOBUIableView);
    d->init();
}
...
/*!
  \internal
*/
BOBUIableView::BOBUIableView(BOBUIableViewPrivate &dd, QWidget *parent)
...
/*!
  \reimp
*/
QSize BOBUIableView::viewportSizeHint() const
{
    Q_D(const BOBUIableView);
...
/*!
  \reimp
*/
void BOBUIableView::setModel(QAbstractItemModel *model)
{
    Q_D(BOBUIableView);
    if (model == d->model)
        return;
    ...
    if (model) { //and connect to the new one
        d->modelConnections = {
            QObjectPrivate::connect(model, &QAbstractItemModel::rowsInserted,
                                    d, &BOBUIableViewPrivate::updateSpanInsertedRows),
            QObjectPrivate::connect(model, &QAbstractItemModel::columnsInserted,
                                    d, &BOBUIableViewPrivate::updateSpanInsertedColumns),
            QObjectPrivate::connect(model, &QAbstractItemModel::rowsRemoved,
                                    d, &BOBUIableViewPrivate::updateSpanRemovedRows),
            QObjectPrivate::connect(model, &QAbstractItemModel::columnsRemoved,
                                    d, &BOBUIableViewPrivate::updateSpanRemovedColumns)
        };
    }
...
/*!
  \reimp
*/
void BOBUIableView::setRootIndex(const QModelIndex &index)
{
    Q_D(BOBUIableView);
    if (index == d->root) {
        viewport()->update();
...
/*!
  \internal
*/
void BOBUIableView::doItemsLayout()
{
    Q_D(BOBUIableView);
...
/*!
  \reimp
*/
void BOBUIableView::setSelectionModel(QItemSelectionModel *selectionModel)
{
    Q_D(BOBUIableView);
...
*/
QHeaderView *BOBUIableView::horizontalHeader() const
{
    Q_D(const BOBUIableView);
...
*/
QHeaderView *BOBUIableView::verticalHeader() const
{
    Q_D(const BOBUIableView);
...
*/
void BOBUIableView::setHorizontalHeader(QHeaderView *header)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::setVerticalHeader(QHeaderView *header)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::scrollContentsBy(int dx, int dy)
{
    Q_D(BOBUIableView);
...
/*!
  \reimp
*/
void BOBUIableView::initViewItemOption(QStyleOptionViewItem *option) const
{
...
*/
void BOBUIableView::paintEvent(QPaintEvent *event)
{
    Q_D(BOBUIableView);
...
*/
QModelIndex BOBUIableView::indexAt(const QPoint &pos) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    \sa verticalOffset()
*/
int BOBUIableView::horizontalOffset() const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    \sa horizontalOffset()
*/
int BOBUIableView::verticalOffset() const
{
    Q_D(const BOBUIableView);
...
/*!
    \fn QModelIndex BOBUIableView::moveCursor(CursorAction cursorAction, BobUI::KeyboardModifiers modifiers)
...
*/
QModelIndex BOBUIableView::moveCursor(CursorAction cursorAction, BobUI::KeyboardModifiers modifiers)
{
    Q_D(BOBUIableView);
    Q_UNUSED(modifiers);
    ...
    case MoveHome:
        visualColumn = d->nextActiveVisualColumn(visualRow, 0, right,
                                                 BOBUIableViewPrivate::SearchDirection::Increasing);
    ...
    case MoveEnd:
        visualColumn = d->nextActiveVisualColumn(visualRow, right, -1,
                                                 BOBUIableViewPrivate::SearchDirection::Decreasing);
        if (modifiers & BobUI::ControlModifier)
...
/*!
    \fn void BOBUIableView::setSelection(const QRect &rect,
    QItemSelectionModel::SelectionFlags flags)

    Selects the items within the given \a rect and in accordance with
    the specified selection \a flags.
*/
void BOBUIableView::setSelection(const QRect &rect, QItemSelectionModel::SelectionFlags command)
{
    Q_D(BOBUIableView);
...
*/
QRegion BOBUIableView::visualRegionForSelection(const QItemSelection &selection) const
{
    Q_D(const BOBUIableView);
...
/*!
  \reimp
*/
QModelIndexList BOBUIableView::selectedIndexes() const
{
    Q_D(const BOBUIableView);
...
*/
void BOBUIableView::rowCountChanged(int oldCount, int newCount )
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::columnCountChanged(int, int)
{
    Q_D(BOBUIableView);
    updateGeometries();
    if (horizontalScrollMode() == QAbstractItemView::ScrollPerItem)
...
/*!
    \reimp
*/
void BOBUIableView::updateGeometries()
{
    Q_D(BOBUIableView);
...
*/
int BOBUIableView::sizeHintForRow(int row) const
{
    Q_D(const BOBUIableView);
...
*/
int BOBUIableView::sizeHintForColumn(int column) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    row.
*/
int BOBUIableView::rowViewportPosition(int row) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    \sa columnAt()
*/
int BOBUIableView::rowAt(int y) const
{
    Q_D(const BOBUIableView);
...
*/
void BOBUIableView::setRowHeight(int row, int height)
{
    Q_D(const BOBUIableView);
...
*/
int BOBUIableView::rowHeight(int row) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    column.
*/
int BOBUIableView::columnViewportPosition(int column) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    \sa rowAt()
*/
int BOBUIableView::columnAt(int x) const
{
    Q_D(const BOBUIableView);
...
*/
void BOBUIableView::setColumnWidth(int column, int width)
{
    Q_D(const BOBUIableView);
...
*/
int BOBUIableView::columnWidth(int column) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    \sa isColumnHidden()
*/
bool BOBUIableView::isRowHidden(int row) const
{
    Q_D(const BOBUIableView);
...
/*!
    ...
    \sa setColumnHidden()
*/
void BOBUIableView::setRowHidden(int row, bool hide)
{
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa isRowHidden()
*/
bool BOBUIableView::isColumnHidden(int column) const
{
    Q_D(const BOBUIableView);
...
/*!
  ...
  \sa setRowHidden()
*/
void BOBUIableView::setColumnHidden(int column, bool hide)
{
    Q_D(BOBUIableView);
...
/*!
    \property BOBUIableView::sortingEnabled
...
/*!
  ...
  order
 */
void BOBUIableView::setSortingEnabled(bool enable)
{
    Q_D(BOBUIableView);
    ...
    if (enable) {
        ...
        d->dynHorHeaderConnections = {
            QObjectPrivate::connect(d->horizontalHeader, &QHeaderView::sortIndicatorChanged,
                                    d, &BOBUIableViewPrivate::sortIndicatorChanged)
        };
    } else {
        d->dynHorHeaderConnections = {
            connect(d->horizontalHeader, &QHeaderView::sectionPressed,
                    this, &BOBUIableView::selectColumn),
...
bool BOBUIableView::isSortingEnabled() const
{
    Q_D(const BOBUIableView);
...
/*!
    \property BOBUIableView::showGrid
...
*/
bool BOBUIableView::showGrid() const
{
    Q_D(const BOBUIableView);
    return d->showGrid;
}

void BOBUIableView::setShowGrid(bool show)
{
    Q_D(BOBUIableView);
    if (d->showGrid != show) {
        d->showGrid = show;
...
/*!
  \property BOBUIableView::gridStyle
  \brief  the pen style used to draw the grid.

  This property holds the style used when drawing the grid (see \l{showGrid}).
*/
BobUI::PenStyle BOBUIableView::gridStyle() const
{
    Q_D(const BOBUIableView);
    return d->gridStyle;
}

void BOBUIableView::setGridStyle(BobUI::PenStyle style)
{
    Q_D(BOBUIableView);
...
/*!
    \property BOBUIableView::wordWrap
...
*/
void BOBUIableView::setWordWrap(bool on)
{
    Q_D(BOBUIableView);
...
bool BOBUIableView::wordWrap() const
{
    Q_D(const BOBUIableView);
...
#if BOBUI_CONFIG(abstractbutton)
/*!
    \property BOBUIableView::cornerButtonEnabled
    ...
    This property is \c true by default.
*/
void BOBUIableView::setCornerButtonEnabled(bool enable)
{
    Q_D(BOBUIableView);
    d->cornerWidget->setEnabled(enable);
}

bool BOBUIableView::isCornerButtonEnabled() const
{
    Q_D(const BOBUIableView);
...
*/
QRect BOBUIableView::visualRect(const QModelIndex &index) const
{
    Q_D(const BOBUIableView);
...
*/
void BOBUIableView::scrollTo(const QModelIndex &index, ScrollHint hint)
{
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa columnResized()
*/
void BOBUIableView::rowResized(int row, int, int)
{
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa rowResized()
*/
void BOBUIableView::columnResized(int column, int, int)
{
    Q_D(BOBUIableView);
...
/*!
 \reimp
 */
void BOBUIableView::timerEvent(BOBUIimerEvent *event)
{
    Q_D(BOBUIableView);
...
#if BOBUI_CONFIG(draganddrop)
/*! \reimp */
void BOBUIableView::dropEvent(QDropEvent *event)
{
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa columnMoved()
*/
void BOBUIableView::rowMoved(int row, int oldIndex, int newIndex)
{
    Q_UNUSED(row);
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa rowMoved()
*/
void BOBUIableView::columnMoved(int column, int oldIndex, int newIndex)
{
    Q_UNUSED(column);
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa selectColumn()
*/
void BOBUIableView::selectRow(int row)
{
    Q_D(BOBUIableView);
...
/*!
    ...
    \sa selectRow()
*/
void BOBUIableView::selectColumn(int column)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::hideRow(int row)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::hideColumn(int column)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::showRow(int row)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::showColumn(int column)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::resizeRowToContents(int row)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::resizeRowsToContents()
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::resizeColumnToContents(int column)
{
    Q_D(BOBUIableView);
...
*/
void BOBUIableView::resizeColumnsToContents()
{
    Q_D(BOBUIableView);
...
/*!
  ...
  \sa sortingEnabled
 */
void BOBUIableView::sortByColumn(int column, BobUI::SortOrder order)
{
    Q_D(BOBUIableView);
    if (column < -1)
        return;
...
/*!
    \internal
*/
void BOBUIableView::verticalScrollbarAction(int action)
{
...
/*!
    \internal
*/
void BOBUIableView::horizontalScrollbarAction(int action)
{
...
/*!
  \reimp
*/
bool BOBUIableView::isIndexHidden(const QModelIndex &index) const
{
    Q_D(const BOBUIableView);
...
/*!
    \fn void BOBUIableView::setSpan(int row, int column, int rowSpanCount, int columnSpanCount)
...
*/
void BOBUIableView::setSpan(int row, int column, int rowSpan, int columnSpan)
{
    Q_D(BOBUIableView);
...
*/
int BOBUIableView::rowSpan(int row, int column) const
{
    Q_D(const BOBUIableView);
...
/*!
  ...
  \sa setSpan(), rowSpan()
*/
int BOBUIableView::columnSpan(int row, int column) const
{
    Q_D(const BOBUIableView);
...
void BOBUIableViewPrivate::selectRow(int row, bool anchor)
{
    Q_Q(BOBUIableView);

    if (q->selectionBehavior() == BOBUIableView::SelectColumns
        || (q->selectionMode() == BOBUIableView::SingleSelection
            && q->selectionBehavior() == BOBUIableView::SelectItems))
        return;
    ...
    if (row >= 0 && row < model->rowCount(root)) {
        ...
        if (q->selectionMode() != BOBUIableView::SingleSelection
...
void BOBUIableViewPrivate::selectColumn(int column, bool anchor)
{
    Q_Q(BOBUIableView);

    if (q->selectionBehavior() == BOBUIableView::SelectRows
        || (q->selectionMode() == BOBUIableView::SingleSelection
            && q->selectionBehavior() == BOBUIableView::SelectItems))
        return;
...
/*!
  \reimp
 */
void BOBUIableView::currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
#if BOBUI_CONFIG(accessibility)
...
/*!
  \reimp
 */
void BOBUIableView::selectionChanged(const QItemSelection &selected,
...
int BOBUIableView::visualIndex(const QModelIndex &index) const
{
    return index.row();
}
...
#include "bobuiableview.moc"

#include "moc_bobuiableview.cpp"
