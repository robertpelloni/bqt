...
static inline bool shouldEnableInputMethod(BOBUIextBrowser *texbrowser)
{
#if defined (Q_OS_ANDROID)
    return !texbrowser->isReadOnly() || (texbrowser->textInteractionFlags() & BobUI::TextSelectableByMouse);
#else
    return !texbrowser->isReadOnly();
#endif
}

Q_STATIC_LOGGING_CATEGORY(lcBrowser, "bobui.text.browser")

class BOBUIextBrowserPrivate : public BOBUIextEditPrivate
{
    Q_DECLARE_PUBLIC(BOBUIextBrowser)
public:
    inline BOBUIextBrowserPrivate()
    ...
    struct HistoryEntry {
        ...
        BOBUIextDocument::ResourceType type = BOBUIextDocument::UnknownResource;
    };
    ...
    BOBUIextDocument::ResourceType currentType;
    ...
    void setSource(const QUrl &url, BOBUIextDocument::ResourceType type);

    // re-imlemented from BOBUIextEditPrivate
...
Q_DECLARE_TYPEINFO(BOBUIextBrowserPrivate::HistoryEntry, Q_RELOCATABLE_TYPE);

QString BOBUIextBrowserPrivate::findFile(const QUrl &name) const
{
    QString fileName;
...
QUrl BOBUIextBrowserPrivate::resolveUrl(const QUrl &url) const
{
    if (!url.isRelative())
        return url;
...
void BOBUIextBrowserPrivate::activateAnchor(const QString &href)
{
    if (href.isEmpty())
        return;
    Q_Q(BOBUIextBrowser);
...
void BOBUIextBrowserPrivate::highlightLink(const QString &anchor)
{
    if (anchor.isEmpty()) {
#ifndef BOBUI_NO_CURSOR
...
#ifndef BOBUI_NO_CURSOR
        viewport->setCursor(BobUI::PointingHandCursor);
#endif
...
void BOBUIextBrowserPrivate::setSource(const QUrl &url, BOBUIextDocument::ResourceType type)
{
    Q_Q(BOBUIextBrowser);
#ifndef BOBUI_NO_CURSOR
    if (q->isVisible())
    ...
    if (url.isValid()
        ...
        if (Q_UNLIKELY(txt.isEmpty()))
            qWarning("BOBUIextBrowser: No document for %s", url.toString().toLatin1().constData());

        if (q->isVisible()) {
            const QStringView firstTag = QStringView{txt}.left(txt.indexOf(u'>') + 1);
            if (firstTag.startsWith("<bobui"_L1) && firstTag.contains("type"_L1) && firstTag.contains("detail"_L1)) {
#ifndef BOBUI_NO_CURSOR
    ...
    if (doSetText) {
        // Setting the base URL helps BOBUIextDocument::resource() to find resources with relative paths.
        // But don't set it unless it contains the document's path, because BOBUIextBrowserPrivate::resolveUrl()
        ...
        q->document()->setMetaInformation(BOBUIextDocument::DocumentUrl, currentURL.toString());
        qCDebug(lcBrowser) << "loading" << currentURL << "base" << q->document()->baseUrl() << "type" << type << txt.size() << "chars";
#if BOBUI_CONFIG(textmarkdownreader)
        if (type == BOBUIextDocument::MarkdownResource)
            q->BOBUIextEdit::setMarkdown(txt);
        else
#endif
#ifndef BOBUI_NO_TEXTHTMLPARSER
        q->BOBUIextEdit::setHtml(txt);
#else
        q->BOBUIextEdit::setPlainText(txt);
#endif

#ifdef BOBUI_KEYPAD_NAVIGATION
        prevFocus.movePosition(BOBUIextCursor::Start);
#endif
    }
...
#ifdef BOBUI_KEYPAD_NAVIGATION
void BOBUIextBrowserPrivate::keypadMove(bool next)
{
    Q_Q(BOBUIextBrowser);
    ...
    if (!focusIt && !foundNextAnchor) {
        if (next) {
            ...
            if (yOffset == 0) {
                prevFocus.movePosition(BOBUIextCursor::End);
...
BOBUIextBrowserPrivate::HistoryEntry BOBUIextBrowserPrivate::createHistoryEntry() const
{
    HistoryEntry entry;
    ...
    const BOBUIextCursor cursor = control->textCursor();
...
void BOBUIextBrowserPrivate::restoreHistoryEntry(const HistoryEntry &entry)
{
    ...
    if (entry.focusIndicatorAnchor != -1 && entry.focusIndicatorPosition != -1) {
        BOBUIextCursor cursor(control->document());
        cursor.setPosition(entry.focusIndicatorAnchor);
        cursor.setPosition(entry.focusIndicatorPosition, BOBUIextCursor::KeepAnchor);
        control->setTextCursor(cursor);
...
/*!
    ...
    \brief The BOBUIextBrowser class provides a rich text browser with hypertext navigation.

    \ingroup richtext-processing
    \inmodule BobUIWidgets

    This class extends BOBUIextEdit (in read-only mode), adding some navigation
    ...
    use BOBUIextEdit. If you want a text browser without hypertext navigation
    use BOBUIextEdit, and use BOBUIextEdit::setReadOnly() to disable
    editing. If you just need to display a small piece of rich text
    use QLabel.

    \section1 Document Source and Contents

    The contents of BOBUIextEdit are set with setHtml() or setPlainText(),
    but BOBUIextBrowser also implements the setSource() function, making it
    ...
    BOBUIextBrowser provides backward() and forward() slots which you can
    ...
    If you want to load documents stored in the BobUI resource system use
    ...
    \sa BOBUIextEdit, BOBUIextDocument
*/

/*!
    \property BOBUIextBrowser::modified
    \brief whether the contents of the text browser have been modified
*/

/*!
    \property BOBUIextBrowser::readOnly
...
/*!
    \property BOBUIextBrowser::undoRedoEnabled
    \brief whether the text browser supports undo/redo operations

    By default, this property is \c false.
*/

void BOBUIextBrowserPrivate::init()
{
    Q_Q(BOBUIextBrowser);
    control->setTextInteractionFlags(BobUI::TextBrowserInteraction);
#ifndef BOBUI_NO_CURSOR
    viewport->setCursor(oldCursor);
#endif
    q->setAttribute(BobUI::WA_InputMethodEnabled, shouldEnableInputMethod(q));
    q->setUndoRedoEnabled(false);
    viewport->setMouseTracking(true);
    connections = {
        QObjectPrivate::connect(q->document(), &BOBUIextDocument::contentsChanged,
                                this, &BOBUIextBrowserPrivate::documentModified),
        QObjectPrivate::connect(control, &QWidgetTextControl::linkActivated,
                                this, &BOBUIextBrowserPrivate::activateAnchor),
        QObjectPrivate::connect(control, &QWidgetTextControl::linkHovered,
                                this, &BOBUIextBrowserPrivate::highlightLink),
    };
}

/*!
    Constructs an empty BOBUIextBrowser with parent \a parent.
*/
BOBUIextBrowser::BOBUIextBrowser(QWidget *parent)
    : BOBUIextEdit(*new BOBUIextBrowserPrivate, parent)
{
    Q_D(BOBUIextBrowser);
    d->init();
}
...
/*!
    \internal
*/
BOBUIextBrowser::~BOBUIextBrowser()
{
}

/*!
    \property BOBUIextBrowser::source
    \brief the name of the displayed document.

    This is a an invalid url if no document is displayed or if the
    source is unknown.

    When setting this property BOBUIextBrowser tries to find a document
    ...
    If the first tag in the document is \c{<bobui type=detail>}, the
    ...
    the named document with \l BOBUIextDocument::setHtml() or
    \l BOBUIextDocument::setMarkdown(), depending on whether the filename ends
...
*/
QUrl BOBUIextBrowser::source() const
{
    Q_D(const BOBUIextBrowser);
    if (d->stack.isEmpty())
        return QUrl();
    else
...
/*!
    \property BOBUIextBrowser::sourceType
    \brief the type of the displayed document

    This is BOBUIextDocument::UnknownResource if no document is displayed or if
...
*/
BOBUIextDocument::ResourceType BOBUIextBrowser::sourceType() const
{
    Q_D(const BOBUIextBrowser);
    if (d->stack.isEmpty())
        return BOBUIextDocument::UnknownResource;
    else
        return d->stack.top().type;
}

/*!
    \property BOBUIextBrowser::searchPaths
    \brief the search paths used by the text browser to find supporting
    content

    BOBUIextBrowser uses this list to locate images and documents.

    By default, this property contains an empty string list.
*/

QStringList BOBUIextBrowser::searchPaths() const
{
    Q_D(const BOBUIextBrowser);
    return d->searchPaths;
}

void BOBUIextBrowser::setSearchPaths(const QStringList &paths)
{
    Q_D(BOBUIextBrowser);
    d->searchPaths = paths;
}
...
/*!
    ...
    If \a type is \l {BOBUIextDocument::UnknownResource}{UnknownResource}
    ...
    loaded via \l BOBUIextDocument::setMarkdown(); otherwise it will be loaded via
    \l BOBUIextDocument::setHtml(). This detection can be bypassed by specifying
    the \a type explicitly.
*/
void BOBUIextBrowser::setSource(const QUrl &url, BOBUIextDocument::ResourceType type)
{
...
/*!
    ...
    setSource() calls doSetSource.  In BobUI 5, setSource(const QUrl &url) was virtual.
    In BobUI 6, doSetSource() is virtual instead, so that it can be overridden in subclasses.
*/
void BOBUIextBrowser::doSetSource(const QUrl &url, BOBUIextDocument::ResourceType type)
{
    Q_D(BOBUIextBrowser);

    const BOBUIextBrowserPrivate::HistoryEntry historyEntry = d->createHistoryEntry();
    ...
    BOBUIextBrowserPrivate::HistoryEntry entry;
    entry.url = url;
...
/*!
    \fn void BOBUIextBrowser::backwardAvailable(bool available)
...
/*!
    \fn void BOBUIextBrowser::forwardAvailable(bool available)
...
/*!
    \fn void BOBUIextBrowser::historyChanged()
    \since 4.4
...
/*!
    \fn void BOBUIextBrowser::sourceChanged(const QUrl &src)
...
/*!  \fn void BOBUIextBrowser::highlighted(const QUrl &link)
...
/*!
    \fn void BOBUIextBrowser::anchorClicked(const QUrl &link)
...
/*!
    ...
    \endtable
*/
void BOBUIextBrowser::keyPressEvent(QKeyEvent *ev)
{
#ifdef BOBUI_KEYPAD_NAVIGATION
    Q_D(BOBUIextBrowser);
    switch (ev->key()) {
    ...
    if (ev->modifiers() & BobUI::AltModifier) {
        switch (ev->key()) {
        ...
        if (ev->key() == BobUI::Key_Up) {
            d->keypadMove(false);
            return;
        } else if (ev->key() == BobUI::Key_Down) {
            d->keypadMove(true);
            return;
        }
    }
...
/*!
    \reimp
*/
void BOBUIextBrowser::mouseMoveEvent(QMouseEvent *e)
{
    BOBUIextEdit::mouseMoveEvent(e);
}

/*!
    \reimp
*/
void BOBUIextBrowser::mousePressEvent(QMouseEvent *e)
{
    BOBUIextEdit::mousePressEvent(e);
}

/*!
    \reimp
*/
void BOBUIextBrowser::mouseReleaseEvent(QMouseEvent *e)
{
    BOBUIextEdit::mouseReleaseEvent(e);
}

/*!
    \reimp
*/
void BOBUIextBrowser::focusOutEvent(QFocusEvent *ev)
{
#ifndef BOBUI_NO_CURSOR
    Q_D(BOBUIextBrowser);
    d->viewport->setCursor((!(d->control->textInteractionFlags() & BobUI::TextEditable)) ? d->oldCursor : BobUI::IBeamCursor);
#endif
    BOBUIextEdit::focusOutEvent(ev);
}

/*!
    \reimp
*/
bool BOBUIextBrowser::focusNextPrevChild(bool next)
{
    Q_D(BOBUIextBrowser);
    ...
    return BOBUIextEdit::focusNextPrevChild(next);
}

/*!
  \reimp
*/
void BOBUIextBrowser::paintEvent(QPaintEvent *e)
{
    Q_D(BOBUIextBrowser);
...
/*!
    ...
    \row    \li BOBUIextDocument::HtmlResource  \li QString or QByteArray
    \row    \li BOBUIextDocument::ImageResource \li QImage, QPixmap or QByteArray
    \row    \li BOBUIextDocument::StyleSheetResource \li QString or QByteArray
    \row    \li BOBUIextDocument::MarkdownResource \li QString or QByteArray
    \endtable
*/
QVariant BOBUIextBrowser::loadResource(int /*type*/, const QUrl &name)
{
    Q_D(BOBUIextBrowser);
...
*/
bool BOBUIextBrowser::isBackwardAvailable() const
{
    Q_D(const BOBUIextBrowser);
...
*/
bool BOBUIextBrowser::isForwardAvailable() const
{
    Q_D(const BOBUIextBrowser);
...
/*!
    ...
    \sa backward(), forward()
*/
void BOBUIextBrowser::clearHistory()
{
    Q_D(BOBUIextBrowser);
    d->forwardStack.clear();
    if (!d->stack.isEmpty()) {
        BOBUIextBrowserPrivate::HistoryEntry historyEntry = d->stack.top();
        d->stack.clear();
...
/*!
   Returns the url of the HistoryItem.
    ...
    \row \li\a{i} == 0 \li current, see BOBUIextBrowser::source()
    \row \li \a{i} > 0  \li \l forward() history
    \endtable

    \since 4.4
*/
QUrl BOBUIextBrowser::historyUrl(int i) const
{
    Q_D(const BOBUIextBrowser);
...
/*!
    ...
    \snippet code/src_gui_widgets_bobuiextbrowser.cpp 0

    \since 4.4
*/
QString BOBUIextBrowser::historyTitle(int i) const
{
    Q_D(const BOBUIextBrowser);
...
/*!
    ...
    \since 4.4
*/
int BOBUIextBrowser::forwardHistoryCount() const
{
    Q_D(const BOBUIextBrowser);
...
/*!
    ...
    \since 4.4
*/
int BOBUIextBrowser::backwardHistoryCount() const
{
    Q_D(const BOBUIextBrowser);
    return d->stack.size()-1;
}
...
/*!
    \property BOBUIextBrowser::openExternalLinks
    \since 4.2

    Specifies whether BOBUIextBrowser should automatically open links to external
...
*/
bool BOBUIextBrowser::openExternalLinks() const
{
    Q_D(const BOBUIextBrowser);
    return d->openExternalLinks;
}

void BOBUIextBrowser::setOpenExternalLinks(bool open)
{
    Q_D(BOBUIextBrowser);
...
/*!
   \property BOBUIextBrowser::openLinks
   \since 4.3

   This property specifies whether BOBUIextBrowser should automatically open links the user tries to
...
bool BOBUIextBrowser::openLinks() const
{
    Q_D(const BOBUIextBrowser);
    return d->openLinks;
}

void BOBUIextBrowser::setOpenLinks(bool open)
{
    Q_D(BOBUIextBrowser);
    d->openLinks = open;
}
...
/*! \reimp */
bool BOBUIextBrowser::event(QEvent *e)
{
...
#include "moc_bobuiextbrowser.cpp"
