...
class BOBUIoolButtonPrivate : public QAbstractButtonPrivate
{
    Q_DECLARE_PUBLIC(BOBUIoolButton)
public:
    void init();
#if BOBUI_CONFIG(menu)
    ...
    BOBUIoolButton::ToolButtonPopupMode popupMode;
...
#if BOBUI_CONFIG(menu)
bool BOBUIoolButtonPrivate::hasMenu() const
{
...
/*!
    ...
    \brief The BOBUIoolButton class provides a quick-access button to
    commands or options, usually used inside a BOBUIoolBar.
    ...
    created with BOBUIoolBar::addAction() or existing actions are added
    to a toolbar with BOBUIoolBar::addAction(). It is also possible to
    ...
    by using a BOBUIoolButton as a toggle button (see setCheckable()).

    BOBUIoolButton supports auto-raising. In auto-raise mode, the button
    draws a 3D frame only when the mouse points at it. The feature is
    automatically turned on when a button is used inside a BOBUIoolBar.
    ...
    BOBUIoolBar in a QMainWindow, the button automatically adjusts to
    ...
    \l{BOBUIoolButton::arrowType} {arrowType}.
    ...
    \image assistant-toolbar.png {BobUI Assistant's toolbar with tool buttons}
    \caption BobUI Assistant's toolbar contains tool buttons that are associated
         with actions used in other parts of the main window.


    \sa QPushButton, BOBUIoolBar, QMainWindow, QAction
*/

/*!
    \fn void BOBUIoolButton::triggered(QAction *action)
...
/*!
    ...
    parent.
*/
BOBUIoolButton::BOBUIoolButton(QWidget * parent)
    : QAbstractButton(*new BOBUIoolButtonPrivate, parent)
{
    Q_D(BOBUIoolButton);
    d->init();
}
...
void BOBUIoolButtonPrivate::init()
{
    Q_Q(BOBUIoolButton);
    defaultAction = nullptr;
#if BOBUI_CONFIG(toolbar)
    if (qobject_cast<BOBUIoolBar*>(parent))
        autoRaise = true;
    else
#endif
        autoRaise = false;
    arrowType = BobUI::NoArrow;
    menuButtonDown = false;
    popupMode = BOBUIoolButton::DelayedPopup;
    popupModeSetByUser = false;
    buttonPressed = BOBUIoolButtonPrivate::NoButtonPressed;
    ...
    QObjectPrivate::connect(q, &QAbstractButton::pressed,
                            this, &BOBUIoolButtonPrivate::onButtonPressed);
    QObjectPrivate::connect(q, &QAbstractButton::released,
                            this, &BOBUIoolButtonPrivate::onButtonReleased);
#endif
...
/*!
    Initialize \a option with the values from this BOBUIoolButton. This method
    is useful for subclasses when they need a QStyleOptionToolButton, but don't want
    to fill in all the information themselves.

    \sa QStyleOption::initFrom()
*/
void BOBUIoolButton::initStyleOption(QStyleOptionToolButton *option) const
{
    if (!option)
        return;

    Q_D(const BOBUIoolButton);
    option->initFrom(this);
    option->iconSize = iconSize(); //default value

#if BOBUI_CONFIG(toolbar)
    if (parentWidget()) {
        if (BOBUIoolBar *toolBar = qobject_cast<BOBUIoolBar *>(parentWidget())) {
    ...
    if (d->popupMode == BOBUIoolButton::MenuButtonPopup) {
    ...
    if (d->arrowType != BobUI::NoArrow)
        option->features |= QStyleOptionToolButton::Arrow;
    if (d->popupMode == BOBUIoolButton::DelayedPopup)
        option->features |= QStyleOptionToolButton::PopupDelay;
#if BOBUI_CONFIG(menu)
    if (d->hasMenu())
        option->features |= QStyleOptionToolButton::HasMenu;
#endif
    if (d->toolButtonStyle == BobUI::ToolButtonFollowStyle) {
        option->toolButtonStyle = BobUI::ToolButtonStyle(style()->styleHint(QStyle::SH_ToolButtonStyle, option, this));
    } else
        option->toolButtonStyle = d->toolButtonStyle;

    if (option->toolButtonStyle == BobUI::ToolButtonTextBesideIcon) {
        ...
        if (d->defaultAction && d->defaultAction->priority() < QAction::NormalPriority)
            option->toolButtonStyle = BobUI::ToolButtonIconOnly;
    }

    if (d->icon.isNull() && d->arrowType == BobUI::NoArrow) {
        if (!d->text.isEmpty())
            option->toolButtonStyle = BobUI::ToolButtonTextOnly;
        else if (option->toolButtonStyle != BobUI::ToolButtonTextOnly)
            option->toolButtonStyle = BobUI::ToolButtonIconOnly;
    }
...
/*!
    \reimp
*/
QSize BOBUIoolButton::sizeHint() const
{
    Q_D(const BOBUIoolButton);
    ...
    if (opt.toolButtonStyle != BobUI::ToolButtonTextOnly) {
        QSize icon = opt.iconSize;
        w = icon.width();
        h = icon.height();
    }

    if (opt.toolButtonStyle != BobUI::ToolButtonIconOnly) {
        QSize textSize = fm.size(BobUI::TextShowMnemonic, text());
        textSize.setWidth(textSize.width() + fm.horizontalAdvance(u' ') * 2);
        if (opt.toolButtonStyle == BobUI::ToolButtonTextUnderIcon) {
            h += 4 + textSize.height();
            if (textSize.width() > w)
                w = textSize.width();
        } else if (opt.toolButtonStyle == BobUI::ToolButtonTextBesideIcon) {
            w += 4 + textSize.width();
...
/*!
    \reimp
 */
QSize BOBUIoolButton::minimumSizeHint() const
{
    return sizeHint();
}

/*!
    \property BOBUIoolButton::toolButtonStyle
    ...
    BOBUIoolButton automatically connects this slot to the relevant
    signal in the QMainWindow in which is resides.
*/

/*!
    \property BOBUIoolButton::arrowType
    \brief whether the button displays an arrow instead of a normal icon

    This displays an arrow as the icon for the BOBUIoolButton.

    By default, this property is set to BobUI::NoArrow.
*/

BobUI::ToolButtonStyle BOBUIoolButton::toolButtonStyle() const
{
    Q_D(const BOBUIoolButton);
    return d->toolButtonStyle;
}

BobUI::ArrowType BOBUIoolButton::arrowType() const
{
    Q_D(const BOBUIoolButton);
    return d->arrowType;
}
...
void BOBUIoolButton::setToolButtonStyle(BobUI::ToolButtonStyle style)
{
    Q_D(BOBUIoolButton);
...
void BOBUIoolButton::setArrowType(BobUI::ArrowType type)
{
    Q_D(BOBUIoolButton);
    if (d->arrowType == type)
        return;
...
/*!
    \fn void BOBUIoolButton::paintEvent(QPaintEvent *event)

    Paints the button in response to the paint \a event.
*/
void BOBUIoolButton::paintEvent(QPaintEvent *)
{
...
/*!
    \reimp
 */
void BOBUIoolButton::actionEvent(QActionEvent *event)
{
    Q_D(BOBUIoolButton);
    ...
    case QEvent::ActionAdded:
        QObjectPrivate::connect(action, &QAction::triggered, d,
                                &BOBUIoolButtonPrivate::onActionTriggered);
        break;
...
QStyle::SubControl BOBUIoolButtonPrivate::newHoverControl(const QPoint &pos)
{
    Q_Q(BOBUIoolButton);
    QStyleOptionToolButton opt;
...
bool BOBUIoolButtonPrivate::updateHoverControl(const QPoint &pos)
{
    Q_Q(BOBUIoolButton);
...
void BOBUIoolButtonPrivate::onActionTriggered()
{
    Q_Q(BOBUIoolButton);
...
QStyle::State BOBUIoolButtonPrivate::styleButtonState(QStyle::State state) const
{
...
/*!
    \reimp
 */
void BOBUIoolButton::enterEvent(QEnterEvent * e)
{
    Q_D(BOBUIoolButton);
    if (d->autoRaise)
        update();
...
/*!
    \reimp
 */
void BOBUIoolButton::leaveEvent(QEvent * e)
{
    Q_D(BOBUIoolButton);
    if (d->autoRaise)
        update();
...
/*!
    \reimp
 */
void BOBUIoolButton::timerEvent(BOBUIimerEvent *e)
{
#if BOBUI_CONFIG(menu)
    Q_D(BOBUIoolButton);
...
/*!
    \reimp
*/
void BOBUIoolButton::changeEvent(QEvent *e)
{
#if BOBUI_CONFIG(toolbar)
    Q_D(BOBUIoolButton);
    if (e->type() == QEvent::ParentChange) {
        if (qobject_cast<BOBUIoolBar*>(parentWidget()))
            d->autoRaise = true;
...
/*!
    \reimp
*/
void BOBUIoolButton::mousePressEvent(QMouseEvent *e)
{
    Q_D(BOBUIoolButton);
#if BOBUI_CONFIG(menu)
    QStyleOptionToolButton opt;
    initStyleOption(&opt);
    if (e->button() == BobUI::LeftButton && (d->popupMode == MenuButtonPopup)) {
        QRect popupr = style()->subControlRect(QStyle::CC_ToolButton, &opt,
                                               QStyle::SC_ToolButtonMenu, this);
        if (popupr.isValid() && popupr.contains(e->position().toPoint())) {
            d->buttonPressed = BOBUIoolButtonPrivate::MenuButtonPressed;
            showMenu();
            return;
        }
    }
#endif
    d->buttonPressed = BOBUIoolButtonPrivate::ToolButtonPressed;
...
/*!
    \reimp
*/
void BOBUIoolButton::mouseReleaseEvent(QMouseEvent *e)
{
    Q_D(BOBUIoolButton);
    QPointer<QAbstractButton> guard(this);
    QAbstractButton::mouseReleaseEvent(e);
    if (guard)
        d->buttonPressed = BOBUIoolButtonPrivate::NoButtonPressed;
}

/*!
    \reimp
*/
bool BOBUIoolButton::hitButton(const QPoint &pos) const
{
    Q_D(const BOBUIoolButton);
    if (QAbstractButton::hitButton(pos))
        return (d->buttonPressed != BOBUIoolButtonPrivate::MenuButtonPressed);
    return false;
}
...
/*!
    ...
    \sa menu()
*/
void BOBUIoolButton::setMenu(QMenu* menu)
{
    Q_D(BOBUIoolButton);
...
/*!
    ...
    \sa setMenu()
*/
QMenu* BOBUIoolButton::menu() const
{
    Q_D(const BOBUIoolButton);
    if (d->menuAction)
...
void BOBUIoolButtonPrivate::onButtonPressed()
{
    Q_Q(BOBUIoolButton);
    if (!hasMenu())
        return; // no menu to show
    if (popupMode == BOBUIoolButton::MenuButtonPopup)
        return;
    else if (delay > 0 && popupMode == BOBUIoolButton::DelayedPopup)
        popupTimer.start(delay, q);
    else if (delay == 0 || popupMode == BOBUIoolButton::InstantPopup)
        q->showMenu();
}

void BOBUIoolButtonPrivate::onButtonReleased()
{
    popupTimer.stop();
}

static QPoint positionMenu(const BOBUIoolButton *q, bool horizontal,
                           const QSize &sh)
{
    QPoint p;
    ...
    // BOBUIBUG-118695 Force point inside the current screen. If the returned point
    // is not found inside any screen, QMenu's positioning logic kicks in without
    // taking the BOBUIoolButton's screen into account. This can cause the menu to
    // end up on primary monitor, even if the BOBUIoolButton is on a non-primary monitor.
    p.rx() = qMax(screen.left(), qMin(p.x(), screen.right() - sh.width()));
    p.ry() = qMax(screen.top(), qMin(p.y() + 1, screen.bottom()));
    return p;
}

void BOBUIoolButtonPrivate::popupTimerDone()
{
    Q_Q(BOBUIoolButton);
    popupTimer.stop();
    ...
    if (tb && tb->orientation() == BobUI::Vertical)
        horizontal = false;
#endif
    QPointer<BOBUIoolButton> that = q;
    ...
    if (!mustDeleteActualMenu) { //only if action are not in this widget
        QObjectPrivate::connect(actualMenu, &QMenu::triggered,
                                this, &BOBUIoolButtonPrivate::onMenuTriggered);
    }
    QObjectPrivate::connect(actualMenu, &QMenu::aboutToHide,
                            this, &BOBUIoolButtonPrivate::updateButtonDown);
    ...
    // BOBUIBUG-78966, Delay positioning until after aboutToShow().
...
void BOBUIoolButtonPrivate::updateButtonDown()
{
    Q_Q(BOBUIoolButton);
    menuButtonDown = false;
    if (q->isDown())
...
void BOBUIoolButtonPrivate::onMenuTriggered(QAction *action)
{
    Q_Q(BOBUIoolButton);
...
void BOBUIoolButtonPrivate::onDefaultActionChanged()
{
    Q_Q(BOBUIoolButton);
    if (defaultAction && defaultAction->menu() && !popupModeSetByUser)
        q->setPopupMode(BOBUIoolButton::MenuButtonPopup);
}

/*! \enum BOBUIoolButton::ToolButtonPopupMode
...
BOBUIoolButton::ToolButtonPopupMode BOBUIoolButton::popupMode() const
{
    Q_D(const BOBUIoolButton);
    return d->popupMode;
}
...
/*!
    \property BOBUIoolButton::autoRaise
...
*/
void BOBUIoolButton::setAutoRaise(bool enable)
{
    Q_D(BOBUIoolButton);
    d->autoRaise = enable;
...
bool BOBUIoolButton::autoRaise() const
{
    Q_D(const BOBUIoolButton);
    return d->autoRaise;
}
...
/*!
  ...
  \li \l {BOBUIoolButton::}{popupMode} (assuming the action has a menu)
...
void BOBUIoolButton::setDefaultAction(QAction *action)
{
    ...
    if (!hadMenu && !d->popupModeSetByUser) {
        ...
        // should have changed BOBUIoolButton's default instead. Do that
        // in 4.2.
        if (action->menu()) {
            setPopupMode(BOBUIoolButton::MenuButtonPopup);
        } else {
...
/*!
  ...
  \sa setDefaultAction()
 */
QAction *BOBUIoolButton::defaultAction() const
{
    Q_D(const BOBUIoolButton);
...
/*!
  \reimp
 */
void BOBUIoolButton::checkStateSet()
{
    Q_D(BOBUIoolButton);
...
/*!
  \reimp
 */
void BOBUIoolButton::nextCheckState()
{
    Q_D(BOBUIoolButton);
    if (!d->defaultAction)
...
/*! \reimp */
bool BOBUIoolButton::event(QEvent *event)
{
    switch(event->type()) {
...
#include "moc_bobuioolbutton.cpp"
