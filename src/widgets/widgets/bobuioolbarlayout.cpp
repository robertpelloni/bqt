...
BOBUIoolBarItem::BOBUIoolBarItem(QWidget *widget)
    : QWidgetItem(widget), action(nullptr), customWidget(false)
{
}

bool BOBUIoolBarItem::isEmpty() const
{
...
BOBUIoolBarLayout::BOBUIoolBarLayout(QWidget *parent)
        ...
        expanding(false), empty(true), expandFlag(false), popupMenu(nullptr)
{
    ...
    extension = new BOBUIoolBarExtension(tb);
    extension->setFocusPolicy(BobUI::NoFocus);
    extension->hide();
    QObject::connect(tb, SIGNAL(orientationChanged(BobUI::Orientation)),
                     extension, SLOT(setOrientation(BobUI::Orientation)));

    setUsePopupMenu(qobject_cast<QMainWindow*>(tb->parentWidget()) == 0);
}

BOBUIoolBarLayout::~BOBUIoolBarLayout()
{
    while (!items.isEmpty()) {
        BOBUIoolBarItem *item = items.takeFirst();
...
void BOBUIoolBarLayout::updateMarginAndSpacing()
{
    BOBUIoolBar *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
        return;
...
bool BOBUIoolBarLayout::hasExpandFlag() const
{
    return expandFlag;
}

void BOBUIoolBarLayout::setUsePopupMenu(bool set)
{
    if (!dirty && ((popupMenu == nullptr) == set))
        invalidate();
    if (!set) {
        QObject::connect(extension, SIGNAL(clicked(bool)),
                        this, SLOT(setExpanded(bool)), BobUI::UniqueConnection);
        extension->setPopupMode(BOBUIoolButton::DelayedPopup);
        ...
        extension->setPopupMode(BOBUIoolButton::InstantPopup);
        if (!popupMenu) {
...
void BOBUIoolBarLayout::checkUsePopupMenu()
{
    BOBUIoolBar *tb = static_cast<BOBUIoolBar *>(parent());
    QMainWindow *mw = qobject_cast<QMainWindow *>(tb->parent());
    BobUI::Orientation o = tb->orientation();
    setUsePopupMenu(!mw || tb->isFloating() || perp(o, expandedSize(mw->size())) >= perp(o, mw->size()));
}

void BOBUIoolBarLayout::addItem(QLayoutItem*)
{
    qWarning("BOBUIoolBarLayout::addItem(): please use addAction() instead");
    return;
}

QLayoutItem *BOBUIoolBarLayout::itemAt(int index) const
{
...
QLayoutItem *BOBUIoolBarLayout::takeAt(int index)
{
    if (index < 0 || index >= items.size())
        return nullptr;
    BOBUIoolBarItem *item = items.takeAt(index);
    ...
    if (widgetAction != nullptr && item->customWidget) {
        ...
        // destroy the BOBUIoolButton/BOBUIoolBarSeparator
        item->widget()->hide();
...
void BOBUIoolBarLayout::insertAction(int index, QAction *action)
{
    index = qMax(0, index);
    index = qMin(items.size(), index);

    BOBUIoolBarItem *item = createItem(action);
    if (item) {
...
int BOBUIoolBarLayout::indexOf(const QAction *action) const
{
...
int BOBUIoolBarLayout::count() const
{
    return items.size();
}

bool BOBUIoolBarLayout::isEmpty() const
{
    if (dirty)
        updateGeomArray();
    return empty;
...
void BOBUIoolBarLayout::invalidate()
{
    dirty = true;
    QLayout::invalidate();
}

BobUI::Orientations BOBUIoolBarLayout::expandingDirections() const
{
    if (dirty)
        updateGeomArray();
    BOBUIoolBar *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
        return {};
    BobUI::Orientation o = tb->orientation();
    return expanding ? BobUI::Orientations(o) : BobUI::Orientations{};
}

bool BOBUIoolBarLayout::movable() const
{
    BOBUIoolBar *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
        return false;
...
void BOBUIoolBarLayout::updateGeomArray() const
{
    if (!dirty)
        return;

    BOBUIoolBarLayout *that = const_cast<BOBUIoolBarLayout*>(this);
    ...
    int count = 0;
    for (int i = 0; i < items.size(); ++i) {
        BOBUIoolBarItem *item = items.at(i);

        QSize max = item->maximumSize();
        QSize min = item->minimumSize();
        QSize hint = item->sizeHint();
        BobUI::Orientations exp = item->expandingDirections();
    ...
    that->minSize += QSize(pick(BobUI::Horizontal, margins), pick(BobUI::Vertical, margins));
    if (items.size() > 1)
        rpick(o, that->minSize) += spacing + extensionExtent;

    rpick(o, that->hint) += handleExtent;
    that->hint += QSize(pick(BobUI::Horizontal, margins), pick(BobUI::Vertical, margins));
    that->dirty = false;
}

static bool defaultWidgetAction(BOBUIoolBarItem *item)
{
    QWidgetAction *a = qobject_cast<QWidgetAction*>(item->action);
    return a != nullptr && a->defaultWidget() == item->widget();
}

void BOBUIoolBarLayout::updateMacBorderMetrics()
{
#ifdef Q_OS_MACOS
    BOBUIoolBar *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
    ...
    if (mainWindow->toolBarArea(tb) == BobUI::TopToolBarArea) {
...
void BOBUIoolBarLayout::setGeometry(const QRect &rect)
{
    BOBUIoolBar *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
        return;
    ...
    if (expanded || animating || ranOutOfSpace) {
        BobUI::ToolBarArea area = BobUI::TopToolBarArea;
        ...
        if (area == BobUI::LeftToolBarArea || area == BobUI::TopToolBarArea)
...
bool BOBUIoolBarLayout::layoutActions(const QSize &size)
{
    if (dirty)
        updateGeomArray();
    ...
    while (i < items.size()) {
        ...
        for (int j = start; j < i; ++j) {
            BOBUIoolBarItem *item = items.at(j);
...
QSize BOBUIoolBarLayout::expandedSize(const QSize &size) const
{
    if (dirty)
        updateGeomArray();
    ...
    w += pick(BobUI::Horizontal, margins) + handleExtent + spacing + extensionExtent;
    w = qMax(w, min_w);
    if (win != nullptr)
        w = qMin(w, pick(o, win->size()));
    h += pick(BobUI::Vertical, margins) - spacing; //there is no spacing before the first row
...
void BOBUIoolBarLayout::setExpanded(bool exp)
{
    QWidget *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
        return;
...
QSize BOBUIoolBarLayout::minimumSize() const
{
    if (dirty)
        updateGeomArray();
    return minSize;
...
QSize BOBUIoolBarLayout::sizeHint() const
{
    if (dirty)
        updateGeomArray();
    return hint;
...
BOBUIoolBarItem *BOBUIoolBarLayout::createItem(QAction *action)
{
    ...
    BOBUIoolBar *tb = qobject_cast<BOBUIoolBar*>(parentWidget());
    if (!tb)
        return (BOBUIoolBarItem *)nullptr;

    if (QWidgetAction *widgetAction = qobject_cast<QWidgetAction *>(action)) {
        widget = widgetAction->requestWidget(tb);
        if (widget != nullptr) {
            widget->setAttribute(BobUI::WA_LayoutUsesWidgetRect);
            customWidget = true;
        }
    } else if (action->isSeparator()) {
        BOBUIoolBarSeparator *sep = new BOBUIoolBarSeparator(tb);
        connect(tb, SIGNAL(orientationChanged(BobUI::Orientation)),
                sep, SLOT(setOrientation(BobUI::Orientation)));
        widget = sep;
    }

    if (!widget) {
        BOBUIoolButton *button = new BOBUIoolButton(tb);
        button->setAutoRaise(true);
        button->setFocusPolicy(BobUI::NoFocus);
        ...
        QObject::connect(tb, SIGNAL(toolButtonStyleChanged(BobUI::ToolButtonStyle)),
                         button, SLOT(setToolButtonStyle(BobUI::ToolButtonStyle)));
    ...
    widget->hide();
    BOBUIoolBarItem *result = new BOBUIoolBarItem(widget);
    if (standardButtonWidget)
        result->setAlignment(BobUI::AlignJustify);
...
#include "moc_bobuioolbarlayout_p.cpp"
