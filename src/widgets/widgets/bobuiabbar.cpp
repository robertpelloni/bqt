...
void BOBUIabBarPrivate::updateMacBorderMetrics()
{
...
/*!
    \internal
    This is basically BOBUIabBar::initStyleOption() but
    without the expensive QFontMetrics::elidedText() call.
*/

void BOBUIabBarPrivate::initBasicStyleOption(QStyleOptionTab *option, int tabIndex) const
{
    Q_Q(const BOBUIabBar);
    const int totalTabs = tabList.size();

    if (!option || (tabIndex < 0 || tabIndex >= totalTabs))
        return;

    const BOBUIabBarPrivate::Tab &tab = *tabList.at(tabIndex);
    option->initFrom(q);
...
#if BOBUI_CONFIG(tabwidget)
    if (const BOBUIabWidget *tw = qobject_cast<const BOBUIabWidget *>(q->parentWidget())) {
        option->features |= QStyleOptionTab::HasFrame;
        if (tw->cornerWidget(BobUI::TopLeftCorner) || tw->cornerWidget(BobUI::BottomLeftCorner))
            option->cornerWidgets |= QStyleOptionTab::LeftCornerWidget;
        if (tw->cornerWidget(BobUI::TopRightCorner) || tw->cornerWidget(BobUI::BottomRightCorner))
...
/*!
    ...
    \sa QStyleOption::initFrom(), BOBUIabWidget::initStyleOption()
*/
void BOBUIabBar::initStyleOption(QStyleOptionTab *option, int tabIndex) const
{
    Q_D(const BOBUIabBar);
    ...
    option->text = fontMetrics().elidedText(option->text, d->elideMode, textRect.width(),
                        BobUI::TextShowMnemonic);
}

/*!
    \class BOBUIabBar
    \brief The BOBUIabBar class provides a tab bar, e.g. for use in tabbed dialogs.

    \ingroup basicwidgets
    \inmodule BobUIWidgets

    BOBUIabBar is straightforward to use; it draws the tabs using one of
    the predefined \l{BOBUIabBar::Shape}{shapes}, and emits a
    signal when a tab is selected. It can be subclassed to tailor the
    look and feel. BobUI also provides a ready-made \l{BOBUIabWidget}.
    ...
    multi-page tool palette). The default in BOBUIabBar is \l
    RoundedNorth.

    The most important part of BOBUIabBar's API is the currentChanged()
    ...
    BOBUIabBar creates automatic mnemonic keys in the manner of QAbstractButton;
    ...
    \row \li \inlineimage {fusion-tabbar.png} {Tab bar with three tabs}
         \li A tab bar shown in the \l{BobUI Widget Gallery}{Fusion widget style}.
...
/*!
    \enum BOBUIabBar::Shape

    This enum type lists the built-in shapes supported by BOBUIabBar. Treat these
...
/*!
    \fn void BOBUIabBar::currentChanged(int index)
    ...
    (for example, if there are no tab in the BOBUIabBar)
*/

/*!
    \fn void BOBUIabBar::tabCloseRequested(int index)
    \since 4.5
...
/*!
    \fn void BOBUIabBar::tabBarClicked(int index)
...
/*!
    \fn void BOBUIabBar::tabBarDoubleClicked(int index)
...
void BOBUIabBarPrivate::init()
{
    Q_Q(BOBUIabBar);
    leftB = new BOBUIoolButton(q);
    leftB->setObjectName(u"ScrollLeftButton"_s);
    leftB->setAutoRepeat(true);
    QObjectPrivate::connect(leftB, &BOBUIoolButton::clicked,
                            this, &BOBUIabBarPrivate::scrollTabs);
    leftB->hide();
    rightB = new BOBUIoolButton(q);
    rightB->setObjectName(u"ScrollRightButton"_s);
    rightB->setAutoRepeat(true);
    QObjectPrivate::connect(rightB, &BOBUIoolButton::clicked,
                            this, &BOBUIabBarPrivate::scrollTabs);
    rightB->hide();
#ifdef BOBUI_KEYPAD_NAVIGATION
    if (QApplicationPrivate::keypadNavigationEnabled()) {
        leftB->setFocusPolicy(BobUI::NoFocus);
        rightB->setFocusPolicy(BobUI::NoFocus);
...
#if BOBUI_CONFIG(accessibility)
    leftB->setAccessibleName(BOBUIabBar::tr("Scroll Left"));
    rightB->setAccessibleName(BOBUIabBar::tr("Scroll Right"));
#endif
    q->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
    elideMode = BobUI::TextElideMode(q->style()->styleHint(QStyle::SH_TabBar_ElideMode, nullptr, q));
    useScrollButtons = !q->style()->styleHint(QStyle::SH_TabBar_PreferNoArrows, nullptr, q);
}

int BOBUIabBarPrivate::indexAtPos(const QPoint &p) const
{
    Q_Q(const BOBUIabBar);
...
void BOBUIabBarPrivate::layoutTabs()
{
    Q_Q(BOBUIabBar);
    layoutDirty = false;
    QSize size = q->size();
    ...
    BobUI::Alignment tabAlignment = BobUI::Alignment(q->style()->styleHint(QStyle::SH_TabBar_Alignment, nullptr, q));
    ...
    tabChain[tabChainIndex].expansive = (!expanding)
                                        && (tabAlignment != BobUI::AlignLeft)
                                        && (tabAlignment != BobUI::AlignJustify);
    ...
    if (useScrollButtons && tabList.size() && last > available) {
        ...
        if (vertTabs) {
            ...
            leftB->setArrowType(BobUI::UpArrow);
            rightB->setArrowType(BobUI::DownArrow);
        } else if (q->layoutDirection() == BobUI::RightToLeft) {
            ...
            leftB->setArrowType(BobUI::RightArrow);
            rightB->setArrowType(BobUI::LeftArrow);
        } else {
            scrollButtonLeftRect.setWidth(scrollButtonWidth);
            scrollButtonRightRect.setX(scrollButtonRightRect.right() + 1 - scrollButtonWidth);
            scrollButtonRightRect.setWidth(scrollButtonWidth);
            leftB->setArrowType(BobUI::LeftArrow);
            rightB->setArrowType(BobUI::RightArrow);
        }
...
QRect BOBUIabBarPrivate::normalizedScrollRect(int index)
{
    ...
    // always return the rect as horizontal BobUI::LeftToRight, even if the
    ...
    if (verticalTabs(shape)) {
        ...
        if (q->layoutDirection() == BobUI::RightToLeft) {
            scrollButtonLeftRect = QStyle::visualRect(BobUI::RightToLeft, q->rect(), scrollButtonLeftRect);
            scrollButtonRightRect = QStyle::visualRect(BobUI::RightToLeft, q->rect(), scrollButtonRightRect);
            tearLeftRect = QStyle::visualRect(BobUI::RightToLeft, q->rect(), tearLeftRect);
            tearRightRect = QStyle::visualRect(BobUI::RightToLeft, q->rect(), tearRightRect);
        }
...
int BOBUIabBarPrivate::hoveredTabIndex() const
{
    if (dragInProgress)
        return currentIndex;
    if (hoverIndex >= 0)
...
void BOBUIabBarPrivate::makeVisible(int index)
{
    Q_Q(BOBUIabBar);
    if (!validIndex(index))
        return;
...
void BOBUIabBarPrivate::killSwitchTabTimer()
{
    switchTabTimer.stop();
    switchTabCurrentIndex = -1;
}

void BOBUIabBarPrivate::layoutTab(int index)
{
    Q_Q(BOBUIabBar);
    Q_ASSERT(index >= 0);
...
void BOBUIabBarPrivate::layoutWidgets(int start)
{
    Q_Q(BOBUIabBar);
...
void BOBUIabBarPrivate::autoHideTabs()
{
    Q_Q(BOBUIabBar);
...
void BOBUIabBarPrivate::closeTab()
{
    Q_Q(BOBUIabBar);
    QObject *object = q->sender();
    int tabToClose = -1;
    BOBUIabBar::ButtonPosition closeSide = (BOBUIabBar::ButtonPosition)q->style()->styleHint(QStyle::SH_TabBar_CloseButtonPosition, nullptr, q);
    for (int i = 0; i < tabList.size(); ++i) {
        if (closeSide == BOBUIabBar::LeftSide) {
...
void BOBUIabBarPrivate::scrollTabs()
{
    Q_Q(BOBUIabBar);
...
void BOBUIabBarPrivate::refresh()
{
    Q_Q(BOBUIabBar);
...
*/
BOBUIabBar::BOBUIabBar(QWidget* parent)
    :QWidget(*new BOBUIabBarPrivate, parent, { })
{
    Q_D(BOBUIabBar);
    d->init();
}
...
*/
BOBUIabBar::~BOBUIabBar()
{
}

/*!
    \property BOBUIabBar::shape
...
BOBUIabBar::Shape BOBUIabBar::shape() const
{
    Q_D(const BOBUIabBar);
    return d->shape;
}

void BOBUIabBar::setShape(Shape shape)
{
    Q_D(BOBUIabBar);
    if (d->shape == shape)
        return;
...
bool BOBUIabBar::drawBase() const
{
    Q_D(const BOBUIabBar);
    return d->drawBase;
}
...
/*!
    ...
    tab's index.
*/
int BOBUIabBar::addTab(const QString &text)
{
...
*/
int BOBUIabBar::addTab(const QIcon& icon, const QString &text)
{
...
/*!
    ...
    tab's index.
*/
int BOBUIabBar::insertTab(int index, const QString &text)
{
...
/*!\overload
    ...
    If the BOBUIabBar was empty before this function is called, the inserted tab
...
*/
int BOBUIabBar::insertTab(int index, const QIcon& icon, const QString &text)
{
    Q_D(BOBUIabBar);
    if (!d->validIndex(index)) {
        index = d->tabList.size();
        d->tabList.append(new BOBUIabBarPrivate::Tab(icon, text));
    } else {
        d->tabList.insert(index, new BOBUIabBarPrivate::Tab(icon, text));
    }
 ...
 */
void BOBUIabBar::removeTab(int index)
{
    Q_D(BOBUIabBar);
    if (d->validIndex(index)) {
...
/*!
    ...
    returns \c false.
*/
bool BOBUIabBar::isTabEnabled(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
        return tab->enabled;
    return false;
}
...
*/
void BOBUIabBar::setTabEnabled(int index, bool enabled)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index)) {
        tab->enabled = enabled;
#ifndef BOBUI_NO_SHORTCUT
...
/*!
    ...
    \since 5.15
*/
bool BOBUIabBar::isTabVisible(int index) const
{
    Q_D(const BOBUIabBar);
    if (d->validIndex(index))
...
/*!
    ...
    \since 5.15
*/
void BOBUIabBar::setTabVisible(int index, bool visible)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index)) {
...
*/
QString BOBUIabBar::tabText(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
        return tab->text;
...
*/
void BOBUIabBar::setTabText(int index, const QString &text)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index)) {
...
/*!
    ...
    \sa setTabTextColor()
*/
QColor BOBUIabBar::tabTextColor(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
...
/*!
    ...
    If an invalid color is specified, the tab will use the BOBUIabBar foreground role instead.

    \sa tabTextColor()
*/
void BOBUIabBar::setTabTextColor(int index, const QColor &color)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index)) {
        tab->textColor = color;
...
*/
QIcon BOBUIabBar::tabIcon(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
        return tab->icon;
...
*/
void BOBUIabBar::setTabIcon(int index, const QIcon & icon)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index)) {
...
*/
void BOBUIabBar::setTabToolTip(int index, const QString & tip)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index))
        tab->toolTip = tip;
...
/*!
    ...
    string if \a index is out of range.
*/
QString BOBUIabBar::tabToolTip(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
        return tab->toolTip;
...
/*!
    ...
    to \a text.
*/
void BOBUIabBar::setTabWhatsThis(int index, const QString &text)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index))
        tab->whatsThis = text;
...
*/
QString BOBUIabBar::tabWhatsThis(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
...
*/
void BOBUIabBar::setTabData(int index, const QVariant & data)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index))
        tab->data = data;
...
*/
QVariant BOBUIabBar::tabData(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
        return tab->data;
...
*/
QRect BOBUIabBar::tabRect(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index)) {
        if (d->layoutDirty)
            const_cast<BOBUIabBarPrivate*>(d)->layoutTabs();
        if (!tab->visible)
            return QRect();
        QRect r = tab->rect;
...
int BOBUIabBar::tabAt(const QPoint &position) const
{
    Q_D(const BOBUIabBar);
...
void BOBUIabBar::setCurrentIndex(int index)
{
    Q_D(BOBUIabBar);
    ...
    if (auto tab = d->at(index)) {
        ...
        // will most certainly do the same). BOBUIBUG-6905
...
/*!
    ...
    \sa BOBUIabWidget::iconSize
*/
QSize BOBUIabBar::iconSize() const
{
    Q_D(const BOBUIabBar);
...
void BOBUIabBar::setIconSize(const QSize &size)
{
    Q_D(BOBUIabBar);
    d->iconSize = size;
    d->layoutDirty = true;
...
/*!
    \property BOBUIabBar::count
    \brief the number of tabs in the tab bar
*/

int BOBUIabBar::count() const
{
    Q_D(const BOBUIabBar);
...
/*!\reimp
 */
QSize BOBUIabBar::sizeHint() const
{
    Q_D(const BOBUIabBar);
    if (d->layoutDirty)
        const_cast<BOBUIabBarPrivate*>(d)->layoutTabs();
...
/*!\reimp
 */
QSize BOBUIabBar::minimumSizeHint() const
{
    Q_D(const BOBUIabBar);
    if (d->layoutDirty)
        const_cast<BOBUIabBarPrivate*>(d)->layoutTabs();
...
static QString computeElidedText(BobUI::TextElideMode mode, const QString &text)
{
    if (text.size() <= 3)
        return text;
...
QSize BOBUIabBar::minimumTabSizeHint(int index) const
{
    Q_D(const BOBUIabBar);
    BOBUIabBarPrivate::Tab *tab = d->tabList.at(index);
...
*/
QSize BOBUIabBar::tabSizeHint(int index) const
{
    ...
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index)) {
        ...
        if (it == d->textSizes.end())
           it = d->textSizes.insert(tab->text, fm.size(BobUI::TextShowMnemonic, tab->text));
...
/*!
  ...
  \sa tabRemoved()
 */
void BOBUIabBar::tabInserted(int index)
{
    Q_UNUSED(index);
}
...
/*!
  ...
  \sa tabInserted()
 */
void BOBUIabBar::tabRemoved(int index)
{
    Q_UNUSED(index);
}
...
/*!
  ...
  \sa tabRect()
 */
void BOBUIabBar::tabLayoutChange()
{
}
...
/*!\reimp
 */
void BOBUIabBar::showEvent(QShowEvent *)
{
    Q_D(BOBUIabBar);
    if (d->layoutDirty)
        d->refresh();
...
/*!\reimp
 */
void BOBUIabBar::hideEvent(QHideEvent *)
{
    Q_D(BOBUIabBar);
    d->updateMacBorderMetrics();
}
...
/*!\reimp
 */
bool BOBUIabBar::event(QEvent *event)
{
    Q_D(BOBUIabBar);
    switch (event->type()) {
...
#if BOBUI_CONFIG(tooltip)
    case QEvent::ToolTip:
        if (const BOBUIabBarPrivate::Tab *tab = d->at(tabAt(static_cast<QHelpEvent*>(event)->pos()))) {
            if (!tab->toolTip.isEmpty()) {
                BOBUIoolTip::showText(static_cast<QHelpEvent*>(event)->globalPos(), tab->toolTip, this);
                return true;
            }
        }
        break;
    ...
    case QEvent::QEvent::QueryWhatsThis: {
        const BOBUIabBarPrivate::Tab *tab = d->at(d->indexAtPos(static_cast<QHelpEvent*>(event)->pos()));
        if (!tab || tab->whatsThis.isEmpty())
            event->ignore();
        return true;
    }
    case QEvent::WhatsThis:
        if (const BOBUIabBarPrivate::Tab *tab = d->at(d->indexAtPos(static_cast<QHelpEvent*>(event)->pos()))) {
    ...
    case QEvent::Shortcut: {
        ...
        for (int i = 0; i < d->tabList.size(); ++i) {
            const BOBUIabBarPrivate::Tab *tab = d->tabList.at(i);
...
/*!\reimp
 */
void BOBUIabBar::resizeEvent(QResizeEvent *)
{
    Q_D(BOBUIabBar);
    if (d->layoutDirty)
        updateGeometry();
...
/*!\reimp
 */
void BOBUIabBar::paintEvent(QPaintEvent *)
{
    Q_D(BOBUIabBar);

    QStyleOptionTabBarBase optTabBase;
    BOBUIabBarPrivate::initStyleBaseOption(&optTabBase, this, size());
    ...
    // want the tab underneath to shine through, so clip the button area; BOBUIBUG-50866
    ...
    for (int i = 0; i < d->tabList.size(); ++i) {
        ...
        if (!tab->visible)
            continue;
        for (const auto side : { BOBUIabBar::LeftSide, BOBUIabBar::RightSide }) {
 ...
 */
void BOBUIabBarPrivate::calculateFirstLastVisible(int index, bool visible, bool remove)
{
    if (visible) {
 ...
 */
int BOBUIabBarPrivate::selectNewCurrentIndexFrom(int fromIndex)
{
    int newindex = -1;
 ...
 */
int BOBUIabBarPrivate::calculateNewPosition(int from, int to, int index) const
{
    if (index == from)
        return to;
 ...
 */
void BOBUIabBar::moveTab(int from, int to)
{
    Q_D(BOBUIabBar);
    if (from == to
        || !d->validIndex(from)
...
void BOBUIabBarPrivate::slide(int from, int to)
{
    Q_Q(BOBUIabBar);
    if (from == to
            || !validIndex(from)
...
void BOBUIabBarPrivate::moveTab(int index, int offset)
{
    if (!validIndex(index))
        return;
...
/*!\reimp
*/
void BOBUIabBar::mousePressEvent(QMouseEvent *event)
{
    Q_D(BOBUIabBar);
...
/*!\reimp
 */
void BOBUIabBar::mouseMoveEvent(QMouseEvent *event)
{
    Q_D(BOBUIabBar);
    if (d->movable) {
        // Be safe!
...
void BOBUIabBarPrivate::setupMovableTab()
{
    Q_Q(BOBUIabBar);
    if (!movingTab)
    ...
    grabImage.fill(BobUI::transparent);
...
void BOBUIabBarPrivate::moveTabFinished(int index)
{
    Q_Q(BOBUIabBar);
...
/*!\reimp
*/
void BOBUIabBar::mouseReleaseEvent(QMouseEvent *event)
{
    Q_D(BOBUIabBar);

    if (d->closeButtonOnTabs && event->button() == BobUI::MiddleButton) {
...
/*!\reimp
 */
void BOBUIabBar::mouseDoubleClickEvent(QMouseEvent *event)
{
    Q_D(BOBUIabBar);
    const QPoint pos = event->position().toPoint();
    const bool isEventInCornerButtons = (!d->leftB->isHidden() && d->leftB->geometry().contains(pos))
...
/*!\reimp
 */
void BOBUIabBar::keyPressEvent(QKeyEvent *event)
{
    Q_D(BOBUIabBar);
    ...
    int offset = event->key() == (isRightToLeft() ? BobUI::Key_Right : BobUI::Key_Left) ? -1 : 1;
 ...
 */
#if BOBUI_CONFIG(wheelevent)
void BOBUIabBar::wheelEvent(QWheelEvent *event)
{
    Q_D(BOBUIabBar);
...
void BOBUIabBarPrivate::setCurrentNextEnabledIndex(int offset)
{
    Q_Q(BOBUIabBar);
...
/*!\reimp
 */
void BOBUIabBar::changeEvent(QEvent *event)
{
    Q_D(BOBUIabBar);
    switch (event->type()) {
    case QEvent::StyleChange:
        if (!d->elideModeSetByUser)
            d->elideMode = BobUI::TextElideMode(style()->styleHint(QStyle::SH_TabBar_ElideMode, nullptr, this));
...
/*!
    \reimp
*/
void BOBUIabBar::timerEvent(BOBUIimerEvent *event)
{
    Q_D(BOBUIabBar);
...
/*!
    ...
    \sa BOBUIabWidget::elideMode, usesScrollButtons, QStyle::SH_TabBar_ElideMode
*/

BobUI::TextElideMode BOBUIabBar::elideMode() const
{
    Q_D(const BOBUIabBar);
    return d->elideMode;
}

void BOBUIabBar::setElideMode(BobUI::TextElideMode mode)
{
    Q_D(BOBUIabBar);
    d->elideMode = mode;
...
/*!
    \property BOBUIabBar::usesScrollButtons
    ...
    \sa elideMode, BOBUIabWidget::usesScrollButtons, QStyle::SH_TabBar_PreferNoArrows
*/
bool BOBUIabBar::usesScrollButtons() const
{
    return d_func()->useScrollButtons;
}

void BOBUIabBar::setUsesScrollButtons(bool useButtons)
{
    Q_D(BOBUIabBar);
...
/*!
    \property BOBUIabBar::tabsClosable
...
bool BOBUIabBar::tabsClosable() const
{
    Q_D(const BOBUIabBar);
    return d->closeButtonOnTabs;
}

void BOBUIabBar::setTabsClosable(bool closable)
{
    Q_D(BOBUIabBar);
...
/*!
    \enum BOBUIabBar::ButtonPosition
    \since 4.5
...
/*!
    \enum BOBUIabBar::SelectionBehavior
    \since 4.5

    This enum type lists the behavior of BOBUIabBar when a tab is removed
...
/*!
    \property BOBUIabBar::selectionBehaviorOnRemove
...
BOBUIabBar::SelectionBehavior BOBUIabBar::selectionBehaviorOnRemove() const
{
    Q_D(const BOBUIabBar);
    return d->selectionBehaviorOnRemove;
}

void BOBUIabBar::setSelectionBehaviorOnRemove(BOBUIabBar::SelectionBehavior behavior)
{
    Q_D(BOBUIabBar);
...
/*!
    \property BOBUIabBar::expanding
    \brief When expanding is true BOBUIabBar will expand the tabs to use the empty space.
    \since 4.5

    By default the value is true.

    \sa BOBUIabWidget::documentMode
*/

bool BOBUIabBar::expanding() const
{
    Q_D(const BOBUIabBar);
    return d->expanding;
}

void BOBUIabBar::setExpanding(bool enabled)
{
    Q_D(BOBUIabBar);
...
bool BOBUIabBar::isMovable() const
{
    Q_D(const BOBUIabBar);
    return d->movable;
}

void BOBUIabBar::setMovable(bool movable)
{
    Q_D(BOBUIabBar);
    d->movable = movable;
}
...
/*!
    \property BOBUIabBar::documentMode
    ...
    \sa BOBUIabWidget::documentMode
*/
bool BOBUIabBar::documentMode() const
{
    return d_func()->documentMode;
}

void BOBUIabBar::setDocumentMode(bool enabled)
{
    Q_D(BOBUIabBar);
...
bool BOBUIabBar::autoHide() const
{
    Q_D(const BOBUIabBar);
    return d->autoHide;
}

void BOBUIabBar::setAutoHide(bool hide)
{
    Q_D(BOBUIabBar);
    if (d->autoHide == hide)
        return;
...
/*!
    \property BOBUIabBar::changeCurrentOnDrag
...
bool BOBUIabBar::changeCurrentOnDrag() const
{
    Q_D(const BOBUIabBar);
    return d->changeCurrentOnDrag;
}

void BOBUIabBar::setChangeCurrentOnDrag(bool change)
{
    Q_D(BOBUIabBar);
...
/*!
    ...
    \sa tabsClosable()
  */
void BOBUIabBar::setTabButton(int index, ButtonPosition position, QWidget *widget)
{
    Q_D(BOBUIabBar);
...
/*!
    ...
    if one is not set.
  */
QWidget *BOBUIabBar::tabButton(int index, ButtonPosition position) const
{
    Q_D(const BOBUIabBar);
...
*/
void BOBUIabBar::setAccessibleTabName(int index, const QString &name)
{
    Q_D(BOBUIabBar);
    if (BOBUIabBarPrivate::Tab *tab = d->at(index)) {
...
/*!
    ...
    string if \a index is out of range.
*/
QString BOBUIabBar::accessibleTabName(int index) const
{
    Q_D(const BOBUIabBar);
    if (const BOBUIabBarPrivate::Tab *tab = d->at(index))
...
void CloseButton::paintEvent(QPaintEvent *)
{
    ...
    if (const BOBUIabBar *tb = qobject_cast<const BOBUIabBar *>(parent())) {
        int index = tb->currentIndex();
        BOBUIabBar::ButtonPosition position = (BOBUIabBar::ButtonPosition)style()->styleHint(QStyle::SH_TabBar_CloseButtonPosition, nullptr, tb);
...
#if BOBUI_CONFIG(animation)
void BOBUIabBarPrivate::Tab::TabBarAnimation::updateCurrentValue(const QVariant &current)
{
    priv->moveTab(priv->tabList.indexOf(tab), current.toInt());
}

void BOBUIabBarPrivate::Tab::TabBarAnimation::updateState(QAbstractAnimation::State newState, QAbstractAnimation::State)
{
...
#include "moc_bobuiabbar.cpp"
#include "bobuiabbar.moc"
