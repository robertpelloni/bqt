...
# bobui6_wrap_ui(outfiles inputfile ... )

function(bobui6_wrap_ui outfiles )
    set(options)
    set(oneValueArgs)
    ...
    foreach(it ${ui_files})
        ...
        add_custom_command(OUTPUT ${outfile}
          DEPENDS ${BOBUI_CMAKE_EXPORT_NAMESPACE}::uic
          COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::uic
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_wrap_ui outfiles)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_wrap_ui("${outfiles}" ${ARGN})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_wrap_ui("${outfiles}" ${ARGN})
        endif()
        set("${outfiles}" "${${outfiles}}" PARENT_SCOPE)
    endfunction()
endif()

function(bobui6_add_ui target)
    ...
    if(NOT "${raw_include_prefix}" STREQUAL "")
        ...
        # be `_/_`
        function(_bobui_internal_generate_dash_path_from_input input_path
        ...
        # correctly
        _bobui_internal_generate_dash_path_from_input("${raw_include_prefix}"
        ...
        # _bobui_internal_generate_dash_path_from_input() function
        if(WIN32)
            set(dummy_path_for_relative_calculation
        ...
        _bobui_internal_relative_path(raw_include_prefix_to_compare
            BASE_DIRECTORY "${dummy_path_for_relative_calculation}"
            OUTPUT_VARIABLE normalized_include_prefix
        )

        _bobui_internal_generate_dash_path_from_input("${normalized_include_prefix}"
            additional_path)
    endif()
    ...
    foreach(source_file ${sources})
        ...
        set(file_ui_folder "${CMAKE_CURRENT_BINARY_DIR}/.bobui/${short_hash}")
        ...
        add_custom_command(OUTPUT ${outfile}
            ...
            COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::uic ${ui_options} -o
            ${outfile} ${infile}
    ...
    # enabled after bobui_add_ui is called. If AUTOUIC is enabled, we show an
    # error message to the user.
    get_target_property(is_guard_on ${target} _bobui_ui_property_check_guard)
    if(NOT is_guard_on)
...
${CMAKE_COMMAND};-E;true>"
        ...
        if (NOT BOBUI_NO_MIX_UI_AUTOUIC_CHECK)
            set_target_properties(${target} PROPERTIES
                _bobui_ui_property_check_guard ON)
            add_custom_target(${target}_ui_property_check
                ALL DEPENDS ${ui_property_check_dummy_file})
            _bobui_internal_assign_to_internal_targets_folder(
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_ui)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_add_ui(${ARGN})
        else()
            message(FATAL_ERROR "bobui_add_ui() is only available in BobUI 6.")
        endif()
    endfunction()
endif()
