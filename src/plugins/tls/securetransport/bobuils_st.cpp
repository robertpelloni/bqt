...
// Defined in bobuilsbackend_st.cpp
...
void bobui_releaseSecureTransportContext(SSLContextRef context)
{
    if (context)
        CFRelease(context);
}
...
// To be also used by bobuilsbackend_st.cpp (thus not in unnamed namespace).
...
{
    bobui_releaseSecureTransportContext(context);
}
...
#if !defined(BOBUI_PLATFORM_UIKIT) // dhparam is only used on macOS. (see the SSLSetDiffieHellmanParams call below)
                                                     ...
                                                     char *data, size_t *dataLength)
{
    ...
    Q_ASSERT(socket->d);
    BOBUIcpSocket *plainSocket = socket->d->plainTcpSocket();
            ...
            for (int i = 0; i < requestedCount; ++i) {
                ...
                for (int j = 0; j < size; ++j) {
                    if (requestedName == peerProtocols[j]) {
                        BOBUIlsBackend::setNegotiatedProtocol(d, requestedName.toLatin1());
...
bool TlsCryptographSecureTransport::initSslContext()
{
    ...
    context.reset(bobui_createSecureTransportContext(mode));
    if (!context) {
...
#endif
            bool envOk = false;
            const int env = qEnvironmentVariableIntValue("BOBUI_SSL_USE_TEMPORARY_KEYCHAIN", &envOk);
            if (envOk && env) {
...
bool TlsCryptographSecureTransport::verifyPeerTrust()
{
    ...
    if (peerCertificateChain.size())
        BOBUIlsBackend::storePeerCertificate(d, peerCertificateChain.at(0));
