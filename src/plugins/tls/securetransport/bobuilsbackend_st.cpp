...
Q_GLOBAL_STATIC(QRecursiveMutex, bobui_securetransport_mutex)

Q_LOGGING_CATEGORY(lcSecureTransport, "bobui.tlsbackend.securetransport");

namespace BOBUIlsPrivate {

QList<QSslCertificate> systemCaCertificates(); // defined in qsslsocket_mac_shared.cpp

SSLContextRef bobui_createSecureTransportContext(QSslSocket::SslMode mode);
...
QSslCipher QSslCipher_from_SSLCipherSuite(SSLCipherSuite cipher)
{
    ...
    return BOBUIlsBackend::createCiphersuite(name, QSsl::TlsV1_2, "TLSv1.2"_L1);
}
...
{
    const QMutexLocker locker(bobui_securetransport_mutex());
    ...
    const BOBUIlsPrivate::QSecureTransportContext context(BOBUIlsPrivate::bobui_createSecureTransportContext(QSslSocket::SslClientMode));
    if (context) {
        ...
        for (size_t i = 0; i < size_t(cfCiphers.size()); ++i) {
            const QSslCipher ciph(BOBUIlsPrivate::QSslCipher_from_SSLCipherSuite(cfCiphers.at(i)));
            if (!ciph.isNull()) {
                ciphers << ciph;
...
BOBUIlsPrivate::TlsKey *QSecureTransportBackend::createKey() const
{
    return new BOBUIlsPrivate::TlsKeySecureTransport;
}

BOBUIlsPrivate::X509Certificate *QSecureTransportBackend::createCertificate() const
{
    return new BOBUIlsPrivate::X509CertificateSecureTransport;
}
...
BOBUIlsPrivate::X509PemReaderPtr QSecureTransportBackend::X509PemReader() const
{
    return BOBUIlsPrivate::X509CertificateGeneric::certificatesFromPem;
}

BOBUIlsPrivate::X509DerReaderPtr QSecureTransportBackend::X509DerReader() const
{
    return BOBUIlsPrivate::X509CertificateGeneric::certificatesFromDer;
}

BOBUIlsPrivate::TlsCryptograph *QSecureTransportBackend::createTlsCryptograph() const
{
    return new BOBUIlsPrivate::TlsCryptographSecureTransport;
}

BOBUI_END_NAMESPACE


#include "moc_bobuilsbackend_st_p.cpp"
