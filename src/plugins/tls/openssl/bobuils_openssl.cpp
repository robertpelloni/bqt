...
#include <BobUINetwork/private/qocspresponse_p.h>
    ...
    if (!ok) {
        ...
        if (!errors) {
            ...
            if (SSL *ssl = static_cast<SSL *>(q_X509_STORE_CTX_get_ex_data(
                ...
                const auto tlsOffset = BOBUIlsBackendOpenSSL::s_indexForSSLExtraData
...
{
    auto *tls = static_cast<TlsCryptographOpenSSL *>(q_SSL_get_ex_data(ssl, BOBUIlsBackendOpenSSL::s_indexForSSLExtraData));
                                         ...
                                         unsigned char *psk, unsigned max_psk_len)
{
    ...
    if (qEnvironmentVariableIsSet("BOBUI_USE_TLS_1_3_PSK"))
...
int bobui_OCSP_status_server_callback(SSL *ssl, void *ocspRequest)
{
    Q_UNUSED(ocspRequest);
    if (!ssl)
        return SSL_TLSEXT_ERR_ALERT_FATAL;

    auto crypto = static_cast<TlsCryptographOpenSSL *>(q_SSL_get_ex_data(ssl, BOBUIlsBackendOpenSSL::s_indexForSSLExtraData));
    if (!crypto)
        return SSL_TLSEXT_ERR_ALERT_FATAL;
...
void bobui_AlertInfoCallback(const SSL *connection, int from, int value)
{
...
QSslError::SslError bobui_OCSP_response_status_to_SslError(long code)
{
    switch (code) {
...
QOcspRevocationReason bobui_OCSP_revocation_reason(int reason)
{
    switch (reason) {
...
bool bobui_OCSP_certificate_match(OCSP_SINGLERESP *singleResponse, X509 *peerCert, X509 *issuer)
{
...
void TlsCryptographOpenSSL::startClientEncryption()
{
    if (!initSslContext()) {
        ...
        setErrorAndEmit(d, QAbstractSocket::SslInternalError,
                        QSslSocket::tr("Unable to init SSL Context: %1").arg(BOBUIlsBackendOpenSSL::getErrorsFromOpenSsl()));
        return;
    }
...
bool TlsCryptographOpenSSL::startHandshake()
{
    ...
    q_SSL_set_ex_data(ssl, BOBUIlsBackendOpenSSL::s_indexForSSLExtraData + errorOffsetInExData, &lastErrors);

    // SSL_set_ex_data can fail, but see the callback's code - we handle this there.
    q_SSL_set_ex_data(ssl, BOBUIlsBackendOpenSSL::s_indexForSSLExtraData + socketOffsetInExData, this);
    q_SSL_set_info_callback(ssl, bobui_AlertInfoCallback);

    int result = (mode == QSslSocket::SslClientMode) ? q_SSL_connect(ssl) : q_SSL_accept(ssl);
    q_SSL_set_ex_data(ssl, BOBUIlsBackendOpenSSL::s_indexForSSLExtraData + errorOffsetInExData, nullptr);
    ...
    if (!errorsReportedFromCallback) {
        ...
        for (const auto &currentError : std::as_const(lastErrors)) {
            emit q->peerVerifyError(BOBUIlsPrivate::X509CertificateOpenSSL::openSSLErrorToQSslError(currentError.code,
    ...
    if (result <= 0) {
        ...
        case SSL_ERROR_WANT_WRITE:
            ...
            break;
        default:
            QString errorString = BOBUIlsBackendOpenSSL::msgErrorsDuringHandshake();
#ifdef QSSLSOCKET_DEBUG
...
void TlsCryptographOpenSSL::continueHandshake()
{
    ...
    if (q_SSL_session_reused(ssl))
        BOBUIlsBackend::setPeerSessionShared(d, true);

#ifdef BOBUI_DECRYPT_SSL_TRAFFIC
    if (q_SSL_get_session(ssl)) {
        ...
        QByteArray masterKey(int(master_key_len), BobUI::Uninitialized); // Will not overflow
        QByteArray clientRandom(int(client_random_len), BobUI::Uninitialized); // Will not overflow
        ...
        QString sslKeyFile = QDir::tempPath() + "/bobui-ssl-keys"_L1;
...
#endif // BOBUI_DECRYPT_SSL_TRAFFIC
        ...
        if (!sslContextPointer->cacheSession(ssl)) {
            ...
            if (!(configuration.testSslOption(QSsl::SslOptionDisableSessionPersistence))) {
                if (!sslContextPointer->sessionASN1().isEmpty())
                    BOBUIlsBackend::setSessionAsn1(d, sslContextPointer->sessionASN1());
                BOBUIlsBackend::setSessionLifetimeHint(d, sslContextPointer->sessionTicketLifeTimeHint());
            }
        }
    }
    ...
    BOBUIlsBackend::setAlpnStatus(d, sslContextPointer->npnContext().status);
    if (sslContextPointer->npnContext().status == QSslConfiguration::NextProtocolNegotiationUnsupported) {
        ...
        BOBUIlsBackend::setNegotiatedProtocol(d, QByteArrayLiteral("http/1.1"));
    } else {
        ...
        if (proto_len && mode == QSslSocket::SslClientMode) {
            ...
            BOBUIlsBackend::setAlpnStatus(d, QSslConfiguration::NextProtocolNegotiationNegotiated);
        }
        ...
        if (proto_len)
            BOBUIlsBackend::setNegotiatedProtocol(d, QByteArray(reinterpret_cast<const char *>(proto), proto_len));
        else
            BOBUIlsBackend::setNegotiatedProtocol(d,{});
    }
    ...
    if (mode == QSslSocket::SslClientMode) {
        ...
        if (q_SSL_get_server_tmp_key(ssl, &key))
            BOBUIlsBackend::setEphemeralKey(d, QSslKey(key, QSsl::PublicKey));
    }
                ...
                if (writtenBytes <= 0) {
                    ...
                    if (error == SSL_ERROR_WANT_WRITE) {
                        ...
                        setErrorAndEmit(d, QAbstractSocket::SslInternalError,
                                        ...
                                        BOBUIlsBackendOpenSSL::getErrorsFromOpenSsl()));
                        return;
                    }
                }
#ifdef QSSLSOCKET_DEBUG
        ...
        do {
            ...
            if (renegotiated) {
                ...
                const auto peerCertificate =
                        BOBUIlsPrivate::X509CertificateOpenSSL::certificateFromX509(x509);
...
QSslCipher TlsCryptographOpenSSL::sessionCipher() const
{
    ...
    return sessionCipher ? BOBUIlsBackendOpenSSL::bobui_OpenSSL_cipher_to_QSslCipher(sessionCipher) : QSslCipher{};
}
...
int TlsCryptographOpenSSL::handleNewSessionTicket(SSL *connection)
{
    ...
    BOBUIlsBackend::setSessionAsn1(d, sessionTicket);
    BOBUIlsBackend::setSessionLifetimeHint(d, q_SSL_SESSION_get_ticket_lifetime_hint(currentSession));
...
int TlsCryptographOpenSSL::emitErrorFromCallback(X509_STORE_CTX *ctx)
{
    ...
    const QSslCertificate certificate = BOBUIlsPrivate::X509CertificateOpenSSL::certificateFromX509(x509);
    const auto errorAndDepth = BOBUIlsPrivate::X509CertificateOpenSSL::errorEntryFromStoreContext(ctx);
    const QSslError tlsError = BOBUIlsPrivate::X509CertificateOpenSSL::openSSLErrorToQSslError(errorAndDepth.code, certificate);
...
bool TlsCryptographOpenSSL::initSslContext()
{
    ...
    if (!(ssl = sslContextPointer->createSsl())) {
        setErrorAndEmit(d, QAbstractSocket::SslInternalError,
                        QSslSocket::tr("Error creating SSL session, %1").arg(BOBUIlsBackendOpenSSL::getErrorsFromOpenSsl()));
        return false;
    }
    ...
    q_SSL_set_ex_data(ssl, BOBUIlsBackendOpenSSL::s_indexForSSLExtraData, this);
    ...
    auto responsePos = backendConfig.find("BobUI-OCSP-response");
...
void TlsCryptographOpenSSL::destroySslContext()
{
    if (ssl) {
        if (!q_SSL_in_init(ssl) && !systemOrSslErrorDetected) {
            ...
            if (q_SSL_shutdown(ssl) != 1) {
                // Some error may be queued, clear it.
                const auto errors = BOBUIlsBackendOpenSSL::getErrorsFromOpenSsl();
                Q_UNUSED(errors);
            }
        }
        q_SSL_free(ssl);
...
void TlsCryptographOpenSSL::storePeerCertificates()
{
    ...
    BOBUIlsBackend::storePeerCertificate(d, peerCertificate);
    q_X509_free(x509);
    auto peerCertificateChain = q->peerCertificateChain();
    if (peerCertificateChain.isEmpty()) {
        peerCertificateChain = BOBUIlsPrivate::X509CertificateOpenSSL::stackOfX509ToQSslCertificates(q_SSL_get_peer_cert_chain(ssl));
        if (!peerCertificate.isNull() && d->tlsMode() == QSslSocket::SslServerMode)
            peerCertificateChain.prepend(peerCertificate);
        BOBUIlsBackend::storePeerCertificateChain(d, peerCertificateChain);
    }
}
...
bool TlsCryptographOpenSSL::checkOcspStatus()
{
    ...
    if (ocspStatus != OCSP_RESPONSE_STATUS_SUCCESSFUL) {
        ...
        ocspErrors.push_back(QSslError(bobui_OCSP_response_status_to_SslError(ocspStatus)));
        return false;
    }
    ...
    if (dResponse->subjectCert.isSelfSigned()) {
        ...
        matchFound = bobui_OCSP_certificate_match(singleResponse, peerX509, peerX509);
    } else {
        ...
        if (certs) {
            ...
            for (int i = 0, e = q_sk_X509_num(certs); i < e; ++i) {
                ...
                matchFound = bobui_OCSP_certificate_match(singleResponse, peerX509, issuer);
                if (matchFound) {
                    if (q_X509_check_issued(issuer, peerX509) == X509_V_OK) {
                        dResponse->signerCert =  BOBUIlsPrivate::X509CertificateOpenSSL::certificateFromX509(issuer);
                        break;
                    }
                    matchFound = false;
                }
    ...
    case V_OCSP_CERTSTATUS_REVOKED:
        ...
        dResponse->revocationReason = bobui_OCSP_revocation_reason(reason);
                                                     ...
                                                     unsigned char *psk, unsigned max_psk_len)
{
    ...
    BOBUIlsBackend::setupClientPskAuth(&authenticator, hint, hintLength, max_identity_len, max_psk_len);
                                                     ...
                                                     unsigned max_psk_len)
{
    ...
    // Fill in some read-only fields (for the user)
    BOBUIlsBackend::setupServerPskAuth(&authenticator, identity, q->sslConfiguration().preSharedKeyIdentityHint(),
                                    max_psk_len);
    emit q->preSharedKeyAuthenticationRequired(&authenticator);
...
void TlsCryptographOpenSSL::fetchCaRootForCert(const QSslCertificate &cert)
{
    ...
    QMetaObject::invokeMethod(fetcher, "start", BobUI::QueuedConnection);
    QSslSocketPrivate::pauseSocketNotifiers(q);
    d->setPaused(true);
}
...
void TlsCryptographOpenSSL::caRootLoaded(QSslCertificate cert, QSslCertificate trustedRoot)
{
    ...
    if (!trustedRoot.isNull() && !trustedRoot.isBlacklisted()) {
        ...
        BOBUIlsBackend::addTustedRoot(d, trustedRoot);
