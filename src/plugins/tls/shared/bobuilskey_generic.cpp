...
#include "bobuilskey_generic_p.h"
...
// The code here is essentially what we had in qsslkey_bobui.cpp before, with
    ...
    */
    ...
    if (iterator == oidCipherMap.cend()) {
        qWarning()
            ...
            << "\nFile a bug report to BobUI (include the line above).";
        return {};
    }
    ...
    if (Q_LIKELY(keyDerivationAlgorithm == PKCS5_PBKDF2_ENCRYPTION_OID)) {
        ...
        qWarning()
            ...
            << "\nFile a bugreport to BobUI (include the line above).";
        return {};
    }
                         ...
                         const QByteArray &passPhrase)
{
    ...
    */
    ...
    if (iterator == pbes1OidHashFunctionMap.cend()) {
        // BobUI was compiled with ONLY_SHA1 (or it's MD2)
        return {};
    }
...
void TlsKeyGeneric::fromHandle(BobUI::HANDLE handle, KeyType expectedType)
{
    opaque = handle;
