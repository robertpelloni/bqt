...
#include <BobUINetwork/qsslcertificateextension.h>
...
Q_LOGGING_CATEGORY(lcTlsBackendSchannel, "bobui.tlsbackend.schannel");

// Defined in qsslsocket_bobui.cpp.
...
BOBUI_WARNING_POP
                ...
                ciphers.append(BOBUIlsBackend::createCiphersuite(QLatin1StringView(cipher.openSslCipherSuite),
...
Q_GLOBAL_STATIC(QRecursiveMutex, bobui_schannel_mutex)
...
{
    const QMutexLocker<QRecursiveMutex> locker(bobui_schannel_mutex);
...
{
    setDefaultSupportedCiphers(BOBUIlsPrivate::defaultCiphers());
    setDefaultCiphers(BOBUIlsPrivate::defaultCiphers());
}
...
BOBUIlsPrivate::TlsKey *QSchannelBackend::createKey() const
{
    return new BOBUIlsPrivate::TlsKeySchannel;
}

BOBUIlsPrivate::X509Certificate *QSchannelBackend::createCertificate() const
{
    return new BOBUIlsPrivate::X509CertificateSchannel;
}
...
BOBUIlsPrivate::TlsCryptograph *QSchannelBackend::createTlsCryptograph() const
{
    return new BOBUIlsPrivate::TlsCryptographSchannel;
}

QList<QSslCertificate> QSchannelBackend::systemCaCertificatesImplementation()
{
    // Similar to non-Darwin version found in bobuilsbackend_openssl.cpp,
    // BOBUIlsPrivate::systemCaCertificates function.
    ...
    if (hSystemStore) {
        ...
        while ((pc = CertFindCertificateInStore(hSystemStore.get(), X509_ASN_ENCODING, 0,
            ...
            systemCerts.append(BOBUIlsPrivate::X509CertificateSchannel::QSslCertificate_from_CERT_CONTEXT(pc));
        }
    }
    return systemCerts;
}

BOBUIlsPrivate::X509PemReaderPtr QSchannelBackend::X509PemReader() const
{
    return BOBUIlsPrivate::X509CertificateGeneric::certificatesFromPem;
}

BOBUIlsPrivate::X509DerReaderPtr QSchannelBackend::X509DerReader() const
{
    return BOBUIlsPrivate::X509CertificateGeneric::certificatesFromDer;
}

BOBUIlsPrivate::X509Pkcs12ReaderPtr QSchannelBackend::X509Pkcs12Reader() const
{
    return BOBUIlsPrivate::X509CertificateSchannel::importPkcs12;
}
...
/*!
    ...
    BobUI's own SslProtocol type.

    Only one protocol should be passed in at a time.
*/
QSsl::SslProtocol toBobUISslProtocol(DWORD protocol)
{
...
qint64 readToBuffer(QByteArray &buffer, BOBUIcpSocket *plainSocket)
{
...
{
    return qEnvironmentVariableIsSet("BOBUI_SCH_DEFAULT_CREDS") ? 0 : SCH_CRED_NO_DEFAULT_CREDS;
}
} // anonymous namespace
...
bool TlsCryptographSchannel::createContext()
{
    ...
    bool useAlpn = false;
#ifdef SUPPORTS_ALPN
    BOBUIlsBackend::setAlpnStatus(d, QSslConfiguration::NextProtocolNegotiationNone);
...
#define CHECK_STATUS(status)                                                  \
    ...
    if (!allowedProtos.isEmpty()) {
        ...
        if (alpn.ProtoNegoStatus == SecApplicationProtocolNegotiationStatus_Success) {
            ...
            BOBUIlsBackend::setNegotiatedProtocol(d, negotiatedProto);
            BOBUIlsBackend::setAlpnStatus(d, QSslConfiguration::NextProtocolNegotiationNegotiated);
        } else {
            BOBUIlsBackend::setNegotiatedProtocol(d, {});
            BOBUIlsBackend::setAlpnStatus(d, QSslConfiguration::NextProtocolNegotiationUnsupported);
        }
    }
#endif // supports ALPN
...
QSslCipher TlsCryptographSchannel::sessionCipher() const
{
    ...
    const auto sessionProtocol = toBobUISslProtocol(connectionInfo.dwProtocol);
...
QSsl::SslProtocol TlsCryptographSchannel::sessionProtocol() const
{
    ...
    return toBobUISslProtocol(connectionInfo.dwProtocol);
}
    ...
    if (!configuration.localCertificateChain().isEmpty()) {
        ...
        if (localCertificateStore == nullptr) {
            ...
            if (localCertificateStore) {
                ...
                if (certificateContext) {
                    auto *backend = BOBUIlsBackend::backend<X509CertificateSchannel>(
...
bool TlsCryptographSchannel::verifyCertContext(CERT_CONTEXT *certContext)
{
    ...
    BOBUIlsBackend::clearPeerCertificates(d);
    ...
    static auto getCertificateFromChainElement = [](CERT_CHAIN_ELEMENT *element) {
        ...
        return BOBUIlsPrivate::X509CertificateSchannel::QSslCertificate_from_CERT_CONTEXT(certContext);
    };
    ...
    if (!peerCertificateChain.isEmpty())
        BOBUIlsBackend::storePeerCertificate(d, peerCertificateChain.constFirst());

    const auto &configuration = q->sslConfiguration(); // Probably, updated by BOBUIlsBackend::storePeerCertificate etc.
