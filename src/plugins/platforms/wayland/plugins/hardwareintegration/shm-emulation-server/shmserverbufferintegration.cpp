...
static QOpenGLTexture *createTextureFromShm(const QString &key, int w, int h, int bpl, int format)
{
    ...
    switch (format) {
        case BobUIWayland::bobui_shm_emulation_server_buffer::format_RGBA32:
            imageFormat = QImage::Format_RGBA8888;
            break;
        case BobUIWayland::bobui_shm_emulation_server_buffer::format_A8:
...
QWaylandServerBuffer *ShmServerBufferIntegration::serverBuffer(struct bobui_server_buffer *buffer)
{
...
{
    Q_UNUSED(version);
    if (interface == "bobui_shm_emulation_server_buffer") {
        auto *integration = static_cast<ShmServerBufferIntegration *>(data);
        integration->BobUIWayland::bobui_shm_emulation_server_buffer::init(registry, id, 1);
    }
}


void BobUIWaylandClient::ShmServerBufferIntegration::shm_emulation_server_buffer_server_buffer_created(bobui_server_buffer *id, const QString &key, int32_t width, int32_t height, int32_t bytes_per_line, int32_t format)
{
