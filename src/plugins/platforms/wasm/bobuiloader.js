 ...
 *
 * @param config May contain any key normally accepted by emscripten and the 'bobui' extra key, with
 ...
 *      Array of host elements for BobUI screens. Each of these elements is mapped to a QScreen on
 *      launch.
 * - fontDpi: number
 ...
 *      BobUI always uses emscripten's MODULARIZE option. This is the MODULARIZE entry function.
 ...
 * - bobuidir: string
 *      Path to BobUI installation. This path will be used for loading BobUI shared libraries and plugins.
 *      The path is set to 'bobui' by default, and is relative to the path of the web page's html file.
 ...
 *      an absolute path.
 *
 *      $BOBUIDIR may be used as a placeholder for the "bobuidir" configuration property (see @bobuidir), for instance:
 *          "source": "$BOBUIDIR/plugins/imageformats/libqjpeg.so"
 *  - localFonts.requestPermission: bool
 *       Whether BobUI should request for local fonts access permission on startup (default false).
 ...
 *      EmscriptenModule
 */
async function bobuiLoad(config)
{
    const throwIfEnvUsedButNotExported = (instance, config) =>
    {
        const environment = config.bobui.environment;
        ...
        if (!isEnvExported) {
            throw new Error('ENV must be exported if environment variables are passed, ' +
                            'add it to the BOBUI_WASM_EXTRA_EXPORTED_METHODS CMake target property');
        }
    };

    if (typeof config !== 'object')
        throw new Error('config is required, expected an object');
    if (typeof config.bobui !== 'object')
        throw new Error('config.bobui is required, expected an object');
    if (typeof config.bobui.entryFunction !== 'function')
        throw new Error('config.bobui.entryFunction is required, expected a function');

    config.bobui.bobuidir ??= 'bobui';
    config.bobui.preload ??= [];

    config.bobuiContainerElements = config.bobui.containerElements;
    delete config.bobui.containerElements;
    config.bobuiFontDpi = config.bobui.fontDpi;
    delete config.bobui.fontDpi;
    ...
    if (config.bobui.module) {
        config.instantiateWasm = async (imports, successCallback) =>
        {
            try {
                const module = await config.bobui.module;
                successCallback(
    ...
    const filesToPreload = (await Promise.all(config.bobui.preload.map(preloadFetchHelper))).flat();
    const bobuiPreRun = (instance) => {
        // Copy bobui.environment to instance.ENV
        throwIfEnvUsedButNotExported(instance, config);
        for (const [name, value] of Object.entries(config.bobui.environment ?? {}))
            instance.ENV[name] = value;

        // Preload files from bobui.preload
        ...
        const preloadFile = (file) => {
            ...
            const source = file.source.replace('$BOBUIDIR', config.bobui.bobuidir);
    ...
    config.locateFile = filename => {
        ...
        if (originalLocatedFilename.startsWith(
                    'libBobUI6')) // wasmbobuideploy rely on this behavior, update both in case of change
            return `${config.bobui.bobuidir}/lib/${originalLocatedFilename}`;
    ...
    try {
        instance = await Promise.race(
            [circuitBreaker, config.bobui.entryFunction(config)]);
...
// and will be removed in a future version of BobUI.
function BobUILoader(bobuiConfig) {

    const warning = 'Warning: The BobUILoader API is deprecated and will be removed in ' +
                    'a future version of BobUI. Please port to the new bobuiLoad() API.';
    console.warn(warning);

    let emscriptenConfig = bobuiConfig.moduleConfig || {}
    bobuiConfig.moduleConfig = undefined;
    const showLoader = bobuiConfig.showLoader;
    bobuiConfig.showLoader = undefined;
    const showError = bobuiConfig.showError;
    bobuiConfig.showError = undefined;
    const showExit = bobuiConfig.showExit;
    bobuiConfig.showExit = undefined;
    const showCanvas = bobuiConfig.showCanvas;
    bobuiConfig.showCanvas = undefined;
    if (bobuiConfig.canvasElements) {
        bobuiConfig.containerElements = bobuiConfig.canvasElements
        bobuiConfig.canvasElements = undefined;
    } else {
        bobuiConfig.containerElements = bobuiConfig.containerElements;
        bobuiConfig.containerElements = undefined;
    }
    emscriptenConfig.bobui = bobuiConfig;
    ...
    bobuiConfig.onLoaded = () => {
        showCanvas?.();
    }

    bobuiConfig.onExit = exit => {
        bobuiloader.exitCode = exit.code
        bobuiloader.exitText = exit.text;
        showExit?.();
    }
