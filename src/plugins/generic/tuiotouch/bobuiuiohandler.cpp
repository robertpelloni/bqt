...
#include "bobuiuiocursor_p.h"
#include "bobuiuiotoken_p.h"
...
Q_LOGGING_CATEGORY(lcTuioHandler, "bobui.qpa.tuio.handler")
Q_LOGGING_CATEGORY(lcTuioSource, "bobui.qpa.tuio.source")
Q_LOGGING_CATEGORY(lcTuioSet, "bobui.qpa.tuio.set")
...
static bool forceDelivery = qEnvironmentVariableIsSet("BOBUI_TUIOTOUCH_DELIVER_WITHOUT_FOCUS");

BOBUIuioHandler::BOBUIuioHandler(const QString &specification)
{
    ...
    if (rotationAngle)
        m_transform = BOBUIransform::fromTranslate(0.5, 0.5).rotate(rotationAngle).translate(-0.5, -0.5);

    if (invertx)
        m_transform *= BOBUIransform::fromTranslate(0.5, 0.5).scale(-1.0, 1.0).translate(-0.5, -0.5);
    ...
    connect(&m_socket, &QUdpSocket::readyRead, this, &BOBUIuioHandler::processPackets);
}

BOBUIuioHandler::~BOBUIuioHandler()
{
}

void BOBUIuioHandler::processPackets()
{
...
void BOBUIuioHandler::process2DCurSource(const QOscMessage &message)
{
    QList<QVariant> arguments = message.arguments();
    if (arguments.size() != 2) {
...
void BOBUIuioHandler::process2DCurAlive(const QOscMessage &message)
{
    QList<QVariant> arguments = message.arguments();
    ...
    QMap<int, BOBUIuioCursor> oldActiveCursors = m_activeCursors;
    QMap<int, BOBUIuioCursor> newActiveCursors;
    ...
    for (int i = 1; i < arguments.size(); ++i) {
        ...
        if (!oldActiveCursors.contains(cursorId)) {
            // newly active
            BOBUIuioCursor cursor(cursorId);
            ...
            BOBUIuioCursor cursor = oldActiveCursors.value(cursorId);
    ...
    QMap<int, BOBUIuioCursor>::ConstIterator it = oldActiveCursors.constBegin();
...
void BOBUIuioHandler::process2DCurSet(const QOscMessage &message)
{
    QList<QVariant> arguments = message.arguments();
    if (arguments.size() < 7) {
    ...
    QMap<int, BOBUIuioCursor>::Iterator it = m_activeCursors.find(cursorId);
...
QWindowSystemInterface::TouchPoint BOBUIuioHandler::cursorToTouchPoint(const BOBUIuioCursor &tc, QWindow *win)
{
...
void BOBUIuioHandler::process2DCurFseq(const QOscMessage &message)
{
    ...
    for (const BOBUIuioCursor &tc : std::as_const(m_activeCursors)) {
        QWindowSystemInterface::TouchPoint tp = cursorToTouchPoint(tc, win);
        tpl.append(tp);
    }

    for (const BOBUIuioCursor &tc : std::as_const(m_deadCursors)) {
        QWindowSystemInterface::TouchPoint tp = cursorToTouchPoint(tc, win);
...
void BOBUIuioHandler::process2DObjSource(const QOscMessage &message)
{
    QList<QVariant> arguments = message.arguments();
    if (arguments.size() != 2) {
        qCWarning(lcTuioSource ) << "Ignoring malformed TUIO source message: " << arguments.size();
...
void BOBUIuioHandler::process2DObjAlive(const QOscMessage &message)
{
    QList<QVariant> arguments = message.arguments();
    ...
    // new data source from the input. is this correct, or do we need to store
    // changes and only process the deltas on fseq?
    QMap<int, BOBUIuioToken> oldActiveTokens = m_activeTokens;
    QMap<int, BOBUIuioToken> newActiveTokens;
    ...
    for (int i = 1; i < arguments.size(); ++i) {
        ...
        if (!oldActiveTokens.contains(sessionId)) {
            // newly active
            BOBUIuioToken token(sessionId);
            token.setState(QEventPoint::State::Pressed);
            newActiveTokens.insert(sessionId, token);
        } else {
            // we already know about it, remove it so it isn't marked as released
            BOBUIuioToken token = oldActiveTokens.value(sessionId);
    ...
    // anything left is dead now
    QMap<int, BOBUIuioToken>::ConstIterator it = oldActiveTokens.constBegin();
...
void BOBUIuioHandler::process2DObjSet(const QOscMessage &message)
{
    QList<QVariant> arguments = message.arguments();
    if (arguments.size() < 7) {
        qCWarning(lcTuioSet) << "Ignoring malformed TUIO set message with too few arguments: " << arguments.size();
    ...
    QMap<int, BOBUIuioToken>::Iterator it = m_activeTokens.find(id);
...
QWindowSystemInterface::TouchPoint BOBUIuioHandler::tokenToTouchPoint(const BOBUIuioToken &tc, QWindow *win)
{
    QWindowSystemInterface::TouchPoint tp;
    tp.id = tc.id();
...
void BOBUIuioHandler::process2DObjFseq(const QOscMessage &message)
{
    Q_UNUSED(message); // TODO: do we need to do anything with the frame id?
    ...
    for (const BOBUIuioToken & t : std::as_const(m_activeTokens)) {
        QWindowSystemInterface::TouchPoint tp = tokenToTouchPoint(t, win);
        tpl.append(tp);
    }

    for (const BOBUIuioToken & t : std::as_const(m_deadTokens)) {
        QWindowSystemInterface::TouchPoint tp = tokenToTouchPoint(t, win);
        tp.state = QEventPoint::State::Released;
...
#include "moc_bobuiuiohandler_p.cpp"
