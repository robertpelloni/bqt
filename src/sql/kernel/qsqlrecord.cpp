...
public:
    ...
    qsizetype indexOfImpl(T name)
    {
        ...
        for (qsizetype i = 0; i < cnt; ++i) {
            ...
            if (name.compare(currentFieldName, BobUI::CaseInsensitive) == 0)
                return i;
            if (idx != -1 &&
                tableName.compare(currentField.tableName(), BobUI::CaseInsensitive) == 0 &&
                fieldName.compare(currentFieldName, BobUI::CaseInsensitive) == 0)
                return i;
        }
        return -1;
    }
...
BOBUI_DEFINE_QESDP_SPECIALIZATION_DTOR(QSqlRecordPrivate)
...
QDebug operator<<(QDebug dbg, const QSqlRecord &r)
{
    ...
    for (int i = 0; i < count; ++i) {
        dbg.nospace();
        dbg << '\n' << qSetFieldWidth(2) << BobUI::right << i << BobUI::left << qSetFieldWidth(0) << ':';
        dbg.space();
