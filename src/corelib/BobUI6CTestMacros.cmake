...
set(_bobui_internal_skip_build_test_env_var
    "BOBUI_CMAKE_SKIP_BUILD_TESTS")
set(_bobui_internal_skip_build_test_message
    "Skipping build test as requested by ${_bobui_internal_skip_build_test_env_var}")
set(_bobui_internal_skip_build_test_regex
    "^${_bobui_internal_skip_build_test_message}\n$")
set(_bobui_internal_skip_build_test_pre_run
    "if(DEFINED ENV{${_bobui_internal_skip_build_test_env_var}})"
      "message(\"${_bobui_internal_skip_build_test_message}\")"
      "return()"
    "endif()"
)
...
function(_bobui_internal_get_ninja_wrapper ninja_path out_wrapper_path)
    if(BOBUI_INTERNAL_CTEST_NINJA_WRAPPER AND EXISTS "${BOBUI_INTERNAL_CTEST_NINJA_WRAPPER}")
        set(${out_wrapper_path} "${BOBUI_INTERNAL_CTEST_NINJA_WRAPPER}" PARENT_SCOPE)
        return()
    endif()
    ...
    set(script_name "bobui-internal-ninja")
    ...
    set(wrapper_out "${CMAKE_BINARY_DIR}/.bobui/${script_name}${wrapper_extension}")
    ...
    set(BOBUI_INTERNAL_CTEST_NINJA_WRAPPER "${wrapper_out}" CACHE STRING
        "Internal BobUI ninja wrapper for ctest tests")

    set(${out_wrapper_path} "${BOBUI_INTERNAL_CTEST_NINJA_WRAPPER}" PARENT_SCOPE)
endfunction()

# The function collects configuring options for the test projects generated by BobUI cmake tests.
# Arguments:
#   OUT_PREFIX_PATH <variable name>: stores the CMAKE_PREFIX_PATH value in the output variable.
function(_bobui_internal_get_cmake_test_configure_options out_var)
    ...
    if(BOBUI_BUILD_DIR)
        list(APPEND option_list "-DBOBUI_BUILD_DIR=${BOBUI_BUILD_DIR}")
    endif()

    # Forward whatever hints were used in find_package(BobUI6) to the ctest configure
    foreach(hint IN ITEMS BobUI6_ROOT BOBUI6_ROOT)
        if(DEFINED ${hint})
    ...
    list(APPEND option_list "-DBOBUI_INTERNAL_IS_CMAKE_BUILD_TEST=ON")

    if(APPLE AND CMAKE_OSX_ARCHITECTURES)
        list(LENGTH CMAKE_OSX_ARCHITECTURES osx_arch_count)

        # When BobUI is built as universal config (macOS or iOS), force CMake build tests to build one
        ...
        if(osx_arch_count GREATER 1)
            list(APPEND option_list "-DBOBUI_FORCE_SINGLE_BOBUI_OSX_ARCHITECTURE=ON")
        endif()
    endif()
    ...
    list(APPEND option_list "-DBOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH=${additional_prefixes}")

    set(${out_var} "${option_list}" PARENT_SCOPE)
endfunction()

function(_bobui_internal_set_up_test_run_environment testname)
    ...
    # This is copy-pasted from bobui_add_test and adapted to the standalone project case.
    ...
    # If part of BobUI build or standalone tests, use the build internals install prefix.
    # If the tests are configured as a separate project, use the BobUI6 package provided install
    # prefix.
    ...
    foreach(install_prefix ${install_prefixes})
        set(test_env_path "${test_env_path}${BOBUI_PATH_SEPARATOR}${install_prefix}/${INSTALL_BINDIR}")
    endforeach()
    ...
    set_property(TEST "${testname}" APPEND PROPERTY ENVIRONMENT "BOBUI_TEST_RUNNING_IN_CTEST=1")
    ...
    if(NOT arg_NO_PLUGIN_PATH)
        ...
        set_property(TEST "${testname}"
            APPEND PROPERTY ENVIRONMENT "BOBUI_PLUGIN_PATH=${plugin_paths_joined}")
    endif()
endfunction()
...
macro(_bobui_internal_test_expect_pass _dir)
    if(WASM)
        return()
    endif()
    ...
    elseif(CMAKE_GENERATOR MATCHES "Ninja")
        ...
        _bobui_internal_get_ninja_wrapper("${CMAKE_MAKE_PROGRAM}" ninja_wrapper)
    ...
    # configuration. Important when using BobUI multi-config builds which won't have CMAKE_BUILD_TYPE
    # set.
    set(build_type "")
    ...
    if(BOBUI_CMAKE_TESTS_ADDITIONAL_CONFIGURE_OPTIONS)
        list(APPEND additional_configure_args ${BOBUI_CMAKE_TESTS_ADDITIONAL_CONFIGURE_OPTIONS})
    endif()
    if(DEFINED ENV{BOBUI_CMAKE_TESTS_ADDITIONAL_CONFIGURE_OPTIONS})
        list(APPEND additional_configure_args $ENV{BOBUI_CMAKE_TESTS_ADDITIONAL_CONFIGURE_OPTIONS})
    endif()

    # When building an iOS CMake test in the CI using a universal BobUI build, target the simulator
    ...
    if(IOS)
        ...
        if(BOBUI_OSX_ARCHITECTURES)
            list(LENGTH BOBUI_OSX_ARCHITECTURES osx_arch_count)
        endif()
        ...
        if(build_environment STREQUAL "ci"
            ...
            AND NOT BOBUI_APPLE_SDK
            AND NOT BOBUI_NO_IOS_BUILD_ADJUSTMENT_IN_CI)
    ...
    _bobui_internal_get_cmake_test_configure_options(option_list)
    ...
    _bobui_internal_create_command_script(
      COMMAND ${CMAKE_CTEST_COMMAND} ${ctest_command_args}
      COMMAND_ECHO STDOUT
      OUTPUT_FILE "${wrapper_file}"
      PRE_RUN ${_bobui_internal_skip_build_test_pre_run}
    )
    ...
    _bobui_internal_make_check_target(${testname})
    ...
    if(_ARGS_BINARY)
        ...
        _bobui_internal_set_up_test_run_environment("${testname}" ${run_env_args})
    endif()
...
function(_bobui_internal_add_qmake_test dir_name)
    ...
    if(BobUIBase_BINARY_DIR AND NOT BOBUI_BUILD_STANDALONE_TESTS)
        set(qmake_dir "${BobUIBase_BINARY_DIR}/${INSTALL_BINDIR}")
    else()
        set(qmake_dir "${BOBUI6_INSTALL_PREFIX}/${BOBUI6_INSTALL_BINS}")
    endif()
    ...
    if(ENV{BOBUI_QMAKE_TEST_BUILD_TOOL})
        set(build_tool "$ENV{BOBUI_QMAKE_TEST_BUILD_TOOL}")
    elseif(MSVC)
    ...
    set(build_tool_args "")
    if(ENV{BOBUI_QMAKE_TEST_BUILD_TOOL_OPTIONS})
        set(build_tool_args "$ENV{BOBUI_QMAKE_TEST_BUILD_TOOL_OPTIONS}")
    endif()
...
# Arguments: See _bobui_internal_test_expect_pass
macro(_bobui_internal_test_expect_fail)
  _bobui_internal_test_expect_pass(${ARGV})
...
macro(_bobui_internal_test_expect_build_fail _dir)
  string(REPLACE "(" "_" testname "${_dir}")
  string(REPLACE ")" "_" testname "${testname}")
  ...
  _bobui_internal_get_cmake_test_configure_options(option_list OUT_PREFIX_PATH __expect_fail_prefixes)

  file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/failbuild/${_dir}/${_dir}/FindPackageHints.cmake"
      "set(BobUI6Tests_PREFIX_PATH \"${__expect_fail_prefixes}\")
...
function(_bobui_internal_test_module_includes)

  set(all_args ${ARGN})
  set(packages_string "")
  set(libraries_string "")

  foreach(_package ${BobUI6_MODULE_TEST_DEPENDS})
    set(packages_string
      "
      ${packages_string}
      find_package(BobUI6${_package} 6.0.0 REQUIRED)
      "
    )
  endforeach()

  while(all_args)
    list(GET all_args 0 bobuimodule)
    list(REMOVE_AT all_args 0 1)

    set(CMAKE_MODULE_VERSION ${CMAKE_${bobuimodule}_MODULE_MAJOR_VERSION}.${CMAKE_${bobuimodule}_MODULE_MINOR_VERSION}.${CMAKE_${bobuimodule}_MODULE_PATCH_VERSION} )

    set(packages_string
      "${packages_string}
      find_package(BobUI6${bobuimodule} 6.0.0 REQUIRED)\n")

    list(FIND CMAKE_MODULES_UNDER_TEST ${bobuimodule} _findIndex)
    if (NOT _findIndex STREQUAL -1)
        set(packages_string
          "${packages_string}
          if(NOT \"\${BobUI6${bobuimodule}_VERSION}\" VERSION_EQUAL ${CMAKE_MODULE_VERSION})
            message(SEND_ERROR \"BobUI6${bobuimodule}_VERSION variable was not ${CMAKE_MODULE_VERSION}. Got \${BobUI6${bobuimodule}_VERSION} instead.\")
          endif()
          if(NOT \"\${BobUI6${bobuimodule}_VERSION_MAJOR}\" VERSION_EQUAL ${CMAKE_${bobuimodule}_MODULE_MAJOR_VERSION})
            message(SEND_ERROR \"BobUI6${bobuimodule}_VERSION_MAJOR variable was not ${CMAKE_${bobuimodule}_MODULE_MAJOR_VERSION}. Got \${BobUI6${bobuimodule}_VERSION_MAJOR} instead.\")
          endif()
          if(NOT \"\${BobUI6${bobuimodule}_VERSION_MINOR}\" VERSION_EQUAL ${CMAKE_${bobuimodule}_MODULE_MINOR_VERSION})
            message(SEND_ERROR \"BobUI6${bobuimodule}_VERSION_MINOR variable was not ${CMAKE_${bobuimodule}_MODULE_MINOR_VERSION}. Got \${BobUI6${bobuimodule}_VERSION_MINOR} instead.\")
          endif()
          if(NOT \"\${BobUI6${bobuimodule}_VERSION_PATCH}\" VERSION_EQUAL ${CMAKE_${bobuimodule}_MODULE_PATCH_VERSION})
            message(SEND_ERROR \"BobUI6${bobuimodule}_VERSION_PATCH variable was not ${CMAKE_${bobuimodule}_MODULE_PATCH_VERSION}. Got \${BobUI6${bobuimodule}_VERSION_PATCH} instead.\")
          endif()\n"
        )
    endif()
    set(libraries_string "${libraries_string} BobUI6::${bobuimodule}")
  endwhile()
  ...
  while(all_args)
    list(GET all_args 0 bobuimodule)
    list(GET all_args 1 bobuiclass)
    if (${bobuiclass}_NAMESPACE)
      set(bobuiinstancetype ${${bobuiclass}_NAMESPACE}::${bobuiclass})
    else()
      set(bobuiinstancetype ${bobuiclass})
    endif()
    list(REMOVE_AT all_args 0 1)
    set(includes_string
      "${includes_string}
      #include <${bobuiclass}>
      #include <BobUI${bobuimodule}/${bobuiclass}>
      #include <BobUI${bobuimodule}>
      #include <BobUI${bobuimodule}/BobUI${bobuimodule}>"
    )
    set(instances_string
    "${instances_string}
    ${bobuiinstancetype} local${bobuiclass};
    ")
  endwhile()
  ...
  _bobui_internal_make_check_target(${project_name_lower}_module_includes
