...
######################################
#
#       Macros for building BobUI files
#
######################################
...
set_property(GLOBAL PROPERTY __bobui_core_macros_module_base_dir "${CMAKE_CURRENT_LIST_DIR}")

# macro used to create the names of output files preserving relative dirs
macro(_bobui_internal_make_output_file infile prefix ext outfile )
...
macro(_bobui_internal_get_moc_flags _moc_flags)
    set(${_moc_flags})
    ...
    _bobui_internal_get_moc_compiler_flavor_flags(flavor_flags)
    set(${_moc_flags} ${${_moc_flags}} ${flavor_flags})
endmacro()

# helper macro to set up a moc rule
function(_bobui_internal_create_moc_command infile outfile moc_flags moc_options
    ...
    add_custom_command(OUTPUT ${outfile} ${extra_output_files}
                       COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::moc ${_moc_extra_parameters_file}
                       DEPENDS ${infile} ${moc_depends}
                       ${_moc_working_dir}
                       VERBATIM)
    set_source_files_properties(${infile} PROPERTIES SKIP_AUTOMOC ON)
    _bobui_internal_set_source_file_generated(SOURCES ${outfile} ${extra_output_files} SKIP_AUTOGEN)
endfunction()

function(bobui6_generate_moc infile outfile )
    # get include dirs and flags
    _bobui_internal_get_moc_flags(moc_flags)
    ...
    _bobui_internal_create_moc_command(${abs_infile} ${_outfile} "${moc_flags}" "" "${moc_target}"
        "" # moc_depends
        "" # out_json_file
    )
endfunction()
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_generate_moc)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_generate_moc(${ARGV})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_generate_moc(${ARGV})
        endif()
    endfunction()
endif()

function(bobui6_wrap_cpp)
    # check if the first argument is a target
    if(TARGET ${ARGV0})
        _bobui_internal_wrap_cpp(__bobui_internal_target_signature_marker
            TARGET ${ARGV}
        )
    else()
        set(output_parameter ${ARGV0})
        _bobui_internal_wrap_cpp(${ARGV})
        set(${output_parameter} "${${output_parameter}}" PARENT_SCOPE)
    endif()

    # Forward further return values of _bobui_internal_wrap_cpp to the caller.
    set(no_value_options "")
    set(single_value_options __BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES)
    set(multi_value_options "")
    cmake_parse_arguments(arg
        "${no_value_options}" "${single_value_options}" "${multi_value_options}" ${ARGV}
    )
    if(NOT "${arg___BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES}" STREQUAL "")
        set(${arg___BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES}
            "${${arg___BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES}}" PARENT_SCOPE)
    endif()
endfunction()

function(_bobui_internal_wrap_cpp outfiles_var)
    # get include dirs
    _bobui_internal_get_moc_flags(moc_flags)
    ...
    set(oneValueArgs
        TARGET
        __BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES
    )
    ...
    foreach(it ${moc_files})
        ...
        if(it_ext MATCHES "${HEADER_REGEX}")
            _bobui_internal_make_output_file("${it}" moc_ cpp outfile)
            ...
            if(found_extension)
                if(TARGET ${moc_target})
                    _bobui_internal_make_output_file(${it} "" moc outfile)
                    list(APPEND outfiles "${outfile}")
                    ...
                    if("${moc_target}" STREQUAL "")
                        string(JOIN "" err_msg
                            "bobui6_wrap_cpp: TARGET parameter is empty. "
                            "Since the file ${it} is a source file, "
                            "the TARGET option must be specified.")
                    else()
                        string(JOIN "" err_msg
                            "bobui6_wrap_cpp: TARGET \"${moc_target}\" "
                            "not found.")
                    endif()
                    message(FATAL_ERROR "${err_msg}")
                endif()
            else()
                string(JOIN "" err_msg "bobui6_wrap_cpp: Unknown file extension: "
                    "\"\.${it_ext}\".")
                message(FATAL_ERROR "${err_msg}")
            endif()
        endif()
        ...
        if(_WRAP_CPP___BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES)
            set(out_json_file_var "out_json_file")
        endif()

        _bobui_internal_create_moc_command(
    ...
    if(NOT outfiles STREQUAL "")
        ...
        if(TARGET ${moc_target})
            ...
            if(NOT moc_target_source_dir STREQUAL CMAKE_CURRENT_SOURCE_DIR)
                # bobui_wrap_cpp is not called in ${moc_target}'s directory scope.
                # Add a custom target that drives the creation of moc's output files.
                _bobui_internal_unique_target_name(driver_target "_bobui_${moc_target}_moc_driver")
                add_custom_target(${driver_target} DEPENDS ${outfiles})
                _bobui_internal_assign_to_internal_targets_folder(${driver_target})
    ...
    if(metatypes_json_list)
        set(${_WRAP_CPP___BOBUI_INTERNAL_OUTPUT_MOC_JSON_FILES}
            "${metatypes_json_list}" PARENT_SCOPE)
    endif()
endfunction()

# This will override the CMake upstream command, because that one is for BobUI 3.
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_wrap_cpp outfiles)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_wrap_cpp("${outfiles}" ${ARGN})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_wrap_cpp("${outfiles}" ${ARGN})
        endif()
...
# _bobui6_parse_qrc_file(infile _out_depends _rc_depends)
# internal

function(_bobui6_parse_qrc_file infile _out_depends _rc_depends)
    ...
    if(EXISTS "${infile}")
        ...
        _bobui_internal_make_output_file("${infile}" "" "qrc.depends" out_depends)
...
# bobui6_add_binary_resources(target inputfiles ... )

function(bobui6_add_binary_resources target )
    ...
    add_custom_command(OUTPUT ${rcc_destination}
                       DEPENDS ${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc
                       COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc
                       ARGS ${rcc_options} --binary --name ${target} --output ${rcc_destination} ${infiles}
                       DEPENDS
                            ${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc
                            ${rc_depends}
                            ${out_depends}
                            ${infiles}
                       VERBATIM)
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_binary_resources)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_add_binary_resources(${ARGV})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_add_binary_resources(${ARGV})
        endif()
    endfunction()
endif()

function(_bobui_internal_get_bobui_internal_process_resource_args option_args single_args multi_args)
...
# bobui6_add_resources(target resourcename ...
# or
# bobui6_add_resources(outfiles inputfile ... )

function(bobui6_add_resources outfiles )
    if (TARGET ${outfiles})
        cmake_parse_arguments(arg "" "OUTPUT_TARGETS" "" ${ARGN})
        _bobui_internal_process_resource(${ARGV})
        ...
        _bobui_internal_get_bobui_internal_process_resource_args(
        ...
        if(contains_suspicious_args)
            message(WARNING "bobui6_add_resources uses arguments from the"
                " 'bobui6_add_resources(<target> ...)'  signature, but ${outfiles} is not a target."
                " Make sure that the '${outfiles}' target is created before the respective"
                " 'bobui6_add_resources' call."
            )
        endif()
        ...
        if("${rcc_options}" MATCHES "-binary")
            message(WARNING "Use bobui6_add_binary_resources for binary option")
        endif()
        ...
        foreach(it ${rcc_files})
            ...
            add_custom_command(OUTPUT ${outfile}
                               ...
                               DEPENDS ${_rc_depends} "${_out_depends}" ${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc
                               VERBATIM)

            _bobui_internal_set_source_file_generated(SOURCES "${outfile}" SKIP_AUTOGEN)
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_resources outfiles)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_add_resources("${outfiles}" ${ARGN})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_add_resources("${outfiles}" ${ARGN})
        endif()
        if(TARGET ${outfiles})
...
# bobui6_add_big_resources(outfiles inputfile ... )

function(bobui6_add_big_resources outfiles )
    if(CMAKE_GENERATOR STREQUAL "Xcode" AND IOS)
        message(WARNING
            "Due to CMake limitations, bobui6_add_big_resources can't be used when building for iOS. "
            "See https://bugreports.bobui.io/browse/BOBUIBUG-103497 for details. "
            "Falling back to using bobui6_add_resources. "
            "Consider using bobui6_add_resources directly to silence this warning."
        )
        bobui6_add_resources(${ARGV})
        set(${outfiles} ${${outfiles}} PARENT_SCOPE)
        return()
    endif()

    if (CMAKE_VERSION VERSION_LESS 3.9)
        message(FATAL_ERROR, "bobui6_add_big_resources requires CMake 3.9 or newer")
    endif()
    ...
    foreach(it ${rcc_files})
        ...
        string(MAKE_C_IDENTIFIER "_bobui_big_resource_count_${outfilename}" prop)
        ...
        add_custom_command(OUTPUT ${tmpoutfile}
                           COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc ${rcc_options} --name ${outfilename} --pass 1 --output ${tmpoutfile} ${infile}
                           DEPENDS ${infile} ${_rc_depends} "${out_depends}" ${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc
                           COMMENT "Running rcc pass 1 for resource ${outfilename}"
                           VERBATIM)
        add_custom_target(big_resources_${outfilename} ALL DEPENDS ${tmpoutfile})
        _bobui_internal_set_source_file_generated(SOURCES ${tmpoutfile})
        _bobui_internal_add_rcc_pass2(
...
function(_bobui_internal_add_rcc_pass2)
    set(options)
    set(oneValueArgs
        RESOURCE_NAME
        OBJECT_LIB
    ...
    _bobui_internal_set_up_static_runtime_library(${arg_OBJECT_LIB})
    target_compile_definitions(${arg_OBJECT_LIB} PUBLIC
        "$<TARGET_PROPERTY:BobUI6::Core,INTERFACE_COMPILE_DEFINITIONS>")
    set_target_properties(${arg_OBJECT_LIB} PROPERTIES
        AUTOMOC OFF
        AUTOUIC OFF
        _bobui_internal_is_rcc_pass2_obj_lib TRUE
    )
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_big_resources outfiles)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_add_big_resources(${outfiles} ${ARGN})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_add_big_resources(${outfiles} ${ARGN})
        endif()
        set("${outfiles}" "${${outfiles}}" PARENT_SCOPE)
    endfunction()
endif()

function(_bobui_internal_apply_win_prefix_and_suffix target)
    if(WIN32)
        # Table of prefix / suffixes for MSVC libraries as qmake expects them to be created.
        # static - BobUI6EdidSupport.lib (platform support libraries / or static BobUICore, etc)
        # shared - BobUI6Core.dll
        # shared import library - BobUI6Core.lib
        # module aka BobUI plugin - qwindows.dll
        ...
        # static - libBobUI6EdidSupport.a (platform support libraries / or static BobUICore, etc)
        # shared - BobUI6Core.dll
        # shared import library - libBobUI6Core.a
        # module aka BobUI plugin - qwindows.dll
...
set(_BobUI6_COMPONENT_PATH "${CMAKE_CURRENT_LIST_DIR}/..")

# Wrapper function that adds an executable with some BobUI specific behavior.
...
# target properties.
function(bobui6_add_executable target)
    cmake_parse_arguments(PARSE_ARGV 1 arg "MANUAL_FINALIZATION" "" "")

    _bobui_internal_warn_about_example_add_subdirectory()

    _bobui_internal_create_executable("${target}" ${arg_UNPARSED_ARGUMENTS})
    target_link_libraries("${target}" PRIVATE BobUI6::Core)
    set_property(TARGET ${target} PROPERTY _bobui_expects_finalization TRUE)

    if(arg_MANUAL_FINALIZATION)
        # Caller says they will call bobui6_finalize_target() themselves later
        return()
    endif()

    _bobui_internal_finalize_target_defer("${target}")
endfunction()

# Just like for bobui_add_resources, we should disable zstd compression when cross-compiling to a
# target that doesn't support zstd decompression, even if the host tool supports it.
# Allow an opt out via a BOBUI_NO_AUTORCC_ZSTD variable.
function(_bobui_internal_disable_autorcc_zstd_when_not_supported target)
    if(TARGET "${target}"
            AND DEFINED BOBUI_FEATURE_zstd
            AND NOT BOBUI_FEATURE_zstd
            AND NOT BOBUI_NO_AUTORCC_ZSTD)
        get_target_property(target_type ${target} TYPE)
...
function(_bobui_internal_link_to_platform_example_internal target)
    ...
    if(BOBUI_INTERNAL_IS_EXAMPLE_EP_BUILD
        OR BOBUI_INTERNAL_IS_EXAMPLE_IN_TREE_BUILD)
        target_link_libraries("${target}" PRIVATE
            ${BOBUI_CMAKE_EXPORT_NAMESPACE}::PlatformExampleInternal)
    endif()
endfunction()

# Set up warnings as errors for targets built in example projects.
function(_bobui_internal_setup_warnings_are_errors_for_example_target target)
    # Only enable warnings as errors when the global variable is enabled and the repo is known
    # to have clean examples.
    if(BOBUI_INTERNAL_IS_EXAMPLE_EP_BUILD
        OR BOBUI_INTERNAL_IS_EXAMPLE_IN_TREE_BUILD)
        if(WARNINGS_ARE_ERRORS AND BOBUI_REPO_EXAMPLES_WARNINGS_CLEAN)
            _bobui_internal_set_skip_warnings_are_errors("${target}" FALSE)
        else()
            _bobui_internal_set_skip_warnings_are_errors("${target}" TRUE)
        endif()
    endif()
endfunction()

function(_bobui_internal_create_executable target)
    if(ANDROID)
        list(REMOVE_ITEM ARGN "WIN32" "MACOSX_BUNDLE")
        cmake_policy(PUSH)
        __bobui_internal_set_cmp0156()
        ...
        set_target_properties("${target}" PROPERTIES
            ...
            _bobui_android_apply_arch_suffix_called_from_bobui_impl TRUE
            _bobui_android_target_type APPLICATION
        )

        bobui6_android_apply_arch_suffix("${target}")
        if(BOBUI_USE_ANDROID_MODERN_BUNDLE)
            _bobui_internal_set_android_application_gradle_defaults(${target})
        endif()
    else()
        cmake_policy(PUSH)
        __bobui_internal_set_cmp0156()
    ...
    _bobui_internal_disable_autorcc_zstd_when_not_supported("${target}")
    _bobui_internal_link_to_platform_example_internal("${target}")
    _bobui_internal_setup_warnings_are_errors_for_example_target("${target}")
    _bobui_internal_set_up_static_runtime_library("${target}")
endfunction()

function(_bobui_internal_finalize_executable target)
    ...
    # to BobUI::* targets. For the cases where this isn't so, the project will be
    # responsible for calling any relevant functions themselves instead of
    # relying on these automatic finalization calls.
    set(finalizers)

    __bobui_internal_collect_all_target_dependencies("${target}" dep_targets)
    ...
    if(finalizers)
        if(CMAKE_VERSION VERSION_LESS 3.18)
            ...
            endforeach()
            set(finalizer_file "${CMAKE_CURRENT_BINARY_DIR}/.bobui/finalize_${target}.cmake")
    ...
    if(EMSCRIPTEN)
        _bobui_internal_finalize_wasm_app("${target}")
    endif()

    if(APPLE)
        if(NOT CMAKE_SYSTEM_NAME OR CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            # macOS
            _bobui_internal_finalize_macos_app("${target}")
        else()
            _bobui_internal_finalize_uikit_app("${target}")
        endif()
    endif()

    # For finalizer mode of plugin importing to work safely, we need to know the list of BobUI
    # dependencies the target has, but those might be added later than the bobui_add_executable call.
    # Most of our examples are like that. Only enable finalizer mode when we are sure that the user
    # manually called bobui_finalize_target at the end of their CMake project, or it was automatically
    # done via a deferred call. This is also applicable to the resource object finalizer.
    get_target_property(is_immediately_finalized "${target}" _bobui_is_immediately_finalized)
    if(NOT is_immediately_finalized)
        __bobui_internal_apply_plugin_imports_finalizer_mode("${target}")
        __bobui_internal_process_dependency_object_libraries("${target}")
    endif()
endfunction()
...
function(_bobui_internal_delay_finalization_until_after defer_id)
    set_property(DIRECTORY APPEND PROPERTY bobui_internal_finalizers_wait_for_ids "${defer_id}")
endfunction()

function(bobui6_finalize_target target)
    set_property(TARGET ${target} PROPERTY _bobui_expects_finalization FALSE)
    if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.19")
        cmake_language(DEFER GET_CALL_IDS ids_queued)
        get_directory_property(wait_for_ids bobui_internal_finalizers_wait_for_ids)
        while(wait_for_ids)
            ...
            if(id_to_wait_for IN_LIST ids_queued)
                ...
                set_directory_properties(PROPERTIES
                    bobui_internal_finalizers_wait_for_ids "${wait_for_ids}"
                )
                return()
            endif()
        ...
        set_directory_properties(PROPERTIES bobui_internal_finalizers_wait_for_ids "")
    endif()
    ...
    get_target_property(is_finalized "${target}" _bobui_is_finalized)
    if(is_finalized)
        message(AUTHOR_WARNING
            "Tried to call bobui6_finalize_target twice on target '${target}'. "
            "Did you forget to specify MANUAL_FINALIZATION to bobui6_add_executable, "
            "bobui6_add_library or bobui6_add_plugin?")
        return()
    endif()

    _bobui_internal_expose_deferred_files_to_ide(${target})
    _bobui_internal_finalize_source_groups(${target})
    get_target_property(target_type ${target} TYPE)
    get_target_property(android_type "${target}" _bobui_android_target_type)

    if(target_type STREQUAL "EXECUTABLE" OR android_type STREQUAL "APPLICATION")
        _bobui_internal_finalize_executable(${ARGV})
    endif()
    ...
    _bobui_internal_work_around_autogen_discarded_dependencies_from_target_libs("${target}")
    ...
    if(target_type STREQUAL "SHARED_LIBRARY" OR
        ...
        if(uses_automoc AND NOT is_immediately_finalized)
            bobui6_extract_metatypes(${target})
        endif()
    endif()

    set_target_properties(${target} PROPERTIES _bobui_is_finalized TRUE)
endfunction()

function(_bobui_internal_finalize_target_defer target)
    ...
    if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.19)
        ...
        cmake_language(EVAL CODE "cmake_language(DEFER CALL bobui6_finalize_target ${target})")
    elseif(BOBUI_BUILDING_BOBUI AND BOBUI_INTERNAL_USE_POOR_MANS_SCOPE_FINALIZER)
        bobui_add_list_file_finalizer(bobui6_finalize_target "${target}")
    else()
        set_target_properties("${target}" PROPERTIES _bobui_is_immediately_finalized TRUE)
        bobui6_finalize_target("${target}")
    endif()
endfunction()

function(_bobui_internal_finalize_source_groups target)
    ...
    get_target_property(resource_source_files "${target}" _bobui_resource_source_files)
    ...
    if(NOT BOBUI_NO_AUTO_RESOURCE_SOURCE_GROUPS)
        source_group("Resources" FILES ${resource_source_files})
    endif()
endfunction()

function(_bobui_internal_darwin_permission_finalizer target)
    get_target_property(plist_file "${target}" MACOSX_BUNDLE_INFO_PLIST)
    if(NOT plist_file)
        return()
    endif()
    foreach(plugin_target IN LISTS BOBUI_ALL_PLUGINS_FOUND_BY_FIND_PACKAGE_permissions)
        set(versioned_plugin_target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${plugin_target}")
        get_target_property(usage_descriptions
            ${versioned_plugin_target}
            _bobui_info_plist_usage_descriptions)
        foreach(usage_description_key IN LISTS usage_descriptions)
            ...
            if(usage_description AND NOT plist_error)
                set_target_properties("${target}"
                    PROPERTIES "_bobui_has_${plugin_target}_usage_description" TRUE)
                bobui6_import_plugins(${target} INCLUDE ${versioned_plugin_target})
            endif()
        endforeach()
    endforeach()
endfunction()
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_executable)
        bobui6_add_executable(${ARGV})
    endfunction()
    function(bobui_finalize_target)
        bobui6_finalize_target(${ARGV})
    endfunction()

    # Kept for compatibility with BobUI Creator 4.15 wizards
    function(bobui_finalize_executable)
        bobui6_finalize_target(${ARGV})
    endfunction()
endif()

function(_bobui_get_plugin_name_with_version target out_var)
    string(REGEX REPLACE "^BobUI::(.+)" "BobUI${BOBUI_DEFAULT_MAJOR_VERSION}::\\1"
           bobui_plugin_with_version "${target}")
    if(TARGET "${bobui_plugin_with_version}")
        set("${out_var}" "${bobui_plugin_with_version}" PARENT_SCOPE)
    else()
        set("${out_var}" "" PARENT_SCOPE)
    endif()
endfunction()

macro(_bobui_import_plugin target plugin)
    set(_final_plugin_name "${plugin}")
    if(NOT TARGET "${plugin}")
        _bobui_get_plugin_name_with_version("${plugin}" _bobui_plugin_with_version_name)
        if(TARGET "${_bobui_plugin_with_version_name}")
            set(_final_plugin_name "${_bobui_plugin_with_version_name}")
        endif()
    endif()

    if(NOT TARGET "${_final_plugin_name}")
        message(
            "Warning: plug-in ${_final_plugin_name} is not known to the current BobUI installation.")
    else()
        get_target_property(_plugin_class_name "${_final_plugin_name}" BOBUI_PLUGIN_CLASS_NAME)
        if(_plugin_class_name)
            set_property(TARGET "${target}" APPEND PROPERTY BOBUI_PLUGINS "${plugin}")
        endif()
    endif()
endmacro()

function(_bobui_internal_disable_static_default_plugins target)
    set_target_properties(${target} PROPERTIES BOBUI_DEFAULT_PLUGINS 0)
endfunction()

function(bobui6_import_plugins target)
    cmake_parse_arguments(arg "NO_DEFAULT" "" "INCLUDE;EXCLUDE;INCLUDE_BY_TYPE;EXCLUDE_BY_TYPE" ${ARGN})

    # Handle NO_DEFAULT
    if(${arg_NO_DEFAULT})
        _bobui_internal_disable_static_default_plugins("${target}")
    endif()

    # Handle INCLUDE
    foreach(plugin ${arg_INCLUDE})
        _bobui_import_plugin("${target}" "${plugin}")
    endforeach()

    # Handle EXCLUDE
    foreach(plugin ${arg_EXCLUDE})
        set_property(TARGET "${target}" APPEND PROPERTY BOBUI_NO_PLUGINS "${plugin}")
    endforeach()
    ...
    foreach(_arg ${arg_INCLUDE_BY_TYPE})
        ...
        list(FIND BOBUI_ALL_PLUGIN_TYPES_FOUND_VIA_FIND_PACKAGE "${_plugin_type}" _has_plugin_type)

        if(${_has_plugin_type} GREATER_EQUAL 0)
           set(_current_type "${_plugin_type}")
        else()
            if("${_current_type}" STREQUAL "")
                message(FATAL_ERROR "bobui_import_plugins: invalid syntax for INCLUDE_BY_TYPE")
            endif()

            # Check if passed plugin target name is a version-less one, and make a version-full
            # one.
            set_property(TARGET "${target}" APPEND PROPERTY "BOBUI_PLUGINS_${_current_type}" "${_arg}")
            set_property(TARGET "${target}" APPEND PROPERTY "_bobui_plugins_by_type" "${_arg}")
            _bobui_get_plugin_name_with_version("${_arg}" bobui_plugin_with_version)

            # TODO: Do we really need this check? We didn't have it in BobUI5, and plugin targets
            # wrapped in genexes end up causing warnings, but we explicitly use GENEX_EVAL to
            # support them.
            if(NOT TARGET "${_arg}" AND NOT TARGET "${bobui_plugin_with_version}")
                message("Warning: plug-in ${_arg} is not known to the current BobUI installation.")
            endif()
        endif()
    endforeach()
    ...
    foreach(_arg ${arg_EXCLUDE_BY_TYPE})
        string(REGEX REPLACE "[-/]" "_" _plugin_type "${_arg}")
        set_property(TARGET "${target}" PROPERTY "BOBUI_PLUGINS_${_plugin_type}" "-")
    endforeach()
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_import_plugins)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 5)
            bobui5_import_plugins(${ARGV})
        elseif(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_import_plugins(${ARGV})
        endif()
    endfunction()
endif()

# This function is currently in Technical Preview. It's signature may change or be removed entirely.
function(bobui6_set_finalizer_mode target)
    ...
    if(NOT arg_MODES)
        message(FATAL_ERROR "No modes were specified in bobui6_set_finalizer_mode() call.")
    endif()
    ...
    foreach(mode ${arg_MODES})
        __bobui_internal_enable_finalizer_mode("${target}" "${mode}" "${value}")
    endforeach()
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_set_finalizer_mode)
        bobui6_set_finalizer_mode(${ARGV})
    endfunction()
endif()

function(_bobui_internal_assign_to_internal_targets_folder target)
    get_property(folder_name GLOBAL PROPERTY BOBUI_TARGETS_FOLDER)
    if(NOT "${folder_name}" STREQUAL "")
        set_property(TARGET ${target} PROPERTY FOLDER "${folder_name}")
    endif()
endfunction()

# Returns the metatypes build dir where the BobUI build system places module metatypes json files and
# other supporting metatypes files like ${target}_json_file_list.txt.
# The path is usually the target's BINARY_DIR + "/meta_types"
function(_bobui_internal_get_metatypes_build_dir out_var target)
...
function(_bobui_internal_get_target_autogen_build_dir target out_var)
...
function(_bobui_internal_get_target_autogen_info_dir target out_var)
    get_target_property(target_binary_dir ${target} BINARY_DIR)
    set(autogen_info_dir "${target_binary_dir}/CMakeFiles/${target}_autogen.dir")
    set(${out_var} "${autogen_info_dir}" PARENT_SCOPE)
endfunction()

function(_bobui_internal_should_install_metatypes target)
    set(args_option
        INTERNAL_INSTALL
    )
    set(args_single
    ...
    # Executable metatypes.json files should not be installed. BobUI non-prefix builds should also
...
function(_bobui_internal_get_metatypes_install_dir internal_install_dir arch_data_dir out_var)
    # Automatically fill default install args when not specified.
    if(NOT internal_install_dir)
        # INSTALL_ARCHDATADIR is not set when BobUIBuildInternals is not loaded
        # (when not doing a BobUI build). Default to a hardcoded location for user
...
function(_bobui_internal_assign_build_metatypes_files_and_properties target)
    get_target_property(existing_meta_types_file ${target} INTERFACE_BOBUI_META_TYPES_BUILD_FILE)
    ...
    set(consumes_metatypes "$<BOOL:$<TARGET_PROPERTY:BOBUI_CONSUMES_METATYPES>>")
    ...
    set_target_properties(${target} PROPERTIES
        INTERFACE_BOBUI_MODULE_HAS_META_TYPES YES
        # The property name is a bit misleading, it's not wrapped in a genex.
        INTERFACE_BOBUI_META_TYPES_BUILD_FILE "${metatypes_file_path}"
        INTERFACE_BOBUI_META_TYPES_FILE_NAME "${metatypes_file_name}"
    )
endfunction()

# Same as above, but with $<INSTALL_INTERFACE>.
function(_bobui_internal_assign_install_metatypes_files_and_properties target)
    get_target_property(existing_meta_types_file ${target} INTERFACE_BOBUI_META_TYPES_INSTALL_FILE)
    if (existing_meta_types_file)
        return()
    endif()
    ...
    get_target_property(metatypes_file_name "${target}" INTERFACE_BOBUI_META_TYPES_FILE_NAME)

    if(NOT metatypes_file_name)
        message(FATAL_ERROR "INTERFACE_BOBUI_META_TYPES_FILE_NAME of target ${target} is empty")
    endif()
    ...
    set_target_properties(${target} PROPERTIES
        INTERFACE_BOBUI_META_TYPES_INSTALL_FILE "${metatypes_file_install_path}"
    )
endfunction()


function(bobui6_extract_metatypes target)
    ...
    set(args_option
        ...
        # When given, metatypes files will be installed into the default BobUI
        # metatypes folder. Only to be used by the BobUI build.
        __BOBUI_INTERNAL_INSTALL
    )
    set(args_single
        # TODO: Move this into a separate internal function, so it doesn't pollute the public one.
        # Location where to install the metatypes file. Only used if
        # __BOBUI_INTERNAL_INSTALL is given. It defaults to the
        # ${CMAKE_INSTALL_PREFIX}/${INSTALL_ARCHDATADIR}/metatypes directory.
        # Executable metatypes files are never installed.
        __BOBUI_INTERNAL_INSTALL_DIR
    ...
    _bobui_internal_get_metatypes_build_dir(metatypes_dir "${target}")
    ...
    _bobui_internal_get_target_autogen_build_dir(${target} target_autogen_build_dir)
    _bobui_internal_get_target_autogen_info_dir(${target} target_autogen_info_dir)
    ...
    if (uses_automoc)
        ...
        if (CMAKE_VERSION VERSION_GREATER_EQUAL "3.17") # Requires automoc changes present only in 3.17
            if(CMAKE_GENERATOR STREQUAL "Ninja" OR
               ...
               (CMAKE_GENERATOR MATCHES "Makefiles" AND
                ...
                if(DEFINED BOBUI_USE_CMAKE_DEPFILES)
                    set(use_dep_files ${BOBUI_USE_CMAKE_DEPFILES})
                else()
        ...
        if (NOT use_dep_files)
            ...
            # cmBobUIAutoGenInitializer::InitAutogenTarget() can take one of two code paths on how to
            ...
            # cmBobUIAutoGenInitializer::AutogenTarget.DependFiles, which disables the PRE_BUILD path.
            if(CMAKE_GENERATOR MATCHES "Visual Studio")
                ...
                set_source_files_properties("${dummy_out_file}" PROPERTIES
                    SKIP_AUTOGEN OFF)
                _bobui_internal_set_source_file_generated(SOURCES "${dummy_out_file}")
                target_sources("${target}" PRIVATE "${dummy_out_file}")
            endif()

            add_custom_target(${target}_automoc_json_extraction
                DEPENDS ${BOBUI_CMAKE_EXPORT_NAMESPACE}::cmake_automoc_parser
                BYPRODUCTS
                    ${type_list_file}
                    "${cmake_automoc_parser_timestamp}"
                COMMAND
                    ${BOBUI_CMAKE_EXPORT_NAMESPACE}::cmake_automoc_parser
            ...
            _bobui_internal_assign_to_internal_targets_folder(${target}_automoc_json_extraction)
        else()
            ...
            if (is_multi_config AND CMAKE_VERSION VERSION_GREATER_EQUAL "3.29"
                AND NOT BOBUI_INTERNAL_USE_OLD_AUTOGEN_GRAPH_MULTI_CONFIG_METATYPES)
    ...
    set(metatypes_file_name "bobui6${target_lowercase}_metatypes.json")
    set(metatypes_file "${metatypes_dir}/${metatypes_file_name}")
    set(metatypes_file_gen "${metatypes_dir}/${metatypes_file_name}.gen")

    set(metatypes_dep_file_name "bobui6${target_lowercase}_metatypes_dep.txt")
    ...
    add_custom_command(
        ...
        BYPRODUCTS
            ${metatypes_file}
        DEPENDS ${BOBUI_CMAKE_EXPORT_NAMESPACE}::moc ${automoc_dependencies} ${manual_dependencies}
        COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::moc
    ...
    _bobui_internal_set_source_file_generated(
    ...
    _bobui_internal_assign_build_metatypes_files_and_properties(
        "${target}"
        METATYPES_FILE_NAME "${metatypes_file_name}"
        METATYPES_FILE_PATH "${metatypes_file}"
    )

    if(arg___BOBUI_INTERNAL_INSTALL)
        set(internal_install_option "INTERNAL_INSTALL")
    else()
        set(internal_install_option "")
    endif()

    # TODO: Clean up BobUI-specific installation not to happen in the public api.
    # Check whether the metatype files should be installed.
    _bobui_internal_should_install_metatypes("${target}"
        ${internal_install_option}
        OUT_VAR should_install
    )

    if(should_install)
        _bobui_internal_get_metatypes_install_dir(
            "${arg___BOBUI_INTERNAL_INSTALL_DIR}"
            "${INSTALL_ARCHDATADIR}"
            install_dir
        )

        # Propagate the install time metatypes file.
        _bobui_internal_assign_install_metatypes_files_and_properties(
            "${target}"
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_extract_metatypes)
        bobui6_extract_metatypes(${ARGV})
...
#
# BOBUI_TARGET_COMPANY_NAME: RC Company name
# BOBUI_TARGET_DESCRIPTION: RC File Description
# BOBUI_TARGET_VERSION: RC File and Product Version
# BOBUI_TARGET_COPYRIGHT: RC LegalCopyright
# BOBUI_TARGET_PRODUCT_NAME: RC ProductName
# BOBUI_TARGET_COMMENTS: RC Comments
# BOBUI_TARGET_ORIGINAL_FILENAME: RC Original FileName
# BOBUI_TARGET_TRADEMARKS: RC LegalTrademarks
# BOBUI_TARGET_INTERNALNAME: RC InternalName
# BOBUI_TARGET_RC_ICONS: List of paths to icon files
#
# If you do not wish to auto-generate rc files, it's possible to provide your
# own RC file by setting the property BOBUI_TARGET_WINDOWS_RC_FILE with a path to
# an existing rc file.
function(_bobui_internal_generate_win32_rc_file target)
    ...
    get_target_property(target_rc_file ${target} BOBUI_TARGET_WINDOWS_RC_FILE)
    get_target_property(target_version ${target} BOBUI_TARGET_VERSION)
    ...
    if (target_rc_file)
        ...
        set(company_name "")
        get_target_property(target_company_name ${target} BOBUI_TARGET_COMPANY_NAME)
        if (target_company_name)
            set(company_name "${target_company_name}")
        endif()

        set(file_description "")
        get_target_property(target_description ${target} BOBUI_TARGET_DESCRIPTION)
        if (target_description)
            set(file_description "${target_description}")
        endif()

        set(legal_copyright "")
        get_target_property(target_copyright ${target} BOBUI_TARGET_COPYRIGHT)
        if (target_copyright)
            set(legal_copyright "${target_copyright}")
        endif()

        set(product_name "")
        get_target_property(target_product_name ${target} BOBUI_TARGET_PRODUCT_NAME)
        ...
        set(comments "")
        get_target_property(target_comments ${target} BOBUI_TARGET_COMMENTS)
        if (target_comments)
            set(comments "${target_comments}")
        endif()

        set(legal_trademarks "")
        get_target_property(target_trademarks ${target} BOBUI_TARGET_TRADEMARKS)
        ...
        get_target_property(target_original_file_name ${target} BOBUI_TARGET_ORIGINAL_FILENAME)
        if (target_original_file_name)
            set(original_file_name "${target_original_file_name}")
        endif()

        set(internal_name "")
        get_target_property(target_internal_name ${target} BOBUI_TARGET_INTERNALNAME)
        if (target_internal_name)
            set(internal_name "${target_internal_name}")
        endif()

        set(icons "")
        get_target_property(target_icons ${target} BOBUI_TARGET_RC_ICONS)
        if (target_icons)
            set(index 1)
...
#
function(_bobui_internal_generate_longpath_win32_rc_file_and_manifest target)
    set(prohibited_target_types INTERFACE_LIBRARY STATIC_LIBRARY OBJECT_LIBRARY)
    get_target_property(target_type ${target} TYPE)
    if(target_type IN_LIST prohibited_target_types)
        return()
...
<application  xmlns="urn:schemas-microsoft-com:asm.v3">
    ...
    _bobui_internal_set_source_file_generated(SOURCES ${outputs})
endfunction()

function(__bobui_get_relative_resource_path_for_file output_alias file)
    get_property(alias SOURCE ${file} PROPERTY BOBUI_RESOURCE_ALIAS)
    if (NOT alias)
        set(alias "${file}")
        if(IS_ABSOLUTE "${file}")
            message(FATAL_ERROR
                "The source file '${file}' was specified with an absolute path and is used in a BobUI "
                "resource. Please set the BOBUI_RESOURCE_ALIAS property on that source file to a "
...
function(__bobui_internal_propagate_object_files target objects)
    set(options "")
    set(single_args "")
    ...
    set(platform_link_order_property
        "$<TARGET_PROPERTY:${BOBUI_CMAKE_EXPORT_NAMESPACE}::Platform,_bobui_link_order_matters>"
    )
    ...
    # In user builds the _bobui_cmp0099_policy_check is set to FALSE or $<TARGET_POLICY:CMP0099>
    # depending on the used CMake version.
    # See __bobui_internal_check_cmp0099_available for details.
    set(cmp0099_policy_check_property
        "$<TARGET_PROPERTY:${BOBUI_CMAKE_EXPORT_NAMESPACE}::Platform,_bobui_cmp0099_policy_check>"
    )
    ...
    # before calling find_package(BobUI...).
...
function(__bobui_internal_propagate_object_library target object_library)
    ...
    if(NOT is_imported)
        target_link_libraries(${object_library} PRIVATE ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Platform)
        _bobui_internal_copy_dependency_properties(${object_library} ${target} PRIVATE_ONLY)
    endif()
    ...
    # All object libraries mark themselves with the _is_bobui_propagated_object_library property.
    # Using a finalizer approach we walk through the target dependencies and look for libraries
    # using the _is_bobui_propagated_object_library property. Then, objects of the collected libraries
    ...
    # libraries.
    set_property(TARGET ${object_library} PROPERTY _is_bobui_propagated_object_library TRUE)
    if(NOT is_imported)
        set_property(TARGET ${object_library} APPEND PROPERTY
            EXPORT_PROPERTIES _is_bobui_propagated_object_library
        )
    endif()

    # Keep the implicit linking if finalizers are not used.
    set(not_finalizer_mode_condition
        "$<NOT:$<BOOL:$<TARGET_PROPERTY:_bobui_object_libraries_finalizer_mode>>>"
    )

    # Use TARGET_NAME to have the correct namespaced name in the exports.
    set(objects "$<TARGET_OBJECTS:$<TARGET_NAME:${object_library}>>")

    __bobui_internal_propagate_object_files(${target} ${objects}
...
function(__bobui_propagate_generated_resource target resource_name generated_source_code output_generated_target)
    get_target_property(type ${target} TYPE)
    if(type STREQUAL STATIC_LIBRARY)
        get_target_property(resource_count ${target} _bobui_generated_resource_target_count)
        if(NOT resource_count)
            set(resource_count "0")
        endif()
        math(EXPR resource_count "${resource_count} + 1")
        set_target_properties(${target} PROPERTIES _bobui_generated_resource_target_count ${resource_count})

        __bobui_internal_generate_init_resource_source_file(
        ...
        # Needed so that bobuisymbolmacros.h and its dependent headers are already created / syncbobui'ed.
        if(TARGET Core_sync_headers)
            set(headers_available_target "Core_sync_headers")
        else()
            set(headers_available_target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core")
        endif()
        add_dependencies(${resource_target} ${headers_available_target})
        target_compile_definitions("${resource_target}" PRIVATE
            "$<TARGET_PROPERTY:${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core,INTERFACE_COMPILE_DEFINITIONS>"
        )
        target_include_directories("${resource_target}" PRIVATE
            "$<TARGET_PROPERTY:${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core,INTERFACE_INCLUDE_DIRECTORIES>"
        )
        _bobui_internal_set_up_static_runtime_library("${resource_target}")
        ...
        if(NOT target STREQUAL "Core")
            target_link_libraries(${resource_target} INTERFACE ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core)
        endif()
        set_property(TARGET ${resource_target} APPEND PROPERTY _bobui_resource_name ${resource_name})
        ...
        set_property(TARGET ${resource_target} APPEND PROPERTY
            _bobui_resource_generated_cpp_relative_path "${generated_cpp_file_relative_path}")

        if(target STREQUAL "Core")
            set(skip_direct_linking NO_LINK_OBJECT_LIBRARY_REQUIREMENTS_TO_TARGET)
        endif()
        __bobui_internal_propagate_object_library(${target} ${resource_target}
            ${skip_direct_linking}
        )
...
function(__bobui_internal_sanitize_resource_name out_var name)
...
function(__bobui_internal_generate_init_resource_source_file out_var target resource_name)
    get_property(__bobui_core_macros_module_base_dir GLOBAL PROPERTY __bobui_core_macros_module_base_dir)
    set(template_file "${__bobui_core_macros_module_base_dir}/BobUI6CoreResourceInit.in.cpp")

    # Gets replaced in the template
    __bobui_internal_sanitize_resource_name(RESOURCE_NAME "${resource_name}")
    set(resource_init_path "${CMAKE_CURRENT_BINARY_DIR}/.bobui/rcc/qrc_${resource_name}_init.cpp")
...
function(_bobui_internal_expose_source_file_to_ide target file)
    get_target_property(target_expects_finalization ${target} _bobui_expects_finalization)
    if(target_expects_finalization AND CMAKE_VERSION VERSION_GREATER_EQUAL "3.19")
        ...
        set_property(TARGET ${target} APPEND PROPERTY _bobui_deferred_files ${file})
        return()
    else()
        get_target_property(is_finalized "${target}" _bobui_is_finalized)
        if(is_finalized)
    ...
    get_source_file_property(
        target_dependency "${file}" ${scope_args} _bobui_resource_target_dependency)
...
# Adds the files that were added to _bobui_deferred_files to SOURCES.
# Sets HEADER_FILES_ONLY if they did not exist yet in SOURCES.
function(_bobui_internal_expose_deferred_files_to_ide target)
    get_target_property(new_sources ${target} _bobui_deferred_files)
    if(NOT new_sources)
        return()
    endif()
...
function(_bobui_internal_escape_xml_characters input output_variable)
    set(no_value_options "")
...
#
# Alias settings for files need to be set via the BOBUI_RESOURCE_ALIAS property
...
#
function(_bobui_internal_process_resource target resourceName)
    _bobui_internal_get_bobui_internal_process_resource_args(options oneValueArgs multiValueArgs)
    ...
    # set/get source properties works incorrect and may have the same BOBUI_RESOURCE_ALIAS
    ...
    if(NOT "${rcc_BASE}" STREQUAL "")
        ...
        foreach(file_path IN LISTS resource_files)
            get_source_file_property(alias "${file_path}" BOBUI_RESOURCE_ALIAS)
            if(alias STREQUAL "NOTFOUND")
                ...
                set_property(SOURCE "${file_path}" PROPERTY BOBUI_RESOURCE_ALIAS "${rel_file}")
            endif()
        endforeach()
    endif()

    if(ANDROID)
        if(COMMAND _bobui_internal_collect_qml_root_paths)
            _bobui_internal_collect_qml_root_paths(${target} ${resource_files})
        endif()
    endif()

    if(NOT rcc_PREFIX)
        get_target_property(rcc_PREFIX ${target} BOBUI_RESOURCE_PREFIX)
        if (NOT rcc_PREFIX)
            set(rcc_PREFIX "/")
        endif()
    endif()
    ...
    set(generatedResourceFile "${CMAKE_CURRENT_BINARY_DIR}/.bobui/rcc/${resourceName}.qrc")
    _bobui_internal_expose_source_file_to_ide(${target} ${generatedResourceFile})
    ...
    # <RCC><qresource ...>
    set(qrcContents "<RCC>\n  <qresource")
    _bobui_internal_escape_xml_characters("${rcc_PREFIX}" escaped_rcc_PREFIX SUBSET [[&<"]])
    ...
    foreach(file IN LISTS resource_files)
        __bobui_get_relative_resource_path_for_file(file_resource_path ${file})

        if (NOT IS_ABSOLUTE ${file})
            set(file "${CMAKE_CURRENT_SOURCE_DIR}/${file}")
        endif()

        get_property(is_empty SOURCE ${file} PROPERTY BOBUI_DISCARD_FILE_CONTENTS)

        # <file ...>...</file>
        # We need XML escaping; alias is a quote enclosed attribute, file is text
        _bobui_internal_escape_xml_characters(
            "${file_resource_path}"
        ...
        _bobui_internal_expose_source_file_to_ide(${target} "${file}")
    endforeach()

    set_property(TARGET ${target}
        APPEND PROPERTY _bobui_resource_source_files ${resource_files})

    # </qresource></RCC>
    string(APPEND qrcContents "  </qresource>\n</RCC>\n")

    get_property(__bobui_core_macros_module_base_dir GLOBAL PROPERTY __bobui_core_macros_module_base_dir)
    set(template_file "${__bobui_core_macros_module_base_dir}/BobUI6CoreConfigureFileTemplate.in")
    set(bobui_core_configure_file_contents "${qrcContents}")
    ...
    # with zstd support, it expects the target BobUICore to be able to decompress zstd compressed
    # content. This might be true with qmake where host tools are built as part of the
    # cross-compiled BobUI, but with CMake we build tools separate from the cross-compiled BobUI.
    ...
    elseif(rcc_BIG_RESOURCES)
        set(generatedOutfile "${CMAKE_CURRENT_BINARY_DIR}/.bobui/rcc/qrc_${resourceName}_tmp.cpp")
    else()
        set(generatedOutfile "${CMAKE_CURRENT_BINARY_DIR}/.bobui/rcc/qrc_${resourceName}.cpp")
    endif()
    ...
    add_custom_command(OUTPUT "${generatedOutfile}"
                       COMMAND "${BOBUI_CMAKE_EXPORT_NAMESPACE}::rcc" ${rccArgs} ${rccArgsAllPasses}
                       DEPENDS
    ...
    if(rcc_BIG_RESOURCES)
        ...
        set(generatedOutfile
            "${CMAKE_CURRENT_BINARY_DIR}/.bobui/rcc/qrc_${resourceName}${CMAKE_CXX_OUTPUT_EXTENSION}")
        _bobui_internal_add_rcc_pass2(
        ...
        if(type STREQUAL STATIC_LIBRARY)
            ...
            __bobui_internal_propagate_object_files(${target} "${generatedOutfile}")
        else()
            target_sources(${target} PRIVATE ${generatedOutfile})
        endif()
    else()
        __bobui_propagate_generated_resource(${target} ${resourceName} "${generatedOutfile}"
            output_targets)
    endif()

    set_property(TARGET ${target}
        APPEND PROPERTY _bobui_generated_qrc_files "${generatedResourceFile}")
...
macro(_bobui_internal_get_add_plugin_keywords option_args single_args multi_args)
    set(${option_args}
        STATIC
        SHARED
...
function(bobui6_add_plugin target)
    _bobui_internal_get_add_plugin_keywords(opt_args single_args multi_args)
    ...
    elseif(arg_SHARED)
        ...
        # If no explicit STATIC/SHARED option is set, default to the flavor of the BobUI build.
    ...
    # The default of _bobui_internal_add_library creates SHARED in a shared BobUI build, so we need to
    ...
    _bobui_internal_add_library(${target} ${type_to_create} ${arg_UNPARSED_ARGUMENTS})
    set_property(TARGET ${target} PROPERTY _bobui_expects_finalization TRUE)

    get_target_property(target_type "${target}" TYPE)
    if (target_type STREQUAL "STATIC_LIBRARY")
        target_compile_definitions(${target} PRIVATE BOBUI_STATICPLUGIN)
    endif()
    ...
    if (NOT "${arg_PLUGIN_TYPE}" STREQUAL "qml_plugin")
        if (NOT arg_CLASS_NAME)
            ...
            if(NOT "${target}" STREQUAL "${plugin_class_name}"
                AND target_type STREQUAL "STATIC_LIBRARY" AND NOT BOBUI_SKIP_PLUGIN_CLASS_NAME_WARNING)
                message(WARNING "The target name '${target}' is not a valid C++ class name and"
                    ...
                    " explicitly. Setting BOBUI_SKIP_PLUGIN_CLASS_NAME_WARNING to ON suppresses this"
                    " warning.")
            endif()
        else()
    ...
    _bobui_internal_is_c_identifier(is_c_indentifier "${plugin_class_name}")
    ...
    set_target_properties(${target} PROPERTIES BOBUI_PLUGIN_CLASS_NAME "${plugin_class_name}")

    # Create a plugin initializer object library for static plugins.
    # It contains a Q_IMPORT_PLUGIN(BOBUI_PLUGIN_CLASS_NAME) call.
    ...
    # Qml plugin inits are handled in BobUI6QmlMacros.
    if(NOT "${arg_PLUGIN_TYPE}" STREQUAL "qml_plugin"
            AND target_type STREQUAL "STATIC_LIBRARY")
        __bobui_internal_add_static_plugin_init_object_library("${target}" plugin_init_target)

        if(arg_OUTPUT_TARGETS)
            set(${arg_OUTPUT_TARGETS} ${plugin_init_target} PARENT_SCOPE)
        endif()

        # We don't automatically propagate the plugin init library for BobUI provided plugins, because
        # there are 2 other code paths that take care of that, one involving finalizers and the
        # other regular usage requirements.
        if(NOT arg___BOBUI_INTERNAL_NO_PROPAGATE_PLUGIN_INITIALIZER)
            __bobui_internal_propagate_object_library("${target}" "${plugin_init_target}")
        endif()
    else()
        if(arg_OUTPUT_TARGETS)
    ...
    target_compile_definitions(${target} PRIVATE
        BOBUI_PLUGIN
        BOBUI_DEPRECATED_WARNINGS
    )

    if(target_type STREQUAL "MODULE_LIBRARY")
        if(NOT TARGET bobui_internal_plugins)
            add_custom_target(bobui_internal_plugins)
            _bobui_internal_assign_to_internal_targets_folder(bobui_internal_plugins)
        endif()
        add_dependencies(bobui_internal_plugins ${target})
    endif()
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_plugin)
        bobui6_add_plugin(${ARGV})
        cmake_parse_arguments(PARSE_ARGV 1 arg "" "OUTPUT_TARGETS" "")
        if(arg_OUTPUT_TARGETS)
            set(${arg_OUTPUT_TARGETS} ${${arg_OUTPUT_TARGETS}} PARENT_SCOPE)
        endif()
...
# Creates a library by forwarding arguments to add_library, applies some BobUI naming file name naming
# conventions and ensures the execution of BobUI specific finalizers.
function(bobui6_add_library target)
    cmake_parse_arguments(PARSE_ARGV 1 arg "MANUAL_FINALIZATION" "" "")

    _bobui_internal_add_library("${target}" ${arg_UNPARSED_ARGUMENTS})
    set_property(TARGET ${target} PROPERTY _bobui_expects_finalization TRUE)
...
#
# Applies some BobUI specific behaviors:
# - If no type option is specified, rather than defaulting to STATIC it defaults to STATIC or SHARED
#   depending on the BobUI configuration.
# - Applies BobUI specific prefixes and suffixes to file names depending on platform.
function(_bobui_internal_add_library target)
    set(opt_args
        STATIC
        SHARED
        MODULE
    ...
    # If no explicit type option is set, default to the flavor of the BobUI build.
    ...
    if(NOT arg_STATIC AND NOT arg_SHARED AND NOT arg_MODULE AND NOT arg_INTERFACE
            AND NOT arg_OBJECT)
        if(DEFINED BUILD_SHARED_LIBS AND NOT BOBUI_BUILDING_BOBUI AND NOT BOBUI_BUILD_STANDALONE_TESTS)
            __bobui_internal_setup_policy(BOBUIP0003 "6.7.0"
                "BUILD_SHARED_LIBS is set to ${BUILD_SHARED_LIBS} but it has no effect on\
                default library type created by BobUI CMake API commands. The default library type\
                is set to the BobUI build type.\
                This behavior can be changed by setting BOBUIP0003 to NEW.\
                Check https://doc.bobui.io/bobui-6/bobui-cmake-policy-bobuip0003.html for policy details."
            )
            bobui6_policy(GET BOBUIP0003 build_shared_libs_policy)
        else()
            set(build_shared_libs_policy "")
        endif()

        if(build_shared_libs_policy STREQUAL "NEW" OR BOBUI_BUILDING_BOBUI OR BOBUI_BUILD_STANDALONE_TESTS)
            if(BUILD_SHARED_LIBS OR (NOT DEFINED BUILD_SHARED_LIBS AND BOBUI6_IS_SHARED_LIBS_BUILD))
                set(type_to_create SHARED)
            else()
                set(type_to_create STATIC)
            endif()
    ...
    if(NOT type_to_create STREQUAL "INTERFACE" AND NOT type_to_create STREQUAL "OBJECT")
        _bobui_internal_apply_win_prefix_and_suffix("${target}")
    endif()

    if(arg_MODULE AND APPLE)
        # CMake defaults to using .so extensions for loadable modules, aka plugins,
        # but BobUI plugins are actually suffixed with .dylib.
        set_property(TARGET "${target}" PROPERTY SUFFIX ".dylib")
    endif()

    if(ANDROID)
        set_property(TARGET "${target}"
                     PROPERTY _bobui_android_apply_arch_suffix_called_from_bobui_impl TRUE)
        bobui6_android_apply_arch_suffix("${target}")
    endif()
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_add_library)
        bobui6_add_library(${ARGV})
    endfunction()
endif()

# TODO: Remove once all repositories use bobui_internal_add_example instead of add_subdirectory.
macro(_bobui_internal_override_example_install_dir_to_dot)
    # Set INSTALL_EXAMPLEDIR to ".".
    # This overrides the install destination of unclean BobUI example projects to install directly
    # to CMAKE_INSTALL_PREFIX.
    if(BOBUI_INTERNAL_SET_EXAMPLE_INSTALL_DIR_TO_DOT)
        set(INSTALL_EXAMPLEDIR ".")
        set(_bobui_internal_example_dir_set_to_dot TRUE)
    endif()
endmacro()

function(bobui6_allow_non_utf8_sources target)
    set_target_properties("${target}" PROPERTIES BOBUI_NO_UTF8_SOURCE TRUE)
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_allow_non_utf8_sources)
        bobui6_allow_non_utf8_sources(${ARGV})
    endfunction()
endif()

function(_bobui_internal_apply_strict_cpp target)
    ...
    if(NOT BOBUI_ENABLE_CXX_EXTENSIONS)
        get_target_property(target_type "${target}" TYPE)
        if(NOT target_type STREQUAL "INTERFACE_LIBRARY")
            set_target_properties("${target}" PROPERTIES
                CXX_EXTENSIONS OFF
...
# both sets.
function(_bobui_internal_copy_dependency_properties target dependency)
...
Please use _bobui_internal_copy_dependency_properties without these options to copy a set of
...
function(bobui6_disable_unicode_defines target)
    set_target_properties(${target} PROPERTIES BOBUI_NO_UNICODE_DEFINES TRUE)
endfunction()
...
function(bobui6_finalize_project)
    if(NOT CMAKE_CURRENT_SOURCE_DIR STREQUAL CMAKE_SOURCE_DIR)
        message("bobui6_finalize_project is called not in the top-level CMakeLists.txt.")
    endif()
    if(ANDROID)
        _bobui_internal_collect_apk_dependencies()
    endif()
endfunction()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_finalize_project)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_finalize_project()
        else()
            message(FATAL_ERROR "bobui_finalize_project() is only available in BobUI 6.")
        endif()
    endfunction()

    function(bobui_disable_unicode_defines)
        bobui6_disable_unicode_defines(${ARGV})
    endfunction()
endif()

function(_bobui_internal_get_deploy_impl_dir var)
    set(${var} "${CMAKE_BINARY_DIR}/.bobui" PARENT_SCOPE)
endfunction()

function(_bobui_internal_add_deploy_support deploy_support_file)
    get_filename_component(deploy_support_file "${deploy_support_file}" REALPATH)

    set(target ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core)
    get_target_property(aliased_target ${target} ALIASED_TARGET)
    if(aliased_target)
        set(target ${aliased_target})
    endif()

    get_property(scripts TARGET ${target} PROPERTY _bobui_deploy_support_files)
    if(NOT "${deploy_support_file}" IN_LIST scripts)
        set_property(TARGET ${target} APPEND PROPERTY
            _bobui_deploy_support_files "${deploy_support_file}"
        )
    endif()
endfunction()

# Sets up the commands for use at install/deploy time
function(_bobui_internal_setup_deploy_support)
    if(BOBUI_SKIP_SETUP_DEPLOYMENT)
        return()
    endif()
    ...
    # Always set BOBUI_DEPLOY_SUPPORT in the caller's scope, even if we've generated
    ...
    _bobui_internal_get_deploy_impl_dir(deploy_impl_dir)

    get_cmake_property(is_multi_config GENERATOR_IS_MULTI_CONFIG)
    if(is_multi_config)
        set(BOBUI_DEPLOY_SUPPORT "${deploy_impl_dir}/BobUIDeploySupport-$<CONFIG>.cmake")
    else()
        set(BOBUI_DEPLOY_SUPPORT "${deploy_impl_dir}/BobUIDeploySupport.cmake")
    endif()
    set(BOBUI_DEPLOY_SUPPORT "${BOBUI_DEPLOY_SUPPORT}" PARENT_SCOPE)

    get_property(have_generated_file GLOBAL PROPERTY _bobui_have_generated_deploy_support)
    if(have_generated_file)
        return()
    endif()
    set_property(GLOBAL PROPERTY _bobui_have_generated_deploy_support TRUE)
    ...
    if(CMAKE_VERSION GREATER_EQUAL "3.19.0")
        if(is_multi_config)
            set(targets_file "${deploy_impl_dir}/BobUIDeployTargets-$<CONFIG>.cmake")
        else()
            set(targets_file "${deploy_impl_dir}/BobUIDeployTargets.cmake")
        endif()
        cmake_language(EVAL CODE
            "cmake_language(DEFER
                DIRECTORY [[${CMAKE_SOURCE_DIR}]]
                CALL _bobui_internal_write_target_deploy_info [[${targets_file}]])"
        )
        _bobui_internal_add_deploy_support("${targets_file}")
    endif()

    # Make sure to look under the BobUI bin dir with find_program, rather than randomly picking up
    # a deploybobui tool in the system.
    # BOBUI6_INSTALL_PREFIX is not set during BobUI build, so add the hints conditionally.
    set(find_program_hints)
    if(BOBUI6_INSTALL_PREFIX)
        set(find_program_hints HINTS ${BOBUI6_INSTALL_PREFIX}/${BOBUI6_INSTALL_BINS})
    endif()
    ...
    # tool, so we cannot use something like $<TARGET_FILE:macdeploybobui> directly.
    if(APPLE AND NOT IOS)
        find_program(MACDEPLOYBOBUI_EXECUTABLE macdeploybobui
            ${find_program_hints})
        set(fallback "$<$<BOOL:${MACDEPLOYBOBUI_EXECUTABLE}>:${MACDEPLOYBOBUI_EXECUTABLE}>")
        set(target_if_exists "$<TARGET_NAME_IF_EXISTS:${BOBUI_CMAKE_EXPORT_NAMESPACE}::macdeploybobui>")
        set(have_deploy_tool "$<BOOL:${target_if_exists}>")
        set(safe_target_file
            "$<TARGET_FILE:$<IF:${have_deploy_tool},${target_if_exists},${target}>>")
        set(__BOBUI_DEPLOY_TOOL "$<IF:${have_deploy_tool},${safe_target_file},${fallback}>")
    elseif(WIN32)
        find_program(WINDEPLOYBOBUI_EXECUTABLE windeploybobui
            ${find_program_hints})
        set(fallback "$<$<BOOL:${WINDEPLOYBOBUI_EXECUTABLE}>:${WINDEPLOYBOBUI_EXECUTABLE}>")
        set(target_if_exists "$<TARGET_NAME_IF_EXISTS:${BOBUI_CMAKE_EXPORT_NAMESPACE}::windeploybobui>")
        set(have_deploy_tool "$<BOOL:${target_if_exists}>")
        set(safe_target_file
            "$<TARGET_FILE:$<IF:${have_deploy_tool},${target_if_exists},${target}>>")
        set(__BOBUI_DEPLOY_TOOL "$<IF:${have_deploy_tool},${safe_target_file},${fallback}>")
    elseif(UNIX AND NOT APPLE AND NOT ANDROID AND NOT CMAKE_CROSSCOMPILING)
        set(__BOBUI_DEPLOY_TOOL "GRD")
    else()
        # Android is handled as a build target, not via this install-based approach.
        # Therefore, we don't consider androiddeploybobui here.
        set(__BOBUI_DEPLOY_TOOL "")
    endif()
    ...
    _bobui_internal_add_deploy_support("${CMAKE_CURRENT_LIST_DIR}/BobUI6CoreDeploySupport.cmake")

    set(deploy_ignored_lib_dirs "")
    if(__BOBUI_DEPLOY_TOOL STREQUAL "GRD" AND NOT "${BOBUI6_INSTALL_PREFIX}" STREQUAL "")
        # Set up the directories we want to ignore when running file(GET_RUNTIME_DEPENDENCIES).
        # If the BobUI prefix is the root of one of those directories, don't ignore that directory.
        # For example, if BobUI's installation prefix is /usr, then we don't want to ignore /usr/lib.
        foreach(link_dir IN LISTS CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES)
            file(RELATIVE_PATH relative_dir "${BOBUI6_INSTALL_PREFIX}" "${link_dir}")
            if(relative_dir STREQUAL "")
                # The BobUI prefix is exactly ${link_dir}.
                continue()
            endif()
            if(IS_ABSOLUTE "${relative_dir}" OR relative_dir MATCHES "^\\.\\./")
                # The BobUI prefix is outside of ${link_dir}.
    ...
    if("${BOBUI_DEPLOY_FORCE_ADJUST_RPATHS}" STREQUAL "")
        ...
        set(must_adjust_plugins_rpath "${BOBUI_DEPLOY_FORCE_ADJUST_RPATHS}")
    endif()

    # Find the patchelf executable if necessary.
    if(must_adjust_plugins_rpath)
        if(CMAKE_VERSION VERSION_LESS "3.21")
            set(BOBUI_DEPLOY_USE_PATCHELF ON)
        endif()
        if(BOBUI_DEPLOY_USE_PATCHELF)
            find_program(BOBUI_DEPLOY_PATCHELF_EXECUTABLE patchelf)
            if(NOT BOBUI_DEPLOY_PATCHELF_EXECUTABLE)
                set(BOBUI_DEPLOY_PATCHELF_EXECUTABLE "patchelf")
                message(WARNING "The patchelf executable could not be located. "
                    "To use BobUI's CMake deployment API, install patchelf or upgrade CMake to 3.21 "
                    "or newer.")
            endif()
        endif()
    endif()

    # Generate path to the bobuipaths executable or script, that will give info about the target
    # platform, but which can run on the host. Needed for windeploybobui when cross-compiling from
    # an x86_64 host to an arm64 target, so it knows which architecture libraries should be
    # deployed.
    set(base_name "bobuipaths")
    set(base_names "")

    get_property(bobui_major_version TARGET "${target}" PROPERTY INTERFACE_BOBUI_MAJOR_VERSION)
    if(bobui_major_version)
        list(APPEND base_names "${base_name}${bobui_major_version}")
    endif()
    list(APPEND base_names "${base_name}")

    set(bobuipaths_name_candidates "")
    foreach(base_name IN LISTS base_names)
        if(CMAKE_HOST_WIN32)
            if(CMAKE_CROSSCOMPILING)
                set(bobui_paths_ext ".bat")
                # Depending on whether BOBUI_FORCE_BUILD_TOOLS was set when building BobUI, a 'host-'
                # prefix is prepended to the created bobuipaths wrapper, not to collide with the
                # cross-compiled excutable.
                # Rather than exporting that BOBUI_FORCE_BUILD_TOOLS to be available during user
                # project configuration, search for both, with the bare one searched first.
                list(APPEND bobuipaths_name_candidates "${base_name}${bobui_paths_ext}")
                list(APPEND bobuipaths_name_candidates "host-${base_name}${bobui_paths_ext}")
            else()
                set(bobui_paths_ext ".exe")
                list(APPEND bobuipaths_name_candidates "${base_name}${bobui_paths_ext}")
            endif()
        else()
            list(APPEND bobuipaths_name_candidates "${base_name}")
        endif()
    endforeach()

    set(bobuipaths_prefix "${BOBUI6_INSTALL_PREFIX}/${BOBUI6_INSTALL_BINS}")

    set(candidate_paths "")
    foreach(bobuipaths_name_candidate IN LISTS bobuipaths_name_candidates)
        set(candidate_path "${bobuipaths_prefix}/${bobuipaths_name_candidate}")
        list(APPEND candidate_paths "${candidate_path}")
    endforeach()

    set(target_bobuipaths_path "")
    foreach(candidate_path IN LISTS candidate_paths)
        if(EXISTS "${candidate_path}")
            set(target_bobuipaths_path "${candidate_path}")
            break()
        endif()
    endforeach()
    ...
    if(WIN32 AND NOT BOBUI_BUILDING_BOBUI AND NOT BOBUI_NO_BOBUIPATHS_DEPLOYMENT_WARNING
        AND NOT target_bobuipaths_path)
        message(WARNING
            "No bobuipaths executable found for deployment purposes. Candidates searched: \n    "
            "${candidate_paths_joined}"
        )
    endif()

    file(GENERATE OUTPUT "${BOBUI_DEPLOY_SUPPORT}" CONTENT
...
if(NOT BOBUI_DEPLOY_BIN_DIR)
    set(BOBUI_DEPLOY_BIN_DIR \"${CMAKE_INSTALL_BINDIR}\")
endif()
if(NOT BOBUI_DEPLOY_LIBEXEC_DIR)
    set(BOBUI_DEPLOY_LIBEXEC_DIR \"${CMAKE_INSTALL_LIBEXECDIR}\")
endif()
if(NOT BOBUI_DEPLOY_LIB_DIR)
    set(BOBUI_DEPLOY_LIB_DIR \"${CMAKE_INSTALL_LIBDIR}\")
endif()
if(NOT BOBUI_DEPLOY_PLUGINS_DIR)
    set(BOBUI_DEPLOY_PLUGINS_DIR \"${BOBUI6_INSTALL_PLUGINS}\")
endif()
if(NOT BOBUI_DEPLOY_QML_DIR)
    set(BOBUI_DEPLOY_QML_DIR \"${BOBUI6_INSTALL_QML}\")
endif()
if(NOT BOBUI_DEPLOY_TRANSLATIONS_DIR)
    set(BOBUI_DEPLOY_TRANSLATIONS_DIR \"${BOBUI6_INSTALL_TRANSLATIONS}\")
endif()
if(NOT BOBUI_DEPLOY_PREFIX)
    set(BOBUI_DEPLOY_PREFIX \"\$ENV{DESTDIR}\${CMAKE_INSTALL_PREFIX}\")
endif()
if(BOBUI_DEPLOY_PREFIX STREQUAL \"\")
    set(BOBUI_DEPLOY_PREFIX .)
endif()
if(NOT BOBUI_DEPLOY_IGNORED_LIB_DIRS)
    set(BOBUI_DEPLOY_IGNORED_LIB_DIRS \"${deploy_ignored_lib_dirs}\")
endif()

# These are internal implementation details. They may be removed at any time.
set(__BOBUI_DEPLOY_SYSTEM_NAME \"${CMAKE_SYSTEM_NAME}\")
set(__BOBUI_DEPLOY_SHARED_LIBRARY_SUFFIX \"${CMAKE_SHARED_LIBRARY_SUFFIX}\")
set(__BOBUI_DEPLOY_IS_SHARED_LIBS_BUILD \"${BOBUI6_IS_SHARED_LIBS_BUILD}\")
set(__BOBUI_DEPLOY_TOOL \"${__BOBUI_DEPLOY_TOOL}\")
set(__BOBUI_DEPLOY_IMPL_DIR \"${deploy_impl_dir}\")
set(__BOBUI_DEPLOY_VERBOSE \"${BOBUI_ENABLE_VERBOSE_DEPLOYMENT}\")
set(__BOBUI_CMAKE_EXPORT_NAMESPACE \"${BOBUI_CMAKE_EXPORT_NAMESPACE}\")
set(__BOBUI_LIBINFIX \"${BOBUI_LIBINFIX}\")
set(__BOBUI_DEPLOY_GENERATOR_IS_MULTI_CONFIG \"${is_multi_config}\")
set(__BOBUI_DEPLOY_ACTIVE_CONFIG \"$<CONFIG>\")
set(__BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS \"${BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS}\")
set(__BOBUI_DEFAULT_MAJOR_VERSION \"${BOBUI_DEFAULT_MAJOR_VERSION}\")
set(__BOBUI_DEPLOY_BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH \"${BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_PREFIX \"${BOBUI6_INSTALL_PREFIX}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_BINS \"${BOBUI6_INSTALL_BINS}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_DATA \"${BOBUI6_INSTALL_DATA}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_DESCRIPTIONSDIR \"${BOBUI6_INSTALL_DESCRIPTIONSDIR}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_LIBEXECS \"${BOBUI6_INSTALL_LIBEXECS}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_PLUGINS \"${BOBUI6_INSTALL_PLUGINS}\")
set(__BOBUI_DEPLOY_BOBUI_INSTALL_TRANSLATIONS \"${BOBUI6_INSTALL_TRANSLATIONS}\")
set(__BOBUI_DEPLOY_TARGET_BOBUI_PATHS_PATH \"${target_bobuipaths_path}\")
set(__BOBUI_DEPLOY_MUST_ADJUST_PLUGINS_RPATH \"${must_adjust_plugins_rpath}\")
set(__BOBUI_DEPLOY_USE_PATCHELF \"${BOBUI_DEPLOY_USE_PATCHELF}\")
set(__BOBUI_DEPLOY_PATCHELF_EXECUTABLE \"${BOBUI_DEPLOY_PATCHELF_EXECUTABLE}\")
set(__BOBUI_DEPLOY_BOBUI_IS_MULTI_CONFIG_BUILD_WITH_DEBUG \"${is_multi_config_build_with_debug_config}\")
set(__BOBUI_DEPLOY_BOBUI_DEBUG_POSTFIX \"${BOBUI6_DEBUG_POSTFIX}\")

# Define the CMake commands to be made available during deployment.
set(__bobui_deploy_support_files
    \"$<JOIN:$<TARGET_GENEX_EVAL:${target},$<TARGET_PROPERTY:${target},_bobui_deploy_support_files>>,\"
    \">\"
)
foreach(__bobui_deploy_support_file IN LISTS __bobui_deploy_support_files)
    include(\"\${__bobui_deploy_support_file}\")
endforeach()

unset(__bobui_deploy_support_file)
unset(__bobui_deploy_support_files)
")
endfunction()

# Write deployment information for the targets of the project.
function(_bobui_internal_write_target_deploy_info out_file)
    set(targets "")
    ...
    foreach(target IN LISTS targets)
        set(var_prefix "__BOBUI_DEPLOY_TARGET_${target}")
...
function(_bobui_internal_is_examples_deployment_supported_in_current_config out_var out_var_reason)
    # Deployment API doesn't work when examples / tests are built in-tree of a prefix bobui build.
    if(BOBUI_BUILDING_BOBUI AND BOBUI_WILL_INSTALL AND NOT BOBUI_INTERNAL_BUILD_STANDALONE_PARTS)
...
function(_bobui_internal_should_skip_deployment_api out_var out_var_reason)
    set(skip_deployment FALSE)
    _bobui_internal_is_examples_deployment_supported_in_current_config(
    ...
    if(BOBUI_INTERNAL_SKIP_DEPLOYMENT OR (NOT deployment_supported))
    ...
    elseif(BOBUI_INTERNAL_SKIP_DEPLOYMENT)
...
function(_bobui_internal_should_skip_post_build_deployment_api out_var out_var_reason)
    set(skip_deployment FALSE)
    set(deployment_supported TRUE)
...
# The warning can be hidden by setting the BOBUI_INTERNAL_HIDE_NO_OP_DEPLOYMENT_WARNING variable.
function(_bobui_internal_generate_no_op_deploy_script)
    set(no_value_options
    )
    ...
    set(content "
message(DEBUG \"Running no-op deployment script because BOBUI_INTERNAL_SKIP_DEPLOYMENT was ON.\")
")
    if(NOT BOBUI_INTERNAL_HIDE_NO_OP_DEPLOYMENT_WARNING AND arg_SKIP_REASON STREQUAL "PREFIX_BUILD")
        set(content "
...
\"or bobui-internal-configure-tests or bobui-internal-configure-examples if you want deployment to run.\")
")
    endif()

    bobui6_generate_deploy_script(
        ${generate_args}
...
#
function(bobui6_policy mode policy behaviorOrVariable)
    # When building BobUI, tests and examples might expect a policy to be known, but they won't be
    ...
    if(BOBUI_BUILDING_BOBUI AND NOT DEFINED BOBUI_KNOWN_POLICY_${policy})
        get_property(global_known_policies GLOBAL PROPERTY _bobui_global_known_policies)
        if(policy IN_LIST global_known_policies)
            set(BOBUI_KNOWN_POLICY_${policy} TRUE)
        endif()
    endif()

    if (NOT DEFINED BOBUI_KNOWN_POLICY_${policy})
        message(FATAL_ERROR
            "${policy} is not a known BobUI policy. Did you include the necessary BobUI module?"
        )
    endif()
    if (${mode} STREQUAL "SET")
        set(behavior ${behaviorOrVariable})
        if (${behavior} STREQUAL "NEW" OR ${behavior} STREQUAL "OLD")
            set(__BOBUI_INTERNAL_POLICY_${policy} ${behavior} PARENT_SCOPE)
        else()
            message(FATAL_ERROR "BobUI policies must be either set to NEW or OLD, but got ${behavior}")
        endif()
    else(${mode} STREQUAL "GET")
        set(variable "${behaviorOrVariable}")
        set("${variable}" "${__BOBUI_INTERNAL_POLICY_${policy}}" PARENT_SCOPE)
    endif()
endfunction()

# Internal helper function; can be used in any module before doing a policy check
function(__bobui_internal_setup_policy policy sinceversion policyexplanation)
    if(DEFINED __BOBUI_INTERNAL_POLICY_${policy})
        if (__BOBUI_INTERNAL_POLICY_${policy} STREQUAL "OLD")
            # policy is explicitly disabled
            message(DEPRECATION
                "BobUI policy ${policy} is set to OLD. "
                "Support for the old behavior will be removed in a future major version of BobUI."
            )
        endif()
        #else: policy is already enabled, nothing to do
    elseif (${sinceversion} VERSION_LESS_EQUAL __bobui_policy_check_version)
        # we cannot use the public function here as we want to set it in parent scope
        set(__BOBUI_INTERNAL_POLICY_${policy} "NEW" PARENT_SCOPE)
    elseif(NOT "${BOBUI_NO_SHOW_OLD_POLICY_WARNINGS}")
        message(AUTHOR_WARNING
            "BobUI policy ${policy} is not set: "
            "${policyexplanation} "
            "Use the bobui_policy command to set the policy and suppress this warning.\n"
        )
    endif()
endfunction()
...
# calling scope.
macro(bobui6_standard_project_setup)
    ...
    if(NOT BOBUI_NO_STANDARD_PROJECT_SETUP)

        set(__bobui_sps_args_option)
        set(__bobui_sps_args_single
            REQUIRES
            SUPPORTS_UP_TO
            I18N_SOURCE_LANGUAGE
        )
        set(__bobui_sps_args_multi
            I18N_TRANSLATED_LANGUAGES
        )
        cmake_parse_arguments(__bobui_sps_arg
            "${__bobui_sps_args_option}"
            "${__bobui_sps_args_single}"
            "${__bobui_sps_args_multi}"
            ${ARGN}
        )

        if(__bobui_sps_arg_UNPARSED_ARGUMENTS)
            message(FATAL_ERROR "Unexpected arguments: ${arg_UNPARSED_ARGUMENTS}")
        endif()

        # Set the BobUI CMake policy based on the requested version(s)
        set(__bobui_policy_check_version "6.0.0")
        if(BobUI6_VERSION_MAJOR)
            set(__bobui_current_version
                "${BobUI6_VERSION_MAJOR}.${BobUI6_VERSION_MINOR}.${BobUI6_VERSION_PATCH}")
        elseif(BOBUI_BUILDING_BOBUI)
            set(__bobui_current_version
                "${PROJECT_VERSION_MAJOR}.${PROJECT_VERSION_MINOR}.${PROJECT_VERSION_PATCH}")
        else()
            message(FATAL_ERROR "Can not determine BobUI version.")
        endif()
        if(__bobui_sps_arg_REQUIRES)
            if("${__bobui_current_version}" VERSION_LESS "${__bobui_sps_arg_REQUIRES}")
                message(FATAL_ERROR
                    "Project required a BobUI minimum version of ${__bobui_sps_arg_REQUIRES}, "
                    "but current version is only ${__bobui_current_version}.")
            endif()
            set(__bobui_policy_check_version "${__bobui_sps_arg_REQUIRES}")
        endif()
        if(__bobui_sps_arg_SUPPORTS_UP_TO)
            if(__bobui_sps_arg_REQUIRES)
                if(${__bobui_sps_arg_SUPPORTS_UP_TO} VERSION_LESS ${__bobui_sps_arg_REQUIRES})
                    message(FATAL_ERROR "SUPPORTS_UP_TO must be larger than or equal to REQUIRES.")
                endif()
                set(__bobui_policy_check_version "${__bobui_sps_arg_SUPPORTS_UP_TO}")
            else()
        ...
        elseif(NOT APPLE)
            ...
            # are based on).
            file(RELATIVE_PATH __bobui_relDir
                ${CMAKE_CURRENT_BINARY_DIR}/${CMAKE_INSTALL_BINDIR}
                ${CMAKE_CURRENT_BINARY_DIR}/${CMAKE_INSTALL_LIBDIR}
            )
            list(APPEND CMAKE_INSTALL_RPATH $ORIGIN $ORIGIN/${__bobui_relDir})
            list(REMOVE_DUPLICATES CMAKE_INSTALL_RPATH)
            unset(__bobui_reldir)
        endif()
        ...
        get_property(__bobui_use_folders GLOBAL PROPERTY USE_FOLDERS)
        if(__bobui_use_folders OR "${__bobui_use_folders}" STREQUAL "")
            set_property(GLOBAL PROPERTY USE_FOLDERS ON)
            get_property(__bobui_bobui_targets_folder GLOBAL PROPERTY BOBUI_TARGETS_FOLDER)
            if("${__bobui_bobui_targets_folder}" STREQUAL "")
                set(__bobui_bobui_targets_folder BobUIInternalTargets)
                set_property(GLOBAL PROPERTY BOBUI_TARGETS_FOLDER ${__bobui_bobui_targets_folder})
            endif()
            get_property(__bobui_autogen_targets_folder GLOBAL PROPERTY AUTOGEN_TARGETS_FOLDER)
            if("${__bobui_autogen_targets_folder}" STREQUAL "")
                set_property(GLOBAL PROPERTY AUTOGEN_TARGETS_FOLDER ${__bobui_bobui_targets_folder})
            endif()
        endif()
        ...
        # Headers front and center. See also _bobui_internal_finalize_source_groups
        ...
        # I18N support.
        if(DEFINED __bobui_sps_arg_I18N_TRANSLATED_LANGUAGES
                AND NOT DEFINED BOBUI_I18N_TRANSLATED_LANGUAGES)
            set(BOBUI_I18N_TRANSLATED_LANGUAGES ${__bobui_sps_arg_I18N_TRANSLATED_LANGUAGES})
        endif()
        if(NOT DEFINED __bobui_sps_arg_I18N_SOURCE_LANGUAGE)
            set(__bobui_sps_arg_I18N_SOURCE_LANGUAGE en)
        endif()
        if(NOT DEFINED BOBUI_I18N_SOURCE_LANGUAGE)
            set(BOBUI_I18N_SOURCE_LANGUAGE ${__bobui_sps_arg_I18N_SOURCE_LANGUAGE})
        endif()
    endif()
endmacro()

if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    macro(bobui_standard_project_setup)
        bobui6_standard_project_setup(${ARGV})
    endmacro()
    macro(bobui_policy)
        bobui6_policy(${ARGV})
    endmacro()
...
# Store in ${out_var} the i18n catalogs that belong to the passed BobUI modules.
# The catalog "bobuibase" is always added to the result.
#
# Example:
#     _bobui_internal_get_i18n_catalogs_for_modules(catalogs Quick Help)
#     catalogs -> bobuibase;bobuideclarative;bobui_help
function(_bobui_internal_get_i18n_catalogs_for_modules out_var)
    set(result "bobuibase")
    set(modules "${ARGN}")
    set(module_catalog_mapping
        "Bluetooth|Nfc" bobuiconnectivity
        "Help" bobui_help
        "Multimedia(Widgets|QuickPrivate)?" bobuimultimedia
        "Qml|Quick" bobuideclarative
        "SerialPort" bobuiserialport
        "WebEngine" bobuiwebengine
        "WebSockets" bobuiwebsockets
    )
...
function(bobui6_generate_deploy_script)
    set(no_value_options "")
    set(single_value_options
        CONTENT
        OUTPUT_SCRIPT
    ...
    if(CMAKE_VERSION VERSION_LESS "3.19")
        get_target_property(is_immediately_finalized ${arg_TARGET} _bobui_is_immediately_finalized)
        if(is_immediately_finalized)
            message(WARNING
                ...
                "bobui_finalize_target(${arg_TARGET}) after generating the deployment script."
            )
        endif()
    endif()
    ...
    _bobui_internal_get_i18n_catalogs_for_modules(catalogs ${BOBUI_ALL_MODULES_FOUND_VIA_FIND_PACKAGE})
    set(boiler_plate "include(\"${BOBUI_DEPLOY_SUPPORT}\")
include(\"\${CMAKE_CURRENT_LIST_DIR}/${arg_TARGET}-plugins${config_infix}.cmake\" OPTIONAL)
set(__BOBUI_DEPLOY_I18N_CATALOGS \"${catalogs}\")
")
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    macro(bobui_generate_deploy_script)
        if(BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_generate_deploy_script(${ARGV})
        else()
            message(FATAL_ERROR "bobui_generate_deploy_script() is only available in BobUI 6.")
        endif()
    endmacro()
endif()

function(bobui6_generate_deploy_app_script)
    ...
    set(bobui_deploy_runtime_dependencies_options
        # These options are forwarded as is to bobui_deploy_runtime_dependencies.
        DEPLOY_TOOL_OPTIONS
        EXCLUDE_PLUGINS
    ...
    set(multi_value_options
        ${bobui_deploy_runtime_dependencies_options}
    )
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "${no_value_options}" "${single_value_options}" "${multi_value_options}"
    )
    ...
    if(BOBUI6_IS_SHARED_LIBS_BUILD)
        set(bobui_build_type_string "shared BobUI libs")
    else()
        set(bobui_build_type_string "static BobUI libs")
    endif()

    if(CMAKE_CROSSCOMPILING)
        string(APPEND bobui_build_type_string ", cross-compiled")
    endif()

    if(NOT is_bundle)
        string(APPEND bobui_build_type_string ", non-bundle app")
        set(unsupported_platform_extra_message
    ...
    # Forward the arguments that are exactly the same for bobui_deploy_runtime_dependencies.
    foreach(var IN LISTS bobui_deploy_runtime_dependencies_options)
        if(NOT "${arg_${var}}" STREQUAL "")
            list(APPEND common_deploy_args ${var} ${arg_${var}})
        endif()
    endforeach()

    _bobui_internal_should_skip_deployment_api(skip_deployment skip_reason)
    if(skip_deployment)
        _bobui_internal_generate_no_op_deploy_script(
            FUNCTION_NAME "bobui6_generate_deploy_app_script"
            SKIP_REASON "${skip_reason}"
            ${generate_args}
        )
    elseif(APPLE AND NOT IOS AND BOBUI6_IS_SHARED_LIBS_BUILD AND is_bundle)
        # TODO: Consider handling non-bundle applications in the future using the generic cmake
        # runtime dependency feature.
        bobui6_generate_deploy_script(${generate_args}
            CONTENT "
bobui6_deploy_runtime_dependencies(
    EXECUTABLE \"$<TARGET_FILE_NAME:${arg_TARGET}>.app\"
${common_deploy_args})
")

    elseif(WIN32 AND BOBUI6_IS_SHARED_LIBS_BUILD)
        bobui6_generate_deploy_script(${generate_args}
            CONTENT "
bobui6_deploy_runtime_dependencies(
...
")
    ...
    elseif(UNIX AND NOT APPLE AND NOT ANDROID AND BOBUI6_IS_SHARED_LIBS_BUILD
...
")
    ...
    elseif(NOT arg_NO_UNSUPPORTED_PLATFORM_ERROR AND NOT BOBUI_INTERNAL_NO_UNSUPPORTED_PLATFORM_ERROR)
        # Currently we don't deploy runtime dependencies if cross-compiling or using a static BobUI.
        ...
        message(FATAL_ERROR
            ...
            "this target platform (${CMAKE_SYSTEM_NAME}, ${bobui_build_type_string}). "
            ${unsupported_platform_extra_message}
        )
    else()
        set(skip_message
            "_bobui_internal_show_skip_runtime_deploy_message(\"${bobui_build_type_string}\"")
        if(unsupported_platform_extra_message)
            string(APPEND skip_message
                "\n    EXTRA_MESSAGE \"${unsupported_platform_extra_message}\"")
        endif()
        string(APPEND skip_message "\n)")
        bobui6_generate_deploy_script(${generate_args} CONTENT "${skip_message}")
    endif()
...
if(NOT BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    macro(bobui_generate_deploy_app_script)
        bobui6_generate_deploy_app_script(${ARGV})
    endmacro()
endif()
