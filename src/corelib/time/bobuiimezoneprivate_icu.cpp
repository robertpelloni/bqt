...
// BobUI wrapper around ucal_getDefaultTimeZone()
...
// BobUI wrapper around ucal_get() for offsets
...
// BobUI wrapper around bobui_ucal_getTimeZoneTransitionDate & ucal_get
static BOBUIimeZonePrivate::Data ucalTimeZoneTransition(UCalendar *m_ucal,
                                                     UTimeZoneTransitionType type,
                                                     qint64 atMSecsSinceEpoch)
{
    BOBUIimeZonePrivate::Data tran;
    ...
    BOBUIimeZone::TimeType timeType = dst == 0 ? BOBUIimeZone::StandardTime : BOBUIimeZone::DaylightTime;
    using namespace BobUITimeZoneLocale;
    tran.abbreviation = ucalTimeZoneDisplayName(m_ucal, timeType,
                                                BOBUIimeZone::ShortName, QLocale().name().toUtf8());
    return tran;
}
...
// BobUI wrapper around ucal_getDSTSavings()
...
QIcuTimeZonePrivate::QIcuTimeZonePrivate(const QIcuTimeZonePrivate &other)
    : BOBUIimeZonePrivate(other), m_ucal(nullptr)
{
...
QString QIcuTimeZonePrivate::displayName(BOBUIimeZone::TimeType timeType,
                                         ...
                                         const QLocale &locale) const
{
    ...
    if (nameType == BOBUIimeZone::OffsetName) {
        ...
        if (timeType == BOBUIimeZone::DaylightTime)
...
BOBUIimeZonePrivate::Data QIcuTimeZonePrivate::data(qint64 forMSecsSinceEpoch) const
{
...
#endif
    {
        ...
        BOBUIimeZone::TimeType timeType
            = data.daylightTimeOffset ? BOBUIimeZone::DaylightTime : BOBUIimeZone::StandardTime;
        data.abbreviation = ucalTimeZoneDisplayName(m_ucal, timeType, BOBUIimeZone::ShortName,
                                                    QLocale().name().toUtf8());
    }
...
BOBUIimeZonePrivate::Data QIcuTimeZonePrivate::nextTransition(qint64 afterMSecsSinceEpoch) const
{
    // Available in ICU C++ api, and draft C api in v50
#if U_ICU_VERSION_MAJOR_NUM >= 50
...
BOBUIimeZonePrivate::Data QIcuTimeZonePrivate::previousTransition(qint64 beforeMSecsSinceEpoch) const
{
    // Available in ICU C++ api, and draft C api in v50
#if U_ICU_VERSION_MAJOR_NUM >= 50
...
{
    return BobUITimeZoneLocale::ucalKnownTimeZoneId(QString::fromUtf8(ianaId));
}
...
// TODO Available directly in C++ api but not C api, from 4.8 onwards new filter method works
    ...
    return result;
#else
    return BOBUIimeZonePrivate::availableTimeZoneIds(offsetFromUtc);
#endif
}
