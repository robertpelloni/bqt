...
BOBUI_REQUIRE_CONFIG(timezone_locale);
...
inline bool timeToMSecs(QDate date, BOBUIime time, qint64 *msecs)
{
    qint64 dayms = 0;
...
qint64 calculateTransitionForYear(const SYSTEMTIME &rule, int year, int bias)
{
    ...
    const BOBUIime time = BOBUIime(rule.wHour, rule.wMinute, rule.wSecond);
    qint64 msecs = 0;
    if (date.isValid() && time.isValid() && !timeToMSecs(date, time, &msecs)) {
        ...
        return bias && qAddOverflow(msecs, qint64(bias) * 60000, &msecs)
            ? (bias < 0 ? BOBUIimeZonePrivate::minMSecs() : BOBUIimeZonePrivate::maxMSecs())
            : qMax(BOBUIimeZonePrivate::minMSecs(), msecs);
    }
    return BOBUIimeZonePrivate::invalidMSecs();
}
...
struct TransitionTimePair
{
          ...
          dst(calculateTransitionForYear(rule.daylightTimeRule, year, rule.standardTimeBias))
    {
        ...
        */
        if (rule.standardTimeBias + rule.daylightTimeBias == oldYearOffset
            ...
            dst = BOBUIimeZonePrivate::invalidMSecs();
            fakesDst = true;
        }
        if (rule.standardTimeBias == oldYearOffset
            ...
            std = BOBUIimeZonePrivate::invalidMSecs();
            fakesDst = true;
        }
    }
    ...
    bool startsInDst() const
    {
        ...
        return std != BOBUIimeZonePrivate::invalidMSecs()
            && (std < dst || dst == BOBUIimeZonePrivate::invalidMSecs());
    }
    ...
    BOBUIimeZonePrivate::Data ruleToData(const QWinTimeZonePrivate::QWinTransitionRule &rule,
                                      const QWinTimeZonePrivate *tzp, bool isDst) const
    {
        const auto type = isDst ? BOBUIimeZone::DaylightTime : BOBUIimeZone::StandardTime;
        ...
        if (fakesDst && time == BOBUIimeZonePrivate::invalidMSecs())
            time = isDst ? std : dst;
            ...
            if (dynamicKey.isValid()) {
                ...
                for (int year = startYear; year <= endYear; ++year) {
                    ...
                    QWinTransitionRule rule = readRegistryRule(dynamicKey,
                                                               bobui_castToWchar(QString::number(year)),
                                                               &ruleOk);
                    if (ruleOk
...
QString QWinTimeZonePrivate::displayName(BOBUIimeZone::TimeType timeType,
                                         BOBUIimeZone::NameType nameType,
                                         const QLocale &locale) const
{
    // Registry gave us long names for the system locale:
    if (nameType == BOBUIimeZone::LongName && locale == QLocale::system()) {
        switch (timeType) {
        case  BOBUIimeZone::DaylightTime :
            return m_daylightName;
        case  BOBUIimeZone::GenericTime :
            return m_displayName;
        case  BOBUIimeZone::StandardTime :
...
BOBUIimeZonePrivate::Data QWinTimeZonePrivate::data(qint64 forMSecsSinceEpoch) const
{
    ...
    for (int ruleIndex = ruleIndexForYear(m_tranRules, year);
        ...
        if (year < rule.startYear
            ...
            return ruleToData(rule, forMSecsSinceEpoch, BOBUIimeZone::StandardTime);
        }
        ...
        while (year >= endYear) {
            ...
            return ruleToData(rule, forMSecsSinceEpoch,
                              isDst ? BOBUIimeZone::DaylightTime : BOBUIimeZone::StandardTime,
                              pair.fakesDst);
        }
...
BOBUIimeZonePrivate::Data QWinTimeZonePrivate::nextTransition(qint64 afterMSecsSinceEpoch) const
{
...
BOBUIimeZonePrivate::Data QWinTimeZonePrivate::previousTransition(qint64 beforeMSecsSinceEpoch) const
{
        ...
        if (year >= rule.startYear
            ...
            while (year >= endYear) {
                ...
                if (ruleIndex == 0 && pair.beforeInitialDst(year, beforeMSecsSinceEpoch - 1))
                    return ruleToData(rule, minMSecs(), BOBUIimeZone::StandardTime, false);
...
BOBUIimeZonePrivate::Data QWinTimeZonePrivate::ruleToData(const QWinTransitionRule &rule,
                                                       qint64 atMSecsSinceEpoch,
                                                       BOBUIimeZone::TimeType type,
                                                       bool fakeDst) const
{
    Data tran;
    ...
    if (fakeDst) {
        ...
        if (type == BOBUIimeZone::DaylightTime)
            tran.standardTimeOffset += rule.daylightTimeBias * -60;
    } else if (type == BOBUIimeZone::DaylightTime) {
    ...
    tran.abbreviation = localeName(atMSecsSinceEpoch, tran.offsetFromUtc,
                                   type, BOBUIimeZone::ShortName, QLocale::system());
    return tran;
}
