...
#ifndef BOBUIIMEZONELOCALE_P_H
#define BOBUIIMEZONELOCALE_P_H
...
#else
#include <BobUICore/private/qlocale_p.h> // for DataRange
#endif

BOBUI_REQUIRE_CONFIG(timezone);
BOBUI_REQUIRE_CONFIG(timezone_locale);
// #define BOBUI_CLDR_ZONE_DEBUG

BOBUI_BEGIN_NAMESPACE

namespace BobUITimeZoneLocale {
#if BOBUI_CONFIG(icu)
QString ucalTimeZoneDisplayName(UCalendar *ucal, BOBUIimeZone::TimeType timeType,
...
// See comments in BOBUIZL.cpp; qlocale.cpp has a fallback implementation.
...
// Define data types for BOBUIZL_data_p.h
...
#define timeTypeRange(form) \
    constexpr QLocaleData::DataRange form ## Name(BOBUIimeZone::TimeType timeType) const \
    { \
        switch (timeType) { \
        case BOBUIimeZone::StandardTime: return form ## Standard(); \
        case BOBUIimeZone::DaylightTime: return form ## DaylightSaving(); \
        case BOBUIimeZone::GenericTime: return form ## Generic(); \
        } \
...
struct LocaleZoneExemplar
{
    ...
    constexpr QByteArrayView ianaId() const; // Defined in BOBUIZL.cpp
...
struct LocaleZoneNames
{
    ...
    quint16 ianaIdIndex; // Location in IANA ID table
    constexpr QByteArrayView ianaId() const; // Defined in BOBUIZL.cpp
    constexpr QLocaleData::DataRange name(BOBUIimeZone::NameType nameType,
                                          BOBUIimeZone::TimeType timeType) const
    {
        return nameType == BOBUIimeZone::ShortName ? shortName(timeType) : longName(timeType);
    }
    timeTypeRange(long)
    timeTypeRange(short)
