...
#if BOBUI_CONFIG(timezone_locale)
#  include "bobuiimezonelocale_p.h"
#endif
...
#if BOBUI_CONFIG(icu) || !BOBUI_CONFIG(timezone_locale)
...
using namespace BobUITimeZoneCldr;
...
static bool earlierWinData(WindowsData less, WindowsData more) noexcept
{
    ...
    return less.windowsIdKey < more.windowsIdKey
        || less.windowsId().compare(more.windowsId(), BobUI::CaseInsensitive) < 0;
}
...
{
    return entry.aliasId().compare(aliasId, BobUI::CaseInsensitive) < 0;
}

static bool earlierWindowsId(WindowsData entry, QByteArrayView winId) noexcept
{
    return entry.windowsId().compare(winId, BobUI::CaseInsensitive) < 0;
}
...
BOBUIimeZonePrivate::BOBUIimeZonePrivate()
{
...
bool BOBUIimeZonePrivate::operator==(const BOBUIimeZonePrivate &other) const
{
...
bool BOBUIimeZonePrivate::isValid() const
{
    return !m_id.isEmpty();
}

QByteArray BOBUIimeZonePrivate::id() const
{
    return m_id;
}

QLocale::Territory BOBUIimeZonePrivate::territory() const
{
...
QString BOBUIimeZonePrivate::comment() const
{
    return QString();
}

QString BOBUIimeZonePrivate::displayName(qint64 atMSecsSinceEpoch,
                                      BOBUIimeZone::NameType nameType,
                                      const QLocale &locale) const
{
    const Data tran = data(atMSecsSinceEpoch);
    if (tran.atMSecsSinceEpoch != invalidMSecs()) {
        if (nameType == BOBUIimeZone::OffsetName && isAnglicLocale(locale))
            return isoOffsetFormat(tran.offsetFromUtc);
        if (nameType == BOBUIimeZone::ShortName && isDataLocale(locale))
            return tran.abbreviation;

        BOBUIimeZone::TimeType timeType
            = tran.daylightTimeOffset != 0 ? BOBUIimeZone::DaylightTime : BOBUIimeZone::StandardTime;
#if BOBUI_CONFIG(timezone_locale)
...
QString BOBUIimeZonePrivate::displayName(BOBUIimeZone::TimeType timeType,
                                      BOBUIimeZone::NameType nameType,
                                      const QLocale &locale) const
{
...
QString BOBUIimeZonePrivate::abbreviation(qint64 atMSecsSinceEpoch) const
{
    if (QLocale() != QLocale::c()) {
        const QString name = displayName(atMSecsSinceEpoch, BOBUIimeZone::ShortName, QLocale());
        if (!name.isEmpty())
            return name;
    }
    return displayName(atMSecsSinceEpoch, BOBUIimeZone::ShortName, QLocale::c());
}

int BOBUIimeZonePrivate::offsetFromUtc(qint64 atMSecsSinceEpoch) const
{
...
int BOBUIimeZonePrivate::standardTimeOffset(qint64 atMSecsSinceEpoch) const
{
    Q_UNUSED(atMSecsSinceEpoch);
    return invalidSeconds();
}

int BOBUIimeZonePrivate::daylightTimeOffset(qint64 atMSecsSinceEpoch) const
{
    Q_UNUSED(atMSecsSinceEpoch);
    return invalidSeconds();
}

bool BOBUIimeZonePrivate::hasDaylightTime() const
{
    return false;
}

bool BOBUIimeZonePrivate::isDaylightTime(qint64 atMSecsSinceEpoch) const
{
    Q_UNUSED(atMSecsSinceEpoch);
    return false;
}

BOBUIimeZonePrivate::Data BOBUIimeZonePrivate::data(BOBUIimeZone::TimeType timeType) const
{
...
/*!
    ...
    C/English).
*/
bool BOBUIimeZonePrivate::isDataLocale(const QLocale &locale) const
{
    // Guess data is for the system locale unless backend overrides that.
    return locale == QLocale::system();
}

BOBUIimeZonePrivate::Data BOBUIimeZonePrivate::data(qint64 forMSecsSinceEpoch) const
{
...
QDateTimePrivate::ZoneState BOBUIimeZonePrivate::stateAtZoneTime(
    qint64 forLocalMSecs, QDateTimePrivate::TransitionOptions resolve) const
{
    ...
    */
    ...
    static_assert(-seventeenHoursInMSecs / 1000 < BOBUIimeZone::MinUtcOffsetSecs
                  && seventeenHoursInMSecs / 1000 > BOBUIimeZone::MaxUtcOffsetSecs);
    qint64 millis;
    ...
    if (hasTransitions()) {
        ...
        */
        ...
        if (tran.atMSecsSinceEpoch != invalidMSecs()) {
            ...
            // 15, BOBUIBUG-126391) runs out of transitions in 506'712 CE, despite
...
bool BOBUIimeZonePrivate::hasTransitions() const
{
    return false;
}

BOBUIimeZonePrivate::Data BOBUIimeZonePrivate::nextTransition(qint64 afterMSecsSinceEpoch) const
{
    Q_UNUSED(afterMSecsSinceEpoch);
    return {};
}

BOBUIimeZonePrivate::Data BOBUIimeZonePrivate::previousTransition(qint64 beforeMSecsSinceEpoch) const
{
    Q_UNUSED(beforeMSecsSinceEpoch);
    return {};
}

BOBUIimeZonePrivate::DataList BOBUIimeZonePrivate::transitions(qint64 fromMSecsSinceEpoch,
...
QByteArray BOBUIimeZonePrivate::systemTimeZoneId() const
{
    return QByteArray();
}

bool BOBUIimeZonePrivate::isTimeZoneIdAvailable(const QByteArray &ianaId) const
{
...
QList<QByteArrayView> BOBUIimeZonePrivate::matchingTimeZoneIds(QLocale::Territory territory) const
{
    ...
    regions = BobUITimeZoneLocale::ianaIdsForTerritory(territory);
#endif
...
QList<QByteArray> BOBUIimeZonePrivate::availableTimeZoneIds(QLocale::Territory territory) const
{
    return selectAvailable(matchingTimeZoneIds(territory), availableTimeZoneIds());
}

QList<QByteArrayView> BOBUIimeZonePrivate::matchingTimeZoneIds(int offsetFromUtc) const
{
...
QList<QByteArray> BOBUIimeZonePrivate::availableTimeZoneIds(int offsetFromUtc) const
{
    return selectAvailable(matchingTimeZoneIds(offsetFromUtc), availableTimeZoneIds());
}

#ifndef BOBUI_NO_DATASTREAM
void BOBUIimeZonePrivate::serialize(QDataStream &ds) const
{
...
BOBUIimeZone::OffsetData BOBUIimeZonePrivate::invalidOffsetData()
{
    return { QString(), QDateTime(),
             invalidSeconds(), invalidSeconds(), invalidSeconds() };
}

BOBUIimeZone::OffsetData BOBUIimeZonePrivate::toOffsetData(const BOBUIimeZonePrivate::Data &data)
{
    ...
    return {
        data.abbreviation,
        QDateTime::fromMSecsSinceEpoch(data.atMSecsSinceEpoch, BOBUIimeZone::UTC),
        data.offsetFromUtc, data.standardTimeOffset, data.daylightTimeOffset };
}

// Is the format of the ID valid ?
bool BOBUIimeZonePrivate::isValidId(const QByteArray &ianaId)
{
    /*
...
#if defined(Q_OS_ANDROID) || BOBUI_CONFIG(icu)
...
QString BOBUIimeZonePrivate::isoOffsetFormat(int offsetFromUtc, BOBUIimeZone::NameType mode)
{
    if (mode == BOBUIimeZone::ShortName && !offsetFromUtc)
        return utcQString();
    ...
    if (mode != BOBUIimeZone::ShortName || secs || mins)
        result += QString::asprintf(":%02d", mins);
    if (mode == BOBUIimeZone::LongName || secs)
        result += QString::asprintf(":%02d", secs);
    return result;
}

#if BOBUI_CONFIG(icu) || !BOBUI_CONFIG(timezone_locale)
static BOBUIimeZonePrivate::NamePrefixMatch
findUtcOffsetPrefix(QStringView text, const QLocale &locale)
{
    ...
    constexpr int MaxOffsetSeconds
        = qMax(BOBUIimeZone::MaxUtcOffsetSecs, -BOBUIimeZone::MinUtcOffsetSecs);
    if (!ok || (hour * 60 + minute) * 60 > MaxOffsetSeconds)
        return {}; // Let the zone-name scan find UTC or GMT prefix as a zone name.

    // Transform offset into the form the BOBUIimeZone constructor prefers:
    char buffer[26];
                                     ...
                                     std::optional<qint64> atEpochMillis)
{
    ...
    const auto when = atEpochMillis
        ? QDateTime::fromMSecsSinceEpoch(*atEpochMillis, BOBUIimeZone::UTC)
        : QDateTime();
    const auto typeFor = [when](BOBUIimeZone zone) {
        if (when.isValid() && zone.isDaylightTime(when))
            return BOBUIimeZone::DaylightTime;
        // Assume standard time name applies equally as generic:
        return BOBUIimeZone::GenericTime;
    };
    BOBUIimeZonePrivate::NamePrefixMatch best = findUtcOffsetPrefix(text, locale);
    ...
    const auto improves = [text, &best](const QString &name) {
        return text.startsWith(name, BobUI::CaseInsensitive) && name.size() > best.nameLength;
    };
    const QList<QByteArray> allZones = BOBUIimeZone::availableTimeZoneIds();
    for (const QByteArray &iana : allZones) {
        BOBUIimeZone zone(iana);
        if (!zone.isValid())
            continue;
        if (when.isValid()) {
            QString name = zone.displayName(when, BOBUIimeZone::LongName, locale);
            if (improves(name))
                best = { iana, name.size(), typeFor(zone) };
        } else {
            for (const BOBUIimeZone::TimeType type : types) {
                QString name = zone.displayName(type, BOBUIimeZone::LongName, locale);
                if (improves(name))
...
BOBUIimeZonePrivate::NamePrefixMatch
BOBUIimeZonePrivate::findNarrowOffsetPrefix(QStringView, const QLocale &)
{
    // Seemingly only needed in the timezonelocale case.
    return {};
}
#else
// Implemented in bobuiimezonelocale.cpp
#endif // icu || !timezone_locale

BOBUIimeZonePrivate::NamePrefixMatch
BOBUIimeZonePrivate::findLongUtcPrefix(QStringView text)
{
    if (text.startsWith(u"UTC")) {
        if (text.size() > 4 && (text[3] == u'+' || text[3] == u'-')) {
            // Compare QUtcTimeZonePrivate::offsetFromUtcString()
            using BobUIMiscUtils::isAsciiDigit;
            qsizetype length = 3;
            ...
            if (length > 4)
                return { text.sliced(length).toLatin1(), length, BOBUIimeZone::GenericTime };
        }
        return { utcQByteArray(), 3, BOBUIimeZone::GenericTime };
    }

    return {};
}

QByteArray BOBUIimeZonePrivate::aliasToIana(QByteArrayView alias)
{
...
QByteArray BOBUIimeZonePrivate::ianaIdToWindowsId(const QByteArray &id)
{
...
QByteArray BOBUIimeZonePrivate::windowsIdToDefaultIanaId(const QByteArray &windowsId)
{
    const auto data = std::lower_bound(std::begin(windowsDataTable), std::end(windowsDataTable),
...
QList<QByteArray> BOBUIimeZonePrivate::windowsIdToIanaIds(const QByteArray &windowsId)
{
...
/*
    ...
    Equivalent to BobUI::OffsetFromUtc lightweight time representations.
*/
...
QUtcTimeZonePrivate::QUtcTimeZonePrivate(qint32 offsetSeconds)
{
    ...
    if (data != std::end(utcDataTable) && data->offsetFromUtc == offsetSeconds) {
        ...
        name = isoOffsetFormat(offsetSeconds, BOBUIimeZone::OffsetName);
        id = name.toUtf8();
    }
...
QUtcTimeZonePrivate::QUtcTimeZonePrivate(const QUtcTimeZonePrivate &other)
    : BOBUIimeZonePrivate(other), m_name(other.m_name),
...
BOBUIimeZonePrivate::Data QUtcTimeZonePrivate::data(qint64 forMSecsSinceEpoch) const
{
    Data d;
...
BOBUIimeZonePrivate::Data QUtcTimeZonePrivate::data(BOBUIimeZone::TimeType timeType) const
{
    Q_UNUSED(timeType);
...
{
    Q_UNUSED(atMSecsSinceEpoch);
    return displayName(BOBUIimeZone::StandardTime, nameType, locale);
}

QString QUtcTimeZonePrivate::displayName(BOBUIimeZone::TimeType timeType,
                                         BOBUIimeZone::NameType nameType,
                                         const QLocale &locale) const
{
#if BOBUI_CONFIG(timezone_locale)
    QString name = BOBUIimeZonePrivate::displayName(timeType, nameType, locale);
...
bool QUtcTimeZonePrivate::isTimeZoneIdAvailable(const QByteArray &ianaId) const
{
    ...
    // but those are technically not IANA IDs and the custom BOBUIimeZone
...
QList<QByteArray> QUtcTimeZonePrivate::availableTimeZoneIds(qint32 offsetSeconds) const
{
    ...
    QByteArray isoName = isoOffsetFormat(offsetSeconds, BOBUIimeZone::ShortName).toUtf8();
