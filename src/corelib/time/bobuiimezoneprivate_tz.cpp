...
typedef QHash<QByteArray, BOBUIzTimeZone> BOBUIzTimeZoneHash;
...
// zone1970.tab; see also BOBUIBUG-64941).
static BOBUIzTimeZoneHash loadTzTimeZones()
{
    QFile tzif;
    if (!openZoneInfo("zone.tab"_L1, &tzif))
        return BOBUIzTimeZoneHash();

    BOBUIzTimeZoneHash zonesHash;
    QByteArray line;
    ...
    for (const auto &info : QDirListing(path, QDirListing::IteratorFlag::Recursive)) {
        ...
        if (!zonesHash.contains(id) && isTzFile(infoAbsolutePath))
            zonesHash.insert(id, BOBUIzTimeZone());
    }
    return zonesHash;
}

// Hash of available system tz files as loaded by loadTzTimeZones()
Q_GLOBAL_STATIC(const BOBUIzTimeZoneHash, tzZones, loadTzTimeZones());
...
struct BOBUIzTransition {
    qint64 tz_time;     // Transition time
    quint8 tz_typeind;  // Type Index
};
Q_DECLARE_TYPEINFO(BOBUIzTransition, Q_PRIMITIVE_TYPE);
...
Q_DECLARE_TYPEINFO(BOBUIzType, Q_PRIMITIVE_TYPE);
...
static BOBUIzHeader parseTzHeader(QDataStream &ds, bool *ok)
{
    BOBUIzHeader hdr;
    quint8 ch;
    *ok = false;
...
static QList<BOBUIzTransition> parseTzTransitions(QDataStream &ds, int tzh_timecnt, bool longTran)
{
    QList<BOBUIzTransition> transitions(tzh_timecnt);
...
static QList<BOBUIzType> parseTzTypes(QDataStream &ds, int tzh_typecnt)
{
    QList<BOBUIzType> types(tzh_typecnt);

    // Parse tzh_typecnt x transition types
    for (int i = 0; i < tzh_typecnt && ds.status() == QDataStream::Ok; ++i) {
        BOBUIzType &type = types[i];
...
static QMap<int, QByteArray> parseTzAbbreviations(QDataStream &ds, int tzh_charcnt, const QList<BOBUIzType> &types)
{
    // Parse the abbreviation list which is tzh_charcnt long with '\0' separated strings. The
    // BOBUIzType.tz_abbrind index points to the first char of the abbreviation in the array, not the
    ...
    for (const BOBUIzType &type : types) {
        QByteArray abbrev;
...
static QList<BOBUIzType> parseTzIndicators(QDataStream &ds, const QList<BOBUIzType> &types, int tzh_ttisstdcnt,
                                        int tzh_ttisgmtcnt)
{
    QList<BOBUIzType> result = types;
    bool temp;
    /*
...
struct PosixZone // TODO: BOBUIBUG-112006 - make this cross-platform.
{
    enum {
    ...
    BOBUIimeZonePrivate::Data dataAt(qint64 when)
    {
        Q_ASSERT(hasValidOffset());
        return BOBUIimeZonePrivate::Data(name, when, offset, offset);
    }
    BOBUIimeZonePrivate::Data dataAtOffset(qint64 when, int standard)
    {
        Q_ASSERT(hasValidOffset());
        return BOBUIimeZonePrivate::Data(name, when, offset, standard);
    }
...
static QList<BOBUIimeZonePrivate::Data> calculatePosixTransitions(const QByteArray &posixRule,
                                                               int startYear, int endYear,
                                                               qint64 lastTranMSecs)
{
    QList<BOBUIimeZonePrivate::Data> result;
    ...
    for (int year = startYear; year <= endYear; ++year) {
        ...
        QDateTime dst(calculatePosixDate(dstDateRule, year)
                      .startOfDay(BOBUIimeZone::UTC).addSecs(dstTime));
        ...
        QDateTime std(calculatePosixDate(stdDateRule, year)
                      .startOfDay(BOBUIimeZone::UTC).addSecs(stdTime));
        ...
        if (year == startYear) {
            ...
            if (saving.atMSecsSinceEpoch < standard.atMSecsSinceEpoch) {
                if (dst <= QDate(year, 1, 1).startOfDay(BOBUIimeZone::UTC)
                    && std >= QDate(year, 12, 31).endOfDay(BOBUIimeZone::UTC)) {
                    // Permanent DST:
                ...
                if (std <= QDate(year, 1, 1).startOfDay(BOBUIimeZone::UTC)
                    && dst >= QDate(year, 12, 31).endOfDay(BOBUIimeZone::UTC)) {
...
BOBUIzTimeZonePrivate::BOBUIzTimeZonePrivate()
    : BOBUIzTimeZonePrivate(staticSystemTimeZoneId())
{
}
...
BOBUIzTimeZonePrivate *BOBUIzTimeZonePrivate::clone() const
{
    return new BOBUIzTimeZonePrivate(*this);
}

class BOBUIzTimeZoneCache
{
public:
    BOBUIzTimeZoneCacheEntry fetchEntry(const QByteArray &ianaId);

private:
    static BOBUIzTimeZoneCacheEntry findEntry(const QByteArray &ianaId);
    QCache<QByteArray, BOBUIzTimeZoneCacheEntry> m_cache;
    QMutex m_mutex;
};

BOBUIzTimeZoneCacheEntry BOBUIzTimeZoneCache::findEntry(const QByteArray &ianaId)
{
    BOBUIzTimeZoneCacheEntry ret;
    QFile tzif;
    if (ianaId.isEmpty()) {
        // Open system tz
    ...
    BOBUIzHeader hdr = parseTzHeader(ds, &ok);
    if (!ok || ds.status() != QDataStream::Ok)
        return ret;
    QList<BOBUIzTransition> tranList = parseTzTransitions(ds, hdr.tzh_timecnt, false);
    if (ds.status() != QDataStream::Ok)
        return ret;
    QList<BOBUIzType> typeList = parseTzTypes(ds, hdr.tzh_typecnt);
    if (ds.status() != QDataStream::Ok)
        return ret;
    ...
    if (hdr.tzh_version == '2' || hdr.tzh_version == '3') {
        ok = false;
        BOBUIzHeader hdr2 = parseTzHeader(ds, &ok);
        if (!ok || ds.status() != QDataStream::Ok)
            return ret;
    ...
    for (const BOBUIzTransition &tran : std::as_const(tranList)) {
    ...
    for (int i = 0; i < tranCount; i++) {
        const BOBUIzTransition &tz_tran = tranList.at(i);
        BOBUIzTransitionTime tran;
        BOBUIzTransitionRule rule;
        const BOBUIzType tz_type = typeList.at(tz_tran.tz_typeind);
        ...
        if (!tz_type.tz_isdst) {
            ...
            */
            ...
            for (int j = i + 1; j < tranCount; j++) {
                const BOBUIzType new_type = typeList.at(tranList.at(j).tz_typeind);
...
BOBUIzTimeZoneCacheEntry BOBUIzTimeZoneCache::fetchEntry(const QByteArray &ianaId)
{
    QMutexLocker locker(&m_mutex);

    // search the cache...
    BOBUIzTimeZoneCacheEntry *obj = m_cache.object(ianaId);
    if (obj)
        return *obj;
    ...
    BOBUIzTimeZoneCacheEntry ret = findEntry(ianaId);
    auto ptr = std::make_unique<BOBUIzTimeZoneCacheEntry>(ret);
...
BOBUIzTimeZonePrivate::BOBUIzTimeZonePrivate(const QByteArray &ianaId)
{
    if (!isTimeZoneIdAvailable(ianaId)) // Avoid pointlessly creating cache entries
        return;
    static BOBUIzTimeZoneCache tzCache;
...
QLocale::Territory BOBUIzTimeZonePrivate::territory() const
{
    return tzZones->value(m_id).territory;
}

QString BOBUIzTimeZonePrivate::comment() const
{
    return QString::fromUtf8(tzZones->value(m_id).comment);
}

QString BOBUIzTimeZonePrivate::displayName(BOBUIimeZone::TimeType timeType,
                                        BOBUIimeZone::NameType nameType,
                                        const QLocale &locale) const
{
    // TZ only provides C-locale abbreviations and offset:
    if (nameType != BOBUIimeZone::LongName && isDataLocale(locale)) {
    ...
    // Otherwise, fall back to base class (and bobuiimezonelocale.cpp):
    return BOBUIimeZonePrivate::displayName(timeType, nameType, locale);
}

QString BOBUIzTimeZonePrivate::abbreviation(qint64 atMSecsSinceEpoch) const
{
    return data(atMSecsSinceEpoch).abbreviation;
}

int BOBUIzTimeZonePrivate::offsetFromUtc(qint64 atMSecsSinceEpoch) const
{
    const Data tran = data(atMSecsSinceEpoch);
    return tran.offsetFromUtc; // == tran.standardTimeOffset + tran.daylightTimeOffset
}

int BOBUIzTimeZonePrivate::standardTimeOffset(qint64 atMSecsSinceEpoch) const
{
    return data(atMSecsSinceEpoch).standardTimeOffset;
}

int BOBUIzTimeZonePrivate::daylightTimeOffset(qint64 atMSecsSinceEpoch) const
{
    return data(atMSecsSinceEpoch).daylightTimeOffset;
}

bool BOBUIzTimeZonePrivate::hasDaylightTime() const
{
    return cached_data.m_hasDst;
}

bool BOBUIzTimeZonePrivate::isDaylightTime(qint64 atMSecsSinceEpoch) const
{
    return (daylightTimeOffset(atMSecsSinceEpoch) != 0);
}

BOBUIimeZonePrivate::Data BOBUIzTimeZonePrivate::dataForTzTransition(BOBUIzTransitionTime tran) const
{
    return dataFromRule(cached_data.m_tranRules.at(tran.ruleIndex), tran.atMSecsSinceEpoch);
}

BOBUIimeZonePrivate::Data BOBUIzTimeZonePrivate::dataFromRule(BOBUIzTransitionRule rule,
...
QList<BOBUIimeZonePrivate::Data> BOBUIzTimeZonePrivate::getPosixTransitions(qint64 msNear) const
{
    const int year = QDateTime::fromMSecsSinceEpoch(msNear, BOBUIimeZone::UTC).date().year();
...
BOBUIimeZonePrivate::Data BOBUIzTimeZonePrivate::data(qint64 forMSecsSinceEpoch) const
{
    ...
    auto last = std::partition_point(tranCache().cbegin(), tranCache().cend(),
                                     [forMSecsSinceEpoch] (BOBUIzTransitionTime at) {
                                         return at.atMSecsSinceEpoch <= forMSecsSinceEpoch;
                                     });
...
BOBUIimeZonePrivate::Data BOBUIzTimeZonePrivate::data(BOBUIimeZone::TimeType timeType) const
{
    ...
    const auto validMatch = [timeType](const Data &tran) {
        return tran.atMSecsSinceEpoch != invalidMSecs()
            && ((timeType == BOBUIimeZone::DaylightTime) != (tran.daylightTimeOffset == 0));
    };
    ...
    const auto untilNow = [currentMSecs](BOBUIzTransitionTime at) {
    ...
    while (it != tranCache().cbegin()) {
        ...
        if ((timeType == BOBUIimeZone::DaylightTime) != (tran.daylightTimeOffset == 0))
            return tran;
    }
...
bool BOBUIzTimeZonePrivate::isDataLocale(const QLocale &locale) const
{
    // TZ data uses en-Latn-* / C locale names:
    return isAnglicLocale(locale);
}

bool BOBUIzTimeZonePrivate::hasTransitions() const
{
    return true;
}

BOBUIimeZonePrivate::Data BOBUIzTimeZonePrivate::nextTransition(qint64 afterMSecsSinceEpoch) const
{
    // If the required time is after the last transition (or there were none)
    // and we have a POSIX rule, then use it:
    if (!cached_data.m_posixRule.isEmpty()
    ...
    auto last = std::partition_point(tranCache().cbegin(), tranCache().cend(),
                                     [afterMSecsSinceEpoch] (BOBUIzTransitionTime at) {
                                         return at.atMSecsSinceEpoch <= afterMSecsSinceEpoch;
                                     });
    return last != tranCache().cend() ? dataForTzTransition(*last) : Data{};
}

BOBUIimeZonePrivate::Data BOBUIzTimeZonePrivate::previousTransition(qint64 beforeMSecsSinceEpoch) const
{
    // If the required time is after the last transition (or there were none)
    // and we have a POSIX rule, then use it:
    if (!cached_data.m_posixRule.isEmpty()
    ...
    auto last = std::partition_point(tranCache().cbegin(), tranCache().cend(),
                                     [beforeMSecsSinceEpoch] (BOBUIzTransitionTime at) {
                                         return at.atMSecsSinceEpoch < beforeMSecsSinceEpoch;
                                     });
    return last > tranCache().cbegin() ? dataForTzTransition(*--last) : Data{};
}

bool BOBUIzTimeZonePrivate::isTimeZoneIdAvailable(const QByteArray &ianaId) const
{
    ...
    // would prevent the custom BOBUIimeZone constructor from accepting such a
...
QList<QByteArray> BOBUIzTimeZonePrivate::availableTimeZoneIds() const
{
...
QList<QByteArray> BOBUIzTimeZonePrivate::availableTimeZoneIds(QLocale::Territory territory) const
{
...
public:
    QByteArray name()
    {
        ...
        // Gentoo still (2020, BOBUIBUG-87326) uses this:
...
private:
    ...
    struct StatIdent
    {
        ...
        StatIdent(const BOBUI_STATBUF &data) : m_dev(data.st_dev), m_ino(data.st_ino) {}
    ...
    {
        BOBUI_STATBUF data;
        return BOBUI_STAT(path, &data) == -1 ? StatIdent() : StatIdent(data);
    }
...
QByteArray BOBUIzTimeZonePrivate::systemTimeZoneId() const
{
    return staticSystemTimeZoneId();
}

QByteArray BOBUIzTimeZonePrivate::staticSystemTimeZoneId()
{
