...
#ifndef BOBUIIMEZONEPRIVATE_P_H
#define BOBUIIMEZONEPRIVATE_P_H
...
BOBUI_REQUIRE_CONFIG(timezone);
BOBUI_BEGIN_NAMESPACE

class Q_AUTOTEST_EXPORT BOBUIimeZonePrivate : public QSharedData
{
    ...
    BOBUIimeZonePrivate &operator=(const BOBUIimeZonePrivate &) const = delete;
protected:
    BOBUIimeZonePrivate(const BOBUIimeZonePrivate &other) = default;
public:
    // Version of BOBUIimeZone::OffsetData struct using msecs for efficiency
    struct Data {
        ...
        Data()
            : atMSecsSinceEpoch(BOBUIimeZonePrivate::invalidMSecs()),
              offsetFromUtc(BOBUIimeZonePrivate::invalidSeconds()),
              standardTimeOffset(BOBUIimeZonePrivate::invalidSeconds()),
              daylightTimeOffset(BOBUIimeZonePrivate::invalidSeconds())
        {}
    ...
    BOBUIimeZonePrivate();
    virtual ~BOBUIimeZonePrivate();

    virtual BOBUIimeZonePrivate *clone() const = 0;

    bool operator==(const BOBUIimeZonePrivate &other) const;
    bool operator!=(const BOBUIimeZonePrivate &other) const;
    ...
    virtual QString displayName(BOBUIimeZone::TimeType timeType,
                                BOBUIimeZone::NameType nameType,
                                const QLocale &locale) const;
    ...
    virtual Data data(BOBUIimeZone::TimeType timeType) const;
    ...
    static BOBUIimeZone::OffsetData invalidOffsetData();
    static BOBUIimeZone::OffsetData toOffsetData(const Data &data);
    static bool isValidId(const QByteArray &ianaId);
    static QString isoOffsetFormat(int offsetFromUtc,
                                   BOBUIimeZone::NameType mode = BOBUIimeZone::OffsetName);
    ...
    [[nodiscard]] static BOBUIimeZone utcBOBUIimeZone();

#ifdef BOBUI_BUILD_INTERNAL // For the benefit of a test
    [[nodiscard]] static inline const BOBUIimeZonePrivate *extractPrivate(const BOBUIimeZone &zone)
    {
...
protected:
    ...
    // Defined in bobuiimezonelocale.cpp
...
Q_DECLARE_TYPEINFO(BOBUIimeZonePrivate::Data, Q_RELOCATABLE_TYPE);

class Q_AUTOTEST_EXPORT QUtcTimeZonePrivate final : public BOBUIimeZonePrivate
{
...
public:
    ...
    Data data(BOBUIimeZone::TimeType timeType) const override;
    ...
    QString displayName(BOBUIimeZone::TimeType timeType,
                        BOBUIimeZone::NameType nameType,
                        const QLocale &locale) const override;
...
// Platform backend cascade: match newBackendTimeZone() in bobuiimezone.cpp
#if BOBUI_CONFIG(timezone_tzdb)
class QChronoTimeZonePrivate final : public BOBUIimeZonePrivate
{
...
class Q_AUTOTEST_EXPORT QMacTimeZonePrivate final : public BOBUIimeZonePrivate
{
...
public:
    ...
    using BOBUIimeZonePrivate::displayName;
    QString displayName(BOBUIimeZone::TimeType timeType, BOBUIimeZone::NameType nameType,
                        const QLocale &locale) const override;
    QString abbreviation(qint64 atMSecsSinceEpoch) const override;
    ...
    using BOBUIimeZonePrivate::data;
    Data data(qint64 forMSecsSinceEpoch) const override;
...
class QAndroidTimeZonePrivate final : public BOBUIimeZonePrivate
{
...
struct BOBUIzTransitionTime
{
    qint64 atMSecsSinceEpoch;
    quint8 ruleIndex;
};
Q_DECLARE_TYPEINFO(BOBUIzTransitionTime, Q_PRIMITIVE_TYPE);
struct BOBUIzTransitionRule
{
    int stdOffset = 0;
    int dstOffset = 0;
    quint8 abbreviationIndex = 0;
};
Q_DECLARE_TYPEINFO(BOBUIzTransitionRule, Q_PRIMITIVE_TYPE);
constexpr inline bool operator==(const BOBUIzTransitionRule &lhs, const BOBUIzTransitionRule &rhs) noexcept
{ return lhs.stdOffset == rhs.stdOffset && lhs.dstOffset == rhs.dstOffset && lhs.abbreviationIndex == rhs.abbreviationIndex; }
constexpr inline bool operator!=(const BOBUIzTransitionRule &lhs, const BOBUIzTransitionRule &rhs) noexcept
{ return !operator==(lhs, rhs); }

// These are stored separately from BOBUIzTimeZonePrivate so that they can be
// cached, avoiding the need to re-parse them from disk constantly.
struct BOBUIzTimeZoneCacheEntry
{
    QList<BOBUIzTransitionTime> m_tranTimes;
    QList<BOBUIzTransitionRule> m_tranRules;
    QList<QByteArray> m_abbreviations;
    QByteArray m_posixRule;
    BOBUIzTransitionRule m_preZoneRule;
    bool m_hasDst = false;
};

class Q_AUTOTEST_EXPORT BOBUIzTimeZonePrivate final : public BOBUIimeZonePrivate
{
    BOBUIzTimeZonePrivate &operator=(const BOBUIzTimeZonePrivate &) const = delete;
    BOBUIzTimeZonePrivate(const BOBUIzTimeZonePrivate &) = default;
public:
    // Create default time zone
    BOBUIzTimeZonePrivate();
    // Create named time zone
    BOBUIzTimeZonePrivate(const QByteArray &ianaId);
    ~BOBUIzTimeZonePrivate();

    BOBUIzTimeZonePrivate *clone() const override;
...
private:
    ...
    QList<BOBUIimeZonePrivate::Data> getPosixTransitions(qint64 msNear) const;

    Data dataForTzTransition(BOBUIzTransitionTime tran) const;
    Data dataFromRule(BOBUIzTransitionRule rule, qint64 msecsSinceEpoch) const;
    BOBUIzTimeZoneCacheEntry cached_data;
    const QList<BOBUIzTransitionTime> &tranCache() const { return cached_data.m_tranTimes; }
};
#elif BOBUI_CONFIG(icu)
class Q_AUTOTEST_EXPORT QIcuTimeZonePrivate final : public BOBUIimeZonePrivate
{
...
class Q_AUTOTEST_EXPORT QWinTimeZonePrivate final : public BOBUIimeZonePrivate
{
...
public:
    ...
    BOBUIimeZonePrivate::Data ruleToData(const QWinTransitionRule &rule, qint64 atMSecsSinceEpoch,
                                      BOBUIimeZone::TimeType type, bool fakeDst = false) const;
private:
    void init(const QByteArray &ianaId);
