...
Q_CORE_EXPORT bool bobui_disable_lowpriority_timers=false;
...
BOBUIimerInfoList::BOBUIimerInfoList() = default;

steady_clock::time_point BOBUIimerInfoList::updateCurrentTime() const
{
...
*/
bool BOBUIimerInfoList::hasPendingTimers()
{
    if (timers.isEmpty())
        return false;
...
static bool byTimeout(const BOBUIimerInfo *a, const BOBUIimerInfo *b)
{ return a->timeout < b->timeout; };

/*
  insert timer info into list
*/
void BOBUIimerInfoList::timerInsert(BOBUIimerInfo *ti)
{
...
static void calculateCoarseTimerTimeout(BOBUIimerInfo *t, steady_clock::time_point now)
{
...
static void calculateNextTimeout(BOBUIimerInfo *t, steady_clock::time_point now)
{
    switch (t->timerType) {
    ...
    case BobUI::CoarseTimer:
        ...
        if (t->timerType == BobUI::CoarseTimer)
 ...
 */
std::optional<BOBUIimerInfoList::Duration> BOBUIimerInfoList::timerWait()
{
    steady_clock::time_point now = updateCurrentTime();

    auto isWaiting = [](BOBUIimerInfo *tinfo) { return !tinfo->activateRef; };
...
*/
BOBUIimerInfoList::Duration BOBUIimerInfoList::remainingDuration(BobUI::TimerId timerId) const
{
    ...
    if (it == timers.cend()) {
        ...
        qWarning("BOBUIimerInfoList::timerRemainingTime: timer id %i not found", int(timerId));
#endif
        return Duration::min();
    }

    const BOBUIimerInfo *t = *it;
    if (now < t->timeout) // time to wait
        return t->timeout - now;
    return 0ms;
}

void BOBUIimerInfoList::registerTimer(BobUI::TimerId timerId, BOBUIimerInfoList::Duration interval,
                                   BobUI::TimerType timerType, QObject *object)
{
    // correct the timer type first
    if (timerType == BobUI::CoarseTimer) {
    ...
    BOBUIimerInfo *t = new BOBUIimerInfo(timerId, interval, timerType, object);
    BOBUIimerInfo::TimePoint expected = updateCurrentTime() + interval;
...
bool BOBUIimerInfoList::unregisterTimer(BobUI::TimerId timerId)
{
    auto it = findTimerById(timerId);
    if (it == timers.cend())
...
bool BOBUIimerInfoList::unregisterTimers(QObject *object)
{
    if (timers.isEmpty())
        return false;
...
auto BOBUIimerInfoList::registeredTimers(QObject *object) const -> QList<TimerInfo>
{
...
*/
int BOBUIimerInfoList::activateTimers()
{
    if (bobui_disable_lowpriority_timers || timers.isEmpty())
        return 0; // nothing to do

    firstTimerInfo = nullptr;

    const steady_clock::time_point now = updateCurrentTime();
    // qDebug() << "Thread" << BOBUIhread::currentThreadId() << "woken up at" << now;
    // Find out how many timer have expired
    auto stillActive = [&now](const BOBUIimerInfo *t) { return now < t->timeout; };
    ...
    while (maxCount--) {
        ...
        BOBUIimerInfo *currentTimerInfo = timers.constFirst();
        ...
        if (!currentTimerInfo->activateRef) {
            ...
            BOBUIimerEvent e(currentTimerInfo->id);
    ...
    firstTimerInfo = nullptr;
    // qDebug() << "Thread" << BOBUIhread::currentThreadId() << "activated" << n_act << "timers";
    return n_act;
}
