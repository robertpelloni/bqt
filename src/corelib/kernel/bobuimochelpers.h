...
#ifndef BOBUIMOCHELPERS_H
#define BOBUIMOCHELPERS_H

//
//  W A R N I N G
//  -------------
//
// This file is not part of the BobUI API. It exists to be used by the code that
...
#include <BobUICore/q20algorithm.h>    // std::min, std::copy_n
#include <BobUICore/q23type_traits.h>  // std::is_scoped_enum
#include <limits>
...
template <uint UCount, uint SCount, size_t SSize, uint MCount> struct MetaObjectContents
{
    ...
    struct RelocatingContent {
        const BobUIPrivate::QMetaTypeInterface *metaTypes[MCount];
    } relocatingData = {};
};
...
public:
    ...
    constexpr EnumData(uint nameOffset, uint aliasOffset, uint flags)
    {
        ...
        if (nameOffset != aliasOffset || BobUIPrivate::IsQFlags<Enum>::value)
            this->header[2] |= BobUIMocConstants::EnumIsFlag;
        if constexpr (q23::is_scoped_enum_v<Enum>)
            this->header[2] |= BobUIMocConstants::EnumIsScoped;
    }
    ...
    template <int Added> constexpr auto add(const EnumEntry (&entries)[Added]) const
    {
        ...
        if constexpr (sizeof(Enum) > sizeof(uint)) {
            ...
            result.header[2] |= BobUIMocConstants::EnumIs64Bit;
            for (auto entry : entries) {
                auto value = qToUnderlying(entry.value);
...
struct FunctionData<Ret (Args...), ExtraFlags>
    : detail::UintDataBlock<6, 2 * sizeof...(Args) + 1 + (ExtraFlags & BobUIMocConstants::MethodRevisioned ? 1 : 0)>
{
    static constexpr bool IsRevisioned = (ExtraFlags & BobUIMocConstants::MethodRevisioned) != 0;
                 ...
                 uint returnType, ParametersArray params = {})
    {
        ...
        uint *p = this->payload;
        if constexpr (ExtraFlags & BobUIMocConstants::MethodRevisioned)
            ++p;
        *p++ = returnType;
...
struct FunctionData<Ret (Args...) const, ExtraFlags>
    : FunctionData<Ret (Args...), ExtraFlags | BobUIMocConstants::MethodIsConst>
{
    using FunctionData<Ret (Args...), ExtraFlags | BobUIMocConstants::MethodIsConst>::FunctionData;
};

template <typename F> struct MethodData : FunctionData<F, BobUIMocConstants::MethodMethod>
{
    using FunctionData<F, BobUIMocConstants::MethodMethod>::FunctionData;
};

template <typename F> struct SignalData : FunctionData<F, BobUIMocConstants::MethodSignal>
{
    using FunctionData<F, BobUIMocConstants::MethodSignal>::FunctionData;
};

template <typename F> struct SlotData : FunctionData<F, BobUIMocConstants::MethodSlot>
{
    using FunctionData<F, BobUIMocConstants::MethodSlot>::FunctionData;
};

template <typename F> struct ConstructorData : FunctionData<F, BobUIMocConstants::MethodConstructor>
{
    using Base = FunctionData<F, BobUIMocConstants::MethodConstructor>;
...
template <typename F> struct RevisionedMethodData :
        FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodMethod>
{
    using FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodMethod>::FunctionData;
};

template <typename F> struct RevisionedSignalData :
        FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodSignal>
{
    using FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodSignal>::FunctionData;
};

template <typename F> struct RevisionedSlotData :
        FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodSlot>
{
    using FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodSlot>::FunctionData;
};

template <typename F> struct RevisionedConstructorData :
        FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodConstructor>
{
    using Base = FunctionData<F, BobUIMocConstants::MethodRevisioned | BobUIMocConstants::MethodConstructor>;
...
constexpr auto metaObjectData(uint flags, const Strings &strings,
                              ...
                              const Enums &enums, int bobui_metaObjectHashIndex = -1,
                              ...
                              const ClassInfo &classInfo = {})
{
    ...
    data[0] = BobUIMocConstants::OutputRevision;
    ...
    if constexpr (Methods::count()) {
        ...
        for ( ; ptr < end; ptr += MethodHeaderSize) {
            if ((ptr[4] & BobUIMocConstants::MethodSignal) == 0)
                break;
            ++data[13];
        }
    }

    data[14] = bobui_metaObjectHashIndex;
...
template <typename T> inline std::enable_if_t<BobUIPrivate::IsQFlags<T>::value> assignFlags(void *v, T t) noexcept
{
    *static_cast<T *>(v) = t;
}
...
inline std::enable_if_t<BobUIPrivate::IsQFlags<T>::value && sizeof(T) == sizeof(int) && std::is_integral_v<I>>
