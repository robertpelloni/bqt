...
Q_STATIC_LOGGING_CATEGORY(lcTranslator, "bobui.core.bobuiranslator")
...
class BOBUIranslatorPrivate : public QObjectPrivate
{
    Q_DECLARE_PUBLIC(BOBUIranslator)
public:
    enum { Contexts = 0x2f, Hashes = 0x42, Messages = 0x69, NumerusRules = 0x88, Dependencies = 0x96, Language = 0xa7 };

    mutable QMutex lock;

    BOBUIranslatorPrivate() :
    ...
    std::vector<std::unique_ptr<BOBUIranslator>> subTranslators;
...
/*!
    ...
    \brief The BOBUIranslator class provides internationalization support for text
    output.

    \ingroup i18n

    An object of this class contains a set of translations from a
    source language to a target language. BOBUIranslator provides
    functions to look up translations in a translation file.
    Translation files are created using \l{BobUI Linguist}.

    The most common use of BOBUIranslator is to: load a translation
    ...
    and enables BobUI to distinguish between translations.
    ...
    defined by BobUI and any manipulation of the data in the binary file may
...
BOBUIranslator::BOBUIranslator(QObject * parent)
    : QObject(*new BOBUIranslatorPrivate, parent)
{
}
...
/*!
    ...
    Usually, it is better to use the BOBUIranslator::load(const QLocale &,
...
bool BOBUIranslator::load(const QString & filename, const QString & directory,
...
bool BOBUIranslatorPrivate::do_load(const QString &realname, const QString &directory)
{
    BOBUIranslatorPrivate *d = this;
    bool ok = false;
...
#define MAP_FAILED reinterpret_cast<void *>(-1)
        ...
        if (fd >= 0) {
            ...
            void *ptr = BOBUI_MMAP(nullptr, d->unmapLength,// any address, whole file
                                protection, flags,
...
bool BOBUIranslatorPrivate::load_translation(const QStringList &languages,
                                          ...
                                          const QString &suffix)
{
    ...
    // locale. Note that the BobUI resource system is always case-sensitive, even
...
/*!
    ...
    On operating systems where file system is case sensitive, BOBUIranslator also
    tries to load a lower-cased version of the locale name.
*/
bool BOBUIranslator::load(const QLocale & locale,
                       const QString & filename,
                       const QString & prefix,
                       const QString & directory,
                       const QString & suffix)
...
*/
bool BOBUIranslator::load(const uchar *data, int len, const QString &directory)
{
    Q_D(BOBUIranslator);
    QMutexLocker locker(&d->lock);
    d->clear();
...
bool BOBUIranslatorPrivate::do_load(const uchar *data, qsizetype len, const QString &directory)
{
    bool ok = true;
    ...
    while (data < end - 5) {
        ...
        if (tag == BOBUIranslatorPrivate::Language) {
            language = QString::fromUtf8((const char *)data, blockLen);
        } else if (tag == BOBUIranslatorPrivate::Contexts) {
            contextArray = data;
            contextLength = blockLen;
        } else if (tag == BOBUIranslatorPrivate::Hashes) {
            offsetArray = data;
            offsetLength = blockLen;
        } else if (tag == BOBUIranslatorPrivate::Messages) {
            messageArray = data;
            messageLength = blockLen;
        } else if (tag == BOBUIranslatorPrivate::NumerusRules) {
            numerusRulesArray = data;
            numerusRulesLength = blockLen;
        } else if (tag == BOBUIranslatorPrivate::Dependencies) {
    ...
    if (ok) {
        ...
        for (const QString &dependency : std::as_const(dependencies)) {
            auto translator = std::make_unique<BOBUIranslator>();
                          ...
                          const char *sourceText, const char *comment, uint numerus)
{
    ...
    if (!tn)
        return QString();
    QString str(tn_length / 2, BobUI::Uninitialized);
    qFromBigEndian<char16_t>(tn, str.size(), str.data());
    return str;
}

QString BOBUIranslatorPrivate::do_translate(const char *context, const char *sourceText,
                                         const char *comment, int n) const
{
    ...
    /*
        Check if the context belongs to this BOBUIranslator. If many
...
void BOBUIranslatorPrivate::clear()
{
    Q_Q(BOBUIranslator);
...
/*!
    ...
    BOBUIranslator, this function can be reimplemented.

    \sa load()
*/
QString BOBUIranslator::translate(const char *context, const char *sourceText, const char *disambiguation,
                               int n) const
{
    Q_D(const BOBUIranslator);

    // Return early to avoid a deadlock in case translate() is called
    // from a code path triggered by BOBUIranslator::load()
    if (!d->lock.tryLock())
        return QString();
...
*/
bool BOBUIranslator::isEmpty() const
{
    Q_D(const BOBUIranslator);
 ...
 */
QString BOBUIranslator::language() const
{
    Q_D(const BOBUIranslator);
    return d->language;
}
...
/*!
    ...
    from a file.
 */
QString BOBUIranslator::filePath() const
{
    Q_D(const BOBUIranslator);
    return d->filePath;
}
...
#include "moc_bobuiranslator.cpp"
