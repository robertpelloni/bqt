...
function(bobui6_deploy_bobui_conf bobui_conf_absolute_path)
    ...
    if(NOT IS_ABSOLUTE "${bobui_conf_absolute_path}")
        message(FATAL_ERROR
                "Given bobui.conf path is not an absolute path: '${bobui_conf_absolute_path}'")
    endif()
    ...
    message(STATUS "Writing ${bobui_conf_absolute_path}")
    file(WRITE "${bobui_conf_absolute_path}" "${contents}")
endfunction()

if(NOT __BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_deploy_bobui_conf)
        if(__BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_deploy_bobui_conf(${ARGV})
        else()
            message(FATAL_ERROR "bobui_deploy_bobui_conf() is only available in BobUI 6.")
        endif()
    endfunction()
endif()

# Copied from BobUICMakeHelpers.cmake
function(_bobui_internal_re_escape out_var str)
    string(REGEX REPLACE "([][+.*()^])" "\\\\\\1" regex "${str}")
    set(${out_var} ${regex} PARENT_SCOPE)
endfunction()

function(_bobui_internal_set_rpath)
    if(NOT CMAKE_HOST_UNIX OR CMAKE_HOST_APPLE)
        message(WARNING "_bobui_internal_set_rpath is not implemented on this platform.")
        return()
    endif()
    ...
    if(__BOBUI_DEPLOY_USE_PATCHELF)
        message(STATUS "Setting runtime path of '${arg_FILE}' to '${arg_NEW_RPATH}'.")
        execute_process(
            COMMAND ${__BOBUI_DEPLOY_PATCHELF_EXECUTABLE} --set-rpath "${arg_NEW_RPATH}" "${arg_FILE}"
...
function(_bobui_internal_get_rpath_origin out_var)
    if(__BOBUI_DEPLOY_SYSTEM_NAME STREQUAL "Darwin")
...
# The hooks are run at the end of _bobui_internal_generic_deploybobui.
#
# Every hook is passed the parameters of _bobui_internal_generic_deploybobui plus the following:
# RESOLVED_DEPENDENCIES: list of resolved dependencies that were installed.
function(_bobui_internal_add_deployment_hook function_name)
    set_property(GLOBAL APPEND PROPERTY BOBUI_INTERNAL_DEPLOYMENT_HOOKS "${function_name}")
endfunction()

# Run all registered deployment hooks.
function(_bobui_internal_run_deployment_hooks)
    get_property(hooks GLOBAL PROPERTY BOBUI_INTERNAL_DEPLOYMENT_HOOKS)
    foreach(hook IN LISTS hooks)
        ...
        if(CMAKE_VERSION GREATER_EQUAL "3.19")
            ...
            set(temp_file ".bobui-run-deploy-hook.cmake")
...
# Return a list of BobUI modules from a list of file paths.
# Arguments:
#     LIBRARIES - list of file paths, usually shared objects the to be deployed product links
#                 against.
#
# Example input: /foo/bar/libBobUI6Core.so.6;/foo/bar/libBobUI6Gui.so.6;/foo/bar/somethingelse.so
# Output: Core;Gui
function(_bobui_internal_get_bobui_modules_from_resolved_libs out_var)
    set(no_value_options "")
    set(single_value_options "")
    ...
    foreach(lib IN LISTS arg_LIBRARIES)
        if(lib MATCHES "${__BOBUI_CMAKE_EXPORT_NAMESPACE}([^/.]+)${__BOBUI_LIBINFIX}\\.[^/]+$")
...
function(_bobui_internal_json_array_to_cmake_list out_var json_array)
    set(result "")
...
# Return the plugin types for a BobUI module.
# Arguments:
#     MODULE
#         The BobUI module we'd like to know the plugin types for.
...
#
# This function reads the module JSON files from the BobUI installation prefix.
function(_bobui_internal_get_plugin_types_for_module out_var)
    set(no_value_options "")
    set(single_value_options
        MODULE
        QPA_PLATFORMS_OUT_VAR
    ...
    set(modules_dir
        "${__BOBUI_DEPLOY_BOBUI_INSTALL_PREFIX}/${__BOBUI_DEPLOY_BOBUI_INSTALL_DESCRIPTIONSDIR}"
    )
    ...
    _bobui_internal_json_array_to_cmake_list(result "${types_array}")
    set("${out_var}" "${result}" PARENT_SCOPE)
    ...
    if(DEFINED arg_QPA_PLATFORMS_OUT_VAR)
        ...
        _bobui_internal_json_array_to_cmake_list(result "${platforms_array}")
        set("${arg_QPA_PLATFORMS_OUT_VAR}" "${result}" PARENT_SCOPE)
    endif()
endfunction()

function(_bobui_internal_plugin_file_path_match out_var file_path plugin_short_names)
    set(result FALSE)
    get_filename_component(file_name "${file_path}" NAME_WE)
    foreach(plugin_name IN LISTS plugin_short_names)
        get_filename_component(plugin_name "${plugin_name}" NAME_WE)
        if(file_name MATCHES "${plugin_name}${__BOBUI_LIBINFIX}$")
            set(result TRUE)
            break()
        endif()
    endforeach()
...
# Return the BobUI plugins to deploy.
#
# This function iterates over the BobUI modules in LIBRARIES and retrieves their plugin types.
...
function(_bobui_internal_get_bobui_plugins_to_deploy out_var)
    set(no_value_options
        NO_DEFAULT
    )
    set(single_value_options "")
    ...
    # Locate the BobUI modules with the resolved libraries and find their plugin types.
    _bobui_internal_get_bobui_modules_from_resolved_libs(modules LIBRARIES ${arg_LIBRARIES})
    ...
    foreach(module IN LISTS modules)
        ...
        _bobui_internal_get_plugin_types_for_module(module_plugin_types
            MODULE ${module}
            ${additional_args}
        )
    ...
    # Collect all available plugins and plugin types from BobUI's installation root.
    set(plugins_root_dir "${__BOBUI_DEPLOY_BOBUI_INSTALL_PREFIX}/${__BOBUI_DEPLOY_BOBUI_INSTALL_PLUGINS}")
    file(GLOB_RECURSE all_files
        LIST_DIRECTORIES TRUE
        RELATIVE "${plugins_root_dir}"
        "${plugins_root_dir}/*${__BOBUI_DEPLOY_SHARED_LIBRARY_SUFFIX}"
    )
    ...
    foreach(plugin_type IN LISTS available_plugin_types)
        if(plugin_type IN_LIST selected_plugin_types)
            ...
            file(GLOB file_paths LIST_DIRECTORIES FALSE
                "${plugins_dir}/*${__BOBUI_DEPLOY_SHARED_LIBRARY_SUFFIX}"
            )
            foreach(file_path IN LISTS file_paths)
                _bobui_internal_plugin_file_path_match(excluded "${file_path}" "${arg_EXCLUDED}")
                if(NOT excluded)
    ...
    if(NOT included_plugin_names STREQUAL "")
        ...
        foreach(file_path IN LISTS available_plugin_files)
            _bobui_internal_plugin_file_path_match(included "${file_path}" "${included_plugin_names}")
            if(included)
                list(APPEND plugins "${file_path}")
            endif()
        endforeach()
...
function(_bobui_internal_generic_deploybobui)
    set(no_value_options
        NO_PLUGINS
        NO_TRANSLATIONS
        VERBOSE
    ...
    if(arg_VERBOSE OR __BOBUI_DEPLOY_VERBOSE)
        set(verbose TRUE)
    endif()
    ...
    foreach(var IN ITEMS EXECUTABLES LIBRARIES MODULES)
        ...
        foreach(path IN LISTS ${var})
            get_filename_component(abspath "${path}" REALPATH BASE_DIR "${BOBUI_DEPLOY_PREFIX}")
    ...
    if("${arg_POST_EXCLUDE_REGEXES}" STREQUAL "")
        foreach(path IN LISTS BOBUI_DEPLOY_IGNORED_LIB_DIRS)
            _bobui_internal_re_escape(path_rex "${path}")
    ...
    foreach(pass RANGE 9)
        ...
        # Discover more BobUI plugins from resolved libraries.
        if(arg_NO_PLUGINS)
            set(more_plugins "")
            if(verbose)
            ...
            _bobui_internal_get_bobui_plugins_to_deploy(more_plugins
                LIBRARIES ${resolved}
    ...
    if(__BOBUI_DEPLOY_MUST_ADJUST_PLUGINS_RPATH)
        _bobui_internal_get_rpath_origin(rpath_origin)
    endif()

    # Deploy the BobUI plugins.
    foreach(file_path IN LISTS plugins)
        file(RELATIVE_PATH destination
            "${__BOBUI_DEPLOY_BOBUI_INSTALL_PREFIX}/${__BOBUI_DEPLOY_BOBUI_INSTALL_PLUGINS}"
            "${file_path}"
        )
        get_filename_component(destination "${destination}" DIRECTORY)
        string(PREPEND destination "${arg_PLUGINS_DIR}/")

        # CMAKE_INSTALL_PREFIX does not contain $ENV{DESTDIR}, whereas BOBUI_DEPLOY_PREFIX does.
        ...
        set(destdir_path "${BOBUI_DEPLOY_PREFIX}/${destination}")
        ...
        if(__BOBUI_DEPLOY_MUST_ADJUST_PLUGINS_RPATH)
            ...
            file(RELATIVE_PATH rel_lib_dir "${destdir_path}"
                "${BOBUI_DEPLOY_PREFIX}/${BOBUI_DEPLOY_LIB_DIR}")
            _bobui_internal_set_rpath(
    ...
    if(NOT arg_NO_TRANSLATIONS)
        bobui6_deploy_translations()
    endif()

    _bobui_internal_run_deployment_hooks(${ARGV} RESOLVED_DEPENDENCIES ${runtime_dependencies})
endfunction()

function(bobui6_deploy_runtime_dependencies)

    if(NOT __BOBUI_DEPLOY_TOOL)
        message(FATAL_ERROR "No BobUI deploy tool available for this target platform")
    endif()
    ...
    set(multi_value_options
        ...
        # individual platform-specific tools (macdeploybobui, windeploybobui, etc.).
    ...
    if(NOT arg_BIN_DIR)
        set(arg_BIN_DIR "${BOBUI_DEPLOY_BIN_DIR}")
    endif()
    if(NOT arg_LIBEXEC_DIR)
        set(arg_LIBEXEC_DIR "${BOBUI_DEPLOY_LIBEXEC_DIR}")
    endif()
    if(NOT arg_LIB_DIR)
        set(arg_LIB_DIR "${BOBUI_DEPLOY_LIB_DIR}")
    endif()
    if(NOT arg_QML_DIR)
        set(arg_QML_DIR "${BOBUI_DEPLOY_QML_DIR}")
    endif()
    if(NOT arg_PLUGINS_DIR)
        set(arg_PLUGINS_DIR "${BOBUI_DEPLOY_PLUGINS_DIR}")
    endif()

    # macdeploybobui always writes out a bobui.conf file. It will complain if one
    ...
    if(__BOBUI_DEPLOY_SYSTEM_NAME STREQUAL Darwin)
        ...
        # pass to macdeploybobui.
        if(arg_EXECUTABLE MATCHES "^((.*/)?(.*).app)/Contents/MacOS/(.*)$")
            set(arg_EXECUTABLE "${CMAKE_MATCH_1}")
        endif()
    elseif(arg_GENERATE_BOBUI_CONF)
        set(exe_dir "${BOBUI_DEPLOY_BIN_DIR}")
        ...
        bobui6_deploy_bobui_conf("${BOBUI_DEPLOY_PREFIX}/${exe_dir}/bobui.conf"
            PREFIX "${prefix}"
            BIN_DIR "${arg_BIN_DIR}"
            LIB_DIR "${arg_LIB_DIR}"
    ...
    if(arg_VERBOSE OR __BOBUI_DEPLOY_VERBOSE)
        # macdeploybobui supports 0-3: 0=no output, 1=error/warn (default), 2=normal, 3=debug
        # windeploybobui supports 0-2: 0=error/warn (default), 1=verbose, 2=full_verbose
        if(__BOBUI_DEPLOY_SYSTEM_NAME STREQUAL Windows)
            list(APPEND tool_options --verbose 2)
        elseif(__BOBUI_DEPLOY_SYSTEM_NAME STREQUAL Darwin)
    ...
    if(__BOBUI_DEPLOY_SYSTEM_NAME STREQUAL Windows)
        list(APPEND tool_options
            ...
            --translationdir "${BOBUI_DEPLOY_TRANSLATIONS_DIR}"
        )
        ...
        # Specify path to target BobUI's bobuipaths .exe or .bat file, so windeploybobui deploys the correct
        # libraries when cross-compiling from x86_64 to arm64 windows.
        # We need to point to a bobuipaths that will give info about the target platform, but which
        # can run on the host.
        if(__BOBUI_DEPLOY_TARGET_BOBUI_PATHS_PATH AND EXISTS "${__BOBUI_DEPLOY_TARGET_BOBUI_PATHS_PATH}")
            list(APPEND tool_options --bobuipaths "${__BOBUI_DEPLOY_TARGET_BOBUI_PATHS_PATH}")
        else()
            message(WARNING
                "No bobuipaths executable found for target BobUI "
                "at: ${__BOBUI_DEPLOY_TARGET_BOBUI_PATHS_PATH}. "
    ...
    list(APPEND tool_options ${__bobui_deploy_tool_extra_options})

    if(__BOBUI_DEPLOY_TOOL STREQUAL "GRD")
        message(STATUS "Running generic BobUI deploy tool on ${arg_EXECUTABLE}")
        ...
        _bobui_internal_generic_deploybobui(
            LIB_DIR "${arg_LIB_DIR}"
            PLUGINS_DIR "${arg_PLUGINS_DIR}"
            ${tool_options}
        )
    ...
    # Both windeploybobui and macdeploybobui don't differentiate between the different
    ...
    message(STATUS
        "Running BobUI deploy tool for ${arg_EXECUTABLE} in working directory '${BOBUI_DEPLOY_PREFIX}'")
    execute_process(
        COMMAND_ECHO STDOUT
        COMMAND "${__BOBUI_DEPLOY_TOOL}" "${arg_EXECUTABLE}" ${tool_options}
        WORKING_DIRECTORY "${BOBUI_DEPLOY_PREFIX}"
        RESULT_VARIABLE result
    )
    if(result)
        message(FATAL_ERROR "Executing ${__BOBUI_DEPLOY_TOOL} failed: ${result}")
    endif()

endfunction()

if(NOT __BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_deploy_runtime_dependencies)
        if(__BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_deploy_runtime_dependencies(${ARGV})
        else()
            message(FATAL_ERROR "bobui_deploy_runtime_dependencies() is only available in BobUI 6.")
        endif()
    endfunction()
endif()

function(_bobui_internal_show_skip_runtime_deploy_message bobui_build_type_string)
    set(no_value_options "")
    set(single_value_options
        EXTRA_MESSAGE
    )
    ...
    message(STATUS
        ...
        "this target platform (${__BOBUI_DEPLOY_SYSTEM_NAME}, ${bobui_build_type_string}). "
        "${arg_EXTRA_MESSAGE}"
    )
endfunction()
...
function(bobui6_deploy_translations)
    ...
    if(arg_CATALOGS)
        ...
        set(catalogs ${__BOBUI_DEPLOY_I18N_CATALOGS})
    endif()

    get_filename_component(bobui_translations_dir "${__BOBUI_DEPLOY_BOBUI_INSTALL_TRANSLATIONS}" ABSOLUTE
        BASE_DIR "${__BOBUI_DEPLOY_BOBUI_INSTALL_PREFIX}"
    )
    set(locales ${arg_LOCALES})
    if(NOT locales)
        file(GLOB locales RELATIVE "${bobui_translations_dir}" "${bobui_translations_dir}/*.qm")
        list(TRANSFORM locales REPLACE "\\.qm$" "")
        list(TRANSFORM locales REPLACE "^bobui_help" "bobui-help")
    ...
    set(output_dir "${BOBUI_DEPLOY_PREFIX}/${BOBUI_DEPLOY_TRANSLATIONS_DIR}")
    ...
    if(arg_LCONVERT)
        ...
        set(lconvert "${__BOBUI_DEPLOY_BOBUI_INSTALL_PREFIX}/${__BOBUI_DEPLOY_BOBUI_INSTALL_BINS}/lconvert")
        if(CMAKE_HOST_WIN32)
    ...
    if(verbose)
        message(STATUS "Looking for translations in ${bobui_translations_dir}")
    endif()
    foreach(locale IN LISTS locales)
        set(qm_files "")
        foreach(catalog IN LISTS catalogs)
            set(qm_file "${catalog}_${locale}.qm")
            if(EXISTS "${bobui_translations_dir}/${qm_file}")
        ...
        # Merge the .qm files into one bobui_${locale}.qm file.
        set(output_file_name "bobui_${locale}.qm")
        ...
        execute_process(
            ...
            WORKING_DIRECTORY ${bobui_translations_dir}
            RESULT_VARIABLE process_result
            ${extra_options}
        )
        if(NOT process_result EQUAL "0")
...
if(NOT __BOBUI_NO_CREATE_VERSIONLESS_FUNCTIONS)
    function(bobui_deploy_translations)
        if(__BOBUI_DEFAULT_MAJOR_VERSION EQUAL 6)
            bobui6_deploy_translations(${ARGV})
        else()
            message(FATAL_ERROR "bobui_deploy_translations() is only available in BobUI 6.")
        endif()
    endfunction()
endif()
