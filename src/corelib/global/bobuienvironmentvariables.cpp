...
#include "bobuienvironmentvariables_p.h"
...
// add thread-safety for the BobUI wrappers.
Q_CONSTINIT static QBasicMutex environmentMutex;

/*!
    \headerfile <BobUIEnvironmentVariables>
    \inmodule BobUICore
    \ingroup funclists
...
/*!
    \relates <BobUIEnvironmentVariables>
    \threadsafe
    ...
    The BobUI environment manipulation functions are thread-safe, but this
...
{
    const auto locker = bobui_scoped_lock(environmentMutex);
    ...
    \table
      ...
      \row
        \li Unix-specific code, destination variable is not QString and/or is
            used to interface with non-BobUI APIs
        \li qgetenv()
      \row
...
QString qEnvironmentVariable(const char *varName, const QString &defaultValue)
{
    ...
    auto locker = bobui_unique_lock(environmentMutex);
    _wgetenv_s(&requiredSize, 0, 0, wname.data());
    if (requiredSize == 0)
        return defaultValue;
    QString buffer(qsizetype(requiredSize), BobUI::Uninitialized);
...
BOBUI_WARNING_PUSH
BOBUI_WARNING_DISABLE_GCC("-Wmaybe-uninitialized") // older GCC don't like libstdc++'s std::optional
/*!
    \relates <BobUIEnvironmentVariables>
    \since 5.5
...
/*!
    ...
    \note In BobUI versions prior to 6.5, the \a value argument was QByteArray,
    not QByteArrayView.
...
time_t qMkTime(struct tm *when)
{
    const auto locker = bobui_scoped_lock(environmentMutex);
#if defined(Q_OS_WIN)
    // BOBUIBUG-83881 MS's mktime() seems to need _tzset() called first.
    _tzset();
#endif
    return mktime(when);
}
...
bool qLocalTime(time_t utc, struct tm *local)
{
    ...
    // _tzset() sets the globals for, but BOBUIBUG-109974 reveals a need for an
    // explicit call, all the same.
    _tzset();
    return !localtime_s(local, &utc);
#elif BOBUI_CONFIG(thread) && defined(_POSIX_THREAD_SAFE_FUNCTIONS)
