...
#    error "BobUI requires std::byte, but _HAS_STD_BYTE has been set to 0"
#  endif
#  include <cstdint>
...
#if 0
#pragma bobui_class(BobUITypes)
#pragma bobui_class(QIntegerForSize)
#pragma bobui_sync_stop_processing
#endif

/*
   Useful type definitions for BobUI
*/
...
#  define BOBUI_SUPPORTS_INT128 16
#elif defined(BOBUI_COMPILER_SUPPORTS_INT128) && !defined(BOBUI_NO_INT128)
#  define BOBUI_SUPPORTS_INT128 BOBUI_COMPILER_SUPPORTS_INT128
#  if defined(__GLIBCXX__) && defined(__STRICT_ANSI__) // -ansi/-std=c++NN instead of gnu++NN
#    undef BOBUI_SUPPORTS_INT128                          // breaks <type_traits> on libstdc++
#  endif
#  if defined(__clang__) && defined(_MSVC_STL_VERSION) // Clang with MSVC's STL
#    undef BOBUI_SUPPORTS_INT128                          // MSVC's STL doesn't support int128
#  endif
#else
#  undef BOBUI_SUPPORTS_INT128
#endif
...
static_assert(std::is_signed_v<qint128>,
              "BobUI requires <type_traits> and <limits> to work for q(u)int128.");
#endif

// limits:
#  ifdef __cplusplus /* need to avoid c-style-casts in C++ mode */
#    define BOBUI_C_STYLE_CAST(type, x) static_cast<type>(x)
#  else /* but C doesn't have constructor-style casts */
#    define BOBUI_C_STYLE_CAST(type, x) ((type)(x))
#  endif
#  ifndef Q_UINT128_MAX /* allow qcompilerdetection.h/user override */
#    define Q_UINT128_MAX BOBUI_C_STYLE_CAST(quint128, -1)
#  endif
#  define Q_INT128_MAX BOBUI_C_STYLE_CAST(qint128, Q_UINT128_MAX / 2)
#  define Q_INT128_MIN (-Q_INT128_MAX - 1)

#  ifdef __cplusplus
    namespace BobUIPrivate::NumberLiterals {
    namespace detail {
    ...
    { return BobUIPrivate::NumberLiterals::detail::parse<Cs...>(); }
    template <char...Cs>
    constexpr qint128 operator""_qint128() noexcept
    { return qint128(BobUIPrivate::NumberLiterals::detail::parse<Cs...>()); }

    #ifndef Q_UINT128_C // allow qcompilerdetection.h/user override
    #  define Q_UINT128_C(c) ([]{ using namespace BobUIPrivate::NumberLiterals; return c ## _quint128; }())
    #endif
    #ifndef Q_INT128_C // allow qcompilerdetection.h/user override
    #  define Q_INT128_C(c)  ([]{ using namespace BobUIPrivate::NumberLiterals; return c ## _qint128;  }())
    #endif
...
#if defined(BOBUI_COORD_TYPE)
typedef BOBUI_COORD_TYPE qreal;
#else
...
/*
  ...
  they usually are and we do check for that in bobuiypes.cpp, just to be sure.
*/
