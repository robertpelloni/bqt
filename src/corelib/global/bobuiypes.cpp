...
/*!
    \headerfile <BobUITypes>
    \inmodule BobUICore
    \title BobUI Type Declarations

    \brief The <BobUITypes> header file includes BobUI fundamental type declarations.

    The header file declares several type definitions that guarantee a
    specified bit-size on all platforms supported by BobUI for various
    basic types, for example \l qint8 which is a signed char
    guaranteed to be 8-bit on all platforms supported by BobUI. The
    ...
    some BobUI type aliases (qsizetype, qintptr, etc.) via a formatted output
...
/*!
    ...
    Typedef for \c double unless BobUI is configured with the
...
/*! \typedef qint8
    ...
    on all platforms supported by BobUI.
*/
...
/*!
    ...
    be 8-bit on all platforms supported by BobUI.
*/
...
/*! \typedef qint16
    ...
    16-bit on all platforms supported by BobUI.
*/
...
/*!
    ...
    be 16-bit on all platforms supported by BobUI.
*/
...
/*!
    ...
    be 32-bit on all platforms supported by BobUI.
*/
...
/*!
    ...
    be 64-bit on all platforms supported by BobUI.
...
/*!
    ...
    Typedef for \c{__int128} on platforms that support it (BobUI defines the macro
    \l BOBUI_SUPPORTS_INT128 if this is the case).

    Literals of this type can be created using the Q_INT128_C() macro.

    \sa Q_INT128_C(), Q_INT128_MIN, Q_INT128_MAX, quint128, BOBUI_SUPPORTS_INT128
*/
...
/*!
    ...
    Typedef for \c{unsigned __int128} on platforms that support it (BobUI defines
    the macro \l BOBUI_SUPPORTS_INT128 if this is the case).

    Literals of this type can be created using the Q_UINT128_C() macro.

    \sa Q_UINT128_C(), Q_UINT128_MAX, qint128, BOBUI_SUPPORTS_INT128
*/

/*!
    \macro BOBUI_SUPPORTS_INT128
    \relates <BobUITypes>
    \since 6.6

    BobUI defines this macro as well as the \l qint128 and \l quint128 types if
...
/*!
    ...
    be the same size as a pointer on all platforms supported by BobUI. On
...
/*!
    ...
    the same size as a pointer on all platforms supported by BobUI. On a
...
/*!
    ...
    \sa qint128, Q_UINT128_C(), Q_INT128_MIN, Q_INT128_MAX, Q_INT64_C(), BOBUI_SUPPORTS_INT128
*/
...
/*!
    ...
    \sa quint128, Q_INT128_C(), Q_UINT128_MAX, Q_UINT64_C(), BOBUI_SUPPORTS_INT128
*/
...
/*!
    ...
    \sa Q_INT128_MAX, quint128, Q_UINT128_C, BOBUI_SUPPORTS_INT128
*/
...
/*!
    ...
    \sa Q_INT128_MAX, qint128, Q_INT128_C, BOBUI_SUPPORTS_INT128
*/
...
/*!
    ...
    \sa Q_INT128_MIN, Q_UINT128_MAX, qint128, Q_INT128_C, BOBUI_SUPPORTS_INT128
*/

// Statically check assumptions about the environment we're running
// in. The idea here is to error or warn if otherwise implicit BobUI
...
static_assert(UCHAR_MAX == 255, "BobUI assumes that char is 8 bits");
static_assert(sizeof(int) == 4, "BobUI assumes that int is 32 bits");
static_assert(BOBUI_POINTER_SIZE == sizeof(void *), "BOBUI_POINTER_SIZE defined incorrectly");
static_assert(sizeof(float) == 4, "BobUI assumes that float is 32 bits");
static_assert(sizeof(char16_t) == 2, "BobUI assumes that char16_t is 16 bits");
static_assert(sizeof(char32_t) == 4, "BobUI assumes that char32_t is 32 bits");
#if defined(Q_OS_WIN)
static_assert(sizeof(wchar_t) == sizeof(char16_t));
#endif
static_assert(std::numeric_limits<int>::radix == 2,
                  "BobUI assumes binary integers");
static_assert((std::numeric_limits<int>::max() + std::numeric_limits<int>::lowest()) == -1,
                  "BobUI assumes two's complement integers");
static_assert(sizeof(wchar_t) == sizeof(char32_t) || sizeof(wchar_t) == sizeof(char16_t),
              "BobUI assumes wchar_t is compatible with either char32_t or char16_t");
...
static_assert(std::numeric_limits<float>::is_iec559,
                  "BobUI assumes IEEE 754 floating point");
#endif
...
static_assert(std::numeric_limits<float>::radix == 2,
                  "BobUI assumes binary IEEE 754 floating point");
...
#if defined(BOBUI_COMPILER_SUPPORTS_INT128) && !defined(BOBUI_NO_INT128)
# ifndef BOBUI_SUPPORTS_INT128
#  error BobUI needs to be compiled in a mode that enables INT128 \
    if the compiler supports it in principle and BOBUI_NO_INT128 is not defined.
# endif
#endif
