...
BOBUI_WARNING_DISABLE_INTEL(103)
 ...
 *
 * - #if BOBUI_COMPILER_SUPPORTS(XXX)
 ...
 *
 * - #if BOBUI_COMPILER_SUPPORTS_HERE(XXX)
 *   If this test passes, then the compiler is able to generate code for that
 ...
 *   BOBUI_FUNCTION_TARGET(XXX) to cause the compiler to generate code for that
 ...
 *
 * The distinction between BOBUI_COMPILER_SUPPORTS and BOBUI_COMPILER_SUPPORTS_HERE is
 ...
 *      }
 *
 * 2) place the optimized code in a function tagged with BOBUI_FUNCTION_TARGET and
 * surrounded by #if BOBUI_COMPILER_SUPPORTS_HERE(XXX). That code can freely use
 * other BobUI code. The dispatcher function would look like this:
 *
 *      void foo()
 *      {
 *      #if BOBUI_COMPILER_SUPPORTS_HERE(XXX)
...
#define BOBUI_COMPILER_SUPPORTS(x)     (defined BOBUI_COMPILER_SUPPORTS_##x && BOBUI_COMPILER_SUPPORTS_##x)

#if defined(Q_PROCESSOR_ARM_64)
#  define BOBUI_COMPILER_SUPPORTS_HERE(x)    ((defined __ARM_FEATURE_##x && __ARM_FEATURE_##x) || (defined __##x##__ && __##x##__) || BOBUI_COMPILER_SUPPORTS(x))
#  if defined(Q_CC_GNU) || defined(Q_CC_CLANG)
     /* GCC requires attributes for a function */
#    define BOBUI_FUNCTION_TARGET(x)  __attribute__((__target__(BOBUI_FUNCTION_TARGET_STRING_ ## x)))
#  else
#    define BOBUI_FUNCTION_TARGET(x)
#  endif
#elif defined(Q_PROCESSOR_ARM_32)
   /* We do not support for runtime CPU feature switching on ARM32 */
#  define BOBUI_COMPILER_SUPPORTS_HERE(x)    ((defined __ARM_FEATURE_##x && __ARM_FEATURE_##x) || (defined __##x##__ && __##x##__))
#  define BOBUI_FUNCTION_TARGET(x)
#elif defined(Q_PROCESSOR_MIPS)
#  define BOBUI_COMPILER_SUPPORTS_HERE(x)    (defined __##x##__ && __##x##__)
#  define BOBUI_FUNCTION_TARGET(x)
...
#  define BOBUI_COMPILER_SUPPORTS_HERE(x)    BOBUI_COMPILER_SUPPORTS(x)
#  define BOBUI_FUNCTION_TARGET(x)
#elif defined(Q_PROCESSOR_X86)
#  if defined(Q_CC_CLANG) && defined(Q_CC_MSVC) && (Q_CC_CLANG < 1900)
#    define BOBUI_COMPILER_SUPPORTS_HERE(x)    (defined __##x##__ && __##x##__)
#  else
#    define BOBUI_COMPILER_SUPPORTS_HERE(x)    ((defined __##x##__ && __##x##__) || BOBUI_COMPILER_SUPPORTS(x))
#  endif
#  if defined(Q_CC_GNU) || defined(Q_CC_CLANG)
     /* GCC requires attributes for a function */
#    define BOBUI_FUNCTION_TARGET(x)  __attribute__((__target__(BOBUI_FUNCTION_TARGET_STRING_ ## x)))
...
#if defined(__SSE2__) && !defined(BOBUI_COMPILER_SUPPORTS_SSE2) && !defined(BOBUI_BOOTSTRAPPED)
...
#  if defined(BOBUI_COMPILER_SUPPORTS_RDSEED) && defined(Q_OS_QNX)
// The compiler for QNX is missing the intrinsic
#    undef BOBUI_COMPILER_SUPPORTS_RDSEED
#  endif
...
#  include <BobUICore/private/qsimd_x86_p.h>
...
#define BOBUI_FUNCTION_TARGET_STRING_NEON      "neon"
#else
#define BOBUI_FUNCTION_TARGET_STRING_NEON      "+neon" // unused: gcc doesn't support function targets on non-aarch64, and on Aarch64 NEON is always available.
#endif
#ifndef __ARM_NEON__
...
#if defined(Q_PROCESSOR_ARM_64)
#if defined(Q_CC_CLANG)
#define BOBUI_FUNCTION_TARGET_STRING_AES        "aes"
#define BOBUI_FUNCTION_TARGET_STRING_CRC32      "crc"
#define BOBUI_FUNCTION_TARGET_STRING_SVE        "sve"
#elif defined(Q_CC_GNU)
#define BOBUI_FUNCTION_TARGET_STRING_AES        "+crypto"
#define BOBUI_FUNCTION_TARGET_STRING_CRC32      "+crc"
#define BOBUI_FUNCTION_TARGET_STRING_SVE        "+sve"
#elif defined(Q_CC_MSVC)
#define BOBUI_FUNCTION_TARGET_STRING_AES
#define BOBUI_FUNCTION_TARGET_STRING_CRC32
#define BOBUI_FUNCTION_TARGET_STRING_SVE
#endif
#elif defined(Q_PROCESSOR_ARM_32)
#if defined(Q_CC_CLANG)
#define BOBUI_FUNCTION_TARGET_STRING_AES        "armv8-a,crypto"
#define BOBUI_FUNCTION_TARGET_STRING_CRC32      "armv8-a,crc"
#elif defined(Q_CC_GNU)
#define BOBUI_FUNCTION_TARGET_STRING_AES        "arch=armv8-a+crypto"
#define BOBUI_FUNCTION_TARGET_STRING_CRC32      "arch=armv8-a+crc"
#endif
#endif
...
#endif
extern Q_CORE_EXPORT Q_ATOMIC(QCpuFeatureType) BOBUI_MANGLE_NAMESPACE(bobui_cpu_features)[1];
Q_CORE_EXPORT uint64_t BOBUI_MANGLE_NAMESPACE(qDetectCpuFeatures)();
...
#else
    quint64 features = atomic_load_explicit(BOBUI_MANGLE_NAMESPACE(bobui_cpu_features), memory_order_relaxed);
    if (!BOBUI_SUPPORTS_INIT_PRIORITY) {
        if (Q_UNLIKELY(features == 0))
            features = BOBUI_MANGLE_NAMESPACE(qDetectCpuFeatures)();
    }
    return features;
#endif
}
