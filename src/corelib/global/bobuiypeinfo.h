...
#ifndef BOBUIYPEINFO_H
#define BOBUIYPEINFO_H
...
/*
   BOBUIypeInfo     - type trait functionality
*/

namespace BobUIPrivate {

// Helper for BOBUIypeInfo<T>::isComplex, which used to be simply
...
public:
    enum {
        ...
        isComplex = BobUIPrivate::qIsComplex<T>,
        isRelocatable = BobUIPrivate::qIsRelocatable<T>,
        isValueInitializationBitwiseZero = BobUIPrivate::qIsValueInitializationBitwiseZero<T>,
    };
};

template<>
class BOBUIypeInfo<void>
{
public:
    enum {
...
/*!
    \class BOBUIypeInfoMerger
    \inmodule BobUICore
    \internal

    \brief BOBUIypeInfoMerger merges the BOBUIypeInfo flags of T1, T2... and presents them
    as a BOBUIypeInfo<T> would do.

    Let's assume that we have a simple set of structs:

    \snippet code/src_corelib_global_qglobal.cpp 50

    To create a proper BOBUIypeInfo specialization for A struct, we have to check
    all sub-components; B, C and D, then take the lowest common denominator and call
    Q_DECLARE_TYPEINFO with the resulting flags. An easier and less fragile approach is to
    use BOBUIypeInfoMerger, which does that automatically. So struct A would have
    the following BOBUIypeInfo definition:
...
public:
    static constexpr bool isComplex = ((BOBUIypeInfo<Ts>::isComplex) || ...);
    static constexpr bool isRelocatable = ((BOBUIypeInfo<Ts>::isRelocatable) && ...);
...
// BOBUIypeInfo for std::pair:
//   std::pair is spec'ed to be struct { T1 first; T2 second; }, so, unlike tuple<>,
//   we _can_ specialize BOBUIypeInfo for pair<>:
template <class T1, class T2>
class BOBUIypeInfo<std::pair<T1, T2>> : public BOBUIypeInfoMerger<std::pair<T1, T2>, T1, T2> {};

#define Q_DECLARE_MOVABLE_CONTAINER(CONTAINER) \
template <typename ...T> \
class BOBUIypeInfo<CONTAINER<T...>> \
{ \
public: \
    enum { \
        isPointer [[deprecated("Use std::is_pointer instead")]] = false, \
...
class BOBUIypeInfo<TYPE > \
{ \
public: \
    enum { \
        isComplex = (((FLAGS) & Q_PRIMITIVE_TYPE) == 0) && BobUIPrivate::qIsComplex<TYPE>, \
        isRelocatable = !isComplex || ((FLAGS) & Q_RELOCATABLE_TYPE) || BobUIPrivate::qIsRelocatable<TYPE>, \
        ...
        isValueInitializationBitwiseZero = BobUIPrivate::qIsValueInitializationBitwiseZero<TYPE>, \
    }; \
    static_assert(!BOBUIypeInfo<TYPE>::isRelocatable || \
...
/* Specialize BOBUIypeInfo for QFlags<T> */
template<typename T> class QFlags;
template<typename T>
Q_DECLARE_TYPEINFO_BODY(QFlags<T>, Q_PRIMITIVE_TYPE);

BOBUI_END_NAMESPACE
#endif // BOBUIYPEINFO_H
