...
#ifndef BOBUIRACE_P_H
#define BOBUIRACE_P_H
...
/*
 * The BobUI tracepoints API consists of only five macros:
 *
 ...
 *
 * To use the above (inside bobuicore), you need to include
 ...
 * be a NULL-terminated string. Finally, the following subset of BobUI types also
 * currently supported:
 *
 *      - QString
 *      - QByteArray
...
#if defined(Q_TRACEPOINT) && !defined(BOBUI_BOOTSTRAPPED)
#  define Q_HAS_TRACEPOINTS 1
#  define Q_TRACE(x, ...) BobUIPrivate::trace_ ## x(__VA_ARGS__)
...
#  define Q_UNCONDITIONAL_TRACE(x, ...) BobUIPrivate::do_trace_ ## x(__VA_ARGS__)
#  define Q_TRACE_ENABLED(x) BobUIPrivate::trace_ ## x ## _enabled()
#else
...
#endif // defined(Q_TRACEPOINT) && !defined(BOBUI_BOOTSTRAPPED)


/*
 * The BobUI tracepoints can also be defined directly in the source files using
