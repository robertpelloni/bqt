...
template <typename T>
BOBUI_DEPRECATED_VERSION_X_6_0("Do not use foreach/Q_FOREACH with containers which are not implicitly shared. "
...
#define Q_FOREACH_IMPL(variable, name, container)                                             \
    for (auto name = BobUIPrivate::qMakeForeachContainer(container); name.i != name.e; ++name.i) \
