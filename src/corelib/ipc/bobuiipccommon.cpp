...
/*!
    ...
    similar to BobUIIpcCommon::platformSafeKey() below, but using an algorithm
    that is compatible with BobUI 4.4 to 6.6. Additionally, the returned
...
*/
QNativeIpcKey BobUIIpcCommon::legacyPlatformSafeKey(const QString &key, BobUIIpcCommon::IpcType ipcType,
...
*/
QNativeIpcKey BobUIIpcCommon::platformSafeKey(const QString &key, BobUIIpcCommon::IpcType ipcType,
                                           QNativeIpcKey::Type type)
{
    QNativeIpcKey k(type);
    if (key.isEmpty())
...
/*!
    ...
    value as well as the key type are encapsulated in BobUI using the \l
    QNativeIpcKey class.
    ...
    input to those functions, as different versions of BobUI may perform different
    ...
    \section2 Communicating keys to other BobUI processes
    ...
    as the new one (for example, it is a BobUI 5 application), provided the
    options passed to the BobUI configure script are the same.

    \section2 Communicating keys to non-BobUI processes

    When communicating with non-BobUI processes, the application must arrange to
    ...
    type is supported by the BobUI version in question. That is to say, an older
    BobUI will fail to parse the string representation of a key type introduced
    after it was released. However, successfully parsing a string
    representation does not imply the BobUI classes can successfully create an
...
/*!
    ...
    of Unix systems prior to BobUI 6.6; see legacyDefaultTypeForOs() for more
    information.
*/
...
/*!
    ...
    \l{QSharedMemory} and \l{QSystemSemaphore} used to use prior to BobUI 6.6.
    Applications and libraries that must retain compatibility with code using
    either class that was compiled with BobUI prior to version 6.6 can use this
    function to determine what IPC type the other applications may be using.

    Note that this function relies on BobUI having been built with identical
...
#include "moc_bobuiipccommon.cpp"
