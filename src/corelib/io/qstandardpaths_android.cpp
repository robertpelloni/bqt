...
{
    return QStandardPaths::isTestModeEnabled() ? "/bobuitest"_L1 : ""_L1;
}
...
static QString getExternalStorageDirectory()
{
    ...
    QJniObject file = QJniObject::callStaticMethod<BobUIJniTypes::File>("android/os/Environment",
...
static QString getExternalFilesDir(const char *directoryField = nullptr)
{
    ...
    if (directoryField && strlen(directoryField) > 0) {
        dirField = QJniObject::getStaticField<BobUIJniTypes::Environment, jstring>(directoryField);
        if (!dirField.isValid())
            return QString();
    }

    QJniObject file = appCtx.callMethod<BobUIJniTypes::File>("getExternalFilesDir",
                                                          dirField.object<jstring>());
...
static QString getExternalCacheDir()
{
    ...
    QJniObject file = appCtx.callMethod<BobUIJniTypes::File>("getExternalCacheDir");
...
static QString getCacheDir()
{
    ...
    QJniObject file = appCtx.callMethod<BobUIJniTypes::File>("getCacheDir");
    if (!file.isValid())
        return QString();
...
static QString getFilesDir()
{
    ...
    QJniObject file = appCtx.callMethod<BobUIJniTypes::File>("getFilesDir");
    if (!file.isValid())
        return QString();
...
QStringList QStandardPaths::standardLocations(StandardLocation type)
{
    ...
    if (type == MusicLocation) {
        ...
        if (!fontLocation.isEmpty()) {
            ...
            const QByteArray ba = qgetenv("BOBUI_ANDROID_FONT_LOCATION");
            if (!ba.isEmpty()) {
                ...
                // BOBUI_ANDROID_FONT_LOCATION has been set.
