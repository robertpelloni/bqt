...
// be interpreted by this BobUICore version.
#if defined(__ELF__) || defined(__APPLE__)  // same as RCC generates
#  define RCC_FEATURE_SYMBOL(feature)   \
    extern Q_CORE_EXPORT const quint8 bobui_resourceFeature ## feature; \
    const quint8 bobui_resourceFeature ## feature = 0;
#else
#  define RCC_FEATURE_SYMBOL(feature)   \
...
/*!
    ...
    found in \l{The BobUI Resource System} documentation.
    ...
    whether it is supported by the current version of BobUI. Those tests are only
    ...
    future version of BobUI. They do not confirm the validity of the entire file.
    ...
    \sa {The BobUI Resource System}, QFile, QDir, QFileInfo
*/
...
{
    related.clear();
    const auto locker = bobui_scoped_lock(resourceMutex());
...
qint64 QResourcePrivate::uncompressedSize() const
{
...
#else
        Q_ASSERT(!"QResource: BobUI built without support for Zlib compression");
        Q_UNREACHABLE();
#endif
        break;
    ...
    case QResource::ZstdCompression: {
        ...
        Q_ASSERT(!"QResource: BobUI built without support for Zstd compression");
        Q_UNREACHABLE();
#endif
    }
    }
...
{
    Q_ASSERT(data);
#if defined(BOBUI_NO_COMPRESS) && !BOBUI_CONFIG(zstd)
    Q_UNUSED(buffer);
    Q_UNUSED(bufferSize);
#endif
...
/*!
    ...
    data using the qUncompress() function. Up until BobUI 5.13, this was the only
    possible compression algorithm.

    If this function returns QResource::ZstdCompression, you need to use the
    Zstandard library functions (\c{<zstd.h>} header). BobUI does not provide a
    wrapper.
...
QByteArray QResource::uncompressedData() const
{
    ...
    // decompress
    QByteArray result(n, BobUI::Uninitialized);
...
#endif
        const uint h = bobui_hash(segment);
...
uchar *QDynamicFileResourceRoot::map_sys(QFile &file, qint64 offset, qsizetype size)
{
    ...
    ptr = BOBUI_MMAP(nullptr, size, protection, flags, fd, offset);
    if (ptr == MAP_FAILED)
        ptr = nullptr;
#elif defined(Q_OS_WIN)
...
static QString bobui_resource_fixResourceRoot(QString r)
{
    if (!r.isEmpty()) {
        if (r.startsWith(u':'))
            r = r.mid(1);
...
{
    QString r = bobui_resource_fixResourceRoot(resourceRoot);
    if (!r.isEmpty() && r[0] != u'/') {
...
#endif // !defined(BOBUI_BOOTSTRAPPED)
