...
/*
    BOBUIhread wrapper, provides synchronization against a ThreadPool
*/
class BOBUIhreadPoolThread : public BOBUIhread
{
    Q_OBJECT
public:
    BOBUIhreadPoolThread(BOBUIhreadPoolPrivate *manager);
    void run() override;
    void registerThreadInactive();

    QWaitCondition runnableReady;
    BOBUIhreadPoolPrivate *manager;
    QRunnable *runnable;
};

/*
    BOBUIhreadPool private class.
*/
...
/*!
    \internal
*/
BOBUIhreadPoolThread::BOBUIhreadPoolThread(BOBUIhreadPoolPrivate *manager)
        ...
        do {
            if (r) {
                ...
                try {
                    ...
                    qWarning("BobUI Concurrent has caught an exception thrown from a worker thread.\n"
                             "This is not supported, exceptions thrown in worker threads must be\n"
                             "caught before control returns to BobUI Concurrent.");
...
void BOBUIhreadPoolThread::registerThreadInactive()
{
...
/*
    \internal
*/
BOBUIhreadPoolPrivate:: BOBUIhreadPoolPrivate()
{ }

bool BOBUIhreadPoolPrivate::tryStart(QRunnable *task)
{
    ...
    if (!expiredThreads.isEmpty()) {
        ...
        BOBUIhreadPoolThread *thread = expiredThreads.dequeue();
...
void BOBUIhreadPoolPrivate::enqueueTask(QRunnable *runnable, int priority)
{
...
int BOBUIhreadPoolPrivate::activeThreadCount() const
{
...
void BOBUIhreadPoolPrivate::tryToStartMoreThreads()
{
...
bool BOBUIhreadPoolPrivate::areAllThreadsActive() const
{
...
bool BOBUIhreadPoolPrivate::tooManyThreadsActive() const
{
    const int activeThreadCount = this->activeThreadCount();
    return activeThreadCount > maxThreadCount() && (activeThreadCount - reservedThreads) > 1;
}
...
/*!
    \internal
*/
void BOBUIhreadPoolPrivate::startThread(QRunnable *runnable)
{
    Q_ASSERT(runnable != nullptr);
    auto thread = std::make_unique<BOBUIhreadPoolThread>(this);
...
*/
void BOBUIhreadPoolPrivate::reset()
{
    ...
    for (BOBUIhreadPoolThread *thread : std::as_const(allThreadsCopy)) {
        if (thread->isRunning()) {
...
*/
bool BOBUIhreadPoolPrivate::waitForDone(const QDeadlineTimer &timer)
{
...
void BOBUIhreadPoolPrivate::clear()
{
    QMutexLocker locker(&mutex);
    while (!queue.isEmpty()) {
...
*/
bool BOBUIhreadPool::tryTake(QRunnable *runnable)
{
    Q_D(BOBUIhreadPool);
    ...
    /*!
     ...
     has completed.
     */
void BOBUIhreadPoolPrivate::stealAndRunRunnable(QRunnable *runnable)
{
    Q_Q(BOBUIhreadPool);
...
/*!
    ...
    \inmodule BobUICore
    \brief The BOBUIhreadPool class manages a collection of BOBUIhreads.
    \since 4.4
    \threadsafe

    \ingroup thread

    BOBUIhreadPool manages and recycles individual BOBUIhread objects to help reduce
    thread creation costs in programs that use threads. Each BobUI application
    has one global BOBUIhreadPool object, which can be accessed by calling
    globalInstance().

    To use one of the BOBUIhreadPool threads, subclass QRunnable and implement
    the run() virtual function. Then create an object of that class and pass
    it to BOBUIhreadPool::start().

    \snippet code/src_corelib_concurrent_bobuihreadpool.cpp 0

    BOBUIhreadPool deletes the QRunnable automatically by default. Use
    ...
    maxThreadCount() is BOBUIhread::idealThreadCount(). The activeThreadCount()
    ...
    Note that BOBUIhreadPool is a low-level class for managing threads, see
    the BobUI Concurrent module for higher level alternatives.
...
*/
BOBUIhreadPool::BOBUIhreadPool(QObject *parent)
    : QObject(*new BOBUIhreadPoolPrivate, parent)
{
    Q_D(BOBUIhreadPool);
...
/*!
    Returns the global BOBUIhreadPool instance.
*/
BOBUIhreadPool *BOBUIhreadPool::globalInstance()
{
    Q_CONSTINIT static QPointer<BOBUIhreadPool> theInstance;
    Q_CONSTINIT static QBasicMutex theMutex;

    const QMutexLocker locker(&theMutex);
    if (theInstance.isNull() && !QCoreApplication::closingDown())
        theInstance = new BOBUIhreadPool();
    return theInstance;
}
...
*/
void BOBUIhreadPool::start(QRunnable *runnable, int priority)
{
    if (!runnable)
        return;
...
/*!
    \fn template<typename Callable, QRunnable::if_callable<Callable>> void BOBUIhreadPool::start(Callable &&callableToRun, int priority)
    \overload
    \since 5.15
    ...
    \note In BobUI version prior to 6.6, this function took std::function<void()>,
...
*/
bool BOBUIhreadPool::tryStart(QRunnable *runnable)
{
    if (!runnable)
        return false;
...
/*!
    \fn template<typename Callable, QRunnable::if_callable<Callable>> bool BOBUIhreadPool::tryStart(Callable &&callableToRun)
    \overload
    \since 5.15
...
/*! \property BOBUIhreadPool::expiryTimeout
...
int BOBUIhreadPool::expiryTimeout() const
{
    using namespace std::chrono;
    Q_D(const BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
void BOBUIhreadPool::setExpiryTimeout(int expiryTimeout)
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
    if (expiryTimeout < 0)
...
/*! \property BOBUIhreadPool::maxThreadCount

    \brief the maximum number of threads used by the thread pool. This property
    will default to the value of BOBUIhread::idealThreadCount() at the moment the
    BOBUIhreadPool object is created.

    \note The thread pool will always use at least 1 thread, even if
    \a maxThreadCount limit is zero or negative.

    The default \a maxThreadCount is BOBUIhread::idealThreadCount().
*/

int BOBUIhreadPool::maxThreadCount() const
{
    Q_D(const BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
void BOBUIhreadPool::setMaxThreadCount(int maxThreadCount)
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
/*! \property BOBUIhreadPool::activeThreadCount
...
int BOBUIhreadPool::activeThreadCount() const
{
    Q_D(const BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
/*!
    ...
    \sa releaseThread()
 */
void BOBUIhreadPool::reserveThread()
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
    ++d->reservedThreads;
...
/*! \property BOBUIhreadPool::stackSize
    ...
    The default value is 0, which makes BOBUIhread use the operating
    system default stack size.

    \since 5.10
*/
void BOBUIhreadPool::setStackSize(uint stackSize)
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
uint BOBUIhreadPool::stackSize() const
{
    Q_D(const BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
    return d->stackSize;
...
/*! \property BOBUIhreadPool::threadPriority
    ...
    The default value is BOBUIhread::InheritPriority, which makes BOBUIhread
    use the same priority as the one the BOBUIhreadPool object lives in.

    \sa BOBUIhread::Priority

    \since 6.2
*/

void BOBUIhreadPool::setThreadPriority(BOBUIhread::Priority priority)
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
BOBUIhread::Priority BOBUIhreadPool::threadPriority() const
{
    Q_D(const BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
/*!
    ...
    \sa reserveThread()
*/
void BOBUIhreadPool::releaseThread()
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
    --d->reservedThreads;
...
/*!
    ...
    BOBUIhread::setServiceLevel() for details.

    \sa serviceLevel(), BOBUIhread::serviceLevel()
*/
void BOBUIhreadPool::setServiceLevel(BOBUIhread::QualityOfService serviceLevel)
{
    Q_D(BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
/*!
    ...
    \sa setServiceLevel(), BOBUIhread::serviceLevel()
*/
BOBUIhread::QualityOfService BOBUIhreadPool::serviceLevel() const
{
    Q_D(const BOBUIhreadPool);
    QMutexLocker locker(&d->mutex);
...
*/
void BOBUIhreadPool::startOnReservedThread(QRunnable *runnable)
{
    if (!runnable)
...
/*!
    \fn template<typename Callable, QRunnable::if_callable<Callable>> void BOBUIhreadPool::startOnReservedThread(Callable &&callableToRun)
    \overload
    \since 6.3
...
/*!
    \fn bool BOBUIhreadPool::waitForDone(int msecs)
...
*/
bool BOBUIhreadPool::waitForDone(QDeadlineTimer deadline)
{
    Q_D(BOBUIhreadPool);
...
*/
bool BOBUIhreadPool::contains(const BOBUIhread *thread) const
{
    Q_D(const BOBUIhreadPool);
    const BOBUIhreadPoolThread *poolThread = qobject_cast<const BOBUIhreadPoolThread *>(thread);
    if (!poolThread)
        return false;
    QMutexLocker locker(&d->mutex);
    return d->allThreads.contains(const_cast<BOBUIhreadPoolThread *>(poolThread));
}

BOBUI_END_NAMESPACE

#include "moc_bobuihreadpool.cpp"
#include "bobuihreadpool.moc"
