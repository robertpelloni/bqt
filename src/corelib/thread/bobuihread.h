...
class BOBUIhreadPrivate;
class QAbstractEventDispatcher;
class QEventLoopLocker;

class Q_CORE_EXPORT BOBUIhread : public QObject
{
    Q_OBJECT
public:
    Q_DECL_PURE_FUNCTION static BobUI::HANDLE currentThreadId() noexcept;
    static BOBUIhread *currentThread();
    static bool isMainThread() noexcept;
    static int idealThreadCount() noexcept;
    static void yieldCurrentThread();

    explicit BOBUIhread(QObject *parent = nullptr);
    ~BOBUIhread();
    ...
    [[nodiscard]] static BOBUIhread *create(Function &&f, Args &&... args);
...
protected:
    BOBUIhread(BOBUIhreadPrivate &dd, QObject *parent = nullptr);

private:
    Q_DECLARE_PRIVATE(BOBUIhread)
    friend class QEventLoopLocker;

    [[nodiscard]] static BOBUIhread *createThreadImpl(std::future<void> &&future);
    Q_DECL_PURE_FUNCTION static BobUI::HANDLE currentThreadIdImpl() noexcept;
...
template <typename Function, typename... Args>
BOBUIhread *BOBUIhread::create(Function &&f, Args &&... args)
{
...
/*
    ...
    value for anything. In BobUI we use the handle to check if threads are identical,
...
*/
inline BobUI::HANDLE BOBUIhread::currentThreadId() noexcept
{
    // define is undefed if we have to fall back to currentThreadIdImpl
#define BOBUI_HAS_FAST_CURRENT_THREAD_ID
    BobUI::HANDLE tid; // typedef to void*
    ...
    tid = reinterpret_cast<BobUI::HANDLE>(__readgsqword(0x48));
    ...
    tid = reinterpret_cast<BobUI::HANDLE>(__readfsdword(0x24));
#elif defined(Q_PROCESSOR_X86_32) && defined(Q_OS_WIN) // !Q_CC_MSVC
    __asm__("mov %%fs:0x24, %0" : "=r" (tid));
#else
#undef BOBUI_HAS_FAST_CURRENT_THREAD_ID
