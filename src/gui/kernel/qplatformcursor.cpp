...
/*!
    ...
    This method is called by BobUI whenever a QMouseEvent is generated by the
...
/*!
    ...
    \brief This method is called by BobUI whenever the cursor graphic should be changed.
...
void QPlatformCursor::setPos(const QPoint &pos)
{
    ...
    QWindowSystemInterface::handleMouseEvent(nullptr, pos, pos, BobUI::NoButton, BobUI::NoButton, QEvent::MouseMove);
}
...
static QPlatformCursorImage *systemCursorTable[BobUI::LastCursor+1];
...
/*!
    \fn void QPlatformCursorImage::set(BobUI::CursorShape id)

    \brief Calling this method sets the cursor image to the specified shape

    \a id is one of the defined BobUI::CursorShape values.

    If id is invalid, BobUI::BitmapCursor, or unknown by the implementation,
    BobUI::ArrowCursor is used instead.
*/

void QPlatformCursorImage::set(BobUI::CursorShape id)
{
    QPlatformCursorImage *cursor = nullptr;
    if (unsigned(id) <= unsigned(BobUI::LastCursor)) {
    ...
    if (cursor == nullptr) {
        if (!systemCursorTable[BobUI::ArrowCursor])
            createSystemCursor(BobUI::ArrowCursor);
        cursor = systemCursorTable[BobUI::ArrowCursor];
    }
