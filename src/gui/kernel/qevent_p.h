...
class Q_GUI_EXPORT QMutableTouchEvent : public BOBUIouchEvent
{
public:
    QMutableTouchEvent(QEvent::Type eventType = QEvent::TouchBegin,
        ...
        BOBUIouchEvent(eventType, device, modifiers, touchPoints) { }
    ~QMutableTouchEvent() override;

    void setTarget(QObject *target) { m_target = target; }
    void addPoint(const QEventPoint &point);

    static void setTarget(BOBUIouchEvent *e, QObject *target) { e->m_target = target; }
    static void addPoint(BOBUIouchEvent *e, const QEventPoint &point);
};
...
public:
    ...
    QMutableSinglePointEvent(Type type = QEvent::None, const QPointingDevice *device = nullptr, const QEventPoint &point = QEventPoint(),
                             BobUI::MouseButton button = BobUI::NoButton, BobUI::MouseButtons buttons = BobUI::NoButton,
                             BobUI::KeyboardModifiers modifiers = BobUI::NoModifier,
                             BobUI::MouseEventSource source = BobUI::MouseEventSynthesizedByBobUI) :
        QSinglePointEvent(type, device, point, button, buttons, modifiers, source) { }
    ~QMutableSinglePointEvent() override;

    void setSource(BobUI::MouseEventSource s) { m_source = s; }
