...
BOBUI_IMPL_METATYPE_EXTERN_TAGGED(QInputDeviceManager::DeviceType, QInputDeviceManager__DeviceType)

/*!
  \class QInputDeviceManager
  \internal

  \brief QInputDeviceManager acts as a communication hub between BobUIGui and the input handlers.
  ...
  vs. libinput). QWindowSystemInterface is too limiting when BobUI (the platform plugin) is
  ...
  from the input handlers to the rest of BobUI (e.g. the number of connected mice, which may
 ...
 */
BobUI::KeyboardModifiers QInputDeviceManager::keyboardModifiers() const
{
    Q_D(const QInputDeviceManager);
    return d->keyboardModifiers;
}

void QInputDeviceManager::setKeyboardModifiers(BobUI::KeyboardModifiers mods)
{
