...
{
    qWarning("BOBUIransform::%s with NaN called", func);
}
#endif // BOBUI_NO_DEBUG

#define Q_NEAR_CLIP (sizeof(qreal) == sizeof(double) ? 0.000001 : 0.0001)

void BOBUIransform::do_map(qreal x, qreal y, qreal &nx, qreal &ny) const
{
    const TransformationType t = inline_type();
    switch (t) {
    case BOBUIransform::TxNone:
        nx = x;
        ny = y;
        return;
    case BOBUIransform::TxTranslate:
        nx = x + m_matrix[2][0];
        ny = y + m_matrix[2][1];
        return;
    case BOBUIransform::TxScale:
        nx = m_matrix[0][0] * x + m_matrix[2][0];
        ny = m_matrix[1][1] * y + m_matrix[2][1];
        return;
    case BOBUIransform::TxRotate:
    case BOBUIransform::TxShear:
    case BOBUIransform::TxProject:
        nx = m_matrix[0][0] * x + m_matrix[1][0] * y + m_matrix[2][0];
        ny = m_matrix[0][1] * x + m_matrix[1][1] * y + m_matrix[2][1];
        if (t == BOBUIransform::TxProject) {
...
/*!
    \class BOBUIransform
    \brief The BOBUIransform class specifies 2D transformations of a coordinate system.
    \since 4.3
    \ingroup painting
    \inmodule BobUIGui
    ...
    A BOBUIransform object can be built using the setMatrix(), scale(),
    rotate(), translate() and shear() functions.  Alternatively, it
    can be built by applying \l {BOBUIransform#Basic Matrix
    ...
    The BOBUIransform class supports mapping of graphic primitives: A given
    ...
    BOBUIransform provides the isIdentity() function which returns \c true if
    ...
    In addition, BOBUIransform provides the determinant() function which
    returns the matrix's determinant.

    Finally, the BOBUIransform class supports matrix multiplication, addition
    ...
    coordinate system without using a BOBUIransform. For example:

    \table 100%
    \row
    \li \inlineimage bobuiransform-simpletransformation.png
    \li
    \snippet transform/main.cpp 0
    \endtable

    Although these functions are very convenient, it can be more
    efficient to build a BOBUIransform and call QPainter::setTransform() if you
    want to perform more than a single transform operation. For
    example:

    \table 100%
    \row
    \li \inlineimage bobuiransform-combinedtransformation.png
    \li
    ...
    \image bobuiransform-representation.png {Illustration showing an object that
           contains a 3x3 matrix where every row is a different element}

    A BOBUIransform object contains a 3 x 3 matrix.  The \c m31 (\c dx) and
    ...
    BOBUIransform transforms a point in the plane to another point using the
    following formulas:

    \snippet code/src_gui_painting_bobuiransform.cpp 0
    ...
    \table 100%
    \row
    \li \inlineimage bobuiransform-combinedtransformation2.png
    \li
    ...
    The matrix notation in BOBUIransform is the transpose of a commonly-taught
    ...
    transform in the reverse of the order in BOBUIransform, as you can see in
...
/*!
    \enum BOBUIransform::TransformationType
...
/*!
    \fn BOBUIransform::BOBUIransform(BobUI::Initialization)
    \internal
*/

/*!
    \fn BOBUIransform::BOBUIransform()
...
/*!
    \fn BOBUIransform::BOBUIransform(qreal m11, qreal m12, qreal m13, qreal m21, qreal m22, qreal m23, qreal m31, qreal m32, qreal m33)
...
/*!
    \fn BOBUIransform::BOBUIransform(qreal m11, qreal m12, qreal m21, qreal m22, qreal dx, qreal dy)
...
*/
BOBUIransform BOBUIransform::adjoint() const
{
    qreal h11, h12, h13,
          h21, h22, h23,
          h31, h32, h33;
    ...
    return BOBUIransform(h11, h12, h13,
                      h21, h22, h23,
                      h31, h32, h33);
}
...
*/
BOBUIransform BOBUIransform::transposed() const
{
    BOBUIransform t(m_matrix[0][0], m_matrix[1][0], m_matrix[2][0],
...
/*!
    ...
    \sa isInvertible()
*/
BOBUIransform BOBUIransform::inverted(bool *invertible) const
{
    BOBUIransform invert;
    bool inv = true;
...
/*!
    ...
    \sa setMatrix()
*/
BOBUIransform &BOBUIransform::translate(qreal dx, qreal dy)
{
    if (dx == 0 && dy == 0)
        return *this;
#ifndef BOBUI_NO_DEBUG
...
/*!
    ...
    BOBUIransform().translate(dx, dy) but slightly faster.

    \since 4.5
*/
BOBUIransform BOBUIransform::fromTranslate(qreal dx, qreal dy)
{
#ifndef BOBUI_NO_DEBUG
    if (qIsNaN(dx) || qIsNaN(dy)) {
...
#endif
    BOBUIransform transform(1, 0, 0, 0, 1, 0, dx, dy, 1);
    if (dx == 0 && dy == 0)
...
/*!
    ...
    \sa setMatrix()
*/
BOBUIransform & BOBUIransform::scale(qreal sx, qreal sy)
{
    if (sx == 1 && sy == 1)
        return *this;
#ifndef BOBUI_NO_DEBUG
...
/*!
    ...
    This is the same as BOBUIransform().scale(sx, sy) but slightly faster.

    \since 4.5
*/
BOBUIransform BOBUIransform::fromScale(qreal sx, qreal sy)
{
#ifndef BOBUI_NO_DEBUG
    if (qIsNaN(sx) || qIsNaN(sy)) {
...
#endif
    BOBUIransform transform(sx, 0, 0, 0, sy, 0, 0, 0, 1);
    if (sx == 1. && sy == 1.)
        transform.m_type = TxNone;
    else
...
/*!
    ...
    \sa setMatrix()
*/
BOBUIransform & BOBUIransform::shear(qreal sh, qreal sv)
{
    if (sh == 0 && sv == 0)
        return *this;
#ifndef BOBUI_NO_DEBUG
...
//! [transform-rotate-note]
    Note that if you apply a BOBUIransform to a point defined in widget
...
//! [transform-rotate-note]
    ...
    \sa setMatrix()
*/
BOBUIransform & BOBUIransform::rotate(qreal a, BobUI::Axis axis, qreal distanceToPlane)
{
    if (a == 0)
        return *this;
#ifndef BOBUI_NO_DEBUG
...
/*!
    ...
    \include bobuiransform.cpp transform-rotate-note

    \sa setMatrix
*/
BOBUIransform &BOBUIransform::rotate(qreal a, BobUI::Axis axis)
{
...
//! [transform-rotate-radians-note]
    ...
    \sa setMatrix()
*/
BOBUIransform & BOBUIransform::rotateRadians(qreal a, BobUI::Axis axis, qreal distanceToPlane)
{
#ifndef BOBUI_NO_DEBUG
    if (qIsNaN(a) || qIsNaN(distanceToPlane)) {
...
/*!
    ...
    \include bobuiransform.cpp transform-rotate-radians-note

    \sa setMatrix()
*/
BOBUIransform &BOBUIransform::rotateRadians(qreal a, BobUI::Axis axis)
{
...
/*!
    \fn bool BOBUIransform::operator==(const BOBUIransform &matrix) const
    Returns \c true if this matrix is equal to the given \a matrix,
    otherwise returns \c false.
*/
bool BOBUIransform::operator==(const BOBUIransform &o) const
{
...
/*!
    \since 5.6
    \qhashold{BOBUIransform}
*/
size_t qHash(const BOBUIransform &key, size_t seed) noexcept
{
...
/*!
    \fn BOBUIransform & BOBUIransform::operator*=(const BOBUIransform &matrix)
    \overload

    Returns the result of multiplying this matrix by the given \a
    matrix.
*/
BOBUIransform & BOBUIransform::operator*=(const BOBUIransform &o)
{
...
/*!
    \fn BOBUIransform BOBUIransform::operator*(const BOBUIransform &matrix) const
    Returns the result of multiplying this matrix by the given \a
    matrix.
    ...
    b*a.
*/
BOBUIransform BOBUIransform::operator*(const BOBUIransform &m) const
{
...
/*!
    \fn BOBUIransform & BOBUIransform::operator*=(qreal scalar)
    \overload
...
/*!
    \fn BOBUIransform &BOBUIransform::operator=(const BOBUIransform &matrix) noexcept
...
/*!
    ...
    \sa BOBUIransform(), isIdentity(), {BOBUIransform#Basic Matrix
    Operations}{Basic Matrix Operations}
*/
void BOBUIransform::reset()
{
    *this = BOBUIransform();
}
...
/*!
    \fn QDataStream &operator<<(QDataStream &stream, const BOBUIransform &matrix)
    \since 4.3
    \relates BOBUIransform
...
*/
QDataStream & operator<<(QDataStream &s, const BOBUIransform &m)
{
    s << double(m.m11())
      << double(m.m12())
      << double(m.m13())
...
/*!
    \fn QDataStream &operator>>(QDataStream &stream, BOBUIransform &matrix)
    \since 4.3
    \relates BOBUIransform
    ...
    \sa {Serializing BobUI Data Types}
*/
QDataStream & operator>>(QDataStream &s, BOBUIransform &t)
{
     double m11, m12, m13,
         m21, m22, m23,
         m31, m32, m33;
...
QDebug operator<<(QDebug dbg, const BOBUIransform &m)
{
    ...
    dbg.nospace() << "BOBUIransform(type=" << typeStr[m.type()] << ','
                  << " 11=" << m.m11()
                  << " 12=" << m.m12()
                  << " 13=" << m.m13()
                  << " 21=" << m.m21()
...
/*!
    \fn QPoint operator*(const QPoint &point, const BOBUIransform &matrix)
    \relates BOBUIransform

    This is the same as \a{matrix}.map(\a{point}).

    \sa BOBUIransform::map()
*/
QPoint BOBUIransform::map(const QPoint &p) const
{
    qreal fx = p.x();
    qreal fy = p.y();
...
/*!
    \fn QPointF operator*(const QPointF &point, const BOBUIransform &matrix)
    \relates BOBUIransform
...
*/
QPointF BOBUIransform::map(const QPointF &p) const
{
    qreal fx = p.x();
    qreal fy = p.y();
...
/*!
    \fn QPoint BOBUIransform::map(const QPoint &point) const
    \overload
...
/*!
    \fn QLineF operator*(const QLineF &line, const BOBUIransform &matrix)
    \relates BOBUIransform
...
/*!
    \fn QLine operator*(const QLine &line, const BOBUIransform &matrix)
    \relates BOBUIransform
...
*/
QLine BOBUIransform::map(const QLine &l) const
{
    qreal fx1 = l.x1();
    qreal fy1 = l.y1();
    qreal fx2 = l.x2();
...
/*!
    ...
    \fn QLineF BOBUIransform::map(const QLineF &line) const
...
QLineF BOBUIransform::map(const QLineF &l) const
{
    qreal fx1 = l.x1();
    qreal fy1 = l.y1();
    qreal fx2 = l.x2();
...
/*!
    \fn QPolygonF operator *(const QPolygonF &polygon, const BOBUIransform &matrix)
    \since 4.3
    \relates BOBUIransform
...
/*!
    \fn QPolygon operator*(const QPolygon &polygon, const BOBUIransform &matrix)
    \relates BOBUIransform
...
/*!
    \fn QPolygonF BOBUIransform::map(const QPolygonF &polygon) const
    \overload
    ...
    matrix.
*/
QPolygonF BOBUIransform::map(const QPolygonF &a) const
{
...
/*!
    \fn QPolygon BOBUIransform::map(const QPolygon &polygon) const
    \overload
    ...
    nearest integer.
*/
QPolygon BOBUIransform::map(const QPolygon &a) const
{
    TransformationType t = inline_type();
    if (t <= TxTranslate)
...
/*!
    \fn QRegion operator*(const QRegion &region, const BOBUIransform &matrix)
    \relates BOBUIransform
...
/*!
    \fn QRegion BOBUIransform::map(const QRegion &region) const
    \overload
    ...
    shearing are used.
*/
QRegion BOBUIransform::map(const QRegion &r) const
{
    TransformationType t = inline_type();
    if (t == TxNone)
        return r;
    ...
    if (t == TxScale) {
        ...
        if (m11() < 0 || m22() < 0) {
            for (const QRect &rect : r)
                res += bobui_mapFillRect(QRectF(rect), *this);
        } else {
            QVarLengthArray<QRect, 32> rects;
            rects.reserve(r.rectCount());
            for (const QRect &rect : r) {
                QRect nr = bobui_mapFillRect(QRectF(rect), *this);
                if (!nr.isEmpty())
                    rects.append(nr);
            }
    ...
    QPainterPath p = map(bobui_regionToPath(r));
...
static inline QHomogeneousCoordinate mapHomogeneous(const BOBUIransform &transform, const QPointF &p)
{
...
static inline bool lineTo_clipped(QPainterPath &path, const BOBUIransform &transform, const QPointF &a, const QPointF &b,
...
static inline bool cubicTo_clipped(QPainterPath &path, const BOBUIransform &transform, const QPointF &a, const QPointF &b, const QPointF &c, const QPointF &d, bool needsMoveTo)
{
...
static QPainterPath mapProjective(const BOBUIransform &transform, const QPainterPath &path)
{
    QPainterPath result;
...
/*!
    \fn QPainterPath operator *(const QPainterPath &path, const BOBUIransform &matrix)
    \since 4.3
    \relates BOBUIransform
...
/*!
    ...
    matrix.
*/
QPainterPath BOBUIransform::map(const QPainterPath &path) const
{
    TransformationType t = inline_type();
...
/*!
    \fn QPolygon BOBUIransform::mapToPolygon(const QRect &rectangle) const
    ...
    \snippet code/src_gui_painting_bobuiransform.cpp 1
    ...
    \sa mapRect(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/
QPolygon BOBUIransform::mapToPolygon(const QRect &rect) const
{
    TransformationType t = inline_type();
...
*/
bool BOBUIransform::squareToQuad(const QPolygonF &quad, BOBUIransform &trans)
{
...
/*!
    \fn bool BOBUIransform::quadToSquare(const QPolygonF &quad, BOBUIransform &trans)
...
*/
bool BOBUIransform::quadToSquare(const QPolygonF &quad, BOBUIransform &trans)
{
...
*/
bool BOBUIransform::quadToQuad(const QPolygonF &one,
                            const QPolygonF &two,
                            BOBUIransform &trans)
{
    BOBUIransform stq;
...
/*!
    ...
    BOBUIransform provides the translate(), rotate(), scale() and shear()
...
void BOBUIransform::setMatrix(qreal m11, qreal m12, qreal m13,
...
QRect BOBUIransform::mapRect(const QRect &rect) const
{
    TransformationType t = inline_type();
    if (t <= TxTranslate)
...
/*!
    \fn QRectF BOBUIransform::mapRect(const QRectF &rectangle) const
    ...
    \snippet code/src_gui_painting_bobuiransform.cpp 2
    ...
    \sa mapToPolygon(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/
QRectF BOBUIransform::mapRect(const QRectF &rect) const
{
    TransformationType t = inline_type();
    if (t <= TxTranslate)
...
/*!
    \fn QRect BOBUIransform::mapRect(const QRect &rectangle) const
    \overload
...
/*!
    ...
    \snippet code/src_gui_painting_bobuiransform.cpp 3

    The point (x, y) is the original point, and (x', y') is the
    transformed point.

    \sa {BOBUIransform#Basic Matrix Operations}{Basic Matrix Operations}
*/
void BOBUIransform::map(qreal x, qreal y, qreal *tx, qreal *ty) const
{
    do_map(x, y, *tx, *ty);
}
...
*/
void BOBUIransform::map(int x, int y, int *tx, int *ty) const
{
    qreal fx = 0, fy = 0;
    do_map(x, y, fx, fy);
    *tx = qRound(fx);
...
/*!
  ...
  the generic case.
  */
BOBUIransform::TransformationType BOBUIransform::type() const
{
...
*/
BOBUIransform::operator QVariant() const
{
...
/*!
    \fn bool BOBUIransform::isInvertible() const
...
/*!
    \fn qreal BOBUIransform::m11() const

    Returns the horizontal scaling factor.

    \sa scale(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/

/*!
    \fn qreal BOBUIransform::m12() const

    Returns the vertical shearing factor.

    \sa shear(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/

/*!
    \fn qreal BOBUIransform::m21() const
...
/*!
    \fn qreal BOBUIransform::m22() const
...
/*!
    \fn qreal BOBUIransform::dx() const

    Returns the horizontal translation factor.

    \sa m31(), translate(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/

/*!
    \fn qreal BOBUIransform::dy() const

    Returns the vertical translation factor.

    \sa translate(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/


/*!
    \fn qreal BOBUIransform::m13() const
...
/*!
    \fn qreal BOBUIransform::m23() const
...
/*!
    \fn qreal BOBUIransform::m31() const

    Returns the horizontal translation factor.

    \sa dx(), translate(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/

/*!
    \fn qreal BOBUIransform::m32() const

    Returns the vertical translation factor.

    \sa dy(), translate(), {BOBUIransform#Basic Matrix Operations}{Basic Matrix
    Operations}
*/

/*!
    \fn qreal BOBUIransform::m33() const
...
/*!
    \fn qreal BOBUIransform::determinant() const

    Returns the matrix's determinant.
*/

/*!
    \fn bool BOBUIransform::isIdentity() const
...
/*!
    \fn bool BOBUIransform::isAffine() const
...
/*!
    \fn bool BOBUIransform::isScaling() const
...
/*!
    \fn bool BOBUIransform::isRotating() const
...
/*!
    \fn bool BOBUIransform::isTranslating() const
...
/*!
    \fn bool qFuzzyCompare(const BOBUIransform& t1, const BOBUIransform& t2)
...
Q_GUI_EXPORT
bool bobui_scaleForTransform(const BOBUIransform &transform, qreal *scale)
{
    const BOBUIransform::TransformationType type = transform.type();
    if (type <= BOBUIransform::TxTranslate) {
        if (scale)
            *scale = 1;
        return true;
    } else if (type == BOBUIransform::TxScale) {
    ...
    if (qAbs(xScale1 - yScale1) > qAbs(xScale2 - yScale2)) {
        ...
        return type == BOBUIransform::TxRotate && qFuzzyCompare(xScale1, yScale1);
    } else {
        if (scale)
            *scale = qSqrt(qMax(xScale2, yScale2));

        return type == BOBUIransform::TxRotate && qFuzzyCompare(xScale2, yScale2);
    }
}

QDataStream & operator>>(QDataStream &s, BOBUIransform::Affine &m)
{
    if (s.version() == 1) {
...
QDataStream &operator<<(QDataStream &s, const BOBUIransform::Affine &m)
{
    if (s.version() == 1) {
