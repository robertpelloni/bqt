...
{
    return BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(p, v1, v2) < 0;
}
...
//============================================================================//
template<typename T>
class BOBUIriangulator
{
public:
    typedef QVarLengthArray<int, 6> ShortArray;

    //================================//
    // BOBUIriangulator::ComplexToSimple //
    //================================//
    friend class ComplexToSimple;
    class ComplexToSimple
    {
    public:
        inline ComplexToSimple(BOBUIriangulator<T> *parent)
    ...
    //=================================//
    // BOBUIriangulator::SimpleToMonotone //
    //=================================//
    friend class SimpleToMonotone;
    class SimpleToMonotone
    {
    public:
        inline SimpleToMonotone(BOBUIriangulator<T> *parent)
    ...
    //====================================//
    // BOBUIriangulator::MonotoneToTriangles //
    //====================================//
    friend class MonotoneToTriangles;
    class MonotoneToTriangles
    {
    public:
        inline MonotoneToTriangles(BOBUIriangulator<T> *parent)
    ...
    private:
        ...
        BOBUIriangulator<T> *m_parent;
        int m_first;
        int m_length;
    };

    inline BOBUIriangulator()
        : m_vertices(0), m_hint(0) { }

    // Call this only once.
    void initialize(const qreal *polygon, int count, uint hint, const BOBUIransform &matrix);
    // Call this only once.
    void initialize(const QVectorPath &path, const BOBUIransform &matrix, qreal lod);
    // Call this only once.
    void initialize(const QPainterPath &path, const BOBUIransform &matrix, qreal lod);
...
template <typename T>
QVertexSet<T> BOBUIriangulator<T>::triangulate()
{
...
template <typename T>
QVertexSet<T> BOBUIriangulator<T>::polyline()
{
    for (int i = 0; i < m_vertices.size(); ++i) {
        Q_ASSERT(qAbs(m_vertices.at(i).x) < (1 << 21));
        Q_ASSERT(qAbs(m_vertices.at(i).y) < (1 << 21));
...
template <typename T>
void BOBUIriangulator<T>::initialize(const qreal *polygon, int count, uint hint, const BOBUIransform &matrix)
{
    m_hint = hint;
...
template <typename T>
void BOBUIriangulator<T>::initialize(const QVectorPath &path, const BOBUIransform &matrix, qreal lod)
{
    m_hint = path.hints();
    ...
    if (e) {
        for (int i = 0; i < path.elementCount(); ++i, ++e, p += 2) {
            ...
            case QPainterPath::CurveToElement:
                ...
                break;
            default:
                Q_ASSERT_X(0, "BOBUIriangulator::triangulate", "Unexpected element type.");
                break;
            }
        }
    } else {
...
template <typename T>
void BOBUIriangulator<T>::initialize(const QPainterPath &path, const BOBUIransform &matrix, qreal lod)
{
    initialize(bobuiVectorPathForPath(path), matrix, lod);
}
...
//============================================================================//
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::decompose()
{
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::initEdges()
{
...
template <typename T>
bool BOBUIriangulator<T>::ComplexToSimple::calculateIntersection(int left, int right)
{
    ...
    intersection.intersectionPoint = BOBUI_PREPEND_NAMESPACE(qIntersectionPoint)(u1, u2, v1, v2);
...
template <typename T>
bool BOBUIriangulator<T>::ComplexToSimple::edgeIsLeftOfEdge(int leftEdgeIndex, int rightEdgeIndex) const
{
    ...
    if (upper.x > qMax(l.x, u.x))
        return false;
    qint64 d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(upper, l, u);
    // d < 0: left, d > 0: right, d == 0: on top
    if (d == 0)
        d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(m_parent->m_vertices.at(leftEdge.lower()), l, u);
    return d < 0;
}

template <typename T>
QRBTree<int>::Node *BOBUIriangulator<T>::ComplexToSimple::searchEdgeLeftOf(int edgeIndex) const
{
...
template <typename T>
QRBTree<int>::Node *BOBUIriangulator<T>::ComplexToSimple::searchEdgeLeftOf(int edgeIndex, QRBTree<int>::Node *after) const
{
    if (!m_edgeList.root)
        return after;
...
template <typename T>
std::pair<QRBTree<int>::Node *, QRBTree<int>::Node *> BOBUIriangulator<T>::ComplexToSimple::bounds(const QPodPoint &point) const
{
    QRBTree<int>::Node *current = m_edgeList.root;
    ...
    while (current) {
        ...
        qint64 d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(point, v1, v2);
        if (d == 0) {
...
template <typename T>
std::pair<QRBTree<int>::Node *, QRBTree<int>::Node *> BOBUIriangulator<T>::ComplexToSimple::outerBounds(const QPodPoint &point) const
{
    QRBTree<int>::Node *current = m_edgeList.root;
    std::pair<QRBTree<int>::Node *, QRBTree<int>::Node *> result(nullptr, nullptr);
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::splitEdgeListRange(QRBTree<int>::Node *leftmost, QRBTree<int>::Node *rightmost, int vertex, const QIntersectionPoint &intersectionPoint)
{
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::reorderEdgeListRange(QRBTree<int>::Node *leftmost, QRBTree<int>::Node *rightmost)
{
    Q_ASSERT(leftmost && rightmost);
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::sortEdgeList(const QPodPoint eventPoint)
{
    QIntersectionPoint eventPoint2 = BOBUI_PREPEND_NAMESPACE(qIntersectionPoint)(eventPoint);
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::fillPriorityQueue()
{
    m_events.reset();
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::calculateIntersections()
{
    fillPriorityQueue();
    ...
    while (!m_events.isEmpty()) {
        ...
        QIntersectionPoint eventPoint = BOBUI_PREPEND_NAMESPACE(qIntersectionPoint)(event.point);
...
template <typename T>
int BOBUIriangulator<T>::ComplexToSimple::splitEdge(int splitIndex)
{
...
template <typename T>
bool BOBUIriangulator<T>::ComplexToSimple::splitEdgesAtIntersections()
{
    for (int i = 0; i < m_edges.size(); ++i)
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::insertEdgeIntoVectorIfWanted(ShortArray &orderedEdges, int i)
{
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::removeUnwantedEdgesAndConnect()
{
    Q_ASSERT(m_edgeList.root == nullptr);
    ...
    while (!m_events.isEmpty()) {
        ...
        if (m_edgeList.root) {
            ...
            while (current != b.second) {
                ...
                Q_ASSERT(BOBUI_PREPEND_NAMESPACE(qIntersectionPoint)(event.point).isOnLine(m_parent->m_vertices.at(m_edges.at(current->data).from), m_parent->m_vertices.at(m_edges.at(current->data).to)));
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::removeUnusedPoints() {
...
template <typename T>
inline bool BOBUIriangulator<T>::ComplexToSimple::Event::operator < (const Event &other) const
{
...
//============================================================================//
//                BOBUIriangulator::ComplexToSimple::DebugDialog                 //
//============================================================================//

#ifdef Q_TRIANGULATOR_DEBUG
template <typename T>
BOBUIriangulator<T>::ComplexToSimple::DebugDialog::DebugDialog(ComplexToSimple *parent, int currentVertex)
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::DebugDialog::paintEvent(QPaintEvent *)
{
    QPainter p(this);
    ...
    for (int i = 0; i < vertices.size(); ++i) {
        ...
        p.fillRect(QRectF(q.x - halfPointSize, q.y - halfPointSize, 2 * halfPointSize, 2 * halfPointSize), BobUI::red);
    }

    BobUI::GlobalColor colors[6] = {BobUI::red, BobUI::green, BobUI::blue, BobUI::cyan, BobUI::magenta, BobUI::yellow};
    p.setOpacity(0.5);
    int count = 0;
    ...
    p.fillRect(QRectF(q.x - halfPointSize, q.y - halfPointSize, 2 * halfPointSize, 2 * halfPointSize), BobUI::green);
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::DebugDialog::wheelEvent(QWheelEvent *event)
{
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::DebugDialog::mouseMoveEvent(QMouseEvent *event)
{
...
template <typename T>
void BOBUIriangulator<T>::ComplexToSimple::DebugDialog::mousePressEvent(QMouseEvent *event)
{
...
//============================================================================//
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::decompose()
{
    setupDataStructures();
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::setupDataStructures()
{
    int i = 0;
    Edge e;
    e.node = nullptr;
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::removeZeroLengthEdges()
{
    for (int i = 0; i < m_edges.size(); ++i) {
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::fillPriorityQueue()
{
    m_upperVertex.reset();
...
template <typename T>
bool BOBUIriangulator<T>::SimpleToMonotone::edgeIsLeftOfEdge(int leftEdgeIndex, int rightEdgeIndex) const
{
    const Edge &leftEdge = m_edges.at(leftEdgeIndex);
    const Edge &rightEdge = m_edges.at(rightEdgeIndex);
    const QPodPoint &u = m_parent->m_vertices.at(rightEdge.upper());
    const QPodPoint &l = m_parent->m_vertices.at(rightEdge.lower());
    qint64 d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(m_parent->m_vertices.at(leftEdge.upper()), l, u);
    // d < 0: left, d > 0: right, d == 0: on top
    if (d == 0)
        d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(m_parent->m_vertices.at(leftEdge.lower()), l, u);
    return d < 0;
...
// Returns the rightmost edge not to the right of the given edge.
template <typename T>
QRBTree<int>::Node *BOBUIriangulator<T>::SimpleToMonotone::searchEdgeLeftOfEdge(int edgeIndex) const
{
    QRBTree<int>::Node *current = m_edgeList.root;
    QRBTree<int>::Node *result = nullptr;
    while (current) {
...
// Returns the rightmost edge left of the given point.
template <typename T>
QRBTree<int>::Node *BOBUIriangulator<T>::SimpleToMonotone::searchEdgeLeftOfPoint(int pointIndex) const
{
    QRBTree<int>::Node *current = m_edgeList.root;
    QRBTree<int>::Node *result = nullptr;
    while (current) {
        ...
        qint64 d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(m_parent->m_vertices.at(pointIndex), p1, p2);
        if (d <= 0) {
            current = current->left;
        } else {
            result = current;
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::classifyVertex(int i)
{
    ...
    qint64 d = BOBUI_PREPEND_NAMESPACE(qPointDistanceFromLine)(p1, p2, p3);
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::classifyVertices()
{
    for (int i = 0; i < m_edges.size(); ++i)
        classifyVertex(i);
}
...
template <typename T>
bool BOBUIriangulator<T>::SimpleToMonotone::pointIsInSector(const QPodPoint &p, const QPodPoint &v1, const QPodPoint &v2, const QPodPoint &v3)
{
...
template <typename T>
bool BOBUIriangulator<T>::SimpleToMonotone::pointIsInSector(int vertex, int sector)
{
...
template <typename T>
int BOBUIriangulator<T>::SimpleToMonotone::findSector(int edge, int vertex)
{
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::createDiagonal(int lower, int upper)
{
...
template <typename T>
void BOBUIriangulator<T>::SimpleToMonotone::monotoneDecomposition()
{
    if (m_edges.isEmpty())
        return;
...
template <typename T>
bool BOBUIriangulator<T>::SimpleToMonotone::CompareVertices::operator () (int i, int j) const
{
...
//============================================================================//
template <typename T>
void BOBUIriangulator<T>::MonotoneToTriangles::decompose()
{
    QList<T> result;
...
Q_GUI_EXPORT BOBUIriangleSet qTriangulate(const qreal *polygon,
                                       int count, uint hint, const BOBUIransform &matrix,
                                       bool allowUintIndices)
{
    BOBUIriangleSet triangleSet;
    if (allowUintIndices) {
        BOBUIriangulator<quint32> triangulator;
        ...
        BOBUIriangulator<quint16> triangulator;
        triangulator.initialize(polygon, count, hint, matrix);
...
Q_GUI_EXPORT BOBUIriangleSet qTriangulate(const QVectorPath &path,
                                       const BOBUIransform &matrix, qreal lod, bool allowUintIndices)
{
    BOBUIriangleSet triangleSet;
...
BOBUIriangleSet qTriangulate(const QPainterPath &path,
                          const BOBUIransform &matrix, qreal lod, bool allowUintIndices)
{
    BOBUIriangleSet triangleSet;
    if (allowUintIndices) {
