...
#include "bobuiriangulatingstroker_p.h"
#include <qmath.h>
...
void BOBUIriangulatingStroker::endCapOrJoinClosed(const qreal *start, const qreal *cur,
...
void BOBUIriangulatingStroker::process(const QVectorPath &path, const QPen &pen, const QRectF &, QPainter::RenderHints)
{
    ...
    if (realWidth < 2.5 && (cosmetic || m_inv_scale == 1)) {
        if (m_cap_style == BobUI::RoundCap)
            m_cap_style = BobUI::SquareCap;
        if (m_join_style == BobUI::RoundJoin)
            m_join_style = BobUI::MiterJoin;
        m_curvyness_add = 0.5;
    ...
    BobUI::PenCapStyle cap = m_cap_style;
...
void BOBUIriangulatingStroker::moveTo(const qreal *pts)
{
    m_cx = pts[0];
    m_cy = pts[1];
...
void BOBUIriangulatingStroker::cubicTo(const qreal *pts)
{
...
void BOBUIriangulatingStroker::join(const qreal *pts)
{
...
void BOBUIriangulatingStroker::endCap(const qreal *)
{
    switch (m_cap_style) {
    case BobUI::FlatCap:
        break;
...
void BOBUIriangulatingStroker::arcPoints(float cx, float cy, float fromX, float fromY, float toX, float toY, QVarLengthArray<float> &points)
{
    float dx1 = fromX - cx;
    float dy1 = fromY - cy;
    float dx2 = toX - cx;
