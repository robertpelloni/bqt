...
void BOBUI_FASTCALL comp_func_SourceOver_sse2(uint *destPixels, const uint *srcPixels, int length, uint const_alpha)
{
...
void BOBUI_FASTCALL comp_func_Plus_sse2(uint *dst, const uint *src, int length, uint const_alpha)
{
    int x = 0;
...
void BOBUI_FASTCALL comp_func_Source_sse2(uint *dst, const uint *src, int length, uint const_alpha)
{
    if (const_alpha == 255) {
...
void bobui_memfill64_sse2(quint64 *dest, quint64 value, qsizetype count)
{
    ...
    if (misaligned && count) {
        ...
        // quint64 to be 8 -- see BobUIPrivate::AlignOf_WorkaroundForI386Abi in
        // qglobal.h.
        //
        // If the pointer is not aligned to at least 8 bytes, then we'll never
        // in turn hit a multiple of 16 for the bobui_memfillXX_aligned call
        // below.
        if (Q_UNLIKELY(misaligned % sizeof(quint64)))
            return bobui_memfill_template(dest, value, count);
#endif
    ...
    bobui_memfillXX_aligned(dest, _mm_set1_epi64x(value), count * sizeof(quint64));
}
...
void bobui_memfill32_sse2(quint32 *dest, quint32 value, qsizetype count)
{
    ...
    bobui_memfillXX_aligned(dest, _mm_set1_epi32(value), count * sizeof(quint32));
}
#endif // !__haswell__

void BOBUI_FASTCALL comp_func_solid_Source_sse2(uint *destPixels, int length, uint color, uint const_alpha)
{
    if (const_alpha == 255) {
...
void BOBUI_FASTCALL comp_func_solid_SourceOver_sse2(uint *destPixels, int length, uint color, uint const_alpha)
{
...
void bobui_bitmapblit32_sse2_base(QRasterBuffer *rasterBuffer, int x, int y,
                          quint32 color,
...
void bobui_bitmapblit32_sse2(QRasterBuffer *rasterBuffer, int x, int y,
                          ...
                          const uchar *src, int width, int height, int stride)
{
    bobui_bitmapblit32_sse2_base(rasterBuffer, x, y, color.toArgb32(), src, width, height, stride);
}
...
void bobui_bitmapblit8888_sse2(QRasterBuffer *rasterBuffer, int x, int y,
                            ...
                            const uchar *src, int width, int height, int stride)
{
    bobui_bitmapblit32_sse2_base(rasterBuffer, x, y, ARGB2RGBA(color.toArgb32()), src, width, height, stride);
}
...
BOBUI_WARNING_DISABLE_MSVC(4309) // truncation of constant value
...
const uint * BOBUI_FASTCALL bobui_fetch_radial_gradient_sse2(uint *buffer, const Operator *op, const QSpanData *data,
                                                       int y, int x, int length)
{
    return bobui_fetch_radial_gradient_template<QRadialFetchSimd<QSimdSse2>,uint>(buffer, op, data, y, x, length);
}

void bobui_scale_image_argb32_on_argb32_sse2(uchar *destPixels, int dbpl,
                                          ...
                                          int const_alpha)
{
    if (const_alpha != 256) {
        ...
        extern void bobui_scale_image_argb32_on_argb32(uchar *destPixels, int dbpl,
                                               ...
                                               const QRect &clip,
                                               int const_alpha);
        return bobui_scale_image_argb32_on_argb32(destPixels, dbpl, srcPixels, sbpl, srch, targetRect, sourceRect, clip, const_alpha);
    }
...
#endif // BOBUI_COMPILER_SUPPORTS_SSE2
