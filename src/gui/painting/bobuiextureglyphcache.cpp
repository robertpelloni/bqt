...
#include "bobuiextureglyphcache_p.h"
...
BOBUIextureGlyphCache::~BOBUIextureGlyphCache()
{
}

int BOBUIextureGlyphCache::calculateSubPixelPositionCount(glyph_t glyph) const
{
    ...
    for (int i = 0; i < NumSubpixelPositions; ++i) {
        ...
        if (numImages == 0) {
            QPainterPath path;
            QFixedPoint point;
            m_current_fontengine->addGlyphsToPath(&glyph, &point, 1, &path, BOBUIextItem::RenderFlags());
...
bool BOBUIextureGlyphCache::populate(QFontEngine *fontEngine,
                                  qsizetype numGlyphs,
                                  ...
                                  bool includeGlyphCacheScale)
{
    ...
    if (m_w == 0) {
        if (fontEngine->maxCharWidth() <= BOBUI_DEFAULT_TEXTURE_GLYPH_CACHE_WIDTH)
            m_w = BOBUI_DEFAULT_TEXTURE_GLYPH_CACHE_WIDTH;
        else
...
void BOBUIextureGlyphCache::fillInPendingGlyphs()
{
    if (!hasPendingGlyphs())
        return;
...
QImage BOBUIextureGlyphCache::textureMapForGlyph(glyph_t g, const QFixedPoint &subPixelPosition) const
{
    switch (m_format) {
                                          ...
                                          const QFixedPoint &subPixelPosition)
{
    ...
    if (m_format == QFontEngine::Format_A32
        ...
        if (mask.depth() > 1) {
            ...
            mask.invertPixels();
            mask.convertTo(QImage::Format_Mono, BobUI::ThresholdDither);
        }
