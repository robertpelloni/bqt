...
#define ObjectSelectionBrush (BOBUIextFormat::ForegroundBrush + 1)
#define SuppressText 0x5012
#define SuppressBackground 0x513

/*!
    \class BOBUIextLayout::FormatRange
    \reentrant

    \brief The BOBUIextLayout::FormatRange structure is used to apply extra formatting information
    for a specified area in the text layout's content.
    \inmodule BobUIGui

    \sa BOBUIextLayout::setFormats(), BOBUIextLayout::draw()
*/

/*!
    \variable BOBUIextLayout::FormatRange::start
    Specifies the beginning of the format range within the text layout's text.
*/

/*!
    \variable BOBUIextLayout::FormatRange::length
    Specifies the number of characters the format range spans.
*/

/*!
    \variable BOBUIextLayout::FormatRange::format
    Specifies the format to apply.
*/

/*! \fn bool BOBUIextLayout::FormatRange::operator==(const BOBUIextLayout::FormatRange &lhs, const BOBUIextLayout::FormatRange &rhs)
...
/*!
    \class BOBUIextInlineObject
    \reentrant

    \brief The BOBUIextInlineObject class represents an inline object in
    a QAbstractTextDocumentLayout and its implementations.
    \inmodule BobUIGui

    \ingroup richtext-processing

    Normally, you do not need to create a BOBUIextInlineObject. It is
...
/*!
    \fn BOBUIextInlineObject::BOBUIextInlineObject(int i, BOBUIextEngine *e)
    \internal
...
/*!
    \fn BOBUIextInlineObject::BOBUIextInlineObject()

    \internal
*/

/*!
    \fn bool BOBUIextInlineObject::isValid() const
...
*/
QRectF BOBUIextInlineObject::rect() const
{
...
*/
qreal BOBUIextInlineObject::width() const
{
...
*/
qreal BOBUIextInlineObject::ascent() const
{
...
*/
qreal BOBUIextInlineObject::descent() const
{
...
*/
qreal BOBUIextInlineObject::height() const
{
...
*/
void BOBUIextInlineObject::setWidth(qreal w)
{
...
*/
void BOBUIextInlineObject::setAscent(qreal a)
{
...
*/
void BOBUIextInlineObject::setDescent(qreal d)
{
...
*/
int BOBUIextInlineObject::textPosition() const
{
...
*/
int BOBUIextInlineObject::formatIndex() const
{
...
*/
BOBUIextFormat BOBUIextInlineObject::format() const
{
...
*/
BobUI::LayoutDirection BOBUIextInlineObject::textDirection() const
{
    return (eng->layoutData->items[itm].analysis.bidiLevel % 2 ? BobUI::RightToLeft : BobUI::LeftToRight);
}
...
/*!
    ...
    \brief The BOBUIextLayout class is used to lay out and render text.
    \inmodule BobUIGui
    ...
    BOBUIextLayout can be used with both plain and rich text.

    BOBUIextLayout can be used to create a sequence of BOBUIextLine
    ...
    The layout can be seen as a sequence of BOBUIextLine objects; use createLine()
    to create a BOBUIextLine instance, and lineAt() or lineForTextPosition() to retrieve
    created lines.

    Here is a code snippet that demonstrates the layout phase:
    \snippet code/src_gui_text_bobuiextlayout.cpp 0

    The text can then be rendered by calling the layout's draw() function:
    \snippet code/src_gui_text_bobuiextlayout.cpp 1

    It is also possible to draw each line individually, for instance to draw
    the last line that fits into a widget elided:
    \snippet code/src_gui_text_bobuiextlayout.cpp elided
    ...
    The BOBUIextLayout itself can be positioned with setPosition(); it has a
...
/*!
    \enum BOBUIextLayout::CursorMode
...
/*!
    \enum BOBUIextLayout::GlyphRunRetrievalFlag
    \since 6.5
    ...
    \sa glyphRuns(), BOBUIextLine::glyphRuns()
*/

/*!
    \fn BOBUIextEngine *BOBUIextLayout::engine() const
    \internal
...
/*!
    ...
    \sa setText()
*/
BOBUIextLayout::BOBUIextLayout()
{ d = new BOBUIextEngine(); }

/*!
    Constructs a text layout to lay out the given \a text.
*/
BOBUIextLayout::BOBUIextLayout(const QString& text)
{
    d = new BOBUIextEngine();
    d->text = text;
}
...
/*!
    \since 5.13
    \fn BOBUIextLayout::BOBUIextLayout(const QString &text, const QFont &font, const QPaintDevice *paintdevice)
...
BOBUIextLayout::BOBUIextLayout(const QString &text, const QFont &font, const QPaintDevice *paintdevice)
{
    const QFont f(paintdevice ? QFont(font, paintdevice) : font);
    d = new BOBUIextEngine((text.isNull() ? (const QString&)QString::fromLatin1("") : text), f);
}
...
*/
BOBUIextLayout::BOBUIextLayout(const BOBUIextBlock &block)
{
    d = new BOBUIextEngine();
    d->block = block;
}
...
/*!
    \internal
    Sets a raw font, to be used with BOBUIextLayout::glyphRuns.
    Note that this only supports the needs of WebKit.
    Use of this function with e.g. BOBUIextLayout::draw will result
    in undefined behaviour.
*/
void BOBUIextLayout::setRawFont(const QRawFont &rawFont)
{
    d->rawFont = rawFont;
    d->useRawFont = true;
...
/*!
    ...
    \sa font()
*/
void BOBUIextLayout::setFont(const QFont &font)
{
    d->fnt = font;
#ifndef BOBUI_NO_RAWFONT
    d->useRawFont = false;
...
/*!
    ...
    \sa setFont()
*/
QFont BOBUIextLayout::font() const
{
    return d->font();
}
...
/*!
    ...
    Notice that when using this BOBUIextLayout as part of a BOBUIextDocument this
    method will have no effect.

    \sa text()
*/
void BOBUIextLayout::setText(const QString& string)
{
    d->invalidate();
    d->clearLineData();
    d->text = string;
...
/*!
    ...
    \sa setText()
*/
QString BOBUIextLayout::text() const
{
    return d->text;
}
...
/*!
    ...
    \sa textOption()
*/
void BOBUIextLayout::setTextOption(const BOBUIextOption &option)
{
    d->option = option;
}
...
/*!
    ...
    \sa setTextOption()
*/
const BOBUIextOption &BOBUIextLayout::textOption() const
{
    return d->option;
}
...
*/
void BOBUIextLayout::setPreeditArea(int position, const QString &text)
{
    if (d->preeditAreaPosition() == position && d->preeditAreaText() == text)
        return;
    d->setPreeditArea(position, text);

    if (BOBUIextDocumentPrivate::get(d->block) != nullptr)
        BOBUIextDocumentPrivate::get(d->block)->documentChange(d->block.position(), d->block.length());
}
...
/*!
    ...
    \sa preeditAreaText()
*/
int BOBUIextLayout::preeditAreaPosition() const
{
...
*/
QString BOBUIextLayout::preeditAreaText() const
{
...
*/
void BOBUIextLayout::setFormats(const QList<FormatRange> &formats)
{
...
*/
QList<BOBUIextLayout::FormatRange> BOBUIextLayout::formats() const
{
    return d->formats();
}
...
*/
void BOBUIextLayout::clearFormats()
{
...
/*!
    ...
    BOBUIextLayout throws most of the layouting information away after a
    ...
    \sa cacheEnabled()
*/
void BOBUIextLayout::setCacheEnabled(bool enable)
{
...
/*!
    ...
    \sa setCacheEnabled()
*/
bool BOBUIextLayout::cacheEnabled() const
{
...
/*!
    ...
    BOBUIextLayout is backed by a document, you can ignore this and use the option
    in BOBUIextDocument, this option is for widgets like QLineEdit or custom
    widgets without a BOBUIextDocument. Default value is BobUI::LogicalMoveStyle.

    \sa cursorMoveStyle()
*/
void BOBUIextLayout::setCursorMoveStyle(BobUI::CursorMoveStyle style)
{
    d->visualMovement = style == BobUI::VisualMoveStyle;
}

/*!
    The cursor movement style of this BOBUIextLayout. The default is
    BobUI::LogicalMoveStyle.

    \sa setCursorMoveStyle()
*/
BobUI::CursorMoveStyle BOBUIextLayout::cursorMoveStyle() const
{
    return d->visualMovement ? BobUI::VisualMoveStyle : BobUI::LogicalMoveStyle;
}

/*!
    Begins the layout process.

    \warning This will invalidate the layout, so all existing BOBUIextLine objects
    that refer to the previous contents should now be discarded.

    \sa endLayout()
*/
void BOBUIextLayout::beginLayout()
{
#ifndef BOBUI_NO_DEBUG
    if (d->layoutData && d->layoutData->layoutState == BOBUIextEngine::InLayout) {
        qWarning("BOBUIextLayout::beginLayout: Called while already doing layout");
        return;
    }
#endif
    d->invalidate();
    d->clearLineData();
    d->itemize();
    d->layoutData->layoutState = BOBUIextEngine::InLayout;
}
...
/*!
    ...
    \sa beginLayout()
*/
void BOBUIextLayout::endLayout()
{
#ifndef BOBUI_NO_DEBUG
    if (!d->layoutData || d->layoutData->layoutState == BOBUIextEngine::LayoutEmpty) {
        qWarning("BOBUIextLayout::endLayout: Called without beginLayout()");
        return;
    }
#endif
    int l = d->lines.size();
    if (l && d->lines.at(l-1).length < 0) {
        BOBUIextLine(l-1, d).setNumColumns(INT_MAX);
    }
    d->layoutData->layoutState = BOBUIextEngine::LayoutEmpty;
    if (!d->cacheGlyphs)
        d->freeMemory();
}
...
/*!
    ...
    that refer to the previous contents should now be discarded.
*/
void BOBUIextLayout::clearLayout()
{
    d->clearLineData();
}
...
*/
int BOBUIextLayout::nextCursorPosition(int oldPos, CursorMode mode) const
{
...
*/
int BOBUIextLayout::previousCursorPosition(int oldPos, CursorMode mode) const
{
    const QCharAttributes *attributes = d->attributes();
    int len = d->block.isValid() ? d->block.length() - 1
                                 : d->layoutData->string.size();
...
*/
int BOBUIextLayout::rightCursorPosition(int oldPos) const
{
    int newPos = d->positionAfterVisualMovement(oldPos, BOBUIextCursor::Right);
...
*/
int BOBUIextLayout::leftCursorPosition(int oldPos) const
{
    int newPos = d->positionAfterVisualMovement(oldPos, BOBUIextCursor::Left);
//    qDebug("%d -> %d", oldPos, newPos);
    return newPos;
}
...
*/
bool BOBUIextLayout::isValidCursorPosition(int pos) const
{
    const QCharAttributes *attributes = d->attributes();
...
/*!
    ...
    BOBUIextLine::setLineWidth() function is called.

    Once BOBUIextLine::setLineWidth() is called, a new line can be created and
    filled with text. Repeating this process will lay out the whole block
    of text contained in the BOBUIextLayout. If there is no text left to be
    inserted into the layout, the BOBUIextLine returned will not be valid
    (isValid() will return false).
*/
BOBUIextLine BOBUIextLayout::createLine()
{
#ifndef BOBUI_NO_DEBUG
    if (!d->layoutData || d->layoutData->layoutState == BOBUIextEngine::LayoutEmpty) {
        qWarning("BOBUIextLayout::createLine: Called without layouting");
        return BOBUIextLine();
    }
#endif
    if (d->layoutData->layoutState == BOBUIextEngine::LayoutFailed)
        return BOBUIextLine();
    ...
    if (l && d->lines.at(l-1).length < 0) {
        ...
        if (d->maxWidth > QFIXED_MAX / 2) {
            qWarning("BOBUIextLayout: text too long, truncated.");
            return BOBUIextLine();
        }
    }
...
/*!
    ...
    \sa lineAt()
*/
int BOBUIextLayout::lineCount() const
{
    return d->lines.size();
}
...
*/
BOBUIextLine BOBUIextLayout::lineAt(int i) const
{
    return i < lineCount() ? BOBUIextLine(i, d) : BOBUIextLine();
}
...
*/
BOBUIextLine BOBUIextLayout::lineForTextPosition(int pos) const
{
    int lineNum = d->lineNumberForTextPosition(pos);
    return lineNum >= 0 ? lineAt(lineNum) : BOBUIextLine();
}
...
/*!
    ...
    \sa setPosition()
*/
QPointF BOBUIextLayout::position() const
{
    return d->position;
}
...
/*!
    ...
    \sa position()
*/
void BOBUIextLayout::setPosition(const QPointF &p)
{
    d->position = p;
}
...
*/
QRectF BOBUIextLayout::boundingRect() const
{
    if (d->lines.isEmpty())
        return QRectF();
...
/*!
    ...
    \sa maximumWidth()
*/
qreal BOBUIextLayout::minimumWidth() const
{
...
/*!
    ...
    \sa minimumWidth()
*/
qreal BOBUIextLayout::maximumWidth() const
{
...
/*!
    \internal
*/
void BOBUIextLayout::setFlags(int flags)
{
    if (flags & BobUI::TextJustificationForced) {
        d->option.setAlignment(BobUI::AlignJustify);
        d->forceJustification = true;
    }

    if (flags & (BobUI::TextForceLeftToRight|BobUI::TextForceRightToLeft)) {
        d->ignoreBidi = true;
        d->option.setTextDirection((flags & BobUI::TextForceLeftToRight) ? BobUI::LeftToRight : BobUI::RightToLeft);
    }
}

static void addSelectedRegionsToPath(BOBUIextEngine *eng, int lineNumber, const QPointF &pos, BOBUIextLayout::FormatRange *selection,
                                     QPainterPath *region, const QRectF &boundingRect)
{
    const QScriptLine &line = eng->lines[lineNumber];

    BOBUIextLineItemIterator iterator(eng, lineNumber, pos, selection);
...
/*!
    ...
    starting at the position \a from in this BOBUIextLayout. This is an expensive function, and should
    ...
    glyphRuns(from,
              length,
              BOBUIextLayout::GlyphRunRetrievalFlag::GlyphIndexes |
                BOBUIextLayout::GlyphRunRetrievalFlag::GlyphPositions).
...
QList<QGlyphRun> BOBUIextLayout::glyphRuns(int from, int length) const
{
    return glyphRuns(from, length, BOBUIextLayout::GlyphRunRetrievalFlag::DefaultRetrievalFlags);
}
#  endif
#endif
...
*/
#if !defined(BOBUI_NO_RAWFONT)
QList<QGlyphRun> BOBUIextLayout::glyphRuns(int from,
                                        int length,
                                        BOBUIextLayout::GlyphRunRetrievalFlags retrievalFlags) const
{
    if (from < 0)
        from = 0;
    if (length < 0)
    ...
    for (int i=0; i<d->lines.size(); ++i) {
        if (d->lines.at(i).from > from + length)
            ...
            const QList<QGlyphRun> glyphRuns = BOBUIextLine(i, d).glyphRuns(from, length, retrievalFlags);
...
*/
void BOBUIextLayout::draw(QPainter *p, const QPointF &pos, const QList<FormatRange> &selections, const QRectF &clip) const
{
    if (d->lines.isEmpty())
        return;
    ...
    for (int i = 0; i < selections.size(); ++i) {
        ...
        QPainterPath region;
        region.setFillRule(BobUI::WindingFill);

        for (int line = firstLine; line < lastLine; ++line) {
            const QScriptLine &sl = d->lines.at(line);
            BOBUIextLine tl(line, d);
            ...
            if (selection.format.boolProperty(BOBUIextFormat::FullWidthSelection)) {
                ...
                && isLastLineInBlock
                &&!(d->option.flags() & BOBUIextOption::ShowLineAndParagraphSeparators)) {
        ...
        }
        {
            ...
            p->setPen(selection.format.penProperty(BOBUIextFormat::OutlinePen));
            p->setBrush(selection.format.brushProperty(BOBUIextFormat::BackgroundBrush));
            p->drawPath(region);
        ...
        bool hasText = (selection.format.foreground().style() != BobUI::NoBrush);
        bool hasBackground= (selection.format.background().style() != BobUI::NoBrush);

        if (hasBackground) {
            selection.format.setProperty(ObjectSelectionBrush, selection.format.property(BOBUIextFormat::BackgroundBrush));
            // don't just clear the property, set an empty brush that overrides a potential
            // background brush specified in the text
            selection.format.setProperty(BOBUIextFormat::BackgroundBrush, QBrush());
            selection.format.clearProperty(BOBUIextFormat::OutlinePen);
        }
        ...
        p->save();
        p->setClipPath(region, BobUI::IntersectClip);

        for (int line = firstLine; line < lastLine; ++line) {
            BOBUIextLine l(line, d);
    ...
    if (!needsTextButNoBackground.isEmpty()){
        p->save();
        p->setClipPath(needsTextButNoBackground, BobUI::IntersectClip);
...
/*!
    \fn void BOBUIextLayout::drawCursor(QPainter *painter, const QPointF &position, int cursorPosition) const
    \overload
...
*/
void BOBUIextLayout::drawCursor(QPainter *p, const QPointF &pos, int cursorPosition) const
{
    drawCursor(p, pos, cursorPosition, 1);
}

/*!
    \fn void BOBUIextLayout::drawCursor(QPainter *painter, const QPointF &position, int cursorPosition, int width) const

    Draws a text cursor with the current pen and the specified \a width at the given \a position using the
    \a painter specified.
    The corresponding position within the text is specified by \a cursorPosition.
*/
void BOBUIextLayout::drawCursor(QPainter *p, const QPointF &pos, int cursorPosition, int width) const
{
    if (d->lines.isEmpty())
        return;
    ...
    bool toggleAntialiasing = !(p->renderHints() & QPainter::Antialiasing)
                              && (p->transform().type() > BOBUIransform::TxTranslate);
    ...
    const BOBUIransform &deviceTransform = p->deviceTransform();
    const qreal xScale = deviceTransform.m11();
    if (deviceTransform.type() != BOBUIransform::TxScale || std::trunc(xScale) == xScale) {
        ...
        QPen pen(origPen.brush(), qRound(width * xScale), BobUI::SolidLine, BobUI::FlatCap);
...
/*!
    ...
    \brief The BOBUIextLine class represents a line of text inside a BOBUIextLayout.
    \inmodule BobUIGui

    \ingroup richtext-processing

    A text line is usually created by BOBUIextLayout::createLine().
...
/*!
    \enum BOBUIextLine::CursorPosition
...
/*!
    \fn BOBUIextLine::BOBUIextLine(int line, BOBUIextEngine *e)
    \internal
...
/*!
    \fn BOBUIextLine::BOBUIextLine()

    Creates an invalid line.
*/

/*!
    \fn bool BOBUIextLine::isValid() const

    Returns \c true if this text line is valid; otherwise returns \c false.
*/

/*!
    \fn int BOBUIextLine::lineNumber() const
...
*/
QRectF BOBUIextLine::rect() const
{
...
*/
QRectF BOBUIextLine::naturalTextRect() const
{
    const QScriptLine& sl = eng->lines.at(index);
...
*/
qreal BOBUIextLine::width() const
{
...
/*!
    ...
    \sa descent(), height()
*/
qreal BOBUIextLine::ascent() const
{
...
/*!
    ...
    \sa ascent(), height()
*/
qreal BOBUIextLine::descent() const
{
...
*/
qreal BOBUIextLine::height() const
{
...
*/
qreal BOBUIextLine::leading() const
{
...
*/
void BOBUIextLine::setLeadingIncluded(bool included)
{
...
*/
bool BOBUIextLine::leadingIncluded() const
{
...
*/
qreal BOBUIextLine::naturalTextWidth() const
{
...
*/
qreal BOBUIextLine::horizontalAdvance() const
{
...
/*!
    ...
    or end of the text.
*/
void BOBUIextLine::setLineWidth(qreal width)
{
    QScriptLine &line = eng->lines[index];
    if (!eng->layoutData) {
        qWarning("BOBUIextLine: Can't set a line width while not layouting.");
        return;
    }
...
*/
void BOBUIextLine::setNumColumns(int numColumns)
{
    QScriptLine &line = eng->lines[index];
...
/*!
    ...
    width for alignment.
*/
void BOBUIextLine::setNumColumns(int numColumns, qreal alignmentWidth)
{
    QScriptLine &line = eng->lines[index];
...
// fill QScriptLine
void BOBUIextLine::layout_helper(int maxGlyphs)
{
    QScriptLine &line = eng->lines[index];
    line.length = 0;
    ...
    BOBUIextOption::WrapMode wrapMode = eng->option.wrapMode();
    bool breakany = (wrapMode == BOBUIextOption::WrapAnywhere);
    const bool breakWordOrAny = breakany || (wrapMode == BOBUIextOption::WrapAtWordBoundaryOrAnywhere);
    lbh.manualWrap = (wrapMode == BOBUIextOption::ManualWrap || wrapMode == BOBUIextOption::NoWrap);
    ...
    BobUI::Alignment alignment = eng->option.alignment();
    ...
    const bool includeTrailingSpaces = eng->option.flags() & BOBUIextOption::IncludeTrailingSpaces;
    ...
    while (newItem < eng->layoutData->items.size()) {
        ...
        if (current.analysis.flags == QScriptAnalysis::Tab && (alignment & (BobUI::AlignLeft | BobUI::AlignRight | BobUI::AlignCenter | BobUI::AlignJustify))) {
            ...
            if (!line.length && !lbh.tmpData.length)
                line.setDefaultHeight(eng);
            if (eng->option.flags() & BOBUIextOption::ShowLineAndParagraphSeparators) {
                if (lbh.checkFullOtherwiseExtend(line))
                    goto found;
            ...
            if (BOBUIextDocumentPrivate::get(eng->block) != nullptr) {
                BOBUIextInlineObject inlineObject(item, eng);
                BOBUIextFormat f = inlineObject.format();
                eng->docLayout()->positionInlineObject(inlineObject, eng->block.position() + current.position, f);
                BOBUIextCharFormat::VerticalAlignment valign = f.toCharFormat().verticalAlignment();
                if (valign != BOBUIextCharFormat::AlignTop && valign != BOBUIextCharFormat::AlignBottom) {
                    lbh.tmpData.ascent = qMax(lbh.tmpData.ascent, current.ascent);
                    lbh.tmpData.descent = qMax(lbh.tmpData.descent, current.descent);
                }
            }
            ...
            do {
                ...
                // spaces to behave as in previous BobUI versions in the line breaking algorithm.
...
found:
    ...
    if (hasInlineObject && BOBUIextDocumentPrivate::get(eng->block) != nullptr) {
        ...
        for (int item = startItem; item < endItem; ++item) {
            ...
            if (current.analysis.flags == QScriptAnalysis::Object) {
                BOBUIextInlineObject inlineObject(item, eng);
                BOBUIextCharFormat::VerticalAlignment align = inlineObject.format().toCharFormat().verticalAlignment();
    ...
    if (eng->option.wrapMode() == BOBUIextOption::WrapAtWordBoundaryOrAnywhere) {
        if ((lbh.maxGlyphs != INT_MAX && lbh.glyphCount > lbh.maxGlyphs)
            || (lbh.maxGlyphs == INT_MAX && line.textWidth > (line.width -  trailingSpace))) {

            eng->option.setWrapMode(BOBUIextOption::WrapAnywhere);
            layout_helper(lbh.maxGlyphs);
            eng->option.setWrapMode(BOBUIextOption::WrapAtWordBoundaryOrAnywhere);
            return;
        }
    }
...
*/
void BOBUIextLine::setPosition(const QPointF &pos)
{
...
*/
QPointF BOBUIextLine::position() const
{
...
// it out, it contains a number of BOBUIextLines. from() returns the position
...
/*!
    ...
    passed to the BOBUIextLayout.
*/
int BOBUIextLine::textStart() const
{
...
/*!
    ...
    \sa naturalTextWidth()
*/
int BOBUIextLine::textLength() const
{
    if (eng->option.flags() & BOBUIextOption::ShowLineAndParagraphSeparators
...
static void drawBackground(QPainter *p, const BOBUIextCharFormat &chf, const QRectF &r)
{
    QBrush bg = chf.background();
    if (bg.style() != BobUI::NoBrush && !chf.property(SuppressBackground).toBool())
        p->fillRect(r.toAlignedRect(), bg);
}

static void setPen(QPainter *p, const QPen &defaultPen, const BOBUIextCharFormat &chf)
{
    QBrush c = chf.foreground();
    if (c.style() == BobUI::NoBrush)
        p->setPen(defaultPen);
    else
        p->setPen(QPen(c, 0));
}
                                  ...
                                  int textLength)
{
    ...
    BOBUIextItem::RenderFlags renderFlags;
    if (flags.testFlag(QGlyphRun::Overline))
        renderFlags |= BOBUIextItem::Overline;
    if (flags.testFlag(QGlyphRun::Underline))
        renderFlags |= BOBUIextItem::Underline;
    if (flags.testFlag(QGlyphRun::StrikeOut))
        renderFlags |= BOBUIextItem::StrikeOut;
    if (flags.testFlag(QGlyphRun::RightToLeft))
        renderFlags |= BOBUIextItem::RightToLeft;

    fontEngine->getGlyphPositions(glyphLayout, BOBUIransform(), renderFlags, glyphsArray,
                                  positionsArray);
    ...
    if (retrievalFlags & BOBUIextLayout::RetrieveGlyphIndexes)
        glyphs.reserve(glyphsArray.size());
    QList<QPointF> positions;
    if (retrievalFlags & BOBUIextLayout::RetrieveGlyphPositions)
        positions.reserve(glyphsArray.size());
    QList<qsizetype> stringIndexes;
    if (retrievalFlags & BOBUIextLayout::RetrieveStringIndexes)
    ...
    if (retrievalFlags & BOBUIextLayout::RetrieveString)
...
/*!
    \overload
    Returns the glyph indexes and positions for all glyphs in this BOBUIextLine for characters
    in the range defined by \a from and \a length. The \a from index is relative to the beginning
    of the text in the containing BOBUIextLayout, and the range must be within the range of BOBUIextLine
    ...
    \sa BOBUIextLayout::glyphRuns()
*/
QList<QGlyphRun> BOBUIextLine::glyphRuns(int from, int length) const
{
    return glyphRuns(from, length, BOBUIextLayout::GlyphRunRetrievalFlag::DefaultRetrievalFlags);
}
#  endif
...
/*!
    ...
    \sa BOBUIextLayout::glyphRuns()
*/
QList<QGlyphRun> BOBUIextLine::glyphRuns(int from,
                                      int length,
                                      BOBUIextLayout::GlyphRunRetrievalFlags retrievalFlags) const
{
    ...
    BOBUIextLayout::FormatRange selection;
    selection.start = from;
    selection.length = length;

    BOBUIextLineItemIterator iterator(eng, index, QPointF(), &selection);
...
/*!
    \fn void BOBUIextLine::draw(QPainter *painter, const QPointF &position) const

    Draws a line on the given \a painter at the specified \a position.
*/
void BOBUIextLine::draw(QPainter *painter, const QPointF &position) const
{
    draw_internal(painter, position, nullptr);
}

void BOBUIextLine::draw_internal(QPainter *p, const QPointF &origPos,
                              const BOBUIextLayout::FormatRange *selection) const
{
    ...
    const BOBUIextFormatCollection *formatCollection = eng->formatCollection();

    bool suppressColors = (eng->option.flags() & BOBUIextOption::SuppressColors);

    auto prepareFormat = [suppressColors, selection, this](BOBUIextCharFormat &format,
                                                           QScriptItem *si) {
        ...
        if (suppressColors) {
            ...
            format.clearProperty(BOBUIextFormat::TextUnderlineColor);
        }
        if (selection)
    ...
    {
        BOBUIextLineItemIterator iterator(eng, index, pos, selection);
        while (!iterator.atEnd()) {
            QScriptItem &si = iterator.next();
    ...
    QPen pen = p->pen();
    {
        ...
        while (!iterator.atEnd()) {
            ...
            if (si.analysis.flags == QScriptAnalysis::LineOrParagraphSeparator
                && !(eng->option.flags() & BOBUIextOption::ShowLineAndParagraphSeparators))
                continue;
            ...
            if (eng->hasFormats() || selection || formatCollection) {
                ...
                if (valign == BOBUIextCharFormat::AlignSuperScript
                    || valign == BOBUIextCharFormat::AlignSubScript
            ...
            if (si.analysis.flags >= QScriptAnalysis::TabOrObject) {
                ...
                if (eng->hasFormats()) {
                    p->save();
                    if (si.analysis.flags == QScriptAnalysis::Object && BOBUIextDocumentPrivate::get(eng->block)) {
                        ...
                        eng->docLayout()->drawInlineObject(p, itemRect,
                                                           BOBUIextInlineObject(iterator.item, eng),
                        ...
                        QFont f = eng->font(si);
                        BOBUIextItemInt gf(si, &f, format);
                        gf.chars = nullptr;
                        gf.num_chars = 0;
                        gf.width = iterator.itemWidth;
                        QPainterPrivate::get(p)->drawTextItem(QPointF(iterator.x.toReal(), y.toReal()), gf, eng);
                        if (eng->option.flags() & BOBUIextOption::ShowTabsAndSpaces) {
            ...
            BOBUIextItemInt gf(glyphs.mid(iterator.glyphsStart, iterator.glyphsEnd - iterator.glyphsStart),
            ...
            if (format.penProperty(BOBUIextFormat::TextOutline).style() != BobUI::NoPen) {
                QPainterPath path;
                ...
                p->save();
                p->setRenderHint(QPainter::Antialiasing);
                //Currently QPen with a BobUI::NoPen style still returns a default
                //QBrush which != BobUI::NoBrush so we need this specialcase to reset it
                if (p->pen().style() == BobUI::NoPen)
            ...
            if ((si.analysis.flags == QScriptAnalysis::Space
                ...
                && (eng->option.flags() & BOBUIextOption::ShowTabsAndSpaces)) {
...
/*!
    \fn int BOBUIextLine::cursorToX(int cursorPos, Edge edge) const
...
/*!
    ...
    \sa xToCursor()
*/
qreal BOBUIextLine::cursorToX(int *cursorPos, Edge edge) const
{
    const QScriptLine &line = eng->lines[index];
    ...
    if (eng->option.wrapMode() != BOBUIextOption::NoWrap && x > line.x + line.width)
        x = line.x + line.width;
    if (eng->option.wrapMode() != BOBUIextOption::NoWrap && x < 0)
        x = 0;
...
/*!
    \fn int BOBUIextLine::xToCursor(qreal x, CursorPosition cpos) const
    ...
    \sa cursorToX()
*/
int BOBUIextLine::xToCursor(qreal _x, CursorPosition cpos) const
{
...
//             qDebug("      inside run");
            if (si.analysis.flags >= QScriptAnalysis::TabOrObject) {
                if (cpos == BOBUIextLine::CursorOnCharacter)
                    return si.position;
            ...
            if (cpos == BOBUIextLine::CursorOnCharacter) {
                ...
                if (qAbs(x-pos) < dist) {
                    ...
                    return eng->positionInLigature(&si, end, x, pos, -1,
                                                   cpos == BOBUIextLine::CursorOnCharacter);
                }
            }
