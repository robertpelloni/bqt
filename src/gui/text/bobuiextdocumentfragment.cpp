...
BOBUIextCopyHelper::BOBUIextCopyHelper(const BOBUIextCursor &_source, const BOBUIextCursor &_destination, bool forceCharFormat, const BOBUIextCharFormat &fmt)
...
int BOBUIextCopyHelper::convertFormatIndex(const BOBUIextFormat &oldFormat, int objectIndexToSet)
{
    BOBUIextFormat fmt = oldFormat;
    if (objectIndexToSet != -1) {
        ...
        if (newObjectIndex == -1) {
            BOBUIextFormat objFormat = src->formatCollection()->objectFormat(fmt.objectIndex());
...
int BOBUIextCopyHelper::appendFragment(int pos, int endPos, int objectIndex)
{
    BOBUIextDocumentPrivate::FragmentIterator fragIt = src->find(pos);
    ...
    BOBUIextBlock nextBlock = src->blocksFind(pos + 1);
    ...
    if (txtToInsert.size() == 1
        && (txtToInsert.at(0) == QChar::ParagraphSeparator
            || txtToInsert.at(0) == BOBUIextBeginningOfFrame
            || txtToInsert.at(0) == BOBUIextEndOfFrame
           )
       ) {
        ...
        if (nextBlock.textList()) {
            BOBUIextBlock dstBlock = dst->blocksFind(insertPos);
...
void BOBUIextCopyHelper::appendFragments(int pos, int endPos)
{
    Q_ASSERT(pos < endPos);
...
void BOBUIextCopyHelper::copy()
{
    if (cursor.hasComplexSelection()) {
        ...
        BOBUIextTableFormat tableFormat = table->format();
        ...
        for (int r = row_start; r < row_start + num_rows; ++r) {
            for (int c = col_start; c < col_start + num_cols; ++c) {
                ...
                // add the BOBUIextBeginningOfFrame
                ...
                BOBUIextBlock block = src->blocksFind(cellPos);
                if (block.position() == cellPos) {
                    blockIdx = convertFormatIndex(block.blockFormat());
                }

                dst->insertBlock(BOBUIextBeginningOfFrame, insertPos, blockIdx, charFormatIndex);
                ++insertPos;
...
BOBUIextDocumentFragmentPrivate::BOBUIextDocumentFragmentPrivate(const BOBUIextCursor &_cursor)
    : ref(1), doc(new BOBUIextDocument), importedFromPlainText(false)
{
    ...
    BOBUIextDocumentPrivate *p = BOBUIextDocumentPrivate::get(doc);
    p->beginEditBlock();
    BOBUIextCursor destCursor(doc);
    BOBUIextCopyHelper(_cursor, destCursor).copy();
    p->endEditBlock();
...
void BOBUIextDocumentFragmentPrivate::insert(BOBUIextCursor &_cursor) const
{
    if (_cursor.isNull())
        return;

    BOBUIextDocumentPrivate *destPieceTable = _cursor.d->priv;
    destPieceTable->beginEditBlock();

    BOBUIextCursor sourceCursor(doc);
    sourceCursor.movePosition(BOBUIextCursor::End, BOBUIextCursor::KeepAnchor);
    BOBUIextCopyHelper(sourceCursor, _cursor, importedFromPlainText, _cursor.charFormat()).copy();
...
/*!
    ...
    \inmodule BobUIGui
    \brief The BOBUIextDocumentFragment class represents a piece of formatted text
    from a BOBUIextDocument.

    \ingroup richtext-processing
    \ingroup shared

    A BOBUIextDocumentFragment is a fragment of rich text, that can be inserted into
    a BOBUIextDocument. A document fragment can be created from a
    BOBUIextDocument, from a BOBUIextCursor's selection, or from another
...
/*!
    Constructs an empty BOBUIextDocumentFragment.

    \sa isEmpty()
*/
BOBUIextDocumentFragment::BOBUIextDocumentFragment()
    : d(nullptr)
{
}

/*!
    Converts the given \a document into a BOBUIextDocumentFragment.
    Note that the BOBUIextDocumentFragment only stores the document contents, not meta information
    like the document's title.
*/
BOBUIextDocumentFragment::BOBUIextDocumentFragment(const BOBUIextDocument *document)
    : d(nullptr)
{
    if (!document)
        return;

    BOBUIextCursor cursor(const_cast<BOBUIextDocument *>(document));
    cursor.movePosition(BOBUIextCursor::End, BOBUIextCursor::KeepAnchor);
    d = new BOBUIextDocumentFragmentPrivate(cursor);
}

/*!
    Creates a BOBUIextDocumentFragment from the \a{cursor}'s selection.
    If the cursor doesn't have a selection, the created fragment is empty.

    \sa isEmpty(), BOBUIextCursor::selection()
*/
BOBUIextDocumentFragment::BOBUIextDocumentFragment(const BOBUIextCursor &cursor)
    : d(nullptr)
{
...
/*!
    \fn BOBUIextDocumentFragment::BOBUIextDocumentFragment(const BOBUIextDocumentFragment &other)

    Copy constructor. Creates a copy of the \a other fragment.
*/
BOBUIextDocumentFragment::BOBUIextDocumentFragment(const BOBUIextDocumentFragment &rhs)
    : d(rhs.d)
{
    if (d)
        d->ref.ref();
...
/*!
    \fn BOBUIextDocumentFragment &BOBUIextDocumentFragment::operator=(const BOBUIextDocumentFragment &other)

    Assigns the \a other fragment to this fragment.
*/
BOBUIextDocumentFragment &BOBUIextDocumentFragment::operator=(const BOBUIextDocumentFragment &rhs)
{
    if (rhs.d)
        rhs.d->ref.ref();
    if (d && !d->ref.deref())
...
*/
bool BOBUIextDocumentFragment::isEmpty() const
{
    return d == nullptr || d->doc == nullptr || BOBUIextDocumentPrivate::get(d->doc)->length() <= 1;
}
...
*/
QString BOBUIextDocumentFragment::toPlainText() const
{
    if (!d)
        return QString();
...
*/
QString BOBUIextDocumentFragment::toRawText() const
{
    if (!d)
        return QString();
...
/*!
    ...
    \sa toPlainText(), toMarkdown(), BOBUIextDocument::toHtml()
*/
QString BOBUIextDocumentFragment::toHtml() const
{
    if (!d)
        return QString();

    return BOBUIextHtmlExporter(d->doc).toHtml(BOBUIextHtmlExporter::ExportFragment);
}
...
/*!
    ...
    \sa toPlainText(), BOBUIextDocument::toMarkdown()
*/
QString BOBUIextDocumentFragment::toMarkdown(BOBUIextDocument::MarkdownFeatures features) const
{
    if (!d)
        return QString();
...
/*!
    ...
    When inserting such a fragment into a BOBUIextDocument the current char format of
    the BOBUIextCursor used for insertion is used as format for the text.
*/
BOBUIextDocumentFragment BOBUIextDocumentFragment::fromPlainText(const QString &plainText)
{
    BOBUIextDocumentFragment res;

    res.d = new BOBUIextDocumentFragmentPrivate;
    res.d->importedFromPlainText = true;
    BOBUIextCursor cursor(res.d->doc);
...
static BOBUIextListFormat::Style nextListStyle(BOBUIextListFormat::Style style)
{
    if (style == BOBUIextListFormat::ListDisc)
        return BOBUIextListFormat::ListCircle;
    else if (style == BOBUIextListFormat::ListCircle)
        return BOBUIextListFormat::ListSquare;
    return style;
}

BOBUIextHtmlImporter::BOBUIextHtmlImporter(BOBUIextDocument *_doc, const QString &_html, ImportMode mode, const BOBUIextDocument *resourceProvider)
    : indent(0), headingLevel(0), compressNextWhitespace(PreserveWhiteSpace), doc(_doc), importMode(mode)
{
    cursor = BOBUIextCursor(doc);
    wsm = BOBUIextHtmlParserNode::WhiteSpaceNormal;

    QString html = _html;
    const int startFragmentPos = html.indexOf("<!--StartFragment-->"_L1);
    if (startFragmentPos != -1) {
        const auto bobui3RichTextHeader = "<meta name=\"qrichtext\" content=\"1\" />"_L1;

        // Hack for BobUI3
        const bool hasBobUIRichtextMetaTag = html.contains(bobui3RichTextHeader);
        ...
        if (hasBobUIRichtextMetaTag)
            html.prepend(bobui3RichTextHeader);
    }
...
void BOBUIextHtmlImporter::import()
{
    ...
    for (currentNodeIdx = 0; currentNodeIdx < count(); ++currentNodeIdx) {
        ...
        wsm = textEditMode ? BOBUIextHtmlParserNode::WhiteSpacePreWrap : currentNode->wsm;
         ...
         *
         * 3) if the node should result in a BOBUIextBlock create one and
        ...
        if (currentNodeIdx > 0 && (currentNode->parent != currentNodeIdx - 1)) {
                ...
                && currentNode->id != Html_unknown)
            {
                ...
                BOBUIextBlockFormat blockFormat = currentNode->blockFormat;
                blockFormat.setIndent(indent);

                BOBUIextBlockFormat oldFormat = cursor.blockFormat();
                if (oldFormat.hasProperty(BOBUIextFormat::PageBreakPolicy)) {
                    BOBUIextFormat::PageBreakFlags pageBreak = oldFormat.pageBreakPolicy();
                    if (pageBreak == BOBUIextFormat::PageBreak_AlwaysAfter)
                        ...
                        */
                        pageBreak = BOBUIextFormat::PageBreak_AlwaysBefore;
        ...
        if (currentNode->displayMode == BOBUIextHtmlElement::DisplayNone) {
            if (currentNode->id == Html_title)
                doc->setMetaInformation(BOBUIextDocument::DocumentTitle, currentNode->text);
        ...
        if (blockTagClosed
            ...
            && currentNode->displayMode == BOBUIextHtmlElement::DisplayInline) {

            BOBUIextBlockFormat block = currentNode->blockFormat;
        ...
        if (currentNode->isBlock()) {
            BOBUIextHtmlImporter::ProcessNodeResult result = processBlockNode();
...
bool BOBUIextHtmlImporter::appendNodeText()
{
    const int initialCursorPosition = cursor.position();
    BOBUIextCharFormat format = currentNode->charFormat;

    if (wsm == BOBUIextHtmlParserNode::WhiteSpacePre || wsm == BOBUIextHtmlParserNode::WhiteSpacePreWrap)
    ...
    while (it.hasNext()) {
        ...
        if (QChar::isSpace(ch)
            ...
            if (wsm == BOBUIextHtmlParserNode::WhiteSpacePreLine && (ch == u'\n' || ch == u'\r'))
                compressNextWhitespace = PreserveWhiteSpace;
            ...
            if (wsm == BOBUIextHtmlParserNode::WhiteSpacePre
                || textEditMode
               ) {
                if (ch == u'\n') {
                    if (textEditMode)
            ...
            } else if (wsm != BOBUIextHtmlParserNode::WhiteSpacePreWrap) {
                compressNextWhitespace = RemoveWhiteSpace;
                if (wsm == BOBUIextHtmlParserNode::WhiteSpacePreLine && (ch == u'\n' || ch == u'\r'))
                { }
                else if (wsm == BOBUIextHtmlParserNode::WhiteSpaceNoWrap)
                    ch = QChar::Nbsp;
                else
                    ch = u' ';
            }
        ...
        if (ch == u'\n'
            ...
            if (!textToInsert.isEmpty()) {
                if (wsm == BOBUIextHtmlParserNode::WhiteSpacePreLine && textToInsert.at(textToInsert.size() - 1) == u' ')
            ...
            if (fmt.hasProperty(BOBUIextFormat::BlockBottomMargin)) {
                BOBUIextBlockFormat tmp = fmt;
                tmp.clearProperty(BOBUIextFormat::BlockBottomMargin);
                cursor.setBlockFormat(tmp);
            }

            fmt.clearProperty(BOBUIextFormat::BlockTopMargin);
            ...
            if (!namedAnchors.isEmpty()) {
                ...
                namedAnchors.clear();
                format.clearProperty(BOBUIextFormat::IsAnchor);
                format.clearProperty(BOBUIextFormat::AnchorName);
            } else {
...
BOBUIextHtmlImporter::ProcessNodeResult BOBUIextHtmlImporter::processSpecialNodes()
{
    switch (currentNode->id) {
        case Html_body:
            if (currentNode->charFormat.background().style() != BobUI::NoBrush) {
                BOBUIextFrameFormat fmt = doc->rootFrame()->frameFormat();
                fmt.setBackground(currentNode->charFormat.background());
                doc->rootFrame()->setFrameFormat(fmt);
                const_cast<BOBUIextHtmlParserNode *>(currentNode)->charFormat.clearProperty(BOBUIextFormat::BackgroundBrush);
            }
            compressNextWhitespace = RemoveWhiteSpace;
            break;
        ...
        case Html_ol:
        case Html_ul: {
            BOBUIextListFormat::Style style = currentNode->listStyle;

            if (currentNode->id == Html_ul && !currentNode->hasOwnListStyle && currentNode->parent) {
                const BOBUIextHtmlParserNode *n = &at(currentNode->parent);
                while (n) {
                    if (n->id == Html_ul) {
        ...
        case Html_img: {
            ...
            if (!currentNode->text.isEmpty())
                fmt.setProperty(BOBUIextFormat::ImageTitle, currentNode->text);
            if (!currentNode->imageAlt.isEmpty())
                fmt.setProperty(BOBUIextFormat::ImageAltText, currentNode->imageAlt);
            ...
            cursor.insertImage(fmt, BOBUIextFrameFormat::Position(currentNode->cssFloat));

            cursor.movePosition(BOBUIextCursor::PreviousCharacter, BOBUIextCursor::KeepAnchor);
        ...
        case Html_hr: {
            ...
            blockFormat.setProperty(BOBUIextFormat::BlockTrailingHorizontalRulerWidth, currentNode->width);
...
bool BOBUIextHtmlImporter::closeTag()
{
    const BOBUIextHtmlParserNode *closedNode = &at(currentNodeIdx - 1);
...
BOBUIextHtmlImporter::Table BOBUIextHtmlImporter::scanTable(int tableNodeIdx)
{
    Table table;
    table.columns = 0;

    QList<BOBUIextLength> columnWidths;
    ...
    for (int row : std::as_const(rowNodes)) {
        ...
        for (int cell : at(row).children) {
            if (at(cell).isTableCell()) {
                ...
                const BOBUIextHtmlParserNode &c = at(cell);
                ...
                for (int i = currentColumn; i < currentColumn + c.tableCellColSpan; ++i) {
                    if (columnWidths.at(i).type() == BOBUIextLength::VariableLength) {
                        BOBUIextLength w = c.width;
                        if (c.tableCellColSpan > 1 && w.type() != BOBUIextLength::VariableLength)
                            w = BOBUIextLength(w.type(), w.value(100.) / c.tableCellColSpan);
                        columnWidths[i] = w;
                    }
    ...
    if (qFuzzyCompare(fmt.leftMargin(), fmt.rightMargin())
        ...
        fmt.setProperty(BOBUIextFormat::FrameMargin, fmt.leftMargin());
    ...
    if (node.blockFormat.hasProperty(BOBUIextFormat::PageBreakPolicy))
        fmt.setPageBreakPolicy(node.blockFormat.pageBreakPolicy());

    if (node.blockFormat.hasProperty(BOBUIextFormat::LayoutDirection))
        fmt.setLayoutDirection(node.blockFormat.layoutDirection());
    if (node.charFormat.background().style() != BobUI::NoBrush)
        fmt.setBackground(node.charFormat.background());
    fmt.setPosition(BOBUIextFrameFormat::Position(node.cssFloat));
    ...
    if (node.isTextFrame) {
        ...
        BOBUIextTable *textTable = cursor.insertTable(table.rows, table.columns, fmt.toTableFormat());
...
BOBUIextHtmlImporter::ProcessNodeResult BOBUIextHtmlImporter::processBlockNode()
{
    BOBUIextBlockFormat block;
    ...
    if (currentNode->isTableCell() && !tables.isEmpty()) {
        ...
        if (!t.isTextFrame && !t.currentCell.atEnd()) {
            BOBUIextTableCell cell = t.currentCell.cell();
            if (cell.isValid()) {
                BOBUIextTableCellFormat fmt = cell.format().toTableCellFormat();
                ...
                if (tableCellBorderStyle(currentNodeIdx, QCss::TopEdge) != BOBUIextFrameFormat::BorderStyle_None)
                    fmt.setTopBorderStyle(tableCellBorderStyle(currentNodeIdx, QCss::TopEdge));
                if (tableCellBorderStyle(currentNodeIdx, QCss::RightEdge) != BOBUIextFrameFormat::BorderStyle_None)
                    fmt.setRightBorderStyle(tableCellBorderStyle(currentNodeIdx, QCss::RightEdge));
                if (tableCellBorderStyle(currentNodeIdx, QCss::BottomEdge) != BOBUIextFrameFormat::BorderStyle_None)
                    fmt.setBottomBorderStyle(tableCellBorderStyle(currentNodeIdx, QCss::BottomEdge));
                if (tableCellBorderStyle(currentNodeIdx, QCss::LeftEdge) != BOBUIextFrameFormat::BorderStyle_None)
                    fmt.setLeftBorderStyle(tableCellBorderStyle(currentNodeIdx, QCss::LeftEdge));
                if (tableCellBorderBrush(currentNodeIdx, QCss::TopEdge) != BobUI::NoBrush)
                    fmt.setTopBorderBrush(tableCellBorderBrush(currentNodeIdx, QCss::TopEdge));
                if (tableCellBorderBrush(currentNodeIdx, QCss::RightEdge) != BobUI::NoBrush)
                    fmt.setRightBorderBrush(tableCellBorderBrush(currentNodeIdx, QCss::RightEdge));
                if (tableCellBorderBrush(currentNodeIdx, QCss::BottomEdge) != BobUI::NoBrush)
                    fmt.setBottomBorderBrush(tableCellBorderBrush(currentNodeIdx, QCss::BottomEdge));
                if (tableCellBorderBrush(currentNodeIdx, QCss::LeftEdge) != BobUI::NoBrush)
    ...
    // list.
    const BOBUIextHtmlParserNode *parentNode = currentNode->parent ? &at(currentNode->parent) : nullptr;
    ...
    if (wsm == BOBUIextHtmlParserNode::WhiteSpacePre
            || wsm == BOBUIextHtmlParserNode::WhiteSpaceNoWrap) {
        block.setNonBreakableLines(true);
        modifiedBlockFormat = true;
    }

    if (currentNode->charFormat.background().style() != BobUI::NoBrush && !currentNode->isTableCell()) {
...
void BOBUIextHtmlImporter::appendBlock(const BOBUIextBlockFormat &format, BOBUIextCharFormat charFmt)
{
    if (!namedAnchors.isEmpty()) {
...
/*!
    \fn BOBUIextDocumentFragment BOBUIextDocumentFragment::fromHtml(const QString &text, const BOBUIextDocument *resourceProvider)
    \since 4.2

    Returns a BOBUIextDocumentFragment based on the arbitrary piece of
...
BOBUIextDocumentFragment BOBUIextDocumentFragment::fromHtml(const QString &html, const BOBUIextDocument *resourceProvider)
{
    BOBUIextDocumentFragment res;
    res.d = new BOBUIextDocumentFragmentPrivate;

    BOBUIextHtmlImporter importer(res.d->doc, html, BOBUIextHtmlImporter::ImportToFragment, resourceProvider);
    importer.import();
    return res;
}
...
/*!
    \fn BOBUIextDocumentFragment BOBUIextDocumentFragment::fromMarkdown(const QString &markdown, BOBUIextDocument::MarkdownFeatures features)
    \since 6.4

    Returns a BOBUIextDocumentFragment based on the given \a markdown text with
...
*/
BOBUIextDocumentFragment BOBUIextDocumentFragment::fromMarkdown(const QString &markdown, BOBUIextDocument::MarkdownFeatures features)
{
    BOBUIextDocumentFragment res;
    res.d = new BOBUIextDocumentFragmentPrivate;

    BOBUIextMarkdownImporter(res.d->doc, features).import(markdown);
    return res;
}
