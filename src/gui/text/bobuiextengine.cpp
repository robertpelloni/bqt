...
Q_STATIC_LOGGING_CATEGORY(lcEmojiSegmenter, "bobui.text.emojisegmenter")
#endif

static const float smallCapsFraction = 0.7f;

namespace {
// Helper class used in BOBUIextEngine::itemize
...
private:
    ...
    void generateScriptItemsCapitalize(int start, int length)
    {
        ...
        if (!m_splitter)
            m_splitter = std::make_unique<BOBUIextBoundaryFinder>(BOBUIextBoundaryFinder::Word,
        ...
        if (m_splitter->boundaryReasons() & BOBUIextBoundaryFinder::StartOfItem)
    ...
    std::unique_ptr<BOBUIextBoundaryFinder> m_splitter;
};
...
//                last = current;
    ...
    void resolveImplicitLevelsForIsolatedRun(const Vector<DirectionalRun> &runs, int i)
    {
        ...
        if (BidiDebugEnabled) {
            ...
            IsolatedRunSequenceIterator it(runs, i);
            while (!it.atEnd()) {
                BIDI_DEBUG() << "    " << *it << BobUI::hex << text[*it].unicode() << analysis[*it].bidiDirection;
                ++it;
            }
        }
    ...
    bool process()
    {
        ...
        if (BidiDebugEnabled) {
            ...
            for (int i = 0; i < length; ++i)
                BIDI_DEBUG() << BobUI::hex << "    (" << i << ")" << text[i].unicode() << text[i].direction();
        }
        ...
        if (BidiDebugEnabled) {
            ...
            for (int i = 0; i < length; ++i)
                BIDI_DEBUG() << "    " << i << BobUI::hex << text[i].unicode() << BobUI::dec << (int)analysis[i].bidiLevel;
        }
...
void BOBUIextEngine::bidiReorder(int numItems, const quint8 *levels, int *visualOrder)
{
...
*/
static inline void bobui_getDefaultJustificationOpportunities(const ushort *string, qsizetype length, const QGlyphLayout &g, ushort *log_clusters, int spaceAs)
{
    qsizetype str_pos = 0;
...
static inline void bobui_getJustificationOpportunities(const ushort *string, qsizetype length, const QScriptItem &si, const QGlyphLayout &g, ushort *log_clusters)
{
    ...
    bobui_getDefaultJustificationOpportunities(string, length, g, log_clusters, spaceAs);
}
...
void BOBUIextEngine::shapeLine(const QScriptLine &line)
{
    QFixed x;
    bool first = true;
...
void BOBUIextEngine::shapeText(int item) const
{
    ...
    if (useRawFont) {
        BOBUIextCharFormat f = format(&si);
        QFont font = f.font();
#  if BOBUI_CONFIG(harfbuzz)
...
#  endif
...
#endif
    {
        ...
        if (letterSpacingIsAbsolute && letterSpacing.value())
            letterSpacing *= font.d->dpi / bobui_defaultDpiY();
    }
...
#endif
    {
        ...
        for (int i = 0; i < itemLength; ++i, ++glyph_pos) {
            ...
            initialGlyphs.attributes[glyph_pos].dontPrint =
                    !is_print_char && !(option.flags() & BOBUIextOption::ShowDefaultIgnorables);
...
#if BOBUI_CONFIG(harfbuzz)
    bobui_getJustificationOpportunities(string, itemLength, si, glyphs, logClusters(&si));
#endif
...
int BOBUIextEngine::shapeTextWithHarfbuzzNG(const QScriptItem &si, const ushort *string,
                                         ...
                                         const QMap<QFont::Tag, quint32> &fontFeatures) const
{
    ...
    if (!buffer) {
        ...
        hb_buffer_set_unicode_funcs(buffer, hb_bobui_get_unicode_funcs());
    }
    ...
    QChar::Script script = si.analysis.script < QChar::ScriptCount
                               ...
                               : QChar::Script_Common;
    props.script = hb_bobui_script_to_script(script);
    ...
    for (qsizetype k = 0; k < itemBoundaries.size(); k += 3) {
        ...
        if (Q_UNLIKELY(actualFontEngine->symbol || (option.flags() & BOBUIextOption::ShowDefaultIgnorables)))
        ...
        // shape
        {
            hb_font_t *hb_font = hb_bobui_font_get_for_engine(actualFontEngine);
            Q_ASSERT(hb_font);
            hb_bobui_font_set_use_design_metrics(hb_font, option.useDesignMetrics() ? uint(QFontEngine::DesignMetrics) : 0); // ###
...
void BOBUIextEngine::init(BOBUIextEngine *e)
{
    e->ignoreBidi = false;
    e->cacheGlyphs = false;
...
BOBUIextEngine::BOBUIextEngine()
{
    init(this);
}

BOBUIextEngine::BOBUIextEngine(const QString &str, const QFont &f)
    : text(str),
      fnt(f)
{
    init(this);
...
const QCharAttributes *BOBUIextEngine::attributes() const
{
...
void BOBUIextEngine::shape(int item) const
{
    auto &li = layoutData->items[item];
    if (li.analysis.flags == QScriptAnalysis::Object) {
        ensureSpace(1);
        if (BOBUIextDocumentPrivate::get(block) != nullptr) {
            docLayout()->resizeInlineObject(BOBUIextInlineObject(item, const_cast<BOBUIextEngine *>(this)),
...
void BOBUIextEngine::resetFontEngineCache()
{
...
void BOBUIextEngine::invalidate()
{
    freeMemory();
    minWidth = 0;
    maxWidth = 0;
...
void BOBUIextEngine::clearLineData()
{
    lines.clear();
}

void BOBUIextEngine::validate() const
{
    if (layoutData)
        return;
    ...
    if (BOBUIextDocumentPrivate::get(block) != nullptr) {
        ...
        if (!nextBlockValid && option.flags() & BOBUIextOption::ShowDocumentTerminator) {
            layoutData->string += QLatin1Char('\xA7');
        } else if (option.flags() & BOBUIextOption::ShowLineAndParagraphSeparators) {
...
void BOBUIextEngine::itemize() const
{
    validate();
...
#if !defined(BOBUI_NO_EMOJISEGMENTER)
    const bool disableEmojiSegmenter = QFontEngine::disableEmojiSegmenter() || option.flags().testFlag(BOBUIextOption::DisableEmojiParsing);
    ...
    while (uc < e) {
        ...
        case QChar::ObjectReplacementCharacter:
            {
                const BOBUIextDocumentPrivate *doc_p = BOBUIextDocumentPrivate::get(block);
                if (doc_p != nullptr
        ...
        case QChar::LineSeparator:
            ...
            if (option.flags() & BOBUIextOption::ShowLineAndParagraphSeparators) {
        ...
        case QChar::Space:
        case QChar::Nbsp:
            if (option.flags() & BOBUIextOption::ShowTabsAndSpaces) {
    ...
    const BOBUIextDocumentPrivate *p = BOBUIextDocumentPrivate::get(block);
    if (p) {
        SpecialData *s = specialData;

        BOBUIextDocumentPrivate::FragmentIterator it = p->find(block.position());
        BOBUIextDocumentPrivate::FragmentIterator end = p->find(block.position() + block.length() - 1); // -1 to omit the block separator char
        ...
        while (1) {
            ...
            if (it == end || format != frag->format) {
                ...
                QFont::Capitalization capitalization =
                        formatCollection()->charFormat(format).hasProperty(BOBUIextFormat::FontCapitalization)
                ...
                if (s) {
                    for (const auto &range : std::as_const(s->formats)) {
                        if (range.start + range.length <= prevPosition || range.start >= position)
                            continue;
                        if (range.format.hasProperty(BOBUIextFormat::FontCapitalization)) {
        ...
        if (useRawFont && specialData) {
            ...
            for (int i = 0; i < specialData->formats.size(); ++i) {
                const BOBUIextLayout::FormatRange &range = specialData->formats.at(i);
                const BOBUIextCharFormat &format = range.format;
                if (format.hasProperty(BOBUIextFormat::FontCapitalization)) {
...
bool BOBUIextEngine::isRightToLeft() const
{
    ...
    if (layoutData->string.isEmpty())
        return QGuiApplication::inputMethod()->inputDirection() == BobUI::RightToLeft;
    return layoutData->string.isRightToLeft();
}


int BOBUIextEngine::findItem(int strPos, int firstItem) const
{
    itemize();
...
void textIterator(const BOBUIextEngine *textEngine, int from, int len, QFixed &width, InnerFunc &&innerFunc)
{
...
QFixed BOBUIextEngine::width(int from, int len) const
{
    itemize();

    QFixed w = 0;
//     qDebug("BOBUIextEngine::width(from = %d, len = %d), numItems=%d, strleng=%d", from,  len, items.size(), string.length());
...
glyph_metrics_t BOBUIextEngine::boundingBox(int from,  int len) const
{
    itemize();
...
glyph_metrics_t BOBUIextEngine::tightBoundingBox(int from,  int len) const
{
    itemize();
...
QFont BOBUIextEngine::font(const QScriptItem &si) const
{
    QFont font = fnt;
    if (hasFormats()) {
        BOBUIextCharFormat f = format(&si);
        ...
        const BOBUIextDocumentPrivate *document_d = BOBUIextDocumentPrivate::get(block);
        ...
        BOBUIextCharFormat::VerticalAlignment valign = f.verticalAlignment();
        if (valign == BOBUIextCharFormat::AlignSuperScript || valign == BOBUIextCharFormat::AlignSubScript) {
            if (font.pointSize() != -1)
...
BOBUIextEngine::FontEngineCache::FontEngineCache()
{
    reset();
}
...
QFontEngine *BOBUIextEngine::fontEngine(const QScriptItem &si, QFixed *ascent, QFixed *descent, QFixed *leading) const
{
...
#endif
    {
        if (hasFormats()) {
            if (feCache.prevFontEngine && feCache.prevPosition == si.position && feCache.prevLength == length(&si) && feCache.prevScript == script) {
                ...
                if (BOBUIextDocumentPrivate::get(block) != nullptr && BOBUIextDocumentPrivate::get(block)->layout() != nullptr) {
                    // Make sure we get the right dpi on printers
                    QPaintDevice *pdev = BOBUIextDocumentPrivate::get(block)->layout()->paintDevice();
                    if (pdev)
                        font = QFont(font, pdev);
                } else {
                    font = font.resolve(fnt);
...
void BOBUIextEngine::justify(const QScriptLine &line)
{
//     qDebug("justify: line.gridfitted = %d, line.justified=%d", line.gridfitted, line.justified);
    ...
    if ((option.alignment() & BobUI::AlignHorizontal_Mask) != BobUI::AlignJustify)
        return;
...
void QScriptLine::setDefaultHeight(BOBUIextEngine *eng)
{
    QFont f;
    QFontEngine *e;

    if (BOBUIextDocumentPrivate::get(eng->block) != nullptr && BOBUIextDocumentPrivate::get(eng->block)->layout() != nullptr) {
        f = eng->block.charFormat().font();
        // Make sure we get the right dpi on printers
        QPaintDevice *pdev = BOBUIextDocumentPrivate::get(eng->block)->layout()->paintDevice();
        if (pdev)
            f = QFont(f, pdev);
...
BOBUIextEngine::LayoutData::LayoutData()
{
    memory = nullptr;
    allocated = 0;
...
BOBUIextEngine::LayoutData::LayoutData(const QString &str, void **stack_memory, qsizetype _allocated)
    : string(str)
{
...
bool BOBUIextEngine::LayoutData::reallocate(int totalGlyphs)
{
...
void BOBUIextEngine::freeMemory()
{
    if (!stackEngine) {
        delete layoutData;
        layoutData = nullptr;
...
int BOBUIextEngine::formatIndex(const QScriptItem *si) const
{
    if (specialData && !specialData->resolvedFormats.isEmpty()) {
        BOBUIextFormatCollection *collection = formatCollection();
        Q_ASSERT(collection);
    ...
    BOBUIextDocumentPrivate::FragmentIterator it = p->find(block.position() + pos);
    return it.value()->format;
}


BOBUIextCharFormat BOBUIextEngine::format(const QScriptItem *si) const
{
    if (const BOBUIextFormatCollection *collection = formatCollection())
        return collection->charFormat(formatIndex(si));
    return BOBUIextCharFormat();
}

void BOBUIextEngine::addRequiredBoundaries() const
{
    if (specialData) {
        for (int i = 0; i < specialData->formats.size(); ++i) {
            const BOBUIextLayout::FormatRange &r = specialData->formats.at(i);
            setBoundary(r.start);
...
bool BOBUIextEngine::atWordSeparator(int position) const
{
...
void BOBUIextEngine::setPreeditArea(int position, const QString &preeditText)
{
...
void BOBUIextEngine::setFormats(const QList<BOBUIextLayout::FormatRange> &formats)
{
    if (formats.isEmpty()) {
        if (!specialData)
            return;
...
void BOBUIextEngine::indexFormats()
{
    BOBUIextFormatCollection *collection = formatCollection();
    if (!collection) {
        Q_ASSERT(BOBUIextDocumentPrivate::get(block) == nullptr);
        specialData->formatCollection.reset(new BOBUIextFormatCollection);
        collection = specialData->formatCollection.data();
    }

    // replace with shared copies
    for (int i = 0; i < specialData->formats.size(); ++i) {
        BOBUIextCharFormat &format = specialData->formats[i].format;
...
QString BOBUIextEngine::elidedText(BobUI::TextElideMode mode, QFixed width, int flags, int from, int count) const
{
//    qDebug() << "elidedText; available width" << width.toReal() << "text width:" << this->width(0, layoutData->string.length()).toReal();
    ...
    if (mode == BobUI::ElideRight) {
        ...
        return stringMidRetainingBidiCC(layoutData->string,
                                        ...
                                        from, to,
                                        pos, to - pos);
    } else if (mode == BobUI::ElideMiddle) {
        QFixed leftWidth;
        QFixed rightWidth;
...
void BOBUIextEngine::setBoundary(int strPos) const
{
...
QFixed BOBUIextEngine::calculateTabWidth(int item, QFixed x) const
{
    ...
    if (BOBUIextDocumentPrivate::get(block) != nullptr && BOBUIextDocumentPrivate::get(block)->layout() != nullptr) {
        QPaintDevice *pdev = BOBUIextDocumentPrivate::get(block)->layout()->paintDevice();
        if (pdev)
            dpiScale = QFixed::fromReal(pdev->logicalDpiY() / qreal(bobui_defaultDpiY()));
    } else {
        dpiScale = QFixed::fromReal(fnt.d->dpi / qreal(bobui_defaultDpiY()));
    }

    QList<BOBUIextOption::Tab> tabArray = option.tabs();
    if (!tabArray.isEmpty()) {
        if (isRightToLeft()) { // rebase the tabArray positions.
            auto isLeftOrRightTab = [](const BOBUIextOption::Tab &tab) {
                return tab.type == BOBUIextOption::LeftTab || tab.type == BOBUIextOption::RightTab;
            };
            ...
            if (cit != cend) {
                ...
                while (iter != end) {
                    BOBUIextOption::Tab &tab = *iter;
                    if (tab.type == BOBUIextOption::LeftTab)
                        tab.type = BOBUIextOption::RightTab;
                    else if (tab.type == BOBUIextOption::RightTab)
                        tab.type = BOBUIextOption::LeftTab;
                    ++iter;
                }
            }
        }
        for (const BOBUIextOption::Tab &tabSpec : std::as_const(tabArray)) {
            ...
            if (tab > x) {  // this is the tab we need.
                ...
                if (tabSpec.type == BOBUIextOption::RightTab || tabSpec.type == BOBUIextOption::CenterTab) {
                ...
                else if (tabSpec.type == BOBUIextOption::DelimiterTab)
                ...
                if (tabSectionEnd > si.position) {
                    ...
                    for (int i=item; i < layoutData->items.size(); i++) {
                        ...
                        if (end + item.position == tabSectionEnd && tabSpec.type == BOBUIextOption::DelimiterTab) // remove half of matching char
                            length -= glyphs.advances[end] / 2 * !glyphs.attributes[end].dontPrint;
                    }

                    switch (tabSpec.type) {
                    case BOBUIextOption::CenterTab:
                        length /= 2;
                        Q_FALLTHROUGH();
                    case BOBUIextOption::DelimiterTab:
                    case BOBUIextOption::RightTab:
                        tab = QFixed::fromReal(tabSpec.position) * dpiScale - length;
                        if (tab < x) // default to tab taking no space
                            return QFixed();
                        break;
                    case BOBUIextOption::LeftTab:
                        break;
                    }
                }
                return tab - x;
...
class FormatRangeComparatorByStart {
    const QList<BOBUIextLayout::FormatRange> &list;
public:
    FormatRangeComparatorByStart(const QList<BOBUIextLayout::FormatRange> &list) : list(list) { }
...
class FormatRangeComparatorByEnd {
    const QList<BOBUIextLayout::FormatRange> &list;
public:
    FormatRangeComparatorByEnd(const QList<BOBUIextLayout::FormatRange> &list) : list(list) { }
    bool operator()(int a, int b) {
...
void BOBUIextEngine::resolveFormats() const
{
    ...
    QList<BOBUIextCharFormat> resolvedFormats(layoutData->items.size());
    ...
    for (int i = 0; i < layoutData->items.size(); ++i) {
        ...
        BOBUIextCharFormat &format = resolvedFormats[i];
        if (BOBUIextDocumentPrivate::get(block) != nullptr) {
            // when we have a BOBUIextDocumentPrivate, formatIndex might still return a valid index based
        ...
        if (!currentFormats.isEmpty()) {
            for (int cur : currentFormats) {
                const BOBUIextLayout::FormatRange &range = specialData->formats.at(cur);
...
QFixed BOBUIextEngine::leadingSpaceWidth(const QScriptLine &line)
{
    if (!line.hasTrailingSpaces
        || (option.flags() & BOBUIextOption::IncludeTrailingSpaces)
        || !isRightToLeft())
        return QFixed();
...
QFixed BOBUIextEngine::alignLine(const QScriptLine &line)
{
    QFixed x = 0;
    justify(line);
    ...
    if (!line.justified && line.width != QFIXED_MAX) {
        ...
        if (align & BobUI::AlignJustify && isRightToLeft())
            align = BobUI::AlignRight;
...
QFixed BOBUIextEngine::offsetInLigature(const QScriptItem *si, int pos, int max, int glyph_pos)
{
    unsigned short *logClusters = this->logClusters(si);
...
int BOBUIextEngine::getClusterLength(unsigned short *logClusters,
...
int BOBUIextEngine::positionInLigature(const QScriptItem *si, int end,
...
int BOBUIextEngine::previousLogicalPosition(int oldPos) const
{
...
int BOBUIextEngine::nextLogicalPosition(int oldPos) const
{
    const QCharAttributes *attrs = attributes();
    int len = block.isValid() ? block.length() - 1
                              : layoutData->string.size();
...
int BOBUIextEngine::lineNumberForTextPosition(int pos)
{
    if (!layoutData)
        itemize();
...
std::vector<int> BOBUIextEngine::insertionPointsForLine(int lineNum)
{
    BOBUIextLineItemIterator iterator(this, lineNum);
...
int BOBUIextEngine::endOfLine(int lineNum)
{
...
int BOBUIextEngine::beginningOfLine(int lineNum)
{
    const auto insertionPoints = insertionPointsForLine(lineNum);
    if (insertionPoints.size() > 0)
...
int BOBUIextEngine::positionAfterVisualMovement(int pos, BOBUIextCursor::MoveOperation op)
{
    itemize();

    bool moveRight = (op == BOBUIextCursor::Right);
...
void BOBUIextEngine::addItemDecoration(QPainter *painter, const QLineF &line, ItemDecorationList *decorationList)
{
    if (delayDecorations) {
...
void BOBUIextEngine::addUnderline(QPainter *painter, const QLineF &line)
{
    // qDebug() << "Adding underline:" << line;
    addItemDecoration(painter, line, &underlineList);
}

void BOBUIextEngine::addStrikeOut(QPainter *painter, const QLineF &line)
{
    addItemDecoration(painter, line, &strikeOutList);
}

void BOBUIextEngine::addOverline(QPainter *painter, const QLineF &line)
{
    addItemDecoration(painter, line, &overlineList);
}

void BOBUIextEngine::drawItemDecorationList(QPainter *painter, const ItemDecorationList &decorationList)
{
...
void BOBUIextEngine::drawDecorations(QPainter *painter)
{
...
void BOBUIextEngine::clearDecorations()
{
    underlineList.clear();
    strikeOutList.clear();
    overlineList.clear();
...
void BOBUIextEngine::adjustUnderlines()
{
...
void BOBUIextEngine::adjustUnderlines(ItemDecorationList::iterator start,
...
BOBUIextItemInt::BOBUIextItemInt(const QScriptItem &si, QFont *font, const BOBUIextCharFormat &format)
    : charFormat(format),
      f(font),
...
BOBUIextItemInt::BOBUIextItemInt(const QGlyphLayout &g, QFont *font, const QChar *chars_, int numChars, QFontEngine *fe, const BOBUIextCharFormat &format)
    : charFormat(format),
      num_chars(numChars),
      chars(chars_),
      f(font),
...
void BOBUIextItemInt::initWithScriptItem(const QScriptItem &si)
{
    // explicitly initialize flags so that initFontAttributes can be called
    // multiple times on the same TextItem
    flags = { };
    if (si.analysis.bidiLevel %2)
        flags |= BOBUIextItem::RightToLeft;
    ascent = si.ascent;
    descent = si.descent;

    if (charFormat.hasProperty(BOBUIextFormat::TextUnderlineStyle)) {
        underlineStyle = charFormat.underlineStyle();
    } else if (charFormat.boolProperty(BOBUIextFormat::FontUnderline)
               || f->d->underline) {
        underlineStyle = BOBUIextCharFormat::SingleUnderline;
    }

    // compat
    if (underlineStyle == BOBUIextCharFormat::SingleUnderline)
...
BOBUIextItemInt BOBUIextItemInt::midItem(QFontEngine *fontEngine, int firstGlyphIndex, int numGlyphs) const
{
    BOBUIextItemInt ti = *this;
...
BOBUIransform bobui_true_matrix(qreal w, qreal h, const BOBUIransform &x)
{
    QRectF rect = x.mapRect(QRectF(0, 0, w, h));
    return x * BOBUIransform::fromTranslate(-rect.x(), -rect.y());
}


glyph_metrics_t glyph_metrics_t::transformed(const BOBUIransform &matrix) const
{
    ...
    BOBUIransform xform = bobui_true_matrix(w, h, matrix);
...
BOBUIextLineItemIterator::BOBUIextLineItemIterator(BOBUIextEngine *_eng, int _lineNum, const QPointF &pos,
                                             const BOBUIextLayout::FormatRange *_selection)
    : eng(_eng),
...
QScriptItem &BOBUIextLineItemIterator::next()
{
    x += itemWidth;
...
bool BOBUIextLineItemIterator::getSelectionBounds(QFixed *selectionX, QFixed *selectionWidth) const
{
