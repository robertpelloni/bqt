...
#ifndef BOBUIEXTENGINE_P_H
#define BOBUIEXTENGINE_P_H
...
// this uses the same coordinate system as BobUI, but a different one to freetype.
...
struct Q_GUI_EXPORT glyph_metrics_t
{
    ...
    glyph_metrics_t transformed(const BOBUIransform &xform) const;
...
struct QScriptItem;
/// Internal BOBUIextItem
class BOBUIextItemInt : public BOBUIextItem
{
public:
    inline BOBUIextItemInt() = default;
    BOBUIextItemInt(const QScriptItem &si, QFont *font, const BOBUIextCharFormat &format = BOBUIextCharFormat());
    BOBUIextItemInt(const QGlyphLayout &g, QFont *font, const QChar *chars, int numChars, QFontEngine *fe,
                 const BOBUIextCharFormat &format = BOBUIextCharFormat());

    /// copy the structure items, adjusting the glyphs arrays to the right subarrays.
    /// the width of the returned BOBUIextItemInt is not adjusted, for speed reasons
    BOBUIextItemInt midItem(QFontEngine *fontEngine, int firstGlyphIndex, int numGlyphs) const;
    ...
    BOBUIextCharFormat::UnderlineStyle underlineStyle = BOBUIextCharFormat::NoUnderline;
...
{
    // created and filled in BOBUIextLine::layout_helper
    QScriptLine()
    ...
    void setDefaultHeight(BOBUIextEngine *eng);
...
class Q_GUI_EXPORT BOBUIextEngine {
public:
    enum LayoutState {
        LayoutEmpty,
        InLayout,
    ...
    BOBUIextEngine();
    BOBUIextEngine(const QString &str, const QFont &f);
    ~BOBUIextEngine();
    ...
    inline BOBUIextFormatCollection *formatCollection() const {
        if (BOBUIextDocumentPrivate::get(block) != nullptr)
            return const_cast<BOBUIextFormatCollection *>(BOBUIextDocumentPrivate::get(block)->formatCollection());
        return specialData ? specialData->formatCollection.data() : nullptr;
    }
    BOBUIextCharFormat format(const QScriptItem *si) const;
    inline QAbstractTextDocumentLayout *docLayout() const {
        Q_ASSERT(BOBUIextDocumentPrivate::get(block) != nullptr);
        return BOBUIextDocumentPrivate::get(block)->document()->documentLayout();
    }
...
public:
    ...
    { return visualMovement || (BOBUIextDocumentPrivate::get(block) != nullptr && BOBUIextDocumentPrivate::get(block)->defaultCursorMoveStyle == BobUI::VisualMoveStyle); }
    ...
    { return BOBUIextDocumentPrivate::get(block) != nullptr || (specialData && !specialData->formats.isEmpty()); }
    inline QList<BOBUIextLayout::FormatRange> formats() const
    {
        return specialData ? specialData->formats : QList<BOBUIextLayout::FormatRange>();
    }
    void setFormats(const QList<BOBUIextLayout::FormatRange> &formats);

private:
    static void init(BOBUIextEngine *e);

    struct SpecialData {
        int preeditPosition;
        QString preeditText;
        QList<BOBUIextLayout::FormatRange> formats;
        QList<BOBUIextCharFormat> resolvedFormats;
        // only used when no BOBUIextDocumentPrivate is available
        QScopedPointer<BOBUIextFormatCollection> formatCollection;
    };
...
public:
    ...
    QString elidedText(BobUI::TextElideMode mode, QFixed width, int flags = 0, int from = 0, int count = -1) const;
    ...
    int positionAfterVisualMovement(int oldPos, BOBUIextCursor::MoveOperation op);
...
class Q_GUI_EXPORT QStackTextEngine : public BOBUIextEngine {
public:
...
Q_DECLARE_TYPEINFO(BOBUIextEngine::ItemDecoration, Q_RELOCATABLE_TYPE);

struct BOBUIextLineItemIterator
{
    BOBUIextLineItemIterator(BOBUIextEngine *eng, int lineNum, const QPointF &pos = QPointF(),
                          const BOBUIextLayout::FormatRange *_selection = nullptr);
    ...
    const BOBUIextLayout::FormatRange *selection;
};

BOBUI_END_NAMESPACE

#endif // BOBUIEXTENGINE_P_H
