...
#include <QDebug>
#include <BobUICore/private/qzipwriter_p.h>
...
static QStringView bullet_char(BOBUIextListFormat::Style style)
{
    static_assert(int(BOBUIextListFormat::ListDisc) == -1);
    static_assert(int(BOBUIextListFormat::ListUpperRoman) == -8);
    ...
    const auto map = [](BOBUIextListFormat::Style s) { return -int(s) - 1; };
    static_assert(uint(map(BOBUIextListFormat::ListUpperRoman)) == std::size(chars) - 1);
...
static QString bulletChar(BOBUIextListFormat::Style style)
{
    return bullet_char(style).toString();
}

static QString borderStyleName(BOBUIextFrameFormat::BorderStyle style)
{
    switch (style) {
...
void BOBUIextOdfWriter::writeFrame(QXmlStreamWriter &writer, const BOBUIextFrame *frame)
{
    Q_ASSERT(frame);
...
void BOBUIextOdfWriter::writeBlock(QXmlStreamWriter &writer, const BOBUIextBlock &block)
{
    ...
    for (BOBUIextBlock::Iterator frag = block.begin(); !frag.atEnd(); ++frag) {
        bool isHyperlink = frag.fragment().charFormat().hasProperty(BOBUIextFormat::AnchorHref);
        if (isHyperlink) {
            QString value = frag.fragment().charFormat().property(BOBUIextFormat::AnchorHref).toString();
...
void BOBUIextOdfWriter::writeInlineCharacter(QXmlStreamWriter &writer, const BOBUIextFragment &fragment) const
{
    ...
    if (m_strategy == nullptr) {
        ...
        BOBUIextImageFormat imageFormat = fragment.charFormat().toImageFormat();
        ...
        QUrl url = QUrl(name);
        const QVariant variant = m_document->resource(BOBUIextDocument::ImageResource, url);
        ...
        if (! image.isNull()) {
            ...
            if (imgQuality >= 100 || imgQuality <= 0 || image.hasAlphaChannel()) {
                ...
                // Write images without alpha channel as jpg with quality set by BOBUIextImageFormat
        ...
        if (!data.isEmpty()) {
            if (imageFormat.hasProperty(BOBUIextFormat::ImageWidth)) {
                width = imageFormat.width();
            }
            if (imageFormat.hasProperty(BOBUIextFormat::ImageHeight)) {
...
void BOBUIextOdfWriter::writeFormats(QXmlStreamWriter &writer, const QSet<int> &formats) const
{
    writer.writeStartElement(officeNS, QString::fromLatin1("automatic-styles"));
    QList<BOBUIextFormat> allStyles = m_document->allFormats();
    for (int formatIndex : formats) {
        BOBUIextFormat textFormat = allStyles.at(formatIndex);
        switch (textFormat.type()) {
        case BOBUIextFormat::CharFormat:
...
void BOBUIextOdfWriter::writeBlockFormat(QXmlStreamWriter &writer, BOBUIextBlockFormat format, int formatIndex) const
{
    ...
    if (format.hasProperty(BOBUIextBlockFormat::LineHeightType)) {
        ...
        case BOBUIextBlockFormat::SingleHeight:
    ...
    if (format.hasProperty(BOBUIextFormat::BlockAlignment)) {
        const BobUI::Alignment alignment = format.alignment() & BobUI::AlignHorizontal_Mask;
        QString value;
        if (alignment == BobUI::AlignLeading)
            value = QString::fromLatin1("start");
        else if (alignment == BobUI::AlignTrailing)
            value = QString::fromLatin1("end");
        else if (alignment == (BobUI::AlignLeft | BobUI::AlignAbsolute))
            value = QString::fromLatin1("left");
        else if (alignment == (BobUI::AlignRight | BobUI::AlignAbsolute))
            value = QString::fromLatin1("right");
        else if (alignment == BobUI::AlignHCenter)
            value = QString::fromLatin1("center");
        else if (alignment == BobUI::AlignJustify)
            value = QString::fromLatin1("justify");
        else
            qWarning() << "BOBUIextOdfWriter: unsupported paragraph alignment; " << format.alignment();
        if (! value.isNull())
            writer.writeAttribute(foNS, QString::fromLatin1("text-align"), value);
    }

    if (format.hasProperty(BOBUIextFormat::BlockTopMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-top"), pixelToPoint(qMax(qreal(0.), format.topMargin())) );
    if (format.hasProperty(BOBUIextFormat::BlockBottomMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-bottom"), pixelToPoint(qMax(qreal(0.), format.bottomMargin())) );
    if (format.hasProperty(BOBUIextFormat::BlockLeftMargin) || format.hasProperty(BOBUIextFormat::BlockIndent))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-left"), pixelToPoint(qMax(qreal(0.),
            format.leftMargin() + format.indent())));
    if (format.hasProperty(BOBUIextFormat::BlockRightMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-right"), pixelToPoint(qMax(qreal(0.), format.rightMargin())) );
    if (format.hasProperty(BOBUIextFormat::TextIndent))
        writer.writeAttribute(foNS, QString::fromLatin1("text-indent"), pixelToPoint(format.textIndent()));
    if (format.hasProperty(BOBUIextFormat::PageBreakPolicy)) {
        if (format.pageBreakPolicy() & BOBUIextFormat::PageBreak_AlwaysBefore)
            writer.writeAttribute(foNS, QString::fromLatin1("break-before"), QString::fromLatin1("page"));
        if (format.pageBreakPolicy() & BOBUIextFormat::PageBreak_AlwaysAfter)
    ...
    if (format.hasProperty(BOBUIextFormat::BlockNonBreakableLines))
    ...
    if (format.hasProperty(BOBUIextFormat::TabPositions)) {
        QList<BOBUIextOption::Tab> tabs = format.tabPositions();
        writer.writeStartElement(styleNS, QString::fromLatin1("tab-stops"));
        QList<BOBUIextOption::Tab>::Iterator iterator = tabs.begin();
        while(iterator != tabs.end()) {
            ...
            case BOBUIextOption::DelimiterTab: type = QString::fromLatin1("char"); break;
            case BOBUIextOption::LeftTab: type = QString::fromLatin1("left"); break;
            case BOBUIextOption::RightTab: type = QString::fromLatin1("right"); break;
            case BOBUIextOption::CenterTab: type = QString::fromLatin1("center"); break;
            }
...
void BOBUIextOdfWriter::writeCharacterFormat(QXmlStreamWriter &writer, BOBUIextCharFormat format, int formatIndex) const
{
    writer.writeStartElement(styleNS, QString::fromLatin1("style"));
    ...
    if (format.hasProperty(BOBUIextFormat::FontItalic)
        ...
        const bool italic = format.hasProperty(BOBUIextFormat::FontItalic) ? format.fontItalic() : defaultFont.italic();
        if (italic)
    ...
    if (format.hasProperty(BOBUIextFormat::FontWeight)
        ...
        int weight = format.hasProperty(BOBUIextFormat::FontWeight)
                         ? format.fontWeight()
                         : defaultFont.weight();
    ...
    if (format.hasProperty(BOBUIextFormat::OldFontFamily)
        || format.hasProperty(BOBUIextFormat::FontFamilies)
        || (defaultFontResolveMask & QFont::FamiliesResolved)) {
        const QString fontFamily = (format.hasProperty(BOBUIextFormat::OldFontFamily)
                                    || format.hasProperty(BOBUIextFormat::FontFamilies))
                                        ? format.fontFamilies().toStringList().value(0, QString())
                                        : defaultFont.family();
        writer.writeAttribute(foNS, QString::fromLatin1("font-family"), fontFamily);
    } else {
        writer.writeAttribute(foNS, QString::fromLatin1("font-family"), QString::fromLatin1("Sans")); // BobUI default
    }

    if (format.hasProperty(BOBUIextFormat::FontPointSize)
        || (defaultFontResolveMask & QFont::SizeResolved)) {
        const qreal pointSize = format.hasProperty(BOBUIextFormat::FontPointSize)
    ...
    if (format.hasProperty(BOBUIextFormat::FontCapitalization)
        ...
        QFont::Capitalization capitalization = format.hasProperty(BOBUIextFormat::FontCapitalization)
    ...
    if (format.hasProperty(BOBUIextFormat::FontLetterSpacing) ||
        (defaultFontResolveMask & QFont::LetterSpacingResolved)) {
        const qreal letterSpacing = format.hasProperty(BOBUIextFormat::FontLetterSpacing)
    ...
    if (format.hasProperty(BOBUIextFormat::FontWordSpacing)
        || (defaultFontResolveMask & QFont::WordSpacingResolved)) {
        const qreal wordSpacing = format.hasProperty(BOBUIextFormat::FontWordSpacing)
    ...
    if (format.hasProperty(BOBUIextFormat::FontUnderline)
        || ((defaultFontResolveMask & QFont::UnderlineResolved)
            && !format.hasProperty(BOBUIextFormat::TextUnderlineStyle))) {
        const bool underline = format.hasProperty(BOBUIextFormat::FontUnderline)
    ...
    if (format.hasProperty(BOBUIextFormat::FontStrikeOut)
        ...
        const bool strikeOut = format.hasProperty(BOBUIextFormat::FontStrikeOut)
    ...
    if (format.hasProperty(BOBUIextFormat::FontFixedPitch)) {
        //   bool   fontFixedPitch () const  TODO
    }
    if (format.hasProperty(BOBUIextFormat::TextUnderlineStyle)) {
        QString value;
        switch (format.underlineStyle()) {
        case BOBUIextCharFormat::NoUnderline: value = QString::fromLatin1("none"); break;
        case BOBUIextCharFormat::SingleUnderline: value = QString::fromLatin1("solid"); break;
        case BOBUIextCharFormat::DashUnderline: value = QString::fromLatin1("dash"); break;
        case BOBUIextCharFormat::DotLine: value = QString::fromLatin1("dotted"); break;
        case BOBUIextCharFormat::DashDotLine: value = QString::fromLatin1("dash-dot"); break;
        case BOBUIextCharFormat::DashDotDotLine: value = QString::fromLatin1("dot-dot-dash"); break;
        case BOBUIextCharFormat::WaveUnderline: value = QString::fromLatin1("wave"); break;
        case BOBUIextCharFormat::SpellCheckUnderline: value = QString::fromLatin1("none"); break;
        }
        writer.writeAttribute(styleNS, QString::fromLatin1("text-underline-style"), value);
    }
    if (format.hasProperty(BOBUIextFormat::TextVerticalAlignment)) {
        QString value;
        switch (format.verticalAlignment()) {
        case BOBUIextCharFormat::AlignMiddle:
        case BOBUIextCharFormat::AlignNormal: value = QString::fromLatin1("0%"); break;
        case BOBUIextCharFormat::AlignSuperScript: value = QString::fromLatin1("super"); break;
        case BOBUIextCharFormat::AlignSubScript: value = QString::fromLatin1("sub"); break;
        case BOBUIextCharFormat::AlignTop: value = QString::fromLatin1("100%"); break;
        case BOBUIextCharFormat::AlignBottom : value = QString::fromLatin1("-100%"); break;
        case BOBUIextCharFormat::AlignBaseline: break;
        }
    ...
    if (format.hasProperty(BOBUIextFormat::TextToolTip)) {
        //   QString   toolTip () const  TODO
    }
    if (format.hasProperty(BOBUIextFormat::IsAnchor)) {
        //   bool   isAnchor () const  TODO
    }
    if (format.hasProperty(BOBUIextFormat::AnchorHref)) {
        //   QString   anchorHref () const  TODO
    }
    if (format.hasProperty(BOBUIextFormat::AnchorName)) {
        //   QString   anchorName () const  TODO
    }
    if (format.hasProperty(BOBUIextFormat::ForegroundBrush)) {
...
void BOBUIextOdfWriter::writeListFormat(QXmlStreamWriter &writer, BOBUIextListFormat format, int formatIndex) const
{
    ...
    BOBUIextListFormat::Style style = format.style();
    if (style == BOBUIextListFormat::ListDecimal || style == BOBUIextListFormat::ListLowerAlpha
...
void BOBUIextOdfWriter::writeFrameFormat(QXmlStreamWriter &writer, BOBUIextFrameFormat format, int formatIndex) const
{
    writer.writeStartElement(styleNS, QString::fromLatin1("style"));
    ...
    if (format.hasProperty(BOBUIextFormat::FrameTopMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-top"), pixelToPoint(qMax(qreal(0.), format.topMargin())) );
    if (format.hasProperty(BOBUIextFormat::FrameBottomMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-bottom"), pixelToPoint(qMax(qreal(0.), format.bottomMargin())) );
    if (format.hasProperty(BOBUIextFormat::FrameLeftMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-left"), pixelToPoint(qMax(qreal(0.), format.leftMargin())) );
    if (format.hasProperty(BOBUIextFormat::FrameRightMargin))
        writer.writeAttribute(foNS, QString::fromLatin1("margin-right"), pixelToPoint(qMax(qreal(0.), format.rightMargin())) );

    writer.writeEndElement(); // style

// TODO consider putting the following properties in a bobui-namespace.
// Position   position () const
// qreal   border () const
// QBrush   borderBrush () const
// BorderStyle   borderStyle () const
// qreal   padding () const
// BOBUIextLength   width () const
// BOBUIextLength   height () const
// PageBreakFlags   pageBreakPolicy () const
}

void BOBUIextOdfWriter::writeTableFormat(QXmlStreamWriter &writer, BOBUIextTableFormat format, int formatIndex) const
{
    ...
    if (format.columnWidthConstraints().size()) {
        ...
        for (int colit = 0; colit < format.columnWidthConstraints().size(); ++colit) {
            ...
            QString columnWidth;
            if (format.columnWidthConstraints().at(colit).type() == BOBUIextLength::PercentageLength) {
                columnWidth = QString::number(format.columnWidthConstraints().at(colit).rawValue())
                        + "%"_L1;
            } else if (format.columnWidthConstraints().at(colit).type() == BOBUIextLength::FixedLength) {
                columnWidth = QString::number(format.columnWidthConstraints().at(colit).rawValue())
                        + "pt"_L1;
            } else {
...
void BOBUIextOdfWriter::writeTableCellFormat(QXmlStreamWriter &writer, BOBUIextTableCellFormat format,
    ...
    if (m_cellFormatsInTablesWithBorders.contains(formatIndex)) {
        ...
        for (const auto &tableId : tableIdVector) {
            ...
            if (tmpStyle.isTableFormat()) {
                BOBUIextTableFormat tableFormatTmp = tmpStyle.toTableFormat();
                tableCellStyleElement(writer, formatIndex, format, true, tableId, tableFormatTmp);
            } else {
                qDebug("BOBUIextOdfWriter::writeTableCellFormat: ERROR writing table border format");
            }
        }
    }
...
void BOBUIextOdfWriter::tableCellStyleElement(QXmlStreamWriter &writer, const int &formatIndex,
                                           const BOBUIextTableCellFormat &format,
                                           bool hasBorder, int tableId,
                                           const BOBUIextTableFormat tableFormatTmp) const {
    writer.writeStartElement(styleNS, QString::fromLatin1("style"));
    if (hasBorder) {
        writer.writeAttribute(styleNS, QString::fromLatin1("name"),
    ...
    // BOBUIextFormat::BackgroundImageUrl
    // format.background
    writer.writeEndElement(); // style
}
...
BOBUIextOdfWriter::BOBUIextOdfWriter(const BOBUIextDocument &document, QIODevice *device)
...
bool BOBUIextOdfWriter::writeAll()
{
    if (m_createArchive)
    ...
    if (!m_device->isWritable() && ! m_device->open(QIODevice::WriteOnly)) {
        qWarning("BOBUIextOdfWriter::writeAll: the device cannot be opened for writing");
        return false;
    }
    ...
    BOBUIextDocumentPrivate::FragmentIterator fragIt = BOBUIextDocumentPrivate::get(m_document)->begin();
    QSet<int> formats;
    while (fragIt != BOBUIextDocumentPrivate::get(m_document)->end()) {
    ...
    BOBUIextDocumentPrivate::BlockMap &blocks = const_cast<BOBUIextDocumentPrivate *>(BOBUIextDocumentPrivate::get(m_document))->blockMap();
    BOBUIextDocumentPrivate::BlockMap::Iterator blockIt = blocks.begin();
    while (blockIt != blocks.end()) {
        const BOBUIextBlockData * const block = blockIt.value();
        formats << block->format;
        ++blockIt;
    }

    // add objects for lists, frames and tables
    const QList<BOBUIextFormat> allFormats = m_document->allFormats();
    const QList<int> copy = formats.values();
    for (auto index : copy) {
        BOBUIextObject *object = m_document->objectForFormat(allFormats[index]);
        if (object) {
            formats << object->formatIndex();
            if (auto *tableobject = qobject_cast<BOBUIextTable *>(object)) {
    ...
    BOBUIextFrame *rootFrame = m_document->rootFrame();
