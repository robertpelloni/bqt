...
/*!
    ...
    BobUI pictures use a proprietary binary format. Unlike native picture
    (meta-file) formats on many window systems, BobUI pictures have no
...
const char  *bobui_mfhdr_tag = "QPIC"; // header tag
static const quint16 mfhdr_maj = QDataStream::BobUI_DefaultCompiledVersion; // major version #
...
/*!
    ...
    current release, i.e. for BobUI 4.0 a formatVersion of 7 is the same
    as the default formatVersion of -1.

    Reading pictures generated by earlier versions of BobUI is not
    supported in BobUI 4.0.
*/
      ...
      d_ptr(new QPicturePrivate)
{
    ...
    // still accept the 0 default from before BobUI 3.0.
...
{
public:
    QFakeDevice() { dpi_x = bobui_defaultDpiX(); dpi_y = bobui_defaultDpiY(); }
...
bool QPicture::exec(QPainter *painter, QDataStream &s, int nrecords)
{
    ...
    BOBUIransform worldMatrix = painter->transform();
    worldMatrix.scale(qreal(painter->device()->logicalDpiX()) / qreal(bobui_defaultDpiX()),
                      qreal(painter->device()->logicalDpiY()) / qreal(bobui_defaultDpiY()));
...
//                 painter->drawPoints(a.mid(i1_32, i2_32));
        ...
        case QPicturePrivate::PdcDrawRoundRect:
            if (d->formatMajor <= 5) {
                s >> ir >> i1_16 >> i2_16;
                painter->drawRoundedRect(ir, i1_16, i2_16, BobUI::RelativeSize);
            } else {
                s >> r >> i1_16 >> i2_16;
                painter->drawRoundedRect(r, i1_16, i2_16, BobUI::RelativeSize);
            }
            break;
        ...
        case QPicturePrivate::PdcDrawPolygon:
            if (d->formatMajor <= 5) {
                s >> ia >> i_8;
                painter->drawPolygon(ia, i_8 ? BobUI::WindingFill : BobUI::OddEvenFill);
                ia.clear();
            } else {
                s >> a >> i_8;
                painter->drawPolygon(a, i_8 ? BobUI::WindingFill : BobUI::OddEvenFill);
                a.clear();
            }
            break;
        ...
        case QPicturePrivate::PdcDrawTextItem: {
            ...
            if (d->formatMajor >= 9) {
                ...
                if (dbl != 1.0) {
                    QFakeDevice fake;
                    fake.setDpiX(qRound(dbl*bobui_defaultDpiX()));
                    fake.setDpiY(qRound(dbl*bobui_defaultDpiY()));
                    fnt = QFont(font, &fake);
                }
                ...
                int flags = BobUI::TextSingleLine | BobUI::TextDontClip | BobUI::TextForceLeftToRight;
                int alignment = 0;

                QSizeF size(1, 1);
                if (justificationWidth > 0) {
                    size.setWidth(justificationWidth);
                    flags |= BobUI::TextJustificationForced;
                    alignment |= BobUI::AlignJustify;
                }

                QPointF pt(p.x(), p.y());
                if (d->formatMajor >= QDataStream::BobUI_6_11) {
                    alignment |= BobUI::AlignBaseline;
                } else {
                    QFontMetrics fm(fnt);
                    pt.ry() -= fm.ascent();
                }
                ...
                bobui_format_text(font,
                               QRectF(p, QSizeF(1, 1)),
                               BobUI::TextSingleLine | BobUI::TextDontClip,
                               0,
                               /*opt*/nullptr,
                               str,
                               /*brect=*/nullptr,
        ...
        case QPicturePrivate::PdcDrawImage: {
            ...
            if (d->formatMajor < 4) {
                ...
                painter->drawImage(r, image, sr, BobUI::ImageConversionFlags(ul));
            }
        }
            break;
        ...
        case QPicturePrivate::PdcSetBkMode:
            s >> i_8;
            painter->setBackgroundMode((BobUI::BGMode)i_8);
            break;
        ...
        case QPicturePrivate::PdcSetWMatrix:
            if (d->formatMajor >= 8) {
                ...
                matrix = BOBUIransform(wmatrix[0], wmatrix[1],
                                    wmatrix[2], wmatrix[3],
                                    wmatrix[4], wmatrix[5]);
            }
        ...
        case QPicturePrivate::PdcSetClipRegion:
            s >> rgn >> i_8;
            if (d->formatMajor >= 9) {
                painter->setClipRegion(rgn, BobUI::ClipOperation(i_8));
            } else {
        ...
        case QPicturePrivate::PdcSetClipPath:
            {
                QPainterPath path;
                s >> path >> i_8;
                painter->setClipPath(path, BobUI::ClipOperation(i_8));
                break;
            }
...
int QPicture::metric(PaintDeviceMetric m) const
{
    ...
    switch (m) {
        ...
        case PdmHeight:
            ...
            break;
        case PdmWidthMM:
            val = int(25.4/bobui_defaultDpiX()*brect.width());
            break;
        case PdmHeightMM:
            val = int(25.4/bobui_defaultDpiY()*brect.height());
            break;
        case PdmDpiX:
        case PdmPhysicalDpiX:
...
bool QPicturePrivate::checkFormat()
{
    ...
    if (memcmp(mf_id, bobui_mfhdr_tag, 4) != 0 || bufSize < 12) {   // wrong header id or size
