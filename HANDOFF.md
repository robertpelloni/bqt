...
- Extended the forwarding smoke test to validate direct Concurrent/Test module-package lookup and target creation.
- Extended the publication configure smoke test to validate build/install-style publication for Concurrent/Test compat package files.
...
- Archived a detailed handoff log in `logs/handoffs/2026-04-05-bobui-cmake-module-shims-concurrent-test-session.md`.
...
- The Go baseline remains fully verified while the C++ migration path now includes direct BobUI module-package slices for `Concurrent`, `Core`, `DBus`, `Gui`, `Network`, `OpenGL`, `OpenGLWidgets`, `PrintSupport`, `Sql`, `Test`, `Widgets`, and `Xml`.
