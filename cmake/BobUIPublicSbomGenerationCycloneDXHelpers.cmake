...
function(_bobui_internal_get_staging_area_cydx_file_path out_var)
...
# Similar to _bobui_internal_sbom_begin_project_generate which is the SPDX variant.
function(_bobui_internal_sbom_begin_project_generate_cyclone)
    set(opt_args "")
    set(single_args
        OUTPUT
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(BOM_SERIAL_NUMBER_UUID "")

    if(arg_PROJECT_COMMENT)
        string(APPEND project_comment "${arg_PROJECT_COMMENT}")
    endif()

    _bobui_internal_sbom_set_default_option_value(DOWNLOAD_LOCATION "")
...
version = \"${BOBUI_SBOM_GIT_VERSION}\" # Required
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files_cydx "${create_staging_file}")
endfunction()

# Finalizes the CycloneDX sbom generation for a project.
function(_bobui_internal_sbom_end_project_generate_cyclone)
    ...
    _bobui_internal_sbom_add_recorded_licenses_cydx()
...
function(_bobui_internal_sbom_generate_cyclone_add_package)
    set(opt_args "")
    set(single_args
        PACKAGE
        VERSION
    ...
    _bobui_internal_sbom_set_default_option_value(DOWNLOAD_LOCATION "")
    _bobui_internal_sbom_set_default_option_value(VERSION "")
    _bobui_internal_sbom_set_default_option_value(SUPPLIER "")
    _bobui_internal_sbom_set_default_option_value(LICENSE_CONCLUDED "")
    _bobui_internal_sbom_set_default_option_value(COPYRIGHT "")
    ...
    _bobui_internal_sbom_get_cyclone_component_type(component_type "${arg_SBOM_ENTITY_TYPE}")
    ...
    if(arg_CYDX_PROPERTIES)
        _bobui_internal_sbom_handle_cydx_properties(
            CYDX_PROPERTIES ${arg_CYDX_PROPERTIES}
            OUT_VAR_CYDX_PROPERTIES_STRING properties_field
        )
    endif()

    _bobui_internal_get_staging_area_cydx_file_path(staging_area_spdx_file)
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files_cydx "${package_sbom}")
endfunction()

# Adds all recorded custom licenses to the toml file.
function(_bobui_internal_sbom_add_recorded_licenses_cydx)
    get_property(license_ids GLOBAL PROPERTY _bobui_internal_sbom_cydx_licenses)
    if(NOT license_ids)
        return()
    endif()
    ...
    foreach(license_id IN LISTS license_ids)
        get_property(license_text GLOBAL PROPERTY
            _bobui_internal_sbom_cydx_licenses_${license_id}_text)
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_end_include_files_cydx "${snippet}")

    # Clean up before configuring next repo project.
    set_property(GLOBAL PROPERTY _bobui_internal_sbom_cydx_licenses "")
    foreach(license_id IN LISTS license_ids)
        set_property(GLOBAL PROPERTY _bobui_internal_sbom_cydx_licenses_${license_id}_text "")
    endforeach()
endfunction()
