...
# 10. target_hw
# 11. other
function(_bobui_internal_sbom_compute_security_cpe out_cpe)
    set(opt_args "")
    set(single_args
        PART
        VENDOR
...
# Computes the default security CPE for a given bobui repository.
function(_bobui_internal_sbom_get_cpe_bobui_repo out_var)
    _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
    _bobui_internal_sbom_compute_security_cpe(repo_cpe
        VENDOR "bobui"
...
# Computes the default security CPE for the BobUI framework.
function(_bobui_internal_sbom_get_cpe_bobui out_var)
    _bobui_internal_sbom_compute_security_cpe(bobui_cpe
        VENDOR "bobui"
        PRODUCT "bobui"
        VERSION "${BOBUI_REPO_MODULE_VERSION}"
    )
    set(${out_var} "${bobui_cpe}" PARENT_SCOPE)
endfunction()

# Computes the list of security CPEs for BobUI, including both the repo-specific one and generic one.
function(_bobui_internal_sbom_compute_security_cpe_for_bobui out_cpe_list)
    set(cpe_list "")

    _bobui_internal_sbom_get_cpe_bobui(bobui_cpe)
    list(APPEND cpe_list "${bobui_cpe}")

    _bobui_internal_sbom_get_cpe_bobui_repo(repo_cpe)
