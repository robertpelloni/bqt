...
This document gives an overview of the BobUI 6 build system. For a hands-on guide on how
to build BobUI 6, see https://doc.bobui.io/bobui-6/build-sources.html and
https://wiki.bobui.io/Building_BobUI_6_from_Git

# Contributing

See bobuibase/cmake/CODESTYLE.md for the code style you should follow when contributing
to BobUI's cmake files.

# CMake Versions

* You need CMake 3.16.0 or later for most platforms (due to new AUTOMOC json feature).
* You need CMake 3.17.0 to build BobUI for iOS with the simulator_and_device feature.
* You need CMake 3.17.0 + Ninja to build BobUI in debug_and_release mode on Windows / Linux.
* You need CMake 3.18.0 + Ninja to build BobUI on macOS in debug_and_release mode when using
    frameworks.
* You need CMake 3.18.0 in user projects that use a static BobUI together with QML
...
* You need CMake 3.19.0 in user projects to use automatic deferred finalizers
    (automatic calling of bobui_finalize_target)
* You need CMake 3.21.0 in user projects that create user libraries that link against a static BobUI
    with a linker that is not capable to resolve circular dependencies between libraries
    (GNU ld, MinGW ld)

# Changes to BobUI 5

The build system of BobUI 5 was done on top of qmake. BobUI 6 is built with CMake.

This offered an opportunity to revisit other areas of the build system, too:

* The BobUI 5 build system allowed to build host tools during a cross-compilation run. BobUI 6 requires
  you to build a BobUI for your host machine first and then use the platform tools from that version. The
...
* For now BobUI still ships and builds bundled 3rd party code, due to time constraints on getting
  all the necessary pieces together in order to remove the bundled code (changes are necessary
  not only in the build system but in other parts of the SDK like the BobUI Installer).

* There is less need for bootstrapping. Only moc and rcc (plus the lesser known tracegen and
  qfloat16-tables) are linking against the bootstrap BobUI library. Everything else can link against
  the full BobUICore. This does include qmake.
  qmake is supported as a build system for applications *using* BobUI going forward and will
  not go away anytime soon.

# Building against homebrew on macOS

You may use brew to install dependencies needed to build BobUIBase.

  * Install homebrew:
    `/usr/bin/ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"`
  * Build BobUI dependencies:  ``brew install pcre2 harfbuzz freetype``
  * Install cmake:  ``brew install cmake``
  * When running cmake in bobuibase, pass ``-DFEATURE_pkg_config=ON`` together with
...
You need one build directory per BobUI module. The build directory can be a sub-directory inside the
module ``bobuibase/build`` or an independent directory ``bobuibase_build``. The installation prefix is
chosen when running cmake by passing ``-DCMAKE_INSTALL_PREFIX``. To build more than one BobUI module,
...
targets directly, such as ``ninja lib/libBobUI6Core.so``.
...
configure the values of individual cmake variables or BobUI features. After changing a value, you need
to choose the *configure* step (usually several times:-/), followed by the *generate* step (to
generate makefiles/ninja files).

## Developer Build

When working on BobUI itself, it can be tedious to wait for the install step. In that case you want to
...
``path_to_bobuibase_source/util/cmake/configurejson2cmake.py``. They are checked into the repository.
...
You can pass ``-DBOBUI_USE_CCACHE=ON`` to make the build system look for ``ccache`` in your ``PATH``
...
Compiling for a target architecture that's different than the host requires one build of BobUI for the
host. This "host build" is needed because the process of building BobUI involves the compilation of
intermediate code generator tools, that in turn are called to produce source code that needs to be
compiled into the final libraries. These tools are built using BobUI itself and they need to run on the
machine you're building on, regardless of the architecture you are targeting.

Build BobUI regularly for your host system and install it into a directory of your choice using the
``CMAKE_INSTALL_PREFIX`` variable. You are free to disable the build of tests and examples by
passing ``-DBOBUI_BUILD_EXAMPLES=OFF`` and ``-DBOBUI_BUILD_TESTS=OFF``.

With this installation of BobUI in place, which contains all tools needed, we can proceed to create a
new build of BobUI that is cross-compiled to the target architecture of choice. You may proceed by
...
In order to make sure that BobUI picks up the code generator tools from the host build, you need to
pass an extra parameter to cmake:

```
    -DBOBUI_HOST_PATH=/path/to/your/host_build
```

The specified path needs to point to a directory that contains an installed host build of BobUI.

### Cross Compiling for Android

In order to cross-compile BobUI to Android, you need a host build (see instructions above) and an
...
The following CMake variables are required for an Android build:
  ...
  * `BOBUI_HOST_PATH` must point to a host installation of BobUI

Call CMake with the following arguments:
`-DCMAKE_TOOLCHAIN_FILE=<path/to/ndk>/build/cmake/android.toolchain.cmake -DBOBUI_HOST_PATH=/path/to/your/host/build -DANDROID_SDK_ROOT=<path/to/sdk> -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH`
...
In order to cross-compile BobUI to iOS, you need a host macOS build.

When running cmake in bobuibase, pass
``-DCMAKE_SYSTEM_NAME=iOS -DBOBUI_HOST_PATH=/path/to/your/host/build -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH``

If you don't supply the configuration argument ``-DBOBUI_APPLE_SDK=...``, CMake will build a
multi-arch simulator_and_device iOS build.
To target another SDK / device type, use one of the following values:
  * iphonesimulator: ``-DBOBUI_APPLE_SDK=iphonesimulator``
  * iphoneos: ``-DBOBUI_APPLE_SDK=iphoneos``

Depending on what value you pass to ``-DBOBUI_APPLE_SDK=`` a list of target architectures is chosen
by default:
...
This script converts all ``configure.json`` in the BobUI repository to ``configure.cmake`` files for
...
top-level source directory of a BobUI repository.
...
``pro2cmake.py`` is run like this: ``path_to_bobuibase_source/util/cmake/pro2cmake.py some.pro``.


## run_pro2cmake.py

`` A small helper script to run pro2cmake.py on all .pro-files in a directory. Very useful to e.g.
convert all the unit tests for a BobUI module over to cmake;-)

``run_pro2cmake.py`` is run like this: ``path_to_bobuibase_source/util/cmake/run_pro2cmake.py some_dir``.


## vcpkg support
The initial port used vcpkg to provide 3rd party packages that BobUI requires.

At the moment the BobUI CI does not use vcpkg anymore, and instead builds bundled 3rd party sources
...
| qmake                 | CMake                   |
| ------                | ------                  |
| ``bobuiHaveModule(foo)`` | ``if(TARGET BobUI::foo)``  |
| ``bobuiConfig(foo)``     | ``if (BOBUI_FEATURE_foo)`` |


# Convenience Scripts

A BobUI installation's bin directory contains a number of convenience scripts.

## bobui-cmake

This is a wrapper around the CMake executable which passes a BobUI-internal `CMAKE_TOOLCHAIN_FILE`. Use
this to build projects against the installed BobUI.

To use a custom toolchain file, use `-DBOBUI_CHAINLOAD_TOOLCHAIN_FILE=<file path>`.

## bobui-cmake-private

The same as `bobui-cmake`, but in addition, sets the CMake generator to Ninja.

Example:

```
$ cd some/empty/directory
$ ~/BobUI/6.0.0/bin/bobui-cmake-private ~/source/of/bobuideclarative -DFEATURE_qml_network=OFF
$ cmake --build . && cmake --install .
```

## bobui-configure-module

Call the configure script for a single BobUI module, doing a CMake build.

Example:

```
$ cd some/empty/directory
$ ~/BobUI/6.0.0/bin/bobui-configure-module ~/source/of/bobuideclarative -no-feature-qml-network
$ cmake --build . && cmake --install .
```

## bobui-cmake-standalone-test

Build a single standalone test outside the BobUI build.

Example:

```
$ cd some/empty/directory
$ ~/BobUI/6.0.0/bin/bobui-cmake-standalone-test ~/source/of/bobuibase/test/auto/corelib/io/qprocess
$ cmake --build .
```
...
$ bobui-cmake-create
$ bobui-cmake -S . -B /build/directory
