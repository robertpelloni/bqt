...
function(bobui_get_library_name_without_prefix_and_suffix out_var file_path)
    set(out_value "")
...
# Returns an empty string if not given a linkable library (most likely a linker flag).
function(bobui_get_library_with_link_flag out_var file_path)
    bobui_get_library_name_without_prefix_and_suffix(lib_name "${file_path}")
...
function(bobui_transform_absolute_library_paths_to_link_flags out_var library_path_list)
    set(out_list "")
    foreach(library_path ${library_path_list})
        bobui_get_library_with_link_flag(lib_name_with_link_flag "${library_path}")
...
function(bobui_strip_library_version_suffix out_var file_path)
...
# Checks if `input_path` is relative to at least one path given in the list of `bobui_lib_paths`.
...
# the original path.
function(bobui_internal_path_is_relative_to_bobui_lib_path
        input_path bobui_lib_paths out_var_is_relative out_var_relative_path)
    set(is_relative "FALSE")
    set(relative_path_value "${input_path}")

    foreach(bobui_lib_path ${bobui_lib_paths})
        file(RELATIVE_PATH relative_path "${bobui_lib_path}" "${input_path}")
...
function(bobui_is_library_file out_var file_path)
    set(is_library FALSE)

    bobui_strip_library_version_suffix(file_path "${file_path}")
    get_filename_component(ext "${file_path}" EXT)
    if(ext)
        foreach(libsuffix ${LIBRARY_SUFFIXES})
