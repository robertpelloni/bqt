...
# by bobui_add_test or bobui_add_tool below.
# One-value Arguments:
#     CORE_LIBRARY
...
function(bobui_internal_add_executable name)
    ...
    if ("x${arg_OUTPUT_DIRECTORY}" STREQUAL "x")
        set(arg_OUTPUT_DIRECTORY "${BOBUI_BUILD_DIR}/${INSTALL_BINDIR}")
    endif()

    get_filename_component(arg_OUTPUT_DIRECTORY "${arg_OUTPUT_DIRECTORY}"
        ABSOLUTE BASE_DIR "${BOBUI_BUILD_DIR}")

    if ("x${arg_INSTALL_DIRECTORY}" STREQUAL "x")
        set(arg_INSTALL_DIRECTORY "${INSTALL_BINDIR}")
    endif()

    _bobui_internal_create_executable(${name})
    bobui_internal_mark_as_internal_target(${name})

    set_target_properties(${name} PROPERTIES
        _bobui_is_test_executable ${arg_BOBUI_TEST}
        _bobui_is_manual_test ${arg_BOBUI_MANUAL_TEST}
        _bobui_is_benchmark_test ${arg_BOBUI_BENCHMARK_TEST}
    )
    ...
    if(NOT BOBUI_INTERNAL_SKIP_TEST_FINALIZERS_V2)
        ...
        if(ANDROID
            OR WASM
            OR arg_BOBUI_TEST
            OR arg_BOBUI_MANUAL_TEST
            OR arg_BOBUI_BENCHMARK_TEST)
            set(BOBUI_INTERNAL_USE_POOR_MANS_SCOPE_FINALIZER TRUE)
            _bobui_internal_finalize_target_defer("${name}")
        endif()
    else()
        if(ANDROID)
            # This direct calls the finalizer, which in the v2 way is deferred.
            _bobui_internal_android_executable_finalizer(${name})
        endif()
        if(WASM)
            # This defer calls the finalizer.
            bobui_internal_wasm_add_finalizers(${name})
        endif()
    endif()
    ...
    if(arg_DELAY_TARGET_INFO)
        ...
        # of BobUI apps.
        set_target_properties("${name}" PROPERTIES
            BOBUI_DELAYED_TARGET_VERSION "${arg_VERSION}"
            BOBUI_DELAYED_TARGET_PRODUCT "${arg_TARGET_PRODUCT}"
            BOBUI_DELAYED_TARGET_DESCRIPTION "${arg_TARGET_DESCRIPTION}"
            BOBUI_DELAYED_TARGET_COMPANY "${arg_TARGET_COMPANY}"
            BOBUI_DELAYED_TARGET_COPYRIGHT "${arg_TARGET_COPYRIGHT}"
            )
    else()
        if(NOT arg_TARGET_DESCRIPTION)
            set(arg_TARGET_DESCRIPTION "BobUI ${name}")
        endif()
        bobui_set_target_info_properties(${name} ${ARGN}
            TARGET_DESCRIPTION ${arg_TARGET_DESCRIPTION}
            TARGET_VERSION ${arg_VERSION})
    endif()

    if (WIN32 AND NOT arg_DELAY_RC)
        _bobui_internal_generate_win32_rc_file(${name})
    endif()

    bobui_set_common_target_properties(${name})

    bobui_internal_add_repo_local_defines(${name})

    if(ANDROID)
        # The above call to bobui_set_common_target_properties() sets the symbol
    ...
    bobui_autogen_tools_initial_setup(${name})

    bobui_internal_default_warnings_are_errors("${name}")

    set(extra_libraries "")
    if(arg_CORE_LIBRARY STREQUAL "Bootstrap")
        list(APPEND extra_libraries ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Bootstrap)
    elseif(NOT arg_CORE_LIBRARY STREQUAL "None")
        list(APPEND extra_libraries ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core)
    endif()
    ...
    if(arg_PUBLIC_LIBRARIES)
        message(WARNING
            "bobui_internal_add_executable's PUBLIC_LIBRARIES option is deprecated, and will be "
    ...
    if(NOT arg_EXCEPTIONS)
        bobui_internal_set_exceptions_flags("${name}" "DEFAULT")
    else()
        bobui_internal_set_exceptions_flags("${name}" "${arg_EXCEPTIONS}")
    endif()

    # Check if target needs to be excluded from all target. Also affects bobui_install.
    # Set by bobui_exclude_tool_directories_from_default_target.
    set(exclude_from_all FALSE)
    if(__bobui_exclude_tool_directories)
        foreach(absolute_dir ${__bobui_exclude_tool_directories})
            _bobui_internal_path_is_prefix(absolute_dir "${CMAKE_CURRENT_SOURCE_DIR}"
                in_current_source)
            if(in_current_source)
                set(exclude_from_all TRUE)
                set_target_properties("${name}" PROPERTIES
                    EXCLUDE_FROM_ALL TRUE
                    _bobui_internal_excluded_from_default_target TRUE
                )
                break()
            endif()
        endforeach()
    ...
    if(NOT arg_NO_INSTALL)
        ...
        foreach(cmake_config ${cmake_configs})
            ...
            if(NOT exclude_from_all AND arg_BOBUI_APP AND BOBUI_FEATURE_debug_and_release
        ...
        if(NOT exclude_from_all AND arg_BOBUI_APP AND BOBUI_FEATURE_debug_and_release)
            set(separate_debug_info_executable_arg "BOBUI_EXECUTABLE")
        else()
            unset(separate_debug_info_executable_arg)
        endif()

        bobui_internal_defer_separate_debug_info("${name}"
        ...
        bobui_internal_install_pdb_files(${name} "${arg_INSTALL_DIRECTORY}")
    endif()

    bobui_add_list_file_finalizer(bobui_internal_finalize_executable "${name}")
endfunction()

# Finalizer for all generic internal executables.
function(bobui_internal_finalize_executable target)
    bobui_internal_finalize_executable_separate_debug_info("${target}")
endfunction()
...
#         for an executable. By default the predefined template from the BobUI installation is used.
#     INSTALL_DIRECTORY
...
function(bobui_internal_add_configure_time_executable target)
    set(one_value_args
        CMAKELISTS_TEMPLATE
        INSTALL_DIRECTORY
        OUTPUT_NAME
    ...
    set(output_directory "${BOBUI_BUILD_DIR}/${install_dir}")
    ...
    if(BOBUI_INTERNAL_HAVE_CONFIGURE_TIME_${target} AND
    ...
    if(NOT "${BOBUI_INTERNAL_CMAKE_FLAGS_CONFIGURE_TIME_TOOL_${target}}" STREQUAL "${cmake_flags_arg}")
        set(should_build_at_configure_time TRUE)
    endif()
    ...
    if(should_build_at_configure_time)
        ...
        set(template "${BOBUI_CMAKE_DIR}/BobUIConfigureTimeExecutableCMakeLists.txt.in")
        ...
        set(BOBUI_INTERNAL_CMAKE_FLAGS_CONFIGURE_TIME_TOOL_${target}
            "${cmake_flags_arg}" CACHE INTERNAL "")

        file(WRITE "${timestamp_file}" "")
        set(BOBUI_INTERNAL_HAVE_CONFIGURE_TIME_${target} ${result} CACHE INTERNAL
    ...
    add_executable(${BOBUI_CMAKE_EXPORT_NAMESPACE}::${target} ALIAS ${target})
    set_target_properties(${target} PROPERTIES
        _bobui_internal_configure_time_target TRUE
        _bobui_internal_configure_time_target_build_location "${target_binary_path_relative}"
    ...
    if(NOT arg_NO_INSTALL)
        ...
        bobui_path_join(target_install_dir ${BOBUI_INSTALL_DIR} ${install_dir})
        bobui_install(PROGRAMS "${target_binary_path}" DESTINATION "${target_install_dir}")
    endif()
endfunction()
