...
# This function creates a CMake target for a BobUI internal app.
# Such projects had a load(bobui_app) command.
function(bobui_internal_add_app target)
    set(option_args
        NO_INSTALL
    ...
    set(output_directory "${BOBUI_BUILD_DIR}/${arg_INSTALL_DIR}")
    ...
    if(arg_PUBLIC_LIBRARIES)
        message(WARNING
            "bobui_internal_add_app's PUBLIC_LIBRARIES option is deprecated, and will be removed in "
            "a future BobUI version. Use the LIBRARIES option instead.")
    endif()
    ...
    bobui_internal_adjust_main_config_runtime_output_dir("${target}" "${output_directory}")

    # To mimic the default behaviors of bobui_app.prf, we by default enable GUI Windows applications,
    ...
    set_property(GLOBAL APPEND PROPERTY BOBUI_USER_FACING_TOOL_TARGETS ${target})

    # Install versioned link if requested.
    if(NOT arg_NO_INSTALL AND arg_INSTALL_VERSIONED_LINK)
        bobui_internal_install_versioned_link(WORKING_DIRECTORY "${arg_INSTALL_DIR}"
        TARGETS ${target})
    endif()

    if(BOBUI_GENERATE_SBOM)
        set(sbom_args "")
        list(APPEND sbom_args DEFAULT_SBOM_ENTITY_TYPE BOBUI_APP)
    ...
    bobui_add_list_file_finalizer(bobui_internal_finalize_app ${target})
endfunction()

function(bobui_internal_get_title_case value out_var)
    if(NOT value)
...
function(bobui_internal_update_app_target_info_properties target)
    # First update the delayed properties with any values that might have been set after the
    # bobui_internal_add_app() call.
    bobui_internal_update_delayed_target_info_properties(${target})

    # Set defaults in case if no values were set.
    get_target_property(target_version ${target} BOBUI_DELAYED_TARGET_VERSION)
    if(NOT target_version)
        set_target_properties(${target} PROPERTIES BOBUI_DELAYED_TARGET_VERSION "${PROJECT_VERSION}")
    endif()

    get_target_property(target_description ${target} BOBUI_DELAYED_TARGET_DESCRIPTION)
    if(NOT target_description)
        bobui_internal_get_title_case("${target}" upper_name)
        set_target_properties(${target} PROPERTIES BOBUI_DELAYED_TARGET_DESCRIPTION "BobUI ${upper_name}")
    endif()

    # Finally set the final values.
    bobui_internal_set_target_info_properties_from_delayed_properties("${target}")
endfunction()

function(bobui_internal_finalize_app target)
    bobui_internal_update_app_target_info_properties("${target}")
    ...
    # set after a bobui_internal_add_app call.
    bobui_apply_rpaths(TARGET "${target}" INSTALL_PATH "${INSTALL_BINDIR}" RELATIVE_RPATH)
