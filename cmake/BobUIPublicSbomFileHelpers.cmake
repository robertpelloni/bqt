...
function(_bobui_internal_sbom_handle_target_binary_files target)
    set(opt_args
        NO_INSTALL
        FRAMEWORK
    )
    ...
    if(BOBUI_SBOM_SKIP_BINARY_FILES)
        message(DEBUG "Skipping sbom target file processing ${target} because "
            "BOBUI_SBOM_SKIP_BINARY_FILES is set")
        return()
    endif()
    ...
    set(supported_types
        ...
        # This will be meant for user projects, and are not currently used by BobUI's sbom.
        THIRD_PARTY_LIBRARY
    ...
    get_target_property(excluded_via_property ${target} _bobui_internal_excluded_from_default_target)
    if(excluded_via_property OR BOBUI_INTERNAL_TEST_TARGETS_EXCLUDE_FROM_ALL)
        message(DEBUG "Target ${target} has no binary files to reference in the SBOM "
    ...
    if(arg_SBOM_ENTITY_TYPE STREQUAL "BOBUI_TOOL"
            OR arg_SBOM_ENTITY_TYPE STREQUAL "BOBUI_APP"
        ...
        if(app_is_bundle)
            _bobui_internal_get_executable_bundle_info(bundle "${target}")
            _bobui_internal_path_join(path_suffix "${bundle_contents_binary_dir}" "${path_suffix}")
        endif()

        _bobui_internal_sbom_handle_multi_config_target_binary_file(${target}
            PATH_KIND RUNTIME_PATH
            PATH_SUFFIX "${path_suffix}"
            OPTIONS ${file_common_options}
        )
    elseif(arg_SBOM_ENTITY_TYPE STREQUAL "BOBUI_PLUGIN")
    ...
    elseif(arg_SBOM_ENTITY_TYPE STREQUAL "BOBUI_MODULE"
            OR arg_SBOM_ENTITY_TYPE STREQUAL "BOBUI_THIRD_PARTY_MODULE"
...
# TODO: Consider merging the common parts with _bobui_internal_sbom_add_custom_file somehow.
function(_bobui_internal_sbom_add_binary_file target file_path)
    set(opt_args
        OPTIONAL
        IMPORT_LIBRARY
    )
    ...
    _bobui_internal_sbom_get_package_infix("${arg_PACKAGE_TYPE}" package_infix)

    _bobui_internal_sbom_get_file_spdx_id(
    ...
    if(NOT BOBUI_SBOM_SKIP_SOURCE_FILES)
        _bobui_internal_sbom_add_target_source_files("${target}" "${spdx_id}" source_relationships)
        ...
        message(DEBUG "Skipping sbom source file processing for file '${file_path}' because "
            "BOBUI_SBOM_SKIP_SOURCE_FILES is set")
    endif()
    ...
    _bobui_internal_sbom_generate_add_file(
        FILENAME "${file_path}"
...
# _bobui_internal_sbom_handle_target_custom_file_set.
function(_bobui_internal_sbom_add_files target)
    ...
    get_target_property(file_sets_count "${target}" _bobui_sbom_custom_file_sets_count)
    if(NOT file_sets_count)
        set(file_sets_count 0)
    endif()

    set_property(TARGET "${target}"
        APPEND PROPERTY _bobui_sbom_custom_file_set_${file_sets_count} "${ARGN}")

    math(EXPR file_sets_count "${file_sets_count}+1")
    set_property(TARGET "${target}" PROPERTY _bobui_sbom_custom_file_sets_count "${file_sets_count}")
endfunction()
...
#
# INSTALL_PREFIX - the install prefix for the files, this is usually the install prefix of bobui.
#
# COPYRIGHTS - a list of copyright strings to apply to the files.
function(_bobui_internal_sbom_handle_target_custom_files target)
    set(opt_args
        NO_INSTALL
    )
    set(single_args
    ...
    if(BOBUI_SBOM_SKIP_CUSTOM_FILES)
    ...
    foreach(file_set_index RANGE ${file_sets_count})
        get_target_property(file_set_args "${target}" _bobui_sbom_custom_file_set_${file_set_index})
        ...
        _bobui_internal_sbom_handle_target_custom_file_set("${target}"
...
#   INSTALL_PREFIX
#   COPYRIGHTS
# options are the same as in _bobui_internal_sbom_handle_target_custom_files, and are actually
# forwarded from that function, because they might be set at the target level.
#
# In addition, more options can be passed when called via bobui_internal_sbom_add_files:
#
# They are:
#
# FILE_TYPE - the type of each provided file. Supported types can be found in the implementation of
# _bobui_internal_sbom_get_spdx_v2_3_file_type_for_file().
# Some examples are BOBUI_TRANSLATION, BOBUI_TRANSLATIONS_CATALOG, BOBUI_RESOURCE.
#
...
function(_bobui_internal_sbom_handle_target_custom_file_set target)
    set(opt_args
        ""
    )
    set(single_args
    ...
    foreach(relative_file_path IN LISTS relative_file_paths)
        ...
        _bobui_internal_sbom_handle_multi_config_custom_file(${target}
            PATH_KIND "INSTALL_PATH"
...
#_bobui_internal_sbom_handle_target_custom_file_set.
function(_bobui_internal_sbom_handle_multi_config_custom_file target)
    set(opt_args "")
    set(single_args
        PATH_KIND
        PATH_SUFFIX
    ...
    foreach(config IN LISTS configs)
        _bobui_internal_sbom_get_and_check_multi_config_aware_single_arg_option(
            arg "${arg_PATH_KIND}" "${config}" resolved_path)
        _bobui_internal_sbom_get_target_file_is_optional_in_multi_config("${config}" is_optional)
        _bobui_internal_path_join(file_path "${resolved_path}" "${arg_PATH_SUFFIX}")
        _bobui_internal_sbom_add_custom_file(
            "${target}"
            "${file_path}"
            ${arg_OPTIONS}
            ${is_optional}
...
function(_bobui_internal_sbom_add_custom_file target installed_file_relative_path)
    set(opt_args
        OPTIONAL
    )
    set(single_args
    ...
    if(arg_FILE_TYPE)
        _bobui_internal_sbom_get_spdx_v2_3_file_type_for_file(file_type "${arg_FILE_TYPE}")
    else()
        set(file_type "OTHER")
    endif()
    ...
    # Add source file relationships from which the binary file was generated.
    if(NOT BOBUI_SBOM_SKIP_SOURCE_FILES)
        _bobui_internal_sbom_add_source_files(
            ${sources_option}
            SPDX_ID "${spdx_id}"
        ...
        message(DEBUG "Skipping sbom source file processing for file "
            "'${installed_file_relative_path}' because BOBUI_SBOM_SKIP_SOURCE_FILES is set")
    endif()
...
# There is a list of known SPDX types, and custom BobUI ones.
# Any other type is mapped to OTHER.
# The mapping might change when we start generating spdx v3.0 documents.
function(_bobui_internal_sbom_get_spdx_v2_3_file_type_for_file out_var file_type_in)
    ...
    set(bobui_file_types
        BOBUI_TRANSLATION
        BOBUI_TRANSLATIONS_CATALOG
        BOBUI_RESOURCE
        TRANSLATION
        RESOURCE
        CUSTOM
    ...
    elseif(file_type_in IN_LIST bobui_file_types)
        set(file_type OTHER)
    else()
        set(file_type OTHER)
    endif()
...
function(_bobui_internal_sbom_map_path_to_reproducible_relative_path out_var)
    set(opt_args "")
    set(single_args
        PATH
    ...
    if(NOT arg_REPO_PROJECT_NAME_LOWERCASE)
        _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name)
    else()
    ...
    if(path MATCHES "$<.+>")
        ...
        if(IS_ABSOLUTE "${path}")
            ...
            _bobui_internal_path_is_prefix(PROJECT_SOURCE_DIR "${path}" is_in_source_dir)
            if(NOT is_in_source_dir)
                _bobui_internal_path_is_prefix(PROJECT_BINARY_DIR "${path}" is_in_build_dir)
            endif()
            if(NOT is_in_source_dir AND NOT is_in_build_dir)
                _bobui_internal_path_is_prefix(CMAKE_INSTALL_PREFIX "${path}" is_in_prefix_dir)
            endif()
        else()
        ...
        elseif(is_in_prefix_dir)
            # In a non-prefix build, we put synced headers into the bobuibase build dir, aka the prefix
...
function(_bobui_internal_sbom_add_target_source_files target spdx_id out_relationships)
...
function(_bobui_internal_sbom_add_source_files)
    set(opt_args "")
    set(single_args
        SPDX_ID
    ...
    foreach(source IN LISTS sources)
        ...
        _bobui_internal_sbom_map_path_to_reproducible_relative_path(source_path
            PATH "${source}"
...
function(_bobui_internal_get_executable_bundle_info out_var target)
    get_target_property(target_type ${target} TYPE)
...
function(_bobui_internal_sbom_handle_multi_config_target_binary_file target)
    set(opt_args "")
    set(single_args
        PATH_KIND
        PATH_SUFFIX
    ...
    foreach(config IN LISTS configs)
        ...
        _bobui_internal_sbom_get_target_file_is_optional_in_multi_config("${config}" is_optional)
        _bobui_internal_path_join(file_path "${resolved_path}" "${arg_PATH_SUFFIX}")
        _bobui_internal_sbom_add_binary_file(
            "${target}"
            "${file_path}"
            ${arg_OPTIONS}
            ${is_optional}
...
function(_bobui_internal_sbom_get_multi_config_single_args out_var)
    get_cmake_property(single_args
        _bobui_internal_sbom_multi_config_single_args)
    ...
    set_property(GLOBAL PROPERTY
        _bobui_internal_sbom_multi_config_single_args "${single_args}")
    set(${out_var} ${single_args} PARENT_SCOPE)
endfunction()
...
function(_bobui_internal_sbom_append_multi_config_aware_single_arg_option
...
function(_bobui_internal_sbom_get_and_check_multi_config_aware_single_arg_option
...
function(_bobui_internal_sbom_is_config_optional_in_multi_config config out_var)
    get_cmake_property(is_multi_config GENERATOR_IS_MULTI_CONFIG)

    if(BOBUI_MULTI_CONFIG_FIRST_CONFIG)
        set(first_config_type "${BOBUI_MULTI_CONFIG_FIRST_CONFIG}")
...
function(_bobui_internal_sbom_get_target_file_is_optional_in_multi_config config out_var)
    _bobui_internal_sbom_is_config_optional_in_multi_config("${config}" is_optional)
...
function(_bobui_internal_sbom_get_file_spdx_id target out_var)
    _bobui_internal_sbom_get_sanitized_spdx_id(spdx_id "SPDXRef-PackagedFile-${target}")
