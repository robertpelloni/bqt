...
# bobui_configure_file(OUTPUT output-file <INPUT input-file | CONTENT content>)
...
function(bobui_configure_file)
    ...
    if("CONTENT" IN_LIST ARGV)
        ...
        _bobui_internal_configure_file(CONFIGURE OUTPUT "${arg_OUTPUT}" CONTENT "${arg_CONTENT}")
    elseif(arg_INPUT)
        _bobui_internal_configure_file(CONFIGURE OUTPUT "${arg_OUTPUT}" INPUT "${arg_INPUT}")
    else()
...
#
# TODO: Remove when all usage of bobui_parse_all_arguments were replaced by
# cmake_parse_all_arguments(PARSEARGV) instances
macro(bobui_parse_all_arguments result type flags options multiopts)
...
macro(bobui_debug_print_variables)
...
function(bobui_path_join out_var)
    _bobui_internal_path_join(path ${ARGN})
    set(${out_var} ${path} PARENT_SCOPE)
endfunction()

function(bobui_remove_args out_var)
    _bobui_internal_remove_args(result ${ARGN})
...
function(bobui_re_escape out_var str)
...
# comparison.
function(bobui_create_case_insensitive_regex out_var input)
    set(result "^(")
...
function(bobui_internal_get_target_property out_var target property)
...
function(bobui_internal_write_bobui_package_version_file package_name out_path)
    set(extra_code "")

    # Need to check for FEATURE_developer_build as well, because BOBUI_FEATURE_developer_build is not
...
# Disabling version check because BobUI was configured with -developer-build.
set(__bobui_disable_package_version_check TRUE)
set(__bobui_disable_package_version_check_due_to_developer_build TRUE)")
    endif()

    configure_file("${BOBUI_CMAKE_DIR}/BobUICMakePackageVersionFile.cmake.in" "${out_path}" @ONLY)
endfunction()
