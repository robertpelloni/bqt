...
# Computes which sanitizer options should be set based on features evaluated in bobuibase.
# Sets ECM_ENABLE_SANITIZERS with those options in the function calling scope.
function(bobui_internal_set_up_sanitizer_options)
    ...
    # If BOBUI_FEATURE_sanitize_foo was enabled, make sure to set the appropriate
    # ECM_ENABLE_SANITIZERS value.
    set(enabled_sanitizer_features "")
    foreach(sanitizer_type address memory thread undefined)
        if(BOBUI_FEATURE_sanitize_${sanitizer_type})
            list(APPEND enabled_sanitizer_features "${sanitizer_type}")
        endif()
    endforeach()

    # There's a mismatch between fuzzer-no-link ECM option and fuzzer_no_link BobUI feature.
    if(BOBUI_FEATURE_sanitize_fuzzer_no_link)
...
function(bobui_internal_skip_sanitizer)
    if(CMAKE_CXX_COMPILER_ID MATCHES "GNU" OR
        ...
        foreach(sanitizer ${ECM_ENABLE_SANITIZERS})
            ...
            bobui_internal_remove_compiler_flags(${XSAN_COMPILE_FLAGS})
        endforeach()
...
function(bobui_internal_skip_linking_sanitizer)
    _bobui_internal_collect_buildsystem_targets(all_targets "${CMAKE_CURRENT_SOURCE_DIR}"
        INCLUDE
            STATIC_LIBRARY
            MODULE_LIBRARY
            SHARED_LIBRARY
