...
# CycloneDX formats.
function(_bobui_internal_sbom_get_common_project_variables)
    set(opt_args "")
    set(single_args
        # Forwarded
        OUTPUT
    ...
    _bobui_internal_sbom_set_default_option_value(PROJECT "${PROJECT_NAME}")
    set(${arg_OUT_VAR_PROJECT_NAME} "${arg_PROJECT}" PARENT_SCOPE)

    _bobui_internal_sbom_get_git_version_vars()

    _bobui_internal_path_join(default_sbom_file_name
        "${arg_PROJECT}"
        "${arg_PROJECT}-sbom-${BOBUI_SBOM_GIT_VERSION_PATH}.${arg_DEFAULT_SBOM_FILE_NAME_EXTENSION}")
    _bobui_internal_path_join(default_install_sbom_path
        "\${CMAKE_INSTALL_PREFIX}/" "${CMAKE_INSTALL_DATAROOTDIR}" "${default_sbom_file_name}"
    )

    _bobui_internal_sbom_set_default_option_value(OUTPUT "${default_install_sbom_path}")
    _bobui_internal_sbom_set_default_option_value(OUTPUT_RELATIVE_PATH
    ...
    _bobui_internal_sbom_set_default_option_value(PROJECT_FOR_SPDX_ID "Package-${arg_PROJECT}")
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(SUPPLIER "")
    set(${arg_OUT_VAR_SUPPLIER} "${arg_SUPPLIER}" PARENT_SCOPE)

    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(SUPPLIER_URL
        "${PROJECT_HOMEPAGE_URL}")
    set(${arg_OUT_VAR_SUPPLIER_URL} "${arg_SUPPLIER_URL}" PARENT_SCOPE)

    _bobui_internal_sbom_set_default_option_value(COPYRIGHT "${current_year} ${arg_SUPPLIER}")
...
function(_bobui_internal_sbom_save_common_path_variables_in_global_properties)
    set(opt_args "")
    set(single_args
        OUTPUT
        OUTPUT_RELATIVE_PATH
    ...
    # in the bobuibase build dir.
    if(BOBUI_BUILDING_BOBUI AND (BOBUI_SUPERBUILD OR (NOT BOBUI_WILL_INSTALL)))
        set(build_sbom_root_dir "${BOBUI_BUILD_DIR}")
    else()
    ...
    set_property(GLOBAL PROPERTY _bobui_sbom_build_output_path${suffix} "${build_sbom_path}")
    set_property(GLOBAL PROPERTY _bobui_sbom_build_output_path_without_ext${suffix}
        "${build_sbom_path_without_ext}")
    set_property(GLOBAL PROPERTY _bobui_sbom_build_output_dir${suffix} "${build_sbom_dir}")

    set_property(GLOBAL PROPERTY _bobui_sbom_install_output_path${suffix} "${install_sbom_path}")
    set_property(GLOBAL PROPERTY _bobui_sbom_install_output_path_without_ext${suffix}
        "${install_sbom_path_without_ext}")
    set_property(GLOBAL PROPERTY _bobui_sbom_install_output_dir${suffix} "${install_sbom_dir}")
endfunction()

# Helper function to get SBOM project path values like relative build and install dirs,
function(_bobui_internal_sbom_get_common_path_variables_from_global_properties)
    set(opt_args "")
    set(single_args
        SBOM_FORMAT
    ...
    get_property(sbom_build_output_path GLOBAL PROPERTY _bobui_sbom_build_output_path${suffix})
    get_property(sbom_build_output_path_without_ext GLOBAL PROPERTY
        _bobui_sbom_build_output_path_without_ext${suffix})
    get_property(sbom_build_output_dir GLOBAL PROPERTY _bobui_sbom_build_output_dir${suffix})

    get_property(sbom_install_output_path GLOBAL PROPERTY _bobui_sbom_install_output_path${suffix})
    get_property(sbom_install_output_path_without_ext GLOBAL PROPERTY
        _bobui_sbom_install_output_path_without_ext${suffix})
    get_property(sbom_install_output_dir GLOBAL PROPERTY _bobui_sbom_install_output_dir${suffix})
...
function(_bobui_internal_sbom_create_sbom_staging_file)
    set(opt_args "")
    set(single_args
        CONTENT
        SBOM_FORMAT
    ...
    if(arg_SBOM_FORMAT STREQUAL "SPDX_V2")
        ...
        set(suffix "")
        set(extra_content "
        set(BOBUI_SBOM_EXTERNAL_DOC_REFS \"\")
")
        _bobui_internal_get_staging_area_spdx_file_path(staging_area_file)
    ...
    elseif(arg_SBOM_FORMAT STREQUAL "CYDX_V1_6")
        ...
        set(suffix "_cydx")
        set(extra_content "")
        _bobui_internal_get_staging_area_cydx_file_path(staging_area_file)
...
# properties.
function(_bobui_internal_sbom_save_project_info_in_global_properties)
    set(opt_args "")
    set(single_args
        SUPPLIER
        SUPPLIER_URL
    ...
    set_property(GLOBAL PROPERTY _bobui_sbom_project_supplier "${arg_SUPPLIER}")
    set_property(GLOBAL PROPERTY _bobui_sbom_project_supplier_url "${arg_SUPPLIER_URL}")
    set_property(GLOBAL PROPERTY _bobui_sbom_project_namespace "${arg_NAMESPACE}")

    set_property(GLOBAL PROPERTY _bobui_sbom_project_name "${arg_PROJECT}")
    set_property(GLOBAL PROPERTY _bobui_sbom_project_spdx_id "${arg_PROJECT_SPDX_ID}")
endfunction()

# Helper function to get cmake include files for SBOM generation from global properties.
function(_bobui_internal_sbom_get_cmake_include_files)
    set(opt_args "")
    set(single_args
        SBOM_FORMAT
        OUT_VAR_INCLUDES
    ...
    _bobui_internal_sbom_collect_cmake_include_files(includes
        JOIN_WITH_NEWLINES
        PROPERTIES _bobui_sbom_cmake_include_files${suffix} _bobui_sbom_cmake_end_include_files${suffix}
    )
    ...
    _bobui_internal_sbom_collect_cmake_include_files(before_checksum_includes
        JOIN_WITH_NEWLINES
        PROPERTIES _bobui_sbom_cmake_before_checksum_include_files${suffix}
    )

    # After checksum includes are included after the checksum has been computed and written to the
    # BOBUI_SBOM_VERIFICATION_CODE variable.
    _bobui_internal_sbom_collect_cmake_include_files(after_checksum_includes
        JOIN_WITH_NEWLINES
        PROPERTIES _bobui_sbom_cmake_after_checksum_include_files${suffix}
    )

    # Post generation includes are included for both build and install time sboms, after
    # sbom generation has finished.
    _bobui_internal_sbom_collect_cmake_include_files(post_generation_includes
        JOIN_WITH_NEWLINES
        PROPERTIES _bobui_sbom_cmake_post_generation_include_files${suffix}
    )

    # Verification only makes sense on installation, where the checksums are present.
    _bobui_internal_sbom_collect_cmake_include_files(verify_includes
        JOIN_WITH_NEWLINES
        PROPERTIES _bobui_sbom_cmake_verify_include_files${suffix}
    )
...
function(_bobui_internal_sbom_clear_cmake_include_files)
    set(opt_args "")
    set(single_args
        SBOM_FORMAT
    )
    ...
    set_property(GLOBAL PROPERTY _bobui_sbom_cmake_include_files${suffix} "")
    set_property(GLOBAL PROPERTY _bobui_sbom_cmake_end_include_files${suffix} "")
    set_property(GLOBAL PROPERTY _bobui_sbom_cmake_before_checksum_include_files${suffix} "")
    set_property(GLOBAL PROPERTY _bobui_sbom_cmake_after_checksum_include_files${suffix} "")
    set_property(GLOBAL PROPERTY _bobui_sbom_cmake_post_generation_include_files${suffix} "")
    set_property(GLOBAL PROPERTY _bobui_sbom_cmake_verify_include_files${suffix} "")
endfunction()
...
# sbom generation.
function(_bobui_internal_sbom_create_build_time_sbom_targets)
    set(opt_args "")
    set(single_args
        SBOM_FORMAT
        REPO_PROJECT_NAME_LOWERCASE
        REAL_BOBUI_REPO_PROJECT_NAME_LOWERCASE
    ...
    if(arg_SBOM_FORMAT STREQUAL "SPDX_V2")
        set(suffix "")
        set(extra_content "
        set(BOBUI_SBOM_PACKAGES \"\")
        set(BOBUI_SBOM_PACKAGES_WITH_VERIFICATION_CODES \"\")
")
        _bobui_internal_get_staging_area_spdx_file_path(staging_area_file)
    ...
    _bobui_internal_get_current_project_sbom_dir(sbom_dir)
    set(content "
        # BOBUI_SBOM_BUILD_TIME be set to FALSE at install time, so don't override if it's set.
        # This allows reusing the same cmake file for both build and install.
        if(NOT DEFINED BOBUI_SBOM_BUILD_TIME)
            set(BOBUI_SBOM_BUILD_TIME TRUE)
        endif()
        if(NOT BOBUI_SBOM_OUTPUT_PATH)
            set(BOBUI_SBOM_OUTPUT_DIR \"${arg_SBOM_BUILD_OUTPUT_DIR}\")
            set(BOBUI_SBOM_OUTPUT_PATH \"${arg_SBOM_BUILD_OUTPUT_PATH}\")
            set(BOBUI_SBOM_OUTPUT_PATH_WITHOUT_EXT \"${arg_SBOM_BUILD_OUTPUT_PATH_WITHOUT_EXT}\")
            file(MAKE_DIRECTORY \"${arg_SBOM_BUILD_OUTPUT_DIR}\")
        endif()
        ${extra_content}
        ${arg_INCLUDES}
        if(BOBUI_SBOM_BUILD_TIME)
            message(STATUS \"${final_message}: \${BOBUI_SBOM_OUTPUT_PATH}\")
            configure_file(\"${staging_area_file}\" \"\${BOBUI_SBOM_OUTPUT_PATH}\")
...
")
    ...
    get_cmake_property(bobui_repo_deps _bobui_repo_deps_${arg_REAL_BOBUI_REPO_PROJECT_NAME_LOWERCASE})
    if(bobui_repo_deps)
        foreach(repo_dep IN LISTS bobui_repo_deps)
...
function(_bobui_internal_sbom_setup_multi_config_install_markers)
    set(opt_args "")
    set(single_args
        SBOM_DIR
        SBOM_FORMAT
...
")
    ...
    # _bobui_internal_sbom_setup_sbom_install_code when they are forwarded there.
    string(REPLACE ";" "\\;" install_markers "${install_markers}")

    set(extra_code_begin "
    set(BOBUI_SBOM_INSTALL_MARKERS${suffix} \"${install_markers}\")
    foreach(BOBUI_SBOM_INSTALL_MARKER IN LISTS BOBUI_SBOM_INSTALL_MARKERS${suffix})
        if(NOT EXISTS \"\${BOBUI_SBOM_INSTALL_MARKER}\")
            set(BOBUI_SBOM_INSTALLED_ALL_CONFIGS${suffix} FALSE)
        endif()
    endforeach()
")
    set(extra_code_inner_end "
        foreach(BOBUI_SBOM_INSTALL_MARKER IN LISTS BOBUI_SBOM_INSTALL_MARKERS${suffix})
            message(STATUS
                \"Removing install marker: \${BOBUI_SBOM_INSTALL_MARKER} \")
            file(REMOVE \"\${BOBUI_SBOM_INSTALL_MARKER}\")
        endforeach()
")
...
function(_bobui_internal_sbom_setup_fake_checksum)
    set(opt_args "")
    set(single_args
    ...
    if(BOBUI_SBOM_FAKE_CHECKSUM)
        string(APPEND extra_code "
            set(BOBUI_SBOM_FAKE_CHECKSUM TRUE)")
    endif()
...
function(_bobui_internal_sbom_setup_sbom_install_code)
    set(opt_args "")
    set(single_args
        SBOM_FORMAT
        REPO_PROJECT_NAME_LOWERCASE
...
")
    ...
    set(assemble_sbom_install "
        set(BOBUI_SBOM_INSTALLED_ALL_CONFIGS${suffix} TRUE)
        ${arg_EXTRA_CODE_BEGIN}
        if(BOBUI_SBOM_INSTALLED_ALL_CONFIGS${suffix})
            set(BOBUI_SBOM_BUILD_TIME FALSE)
            set(BOBUI_SBOM_OUTPUT_DIR \"${arg_SBOM_INSTALL_OUTPUT_DIR}\")
            set(BOBUI_SBOM_OUTPUT_PATH \"${arg_SBOM_INSTALL_OUTPUT_PATH}\")
            set(BOBUI_SBOM_OUTPUT_PATH_WITHOUT_EXT \"${arg_SBOM_INSTALL_OUTPUT_PATH_WITHOUT_EXT}\")
