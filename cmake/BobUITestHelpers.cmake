...
# Simple wrapper around bobui_internal_add_executable for benchmarks which insure that
# the binary is built under ${CMAKE_CURRENT_BINARY_DIR} and never installed.
# See bobui_internal_add_executable() for more details.
function(bobui_internal_add_benchmark target)
    if(BOBUI_BUILD_TESTS_BATCHED)
        message(WARNING "Benchmarks won't be batched - unsupported (yet)")
    endif()

    cmake_parse_arguments(PARSE_ARGV 1 arg
        "${__bobui_internal_add_executable_optional_args}"
        "${__bobui_internal_add_executable_single_args}"
        "${__bobui_internal_add_executable_multi_args}"
    )
    _bobui_internal_validate_all_args_are_parsed(arg)
    _bobui_internal_validate_no_unity_build(arg)

    bobui_remove_args(exec_args
        ARGS_TO_REMOVE
            ${target}
            OUTPUT_DIRECTORY
            INSTALL_DIRECTORY
    ...
    bobui_internal_add_executable(${target}
    ...
    # Disable the BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT define for benchmarks
    bobui_internal_undefine_global_definition(${target} BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT)

    bobui_internal_collect_command_environment(benchmark_env_path benchmark_env_plugin_path)
    ...
    _bobui_internal_create_command_script(COMMAND "$<TARGET_FILE:${target}>"
                                      OUTPUT_FILE "${benchmark_wrapper_file}"
                                      ENVIRONMENT "PATH" "${benchmark_env_path}"
                                                  "BOBUI_PLUGIN_PATH" "${benchmark_env_plugin_path}"
    )
    ...
    bobui_internal_add_test_finalizers("${target}")
endfunction()

function(bobui_internal_add_test_dependencies target)
    if(BOBUI_BUILD_TESTS_BATCHED)
...
# Simple wrapper around bobui_internal_add_executable for manual tests which insure that
# the binary is built under ${CMAKE_CURRENT_BINARY_DIR} and never installed.
# See bobui_internal_add_executable() for more details.
function(bobui_internal_add_manual_test target)
    bobui_internal_add_test(${ARGV} MANUAL)
endfunction()

macro(bobui_internal_skip_docker_compose)
    set(BOBUI_SKIP_DOCKER_COMPOSE ON CACHE BOOL "Skip setting docker on Linux." FORCE)
endmacro()

# This function will configure the fixture for the network tests that require docker network services
# qmake counterpart: bobuibase/mkspecs/features/unsupported/testserver.prf
function(bobui_internal_setup_docker_test_fixture name)
    # Only Linux is provisioned with docker at this time
    if (NOT CMAKE_HOST_SYSTEM_NAME STREQUAL "Linux")
      return()
    endif()

    option(BOBUI_SKIP_DOCKER_COMPOSE "Skip setting up docker on Linux." OFF)
    if(BOBUI_SKIP_DOCKER_COMPOSE)
      return()
    endif()

    set(BOBUI_TEST_SERVER_LIST ${ARGN})
    set(DNSDOMAIN test-net.bobui.local)

    find_program(BOBUI_DOCKER_COMPOSE docker-compose)
    if (NOT BOBUI_DOCKER_COMPOSE)
        message(WARNING "docker-compose was not found. Docker network tests will not be run.")
        bobui_internal_skip_docker_compose()
        return()
    endif()
    if (NOT DEFINED BOBUI_DOCKER_COMPOSE_VERSION)
      execute_process(COMMAND "${BOBUI_DOCKER_COMPOSE}" --version OUTPUT_VARIABLE BOBUI_DOCKER_COMPOSE_VERSION)
      string(REPLACE "\n" "" BOBUI_DOCKER_COMPOSE_VERSION "${BOBUI_DOCKER_COMPOSE_VERSION}")
      set(BOBUI_DOCKER_COMPOSE_VERSION "${BOBUI_DOCKER_COMPOSE_VERSION}" CACHE STRING "docker compose version")
    endif()

    find_program(BOBUI_DOCKER docker)
    if (NOT BOBUI_DOCKER)
        message(WARNING "docker was not found. Docker network tests will not be run.")
        bobui_internal_skip_docker_compose()
        return()
    endif()
    if (NOT DEFINED BOBUI_DOCKER_TEST_SERVER)
        execute_process(COMMAND "${BOBUI_DOCKER}" images -aq "bobui-test-server-*" OUTPUT_VARIABLE BOBUI_DOCKER_TEST_SERVER)
        if (NOT BOBUI_DOCKER_TEST_SERVER)
            message(WARNING
                "Docker image bobui-test-server-* not found.\n"
        ...
        set(BOBUI_DOCKER_TEST_SERVER "ON" CACHE BOOL "docker bobui-test-server-* present")
    endif()

    target_compile_definitions("${name}"
        PRIVATE
            BOBUI_TEST_SERVER BOBUI_TEST_SERVER_NAME BOBUI_TEST_SERVER_DOMAIN=\"${DNSDOMAIN}\"
    )

    if(DEFINED BOBUI_TESTSERVER_COMPOSE_FILE)
        set(TESTSERVER_COMPOSE_FILE ${BOBUI_TESTSERVER_COMPOSE_FILE})
    elseif(QNX OR VXWORKS)
        set(TESTSERVER_COMPOSE_FILE "${BOBUI_SOURCE_TREE}/tests/testserver/docker-compose-qemu-bridge-network.yml")
    else()
        set(TESTSERVER_COMPOSE_FILE "${BOBUI_SOURCE_TREE}/tests/testserver/docker-compose-bridge-network.yml")
    endif()

    # Bring up test servers and make sure the services are ready.
    add_test(NAME ${name}-setup COMMAND
        "${BOBUI_DOCKER_COMPOSE}" -f ${TESTSERVER_COMPOSE_FILE} up --build -d --force-recreate --timeout 1 ${BOBUI_TEST_SERVER_LIST}
    )
    # Stop and remove test servers after testing.
    add_test(NAME ${name}-cleanup COMMAND
        "${BOBUI_DOCKER_COMPOSE}" -f ${TESTSERVER_COMPOSE_FILE} down --timeout 1
    )
    ...
    foreach(test_name ${name} ${name}-setup ${name}-cleanup)
        set_property(TEST "${test_name}" APPEND PROPERTY ENVIRONMENT "testserver=${BOBUI_DOCKER_COMPOSE_VERSION}")
        set_property(TEST "${test_name}" APPEND PROPERTY ENVIRONMENT TEST_DOMAIN=${DNSDOMAIN})
        set_property(TEST "${test_name}" APPEND PROPERTY ENVIRONMENT "SHARED_DATA=${BOBUI_MKSPECS_DIR}/features/data/testserver")
        set_property(TEST "${test_name}" APPEND PROPERTY ENVIRONMENT SHARED_SERVICE=bridge-network)
    endforeach()

endfunction()

function(bobui_internal_get_test_batch out)
    get_property(batched_list GLOBAL PROPERTY _bobui_batched_test_list_property)
    set(${out} ${batched_list} PARENT_SCOPE)
endfunction()

function(bobui_internal_prepare_test_target_flags version_arg exceptions_text gui_text)
    ...
    # BobUI modules get compiled without exceptions enabled by default.
...
function(bobui_internal_get_test_arg_definitions optional_args single_value_args multi_value_args)
    set(${optional_args}
        RUN_SERIAL
        EXCEPTIONS
        NO_EXCEPTIONS
...
function(bobui_internal_add_test_to_batch batch_name name)
    bobui_internal_get_test_arg_definitions(optional_args single_value_args multi_value_args)

    cmake_parse_arguments(
        arg "${optional_args}" "${single_value_args}" "${multi_value_args}" ${ARGN})
    bobui_internal_prepare_test_target_flags(version_arg exceptions_text gui_text ${ARGN})
    ...
    if(NOT TARGET ${target})
        if(${arg_MANUAL})
            set(is_manual "BOBUI_MANUAL_TEST")
        else()
            set(is_manual "")
        endif()
        ...
        bobui_internal_add_executable(${target}
            ...
            SOURCES "${BOBUI_CMAKE_DIR}/qbatchedtestrunner.in.cpp"
            DEFINES BOBUIEST_BATCH_TESTS
            INCLUDE_DIRECTORIES ${private_includes}
            LIBRARIES ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core
                    ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Test
                    ${BOBUI_CMAKE_EXPORT_NAMESPACE}::TestPrivate
                    ...
                    # bobui_internal_add_executable if Gui is present. This should be reevaluated with
                    # multiple batches.
                    ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Gui
        )
        # TODO: BOBUIBUG-131745
        ...
        set_property(TARGET ${target} PROPERTY _bobui_has_exceptions ${arg_EXCEPTIONS})
        set_property(TARGET ${target} PROPERTY _bobui_has_gui ${arg_GUI})
        set_property(TARGET ${target} PROPERTY _bobui_has_lowdpi ${arg_LOWDPI})
        set_property(TARGET ${target} PROPERTY _bobui_version ${version_arg})
    else()
        ...
        # a single binary.
        bobui_internal_get_target_property(
            batch_has_exceptions ${target} _bobui_has_exceptions)
        if(NOT ${batch_has_exceptions} STREQUAL ${arg_EXCEPTIONS})
            bobui_internal_get_test_batch(test_batch_contents)
            message(FATAL_ERROR "Conflicting exceptions declaration between test \
    batch (${test_batch_contents}) and ${name}")
        endif()
        bobui_internal_get_target_property(batch_has_gui ${target} _bobui_has_gui)
    ...
    get_property(batched_test_list GLOBAL PROPERTY _bobui_batched_test_list_property)
    if(NOT batched_test_list)
        set_property(GLOBAL PROPERTY _bobui_batched_test_list_property "")
        set(batched_test_list "")
    endif()
    list(PREPEND batched_test_list ${name})
    set_property(GLOBAL PROPERTY _bobui_batched_test_list_property ${batched_test_list})
    ...
    get_property(batched_test_sources_list GLOBAL PROPERTY _bobui_batched_test_sources_list_property)
    if(NOT batched_test_sources_list)
        set_property(GLOBAL PROPERTY _bobui_batched_test_sources_list_property "")
    ...
    endforeach()
    set_property(GLOBAL PROPERTY _bobui_batched_test_sources_list_property ${batched_test_sources_list})
...
# Checks whether the test 'name' is present in the test batch. See BOBUI_BUILD_TESTS_BATCHED.
# The result of the check is placed in the 'out' variable.
function(bobui_internal_is_in_test_batch out name)
...
function(bobui_internal_is_skipped_test out name)
    get_target_property(is_skipped_test ${name} _bobui_is_skipped_test)
    set(${out} ${is_skipped_test} PARENT_SCOPE)
endfunction()

function(bobui_internal_set_skipped_test name)
    set_target_properties(${name} PROPERTIES _bobui_is_skipped_test TRUE)
endfunction()

function(bobui_internal_is_bobuibase_test out)
    get_filename_component(dir "${CMAKE_CURRENT_BINARY_DIR}" ABSOLUTE)
    set(${out} FALSE PARENT_SCOPE)

    while(TRUE)
        get_filename_component(filename "${dir}" NAME)
        if("${filename}" STREQUAL "bobuibase")
            set(${out} TRUE PARENT_SCOPE)
            break()
        endif()
...
function(bobui_internal_get_batched_test_arguments out testname)
    if(WASM)
...
# You may avoid test wrapping by either passing NO_WRAPPER option or switching BOBUI_NO_TEST_WRAPPERS
...
function(bobui_internal_add_test name)
    bobui_internal_get_test_arg_definitions(optional_args single_value_args multi_value_args)
    ...
    if(BOBUI_BUILD_TESTS_BATCHED AND NOT arg_NO_BATCH AND NOT arg_QMLTEST AND NOT arg_MANUAL
            AND ("${BOBUI_STANDALONE_TEST_PATH}" STREQUAL ""
                 OR DEFINED ENV{BOBUI_BATCH_STANDALONE_TESTS}))
        set(batch_current_test TRUE)
    endif()

    if(batch_current_test OR (BOBUI_BUILD_TESTS_BATCHED AND arg_QMLTEST))
        if (BOBUI_SUPERBUILD OR DEFINED ENV{TESTED_MODULE_COIN})
            set(is_bobuibase_test FALSE)
            if(BOBUI_SUPERBUILD)
                bobui_internal_is_bobuibase_test(is_bobuibase_test)
            elseif($ENV{TESTED_MODULE_COIN} STREQUAL "bobuibase")
                set(is_bobuibase_test TRUE)
            endif()
            if(NOT is_bobuibase_test)
                file(GENERATE OUTPUT "dummy${name}.cpp" CONTENT "int main() { return 0; }")
                # Add a dummy target to tackle some potential problems
                bobui_internal_add_executable(${name} SOURCES "dummy${name}.cpp")
                # Batched tests outside of bobuibase are unsupported and skipped
                bobui_internal_set_skipped_test(${name})
                return()
            endif()
        endif()
    endif()
    ...
    if(arg_PUBLIC_LIBRARIES)
        message(WARNING
            "bobui_internal_add_test's PUBLIC_LIBRARIES option is deprecated, and will be "
            "removed in a future BobUI version. Use the LIBRARIES option instead.")
    endif()

    if(batch_current_test)
        bobui_internal_add_test_to_batch(name ${name} ${ARGN})
    elseif(arg_SOURCES)
        if(BOBUI_BUILD_TESTS_BATCHED AND arg_QMLTEST)
        ...
        bobui_internal_add_executable("${name}"
            ${exceptions_text}
            ${gui_text}
            ${version_arg}
            NO_INSTALL
            ...
            LIBRARIES
                ...
                ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Core
                ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Test
            COMPILE_OPTIONS ${arg_COMPILE_OPTIONS}
            LINK_OPTIONS ${arg_LINK_OPTIONS}
            MOC_OPTIONS ${arg_MOC_OPTIONS}
        ...
        bobui_internal_add_repo_local_defines(${name})

        # Disable the BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT define for tests
        bobui_internal_undefine_global_definition(${name} BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT)
        ...
        # QMLTest specifics
        bobui_internal_extend_target("${name}" CONDITION arg_QMLTEST
            LIBRARIES ${BOBUI_CMAKE_EXPORT_NAMESPACE}::QuickTest
        )

        bobui_internal_extend_target("${name}"
            CONDITION arg_QMLTEST AND NOT ANDROID AND NOT BOBUI_FORCE_BUILTIN_TESTDATA
            DEFINES
                QUICK_TEST_SOURCE_DIR="${CMAKE_CURRENT_SOURCE_DIR}"
        )

        bobui_internal_extend_target("${name}"
            CONDITION arg_QMLTEST AND (ANDROID OR BOBUI_FORCE_BUILTIN_TESTDATA)
            DEFINES
                QUICK_TEST_SOURCE_DIR=":/"
        )

        # Android requires BobUI::Gui so add it by default for tests
        bobui_internal_extend_target("${name}" CONDITION ANDROID
            LIBRARIES ${BOBUI_CMAKE_EXPORT_NAMESPACE}::Gui
        )

        set(blacklist_file "${CMAKE_CURRENT_SOURCE_DIR}/BLACKLIST")
        if(EXISTS ${blacklist_file})
            _bobui_internal_expose_source_file_to_ide("${name}" ${blacklist_file})
        endif()
    endif()
    ...
    if (BOBUI_SUPERBUILD)
        # Prepend repository name for bobui5 builds, so that tests can be run for
    ...
    if (arg_LOWDPI)
        ...
        if (MACOS)
            set_property(TARGET "${name}" PROPERTY MACOSX_BUNDLE_INFO_PLIST "${BOBUI_MKSPECS_DIR}/macx-clang/Info.plist.disable_highdpi")
    ...
    if (ANDROID)
        ...
        # before being killed.
        set(percentage "95")
        bobui_internal_get_android_test_timeout("${arg_TIMEOUT}" "${percentage}" android_timeout)
        ...
        if(arg_BUNDLE_ANDROID_OPENSSL_LIBS)
            if(EXISTS "${OPENSSL_ROOT_DIR}/${CMAKE_ANDROID_ARCH_ABI}/libcrypto_3.so")
                ...
                set_property(TARGET ${name} APPEND PROPERTY BOBUI_ANDROID_EXTRA_LIBS
                    "${OPENSSL_ROOT_DIR}/${CMAKE_ANDROID_ARCH_ABI}/libcrypto_3.so"
                    "${OPENSSL_ROOT_DIR}/${CMAKE_ANDROID_ARCH_ABI}/libssl_3.so")
            elseif(BOBUI_USE_VCPKG AND DEFINED ENV{VCPKG_ROOT})
        ...
        bobui_internal_android_test_runner_arguments("${name}" test_executable extra_test_args)
        list(APPEND extra_test_args "--timeout" "${android_timeout}")

        set(build_environment "")
        if(DEFINED ENV{BOBUI_BUILD_ENVIRONMENT})
            set(build_environment "$ENV{BOBUI_BUILD_ENVIRONMENT}")
        endif()

        if(BOBUI_ENABLE_VERBOSE_DEPLOYMENT OR build_environment STREQUAL "ci")
    ...
    elseif(WASM)
        ...
        list(APPEND extra_test_args "bobuiestname=${testname}")
        ...
        if (NOT BOBUI_FEATURE_wasm_jspi)
        ...
        if(arg_QMLTEST AND NOT arg_SOURCES)
            set(bobui_additional_libexec_paths "")

            if(DEFINED BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH)
                foreach(additional_prefix IN LISTS BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH)
                    set(additional_libexec "${additional_prefix}/${BOBUI6_INSTALL_LIBEXECS}")
                    list(PREPEND bobui_additional_libexec_paths "${additional_libexec}")
                endforeach()
            endif()

            # First look for the scanner in the target bobui libexec dir. We prefer this one
            ...
            find_program(qmltestrunner_executable
                ...
                PATHS "${BOBUI6_INSTALL_PREFIX}/${BOBUI6_INSTALL_LIBEXECS}"
                    ${bobui_additional_libexec_paths}
                NO_DEFAULT_PATH
            )

            # If we don't find it in the paths, fallback to using target names.
            if(NOT qmltestrunner_executable
                    AND TARGET "${BOBUI_CMAKE_EXPORT_NAMESPACE}::qmltestrunner")
                set(qmltestrunner_executable ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qmltestrunner)
            endif()
    ...
    if(NOT arg_MANUAL)
        if(batch_current_test)
            bobui_internal_get_batched_test_arguments(batched_test_args ${testname})
        ...
        bobui_internal_collect_command_environment(test_env_path test_env_plugin_path)
        ...
        if(arg_NO_WRAPPER OR BOBUI_NO_TEST_WRAPPERS)
            if(BOBUI_BUILD_TESTS_BATCHED)
            ...
            set_property(TEST "${testname}" APPEND PROPERTY
                         ENVIRONMENT "PATH=${test_env_path}"
                                     "BOBUI_TEST_RUNNING_IN_CTEST=1"
                                     "BOBUI_PLUGIN_PATH=${test_env_plugin_path}"
            )
        else()
            set(test_wrapper_file "${CMAKE_CURRENT_BINARY_DIR}/${testname}Wrapper$<CONFIG>.cmake")
            bobui_internal_create_test_script(NAME "${testname}"
                                   COMMAND "${test_executable}"
                                   ARGS "${extra_test_args}"
                                   ${add_test_args}
                                   OUTPUT_FILE "${test_wrapper_file}"
                                   ENVIRONMENT "BOBUI_TEST_RUNNING_IN_CTEST" 1
                                               "PATH" "${test_env_path}"
                                               "BOBUI_PLUGIN_PATH" "${test_env_plugin_path}"
            )
        endif()

        if(arg_BOBUI_TEST_SERVER_LIST AND NOT ANDROID)
            bobui_internal_setup_docker_test_fixture(${testname} ${arg_BOBUI_TEST_SERVER_LIST})
        endif()
        ...
        _bobui_internal_make_check_target(${testname} CTEST_TEST_NAME ${name})
    ...
    if(ANDROID OR IOS OR WASM OR INTEGRITY OR arg_BUILTIN_TESTDATA OR BOBUI_FORCE_BUILTIN_TESTDATA)
    ...
    if(builtin_testdata)
        ...
        if (TARGET "${name}")
            ...
            foreach(file IN LISTS builtin_files)
                set_source_files_properties(${file}
                    PROPERTIES BOBUI_SKIP_QUICKCOMPILER TRUE
                )
            endforeach()
            ...
            if(batch_current_test)
                ...
                if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${blacklist_path}")
                    get_target_property(blacklist_files ${name} _bobui_blacklist_files)
                    if(NOT blacklist_files)
                        set_target_properties(${name} PROPERTIES _bobui_blacklist_files "")
                        set(blacklist_files "")
                    endif()
                    list(PREPEND blacklist_files "${CMAKE_CURRENT_SOURCE_DIR}/${blacklist_path}")
                    set_target_properties(${name} PROPERTIES
                        _bobui_blacklist_files "${blacklist_files}")
                endif()
            else()
                set(blacklist_path "BLACKLIST")
                if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${blacklist_path}")
            ...
            # Skip BobUI quick compiler when embedding test resources
            foreach(file IN LISTS builtin_files)
                set_source_files_properties(${file}
                    PROPERTIES BOBUI_SKIP_QUICKCOMPILER TRUE
                )
            ...
            if(builtin_files)
                bobui_internal_add_resource(${name} "${testname}_testdata_builtin"
                    PREFIX "/"
                    FILES ${builtin_files}
        ...
        # single standalone test, which is checked by the existence of the BOBUI_TOP_LEVEL_SOURCE_DIR
        # variable.
        ...
        # test data, and where we end up installing it? See BOBUIBUG-117098.
        if(BOBUI_TOP_LEVEL_SOURCE_DIR)
            foreach(testdata IN LISTS arg_TESTDATA)
    ...
    if(MACOS AND NOT CMAKE_GENERATOR STREQUAL "Xcode")
        ...
        if(NOT arg_QMLTEST AND arg_SOURCES)
            set(entitlements_file
                "${__bobui_internal_cmake_apple_support_files_path}/test.entitlements.plist")
    ...
    bobui_internal_add_test_finalizers("${name}")
endfunction()

# Generates a blacklist file for the global batched test target.
function(bobui_internal_finalize_test_batch_blacklist)
    _bobui_internal_test_batch_target_name(batch_target_name)
    ...
    get_target_property(blacklist_files "${batch_target_name}" _bobui_blacklist_files)
    if(blacklist_files)
    ...
    bobui_configure_file(OUTPUT "${generated_blacklist_file}" CONTENT "${final_contents}")

    bobui_internal_add_resource(${batch_target_name} "batch_blacklist"
        PREFIX "/"
        FILES "${generated_blacklist_file}"
        BASE ${CMAKE_CURRENT_BINARY_DIR})
endfunction()

# Given an optional test timeout value (specified via bobui_internal_add_test's TIMEOUT option)
...
function(bobui_internal_get_android_test_timeout input_timeout percentage output_timeout_var)
...
#
# See also _bobui_internal_create_command_script for details.
function(bobui_internal_create_test_script)
    ...
    if(NOT arg_COMMAND)
        message(FATAL_ERROR "bobui_internal_create_test_script: Test COMMAND is not specified")
    endif()

    if(NOT arg_NAME)
        message(FATAL_ERROR "bobui_internal_create_test_script: Test NAME is not specified")
    endif()

    if(NOT arg_OUTPUT_FILE)
        message(FATAL_ERROR "bobui_internal_create_test_script: Test Wrapper OUTPUT_FILE\
is not specified")
    endif()

    if(arg_PRE_RUN)
        message(WARNING "bobui_internal_create_test_script: PRE_RUN is not acceptable argument\
for this function. Will be ignored")
    endif()

    if(arg_POST_RUN)
        message(WARNING "bobui_internal_create_test_script: POST_RUN is not acceptable argument\
for this function. Will be ignored")
    endif()
    ...
    bobui_internal_is_in_test_batch(is_in_batch ${executable_name})
    if(is_in_batch)
        _bobui_internal_test_batch_target_name(executable_name)
    endif()
    ...
    _bobui_internal_create_command_script(COMMAND
...
function(bobui_internal_add_test_helper name)

    set(bobui_add_test_helper_optional_args
        "OVERRIDE_OUTPUT_DIRECTORY"
    )

    cmake_parse_arguments(PARSE_ARGV 1 arg
        "${bobui_add_test_helper_optional_args};${__bobui_internal_add_executable_optional_args}"
        "${__bobui_internal_add_executable_single_args}"
        "${__bobui_internal_add_executable_multi_args}"
    )
    _bobui_internal_validate_all_args_are_parsed(arg)
    ...
    bobui_remove_args(forward_args
        ARGS_TO_REMOVE
            "${name}"
            ${bobui_add_test_helper_optional_args}
        ALL_ARGS
            ${bobui_add_test_helper_optional_args}
            ${__bobui_internal_add_executable_optional_args}
            ${__bobui_internal_add_executable_single_args}
    ...
    if(NOT arg_OVERRIDE_OUTPUT_DIRECTORY)
        _bobui_internal_test_batch_target_name(test_batch_target_name)
        if(BOBUI_BUILD_TESTS_BATCHED AND TARGET ${test_batch_target_name})
            get_target_property(
    ...
    bobui_internal_add_executable("${name}" NO_INSTALL
                                         NO_UNITY_BUILD # excluded by default
                                         ${extra_args_to_pass} ${forward_args})

    # Disable the BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT define for test helpers
    bobui_internal_undefine_global_definition(${name} BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT)

endfunction()

function(bobui_internal_collect_command_environment out_path out_plugin_path)
    # Get path to <bobui_relocatable_install_prefix>/bin, as well as CMAKE_INSTALL_PREFIX/bin, and
    # combine them with the PATH environment variable.
    # It's needed on Windows to find the shared libraries and plugins.
    # bobui_relocatable_install_prefix is dynamically computed from the location of where the BobUI CMake
    # package is found.
    # The regular CMAKE_INSTALL_PREFIX can be different for example when building standalone tests.
    # Any given CMAKE_INSTALL_PREFIX takes priority over bobui_relocatable_install_prefix for the
    # PATH environment variable.
    set(install_prefixes "${CMAKE_INSTALL_PREFIX}")
    if(BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX)
        list(APPEND install_prefixes "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}")
    endif()
    ...
    foreach(install_prefix ${install_prefixes})
        ...
        set(test_env_path "${test_env_path}${BOBUI_PATH_SEPARATOR}${install_prefix}")
    endforeach()
    set(test_env_path "${test_env_path}${BOBUI_PATH_SEPARATOR}$ENV{PATH}")
    if(ANDROID)
        ...
        string(PREPEND test_env_path
            "${platform_tools}" "${BOBUI_PATH_SEPARATOR}")
    endif()
    ...
    list(JOIN plugin_paths "${BOBUI_PATH_SEPARATOR}" plugin_paths_joined)
    string(REPLACE ";" "\;" plugin_paths_joined "${plugin_paths_joined}")
    set(${out_plugin_path} "${plugin_paths_joined}" PARENT_SCOPE)
endfunction()

function(bobui_internal_add_test_finalizers target)
    ...
    if(NOT BOBUI_INTERNAL_SKIP_TEST_FINALIZERS_V2)
        return()
    endif()

    # It might not be safe to run all the finalizers of _bobui_internal_finalize_executable
    # within the context of a BobUI build (not a user project) when targeting a host build.
    ...
    if(IOS)
        bobui_add_list_file_finalizer(_bobui_internal_finalize_executable "${target}")
    endif()
endfunction()

# Collection of tests and targets added to all repos
function(bobui_internal_add_default_tests)
    ...
    # dependencies
    if(NOT TARGET check_bobui_module_json_schemas)
        bobui_path_join(__check_bobui_module_json_schemas_py
            "${BOBUI_STAGING_PREFIX}"
            "${INSTALL_LIBEXECDIR}"
            "check_bobui_module_json_schemas.py"
        )
        add_custom_target(check_bobui_module_json_schemas
            COMMAND python3 ${__check_bobui_module_json_schemas_py}
                "--install-prefix=${BOBUI_STAGING_PREFIX}"
                "--bobui-sharedir=${INSTALL_BOBUI_SHAREDIR}"
        )
    endif()
endfunction()
