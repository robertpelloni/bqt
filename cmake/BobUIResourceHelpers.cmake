...
function(bobui_internal_add_resource target resourceName)
    ...
    _bobui_internal_process_resource(${target} ${resourceName}
        PREFIX "${arg_PREFIX}"
        LANG "${arg_LANG}"
        BASE "${arg_BASE}"
        FILES ${arg_FILES}
   ...
   if (out_targets)
        bobui_install(TARGETS ${out_targets}
        ...
        bobui_internal_install_resource_pdb_files("${out_targets}")
        bobui_internal_record_rcc_object_files("${target}" "${out_targets}"
...
function(bobui_internal_record_rcc_object_files target resource_targets)
    set(args_optional "")
    ...
    foreach(out_target ${resource_targets})
        get_target_property(resource_name ${out_target} _bobui_resource_name)
        ...
        set_property(TARGET ${target} APPEND PROPERTY _bobui_rcc_objects "${rcc_object_file_path}")

        bobui_internal_link_internal_platform_for_object_library("${out_target}"
            PARENT_TARGET "${target}")
    endforeach()
endfunction()

function(bobui_internal_install_resource_pdb_files objlib_targets)
    if(NOT MSVC OR NOT BOBUI_WILL_INSTALL)
        return()
    endif()
    ...
    foreach(target IN LISTS objlib_targets)
        ...
        bobui_internal_install_pdb_files(${target}
