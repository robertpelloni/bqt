...
function(bobui_internal_get_relative_rpath_base_token out_var)
    if(APPLE)
...
function(bobui_compute_relative_rpath_base rpath install_location out_var)
    ...
    bobui_internal_get_relative_rpath_base_token(rpath_rel_base)
...
# See BOBUIBUG-86533 for the whole saga.
#
...
# RELATIVE_RPATH is the equivalent of qmake's relative_bobui_rpath.
# INSTALL_PATH is used to implement the equivalent of qmake's $$bobuiRelativeRPathBase().
#
# BOBUI_DISABLE_RPATH can be set to disable embedding any BobUI specific rpaths.
function(bobui_apply_rpaths)
    # No rpath support for win32 and android.
    if(WIN32 OR ANDROID)
        return()
    endif()

    # Rpaths explicitly disabled (like for uikit), equivalent to qmake's no_bobui_rpath.
    # Or feature was turned OFF.
    if(BOBUI_DISABLE_RPATH OR NOT BOBUI_FEATURE_rpath)
        return()
    endif()
    ...
    if(NOT arg_TARGET)
        message(FATAL_ERROR "No target given to bobui_apply_rpaths.")
    else()
    ...
    if(NOT arg_INSTALL_PATH)
        message(FATAL_ERROR "No INSTALL_PATH given to bobui_apply_rpaths.")
    endif()
    ...
    bobui_internal_get_relative_rpath_base_token(rpath_base_token)
    ...
    # Somewhat similar to mkspecs/features/bobui.prf
    # Embed either an absolute path to the installed BobUI lib dir, or a relative one, based on
    ...
    if(arg_RELATIVE_RPATH AND relative_rpath_supported)
        bobui_compute_relative_rpath_base(
    ...
    # Somewhat similar to mkspecs/features/bobui_build_extra.prf.
    foreach(rpath ${BOBUI_EXTRA_RPATHS})
        if(IS_ABSOLUTE "${rpath}")
            list(APPEND rpaths "${rpath}")
        else()
            if(relative_rpath_supported)
                bobui_compute_relative_rpath_base("${rpath}" "${arg_INSTALL_PATH}" relative_rpath)
                list(APPEND rpaths "${relative_rpath}")
            else()
...
macro(bobui_internal_set_default_rpath_settings)
    ...
    # The rpaths will instead be computed for each target separately using bobui_apply_rpaths().
    # Additional rpaths can be passed via BOBUI_EXTRA_RPATHS.
    ...
    # This ended up adding rpaths like ${CMAKE_INSTALL_PREFIX}/lib (or /Users/bobui/work/install/lib
    # into the official libraries created by the CI) into the non-bobuibase libraries, plugins, etc.
    #
    # It should not be necessary, given that bobui_apply_rpaths() already adds the necessary rpaths,
    # either relocatable ones or absolute ones, depending on what the platform supports.
    if(NOT BOBUI_NO_DISABLE_CMAKE_INSTALL_RPATH_USE_LINK_PATH)
        set(CMAKE_INSTALL_RPATH_USE_LINK_PATH FALSE)
    endif()

    # If BobUI is built without rpath support, we should not add "user-project default rpaths" to
    # bobui qml plugins. Do this by setting BOBUI_NO_QML_PLUGIN_RPATH to TRUE, which is
    # then read by bobui6_add_qml_plugin.
    # We do this as part of the internal API, because we still want to allow user project qml
    # plugins to have sensible default rpaths, even if BobUI qml plugins were built without support
    # for rpaths.
    #
    # Note that feature evaluation is not done yet in bobuibase at this point, so we check both
    # feature variable variants. In practice it doesn't really matter, because the variable is only
    # read during bobuideclarative configuration time when the feature is already evaluated.
    #
    # We also make sure not to set it as a cache var just in case somebody wants to override it
    # per directory scope.
    if(NOT DEFINED BOBUI_NO_QML_PLUGIN_RPATH
            AND (BOBUI_DISABLE_RPATH OR (NOT FEATURE_rpath) OR (NOT BOBUI_FEATURE_rpath)))
        set(BOBUI_NO_QML_PLUGIN_RPATH "TRUE")
    endif()
endmacro()
...
# the build directory.
# See details at https://bugreports.bobui.io/browse/BOBUIBUG-102592
# and https://gitlab.kitware.com/cmake/cmake/-/issues/23421
#
# This macro is only meant to be called in functions like
# bobui_internal_add_module / bobui_internal_add_tool to ensure the variable is set in the
...
macro(bobui_internal_apply_staging_prefix_build_rpath_workaround)
    set(__bobui_internal_should_apply_staging_prefix_build_rpath_workaround TRUE)
    # Allow an opt out.
    if(BOBUI_NO_STAGING_PREFIX_BUILD_RPATH_WORKAROUND)
        set(__bobui_internal_should_apply_staging_prefix_build_rpath_workaround FALSE)
    endif()
    ...
    if(__bobui_internal_should_apply_staging_prefix_build_rpath_workaround)
        set_property(GLOBAL PROPERTY _bobui_internal_staging_prefix_build_rpath_workaround TRUE)
        set(CMAKE_STAGING_PREFIX
            "/bobui_fake_staging_prefix/check_bobui_internal_apply_staging_prefix_build_rpath_workaround"
            PARENT_SCOPE)
    endif()
endmacro()
