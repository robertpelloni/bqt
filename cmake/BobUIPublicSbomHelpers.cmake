...
#
# INSTALL_PREFIX should be passed a value like CMAKE_INSTALL_PREFIX or BOBUI_STAGING_PREFIX.
...
#   BobUI's INSTALL_SBOMDIR.
# The default value is "sbom".
#
# SUPPLIER, SUPPLIER_URL, DOCUMENT_NAMESPACE, COPYRIGHTS are self-explanatory.
function(_bobui_internal_sbom_begin_project)
    # Allow opt out via an internal variable. Will be used in CI for repos like bobuiqa.
    if(BOBUI_INTERNAL_FORCE_NO_GENERATE_SBOM)
        set(BOBUI_GENERATE_SBOM OFF CACHE BOOL "Generate SBOM" FORCE)
    endif()
    ...
    _bobui_internal_sbom_setup_fake_deterministic_build()

    set(opt_args
        USE_GIT_VERSION
        __BOBUI_INTERNAL_HANDLE_BOBUI_REPO
    ...
    if(CMAKE_VERSION LESS_EQUAL "3.19")
        if(BOBUI_IGNORE_MIN_CMAKE_VERSION_FOR_SBOM)
            message(STATUS
                "Using CMake version older than 3.19, and BOBUI_IGNORE_MIN_CMAKE_VERSION_FOR_SBOM was "
                "set to ON. bobui_attribution.json files will not be processed.")
        else()
            message(FATAL_ERROR
                "Generating an SBOM requires CMake version 3.19 or newer. You can pass "
                "-DBOBUI_IGNORE_MIN_CMAKE_VERSION_FOR_SBOM=ON to try to generate the SBOM anyway, "
    ...
    # "Package-" in the spdx id. boot2bobui spdx seems to contain the same.

    if(arg_SBOM_PROJECT_NAME)
        _bobui_internal_sbom_set_root_project_name("${arg_SBOM_PROJECT_NAME}")
    else()
        _bobui_internal_sbom_set_root_project_name("${PROJECT_NAME}")
    endif()

    if(arg_BOBUI_REPO_PROJECT_NAME)
        _bobui_internal_sbom_set_bobui_repo_project_name("${arg_BOBUI_REPO_PROJECT_NAME}")
    else()
        _bobui_internal_sbom_set_bobui_repo_project_name("${PROJECT_NAME}")
    endif()

    _bobui_internal_sbom_get_root_project_name_for_spdx_id(repo_project_name_for_spdx_id)
    ...
    elseif(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO)
        _bobui_internal_sbom_get_default_supplier_url(repo_supplier_url)
    endif()
    ...
    _bobui_internal_handle_sbom_project_version(${sbom_project_version_args})

    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO)
        _bobui_internal_sbom_compute_bobui_uniqueish_document_namespace_infix(
    ...
    if(arg_DOCUMENT_NAMESPACE)
        ...
        if(BOBUI_SBOM_DOCUMENT_NAMESPACE_INFIX)
            string(APPEND repo_spdx_namespace "${BOBUI_SBOM_DOCUMENT_NAMESPACE_INFIX}")
        ...
        elseif(NOT arg_NO_AUTO_DOCUMENT_NAMESPACE_INFIX
                AND NOT BOBUI_SBOM_NO_AUTO_DOCUMENT_NAMESPACE_INFIX)
            _bobui_internal_sbom_compute_uniqueish_document_namespace_infix(
                OUT_VAR_UUID_INFIX_MERGED document_namespace_infix
            )
            string(APPEND repo_spdx_namespace "-${document_namespace_infix}")
        endif()

        if(BOBUI_SBOM_DOCUMENT_NAMESPACE_SUFFIX)
            string(APPEND repo_spdx_namespace "${BOBUI_SBOM_DOCUMENT_NAMESPACE_SUFFIX}")
        ...
        if(BOBUI_SBOM_DOCUMENT_NAMESPACE_INFIX)
            list(APPEND compute_project_namespace_args
                DOCUMENT_NAMESPACE_INFIX "${BOBUI_SBOM_DOCUMENT_NAMESPACE_INFIX}")
        elseif(arg_DOCUMENT_NAMESPACE_INFIX)
            list(APPEND compute_project_namespace_args
        ...
        if(BOBUI_SBOM_DOCUMENT_NAMESPACE_SUFFIX)
            list(APPEND compute_project_namespace_args
                DOCUMENT_NAMESPACE_SUFFIX "${BOBUI_SBOM_DOCUMENT_NAMESPACE_SUFFIX}")
        elseif(arg_DOCUMENT_NAMESPACE_SUFFIX)
            list(APPEND compute_project_namespace_args
                DOCUMENT_NAMESPACE_SUFFIX "${arg_DOCUMENT_NAMESPACE_SUFFIX}")
        endif()

        if(BOBUI_SBOM_DOCUMENT_NAMESPACE_URL_PREFIX)
            list(APPEND compute_project_namespace_args
                DOCUMENT_NAMESPACE_URL_PREFIX "${BOBUI_SBOM_DOCUMENT_NAMESPACE_URL_PREFIX}")
        elseif(arg_DOCUMENT_NAMESPACE_URL_PREFIX)
            list(APPEND compute_project_namespace_args
                DOCUMENT_NAMESPACE_URL_PREFIX "${arg_DOCUMENT_NAMESPACE_URL_PREFIX}")
        endif()

        _bobui_internal_sbom_compute_project_namespace(repo_spdx_namespace
    ...
    if(BOBUI_SBOM_GENERATE_CYDX_V1_6)
        _bobui_internal_sbom_get_cyclone_bom_serial_number(
    ...
    _bobui_internal_sbom_get_project_explicit_version(explicit_version)
    if(explicit_version)
        list(APPEND compute_project_file_name_args VERSION_SUFFIX "${explicit_version}")
    endif()

    _bobui_internal_sbom_compute_project_file_name(repo_project_file_name_spdx
        SPDX_TAG_VALUE
        PROJECT_NAME "${repo_project_name_lowercase}"
        ${compute_project_file_name_args}
    )

    _bobui_internal_sbom_compute_project_file_name(repo_project_file_name_cydx
        CYCLONEDX_TOML
        PROJECT_NAME "${repo_project_name_lowercase}"
        ${compute_project_file_name_args}
    )

    _bobui_internal_path_join(repo_spdx_relative_install_path_spdx
        "${install_sbom_dir}" "${repo_project_file_name_spdx}")
    _bobui_internal_path_join(repo_spdx_relative_install_path_cydx
        "${install_sbom_dir}" "${repo_project_file_name_cydx}")

    # Prepend DESTDIR, to allow relocating installed sbom. Needed for CI.
    _bobui_internal_path_join(repo_spdx_install_path_spdx
        "\$ENV{DESTDIR}${install_prefix}" "${repo_spdx_relative_install_path_spdx}")
    _bobui_internal_path_join(repo_spdx_install_path_cydx
    ...
    elseif(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO)
        _bobui_internal_sbom_get_default_bobui_copyright_header(repo_copyright)
    endif()
    if(repo_copyright)
    ...
    elseif(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO)
        _bobui_internal_sbom_get_default_supplier(repo_supplier)
    endif()
    if(repo_supplier)
    ...
    if(arg_CPE)
        set(bobui_cpe "${arg_CPE}")
    elseif(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO)
        _bobui_internal_sbom_get_cpe_bobui_repo(bobui_cpe)
    else()
        set(bobui_cpe "")
    endif()
    if(bobui_cpe)
        list(APPEND begin_project_generate_args_spdx CPE "${bobui_cpe}")
        list(APPEND begin_project_generate_args_cydx CPE "${bobui_cpe}")
    endif()
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO)
        _bobui_internal_get_configure_line(configure_line)
        if(configure_line)
    ...
    if(BOBUI_SBOM_GENERATE_SPDX_V2)
        _bobui_internal_sbom_begin_project_generate(
    ...
    if(BOBUI_SBOM_GENERATE_CYDX_V1_6)
        _bobui_internal_sbom_begin_project_generate_cyclone(
    ...
    set_property(GLOBAL PROPERTY _bobui_internal_project_attribution_files "")

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_repo_document_namespace
        "${repo_spdx_namespace}")

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_repo_cyclone_dx_bom_serial_number_uuid
        "${cyclone_dx_bom_serial_number_uuid}")

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_relative_installed_repo_document_path
        "${repo_spdx_relative_install_path_spdx}")

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_repo_project_name_lowercase
        "${repo_project_name_lowercase}")

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_install_prefix
        "${arg_INSTALL_PREFIX}")

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_project_spdx_id
        "${repo_project_spdx_id}")

    # Collect project licenses.
    set(license_dirs "")

    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_REPO AND EXISTS "${PROJECT_SOURCE_DIR}/LICENSES")
    ...
    if(BOBUI_SBOM_LICENSE_DIRS)
        list(APPEND license_dir_candidates ${BOBUI_SBOM_LICENSE_DIRS})
    endif()
    ...
    foreach(license_file IN LISTS license_files)
        ...
        _bobui_internal_sbom_add_license(
    ...
    # Make sure that any system library dependencies that have been found via bobui_find_package or
    # _bobui_internal_find_third_party_dependencies have their spdx id registered now.
    _bobui_internal_sbom_record_system_library_spdx_ids()

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_repo_begin_called TRUE)

    _bobui_internal_sbom_setup_project_ops()
endfunction()
...
function(_bobui_internal_sbom_setup_project_ops)
    set(options "")

    if(BOBUI_SBOM_GENERATE_JSON
            OR BOBUI_SBOM_GENERATE_SPDX_V2_JSON
            OR BOBUI_INTERNAL_SBOM_GENERATE_JSON
            OR BOBUI_INTERNAL_SBOM_DEFAULT_CHECKS)
    ...
    # deps are not found.
    if(BOBUI_SBOM_REQUIRE_GENERATE_JSON
            OR BOBUI_SBOM_REQUIRE_GENERATE_SPDX_V2_JSON
            OR BOBUI_INTERNAL_SBOM_GENERATE_JSON
            OR BOBUI_INTERNAL_SBOM_DEFAULT_CHECKS)
        list(APPEND options GENERATE_JSON_REQUIRED)
    endif()

    if(BOBUI_SBOM_VERIFY
            OR BOBUI_SBOM_VERIFY_SPDX_V2
            OR BOBUI_INTERNAL_SBOM_VERIFY
            OR BOBUI_INTERNAL_SBOM_DEFAULT_CHECKS)
        list(APPEND options VERIFY_SBOM)
    endif()

    # Do the same requirement check for SBOM verification.
    if(BOBUI_SBOM_REQUIRE_VERIFY
            OR BOBUI_SBOM_REQUIRE_VERIFY_SPDX_V2
            OR BOBUI_INTERNAL_SBOM_VERIFY
            OR BOBUI_INTERNAL_SBOM_DEFAULT_CHECKS)
    ...
    if(BOBUI_SBOM_REQUIRE_GENERATE_CYDX_V1_6)
        list(APPEND options GENERATE_CYCLONE_DX_V1_6_REQUIRED)
    endif()

    if(BOBUI_SBOM_VERIFY_CYDX_V1_6)
        list(APPEND options VERIFY_CYCLONE_DX_V1_6)
    endif()

    if(BOBUI_SBOM_REQUIRE_VERIFY_CYDX_V1_6)
        list(APPEND options VERIFY_CYCLONE_DX_V1_6_REQUIRED)
    endif()

    if(BOBUI_SBOM_VERBOSE_CYDX_V1_6)
        list(APPEND options VERBOSE_CYCLONE_DX_V1_6)
    endif()

    if(BOBUI_SBOM_VERIFY_NTIA_COMPLIANT
            OR BOBUI_INTERNAL_SBOM_VERIFY_NTIA_COMPLIANT OR BOBUI_INTERNAL_SBOM_DEFAULT_CHECKS)
        list(APPEND options VERIFY_NTIA_COMPLIANT)
    endif()

    if(BOBUI_SBOM_SHOW_TABLE OR BOBUI_INTERNAL_SBOM_SHOW_TABLE OR BOBUI_INTERNAL_SBOM_DEFAULT_CHECKS)
        list(APPEND options SHOW_TABLE)
    endif()

    if(BOBUI_SBOM_AUDIT OR BOBUI_INTERNAL_SBOM_AUDIT OR BOBUI_INTERNAL_SBOM_AUDIT_NO_ERROR)
        list(APPEND options AUDIT)
    endif()

    if(BOBUI_SBOM_AUDIT_NO_ERROR OR BOBUI_INTERNAL_SBOM_AUDIT_NO_ERROR)
    ...
    if(BOBUI_LINT_SOURCE_SBOM)
        list(APPEND options LINT_SOURCE_SBOM)
    endif()

    if(BOBUI_LINT_SOURCE_SBOM_NO_ERROR OR BOBUI_INTERNAL_LINT_SOURCE_SBOM_NO_ERROR)
        list(APPEND options LINT_SOURCE_SBOM_NO_ERROR)
    endif()

    _bobui_internal_sbom_setup_project_ops_generation(${options})
endfunction()
...
function(_bobui_internal_setup_sbom)
    set(opt_args "")
    set(single_args
    ...
    option(BOBUI_SBOM_GENERATE_SPDX_V2_JSON "${spdx_v23_json_help_string}" ON)
    option(BOBUI_SBOM_REQUIRE_GENERATE_SPDX_V2_JSON "${spdx_v23_json_require_help_string}" OFF)

    option(BOBUI_SBOM_VERIFY_SPDX_V2 "${spdx_v23_verify_help_string}" ON)
    option(BOBUI_SBOM_REQUIRE_VERIFY_SPDX_V2 "${spdx_v23_verify_require_help_string}" OFF)
    ...
    option(BOBUI_SBOM_VERIFY_CYDX_V1_6 "${cydx_verify_help_string}" ON)

    set(cydx_verify_require_help_string
        "Error out if SBOM verification dependencies are not found.")
    option(BOBUI_SBOM_REQUIRE_VERIFY_CYDX_V1_6 "${cydx_verify_require_help_string}" OFF)

    set(cydx_verbose_help_string
        "Enable verbose output for CycloneDX generation.")
    option(BOBUI_SBOM_VERBOSE_CYDX_V1_6 "${cydx_verbose_help_string}" OFF)
    ...
    if(DEFINED BOBUI_SBOM_GENERATE_JSON)
        option(BOBUI_SBOM_GENERATE_JSON "Deprecated: ${spdx_v23_json_help_string}" ON)
    endif()
    if(DEFINED BOBUI_SBOM_REQUIRE_GENERATE_JSON)
        option(BOBUI_SBOM_REQUIRE_GENERATE_JSON "Deprecated: ${spdx_v23_json_require_help_string}" OFF)
    endif()
    if(DEFINED BOBUI_SBOM_VERIFY)
        option(BOBUI_SBOM_VERIFY "Deprecated: ${spdx_v23_verify_help_string}" ON)
    endif()
    if(DEFINED BOBUI_SBOM_REQUIRE_VERIFY)
        option(BOBUI_SBOM_REQUIRE_VERIFY "Deprecated: ${spdx_v23_verify_require_help_string}" OFF)
    endif()

    # Semi-public, undocumented options to allow enabling all SBOM stuff, for easier testing.
    if(BOBUI_SBOM_GENERATE_AND_VERIFY_ALL)
        set(BOBUI_SBOM_GENERATE_ALL ON)
        set(BOBUI_SBOM_GENERATE_REQUIRED_ALL ON)
        set(BOBUI_SBOM_VERIFY_REQUIRED_ALL ON)
    endif()

    if(BOBUI_SBOM_GENERATE_ALL)
        set(BOBUI_GENERATE_SBOM ON CACHE BOOL "${sbom_help_string}" FORCE)
        set(BOBUI_SBOM_GENERATE_SPDX_V2 ON CACHE BOOL "${spdx_v2_help_string}" FORCE)
        set(BOBUI_SBOM_GENERATE_SPDX_V2_JSON ON CACHE BOOL "${spdx_v23_json_help_string}" FORCE)
        set(BOBUI_SBOM_GENERATE_CYDX_V1_6 ON CACHE BOOL "${cydx_help_string}" FORCE)
        unset(BOBUI_SBOM_GENERATE_ALL CACHE)
        unset(BOBUI_SBOM_GENERATE_ALL)
    endif()

    if(BOBUI_SBOM_GENERATE_REQUIRED_ALL)
        set(BOBUI_SBOM_REQUIRE_GENERATE_SPDX_V2_JSON ON CACHE BOOL
            "${spdx_v23_json_require_help_string}" FORCE)
        set(BOBUI_SBOM_REQUIRE_GENERATE_CYDX_V1_6 ON CACHE BOOL "${cydx_require_help_string}" FORCE)

        unset(BOBUI_SBOM_GENERATE_REQUIRED_ALL CACHE)
        unset(BOBUI_SBOM_GENERATE_REQUIRED_ALL)
    endif()

    if(BOBUI_SBOM_VERIFY_REQUIRED_ALL)
        set(BOBUI_SBOM_VERIFY_SPDX_V2 ON CACHE BOOL "${spdx_v23_verify_help_string}" FORCE)
        set(BOBUI_SBOM_VERIFY_CYDX_V1_6 ON CACHE BOOL "${cydx_verify_help_string}" FORCE)

        set(BOBUI_SBOM_REQUIRE_VERIFY_SPDX_V2 ON CACHE BOOL "${spdx_v23_verify_require_help_string}"
            FORCE)
        set(BOBUI_SBOM_REQUIRE_VERIFY_CYDX_V1_6 ON CACHE BOOL "${cydx_verify_require_help_string}"
            FORCE)

        unset(BOBUI_SBOM_VERIFY_ALL CACHE)
        unset(BOBUI_SBOM_VERIFY_ALL)
    endif()
    ...
    if(BOBUI_GENERATE_SBOM
            AND BOBUI_SBOM_GENERATE_SPDX_V2_JSON
            AND NOT BOBUI_SBOM_GENERATE_SPDX_V2)
        if(NOT BOBUI_NO_SBOM_INFORMATIONAL_MESSAGES)
            message(STATUS
        ...
        set(BOBUI_SBOM_GENERATE_SPDX_V2_JSON OFF CACHE BOOL "${spdx_v23_json_help_string}" FORCE)
        set(BOBUI_SBOM_VERIFY_SPDX_V2 OFF CACHE BOOL "${spdx_v23_verify_help_string}" FORCE)
    endif()

    # Disable CycloneDX generation if dependencies are not found and it wasn't required.
    if(BOBUI_GENERATE_SBOM
            AND BOBUI_SBOM_GENERATE_CYDX_V1_6
            AND NOT BOBUI_SBOM_REQUIRE_GENERATE_CYDX_V1_6)
        _bobui_internal_sbom_find_cydx_dependencies(OUT_VAR_DEPS_FOUND deps_found)
        if(NOT deps_found)
            if(NOT BOBUI_NO_SBOM_INFORMATIONAL_MESSAGES)
                message(STATUS
            ...
            set(BOBUI_SBOM_GENERATE_CYDX_V1_6 OFF CACHE BOOL "${cydx_help_string}" FORCE)
        endif()
    endif()

    # Disable sbom generation if none of the formats are enabled. Failing to do so will cause
    # errors in _bobui_internal_sbom_begin_project.
    if(BOBUI_GENERATE_SBOM
            AND NOT BOBUI_SBOM_GENERATE_SPDX_V2
            AND NOT BOBUI_SBOM_GENERATE_CYDX_V1_6)
        if(NOT BOBUI_NO_SBOM_INFORMATIONAL_MESSAGES)
            message(STATUS
                "Disabling SBOM generation because none of the supported formats were enabled.")
        endif()
        set(BOBUI_GENERATE_SBOM OFF CACHE BOOL "${sbom_help_string}" FORCE)
    endif()
endfunction()

# Disable SBOM processing for targets created under tests/ or examples/.
# Some repos create mock bobui modules under these directories like bobuideclarative and
# bobuiwebengine.
macro(_bobui_internal_conditionally_disable_sbom_in_current_scope)
    set(BOBUI_GENERATE_SBOM OFF)
endmacro()
...
# and install rules.
function(_bobui_internal_sbom_end_project)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()

    # Run sbom finalization for targets that had it scheduled, but haven't run yet.
    # This can happen when _bobui_internal_sbom_end_project is called within the same
    ...
    # This can happen in user projects, but also it happens for bobuibase with the Platform targets.
    ...
    get_cmake_property(new_targets _bobui_internal_sbom_targets_waiting_for_finalization)
    if(new_targets)
    ...
    while(targets_to_finalize)
        ...
        if(finalization_iterations GREATER 500)
            message(WARNING
                ...
                "Please report to https://bugreports.bobui.io with details "
        ...
        # Clear the list.
        set_property(GLOBAL PROPERTY _bobui_internal_sbom_targets_waiting_for_finalization "")
    ...
    _bobui_internal_sbom_add_recorded_system_libraries()

    # Add any external target dependencies, for CycloneDX generation.
    # E.g. For BobUISvg, we need to create a BobUICore component in the BobUISvg document, so that we
    # can declare a dependency on it.
    if(BOBUI_SBOM_GENERATE_CYDX_V1_6)
        _bobui_internal_sbom_add_cydx_external_target_dependencies()
    endif()

    if(BOBUI_SBOM_GENERATE_SPDX_V2)
        _bobui_internal_sbom_end_project_generate()
    endif()

    if(BOBUI_SBOM_GENERATE_CYDX_V1_6)
        _bobui_internal_sbom_end_project_generate_cyclone()
    endif()

    # Clean up external document ref properties, because each repo needs to start from scratch
    # in a top-level build.
    get_cmake_property(known_external_documents _bobui_known_external_documents)
    set_property(GLOBAL PROPERTY _bobui_known_external_documents "")
    foreach(external_document IN LISTS known_external_documents)
        set_property(GLOBAL PROPERTY _bobui_known_external_documents_${external_document} "")
    endforeach()

    set_property(GLOBAL PROPERTY _bobui_internal_sbom_repo_begin_called FALSE)

    # Add configure-time dependency on project attribution files.
    get_property(attribution_files GLOBAL PROPERTY _bobui_internal_project_attribution_files)
    _bobui_internal_append_cmake_configure_depends(${attribution_files})
endfunction()

# Automatically begins sbom generation for a bobui git repo unless BOBUI_SKIP_SBOM_AUTO_PROJECT is TRUE.
function(_bobui_internal_sbom_auto_begin_bobui_repo_project)
    # Allow skipping auto generation of sbom project, in case it needs to be manually adjusted with
    # extra parameters.
    if(BOBUI_SKIP_SBOM_AUTO_PROJECT)
        return()
    endif()

    _bobui_internal_sbom_begin_bobui_repo_project()
endfunction()

# Sets up sbom generation for a bobui git repo or bobui-git-repo-sub-project (e.g. bobuipdf in bobuiwebengine).
#
# In the case of a bobui-git-repo-sub-project, the function expects the following options:
# - SBOM_PROJECT_NAME (e.g. BobUIPdf)
# - BOBUI_REPO_PROJECT_NAME (e.g. BobUIWebEngine)
#
# Expects the following variables to always be set before the function call:
# - BOBUI_STAGING_PREFIX
# - INSTALL_SBOMDIR
function(_bobui_internal_sbom_begin_bobui_repo_project)
    set(opt_args "")
    set(single_args
        SBOM_PROJECT_NAME
        BOBUI_REPO_PROJECT_NAME
...
# Automatically ends sbom generation for a bobui git repo unless BOBUI_SKIP_SBOM_AUTO_PROJECT is TRUE.
function(_bobui_internal_sbom_auto_end_bobui_repo_project)
    # Allow skipping auto generation of sbom project, in case it needs to be manually adjusted with
    # extra parameters.
    if(BOBUI_SKIP_SBOM_AUTO_PROJECT)
        return()
    ...
    _bobui_internal_sbom_end_bobui_repo_project()
endfunction()

# Ends sbom generation for a bobui git repo or bobui-git-repo-sub-project.
function(_bobui_internal_sbom_end_bobui_repo_project)
...
function(_bobui_internal_sbom_setup_fake_deterministic_build)
    if(NOT DEFINED BOBUI_SBOM_FAKE_DETERMINISTIC_BUILD)
        return()
    endif()

    if(BOBUI_SBOM_FAKE_DETERMINISTIC_BUILD)
        set(value "ON")
    else()
        set(value "OFF")
    endif()

    set(BOBUI_SBOM_FAKE_GIT_VERSION "${value}" CACHE BOOL "SBOM fake git version")
    set(BOBUI_SBOM_FAKE_TIMESTAMP "${value}" CACHE BOOL "SBOM fake timestamp")
    set(BOBUI_SBOM_FAKE_CHECKSUM "${value}" CACHE BOOL "SBOM fake checksums")
endfunction()

# Helper to get purl entry-specific options.
macro(_bobui_internal_get_sbom_purl_parsing_options opt_args single_args multi_args)
    set(${opt_args}
        NO_DEFAULT_BOBUI_PURL
...
# _bobui_internal_sbom_add_target.
macro(_bobui_internal_get_sbom_purl_add_target_options opt_args single_args multi_args)
    set(${opt_args}
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_PURL
    )
    set(${single_args} "")
    set(${multi_args}
        PURLS
...
# Helper to get purl options that should be forwarded from _bobui_internal_sbom_add_target to
# _bobui_internal_sbom_handle_purl_values.
macro(_bobui_internal_get_sbom_purl_handling_options opt_args single_args multi_args)
    set(${opt_args} "")
    set(${single_args}
        SUPPLIER
        SBOM_ENTITY_TYPE
    ...
    _bobui_internal_get_sbom_purl_add_target_options(
...
# Helper to get the options that _bobui_internal_sbom_add_target understands, but that are also
# a safe subset for bobui_internal_add_module, bobui_internal_extend_target, etc to understand.
macro(_bobui_internal_get_sbom_add_target_common_options opt_args single_args multi_args)
    set(${opt_args}
        NO_CURRENT_DIR_ATTRIBUTION
        NO_ATTRIBUTION_LICENSE_ID
        NO_DEFAULT_BOBUI_LICENSE
        NO_DEFAULT_BOBUI_LICENSE_ID_LIBRARIES
        NO_DEFAULT_BOBUI_LICENSE_ID_EXECUTABLES
        NO_DEFAULT_DIRECTORY_BOBUI_LICENSE
        NO_DEFAULT_BOBUI_COPYRIGHTS
        NO_DEFAULT_BOBUI_PACKAGE_VERSION
        NO_DEFAULT_BOBUI_SUPPLIER
        SBOM_INCOMPLETE_3RD_PARTY_DEPENDENCIES
        IS_BOBUI_3RD_PARTY_HEADER_MODULE
        USE_ATTRIBUTION_FILES
        CREATE_SBOM_FOR_EACH_ATTRIBUTION
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_PACKAGE_VERSION
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_SUPPLIER
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_DOWNLOAD_LOCATION
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_LICENSE
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_COPYRIGHTS
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_CPE
        __BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_ATTRIBUTION_FILES
    )
    set(${single_args}
...
# Helper to get all known SBOM specific options, without the ones that bobui_internal_add_module
# and similar functions understand, like LIBRARIES, INCLUDES, etc.
macro(_bobui_internal_get_sbom_specific_options opt_args single_args multi_args)
    set(${opt_args} "")
    set(${single_args} "")
    set(${multi_args} "")

    _bobui_internal_get_sbom_add_target_common_options(
    ...
    _bobui_internal_sbom_get_multi_config_single_args(multi_config_single_args)
    list(APPEND ${single_args} ${multi_config_single_args})
endmacro()

# Helper to get the options that _bobui_internal_sbom_add_target understands.
# Also used in bobui_find_package_extend_sbom.
macro(_bobui_internal_get_sbom_add_target_options opt_args single_args multi_args)
    set(${opt_args}
        NO_INSTALL
    )
    set(${single_args}
    ...
    _bobui_internal_get_sbom_specific_options(
...
# out_var.
function(_bobui_internal_map_sbom_entity_type out_var)
    set(opt_args "")
    set(single_args
        TYPE # deprecated
        SBOM_ENTITY_TYPE
    ...
    elseif(arg_TYPE)
        ...
        if(BOBUI_WARN_DEPRECATED_SBOM_TYPE_OPTION)
...
# For licenses, copyrights, these can either be passed as options, or read from bobui_attribution.json
# files.
# For dependencies, these are either specified via options, or read from properties set on the
# target by bobui_internal_extend_target.
function(_bobui_internal_sbom_add_target target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    _bobui_internal_map_sbom_entity_type(sbom_entity_type ${ARGN})

    # Mark the target as a BobUI module for sbom processing purposes.
    # Needed for non-standard targets like Bootstrap and BobUILibraryInfo, that don't have a BobUI::
    # namespace prefix.
    if(sbom_entity_type STREQUAL BOBUI_MODULE)
        set_target_properties(${target} PROPERTIES _bobui_sbom_is_bobui_module TRUE)
    endif()
    ...
    get_target_property(bobui_package_name "${target}" _bobui_package_name)
    if(bobui_package_name)
        get_target_property(bobui_module_interface_name "${target}" _bobui_module_interface_name)
        set(namespaced_target_name "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${bobui_module_interface_name}")

        string(APPEND package_comment
            "CMake exported target name: ${namespaced_target_name}\n")
        string(APPEND package_comment "Contained in CMake package: ${bobui_package_name}\n")
    endif()

    # Record the target spdx id right now, so we can refer to it in later attribution targets
    # if needed.
    _bobui_internal_sbom_record_target_spdx_id(${target}
    ...
    if(arg_USE_ATTRIBUTION_FILES)
        ...
        _bobui_internal_get_sbom_specific_options(sbom_opt_args sbom_single_args sbom_multi_args)
        ...
        if(NOT arg_NO_CURRENT_DIR_ATTRIBUTION
                AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/bobui_attribution.json")
            list(APPEND attribution_args
                ATTRIBUTION_FILE_PATHS "${CMAKE_CURRENT_SOURCE_DIR}/bobui_attribution.json"
            )
        endif()

        _bobui_internal_sbom_handle_bobui_attribution_files(qa ${attribution_args})
    endif()
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_LICENSE)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_license_expression(${target} ${sbom_add_target_args}
            OUT_VAR bobui_entity_license_expression)
        if(bobui_entity_license_expression)
            _bobui_internal_sbom_join_two_license_ids_with_op(
                "${license_expression}" "AND" "${bobui_entity_license_expression}"
                license_expression)
        endif()
    endif()
    ...
    if(license_expression AND
            arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_LICENSE)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_license_declared_expression(${target}
            ${sbom_add_target_args}
            LICENSE_CONCLUDED_EXPRESSION "${license_expression}"
            OUT_VAR bobui_entity_license_declared_expression)
        if(bobui_entity_license_declared_expression)
            list(APPEND project_package_options_spdx
                LICENSE_DECLARED "${bobui_entity_license_declared_expression}")
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_COPYRIGHTS)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_copyrights(${target} ${sbom_add_target_args}
            OUT_VAR bobui_copyrights)
        if(bobui_copyrights)
            list(APPEND copyrights ${bobui_copyrights})
        endif()
    endif()
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_PACKAGE_VERSION)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_package_version(${target} ${sbom_add_target_args}
            OUT_VAR bobui_entity_package_version)
        if(bobui_entity_package_version)
            set(package_version "${bobui_entity_package_version}")
        endif()
    endif()
    ...
    if(package_version)
        ...
        set_property(TARGET "${target}" PROPERTY _bobui_sbom_package_version "${package_version}")
        set_property(TARGET "${target}" APPEND PROPERTY EXPORT_PROPERTIES _bobui_sbom_package_version)
    endif()
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_SUPPLIER)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_supplier(${target} ${sbom_add_target_args}
            OUT_VAR bobui_entity_supplier)
        if(bobui_entity_supplier)
            set(supplier "${bobui_entity_supplier}")
        endif()
    endif()
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_DOWNLOAD_LOCATION)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_download_location(${target} ${sbom_add_target_args}
            OUT_VAR bobui_entity_download_location)
        if(bobui_entity_download_location)
            set(download_location "${bobui_entity_download_location}")
        endif()
    endif()
    ...
    _bobui_internal_sbom_get_package_purpose("${sbom_entity_type}" package_purpose)
    ...
    if(arg_CPE_VENDOR AND arg_CPE_PRODUCT)
        _bobui_internal_sbom_compute_security_cpe(custom_cpe
    ...
    if(arg_USE_ATTRIBUTION_FILES AND qa_cpes)
        ...
        _bobui_internal_sbom_replace_qa_placeholders(
            VALUES ${qa_cpes}
            ${placeholder_args}
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_CPE)
        _bobui_internal_sbom_forward_sbom_add_target_options_modified(sbom_add_target_args)
        _bobui_internal_sbom_handle_bobui_entity_cpe(${target} ${sbom_add_target_args}
            CPE "${cpe_values}"
            OUT_VAR bobui_cpe_list)
        if(bobui_cpe_list)
            list(APPEND cpe_values ${bobui_cpe_list})
        endif()
    endif()
    ...
    _bobui_internal_get_sbom_purl_add_target_options(purl_opt_args purl_single_args purl_multi_args)
    _bobui_internal_forward_function_args(
        FORWARD_APPEND
        FORWARD_PREFIX arg
    ...
    _bobui_internal_sbom_handle_purl_values(${target} ${purl_args})
    ...
    if(arg_USE_ATTRIBUTION_FILES)
        if(qa_chosen_attribution_file_path)
            _bobui_internal_sbom_map_path_to_reproducible_relative_path(relative_attribution_path
        ...
        if(qa_bobui_usage)
            string(APPEND package_comment "    BobUI usage: ${qa_bobui_usage}\n")
        endif()
    ...
    _bobui_internal_sbom_handle_target_dependencies("${target}"
    ...
    if(external_target_dependencies)
        _bobui_internal_sbom_record_external_target_dependecies(
    ...
    if(BOBUI_SBOM_GENERATE_SPDX_V2)
        _bobui_internal_sbom_generate_add_package(
    ...
    if(BOBUI_SBOM_GENERATE_CYDX_V1_6)
        get_property(external_targets
            GLOBAL PROPERTY _bobui_internal_sbom_external_target_dependencies)
        ...
        if(NOT target IN_LIST external_targets)
            _bobui_internal_sbom_handle_bobui_entity_cydx_properties(
                SBOM_ENTITY_TYPE "${sbom_entity_type}"
                OUT_CYDX_PROPERTIES cydx_properties
            )

            _bobui_internal_sbom_generate_cyclone_add_package(
                PACKAGE "${package_name_for_spdx_id}"
                SPDXID "${package_spdx_id}"
                SBOM_ENTITY_TYPE "${sbom_entity_type}"
    ...
    get_cmake_property(install_prefix _bobui_internal_sbom_install_prefix)
    if(install_prefix)
    ...
    if(BOBUI_SBOM_GENERATE_SPDX_V2)
        _bobui_internal_sbom_handle_target_binary_files("${target}"
            ${no_install_option}
            ${framework_option}
        ...
        _bobui_internal_sbom_handle_target_custom_files("${target}"
            ${no_install_option}
            ${install_prefix_option}
...
function(_bobui_internal_add_sbom target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    if(NOT TARGET "${target}")
        _bobui_internal_create_sbom_target("${target}" ${forward_args})
    endif()
    ...
    _bobui_internal_extend_sbom("${target}" ${forward_args})
endfunction()
...
# target, or for 3rd party sources that get compiled into a regular BobUI target (PCRE sources compiled
# into Bootstrap).
function(_bobui_internal_create_sbom_target target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    set_target_properties(${target} PROPERTIES
        _bobui_sbom_is_custom_sbom_target "TRUE"
...
# during finalization.
function(_bobui_internal_extend_sbom target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    if(NOT TARGET "${target}")
        message(FATAL_ERROR
            "The target ${target} does not exist, use bobui_internal_add_sbom to create "
    ...
    get_target_property(is_system_library "${target}" _bobui_internal_sbom_is_system_library)

    set_property(TARGET ${target} APPEND PROPERTY _bobui_finalize_sbom_args "${forward_args}")
    ...
    if(NOT arg_NO_FINALIZATION AND NOT is_system_library)
        ...
        _bobui_internal_defer_sbom_finalization("${target}" ${immediate_finalization})
    endif()
endfunction()

# Helper to add additional sbom information to targets created by bobui_find_package.
# If the package was not found, and the targets were not created, the functions does nothing.
# This is similar to _bobui_internal_extend_sbom, but is explicit in the fact that the targets might
# not exist.
function(_bobui_find_package_extend_sbom)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()

    _bobui_internal_get_sbom_add_target_options(sbom_opt_args sbom_single_args sbom_multi_args)
    ...
    foreach(target IN LISTS arg_TARGETS)
        if(TARGET "${target}")
            _bobui_internal_extend_sbom("${target}" ${sbom_args})
        else()
...
function(_bobui_internal_defer_sbom_finalization target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    get_target_property(sbom_finalization_requested ${target} _bobui_sbom_finalization_requested)
    if(sbom_finalization_requested)
        # Already requested, nothing to do.
        return()
    endif()
    set_target_properties(${target} PROPERTIES _bobui_sbom_finalization_requested TRUE)

    _bobui_internal_append_to_cmake_property_without_duplicates(
        _bobui_internal_sbom_targets_waiting_for_finalization
        "${target}"
    )

    set(func "_bobui_internal_finalize_sbom")

    if(arg_IMMEDIATE_FINALIZATION)
        _bobui_internal_finalize_sbom(${target})
    elseif(BOBUI_BUILDING_BOBUI)
        bobui_add_list_file_finalizer("${func}" "${target}")
...
function(_bobui_internal_finalize_sbom target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()

    get_target_property(sbom_finalization_done ${target} _bobui_sbom_finalization_done)
    if(sbom_finalization_done)
        # Already done, nothing to do.
        return()
    endif()
    set_target_properties(${target} PROPERTIES _bobui_sbom_finalization_done TRUE)

    get_target_property(sbom_args ${target} _bobui_finalize_sbom_args)
    if(NOT sbom_args)
        set(sbom_args "")
    endif()
    _bobui_internal_sbom_add_target(${target} ${sbom_args})
endfunction()

# Extends the list of targets that are considered dependencies for target.
function(_bobui_internal_extend_sbom_dependencies target)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    _bobui_internal_append_to_target_property_without_duplicates(${target}
        _bobui_extend_target_sbom_dependencies "${arg_SBOM_DEPENDENCIES}"
    )
endfunction()

# Sets the sbom project name for the root project.
function(_bobui_internal_sbom_set_root_project_name project_name)
    set_property(GLOBAL PROPERTY _bobui_internal_sbom_repo_project_name "${project_name}")
endfunction()

# Sets the real bobui repo project name for a given project (e.g. set BobUIWebEngine for project BobUIPdf).
# This is needed to be able to extract the bobui repo dependencies in a top-level build.
function(_bobui_internal_sbom_set_bobui_repo_project_name project_name)
    set_property(GLOBAL PROPERTY _bobui_internal_sbom_bobui_repo_project_name "${project_name}")
endfunction()

# Get repo project_name spdx id reference, needs to start with Package- to be NTIA compliant.
function(_bobui_internal_sbom_get_root_project_name_for_spdx_id out_var)
    _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
...
function(_bobui_internal_sbom_get_root_project_name_lower_case out_var)
    get_cmake_property(project_name _bobui_internal_sbom_repo_project_name)
...
# Returns the lower case real bobui repo project name (e.g. returns 'bobuiwebengine' when building the
# project bobuipdf).
function(_bobui_internal_sbom_get_bobui_repo_project_name_lower_case out_var)
    get_cmake_property(project_name _bobui_internal_sbom_bobui_repo_project_name)

    if(NOT project_name)
        message(FATAL_ERROR "No real BobUI repo SBOM project name was set.")
    endif()
...
function(_bobui_internal_sbom_get_external_document_ref_spdx_id repo_name out_var)
    set(${out_var} "DocumentRef-${repo_name}" PARENT_SCOPE)
endfunction()

# Sanitize a given value to be used as a SPDX id.
function(_bobui_internal_sbom_get_sanitized_spdx_id out_var hint)
...
function(_bobui_internal_sbom_record_target_spdx_id target)
    set(opt_args "")
    set(single_args
        PACKAGE_NAME
        SBOM_ENTITY_TYPE
    ...
    _bobui_internal_sbom_get_spdx_id_for_target("${target}" spdx_id)
    ...
    _bobui_internal_sbom_generate_target_package_spdx_id(package_spdx_id
        SBOM_ENTITY_TYPE "${arg_SBOM_ENTITY_TYPE}"
        PACKAGE_NAME "${package_name_for_spdx_id}"
    )
    _bobui_internal_sbom_save_spdx_id_for_target("${target}"
        SPDX_ID "${package_spdx_id}"
        PACKAGE_NAME "${package_name_for_spdx_id}"
        SBOM_ENTITY_TYPE "${arg_SBOM_ENTITY_TYPE}"
    )
    ...
    _bobui_internal_sbom_save_spdx_id_for_bobui_entity_type(
        "${target}" "${is_bobui_entity_type}" "${package_spdx_id}")
...
function(_bobui_internal_sbom_generate_target_package_spdx_id out_var)
    set(opt_args "")
    set(single_args
        PACKAGE_NAME
        SBOM_ENTITY_TYPE
    ...
    _bobui_internal_sbom_get_root_project_name_for_spdx_id(repo_project_name_spdx_id)
    _bobui_internal_sbom_get_package_infix("${arg_SBOM_ENTITY_TYPE}" package_infix)

    _bobui_internal_sbom_get_sanitized_spdx_id(spdx_id
...
function(_bobui_internal_sbom_save_spdx_id_for_target target)
    set(opt_args "")
    set(single_args
        SPDX_ID
        PACKAGE_NAME
    ...
    _bobui_internal_dealias_target(target_unaliased)

    set_target_properties(${target_unaliased} PROPERTIES
        _bobui_sbom_spdx_id "${spdx_id}")

    # Retrieve repo specific properties.
    get_property(repo_document_namespace
        GLOBAL PROPERTY _bobui_internal_sbom_repo_document_namespace)

    get_property(bom_serial_number_uuid
        GLOBAL PROPERTY _bobui_internal_sbom_repo_cyclone_dx_bom_serial_number_uuid)

    get_property(relative_installed_repo_document_path
        GLOBAL PROPERTY _bobui_internal_sbom_relative_installed_repo_document_path)

    get_property(project_name_lowercase
        GLOBAL PROPERTY _bobui_internal_sbom_repo_project_name_lowercase)

    # And save them on the target.
    set_property(TARGET ${target_unaliased} PROPERTY
        _bobui_sbom_spdx_repo_document_namespace
        "${repo_document_namespace}")

    set_property(TARGET ${target_unaliased} PROPERTY
        _bobui_sbom_cydx_bom_serial_number_uuid
        "${bom_serial_number_uuid}")

    set_property(TARGET ${target_unaliased} PROPERTY
        _bobui_sbom_spdx_relative_installed_repo_document_path
        "${relative_installed_repo_document_path}")

    set_property(TARGET ${target_unaliased} PROPERTY
        _bobui_sbom_spdx_repo_project_name_lowercase
        "${project_name_lowercase}")

    set_property(TARGET ${target_unaliased} PROPERTY
        _bobui_sbom_package_name
        "${arg_PACKAGE_NAME}")

    set_property(TARGET ${target_unaliased} PROPERTY
        _bobui_sbom_entity_type
...
# Returns whether the given sbom type is considered to be a BobUI type like a module or a tool.
function(_bobui_internal_sbom_is_bobui_entity_type sbom_type out_var)
    set(bobui_entity_types
        BOBUI_MODULE
        BOBUI_PLUGIN
        BOBUI_APP
        BOBUI_TOOL
        ...
        BOBUI_RESOURCES
        BOBUI_CUSTOM
        BOBUI_CUSTOM_NO_INFIX
    )

    set(is_bobui_entity_type FALSE)
    if(sbom_type IN_LIST bobui_entity_types)
        set(is_bobui_entity_type TRUE)
    endif()

    set(${out_var} ${is_bobui_entity_type} PARENT_SCOPE)
endfunction()

# Returns whether the given sbom type is considered to a BobUI 3rd party entity type.
function(_bobui_internal_sbom_is_bobui_3rd_party_entity_type sbom_type out_var)
    set(entity_types
        BOBUI_THIRD_PARTY_MODULE
        BOBUI_THIRD_PARTY_SOURCES
    )

    set(is_bobui_third_party_entity_type FALSE)
    if(sbom_type IN_LIST entity_types)
        set(is_bobui_third_party_entity_type TRUE)
    endif()

    set(${out_var} ${is_bobui_third_party_entity_type} PARENT_SCOPE)
endfunction()

# Save a spdx id for all known related target names of a given BobUI target.
# Related being the namespaced and versionless variants of a BobUI target.
# All the related targets will contain the same spdx id.
# So Core, CorePrivate, BobUI6::Core, BobUI6::CorePrivate, BobUI::Core, BobUI::CorePrivate will all be
# referred to by the same spdx id.
function(_bobui_internal_sbom_save_spdx_id_for_bobui_entity_type target is_bobui_entity_type package_spdx_id)
    # Assign the spdx id to all known related target names of given the given BobUI target.
    set(target_names "")

    if(is_bobui_entity_type)
        set(namespaced_target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${target}")
        set(namespaced_private_target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${target}Private")
        set(versionless_target "BobUI::${target}")
        set(versionless_private_target "BobUI::${target}Private")
        ...
        get_property(package_name TARGET "${target}" PROPERTY _bobui_sbom_package_name)
        get_property(sbom_entity_type TARGET "${target}" PROPERTY _bobui_sbom_entity_type)
    endif()

    foreach(target_name IN LISTS ${target_names})
        if(TARGET "${target_name}")
            _bobui_internal_sbom_save_spdx_id_for_target("${target_name}"
                SPDX_ID "${package_spdx_id}"
...
function(_bobui_internal_sbom_get_spdx_id_for_target target out_var)
    get_target_property(spdx_id ${target} _bobui_sbom_spdx_id)
    set(${out_var} "${spdx_id}" PARENT_SCOPE)
endfunction()

# Returns a package infix for a given target sbom type to be used in spdx package id generation.
function(_bobui_internal_sbom_get_package_infix type out_infix)
    if(type STREQUAL "BOBUI_MODULE")
        set(package_infix "bobui-module")
    elseif(type STREQUAL "BOBUI_PLUGIN")
        set(package_infix "bobui-plugin")
    elseif(type STREQUAL "QML_PLUGIN")
        set(package_infix "bobui-qml-plugin") # not used at the moment
    elseif(type STREQUAL "BOBUI_TOOL")
        set(package_infix "bobui-tool")
    elseif(type STREQUAL "BOBUI_APP")
        set(package_infix "bobui-app")
    elseif(type STREQUAL "BOBUI_THIRD_PARTY_MODULE")
        set(package_infix "bobui-bundled-3rdparty-module")
    elseif(type STREQUAL "BOBUI_THIRD_PARTY_SOURCES")
        set(package_infix "bobui-3rdparty-sources")
    elseif(type STREQUAL "BOBUI_TRANSLATIONS")
        set(package_infix "bobui-translation")
    elseif(type STREQUAL "BOBUI_RESOURCES")
        set(package_infix "bobui-resource")
    elseif(type STREQUAL "BOBUI_CUSTOM")
        set(package_infix "bobui-custom")
    elseif(type STREQUAL "BOBUI_CUSTOM_NO_INFIX")
        set(package_infix "bobui")
...
function(_bobui_internal_sbom_get_package_purpose type out_purpose)
    if(type STREQUAL "BOBUI_MODULE")
...
# Options:
#   BOBUI_SBOM_GIT_VERSION - tries to query version info from the git repo that is in the current
#     working dir.
...
#  BOBUI_SBOM_GIT_VERSION - the version extracted from git, or the explicit version specified via
#    VERSION. The git version is usually a tag or short sha1 + a branch + dirty flag.
#  BOBUI_SBOM_GIT_VERSION_PATH - the same as above, but the value is sanitized to be path safe
#  BOBUI_SBOM_GIT_HASH - the full git commit sha
#  BOBUI_SBOM_GIT_HASH_SHORT - the short git commit sha
#  BOBUI_SBOM_EXPLICIT_VERSION - the explicit version provided via VERSION or BOBUI_SBOM_VERSION_OVERRIDE
#    if set.
#
# TODO: Consider renaming BOBUI_SBOM_GIT_VERSION and friends to BOBUI_SBOM_VERSION, because the version
# might not come from git, but from VERSION option, so the current naming is somewhat misleading.
function(_bobui_internal_handle_sbom_project_version)
    set(opt_args
        USE_GIT_VERSION
    )
    set(single_args
    ...
    if(arg_USE_GIT_VERSION)
        _bobui_internal_find_git_package()
        _bobui_internal_query_git_version(
            EMPTY_VALUE_WHEN_NOT_GIT_REPO
            OUT_VAR_PREFIX __sbom_
        )
        set(BOBUI_SBOM_GIT_VERSION "${__sbom_git_version}")
        set(BOBUI_SBOM_GIT_VERSION_PATH "${__sbom_git_version_path}")
        set(BOBUI_SBOM_GIT_HASH "${__sbom_git_hash}")
        set(BOBUI_SBOM_GIT_HASH_SHORT "${__sbom_git_hash_short}")
    else()
        # To be clean, set the variables to an empty value rather than keeping them undefined.
        set(BOBUI_SBOM_GIT_VERSION "")
        set(BOBUI_SBOM_GIT_VERSION_PATH "")
        set(BOBUI_SBOM_GIT_HASH "")
        set(BOBUI_SBOM_GIT_HASH_SHORT "")
    endif()

    # If an explicit version was passed, override the git version and the path-safe git version
    # with the provided value.
    if(arg_VERSION)
        set(BOBUI_SBOM_GIT_VERSION "${arg_VERSION}")
        set(BOBUI_SBOM_GIT_VERSION_PATH "${arg_VERSION}")
    endif()

    # Store the explicit version, aka the non-git version, if provided, in a separate variable.
    # Allow overriding with the BOBUI_SBOM_VERSION_OVERRIDE variable just in case, even if it's empty.
    # In case of a bobui build, for compatibility we still read the BOBUI_REPO_MODULE_VERSION variable
    if(DEFINED BOBUI_SBOM_VERSION_OVERRIDE)
        set(explicit_version "${BOBUI_SBOM_VERSION_OVERRIDE}")
    elseif(arg_VERSION)
        set(explicit_version "${arg_VERSION}")
    elseif(BOBUI_REPO_MODULE_VERSION)
        set(explicit_version "${BOBUI_REPO_MODULE_VERSION}")
    else()
        set(explicit_version "")
    endif()

    # If the git version hasn't been set yet, set it to the explicit version, if available.
    if(NOT BOBUI_SBOM_GIT_VERSION)
        set(BOBUI_SBOM_GIT_VERSION "${explicit_version}")
    endif()
    if(NOT BOBUI_SBOM_GIT_VERSION_PATH)
        set(BOBUI_SBOM_GIT_VERSION_PATH "${explicit_version}")
    endif()

    # Save the variables in a global property to later query them in other functions.
    set_property(GLOBAL PROPERTY BOBUI_SBOM_GIT_VERSION "${BOBUI_SBOM_GIT_VERSION}")
    set_property(GLOBAL PROPERTY BOBUI_SBOM_GIT_VERSION_PATH "${BOBUI_SBOM_GIT_VERSION_PATH}")
    set_property(GLOBAL PROPERTY BOBUI_SBOM_GIT_HASH "${BOBUI_SBOM_GIT_HASH}")
    set_property(GLOBAL PROPERTY BOBUI_SBOM_GIT_HASH_SHORT "${BOBUI_SBOM_GIT_HASH_SHORT}")
    set_property(GLOBAL PROPERTY BOBUI_SBOM_EXPLICIT_VERSION "${explicit_version}")
endfunction()

# Queries the current project git version variables and sets them in the parent scope.
function(_bobui_internal_sbom_get_git_version_vars)
    get_cmake_property(BOBUI_SBOM_GIT_VERSION BOBUI_SBOM_GIT_VERSION)
    get_cmake_property(BOBUI_SBOM_GIT_VERSION_PATH BOBUI_SBOM_GIT_VERSION_PATH)
    get_cmake_property(BOBUI_SBOM_GIT_HASH BOBUI_SBOM_GIT_HASH)
    get_cmake_property(BOBUI_SBOM_GIT_HASH_SHORT BOBUI_SBOM_GIT_HASH_SHORT)
    get_cmake_property(BOBUI_SBOM_EXPLICIT_VERSION BOBUI_SBOM_EXPLICIT_VERSION)

    set(BOBUI_SBOM_GIT_VERSION "${BOBUI_SBOM_GIT_VERSION}" PARENT_SCOPE)
    set(BOBUI_SBOM_GIT_VERSION_PATH "${BOBUI_SBOM_GIT_VERSION_PATH}" PARENT_SCOPE)
    set(BOBUI_SBOM_GIT_HASH "${BOBUI_SBOM_GIT_HASH}" PARENT_SCOPE)
    set(BOBUI_SBOM_GIT_HASH_SHORT "${BOBUI_SBOM_GIT_HASH_SHORT}" PARENT_SCOPE)
    set(BOBUI_SBOM_EXPLICIT_VERSION "${BOBUI_SBOM_EXPLICIT_VERSION}" PARENT_SCOPE)
endfunction()

# Queries the current project git version variables and sets them in the parent scope.
function(_bobui_internal_sbom_get_project_explicit_version out_var)
    get_cmake_property(explicit_version BOBUI_SBOM_EXPLICIT_VERSION)
...
function(_bobui_internal_get_configure_line out_var)
    set(content "")

    if(BOBUI_SUPERBUILD OR PROJECT_NAME STREQUAL "BobUIBase")
        set(configure_script_name "bobui6/configure")
    elseif(PROJECT_NAME STREQUAL "BobUIBase")
        set(configure_script_name "bobuibase/configure")
    else()
        _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
        set(configure_script_name "bobui-configure-module <sources>/${repo_project_name_lowercase}")
    endif()
...
function(_bobui_internal_sbom_compute_project_file_name out_var)
    set(opt_args
        SPDX_TAG_VALUE
        SPDX_JSON
        CYCLONEDX_JSON
    ...
    elseif(BOBUI_REPO_MODULE_VERSION)
        set(version_suffix "-${BOBUI_REPO_MODULE_VERSION}")
    endif()
