...
function(_bobui_internal_sbom_get_cyclone_dx_generator_script_name
        out_var_generator_name
        out_var_generator_relative_dir)
    set(generator_name "bobui_cyclonedx_generator.py")

    _bobui_internal_path_join(generator_relative_dir
        "util" "sbom" "cyclonedx" "bobui_cyclonedx_generator")
...
function(_bobui_internal_sbom_get_cyclone_dx_generator_path out_var)
    _bobui_internal_sbom_get_cyclone_dx_generator_script_name(generator_name generator_relative_dir)

    _bobui_internal_path_join(bobuibase_script_path
        "${BOBUI_SOURCE_TREE}" "${generator_relative_dir}" "${generator_name}")
    _bobui_internal_path_join(installed_script_path
        "${BOBUI6_INSTALL_PREFIX}" "${BOBUI6_INSTALL_LIBEXECS}" "${generator_name}")

    # bobuibase sources available, always use them, regardless if it's a prefix or non-prefix build.
    # Makes development easier.
    if(EXISTS "${bobuibase_script_path}")
        set(script_path "${bobuibase_script_path}")
...
function(_bobui_internal_sbom_parse_cydx_property_entry_options)
    set(opt_args "")
    set(single_args
        CYDX_PROPERTY_NAME
        CYDX_PROPERTY_VALUE
...
function(_bobui_internal_sbom_handle_cydx_properties)
    set(opt_args "")
    set(single_args
    ...
    foreach(prop_idx IN LISTS prop_entry_indices)
        _bobui_internal_sbom_parse_cydx_property_entry_options(
...
function(_bobui_internal_sbom_handle_bobui_entity_cydx_properties)
    set(opt_args "")
    set(single_args
        SBOM_ENTITY_TYPE
        OUT_CYDX_PROPERTIES
    ...
    list(APPEND cydx_properties
            CYDX_PROPERTY_ENTRY
                CYDX_PROPERTY_NAME "bobui:sbom:entity_type"
    ...
    if(is_bobui_entity_type)
        list(APPEND cydx_properties
                CYDX_PROPERTY_ENTRY
                    CYDX_PROPERTY_NAME "bobui:sbom:is_bobui_entity_type"
    ...
    if(is_bobui_3rd_party_entity_type)
        list(APPEND cydx_properties
                CYDX_PROPERTY_ENTRY
                    CYDX_PROPERTY_NAME "bobui:sbom:is_bobui_3rd_party_entity_type"
                    CYDX_PROPERTY_VALUE "true"
        )
    endif()
...
function(_bobui_internal_sbom_get_cyclone_component_type out_var sbom_entity_type)
    set(library_types
        "BOBUI_MODULE"
        "BOBUI_PLUGIN"
        "QML_PLUGIN"
...
# the project / document name (e.g. bobuibase) and its version or git version.
# This should alleviate the reproducibility problem as well.
function(_bobui_internal_sbom_get_cyclone_bom_serial_number)
    set(opt_args "")
    set(single_args
        SPDX_NAMESPACE
        OUT_VAR_UUID
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(SPDX_NAMESPACE "")

    _bobui_internal_sbom_get_document_namespace_uuid_namespace(uuid_namespace)
...
function(_bobui_internal_sbom_get_cydx_external_bom_link target out_var)
    get_target_property(spdx_id "${target}" _bobui_sbom_spdx_id)
    get_target_property(bom_serial_number "${target}" _bobui_sbom_cydx_bom_serial_number_uuid)
...
# later.
function(_bobui_internal_sbom_record_external_target_dependecies)
    set(opt_args "")
    set(single_args "")
    set(multi_args
        TARGETS
    ...
    get_property(existing_ids GLOBAL PROPERTY _bobui_internal_sbom_external_target_dep_ids)
    if(NOT existing_ids)
        set(existing_ids "")
    endif()
    ...
    foreach(target IN LISTS arg_TARGETS)
        ...
        set_property(GLOBAL APPEND PROPERTY _bobui_internal_sbom_external_target_dep_ids "${spdx_id}")

        # This is checked in _bobui_internal_sbom_add_target, to prevent duplicate creation of
        # system library targets.
        set_property(GLOBAL APPEND PROPERTY _bobui_internal_sbom_external_target_dependencies
            "${target}")

        get_target_property(package_name "${target}" _bobui_sbom_package_name)
        get_target_property(sbom_entity_type "${target}" _bobui_sbom_entity_type)
        get_target_property(package_version "${target}" _bobui_sbom_package_version)
        _bobui_internal_sbom_get_cydx_external_bom_link("${target}" external_bom_link)

        set_property(GLOBAL
            PROPERTY "_bobui_internal_sbom_external_target_dep_${spdx_id}_target"
            "${target}")
        set_property(GLOBAL
            PROPERTY "_bobui_internal_sbom_external_target_dep_${spdx_id}_package_name"
            "${package_name}")
        set_property(GLOBAL
            PROPERTY "_bobui_internal_sbom_external_target_dep_${spdx_id}_sbom_entity_type"
            "${sbom_entity_type}")
        set_property(GLOBAL
            PROPERTY "_bobui_internal_sbom_external_target_dep_${spdx_id}_package_version"
            "${package_version}")
        set_property(GLOBAL
            PROPERTY "_bobui_internal_sbom_external_target_dep_${spdx_id}_external_bom_link"
            "${external_bom_link}")
    endforeach()
endfunction()
...
function(_bobui_internal_sbom_add_cydx_external_target_dependencies)
    get_property(spdx_ids GLOBAL PROPERTY _bobui_internal_sbom_external_target_dep_ids)
    if(NOT spdx_ids)
        # Clean up external target dependencies, before configuring next repo project.
        set_property(GLOBAL PROPERTY _bobui_internal_sbom_external_target_dep_ids "")
        set_property(GLOBAL PROPERTY _bobui_internal_sbom_external_target_dependencies "")
        return()
    endif()
    ...
    foreach(spdx_id IN LISTS spdx_ids)
        ...
        get_cmake_property(package_name
            "_bobui_internal_sbom_external_target_dep_${spdx_id}_package_name")
        get_cmake_property(sbom_entity_type
            "_bobui_internal_sbom_external_target_dep_${spdx_id}_sbom_entity_type")
        get_cmake_property(package_version
            "_bobui_internal_sbom_external_target_dep_${spdx_id}_package_version")
        get_cmake_property(external_bom_link
            "_bobui_internal_sbom_external_target_dep_${spdx_id}_external_bom_link")
...
# later.
function(_bobui_internal_sbom_record_license_cydx)
    set(opt_args "")
    set(single_args
        LICENSE_ID
        EXTRACTED_TEXT
    ...
    set_property(GLOBAL APPEND PROPERTY
        _bobui_internal_sbom_cydx_licenses "${arg_LICENSE_ID}")
    set_property(GLOBAL PROPERTY
        _bobui_internal_sbom_cydx_licenses_${arg_LICENSE_ID}_text "${arg_EXTRACTED_TEXT}"
    )
endfunction()
