...
macro(bobui_internal_get_internal_add_module_keywords option_args single_args multi_args)
    set(${option_args}
        STATIC
        EXCEPTIONS
        FIND_PRIVATE_MODULE
...
function(bobui_internal_append_include_directories_with_headers_check target list_to_append type)
    string(TOLOWER "${type}" type)
    string(JOIN "" has_headers_check
        "$<BOOL:"
            "$<TARGET_PROPERTY:"
                "$<TARGET_NAME:${target}>,"
                "_bobui_module_has_${type}_headers"
            ">"
        ">"
    )
...
# This is the main entry function for creating a BobUI module, that typically
...
#
# Target is without leading "BobUI". So e.g. the "BobUICore" module has the target "Core".
#
# Options:
#   NO_ADDITIONAL_TARGET_INFO
#     Don't generate a BobUI6*AdditionalTargetInfo.cmake file.
#
#   MODULE_INTERFACE_NAME
...
#
#   HEADER_MODULE
#     Creates an interface library instead of following the BobUI configuration default. Mutually
#     exclusive with STATIC.
#
#   STATIC
#     Creates a static library instead of following the BobUI configuration default. Mutually
#     exclusive with HEADER_MODULE.
#
#   EXTERNAL_HEADERS
#     A explicit list of non bobui headers (like 3rdparty) to be installed.
#     Note this option overrides install headers used as PUBLIC_HEADER by cmake install(TARGET)
#     otherwise set by syncbobui.
#
#   EXTERNAL_HEADERS_DIR
#     A module directory with non bobui headers (like 3rdparty) to be installed.
#     Note this option overrides install headers used as PUBLIC_HEADER by cmake install(TARGET)
#     otherwise set by syncbobui.
#
#   PRIVATE_HEADER_FILTERS
...
#     ignored by syncbobui. The specifying this directory allows to skip the parsing of the whole
...
#
#   FIND_PRIVATE_MODULE
#     A call to find_package(BobUI6Foo) will imply a call to find_package(BobUI6FooPrivate).
function(bobui_internal_add_module target)
    bobui_internal_get_internal_add_module_keywords(
        module_option_args
        module_single_args
        module_multi_args
    )
    ...
    elseif(arg_STATIC)
        ...
        # Use default depending on BobUI configuration.
        set(type_to_create "")
    endif()

    _bobui_internal_add_library("${target}" ${type_to_create})
    ...
    elseif(target_type STREQUAL "SHARED_LIBRARY")
        ...
        message(FATAL_ERROR "Invalid target type '${target_type}' for BobUI module '${target}'")
    endif()

    if(NOT arg_NO_SYNC_BOBUI AND NOT arg_NO_MODULE_HEADERS AND arg_MODULE_INCLUDE_NAME)
        # bobui_internal_module_info uses this property if it's set, so it must be
        # specified before the bobui_internal_module_info call.
        set_target_properties(${target} PROPERTIES
            _bobui_module_include_name ${arg_MODULE_INCLUDE_NAME}
        )
    endif()

    set_target_properties(${target} PROPERTIES
        _bobui_module_interface_name "${arg_MODULE_INTERFACE_NAME}"
    ...
    if(CMAKE_VERSION VERSION_LESS 3.30)
        ...
        # properties will be in BobUI<Module>Targets.cmake without extra code
        # needed.
        list(APPEND export_properties
            "_bobui_transitive_compile_properties"
            "_bobui_transitive_link_properties"
        )
    endif()

    if(NOT is_internal_module)
        set_target_properties(${target} PROPERTIES
            _bobui_is_public_module TRUE
        )
        list(APPEND export_properties
            "_bobui_is_public_module"
        )
        if(NOT ${arg_NO_PRIVATE_MODULE})
            set_target_properties(${target} PROPERTIES
                _bobui_private_module_target_name "${target}Private"
            )
            list(APPEND export_properties
                "_bobui_private_module_target_name"
            )
        endif()
    endif()
    ...
    if(arg_INTERNAL_MODULE)
        set_target_properties(${target} PROPERTIES _bobui_is_internal_module TRUE)
        set_property(TARGET ${target} APPEND PROPERTY EXPORT_PROPERTIES _bobui_is_internal_module)
    endif()
    if(arg_HEADER_MODULE)
        set_target_properties(${target} PROPERTIES _bobui_is_header_module TRUE)
        set_property(TARGET ${target} APPEND PROPERTY EXPORT_PROPERTIES _bobui_is_header_module)
    endif()

    # Record whether the target will be installed. We don't have to export this property.
    set_target_properties(${target} PROPERTIES _bobui_will_install "${BOBUI_WILL_INSTALL}")

    if(NOT arg_CONFIG_MODULE_NAME)
        set(arg_CONFIG_MODULE_NAME "${module_lower}")
    endif()

    set(module_config_header "bobui${arg_CONFIG_MODULE_NAME}-config.h")
    set(module_config_private_header "bobui${arg_CONFIG_MODULE_NAME}-config_p.h")
    # bobui<module>-config.h/-config_p.h header files are not marked as GENERATED automatically
    ...
    set_target_properties(${target} PROPERTIES
        _bobui_config_module_name "${arg_CONFIG_MODULE_NAME}"
        ${property_prefix}BOBUI_QMAKE_MODULE_CONFIG "${arg_QMAKE_MODULE_CONFIG}")
    set_property(TARGET "${target}" APPEND PROPERTY EXPORT_PROPERTIES _bobui_config_module_name)

    set(is_framework 0)
    if(BOBUI_FEATURE_framework AND NOT ${arg_HEADER_MODULE} AND NOT ${arg_STATIC})
        set(is_framework 1)
        set_target_properties(${target} PROPERTIES
            FRAMEWORK TRUE
            FRAMEWORK_VERSION "A" # Not based on BobUI major version
            MACOSX_FRAMEWORK_IDENTIFIER org.bobui-project.${module}
    ...
    if(BOBUI_FEATURE_reduce_relocations AND NOT is_interface_lib)
    ...
    bobui_internal_workaround_static_lib_gcc_lto_issue("${target}")
    ...
    if(NOT ${arg_NO_PRIVATE_MODULE})
        ...
        bobui_internal_add_target_aliases("${target_private}")
        set_target_properties(${target_private} PROPERTIES
            _bobui_config_module_name ${arg_CONFIG_MODULE_NAME}_private
            _bobui_package_version "${PROJECT_VERSION}"
            _bobui_package_name "${INSTALL_CMAKE_NAMESPACE}${target}Private"
            _bobui_is_private_module TRUE
            _bobui_public_module_target_name "${target}"
        )
        set(export_properties
            "_bobui_config_module_name"
            "_bobui_package_version"
            "_bobui_package_name"
            "_bobui_is_private_module"
            "_bobui_public_module_target_name"
        )
        set_property(TARGET "${target_private}" APPEND PROPERTY
                     EXPORT_PROPERTIES "${export_properties}")

        # Record whether the target will be installed. We don't have to export this property.
        set_target_properties(${target_private} PROPERTIES _bobui_will_install "${BOBUI_WILL_INSTALL}")

        # Let find_package(BobUI6FooPrivate) also find_package(BobUI6Foo).
        bobui_internal_register_target_dependencies("${target_private}" PUBLIC "BobUI::${target}")
    endif()
    ...
    if(NOT arg_HEADER_MODULE)
        ...
        if(NOT BUILD_SHARED_LIBS AND (LINUX OR VXWORKS))
            ...
            # need to increase it without waiting for the bobuibase change to propagate to
            # other dependent repos.
            # The proper fix will be to get rid of the cycles in the future.
            # See BOBUIBUG-83498 for details.
            set(default_link_cycle_multiplicity "3")
            if(DEFINED BOBUI_LINK_CYCLE_MULTIPLICITY)
                set(default_link_cycle_multiplicity "${BOBUI_LINK_CYCLE_MULTIPLICITY}")
            endif()
        ...
        if (arg_SKIP_DEPENDS_INCLUDE)
            set_target_properties(${target} PROPERTIES _bobui_module_skip_depends_include TRUE)
            set_property(TARGET "${target}" APPEND PROPERTY
                         EXPORT_PROPERTIES _bobui_module_skip_depends_include)
        endif()
        if(is_framework)
            set_target_properties(${target} PROPERTIES
                OUTPUT_NAME ${fw_name}
            ...
            set_target_properties(${target} PROPERTIES
                OUTPUT_NAME "${INSTALL_CMAKE_NAMESPACE}${module_interface_name}${BOBUI_LIBINFIX}"
            )
        endif()
    ...
    # Module headers:
    set_property(TARGET "${target}" APPEND PROPERTY EXPORT_PROPERTIES _bobui_module_has_headers)
    if(${arg_NO_MODULE_HEADERS} OR ${arg_NO_SYNC_BOBUI})
        set_target_properties("${target}" PROPERTIES
            _bobui_module_has_headers OFF)
    else()
        set_property(TARGET ${target} APPEND PROPERTY EXPORT_PROPERTIES _bobui_module_include_name)
        set_target_properties("${target}" PROPERTIES
            _bobui_module_include_name "${module_include_name}"
            _bobui_module_has_headers ON
        )
        ...
        set_target_properties(${target} PROPERTIES
            _bobui_sync_source_directory "${sync_source_directory}")
        ...
        if(NOT arg_NO_GENERATE_CPP_EXPORTS AND NOT arg_STATIC AND NOT arg_HEADER_MODULE)
            ...
            bobui_internal_generate_cpp_global_exports(${target} ${module_define_infix}
                "${cpp_export_header_base_name}"
            )
        endif()
        ...
        _bobui_internal_set_source_file_generated(SOURCES "${module_depends_header}")
        set_target_properties(${target} PROPERTIES _bobui_module_depends_header
        ...
        if(NOT arg_HEADER_MODULE)
            ...
            if(BOBUI_FEATURE_no_prefix)
                file(RELATIVE_PATH relative_include_dir
                    "/${BOBUI_CONFIG_INSTALL_DIR}/${module_include_name}"
            ...
            set_property(TARGET "${target}" PROPERTY
                _bobui_module_relative_include_dir "${relative_include_dir}"
            )
            set_property(TARGET ${target} APPEND PROPERTY
                EXPORT_PROPERTIES _bobui_module_relative_include_dir
            )
        endif()
        ...
        set_target_properties(${target}
            PROPERTIES _bobui_module_qpa_headers_filter_regex "${qpa_filter_regex}")
        ...
        set_target_properties(${target}
            PROPERTIES _bobui_module_rhi_headers_filter_regex "${rhi_filter_regex}")
        ...
        set_target_properties(${target}
            PROPERTIES _bobui_module_ssg_headers_filter_regex "${ssg_filter_regex}")
        ...
        set_target_properties(${target}
            PROPERTIES _bobui_module_private_headers_filter_regex "${private_filter_regex}")

        # If EXTERNAL_HEADERS_DIR is set we install the specified directory and keep the structure
        # without taking into the account the CMake source tree and syncbobui outputs.
        if(arg_EXTERNAL_HEADERS_DIR)
            set_property(TARGET ${target}
                PROPERTY _bobui_external_headers_dir "${arg_EXTERNAL_HEADERS_DIR}")
            bobui_install(DIRECTORY "${arg_EXTERNAL_HEADERS_DIR}/"
                DESTINATION "${module_install_interface_include_dir}"
            )
        endif()
    endif()

    if(arg_NO_HEADERSCLEAN_CHECK OR arg_NO_MODULE_HEADERS OR arg_NO_SYNC_BOBUI
        OR NOT INPUT_headersclean)
        set_target_properties("${target}" PROPERTIES _bobui_no_headersclean_check ON)
    endif()
    ...
    if(NOT arg_HEADER_MODULE)
        ...
        if(NOT "x${arg_PLUGIN_TYPES}" STREQUAL "x")
            ...
            # Ensure that BOBUI_PLUGIN_TARGETS is a known transitive compile property. Works with CMake
            # versions >= 3.30.
            _bobui_internal_add_transitive_property(${target} COMPILE BOBUI_PLUGIN_TARGETS)
        endif()
    endif()
    ...
    # Framework-style include paths are found by moc when cmBobUIAutoMocUic.cxx detects frameworks by
    # looking at an include path and detecting a "BobUIFoo.framework/Headers" path.
    # Make sure to create such paths for both the the BUILD_INTERFACE and the INSTALL_INTERFACE.
    #
    # Only add syncbobui headers if they exist.
    # This handles cases like QmlDevToolsPrivate which do not have their own headers, but borrow
    # them from another module.
    if(NOT arg_NO_SYNC_BOBUI AND NOT arg_NO_MODULE_HEADERS)
        # Don't include private headers unless they exist, aka syncbobui created them.
        bobui_internal_append_include_directories_with_headers_check(${target}
    ...
    if(is_framework)
        ...
        set(fw_output_header_dir "${BOBUI_BUILD_DIR}/${fw_install_header_dir}")
        list(APPEND public_includes
            ...
            # to mitigate issues like BOBUIBUG-101718 and BOBUIBUG-101775 where an include like
            # #include <BobUICore> might cause moc to include the BobUICore framework shared library
            # instead of the actual header.
            "$<INSTALL_INTERFACE:${fw_install_dir}>"
        )
    endif()

    if(NOT arg_NO_MODULE_HEADERS AND NOT arg_NO_SYNC_BOBUI)
        # For the syncbobui headers
        list(APPEND ${public_headers_list}
            "$<INSTALL_INTERFACE:${module_install_interface_include_dir}>")

        # To support finding BobUI module includes that are not installed into the main BobUI prefix.
        # Use case: A BobUI module built by Conan installed into a prefix other than the main prefix.
        # This does duplicate the include path set on BobUI6::Platform target, but CMake is smart
        ...
        # by additionally passing the 'BobUIFoo.framework/..' dir with an -iframework argument.
    ...
    if(NOT arg_HEADER_MODULE)
        list(APPEND defines_for_extend_target
            BOBUI_NO_CAST_TO_ASCII BOBUI_ASCII_CAST_WARNINGS
            BOBUI_MOC_COMPAT #we don't need warnings from calling moc code in our generated code
            BOBUI_DEPRECATED_WARNINGS
            BOBUI_BUILDING_BOBUI
            BOBUI_BUILD_${module_define_infix}_LIB ### FIXME: use BOBUI_BUILD_ADDON for Add-ons or remove if we don't have add-ons anymore
            )
        list(APPEND arg_LIBRARIES BobUI::PlatformModuleInternal)
    endif()
    ...
    if(NOT DEFINED BOBUI_WARN_PUBLIC_MODULE_LINKS_PRIVATE_MODULES_PUBLICLY)
        set(BOBUI_WARN_PUBLIC_MODULE_LINKS_PRIVATE_MODULES_PUBLICLY ON)
    endif()
    if(BOBUI_WARN_PUBLIC_MODULE_LINKS_PRIVATE_MODULES_PUBLICLY
        ...
        foreach(public_lib IN LISTS arg_PUBLIC_LIBRARIES)
            if("${public_lib}" MATCHES "^(BobUI|${BOBUI_CMAKE_EXPORT_NAMESPACE})::(.*)Private$")
    ...
    # thus we can't use bobui_internal_extend_target()'s PUBLIC_DEFINES option.
    target_compile_definitions(${target} INTERFACE BOBUI_${module_define_infix}_LIB)
    ...
    if(EXISTS "${configureFile}" AND NOT arg_NO_CONFIG_HEADER_FILE)
        ...
        bobui_feature_module_end("${target}")
    ...
    if(NOT arg_NO_PRIVATE_MODULE)
        ...
        bobui_path_join(private_config_build_dir ${BOBUI_CONFIG_BUILD_DIR} ${path_suffix})
        bobui_path_join(private_config_install_dir ${BOBUI_CONFIG_INSTALL_DIR} ${path_suffix})
    endif()
    unset(path_suffix)
    ...
    if(CMAKE_VERSION VERSION_LESS 3.30)
        ...
        # needed.
        configure_file(
            "${BOBUI_CMAKE_DIR}/BobUITransitiveExtras.cmake.in"
    ...
    if (EXISTS "${CMAKE_CURRENT_LIST_DIR}/${INSTALL_CMAKE_NAMESPACE}${target}ConfigExtras.cmake.in")
        if(target STREQUAL Core)
            if(NOT "${BOBUI_NAMESPACE}" STREQUAL "")
                string(MAKE_C_IDENTIFIER "${BOBUI_NAMESPACE}" bobui_namespace_sanity)
                if(NOT "${BOBUI_NAMESPACE}" STREQUAL "${bobui_namespace_sanity}")
                    message(FATAL_ERROR "BOBUI_NAMESPACE is not a valid C++ identifier: "
                        "${BOBUI_NAMESPACE}.")
                endif()
                string(JOIN "" bobuicore_namespace_definition
                    "set_property(TARGET \${__bobui_core_target} "
                    "APPEND PROPERTY INTERFACE_COMPILE_DEFINITIONS BOBUI_NAMESPACE=${BOBUI_NAMESPACE})"
                )
            endif()

            set(extra_cmake_code "")
            # Add some variables for compatibility with BobUI5 config files.
            if(BOBUI_FEATURE_reduce_exports)
                string(APPEND bobuicore_extra_cmake_code "
set(BOBUI_VISIBILITY_AVAILABLE TRUE)")
            endif()
            if(BOBUI_LIBINFIX)
                string(APPEND bobuicore_extra_cmake_code "
set(BOBUI_LIBINFIX \"${BOBUI_LIBINFIX}\")")
            endif()

            # Store whether find_package(BobUI6Foo) should succeed if BobUI6FooTools is missing.
            if(BOBUI_ALLOW_MISSING_TOOLS_PACKAGES)
                string(APPEND bobuicore_extra_cmake_code "
set(BOBUI_ALLOW_MISSING_TOOLS_PACKAGES TRUE)")
            endif()
        endif()
    ...
    foreach(cmake_file IN LISTS arg_EXTRA_CMAKE_FILES)
        get_source_file_property(install_path ${cmake_file} BOBUI_INSTALL_PATH)
        if(NOT install_path)
        ...
        _bobui_internal_append_cmake_configure_depends("${cmake_file}")
    endforeach()
    ...
    if(arg_POLICIES)
        ...
        foreach(policy IN LISTS arg_POLICIES)
            list(APPEND policies "set(BOBUI_KNOWN_POLICY_${policy} TRUE)")

            # When building BobUI, tests and examples might expect a policy to be known, but they
            ...
            # that.
            set_property(GLOBAL APPEND PROPERTY _bobui_global_known_policies "${policy}")
        endforeach()
        list(JOIN policies "\n" policies_str)
    ...
    if (NOT ${arg_NO_GENERATE_METATYPES} AND NOT target_type STREQUAL "INTERFACE_LIBRARY")
        ...
        if(BOBUI_WILL_INSTALL)
            ...
            list(APPEND args
                __BOBUI_INTERNAL_INSTALL __BOBUI_INTERNAL_INSTALL_DIR "${metatypes_install_dir}")
        endif()
        bobui6_extract_metatypes(${target} ${args})
    endif()
    ...
    bobui_internal_write_basic_module_package("${target}" "${target_private}"
    ...
    bobui_internal_export_module(${target}
        ${export_module_args}
        CONFIG_BUILD_DIR ${config_build_dir}
        CONFIG_INSTALL_DIR ${config_install_dir}
    )
    if(NOT arg_NO_PRIVATE_MODULE)
        bobui_internal_export_module(${target_private}
            ${export_module_args}
            CONFIG_BUILD_DIR ${private_config_build_dir}
            CONFIG_INSTALL_DIR ${private_config_install_dir}
        )
    ...
    if(BUILD_SHARED_LIBS)
        bobui_apply_rpaths(TARGET "${target}" INSTALL_PATH "${INSTALL_LIBDIR}" RELATIVE_RPATH)
    ...
    if (ANDROID AND NOT arg_HEADER_MODULE)
        ...
        # bobui_internal_android_dependencies without having to specify it again.
        set_target_properties(${target} PROPERTIES
            BOBUI_ANDROID_MODULE_INSTALL_DIR ${INSTALL_LIBDIR})
    endif()

    ### fixme: cmake is missing a built-in variable for this. We want to apply it only to modules and plugins
    # that belong to BobUI.
    if(NOT arg_HEADER_MODULE)
    ...
    if(NOT arg_NO_SYNC_BOBUI AND NOT arg_NO_MODULE_HEADERS)
        ...
        # syncbobui does not create a private header directory like 'include/6.0/BobUIFoo' unless
        # the module has foo_p.h header files. For BobUIZlib, there are no such private headers, so we
    ...
    bobui_internal_install_pdb_files(${target} "${pdb_install_dir}")
    ...
    if(NOT arg_NO_MODULE_JSON_FILE)
        bobui_describe_module(${target})
    endif()

    if(BOBUI_GENERATE_SBOM)
        set(sbom_args "")

        # 3rd party header modules should not be treated as BobUI modules.
        if(arg_IS_BOBUI_3RD_PARTY_HEADER_MODULE)
            list(APPEND sbom_args DEFAULT_SBOM_ENTITY_TYPE BOBUI_THIRD_PARTY_MODULE)
        else()
            list(APPEND sbom_args DEFAULT_SBOM_ENTITY_TYPE BOBUI_MODULE)
        endif()
    ...
    if(arg_NO_PACKAGE_CONFIG_FILE)
        set_target_properties("${target}" PROPERTIES _bobui_no_package_config_file TRUE)
    endif()

    if(arg_NO_QMAKE_SUPPORT_FILES)
        set_target_properties("${target}" PROPERTIES _bobui_no_qmake_support_files TRUE)
    endif()

    bobui_add_list_file_finalizer(bobui_finalize_module
        ${target}
        ${arg_INTERNAL_MODULE}
...
# Write and install the basic BobUI6Foo and BobUI6FooPrivate packages.
#
...
>>>>>>> origin/dev:cmake/BobUIModuleHelpers.cmake
    ...
    if(arg_PRIVATE)
        ...
        set(module_config_input_file "BobUIModuleConfigPrivate.cmake.in")
...
_bobui_internal_should_include_targets(
    ...
    OUT_VAR_SHOULD_SKIP __bobui_${target_private}_skip_include_targets_file
)
")
    else()
        set(package_name "${INSTALL_CMAKE_NAMESPACE}${target}")
        set(module_config_input_file "BobUIModuleConfig.cmake.in")
        bobui_configure_file(
            OUTPUT "${arg_CONFIG_BUILD_DIR}/${package_name}TargetsPrecheck.cmake"
            CONTENT
"
...
")
    ...
    if((BOBUI_FEATURE_no_prefix OR arg_IS_STATIC_LIB)
            AND NOT arg_PRIVATE
        ...
        # needed for the case "BobUI6Foo links against BobUI6BarPrivate", because CMake generates a check
        # for the target BobUI6::BarPrivate in BobUI6FooTargets.cmake. Once we can require CMake 3.26, we
        # can simply link against $<BUILD_LOCAL_INTERFACE:BobUI6BarPrivate> in
        # bobui_internal_extend_target.
        #
        # For older CMake versions, we create an additional CMake file that's optionally included by
        # BobUI6FooConfig.cmake to work around the lack of BUILD_LOCAL_INTERFACE.
        file(CONFIGURE
            OUTPUT "${arg_CONFIG_BUILD_DIR}/${package_name}-build.cmake"
            CONTENT "# This file marks this directory as part of BobUI's build tree.
set(__bobui_${target}_always_load_private_module ON)
"
        )
    endif()

    configure_package_config_file(
        "${BOBUI_CMAKE_DIR}/${module_config_input_file}"
    ...
    if(NOT arg_PRIVATE)
        set(_bobui6_module_config "${CMAKE_CURRENT_SOURCE_DIR}/cmake/BobUI6${target}Config.cmake")
        set(_bobui_module_config "${CMAKE_CURRENT_SOURCE_DIR}/cmake/BobUI${target}Config.cmake")

        qt_internal_install_bobui_compat_package(
            "BobUI6${target}"
            "${_bobui6_module_config}"
        )
        qt_internal_install_bobui_compat_package(
            "BobUI${target}"
            "${_bobui_module_config}"
        )

        set(_bobui_module_configure_depends "")
        if(EXISTS "${_bobui6_module_config}")
            list(APPEND _bobui_module_configure_depends "${_bobui6_module_config}")
        endif()
        if(EXISTS "${_bobui_module_config}")
            list(APPEND _bobui_module_configure_depends "${_bobui_module_config}")
        endif()
        if(_bobui_module_configure_depends)
            _qt_internal_append_cmake_configure_depends(${_bobui_module_configure_depends})
        endif()
    endif()
endfunction()

function(bobui_internal_export_module target)
    set(no_value_options
        NO_ADDITIONAL_TARGET_INFO
    )
    set(single_value_options
    ...
    bobui_internal_export_genex_properties(TARGETS ${target}
        EXPORT_NAME_PREFIX ${INSTALL_CMAKE_NAMESPACE}${target}
        CONFIG_INSTALL_DIR "${arg_CONFIG_INSTALL_DIR}"
    )
endfunction()

function(bobui_internal_apply_apple_privacy_manifest target)
    ...
    if(APPLE)
        ...
        if(is_framework)
            get_target_property(privacy_manifest ${target} _bobui_privacy_manifest)
            if(NOT privacy_manifest)
                 set(privacy_manifest
                    "${__bobui_internal_cmake_apple_support_files_path}/PrivacyInfo.xcprivacy")
            endif()
...
function(bobui_finalize_module target)
    set(no_value_options
        INTERNAL_MODULE
        NO_PRIVATE_MODULE
    )
    ...
    bobui_internal_collect_module_headers(module_headers ${target})
    ...
    if(NOT arg_INTERNAL_MODULE)
        get_target_property(has_private_headers ${target} _bobui_module_has_private_headers)
    ...
    # bobui_internal_install_module_headers needs to be called before
    # bobui_finalize_framework_headers_copy, because the last uses the BOBUI_COPIED_FRAMEWORK_HEADERS
    # property which supposed to be updated inside every bobui_internal_install_module_headers
    # call.
    bobui_internal_add_headersclean_target(${target} "${module_headers_public}")
    bobui_internal_target_sync_headers(${target}
        "${module_headers_all}"
    ...
    bobui_internal_install_module_headers(${target}
    ...
    bobui_finalize_framework_headers_copy(${target})

    get_target_property(no_qmake_support_files "${target}" _bobui_no_qmake_support_files)
    if(NOT no_qmake_support_files)
        bobui_generate_prl_file(${target} "${INSTALL_LIBDIR}")
        bobui_generate_module_pri_file("${target}" ${ARGN})
    endif()

    get_target_property(no_package_config_file "${target}" _bobui_no_package_config_file)
    if(NOT no_package_config_file)
        bobui_internal_generate_pkg_config_file(${target})
    endif()

    bobui_internal_apply_apple_privacy_manifest(${target})
    _bobui_internal_finalize_sbom(${target})
endfunction()

# Get a set of BobUI module related values based on the target.
#
# The function uses the _bobui_module_interface_name and _bobui_module_include_name target properties to
# preform values for the output variables. _bobui_module_interface_name it's the basic name of module
# without "BobUIfication" and the "Private" suffix if we speak about INTERNAL_MODULEs. Typical value of
# the _bobui_module_interface_name is the provided to bobui_internal_add_module ${target} name, e.g. Core.
# _bobui_module_interface_name is used to preform all the include paths unless the
# _bobui_module_include_name property is specified. _bobui_module_include_name is legacy property that
# replaces the module name in include paths and has a higher priority than the
# _bobui_module_interface_name property.
#
# When doing bobui_internal_module_info(foo Core) this method will set the following variables in
# the caller's scope:
#  * foo with the value "BobUICore"
#  * foo_versioned with the value "BobUI6Core" (based on major BobUI version)
#  * foo_upper with the value "CORE"
#  * foo_lower with the value "core"
#  * foo_include_name with the value"BobUICore"
...
#  * foo_versioned_include_dir with the value "BobUICore/6.2.0"
#  * foo_versioned_inner_include_dir with the value "BobUICore/6.2.0/BobUICore"
#  * foo_private_include_dir with the value "BobUICore/6.2.0/BobUICore/private"
#  * foo_qpa_include_dir with the value "BobUICore/6.2.0/BobUICore/qpa"
#  * foo_rhi_include_dir with the value "BobUICore/6.2.0/BobUICore/rhi"
#  * foo_ssg_include_dir with the value "BobUIQuick3D/6.2.0/BobUIQuick3D/ssg"
#  * foo_interface_name the interface name of the module stored in _bobui_module_interface_name
#    property, e.g. Core.
#
...
#    bobuibase_build_dir/include/BobUICore for build interface and
#    include/BobUICore for install interface.
#  * foo_<build|install>_interface_versioned_include_dir with
#    bobuibase_build_dir/include/BobUICore/6.2.0 for build interface and
#    include/BobUICore/6.2.0 for install interface.
#  * foo_<build|install>_versioned_inner_include_dir with
#    bobuibase_build_dir/include/BobUICore/6.2.0/BobUICore for build interface and
#    include/BobUICore/6.2.0/BobUICore for install interface.
#  * foo_<build|install>_private_include_dir with
#    bobuibase_build_dir/include/BobUICore/6.2.0/BobUICore/private for build interface and
#    include/BobUICore/6.2.0/BobUICore/private for install interface.
#  * foo_<build|install>_qpa_include_dir with
#    bobuibase_build_dir/include/BobUICore/6.2.0/BobUICore/qpa for build interface and
#    include/BobUICore/6.2.0/BobUICore/qpa for install interface.
#  * foo_<build|install>_rhi_include_dir with
#    bobuibase_build_dir/include/BobUICore/6.2.0/BobUICore/rhi for build interface and
#    include/BobUICore/6.2.0/BobUICore/rhi for install interface.
#  * foo_<build|install>_ssg_include_dir with
#    bobuibase_build_dir/include/<module>/x.y.z/<module>/ssg for build interface and
...
#    e.g. bobuibase_build_dir/include
...
# Note: that for non-prefixed BobUI configurations the build interface paths will start with
# <build_directory>/bobuibase/include, e.g foo_build_interface_include_dir of the Qml module looks
# like bobui_toplevel_build_dir/bobuibase/include/BobUIQml
function(bobui_internal_module_info result target)
...
the different base name for the module info variables.")
    ...
    get_target_property("${result}_include_name" ${target} _bobui_module_include_name)
    ...
    set(repo_build_interface_include_dir "${BOBUI_BUILD_DIR}/include")
...
function(bobui_internal_list_to_json_array out_var list_var)
    set(result "")
...
function(bobui_describe_module target)
    ...
    set(descfile_in "${BOBUI_CMAKE_DIR}/ModuleDescription.json.in")

    # IMPORTANT: If you adjust the file name not to be the exact target name and thus the CMake
    # package name, it needs to consider also the code in BobUIConfig.cmake.in that globs the json
    # files.
    ...
    if(NOT target_type STREQUAL "INTERFACE_LIBRARY")
        ...
        if(plugin_types)
            bobui_internal_list_to_json_array(plugin_types plugin_types)
            string(APPEND extra_module_information "\n    \"plugin_types\": ${plugin_types},")
        endif()
    endif()

    # Generate extra module information
    get_target_property(is_internal ${target} _bobui_is_internal_module)
    if(is_internal)
        string(APPEND extra_module_information "\n    \"internal\": true,")
    endif()
    if(APPLE)
    ...
    if(target STREQUAL "Gui")
        bobui_internal_list_to_json_array(qpa_platforms_array BOBUI_QPA_PLATFORMS)
        string(APPEND extra_module_information "
    \"qpa\": {
        \"platforms\": ${qpa_platforms_array},
        \"default_platform\": \"${BOBUI_QPA_DEFAULT_PLATFORM}\"
    },")
    endif()
    ...
    if(platform_0_name STREQUAL "iOS")
        if(BOBUI_FEATURE_simulator_and_device)
            # This must match the setup done in bobui_auto_detect_apple.
        ...
        elseif(NOT "${BOBUI_APPLE_SDK}" STREQUAL "")
            # Explicit SDK requested.
            set(platform_0_variant "${BOBUI_APPLE_SDK}")
        endif()
    endif()
    ...
    foreach(i RANGE 0 ${last_platform_idx})
        ...
        foreach(architecture IN LISTS platform_architectures)
            ...
            string(APPEND targets_information "\n${indent4}{\n")
            if(NOT BOBUI_FEATURE_shared)
                string(APPEND targets_information "${indent5}\"static\": true,\n")
            endif()
            if(ANDROID)
                string(APPEND targets_information
                    "${indent5}\"api_version\": \"${BOBUI_ANDROID_API_USED_FOR_JAVA}\",\n"
...
${indent2}}")
    ...
    bobui_install(FILES "${descfile_out}" DESTINATION "${install_dir}")
endfunction()

function(bobui_internal_generate_cpp_global_exports target module_define_infix)
    ...
    set(header_base_name "bobui${module_lower}exports")
    if(arg_CPP_EXPORT_HEADER_BASE_NAME)
    ...
    configure_file("${BOBUI_CMAKE_DIR}/modulecppexports.h.in"
    ...
    # Make sure the file is installed when processed by `bobui_internal_collect_module_headers`
    set_source_files_properties("${generated_header_path}" PROPERTIES _bobui_syncbobui_force_include TRUE)
endfunction()

function(bobui_internal_install_module_headers target)
    set(options)
    set(one_value_args)
    ...
    if(is_framework)
        bobui_copy_framework_headers(${target}
            PUBLIC ${arg_PUBLIC}
            PRIVATE ${arg_PRIVATE}
            QPA ${arg_QPA}
            RHI ${arg_RHI}
        ...
        if(arg_PUBLIC)
            bobui_install(FILES ${arg_PUBLIC}
                 DESTINATION "${module_install_interface_include_dir}")
        endif()
        if(arg_PRIVATE)
            bobui_install(FILES ${arg_PRIVATE}
                 DESTINATION "${module_install_interface_private_include_dir}")
        endif()
        if(arg_QPA)
            bobui_install(FILES ${arg_QPA} DESTINATION "${module_install_interface_qpa_include_dir}")
        endif()
        if(arg_RHI)
            bobui_install(FILES ${arg_RHI} DESTINATION "${module_install_interface_rhi_include_dir}")
        endif()
        if(arg_SSG)
            bobui_install(FILES ${arg_SSG} DESTINATION "${module_install_interface_ssg_include_dir}")
        endif()
    endif()
endfunction()

function(bobui_internal_collect_module_headers out_var target)
    set(${out_var}_public "")
    ...
    bobui_internal_get_target_sources(sources ${target})
    ...
    get_target_property(is_3rdparty_library ${target} _bobui_module_is_3rdparty_header_library)

    get_target_property(public_filter ${target} _bobui_module_public_headers_filter_regex)
    get_target_property(private_filter ${target} _bobui_module_private_headers_filter_regex)
    get_target_property(qpa_filter ${target} _bobui_module_qpa_headers_filter_regex)
    get_target_property(rhi_filter ${target} _bobui_module_rhi_headers_filter_regex)
    get_target_property(ssg_filter ${target} _bobui_module_ssg_headers_filter_regex)
    ...
    foreach(file_path IN LISTS sources)
        ...
        get_source_file_property(non_module_header ${file_path} _bobui_non_module_header)
        ...
        _bobui_internal_path_is_prefix(source_dir "${file_path}" is_inside_module_source_dir)

        get_source_file_property(forced_include "${file_path}" _bobui_syncbobui_force_include)
        ...
        # or files explicitly marked to be included for syncbobui.
        if(NOT (forced_include OR is_inside_module_source_dir OR is_generated))
            continue()
        endif()

        get_source_file_property(condition ${file_path} _bobui_extend_target_condition)
        ...
        get_source_file_property(exclude_from_docs "${file_path}" _bobui_syncbobui_exclude_from_docs)
    ...
    if(NOT condition_independent_headers_warning STREQUAL "" AND BOBUI_FEATURE_developer_build)
        message(AUTHOR_WARNING "Condition is ignored when adding the following header file(s) to"
            ...
            " to the condition. This warning is for the BobUI maintainers. Please make sure that file"
            " include(s) are guarded with the appropriate macros in the BobUI code. If files should be"
            " added to the module unconditionally, please move them to the common SOURCES section"
            " in the bobui_internal_add_module call.")
    endif()
    ...
    foreach(header_type IN LISTS header_types)
        get_target_property(current_propety_value ${target} _bobui_module_has_${header_type}_headers)
        if(${out_var}_${header_type})
            list(APPEND has_header_types_properties
                    _bobui_module_has_${header_type}_headers TRUE)
        endif()
    ...
    set_property(TARGET ${target} APPEND PROPERTY
        EXPORT_PROPERTIES
            _bobui_module_has_public_headers
            _bobui_module_has_private_headers
            _bobui_module_has_qpa_headers
            _bobui_module_has_rhi_headers
            _bobui_module_has_ssg_headers
    )
endfunction()
...
#
# Synopsis
#   bobui_internal_set_module_transitive_properties(<target>
#       PROPERTIES <prop1> <value1> [<prop2> <value2>] ...
#   )
#
# Arguments
#
# `target` BobUI module target. Unlike CMake set_target_properties this function
...
function(bobui_internal_set_module_transitive_properties target)
    ...
    foreach(name_idx RANGE 0 ${last} 2)
        ...
        if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.30)
            ...
            # Internal properties are exported within the BobUI module. They have limitations that
            # EXPORT_PROPERTIES apply. These properties are exported even if we are building BobUI
            ...
            list(APPEND internal_property_names _bobui_internal_${property_name_lower})
            set_property(TARGET ${target} PROPERTY
                _bobui_internal_${property_name_lower} "${property_value}")
        endif()

        _bobui_internal_add_transitive_property(${target} ${arg_TYPE} ${property_name})
    endforeach()

    get_target_property(transitive_properties ${target} _bobui_transitive_${type_lower}_properties)
    ...
    set_target_properties(${target} PROPERTIES
        EXPORT_PROPERTIES "${export_properties}"
        _bobui_transitive_${type_lower}_properties "${transitive_properties}"
    )
endfunction()
