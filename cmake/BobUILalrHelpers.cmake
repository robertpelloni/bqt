...
function(bobui_regex_match_and_get input_line regex replacement out_var)
...
function(bobui_qlalr_find_option_in_list input_list regex out_var)
    foreach(line ${input_list})
        bobui_regex_match_and_get("${line}" "${regex}" "\\1" option)
        if(option)
    ...
    endforeach()
    message(FATAL_ERROR "bobui_qlalr_find_option_in_list: Could not extract ${out_var}")
endfunction()
...
function(bobui_process_qlalr consuming_target input_file_list flags)
    # Don't try to extend_target when cross compiling an imported host target (like a tool).
    bobui_is_imported_target("${consuming_target}" is_imported)
    if(is_imported)
        return()
    endif()

    bobui_internal_is_skipped_test(skipped ${consuming_target})
    if(skipped)
        return()
    endif()
    bobui_internal_is_in_test_batch(in_batch ${consuming_target})
    if(in_batch)
        _bobui_internal_test_batch_target_name(consuming_target)
    endif()

    foreach(input_file ${input_file_list})
        file(STRINGS ${input_file} input_file_lines)
        bobui_qlalr_find_option_in_list("${input_file_lines}" "^%parser(.+)" "parser")
        bobui_qlalr_find_option_in_list("${input_file_lines}" "^%decl(.+)" "decl")
        bobui_qlalr_find_option_in_list("${input_file_lines}" "^%impl(.+)" "impl")
        ...
        add_custom_command(
            ...
            COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qlalr ${flags} ${relative_input_file}
            DEPENDS ${BOBUI_CMAKE_EXPORT_NAMESPACE}::qlalr
