...
function(_bobui_internal_sbom_set_default_option_value option_name default)
...
function(_bobui_internal_sbom_set_default_option_value_and_error_if_empty option_name default)
    _bobui_internal_sbom_set_default_option_value("${option_name}" "${default}")
    if(NOT arg_${option_name})
...
# To accommodate multiple projects within a bobui repo (like bobuiwebengine), we need to choose separate
# build dirs for each project.
function(_bobui_internal_get_current_project_sbom_relative_dir out_var)
    _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
    _bobui_internal_sbom_get_bobui_repo_project_name_lower_case(real_bobui_repo_project_name_lowercase)

    if(repo_project_name_lowercase STREQUAL real_bobui_repo_project_name_lowercase)
        set(sbom_dir "bobui_sbom")
    else()
        set(sbom_dir "bobui_sbom/${repo_project_name_lowercase}")
    endif()
    set(${out_var} "${sbom_dir}" PARENT_SCOPE)
endfunction()

# Helper that returns the directory where the intermediate sbom files will be generated.
function(_bobui_internal_get_current_project_sbom_dir out_var)
    _bobui_internal_get_current_project_sbom_relative_dir(relative_dir)
...
function(_bobui_internal_get_staging_area_spdx_file_path out_var)
    _bobui_internal_get_current_project_sbom_dir(sbom_dir)
    _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
...
function(_bobui_internal_sbom_begin_project_generate)
    set(opt_args "")
    set(single_args
        OUTPUT
    ...
    _bobui_internal_sbom_get_common_project_variables(
        ${common_project_args}
    ...
    _bobui_internal_sbom_set_default_option_value(NAMESPACE
        "${arg_SUPPLIER}/spdxdocs/${arg_PROJECT}-${BOBUI_SBOM_GIT_VERSION}")
    _bobui_internal_sbom_set_default_option_value(LICENSE "NOASSERTION")

    _bobui_internal_sbom_set_default_option_value(DOCUMENT_CREATOR_TOOL "BobUI Build System")
...
ExternalRef: SECURITY cpe23Type ${arg_CPE}")
    ...
    set(purl_generic_id "pkg:generic/${arg_SUPPLIER}/${arg_PROJECT}@${BOBUI_SBOM_GIT_VERSION}")
    set(fields "${fields}
ExternalRef: PACKAGE-MANAGER purl ${purl_generic_id}")

    if(BOBUI_SBOM_GIT_VERSION)
        set(fields "${fields}
PackageVersion: ${BOBUI_SBOM_GIT_VERSION}")
    endif()

    get_filename_component(doc_name "${arg_OUTPUT}" NAME_WLE)

    _bobui_internal_sbom_set_default_option_value(DOWNLOAD_LOCATION "NOASSERTION")
...
CreatorComment: <text>This SPDX document was created from CMake ${CMAKE_VERSION}, using the bobui
build system from https://code.bobui.io/cgit/bobui/bobuibase.git/tree/cmake/BobUIPublicSbomHelpers.cmake</text>
Created: ${current_utc}\${BOBUI_SBOM_EXTERNAL_DOC_REFS}
...
ExternalRef: PACKAGE-MANAGER purl pkg:generic/${arg_SUPPLIER}/${arg_PROJECT}@${BOBUI_SBOM_GIT_VERSION}
...
")
    ...
    _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
    _bobui_internal_sbom_create_sbom_staging_file(
        CONTENT "${content}"
        SBOM_FORMAT "SPDX_V2"
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_internal_sbom_dirs "${sbom_dir}")

    _bobui_internal_sbom_save_project_info_in_global_properties(
    ...
    _bobui_internal_sbom_save_common_path_variables_in_global_properties(
        OUTPUT "${arg_OUTPUT}"
        OUTPUT_RELATIVE_PATH "${arg_OUTPUT_RELATIVE_PATH}"
        SBOM_DIR "${sbom_dir}"
    )

    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files "${create_staging_file}")

    set_property(GLOBAL PROPERTY _bobui_sbom_spdx_id_count 0)
    set_property(GLOBAL PROPERTY _bobui_sbom_relationship_counter 0)
endfunction()
...
function(_bobui_internal_sbom_end_project_generate)
    _bobui_internal_sbom_get_common_path_variables_from_global_properties(
        SBOM_FORMAT "SPDX_V2"
    ...
    if(NOT sbom_build_output_path)
        message(FATAL_ERROR "Call _bobui_internal_sbom_begin_project() first")
    endif()

    _bobui_internal_sbom_get_root_project_name_lower_case(repo_project_name_lowercase)
    _bobui_internal_sbom_get_bobui_repo_project_name_lower_case(real_bobui_repo_project_name_lowercase)

    _bobui_internal_sbom_get_cmake_include_files(
        SBOM_FORMAT "SPDX_V2"
    ...
    _bobui_internal_sbom_create_build_time_sbom_targets(
        SBOM_FORMAT "SPDX_V2"
        REPO_PROJECT_NAME_LOWERCASE "${repo_project_name_lowercase}"
        REAL_BOBUI_REPO_PROJECT_NAME_LOWERCASE "${real_bobui_repo_project_name_lowercase}"
    ...
    if(arg_LINT_SOURCE_SBOM AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
    ...
    _bobui_internal_sbom_setup_multi_config_install_markers(
        SBOM_DIR "${sbom_dir}"
        SBOM_FORMAT "SPDX_V2"
        REPO_PROJECT_NAME_LOWERCASE "${repo_project_name_lowercase}"
    ...
    _bobui_internal_sbom_setup_fake_checksum(
...
list(REMOVE_DUPLICATES BOBUI_SBOM_PACKAGES_WITH_VERIFICATION_CODES)
...
foreach(_sbom_package IN LISTS BOBUI_SBOM_PACKAGES_WITH_VERIFICATION_CODES)
    set(_codes \${BOBUI_SBOM_PACKAGES_WITH_VERIFICATION_CODES_\${_sbom_package}})
    list(SORT _codes)
    string(REPLACE \";\" \"\" _codes \"\${_codes}\")
    string(SHA1 _verification_code \"\${_codes}\")
    set(BOBUI_SBOM_VERIFICATION_CODE_\${_sbom_package} \"
...
")
    ...
    _bobui_internal_sbom_setup_sbom_install_code(
        SBOM_FORMAT "SPDX_V2"
        REPO_PROJECT_NAME_LOWERCASE "${repo_project_name_lowercase}"
    ...
    _bobui_internal_sbom_clear_cmake_include_files(
        SBOM_FORMAT "SPDX_V2"
    )
endfunction()
...
# output.
function(_bobui_internal_sbom_collect_cmake_include_files out_var)
    set(opt_args
        JOIN_WITH_NEWLINES
    )
    set(single_args "")
...
function(_bobui_internal_sbom_generate_add_file)
    set(opt_args
        OPTIONAL
    )
    set(single_args
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(FILENAME "")
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(FILETYPE "")
    ...
    _bobui_internal_sbom_get_and_check_spdx_id(
        VARIABLE arg_SPDXID
        ${check_option}
        HINTS "SPDXRef-${arg_FILENAME}"
    )

    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(PARENT_PACKAGE_SPDXID "")

    _bobui_internal_sbom_set_default_option_value(LICENSE "NOASSERTION")
    _bobui_internal_sbom_set_default_option_value(COPYRIGHT "NOASSERTION")

    get_property(sbom_project_spdx_id GLOBAL PROPERTY _bobui_sbom_project_spdx_id)
    ...
    if(NOT arg_RELATIONSHIP)
        ...
        string(REPLACE
            "@BOBUI_SBOM_LAST_SPDXID@" "${arg_SPDXID}" arg_RELATIONSHIP "${arg_RELATIONSHIP}")
    endif()
...
FileCopyrightText: NOASSERTION"
    ...
    _bobui_internal_get_staging_area_spdx_file_path(staging_area_spdx_file)
    ...
    set(content "
        if(NOT EXISTS \"\$ENV{DESTDIR}${install_prefix}/${arg_FILENAME}\"
                AND NOT BOBUI_SBOM_BUILD_TIME AND NOT BOBUI_SBOM_FAKE_CHECKSUM)
            if(NOT ${arg_OPTIONAL})
            ...
            if(NOT BOBUI_SBOM_BUILD_TIME)
                if(BOBUI_SBOM_FAKE_CHECKSUM)
                    set(sha1 \"158942a783ee1095eafacaffd93de73edeadbeef\")
                else()
                    file(SHA1 \"\$ENV{DESTDIR}${install_prefix}/${arg_FILENAME}\" sha1)
                endif()

                set(\"BOBUI_SBOM_PACKAGE_HAS_FILES_${arg_PARENT_PACKAGE_SPDXID}\" true)

                list(APPEND BOBUI_SBOM_PACKAGES_WITH_VERIFICATION_CODES
                    \"${arg_PARENT_PACKAGE_SPDXID}\")
                list(APPEND
                    \"BOBUI_SBOM_PACKAGES_WITH_VERIFICATION_CODES_${arg_PARENT_PACKAGE_SPDXID}\"
                    \"\${sha1}\")
            endif()
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files "${file_sbom_to_install}")
endfunction()
...
# searched:
# - BOBUI6_INSTALL_PREFIX
# - BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH
# - BOBUI_ADDITIONAL_SBOM_DOCUMENT_PATHS
#
...
# relationships.
function(_bobui_internal_sbom_generate_add_external_reference)
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(EXTERNAL_DOCUMENT_FILE_PATH "")

    if(NOT arg_EXTERNAL_DOCUMENT_SPDX_ID)
        get_property(spdx_id_count GLOBAL PROPERTY _bobui_sbom_spdx_id_count)
        set(arg_EXTERNAL_DOCUMENT_SPDX_ID "DocumentRef-${spdx_id_count}")
        math(EXPR spdx_id_count "${spdx_id_count} + 1")
        set_property(GLOBAL PROPERTY _bobui_sbom_spdx_id_count "${spdx_id_count}")
    ...
    if(arg_RELATIONSHIP_STRING STREQUAL "")
        ...
        string(REPLACE
            "@BOBUI_SBOM_LAST_SPDXID@" "${arg_EXTERNAL_DOCUMENT_SPDX_ID}"
    ...
    get_cmake_property(build_sbom_dirs _bobui_internal_sbom_dirs)
    if(build_sbom_dirs)
    ...
    if(BOBUI6_INSTALL_PREFIX)
        list(APPEND install_prefixes ${BOBUI6_INSTALL_PREFIX})
    endif()

    if(BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH)
        list(APPEND install_prefixes ${BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH})
    endif()

    if(BOBUI_ADDITIONAL_SBOM_DOCUMENT_PATHS)
        list(APPEND install_prefixes ${BOBUI_ADDITIONAL_SBOM_DOCUMENT_PATHS})
    endif()
...
")
    ...
    set(content "
        ...
        string(APPEND BOBUI_SBOM_EXTERNAL_DOC_REFS \"
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_end_include_files "${ext_ref_sbom}")
endfunction()

# Helper to add info about a package to the sbom. Usually a package is a mapping to a cmake target.
function(_bobui_internal_sbom_generate_add_package)
    set(opt_args
        ""
    )
    set(single_args
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(PACKAGE "")
    ...
    _bobui_internal_sbom_set_default_option_value(DOWNLOAD_LOCATION "NOASSERTION")
    _bobui_internal_sbom_set_default_option_value(VERSION "unknown")
    _bobui_internal_sbom_set_default_option_value(SUPPLIER "Person: Anonymous")
    _bobui_internal_sbom_set_default_option_value(LICENSE_DECLARED "NOASSERTION")
    _bobui_internal_sbom_set_default_option_value(LICENSE_CONCLUDED "NOASSERTION")
    _bobui_internal_sbom_set_default_option_value(COPYRIGHT "NOASSERTION")
    _bobui_internal_sbom_set_default_option_value(PURPOSE "OTHER")
...
ExternalRef: SECURITY cpe23Type ${cpe}"
    ...
    if(NOT arg_RELATIONSHIP)
        ...
        string(REPLACE "@BOBUI_SBOM_LAST_SPDXID@" "${arg_SPDXID}" arg_RELATIONSHIP "${arg_RELATIONSHIP}")
    endif()

    set(fields "${fields}\\\${BOBUI_SBOM_VERIFICATION_CODE_${arg_SPDXID}}")

    _bobui_internal_get_staging_area_spdx_file_path(staging_area_spdx_file)

    # BOBUI_SBOM_PACKAGE_HAS_FILES_ gets overriden by any added file to 'true'.
    set(content "
        list(APPEND BOBUI_SBOM_PACKAGES \"${arg_SPDXID}\")
        set(\"BOBUI_SBOM_PACKAGE_HAS_FILES_${arg_SPDXID}\" false)
...
FilesAnalyzed: \\\${BOBUI_SBOM_PACKAGE_HAS_FILES_${arg_SPDXID}}
Relationship: ${arg_RELATIONSHIP}
\"
        )
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files "${package_sbom}")
endfunction()
...
function(_bobui_internal_sbom_generate_add_project_relationship)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(RELATIONSHIPS "")

    _bobui_internal_get_staging_area_spdx_file_path(staging_area_spdx_file)

    get_property(counter GLOBAL PROPERTY _bobui_sbom_relationship_counter)
    set(current_counter "${counter}")
    math(EXPR counter "${counter} + 1")
    set_property(GLOBAL PROPERTY _bobui_sbom_relationship_counter "${counter}")
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files "${ext_ref_sbom}")
endfunction()
...
# BOBUI_SBOM_VERIFICATION_CODE variable. Only relevant for install time sboms.
#
...
function(_bobui_internal_sbom_add_cmake_include_step)
    set(opt_args "")
    set(single_args
        STEP
        INCLUDE_PATH
    ...
    if(step STREQUAL "BEGIN")
        set(property "_bobui_sbom_cmake_include_files")
    elseif(step STREQUAL "END")
        set(property "_bobui_sbom_cmake_end_include_files")
    elseif(step STREQUAL "BEFORE_CHECKSUM")
        set(property "_bobui_sbom_cmake_before_checksum_include_files")
    elseif(step STREQUAL "AFTER_CHECKSUM")
        set(property "_bobui_sbom_cmake_after_checksum_include_files")
    elseif(step STREQUAL "POST_GENERATION")
        set(property "_bobui_sbom_cmake_post_generation_include_files")
    elseif(step STREQUAL "VERIFY")
        set(property "_bobui_sbom_cmake_verify_include_files")
    else()
...
function(_bobui_internal_sbom_generate_add_license)
    set(opt_args "")
    set(single_args
        LICENSE_ID
        EXTRACTED_TEXT
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(LICENSE_ID "")
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_end_include_files "${license_sbom}")
endfunction()
...
function(_bobui_internal_sbom_get_and_check_spdx_id)
    set(opt_args "")
    set(single_args
        VARIABLE
        CHECK
    ...
    _bobui_internal_sbom_set_default_option_value_and_error_if_empty(VARIABLE "")
    ...
    if(NOT arg_CHECK)
        ...
        foreach(hint IN LISTS arg_HINTS)
            _bobui_internal_sbom_get_sanitized_spdx_id(id "${hint}")
            if(id)
                set(id "${id}${suffix}")
                break()
            endif()
