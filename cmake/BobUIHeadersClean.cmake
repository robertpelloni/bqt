...
# or `bobui-configure-module`.
function(bobui_internal_add_headersclean_target module_target module_headers)
    if(INPUT_headersclean AND WASM)
        message(FATAL_ERROR "The headersclean targets are not supported on WASM platform.")
    endif()

    get_target_property(no_headersclean_check ${module_target} _bobui_no_headersclean_check)
    ...
    set(hcleanDEFS -DBOBUI_NO_CAST_TO_ASCII
                 -DBOBUI_NO_CAST_FROM_ASCII
                 -DBOBUI_NO_URL_CAST_FROM_STRING
                 -DBOBUI_NO_CAST_FROM_BYTEARRAY
                 -DBOBUI_NO_CONTEXTLESS_CONNECT
                 -DBOBUI_NO_KEYWORDS
                 -DBOBUI_TYPESAFE_FLAGS
                 -DBOBUI_USE_QSTRINGBUILDER
                 -DBOBUI_USE_FAST_OPERATOR_PLUS)
    ...
    # bobuitools/src/designer/src/uiplugin/CMakeLists.txt which ends up causing the following error
    # message:
    #  CMake Error at bobuibase/cmake/BobUIModuleHelpers.cmake:35 (add_library):
    #    INTERFACE_LIBRARY targets may only have whitelisted properties.  The
    #    property "BOBUI_PLUGIN_CLASS_NAME" is not allowed.
    #  Call Stack (most recent call first):
    #    src/designer/src/uiplugin/CMakeLists.txt:7 (bobui_internal_add_module)
    ...
    if (BOBUI_FEATURE_cxx2c)
        set(BOBUI_HEADERS_CLEAN_CXX_STANDARD c++2c)
    elseif (BOBUI_FEATURE_cxx2b)
        set(BOBUI_HEADERS_CLEAN_CXX_STANDARD c++2b)
    else()
        set(BOBUI_HEADERS_CLEAN_CXX_STANDARD c++20)
    endif()
    ...
    if ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "GNU"
        ...
        set(hcleanFLAGS -std=${BOBUI_HEADERS_CLEAN_CXX_STANDARD}
            ...
            -Wredundant-decls # BOBUIBUG-115583
            -fno-operator-names)

        if(BOBUI_FEATURE_reduce_relocations AND UNIX)
        ...
        if(APPLE AND BOBUI_FEATURE_framework)
            ...
            list(APPEND framework_includes
                 "-iframework" "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_LIBDIR}")

            # If additional package prefixes are provided, we consider they can contain frameworks
            # as well.
            foreach(prefix IN LISTS _bobui_additional_packages_prefix_paths)
    ...
    elseif ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
        ...
        get_filename_component(source_path "${BOBUI_MKSPECS_DIR}/features/data/dummy.cpp" REALPATH)
    ...
    bobui_internal_module_info(module ${module_target})
    ...
    foreach(header ${hclean_headers})
        ...
        foreach(dir IN LISTS possible_base_dirs)
            _bobui_internal_path_is_prefix(dir "${input_path}" dir_is_prefix)
            if(dir_is_prefix)
        ...
        add_custom_command(
            ...
            COMMAND ${CMAKE_COMMAND}
                ...
                -P "${BOBUI_CMAKE_DIR}/BobUIModuleHeadersCheck.cmake"
