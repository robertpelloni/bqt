...
# non-empty value.
macro(bobui_internal_genex_get_property var target property)
...
# and '${suffix}'.
macro(bobui_internal_genex_get_joined_property var target property prefix suffix glue)
    bobui_internal_genex_get_property("${var}" "${target}" "${property}")
    set(${var}
...
# Arguments:
#    PRIVATE_CONTENT_FILE specifies the pre-cooked content of BobUI_<version>_PRIVATE_API section.
#       Requires the content file available at build time.
function(bobui_internal_add_linker_version_script target)
    if(WASM)
        return()
    endif()
    ...
    if(TEST_ld_version_script)
        ...
        string(APPEND contents "\n};\nBobUI_${PROJECT_VERSION_MAJOR}")
        if(BOBUI_FEATURE_elf_private_full_version)
            string(APPEND contents ".${PROJECT_VERSION_MINOR}.${PROJECT_VERSION_PATCH}")
        endif()
        string(APPEND contents "_PRIVATE_API { bobui_private_api_tag*;\n")
        ...
        string(APPEND contents "};\n")
        set(current "BobUI_${PROJECT_VERSION_MAJOR}")
        ...
        if(NOT target_type STREQUAL "INTERFACE_LIBRARY")
            ...
            # BOBUI_DECLARE_Q{,E}SDP_SPECIALIZATION_DTOR_WITH_EXPORT macros
            ...
            bobui_internal_genex_get_joined_property(
                linker_exports "${target}" _bobui_extra_elf_linker_script_exports
        ...
        if(NOT target_type STREQUAL "INTERFACE_LIBRARY")
            set(property_genex "$<TARGET_PROPERTY:${target},_bobui_extra_elf_linker_script_content>")
        ...
        set(generator_command ${CMAKE_COMMAND}
            ...
            "-DOUT_FILE=${outfile}"
            -P "${BOBUI_CMAKE_DIR}/BobUIGenerateVersionScript.cmake"
        )
        set(generator_dependencies
            "${arg_PRIVATE_CONTENT_FILE}"
            "${BOBUI_CMAKE_DIR}/BobUIGenerateVersionScript.cmake"
        )
...
function(bobui_internal_add_link_flags_no_undefined target)
    if (NOT BOBUI_BUILD_SHARED_LIBS OR WASM)
        return()
    endif()
    if (VXWORKS)
    ...
    if ((GCC OR CLANG) AND NOT (MSVC OR CMAKE_CXX_SIMULATE_ID STREQUAL "MSVC"))
        if((GCC OR CLANG) AND BOBUI_FEATURE_sanitizer)
            return()
        endif()
        set(previous_CMAKE_REQUIRED_LINK_OPTIONS ${CMAKE_REQUIRED_LINK_OPTIONS})

        set(CMAKE_REQUIRED_LINK_OPTIONS "-Wl,-undefined,error")
        _bobui_internal_get_check_cxx_source_compiles_out_var(test_output_undefined_error extra_args)
        ...
        _bobui_internal_get_check_cxx_source_compiles_out_var(test_output_no_undefined extra_args)
...
function(bobui_internal_apply_gc_binaries_conditional target visibility)
    # Should only be applied when the feature is enabled, aka for static builds.
    if(NOT BOBUI_FEATURE_gc_binaries)
        return()
    endif()
    bobui_internal_apply_gc_binaries("${target}" "${visibility}")
endfunction()

function(bobui_internal_apply_gc_binaries target visibility)
    ...
    if ((GCC OR CLANG) AND NOT WASM AND NOT UIKIT
        ...
        set_property(GLOBAL PROPERTY _bobui_internal_gc_sections_without_genex "${gc_sections_flag}")

        set(gc_sections_flag
            "${clang_or_gcc_begin}${gc_sections_flag}${clang_or_gcc_end}")

        set_property(GLOBAL PROPERTY _bobui_internal_gc_sections_with_genex "${gc_sections_flag}")
    endif()
    if(gc_sections_flag)
...
function(bobui_internal_apply_intel_cet target visibility)
    if(NOT BOBUI_FEATURE_intelcet)
        return()
    endif()
    ...
    if(flags)
        set(opt_out_condition "$<NOT:$<BOOL:$<TARGET_PROPERTY:_bobui_no_intel_cet_harderning>>>")
...
function(bobui_internal_apply_intel_cet_harderning target)
    if(NOT BOBUI_FEATURE_intelcet)
        return()
    endif()
    ...
    if(MSVC)
        ...
        bobui_internal_platform_link_options("${target}" INTERFACE "${condition}")
    else()
...
function(bobui_internal_skip_intel_cet_hardening target)
    set_target_properties("${target}" PROPERTIES _bobui_no_intel_cet_harderning TRUE)
endfunction()
...
function(bobui_internal_set_exceptions_flags target value)
    #INTERFACE libraries compile nothing
    get_target_property(target_type ${target} TYPE)
    if(target_type STREQUAL "INTERFACE_LIBRARY")
        return()
    endif()
    set_target_properties(${target} PROPERTIES _bobui_internal_use_exceptions ${value})
endfunction()

# Deprecated. Replaced by bobui_internal_set_skip_warnings_are_errors.
function(bobui_skip_warnings_are_errors target)
    bobui_internal_set_skip_warnings_are_errors(${target} TRUE)
endfunction()

# Controls the BOBUI_SKIP_WARNINGS_ARE_ERRORS property for the given target.
function(bobui_internal_set_skip_warnings_are_errors target value)
    _bobui_internal_set_skip_warnings_are_errors("${target}" "${value}")
endfunction()

# Sets the default warnings behavior according to the WARNINGS_ARE_ERRORS and
# BOBUI_REPO_NOT_WARNINGS_CLEAN flags.
function(bobui_internal_default_warnings_are_errors target)
    if(WARNINGS_ARE_ERRORS AND NOT BOBUI_REPO_NOT_WARNINGS_CLEAN
       ...
       AND NOT "${CMAKE_GENERATOR}" STREQUAL "Xcode")
        bobui_internal_set_skip_warnings_are_errors("${target}" FALSE)
    else()
        bobui_internal_set_skip_warnings_are_errors("${target}" TRUE)
    endif()
endfunction()

function(bobui_disable_warnings target)
    get_target_property(target_type "${target}" TYPE)
    if(target_type STREQUAL "INTERFACE_LIBRARY")
        return()
    endif()
    set_target_properties("${target}" PROPERTIES BOBUI_COMPILE_OPTIONS_DISABLE_WARNINGS ON)
endfunction()

function(bobui_set_symbol_visibility_preset target value)
    get_target_property(target_type "${target}" TYPE)
    if(target_type STREQUAL "INTERFACE_LIBRARY")
        return()
    endif()
...
function(bobui_set_symbol_visibility_hidden target)
    bobui_set_symbol_visibility_preset("${target}" "hidden")
endfunction()

function(bobui_set_language_standards)
    ## Use the latest standard the compiler supports (same as bobui_common.prf)
    if (BOBUI_FEATURE_cxx2c)
        set(CMAKE_CXX_STANDARD 26 PARENT_SCOPE)
    elseif (BOBUI_FEATURE_cxx2b)
        set(CMAKE_CXX_STANDARD 23 PARENT_SCOPE)
    elseif (BOBUI_FEATURE_cxx20)
...
function(bobui_set_language_standards_interface_compile_features target)
    # Regardless of which C++ standard is used to build BobUI itself, require C++17 when building
    # BobUI applications using CMake (because the BobUI header files use C++17 features).
    set(cpp_feature "cxx_std_17")
    target_compile_features("${target}" INTERFACE ${cpp_feature})
endfunction()

function(bobui_set_msvc_cplusplus_options target visibility)
    # For MSVC we need to explicitly pass -Zc:__cplusplus to get correct __cplusplus.
    # Check bobui_config_compile_test for more info.
...
function(bobui_enable_utf8_sources target)
    set(utf8_flags "")
    if(MSVC)
    ...
    if(utf8_flags)
        # Allow opting out by specifying the BOBUI_NO_UTF8_SOURCE target property.
        set(opt_out_condition "$<NOT:$<BOOL:$<TARGET_PROPERTY:BOBUI_NO_UTF8_SOURCE>>>")
...
function(bobui_internal_enable_unicode_defines)
    if(WIN32)
        set(no_unicode_condition
            "$<NOT:$<BOOL:$<TARGET_PROPERTY:BOBUI_NO_UNICODE_DEFINES>>>")
...
function(bobui_internal_get_all_possible_optimization_flag_values out_var)
    set(flag_values "")
    set(vars BOBUI_CFLAGS_OPTIMIZE BOBUI_CFLAGS_OPTIMIZE_FULL
             BOBUI_CFLAGS_OPTIMIZE_DEBUG BOBUI_CFLAGS_OPTIMIZE_SIZE)
    ...
    if(BOBUI_CFLAGS_OPTIMIZE_VALID_VALUES)
        list(APPEND flag_values ${BOBUI_CFLAGS_OPTIMIZE_VALID_VALUES})
    endif()
...
function(bobui_internal_get_optimize_full_flags out_var)
    set(optimize_full_flags "${BOBUI_CFLAGS_OPTIMIZE_FULL}")
    if(NOT optimize_full_flags)
        set(optimize_full_flags "${BOBUI_CFLAGS_OPTIMIZE}")
    endif()
...
function(bobui_internal_print_optimization_flags_values)
    bobui_internal_get_enabled_languages_for_flag_manipulation(languages)
    bobui_internal_get_configs_for_flag_manipulation(configs)
    bobui_internal_get_target_link_types_for_flag_manipulation(target_link_types)

    bobui_internal_print_optimization_flags_values_helper(
        "${languages}" "${configs}" "${target_link_types}")
endfunction()

# Helper function for printing the optimization flags.
function(bobui_internal_print_optimization_flags_values_helper languages configs target_link_types)
...
function(bobui_internal_get_configs_for_flag_manipulation out_var)
    set(configs RELEASE RELWITHDEBINFO MINSIZEREL DEBUG)

    # Opt into additional non-standard configs for flag removal only.
    if(BOBUI_ADDITIONAL_OPTIMIZATION_FLAG_CONFIGS)
        list(APPEND configs ${BOBUI_ADDITIONAL_OPTIMIZATION_FLAG_CONFIGS})
    endif()
...
function(bobui_internal_get_target_link_types_for_flag_manipulation out_var)
...
function(bobui_internal_get_enabled_languages_for_flag_manipulation out_var)
...
function(bobui_internal_replace_flags_impl flag_var_name match_string replace_string IN_CACHE)
...
# Helper function used to update compiler and linker flags further below
function(bobui_internal_remove_flags_impl flag_var_name flag_values IN_CACHE)
...
# Helper function used to update compiler and linker flags further below
function(bobui_internal_add_flags_impl flag_var_name flags IN_CACHE)
    # This must come before cache variable modification because setting the
    # cache variable with FORCE will overwrite the non-cache variable, but
    # we need to use the original value on entry to this function.
...
function(bobui_internal_remove_known_optimization_flags)
    ...
    if(arg_LANGUAGES)
        ...
        bobui_internal_get_enabled_languages_for_flag_manipulation(enabled_languages)
    endif()

    bobui_internal_get_all_possible_optimization_flag_values(flag_values)
    set(configs ${arg_CONFIGS})

    foreach(lang ${enabled_languages})
        foreach(config ${configs})
            set(flag_var_name "CMAKE_${lang}_FLAGS_${config}")
            bobui_internal_remove_flags_impl(${flag_var_name} "${flag_values}" "${arg_IN_CACHE}")
            set(${flag_var_name} "${${flag_var_name}}" PARENT_SCOPE)
        endforeach()
    endforeach()
endfunction()
...
#       specified.
function(bobui_internal_remove_compiler_flags flags)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        "IN_CACHE;REGEX"
        ""
        "CONFIGS;LANGUAGES"
    ...
    if("${flags}" STREQUAL "")
        message(WARNING "bobui_internal_remove_compiler_flags was called without any flags specified.")
        return()
    endif()
    ...
    foreach(lang ${languages})
        set(flag_var_name "CMAKE_${lang}_FLAGS")
        bobui_internal_remove_flags_impl(${flag_var_name}
            "${flags}"
            "${arg_IN_CACHE}"
            ${extra_options}
        )
...
# Synopsis
#
#   bobui_internal_add_compiler_dependent_flags(<target> <INTERFACE|PUBLIC|PRIVATE>
#       COMPILERS <compiler> ...
...
#
# Example
#   bobui_internal_add_compiler_dependent_flags(tgt PUBLIC
#       COMPILERS ALL
#               OPTIONS -Werror
...
function(bobui_internal_add_compiler_dependent_flags target target_scope)
    ...
    function(_bobui_internal_add_compiler_dependent_flags_error msg)
        # If you are hitting such a function, something must have gone wrong with
        # `bobui_internal_add_compiler_flags` implementation.
        message(FATAL_ERROR
            "${msg}\n"
            "  curr_arg = ${curr_arg}\n"
    ...
    # COMPILERS
    function(_bobui_internal_add_compiler_dependent_flags_do_COMPILERS)
        set(compiler_ex "")
        if(NOT curr_COMPILERS)
            _bobui_internal_add_compiler_dependent_flags_error(
    ...
    # CONDITIONS
    function(_bobui_internal_add_compiler_dependent_flags_do_CONDITIONS)
        function(_bobui_internal_add_compiler_dependent_flags_do_CONDITIONS_end_stack
            stack_in stack_out
        )
        ...
        while(curr_CONDITIONS)
            ...
            elseif(condition_kw MATCHES "(.*)\\)$")
                ...
                _bobui_internal_add_compiler_dependent_flags_do_CONDITIONS_end_stack(
        ...
        _bobui_internal_add_compiler_dependent_flags_do_CONDITIONS_end_stack(stack_0 conditions_ex)
        set(conditions_ex "${conditions_ex}" PARENT_SCOPE)
    endfunction()
    # OPTIONS
    function(_bobui_internal_add_compiler_dependent_flags_do_OPTIONS)
    ...
    while(arg_UNPARSED_ARGUMENTS)
        ...
        if(curr_arg MATCHES "^(COMPILERS|CONDITIONS|OPTIONS)$")
            ...
            if(curr_keyword STREQUAL "COMPILERS")
                _bobui_internal_add_compiler_dependent_flags_do_COMPILERS()
            elseif(curr_keyword STREQUAL "CONDITIONS")
                _bobui_internal_add_compiler_dependent_flags_do_CONDITIONS()
            elseif(curr_keyword STREQUAL "OPTIONS")
                _bobui_internal_add_compiler_dependent_flags_do_OPTIONS()
            endif()
...
#             if not provided, defaults to the list of enabled C-like languages
function(bobui_internal_add_compiler_flags)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "IN_CACHE"
        "FLAGS"
        "CONFIGS;LANGUAGES")
    ...
    foreach(lang ${enabled_languages})
        foreach(config ${configs})
            set(flag_var_name "CMAKE_${lang}_FLAGS_${config}")
            bobui_internal_add_flags_impl(${flag_var_name} "${arg_FLAGS}" "${arg_IN_CACHE}")
            set(${flag_var_name} "${${flag_var_name}}" PARENT_SCOPE)
        endforeach()
    endforeach()
endfunction()
...
# LANGUAGES - optional list of languages like 'C', 'CXX', for which to add the flags
#             if not provided, defaults to the list of enabled C-like languages
function(bobui_internal_add_compiler_flags_for_release_configs)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "IN_CACHE"
        "FLAGS"
        "LANGUAGES")
    ...
    bobui_internal_add_compiler_flags(${args})
...
# BobUI module, like Core or Gui.
function(bobui_internal_add_optimize_full_flags)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "IN_CACHE"
        ""
        "")
    ...
    # BOBUI_USE_DEFAULT_CMAKE_OPTIMIZATION_FLAGS disables forced full optimization.
    if(BOBUI_USE_DEFAULT_CMAKE_OPTIMIZATION_FLAGS)
        return()
    endif()
    ...
    if(BOBUI_FEATURE_optimize_size AND NOT BOBUI_FEATURE_optimize_full)
        return()
    endif()
    ...
    if(BOBUI_FEATURE_optimize_full) # Assume that FEATURE_optimize_full has higher priority.
        list(APPEND configs MINSIZEREL)
    endif()

    bobui_internal_remove_known_optimization_flags(${args} CONFIGS ${configs})

    # If the respective compiler doesn't have optimize_full flags, use regular optimization flags.
    # Mainly MSVC.
    bobui_internal_get_optimize_full_flags(optimize_full_flags)
    list(APPEND args FLAGS "${optimize_full_flags}")

    bobui_internal_add_compiler_flags_for_release_configs(${args})
...
#                  if not provided, defaults to the list of enabled C-like languages
function(bobui_internal_replace_compiler_flags match_string replace_string)
    cmake_parse_arguments(PARSE_ARGV 2 arg
        "IN_CACHE"
        ""
        "CONFIGS;LANGUAGES")
    ...
    foreach(lang ${enabled_languages})
        foreach(config ${configs})
            set(flag_var_name "CMAKE_${lang}_FLAGS_${config}")
            bobui_internal_replace_flags_impl(${flag_var_name}
...
function(bobui_internal_add_linker_flags)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "IN_CACHE"
        "FLAGS"
        "CONFIGS;TYPES")
...
#                  CMAKE_<LINKER_TYPE>_LINKER_FLAGS_<CONFIG> cache variable.
#                  e.g EXE, MODULE, SHARED, STATIC.
function(bobui_internal_replace_linker_flags match_string replace_string)
    cmake_parse_arguments(PARSE_ARGV 2 arg
        "IN_CACHE"
        ""
        "CONFIGS;TYPES")
...
# files and replaces them with flags that BobUI qmake builds expect, for all the usual
...
# See BOBUIBUG-85992 for details.
#
# Note that both the calling scope and the CMake cache are updated.
function(bobui_internal_set_up_config_optimizations_like_in_qmake)
    # Allow opt out.
    if(BOBUI_USE_DEFAULT_CMAKE_OPTIMIZATION_FLAGS)
        return()
    endif()
    ...
    # You can set BOBUI_DEBUG_OPTIMIZATION_FLAGS to see the before and after results.
    if(BOBUI_DEBUG_OPTIMIZATION_FLAGS)
        message(STATUS "")
        message(STATUS "DEBUG: Original CMake optimization flags.\n")
        bobui_internal_print_optimization_flags_values_helper("${enabled_languages}" "${configs}"
                                                           "${target_link_types}")
    endif()

    # Remove known optimization flags.
    bobui_internal_remove_known_optimization_flags(IN_CACHE CONFIGS ${configs})
    ...
    foreach(lang ${enabled_languages})
        foreach(config ${configs})
            ...
            if(config STREQUAL "RELEASE" AND BOBUI_CFLAGS_OPTIMIZE)
                set(value_to_append "${BOBUI_CFLAGS_OPTIMIZE}")
            elseif(config STREQUAL "RELWITHDEBINFO" AND BOBUI_CFLAGS_OPTIMIZE)
                set(value_to_append "${BOBUI_CFLAGS_OPTIMIZE}")

            # MinSizeRel should get the optimize size flag if available, otherwise the regular
            # release flag.
            elseif(config STREQUAL "MINSIZEREL")
                if(BOBUI_CFLAGS_OPTIMIZE_SIZE)
                    set(value_to_append "${BOBUI_CFLAGS_OPTIMIZE_SIZE}")
                else()
                    set(value_to_append "${BOBUI_CFLAGS_OPTIMIZE}")
                endif()
            endif()
            ...
            if(config STREQUAL "DEBUG" AND BOBUI_FEATURE_optimize_debug)
                set(value_to_append "${BOBUI_CFLAGS_OPTIMIZE_DEBUG}")
            endif()

            set(configs_for_optimize_size RELEASE RELWITHDEBINFO)
            if(BOBUI_FEATURE_optimize_size AND config IN_LIST configs_for_optimize_size)
                set(value_to_append "${BOBUI_CFLAGS_OPTIMIZE_SIZE}")
            endif()
            ...
            if(BOBUI_FEATURE_optimize_full AND config IN_LIST configs_for_optimize)
                bobui_internal_get_optimize_full_flags(optimize_full_flags)
            ...
            if(value_to_append)
                bobui_internal_add_flags_impl(${flag_var_name} "${value_to_append}" TRUE)
    ...
    if(MSVC)
        ...
        foreach(flag_value ${flag_values})
            bobui_internal_replace_linker_flags(
                    "${flag_value}" ""
                    CONFIGS ${configs}
        ...
        bobui_internal_add_linker_flags(
                FLAGS "${flag_value}"
                ...
                IN_CACHE)
        bobui_internal_remove_compiler_flags("(^| )/EH[scra-]*( |$)" LANGUAGES CXX CONFIGS ${configs} IN_CACHE REGEX)
    endif()
    ...
    # See BOBUIQAINFRA-3934 for details.
    if(MSVC AND BOBUI_FEATURE_msvc_obj_debug_info)
        bobui_internal_replace_compiler_flags(
                "/Zi" "/Z7"
    ...
    if(ANDROID AND ANDROID_COMPILER_FLAGS MATCHES "(^| )-g")
        bobui_internal_remove_compiler_flags("-g")
        bobui_internal_add_compiler_flags(FLAGS "-g" CONFIGS DEBUG RELWITHDEBINFO)
    endif()
...
# are turned on.
function(bobui_internal_add_optimized_flags_for_debug_config_in_current_scope)
    bobui_internal_remove_known_optimization_flags(CONFIGS DEBUG)
    ...
    if(MSVC)
        ...
        foreach(lang ${enabled_languages})
            ...
            bobui_internal_remove_flags_impl(${flag_var_name} "${default_debug_flags_to_remove}" "")
        endforeach()
    endif()

    bobui_internal_get_optimize_full_flags(optimize_full_flags)
    bobui_internal_add_compiler_flags(FLAGS "${optimize_full_flags}" CONFIGS DEBUG)

    # Set the flags in the parent scope. This will apply the flags to all targets within
    # that directory scope.
    bobui_internal_set_optimized_flags_for_debug_config_in_parent_scope()
endfunction()

# Conditionally adds optimized flags to the calling directory scope, depending on whether
# the BOBUI_FEATURE_optimized_tools is ON, and whether a target or project specific override was not
# set.
# Overrides can be set via:
# -DBOBUI_FORCE_NO_OPTIMIZE_<target>=ON
# -DBOBUI_FORCE_NO_OPTIMIZE_<project_name>=ON
# e.g -DBOBUI_FORCE_NO_OPTIMIZE_moc=ON -DBOBUI_FORCE_NO_OPTIMIZE_bobuideclarative=ON
function(bobui_internal_add_target_optimized_flags_for_debug_config_in_current_scope target)
    set(opt_args "")
    set(single_args
        OUT_VAR_DID_ADD
    )
    ...
    if(BOBUI_FEATURE_optimized_tools
        ...
        bobui_internal_add_optimized_flags_for_debug_config_in_current_scope()
        bobui_internal_set_optimized_flags_for_debug_config_in_parent_scope()
        set(did_add TRUE)
    else()
        set(did_add FALSE)
...
macro(bobui_internal_set_optimized_flags_for_debug_config_in_parent_scope)
    bobui_internal_get_enabled_languages_for_flag_manipulation(__bobui_internal_debug_enabled_languages)
    foreach(__bobui_internal_debug_lang IN LISTS __bobui_internal_debug_enabled_languages)
        set(flag_var_name "CMAKE_${__bobui_internal_debug_lang}_FLAGS_DEBUG")
        set(${flag_var_name} "${${flag_var_name}}" PARENT_SCOPE)
    endforeach()
    unset(__bobui_internal_debug_lang)
    unset(__bobui_internal_debug_enabled_languages)
endmacro()

# Same as bobui_internal_add_target_optimized_flags_for_debug_config_in_current_scope, but also
# propagates the optimized flags to the parent scope.
macro(bobui_internal_add_target_optimized_flags_for_debug_config_in_parent_scope target)
    bobui_internal_add_target_optimized_flags_for_debug_config_in_current_scope("${target}"
        OUT_VAR_DID_ADD __bobui_internal_did_add_optimized_flag_in_current_scope)
    if(__bobui_internal_did_add_optimized_flag_in_current_scope)
        bobui_internal_set_optimized_flags_for_debug_config_in_parent_scope()
    endif()
    unset(__bobui_internal_did_add_optimized_flag_in_current_scope)
endmacro()
