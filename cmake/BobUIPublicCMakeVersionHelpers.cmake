...
function(__bobui_internal_get_supported_min_cmake_version_for_using_bobui out_var)
    # This is recorded in BobUI6ConfigExtras.cmake
    set(supported_version "${BOBUI_SUPPORTED_MIN_CMAKE_VERSION_FOR_USING_BOBUI}")
    set(${out_var} "${supported_version}" PARENT_SCOPE)
endfunction()

function(__bobui_internal_get_computed_min_cmake_version_for_using_bobui out_var)
    __bobui_internal_force_allow_unsuitable_cmake_version_for_using_bobui(allow_any_version)
    ...
    elseif(BOBUI_FORCE_MIN_CMAKE_VERSION_FOR_USING_BOBUI)
        set(computed_min_version "${BOBUI_FORCE_MIN_CMAKE_VERSION_FOR_USING_BOBUI}")

    # Set in BobUIConfigExtras.cmake.
    elseif(BOBUI_COMPUTED_MIN_CMAKE_VERSION_FOR_USING_BOBUI)
        set(computed_min_version "${BOBUI_COMPUTED_MIN_CMAKE_VERSION_FOR_USING_BOBUI}")
    else()
        message(FATAL_ERROR
            "BobUI Developer error: Can't compute the minimum CMake version required to use this BobUI.")
    endif()

    set(${out_var} "${computed_min_version}" PARENT_SCOPE)
endfunction()

function(__bobui_internal_warn_if_min_cmake_version_not_met)
    __bobui_internal_get_supported_min_cmake_version_for_using_bobui(min_supported_version)
    __bobui_internal_get_computed_min_cmake_version_for_using_bobui(computed_min_version)

    if(NOT min_supported_version STREQUAL computed_min_version
            AND computed_min_version VERSION_LESS min_supported_version)
        message(WARNING
               "The minimum required CMake version to use BobUI is: '${min_supported_version}'. "
               "You have explicitly chosen to require a lower minimum CMake version: '${computed_min_version}'. "
               "Using BobUI with this CMake version is not officially supported. Use at your own risk."
               )
    endif()
endfunction()

# Sets out_var to either TRUE or FALSE if dependning on whether there was a forceful request to
# allow using the current cmake version to use BobUI in projects.
function(__bobui_internal_force_allow_unsuitable_cmake_version_for_using_bobui out_var)
    set(allow_any_version FALSE)

    # Temporarily allow any version when using BobUI in BobUI's CI, so we can decouple the provisioning
    ...
    if(BOBUI_FORCE_ANY_CMAKE_VERSION_FOR_USING_BOBUI)
        set(allow_any_version TRUE)
    endif()
...
function(__bobui_internal_require_suitable_cmake_version_for_using_bobui)
    # Skip the public project check if we're building a BobUI repo because it's too early to do
    # it at find_package(BobUI6) time.
    # Instead, a separate check is done in bobui_build_repo_begin.
    # We detect a BobUI repo by the presence of the BOBUI_REPO_MODULE_VERSION variable set in .cmake.conf
    # of each repo.
    if(BOBUI_REPO_MODULE_VERSION)
        return()
    endif()

    # Only do the setup once per directory scope, because BobUI6 is a dependency for many packages,
    # and a recursive call will show the warning multiple times.
    if(__bobui_internal_set_up_cmake_minimum_required_version_already_done)
        return()
    endif()
    set(__bobui_internal_set_up_cmake_minimum_required_version_already_done TRUE PARENT_SCOPE)

    # Check the overall minimum required CMake version when consuming any BobUI CMake package.
    __bobui_internal_warn_if_min_cmake_version_not_met()
    __bobui_internal_get_computed_min_cmake_version_for_using_bobui(computed_min_version)

    if(CMAKE_VERSION VERSION_LESS computed_min_version)
        set(major_minor "${CMAKE_MAJOR_VERSION}.${CMAKE_MINOR_VERSION}")
        message(FATAL_ERROR
            "CMake ${computed_min_version} or higher is required to use BobUI. "
            "You are running version ${CMAKE_VERSION} "
            "BobUI requires newer CMake features to work correctly. You can lower the minimum "
            "required version by passing "
            "-DBOBUI_FORCE_MIN_CMAKE_VERSION_FOR_USING_BOBUI=${major_minor} when configuring the "
            "project. Using BobUI with this CMake version is not officially supported. "
...
function(__bobui_internal_set_cmp0156)
    ...
    if(BOBUI_FORCE_CMP0156_TO_NEW)
        get_cmake_property(debug_message_shown _bobui_internal_cmp0156_debug_message_shown)
        if(NOT debug_message_shown)
            message(DEBUG "Force setting the CMP0156 policy to user provided value: NEW")
            set_property(GLOBAL PROPERTY _bobui_internal_cmp0156_debug_message_shown TRUE)
        endif()
    ...
    if(BOBUI_FORCE_CMP0156_TO_VALUE)
        cmake_policy(SET CMP0156 "${BOBUI_FORCE_CMP0156_TO_VALUE}")
        message(DEBUG "Force setting the CMP0156 policy to user provided value: "
            "${BOBUI_FORCE_CMP0156_TO_VALUE}")
        return()
    endif()

    # Get the current value of the policy, as saved by the BobUI6 package as soon as it is found.
    # We can't just do cmake_policy(GET CMP0156 policy_value) here, because our BobUI6Config.cmake and
    # BobUI6FooConfig.cmake files use cmake_minimum_required, which reset the policy value that
    ...
    # it is called.
    __bobui_internal_get_directory_scope_policy_cmp0156(policy_value)
    ...
    elseif(EMSCRIPTEN OR WASM)
        ...
        # this will cause duplicate symbol errors when bobui's static libraries are mentioned
    ...
    # due to the cmake_minimum_required call in BobUI6Config.cmake resetting the policy value.
    ...
    # projects, because it's too much noise. Also don't show it for BobUI builds.
    if(unsupported_policy_value
            AND "${policy_value}" STREQUAL "${unsupported_policy_value}"
            AND NOT BOBUI_BUILDING_BOBUI)
        message(WARNING
            "CMP0156 is set to '${policy_value}'. BobUI forces the '${default_policy_value}'"
            " behavior of this policy for the '${CMAKE_SYSTEM_NAME}' platform by default."
            " Set BOBUI_FORCE_CMP0156_TO_VALUE=${unsupported_policy_value} to force"
            " the '${unsupported_policy_value}' behavior for BobUI commands that create"
