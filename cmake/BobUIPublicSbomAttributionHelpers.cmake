...
function(_bobui_internal_sbom_handle_bobui_attribution_files out_prefix_outer)
    ...
    foreach(attribution_file_dir_path IN LISTS arg_ATTRIBUTION_FILE_DIR_PATHS)
        get_filename_component(real_path
            "${attribution_file_dir_path}/bobui_attribution.json" REALPATH)
        list(APPEND attribution_files "${real_path}")
        math(EXPR attribution_file_count "${attribution_file_count} + 1")
    endforeach()
    ...
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_ATTRIBUTION_FILES
            AND arg_ATTRIBUTION_PARENT_TARGET_SBOM_ENTITY_TYPE)
        _bobui_internal_sbom_is_bobui_entity_type("${arg_ATTRIBUTION_PARENT_TARGET_SBOM_ENTITY_TYPE}"
            parent_is_bobui_entity_type)
        if(parent_is_bobui_entity_type)
    ...
    foreach(attribution_file_path IN LISTS attribution_files)
        ...
        set_property(GLOBAL APPEND PROPERTY _bobui_internal_project_attribution_files
        ...
        _bobui_internal_sbom_read_bobui_attribution(${out_prefix_file}
        ...
        while("${entry_index}" LESS "${${out_prefix_file}_attribution_entry_count}")
            ...
            if(NOT first_attribution_processed AND NOT arg_CREATE_SBOM_FOR_EACH_ATTRIBUTION)
                ...
                _bobui_internal_sbom_read_bobui_attribution(${out_prefix}
                    GET_DEFAULT_KEYS
                ...
                # Also propagate the internal bobui entity type values like CPE, supplier, PURL
                ...
                # parent one.
                if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_ATTRIBUTION_FILES)
                    set(attribution_entity_type BOBUI_THIRD_PARTY_SOURCES)
                else()
                    set(attribution_entity_type THIRD_PARTY_SOURCES)
                endif()

                _bobui_internal_add_sbom("${attribution_target}"
                ...
                _bobui_internal_extend_sbom_dependencies(${arg_ATTRIBUTION_PARENT_TARGET}
...
# Helper to parse a bobui_attribution.json file and do various operations:
...
function(_bobui_internal_sbom_read_bobui_attribution out_prefix)
    if(NOT BOBUI_GENERATE_SBOM)
        return()
    endif()
    ...
    if(NOT file_path)
        message(FATAL_ERROR "bobui attribution file path not given")
    endif()

    file(READ "${file_path}" contents)
    if(NOT contents)
        message(FATAL_ERROR "bobui attribution file is empty: ${file_path}")
    endif()

    if(NOT arg_GET_DEFAULT_KEYS AND NOT arg_GET_KEY AND NOT arg_GET_ATTRIBUTION_ENTRY_COUNT)
        message(FATAL_ERROR
            "No valid operation specified to _bobui_internal_sbom_read_bobui_attribution call.")
    endif()
    ...
    if(arg_GET_KEY)
        _bobui_internal_sbom_get_attribution_key(${arg_KEY} ${arg_OUT_VAR_VALUE} ${out_prefix})
    endif()
    ...
    if(arg_GET_DEFAULT_KEYS)
        _bobui_internal_sbom_get_attribution_key(Id attribution_id "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(LicenseId license_id "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(License license "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(LicenseFile license_file "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(Version version "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(Homepage homepage "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(Name attribution_name "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(Description description "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(BobUIUsage bobui_usage "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(DownloadLocation download_location "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(Copyright copyrights "${out_prefix}" IS_MULTI_VALUE)
        _bobui_internal_sbom_get_attribution_key(CopyrightFile copyright_file "${out_prefix}")
        _bobui_internal_sbom_get_attribution_key(PURL purls "${out_prefix}" IS_MULTI_VALUE)
        _bobui_internal_sbom_get_attribution_key(CPE cpes "${out_prefix}" IS_MULTI_VALUE)
...
function(_bobui_internal_sbom_handle_attribution_json_array contents)
    set(opt_args "")
    set(single_args
        EXTRACTED_VALUE
        OUT_VAR
...
function(_bobui_internal_sbom_escape_json_content content out_var)
    # Escape backslashes
...
# This macro reads a json key from a bobui_attribution.json file, and assigns the escaped value to
# out_var.
...
macro(_bobui_internal_sbom_get_attribution_key json_key out_var out_prefix)
    ...
    if(NOT "${${out_var}}" STREQUAL "" AND NOT get_error)
        ...
        if(arg_IS_MULTI_VALUE)
            _bobui_internal_sbom_handle_attribution_json_array("${contents}"
        ...
        _bobui_internal_sbom_escape_json_content("${extracted_value}" escaped_content)
...
# Replaces placeholders in CPE and PURL strings read from bobui_attribution.json files.
#
...
function(_bobui_internal_sbom_replace_qa_placeholders)
    set(opt_args "")
    set(single_args
        OUT_VAR
        VERSION
