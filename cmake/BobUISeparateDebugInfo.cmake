...
function(bobui_internal_try_compile_binary_for_strip binary_out_var)
    # Need to find the config.tests files depending whether the bobuibase sources are available.
    ...
    set(bobuibase_config_test_dir "${BOBUI_SOURCE_TREE}/${config_test_dir}")
    set(installed_config_test_dir
        "${_bobui_cmake_dir}/${BOBUI_CMAKE_EXPORT_NAMESPACE}/${config_test_dir}")

    # bobuibase sources available, always use them, regardless of prefix or non-prefix builds.
    if(EXISTS "${bobuibase_config_test_dir}")
        set(src_dir "${bobuibase_config_test_dir}")
    ...
    # happens in BobUI's CI) by choosing a build dir that does not coincide with the installed
    ...
    try_compile(BOBUI_INTERNAL_BUILT_BINARY_FOR_STRIP
        "${binary_dir}"
        "${src_dir}"
...
function(bobui_internal_defer_separate_debug_info target)
    set(opt_args "")
    set(single_args "")
    set(multi_args
    ...
    set_property(TARGET "${target}" PROPERTY _bobui_finalize_separate_debug_info_enabled TRUE)
    set_property(TARGET "${target}" APPEND PROPERTY _bobui_finalize_separate_debug_info_args
        ${arg_SEPARATE_DEBUG_INFO_ARGS}
    )
endfunction()

# Handler to run the separate debug info helper using finalizer args.
function(bobui_internal_finalize_executable_separate_debug_info target)
    get_target_property(enabled "${target}" _bobui_finalize_separate_debug_info_enabled)
    if(NOT enabled)
        return()
    endif()

    get_target_property(args "${target}" _bobui_finalize_separate_debug_info_args)
    if(NOT args)
        set(args "")
    endif()

    bobui_enable_separate_debug_info("${target}" ${args})
endfunction()

# When using the MinGW 11.2.0 toolchain, cmake --install --strip as used by
# bobui-cmake-private-install.cmake, removes the .gnu_debuglink section in binaries and thus
# breaks the separate debug info feature.
#
# Generate a wrapper shell script that passes an option to keep the debug section.
# The wrapper is used when targeting Linux or MinGW with a shared BobUI build.
...
function(bobui_internal_generate_binary_strip_wrapper)
    # Return early if check was done already, if explicitly skipped, or when building a static BobUI.
    if(DEFINED CACHE{BOBUI_INTERNAL_STRIP_SUPPORTS_KEEP_SECTION}
            OR BOBUI_NO_STRIP_WRAPPER
            OR (NOT BOBUI_BUILD_SHARED_LIBS)
        )
        return()
    endif()
    ...
    if(NOT BOBUI_INTERNAL_ORIGINAL_CMAKE_STRIP AND CMAKE_STRIP)
        set(BOBUI_INTERNAL_ORIGINAL_CMAKE_STRIP "${CMAKE_STRIP}" CACHE INTERNAL
            "Original strip binary")
    endif()

    message(STATUS "CMAKE_STRIP (original): ${BOBUI_INTERNAL_ORIGINAL_CMAKE_STRIP}")
    ...
    if((UNIX OR MINGW)
        ...
        # To make reconfiguration more robust when BOBUI_INTERNAL_STRIP_SUPPORTS_KEEP_SECTION is
        # manually removed, make sure to always restore the original strip first, by
        # re-assigning the original value.
        set(CMAKE_STRIP "${BOBUI_INTERNAL_ORIGINAL_CMAKE_STRIP}" CACHE STRING "")

        # Getting path to a binary we can run strip on.
        bobui_internal_try_compile_binary_for_strip(valid_binary_path)
        ...
        # Cache the result.
        set(BOBUI_INTERNAL_STRIP_SUPPORTS_KEEP_SECTION "${keep_section_supported}" CACHE BOOL
            "strip supports --keep-section")

        message(DEBUG
            "bobui_internal_generate_binary_strip_wrapper:\n"
            "original strip: ${BOBUI_INTERNAL_ORIGINAL_CMAKE_STRIP}\n"
        ...
        set(script_name "bobui-internal-strip")
        ...
        # Need to find the libexec input file depending whether the bobuibase sources are available.
        # This mirrors the logic in bobui_set_up_build_internals_paths.
        # TODO: Clean this up, together with bobui_set_up_build_internals_paths to only use the
        # the bobuibase sources when building bobuibase. And perhaps also when doing a non-prefix
        # developer-build.
        set(bobuibase_wrapper_in_path "${BOBUI_SOURCE_TREE}/${wrapper_rel_path}")
        set(installed_wrapper_in_path
            "${_bobui_cmake_dir}/${BOBUI_CMAKE_EXPORT_NAMESPACE}/${wrapper_rel_path}")

        # bobuibase sources available, always use them, regardless of prefix or non-prefix builds.
        if(EXISTS "${bobuibase_wrapper_in_path}")
            set(wrapper_in "${bobuibase_wrapper_in_path}")
        ...
        set(wrapper_out "${BOBUI_BUILD_DIR}/${INSTALL_LIBEXECDIR}/${script_name}${wrapper_extension}")

        # Used in the template file.
        set(original_strip "${BOBUI_INTERNAL_ORIGINAL_CMAKE_STRIP}")

        configure_file("${wrapper_in}" "${wrapper_out}" @ONLY)

        # Override the strip binary to be used by CMake install target.
        set(CMAKE_STRIP "${wrapper_out}" CACHE INTERNAL "Custom BobUI strip wrapper")

        message(STATUS "CMAKE_STRIP (used by BobUI): ${CMAKE_STRIP}")
    endif()
endfunction()

# Enable separate debug information for the given target
function(bobui_enable_separate_debug_info target installDestination)
    set(flags BOBUI_EXECUTABLE)
    if(APPLE)
    ...
    if (NOT BOBUI_FEATURE_separate_debug_info)
        return()
    endif()
    ...
    get_property(skip_separate_debug_info DIRECTORY "${target_source_dir}" PROPERTY _bobui_skip_separate_debug_info)
    ...
    if(APPLE)
        ...
        configure_file(
            "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/BobUISeparateDebugInfo.Info.plist.in"
            "Info.dSYM.plist"
            )
        ...
        if(arg_BOBUI_EXECUTABLE AND BOBUI_FEATURE_debug_and_release)
            ...
            foreach(cmake_config ${cmake_configs})
                ...
                if(NOT (cmake_config STREQUAL BOBUI_MULTI_CONFIG_FIRST_CONFIG))
                    set(install_optional_arg OPTIONAL)
                  else()
                    unset(install_optional_arg)
                endif()
                bobui_install(DIRECTORY ${debug_info_bundle_dir}
        ...
        bobui_install(FILES ${debug_info_target} DESTINATION ${installDestination})
    endif()
    list(APPEND commands
