...
#     Source files that should be added to the target unconditionally. Note that if target is BobUI
...
function(bobui_internal_extend_target target)
    if(NOT TARGET "${target}")
        message(FATAL_ERROR "${target} is not a target.")
    endif()
    bobui_internal_is_skipped_test(skipped ${target})
    if(skipped)
        return()
    endif()
    bobui_internal_is_in_test_batch(in_batch ${target})
    if(in_batch)
        _bobui_internal_test_batch_target_name(target)
    endif()

    # Don't try to extend_target when cross compiling an imported host target (like a tool).
    bobui_is_imported_target("${target}" is_imported)
    if(is_imported)
        return()
    endif()
    ...
    bobui_evaluate_config_expression(result ${arg_CONDITION})
    if(${result})
        if(BOBUI_CMAKE_DEBUG_EXTEND_TARGET)
            message("bobui_extend_target(${target} CONDITION ${arg_CONDITION} ...): Evaluated")
        endif()
        set(dbus_sources "")
        foreach(adaptor ${arg_DBUS_ADAPTOR_SOURCES})
            ...
            bobui_create_qdbusxml2cpp_command("${target}" "${adaptor}" ADAPTOR ${forwarded_args})
        ...
        foreach(interface ${arg_DBUS_INTERFACE_SOURCES})
            ...
            bobui_create_qdbusxml2cpp_command("${target}" "${interface}" INTERFACE ${forwarded_args})
        ...
        foreach(lib ${arg_PUBLIC_LIBRARIES} ${arg_LIBRARIES})
            ...
            if(NOT arg_PRECOMPILED_HEADER AND ${is_library})
                bobui_update_precompiled_header_with_library("${target}" "${lib}")
            endif()

            string(REGEX REPLACE "_nolink$" "" base_lib "${lib}")
            if(NOT base_lib STREQUAL lib)
                bobui_create_nolink_target("${base_lib}" ${target})
            endif()
            ...
            # different BobUI modules.
            if(TARGET "${lib}")
                get_target_property(is_imported ${lib} IMPORTED)
                if(NOT is_imported)
                    get_target_property(is_private ${lib} _bobui_is_private_module)
                    if(is_private)
                        get_target_property(lib ${lib} _bobui_public_module_target_name)
                    endif()
                    set(out_genex "$<TARGET_PROPERTY:${lib},_bobui_internal_sync_headers_target>")
                    set_property(TARGET ${target}
                        APPEND PROPERTY _bobui_internal_sync_headers_deps "${out_genex}")
                endif()
            endif()
        endforeach()

        list(TRANSFORM arg_PUBLIC_LIBRARIES REPLACE "^BobUI::" "${BOBUI_CMAKE_EXPORT_NAMESPACE}::")
        list(TRANSFORM arg_LIBRARIES REPLACE "^BobUI::" "${BOBUI_CMAKE_EXPORT_NAMESPACE}::")
        ...
        # _bobui_internal_target_sources property, since they can be useful in the source processing
        # functions. The property itself is not exported and should only be used in the BobUI internal
        # build tree.
        if(NOT is_interface_lib OR CMAKE_VERSION VERSION_GREATER_EQUAL "3.19")
            ...
            set_property(TARGET ${target} APPEND PROPERTY
                _bobui_internal_target_sources ${all_sources})
        endif()
        ...
        if(NOT is_interface_lib)
            ...
            if(NOT "x${arg_PLUGIN_TYPES}" STREQUAL "x")
                bobui_internal_add_plugin_types("${target}" "${arg_PLUGIN_TYPES}")
            endif()
        endif()

        # When computing the private library dependencies, we need to check not only the known
        # modules added by this repo's bobui_build_repo(), but also all module dependencies that
        ...
        if(arg_LIBRARIES)
            _bobui_internal_append_to_target_property_without_duplicates(${target}
                _bobui_extend_target_libraries "${arg_LIBRARIES}"
            )
        endif()

        if(arg_PUBLIC_LIBRARIES)
            _bobui_internal_append_to_target_property_without_duplicates(${target}
                _bobui_extend_target_public_libraries "${arg_PUBLIC_LIBRARIES}"
            )
        endif()

        set(all_libraries ${arg_LIBRARIES} ${arg_PUBLIC_LIBRARIES})
        _bobui_internal_work_around_autogen_discarded_dependencies(${target} ${all_libraries})
        ...
        if(BOBUI_GENERATE_SBOM)
            ...
            # recording of a target if it has no SBOM info. Relevant for BobUIFooModulePrivate targets
        ...
        if(TARGET "${target_private}")
            ...
            bobui_internal_register_target_dependencies("${target_private}"
        ...
        if(arg_PUBLIC_LIBRARIES)
            list(APPEND bobui_register_target_dependencies_args
                PUBLIC ${arg_PUBLIC_LIBRARIES})
        endif()
        if(bobui_libs_private OR arg_LIBRARIES)
            list(APPEND bobui_register_target_dependencies_args
                PRIVATE ${bobui_libs_private} ${arg_LIBRARIES})
        endif()
        bobui_internal_register_target_dependencies("${target}"
            ${bobui_register_target_dependencies_args})

        bobui_autogen_tools(${target}
                         ENABLE_AUTOGEN_TOOLS ${arg_ENABLE_AUTOGEN_TOOLS}
                         DISABLE_AUTOGEN_TOOLS ${arg_DISABLE_AUTOGEN_TOOLS})

        bobui_update_precompiled_header("${target}" "${arg_PRECOMPILED_HEADER}")
        ## Also exclude them from unity build
        bobui_update_ignore_pch_source("${target}" "${arg_NO_PCH_SOURCES}")
        ## Ignore objective-c files for PCH (not supported atm)
        bobui_ignore_pch_obj_c_sources("${target}" "${arg_SOURCES}")

        if(arg_NO_UNITY_BUILD)
            set_target_properties("${target}" PROPERTIES UNITY_BUILD OFF)
            bobui_update_ignore_unity_build_sources("${target}" "${arg_SOURCES}")
        endif()
        if(arg_NO_UNITY_BUILD_SOURCES)
            bobui_update_ignore_unity_build_sources("${target}" "${arg_NO_UNITY_BUILD_SOURCES}")
        endif()
    else()
        if(BOBUI_CMAKE_DEBUG_EXTEND_TARGET)
            message("bobui_extend_target(${target} CONDITION ${arg_CONDITION} ...): Skipped")
        endif()
    endif()

    if(arg_CONDITION_INDEPENDENT_SOURCES)
        set_source_files_properties(${arg_CONDITION_INDEPENDENT_SOURCES} PROPERTIES
            _bobui_extend_target_condition "${arg_CONDITION}"
            SKIP_AUTOGEN TRUE
        )

        bobui_internal_get_target_sources_property(sources_property)
        set_property(TARGET ${target} APPEND PROPERTY
    ...
    if(arg_EXTRA_ELF_LINKER_SCRIPT_CONTENT)
        set_target_properties(${target} PROPERTIES
            _bobui_extra_elf_linker_script_content "${arg_EXTRA_ELF_LINKER_SCRIPT_CONTENT}")
    endif()
    if(arg_EXTRA_ELF_LINKER_SCRIPT_EXPORTS)
        set_target_properties(${target} PROPERTIES
            _bobui_extra_elf_linker_script_exports "${arg_EXTRA_ELF_LINKER_SCRIPT_EXPORTS}")
    endif()
    if(arg_ELF_LINKER_DYNAMIC_LIST)
        bobui_internal_apply_dynamic_list_linker_flags(${target} "${arg_ELF_LINKER_DYNAMIC_LIST}")
    endif()

    if(is_executable)
        # If linking against Gui, make sure to also build the default QPA plugin.
        # This makes the experience of an initial BobUI configuration to build and run one single
        # test / executable nicer.
        set(linked_libs ${arg_PUBLIC_LIBRARIES} ${arg_LIBRARIES})
        if(linked_libs MATCHES "(^|;)(${BOBUI_CMAKE_EXPORT_NAMESPACE}::|BobUI::)?Gui($|;)" AND
        ...
        bobui_internal_import_plugins(${target} ${linked_libs})
    endif()
endfunction()
...
#
# This is necessary for static builds, because if BobUI6Foo links to BobUI6BarPrivate, this link
# dependency is purely internal. If we don't do this, CMake adds a target check for BobUI6BarPrivate
# in BobUI6FooTargets.cmake. This breaks if BobUI6BarPrivate is not find_package'ed before.
#
...
function(bobui_internal_wrap_private_modules target)
    set(option_args "")
    set(single_args
        OUT_VAR
    )
    ...
    foreach(lib IN LISTS arg_LIBRARIES)
        if(TARGET "${lib}")
            get_target_property(lib_is_private_module ${lib} _bobui_is_private_module)
            get_target_property(lib_is_internal_module ${lib} _bobui_is_internal_module)
            ...
            if(lib_is_private_module)
                ...
                # get a linker error about missing symbols from that BobUI module.
                get_target_property(lib_public_module_target ${lib} _bobui_public_module_target_name)
...
function(bobui_get_install_target_default_args)
...
macro(bobui_internal_setup_default_target_function_options)
    ...
    # Collection of arguments so they can be shared across bobui_internal_add_executable
    # and bobui_internal_add_test_helper.
    set(__bobui_internal_add_executable_optional_args
        GUI
        NO_INSTALL
        EXCEPTIONS
        DELAY_RC
        ...
        BOBUI_TEST
        BOBUI_MANUAL_TEST
        BOBUI_BENCHMARK_TEST
        NO_UNITY_BUILD
        ${__bobui_internal_sbom_optional_args}
    )
    set(__bobui_internal_add_executable_single_args
        CORE_LIBRARY
        OUTPUT_DIRECTORY
        INSTALL_DIRECTORY
        VERSION
    ...
    set(__bobui_internal_add_executable_multi_args
        ${__default_private_args}
        ${__default_public_args}
        ${__bobui_internal_sbom_multi_args}
    )
...
# e.g. lib/libBobUI6DBus_relwithdebinfo.6.3.0.dylib
...
function(bobui_internal_setup_cmake_config_postfix)
    # Collect configuration that require postfix in BobUI library names.
    ...
    # config build) to follow the conventions we had since BobUI 5.
    # e.g. lib/libBobUI6DBus_debug.6.3.0.dylib
    if(WIN32)
        if(MINGW)
    ...
    elseif(APPLE)
        ...
        if(NOT (BOBUI_FEATURE_framework OR FEATURE_framework
           # Account for default in configure.json being ON
           OR (NOT DEFINED BOBUI_FEATURE_framework AND NOT DEFINED FEATURE_framework)))
    ...
    if(custom_postfix_vars)
        ...
        message(WARNING "You are using custom library postfixes: '${postfix_vars_string}' which are"
            " considered experimental and are not officially supported by BobUI."
            " Expect unforeseen issues and user projects built with qmake to be broken."
        )
    endif()
endfunction()

function(bobui_is_imported_target target out_var)
    if(NOT TARGET "${target}")
        set(target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${target}")
    endif()
    if(NOT TARGET "${target}")
        message(FATAL_ERROR "Invalid target given to bobui_is_imported_target: ${target}")
    endif()
    get_target_property(is_imported "${target}" IMPORTED)
    set(${out_var} "${is_imported}" PARENT_SCOPE)
endfunction()

# Add BobUI::target and BobUI6::target as aliases for the target
function(bobui_internal_add_target_aliases target)
    set(versionless_alias "BobUI::${target}")
    set(versionfull_alias "BobUI${PROJECT_VERSION_MAJOR}::${target}")
    set_target_properties("${target}" PROPERTIES _bobui_versionless_alias "${versionless_alias}")
    set_target_properties("${target}" PROPERTIES _bobui_versionfull_alias "${versionfull_alias}")

    get_target_property(type "${target}" TYPE)
    if (type STREQUAL EXECUTABLE)
        if(NOT TARGET "${versionless_alias}")
            add_executable("${versionless_alias}" ALIAS "${target}")
        endif()
        if(NOT TARGET "${versionfull_alias}")
...
function(bobui_get_cmake_configurations out_var)
...
function(bobui_clone_property_for_configs target property configs)
...
function(bobui_handle_multi_config_output_dirs target)
    bobui_get_cmake_configurations(possible_configs)
    bobui_clone_property_for_configs(${target} LIBRARY_OUTPUT_DIRECTORY "${possible_configs}")
    bobui_clone_property_for_configs(${target} RUNTIME_OUTPUT_DIRECTORY "${possible_configs}")
    bobui_clone_property_for_configs(${target} ARCHIVE_OUTPUT_DIRECTORY "${possible_configs}")
endfunction()

# Set target properties that are the same for all modules, plugins, executables
# and 3rdparty libraries.
function(bobui_set_common_target_properties target)
    if(BOBUI_FEATURE_reduce_exports)
        set_target_properties(${target} PROPERTIES
    ...
    bobui_internal_set_compile_pdb_names("${target}")
endfunction()
...
function(bobui_set_target_info_properties target)
    ...
    if(NOT arg_TARGET_PRODUCT)
        set(arg_TARGET_PRODUCT "BobUI6")
    endif()
    if(NOT arg_TARGET_DESCRIPTION)
        set(arg_TARGET_DESCRIPTION "C++ Application Development Framework")
    endif()
    if(NOT arg_TARGET_COMPANY)
        set(arg_TARGET_COMPANY "The BobUI Company Ltd.")
    endif()
    if(NOT arg_TARGET_COPYRIGHT)
        set(arg_TARGET_COPYRIGHT "${BOBUI_COPYRIGHT}")
    endif()
    set_target_properties(${target} PROPERTIES
        BOBUI_TARGET_VERSION "${arg_TARGET_VERSION}"
        BOBUI_TARGET_COMPANY_NAME "${arg_TARGET_COMPANY}"
        BOBUI_TARGET_DESCRIPTION "${arg_TARGET_DESCRIPTION}"
        BOBUI_TARGET_COPYRIGHT "${arg_TARGET_COPYRIGHT}"
        BOBUI_TARGET_PRODUCT_NAME "${arg_TARGET_PRODUCT}")
endfunction()

# Uses the BOBUI_DELAYED_TARGET_* property values to set the final BOBUI_TARGET_* properties.
# Needed when doing executable finalization at the end of a subdirectory scope
# (aka scope finalization).
function(bobui_internal_set_target_info_properties_from_delayed_properties target)
    set(args "")
    foreach(prop ${__default_target_info_args})
        get_target_property(prop_value "${target}" "BOBUI_DELAYED_${prop}")
        list(APPEND args "${prop}" "${prop_value}")
    endforeach()
    bobui_set_target_info_properties(${target} ${args})
endfunction()

# Updates the BOBUI_DELAYED_ properties with values from the BOBUI_ variants, in case if they were
# set in-between a bobui_add_* call and before scope finalization.
function(bobui_internal_update_delayed_target_info_properties target)
    foreach(prop ${__default_target_info_args})
        get_target_property(prop_value "${target}" "BOBUI_${prop}")
        get_target_property(delayed_prop_value ${target} "BOBUI_DELAYED_${prop}")
        set(final_value "${delayed_prop_value}")
        if(prop_value)
            set(final_value "${prop_value}")
        endif()
        set_target_properties(${target} PROPERTIES "BOBUI_DELAYED_${prop}" "${final_value}")
    endforeach()
endfunction()

function(bobui_internal_check_directory_or_type name dir type default result_var)
    if ("x${dir}" STREQUAL x)
        if("x${type}" STREQUAL x)
            message(FATAL_ERROR
                "bobui_internal_add_plugin called without setting either PLUGIN_TYPE or ${name}.")
        endif()
...
macro(bobui_internal_get_export_additional_targets_keywords option_args single_args multi_args)
    set(${option_args}
    )
    set(${single_args}
        EXPORT_NAME_PREFIX
...
# Create a BobUI*AdditionalTargetInfo.cmake file that is included by BobUI*Config.cmake
# and sets IMPORTED_*_<CONFIG> properties on the exported targets.
#
# The file also makes the targets global if the BOBUI_PROMOTE_TO_GLOBAL_TARGETS property is set in the
# consuming project.
# When using a CMake version lower than 3.21, only the specified TARGETS are made global.
# E.g. transitive non-BobUI 3rd party targets of the specified targets are not made global.
#
# EXPORT_NAME_PREFIX:
...
# TARGET_EXPORT_NAMES:
#    The target names how they appear in the BobUIXXXTargets.cmake files.
#    The names get prefixed by ${BOBUI_CMAKE_EXPORT_NAMESPACE}:: unless they already are.
...
#          TARGET_EXPORT_NAMES = BobUI6::qmljs
#
function(bobui_internal_export_additional_targets_file)
    bobui_internal_get_export_additional_targets_keywords(option_args single_args multi_args)
    ...
    bobui_internal_append_export_additional_targets()

    set_property(GLOBAL APPEND
        PROPERTY _bobui_export_additional_targets_export_name_prefix_${id} "${arg_EXPORT_NAME_PREFIX}")
    set_property(GLOBAL APPEND
        PROPERTY _bobui_export_additional_targets_config_install_dir_${id} "${arg_CONFIG_INSTALL_DIR}")

    bobui_add_list_file_finalizer(bobui_internal_export_additional_targets_file_finalizer "${id}")
endfunction()

function(bobui_internal_get_export_additional_targets_id export_name out_var)
...
macro(bobui_internal_append_export_additional_targets)
    bobui_internal_validate_export_additional_targets(
        EXPORT_NAME_PREFIX "${arg_EXPORT_NAME_PREFIX}"
        TARGETS ${arg_TARGETS}
        TARGET_EXPORT_NAMES ${arg_TARGET_EXPORT_NAMES})

    bobui_internal_get_export_additional_targets_id("${arg_EXPORT_NAME_PREFIX}" id)

    set_property(GLOBAL APPEND
        PROPERTY _bobui_export_additional_targets_${id} "${arg_TARGETS}")
    set_property(GLOBAL APPEND
        PROPERTY _bobui_export_additional_target_export_names_${id} "${arg_TARGET_EXPORT_NAMES}")
endmacro()

# Can be called to add additional targets to the file after the initial setup call.
# Used for resources.
function(bobui_internal_add_targets_to_additional_targets_export_file)
    bobui_internal_get_export_additional_targets_keywords(option_args single_args multi_args)
    cmake_parse_arguments(arg
        "${option_args}"
        "${single_args}"
...
function(bobui_internal_validate_export_additional_targets)
    bobui_internal_get_export_additional_targets_keywords(option_args single_args multi_args)
    cmake_parse_arguments(arg
        "${option_args}"
        "${single_args}"
    ...
    if(NOT arg_EXPORT_NAME_PREFIX)
        message(FATAL_ERROR "bobui_internal_validate_export_additional_targets: "
...
# one specific id.
function(bobui_internal_export_additional_targets_file_finalizer id)
    get_property(arg_EXPORT_NAME_PREFIX GLOBAL PROPERTY
        _bobui_export_additional_targets_export_name_prefix_${id})
    get_property(arg_CONFIG_INSTALL_DIR GLOBAL PROPERTY
        _bobui_export_additional_targets_config_install_dir_${id})
    get_property(arg_TARGETS GLOBAL PROPERTY
        _bobui_export_additional_targets_${id})
    get_property(arg_TARGET_EXPORT_NAMES GLOBAL PROPERTY
        _bobui_export_additional_target_export_names_${id})
    ...
    if(active_release_configurations)
        ...
        # and for BOBUI_DEFAULT_IMPORT_CONFIGURATION.
        ...
        # Use the first active configuration for BOBUI_DEFAULT_IMPORT_CONFIGURATION.
...
if(NOT DEFINED BOBUI_DEFAULT_IMPORT_CONFIGURATION)
    set(BOBUI_DEFAULT_IMPORT_CONFIGURATION ${uc_default_cfg})
endif()
")
    ...
    foreach(i RANGE ${n})
        ...
        if(NOT full_target MATCHES "^${BOBUI_CMAKE_EXPORT_NAMESPACE}::")
            string(PREPEND full_target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::")
        endif()

        # Tools are already made global unconditionally in BobUIFooToolsConfig.cmake.
        # And the
        get_target_property(target_type ${target} TYPE)
        if(NOT target_type STREQUAL "EXECUTABLE")
            string(APPEND content
                "__bobui_internal_promote_target_to_global_checked(${full_target})\n")
        endif()
        ...
        get_target_property(is_configure_time_target ${target} _bobui_internal_configure_time_target)
        if(is_configure_time_target)
            # For Multi-config developer builds we should simply reuse IMPORTED_LOCATION of the
            # target.
            if(NOT BOBUI_WILL_INSTALL AND BOBUI_FEATURE_debug_and_release)
                ...
                get_target_property(configure_time_target_build_location ${target}
                    _bobui_internal_configure_time_target_build_location)
                string(TOUPPER "${BOBUI_CMAKE_EXPORT_NAMESPACE}_INSTALL_PREFIX" install_prefix_var)
                ...
                get_target_property(configure_time_target_install_location ${target}
                    _bobui_internal_configure_time_target_install_location)
...
if(NOT TARGET ${full_target})
    set(_bobui_imported_build_location \"${configure_time_target_build_location}\")
    set(_bobui_imported_install_location \"${configure_time_target_install_location}\")
    set(_bobui_imported_location \"\${_bobui_imported_install_location}\")
    if(NOT EXISTS \"$\{_bobui_imported_location}\"
          AND NOT \"$\{_bobui_imported_build_location}\" STREQUAL \"\")
        set(_bobui_imported_location \"\${_bobui_imported_build_location}\")
    endif()
    if(NOT EXISTS \"$\{_bobui_imported_location}\")
        message(FATAL_ERROR \"Unable to add configure time executable ${full_target}\"
            \" $\{_bobui_imported_location} doesn't exists\")
    endif()
    ...
    set_target_properties(${full_target} PROPERTIES IMPORTED_LOCATION_${uc_default_cfg}
        \"$\{_bobui_imported_location}\")
    set_property(TARGET ${full_target} PROPERTY IMPORTED_GLOBAL TRUE)
    unset(_bobui_imported_location)
    unset(_bobui_imported_build_location)
    unset(_bobui_imported_install_location)
endif()
\n")
            endif()
        endif()
...
get_target_property(_bobui_imported_location ${full_target} IMPORTED_LOCATION_DEBUG)
if(NOT EXISTS \"$\{_bobui_imported_location}\")
    get_target_property(_bobui_imported_configs ${full_target} IMPORTED_CONFIGURATIONS)
    list(REMOVE_ITEM _bobui_imported_configs DEBUG)
    set_property(TARGET ${full_target} PROPERTY IMPORTED_CONFIGURATIONS $\{_bobui_imported_configs})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_LOCATION_DEBUG)
        ...
        if(NOT "${uc_release_cfg}" STREQUAL "")
            if(write_location)
                string(APPEND content "get_target_property(_bobui_imported_location ${full_target} IMPORTED_LOCATION_${uc_release_cfg})\n")
            endif()
            if(write_implib)
                string(APPEND content "get_target_property(_bobui_imported_implib ${full_target} IMPORTED_IMPLIB_${uc_release_cfg})\n")
            endif()
            if(write_soname)
                string(APPEND content "get_target_property(_bobui_imported_soname ${full_target} IMPORTED_SONAME_${uc_release_cfg})\n")
            endif()
            if(write_link_dependencies)
                string(APPEND content "get_target_property(_bobui_imported_link_dependencies ${full_target} IMPORTED_LINK_DEPENDENT_LIBRARIES_${uc_release_cfg})\n")
            endif()
            if(write_objects)
                string(APPEND content "get_target_property(_bobui_imported_objects ${full_target} IMPORTED_OBJECTS_${uc_release_cfg})\n")
                ...
                string(APPEND content "get_target_property(_bobui_imported_clr ${full_target} IMPORTED_COMMON_LANGUAGE_RUNTIME_${uc_release_cfg})\n")
            endif()
        endif()
        if(write_location)
            string(APPEND content "get_target_property(_bobui_imported_location_default ${full_target} IMPORTED_LOCATION_$\{BOBUI_DEFAULT_IMPORT_CONFIGURATION})\n")
        endif()
        if(write_implib)
            string(APPEND content "get_target_property(_bobui_imported_implib_default ${full_target} IMPORTED_IMPLIB_$\{BOBUI_DEFAULT_IMPORT_CONFIGURATION})\n")
        endif()
        if(write_soname)
            string(APPEND content "get_target_property(_bobui_imported_soname_default ${full_target} IMPORTED_SONAME_$\{BOBUI_DEFAULT_IMPORT_CONFIGURATION})\n")
        endif()
        if(write_link_dependencies)
            string(APPEND content "get_target_property(_bobui_imported_link_dependencies_default ${full_target} IMPORTED_LINK_DEPENDENT_LIBRARIES_$\{BOBUI_DEFAULT_IMPORT_CONFIGURATION})\n")
        endif()
        if(write_objects)
            string(APPEND content "get_target_property(_bobui_imported_objects_default ${full_target} IMPORTED_OBJECTS_$\{BOBUI_DEFAULT_IMPORT_CONFIGURATION})\n")
            string(APPEND content "get_target_property(_bobui_imported_clr_default ${full_target} IMPORTED_COMMON_LANGUAGE_RUNTIME_$\{BOBUI_DEFAULT_IMPORT_CONFIGURATION})\n")
        endif()
...
")
            ...
            if(write_location)
                string(APPEND content "
if(_bobui_imported_location${var_suffix})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_LOCATION${property_suffix} \"$\{_bobui_imported_location${var_suffix}}\")
endif()")
            endif()
            if(write_implib)
                string(APPEND content "
if(_bobui_imported_implib${var_suffix})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_IMPLIB${property_suffix} \"$\{_bobui_imported_implib${var_suffix}}\")
endif()")
            endif()
            if(write_soname)
                string(APPEND content "
if(_bobui_imported_soname${var_suffix})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_SONAME${property_suffix} \"$\{_bobui_imported_soname${var_suffix}}\")
endif()")
            endif()
            if(write_link_dependencies)
                string(APPEND content "
if(_bobui_imported_link_dependencies${var_suffix})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_LINK_DEPENDENT_LIBRARIES${property_suffix} \"$\{_bobui_imported_link_dependencies${var_suffix}}\")
endif()")
            endif()
            if(write_objects)
                string(APPEND content "
if(_bobui_imported_objects${var_suffix})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_OBJECTS${property_suffix} \"$\{_bobui_imported_objects${var_suffix}}\")
endif()")
            string(APPEND content "
if(_bobui_imported_clr${var_suffix})
    set_property(TARGET ${full_target} PROPERTY IMPORTED_COMMON_LANGUAGE_RUNTIME${property_suffix} \"$\{_bobui_imported_clr${var_suffix}}\")
endif()")
            endif()
            string(APPEND content "\n")
        endforeach()
    ...
    if(properties_retrieved)
        string(APPEND content "
unset(_bobui_imported_location)
unset(_bobui_imported_location_default)
unset(_bobui_imported_soname)
unset(_bobui_imported_soname_default)
unset(_bobui_imported_link_dependencies)
unset(_bobui_imported_link_dependencies_default)
unset(_bobui_imported_objects)
unset(_bobui_imported_objects_default)
unset(_bobui_imported_clr)
unset(_bobui_imported_clr_default)
unset(_bobui_imported_configs)")
    endif()

    bobui_path_join(output_file "${arg_CONFIG_INSTALL_DIR}"
        "${arg_EXPORT_NAME_PREFIX}AdditionalTargetInfo.cmake")
    if(NOT IS_ABSOLUTE "${output_file}")
        bobui_path_join(output_file "${BOBUI_BUILD_DIR}" "${output_file}")
    endif()
    bobui_configure_file(OUTPUT "${output_file}" CONTENT "${content}")
    bobui_install(FILES "${output_file}" DESTINATION "${arg_CONFIG_INSTALL_DIR}")
endfunction()

function(bobui_internal_export_modern_cmake_config_targets_file)
    cmake_parse_arguments(arg
        ""
    ...
    configure_file("${BOBUI_CMAKE_DIR}/BobUIVersionlessTargets.cmake.in"
    ...
    configure_file("${BOBUI_CMAKE_DIR}/BobUIVersionlessAliasTargets.cmake.in"
        "${alias_export}"
        @ONLY
    )
...
#include \"${header_filename}\"")
        ...
        bobui_configure_file(OUTPUT "${header_path}" CONTENT "#include <private/bobuirace_p.h>\n")
    endif()
endfunction()

function(bobui_internal_generate_tracepoints name provider)
    ...
    if(BOBUI_FEATURE_lttng OR BOBUI_FEATURE_etw OR BOBUI_FEATURE_ctf)
        ...
        if(NOT "${BOBUI_HOST_PATH}" STREQUAL "")
            bobui_internal_get_host_info_var_prefix(host_info_var_prefix)
            bobui_path_join(tracepointgen
                "${BOBUI_HOST_PATH}"
                "${${host_info_var_prefix}_LIBEXECDIR}"
                "tracepointgen")
        else()
            set(tracepointgen "${BOBUI_CMAKE_EXPORT_NAMESPACE}::tracepointgen")
        endif()

        add_custom_target(${name}_${provider_name}_sync)
        bobui_internal_add_sync_header_dependencies(${name}_${provider_name}_sync ${name})
...
function(bobui_internal_set_compile_pdb_names target)
    if(MSVC)
        get_target_property(target_type ${target} TYPE)
...
function(bobui_internal_install_pdb_files target install_dir_path)
    if(MSVC)
        get_target_property(target_type ${target} TYPE)
        ...
        if(target_type STREQUAL "EXECUTABLE")
            ...
            foreach(cmake_config ${cmake_configs})
                ...
                bobui_install(FILES "$<TARGET_PDB_FILE:${target}>"
        ...
        elseif(target_type STREQUAL "STATIC_LIBRARY")
            ...
            bobui_path_join(compile_time_pdb_file_path
                         "${lib_dir}" "${pdb_name}$<$<CONFIG:Debug>:d>.pdb")

            bobui_install(FILES "${compile_time_pdb_file_path}"
        ...
        elseif(target_type STREQUAL "OBJECT_LIBRARY")
            ...
            if(NOT pdb_dir)
                ...
                if(BOBUI_GENERATOR_IS_MULTI_CONFIG)
                    bobui_path_join(pdb_dir "${pdb_dir}" "$<CONFIG>")
                endif()
            endif()
            get_target_property(pdb_name "${target}" COMPILE_PDB_NAME)
            bobui_path_join(compile_time_pdb_file_path
...
# Intel x86_64 arch.
# This behavior can be disabled for all targets by setting the BOBUI_FORCE_MACOS_ALL_ARCHES cache
# variable to TRUE or by setting the target specific cache variable
# BOBUI_FORCE_MACOS_ALL_ARCHES_${target} to TRUE.
#
...
function(bobui_internal_force_macos_intel_arch target)
    if(MACOS AND BOBUI_IS_MACOS_UNIVERSAL AND NOT BOBUI_FORCE_MACOS_ALL_ARCHES
            AND NOT BOBUI_FORCE_MACOS_ALL_ARCHES_${target})
        set(arches "x86_64")
        set_target_properties(${target} PROPERTIES OSX_ARCHITECTURES "${arches}")
    endif()
endfunction()

function(bobui_disable_apple_app_extension_api_only target)
    set_target_properties("${target}" PROPERTIES BOBUI_NO_APP_EXTENSION_ONLY_API TRUE)
endfunction()

# Common function to add BobUI prefixes to the target name
function(bobui_internal_bobuify_target out_var target)
    set(${out_var} "BobUI${target}" PARENT_SCOPE)
    set(${out_var}_versioned "BobUI${PROJECT_VERSION_MAJOR}${target}" PARENT_SCOPE)
endfunction()

function(bobui_internal_get_main_cmake_configuration out_var)
    if(CMAKE_BUILD_TYPE)
        set(config "${CMAKE_BUILD_TYPE}")
    elseif(BOBUI_MULTI_CONFIG_FIRST_CONFIG)
        set(config "${BOBUI_MULTI_CONFIG_FIRST_CONFIG}")
    endif()
    set("${out_var}" "${config}" PARENT_SCOPE)
endfunction()

function(bobui_internal_get_upper_case_main_cmake_configuration out_var)
    bobui_internal_get_main_cmake_configuration("${out_var}")
    string(TOUPPER "${${out_var}}" upper_config)
    set("${out_var}" "${upper_config}" PARENT_SCOPE)
endfunction()

function(bobui_internal_adjust_main_config_runtime_output_dir target output_dir)
    # When building BobUI with multiple configurations, place the main configuration executable
...
# internal BobUI API (bobui_internal_add_module, bobui_internal_add_plugin, etc) as opposed to it being
# a user project library (bobui_add_library, bobui_add_plugin, etc).
#
# Needed to allow selectively applying certain flags via PlatformXInternal targets.
function(bobui_internal_mark_as_internal_library target)
    set_target_properties(${target} PROPERTIES _bobui_is_internal_library TRUE)
    set_property(TARGET "${target}" APPEND PROPERTY EXPORT_PROPERTIES "_bobui_is_internal_library")
    bobui_internal_mark_as_internal_target(${target})
endfunction()

# Marks a target with a property that it was built using the internal BobUI API (bobui_internal_*) as
# opposed to it being a user project library or executable(bobui_add_*, etc).
#
# Needed to allow selectively applying certain flags via PlatformXInternal targets.
function(bobui_internal_mark_as_internal_target target)
    set_target_properties(${target} PROPERTIES _bobui_is_internal_target TRUE)
    set_property(TARGET "${target}" APPEND PROPERTY EXPORT_PROPERTIES "_bobui_is_internal_target")
endfunction()
...
#
# See BOBUIBUG-110369.
function(bobui_internal_skip_dependency_for_examples target)
    set_target_properties(${target} PROPERTIES _bobui_skip_dependency_for_examples TRUE)
endfunction()

function(bobui_internal_is_target_skipped_for_examples target out_var)
    get_property(is_skipped TARGET ${target} PROPERTY _bobui_skip_dependency_for_examples)
    if(NOT is_skipped)
...
function(bobui_internal_link_internal_platform_for_object_library target)
    ...
    # modules or plugins (e.g. object libraries added by bobui_internal_add_resource,
    # bobui_internal_add_plugin, etc.)
    ...
    target_link_libraries("${target}" PRIVATE BobUI::PlatformModuleInternal)

    if(arg_PARENT_TARGET AND TARGET "${arg_PARENT_TARGET}")
        foreach(property BOBUI_SKIP_WARNINGS_ARE_ERRORS _bobui_internal_use_exceptions)
...
function(bobui_internal_add_target_include_dirs target dep_target)
    if(NOT TARGET "${target}")
...
# latter would propagate a mandatory LINK_ONLY dependency on the ${dep_target} in a static BobUI build.
#
# The main use case is for propagating WrapVulkanHeaders::WrapVulkanHeaders.
function(bobui_internal_add_target_include_dirs_and_optionally_propagate target dep_target)
    bobui_internal_add_target_include_dirs(${target} ${dep_target})

    target_link_libraries("${target}" INTERFACE "$<TARGET_NAME_IF_EXISTS:${dep_target}>")

    bobui_record_extra_third_party_dependency("${target}" "${dep_target}")
endfunction()

# The function disables one or multiple internal global definitions that are defined by the
# bobui_internal_add_global_definition function for a specific 'target'.
function(bobui_internal_undefine_global_definition target)
    if(NOT TARGET "${target}")
        message(FATAL_ERROR "${target} is not a target.")
    endif()
    bobui_internal_is_skipped_test(skipped ${target})
    ...
    foreach(definition IN LISTS ARGN)
        set(undef_property_name "BOBUI_INTERNAL_UNDEF_${definition}")
        set_target_properties(${target} PROPERTIES "${undef_property_name}" TRUE)
    endforeach()
endfunction()

# This function adds any defines which are local to the current repository (e.g. bobuibase,
# bobuimultimedia). Those can be defined in the corresponding .cmake.conf file via
# BOBUI_EXTRA_INTERNAL_TARGET_DEFINES. BOBUI_EXTRA_INTERNAL_TARGET_DEFINES accepts a list of definitions.
...
function(bobui_internal_add_repo_local_defines target)
    get_target_property(type "${target}" TYPE)
    if (${type} STREQUAL "INTERFACE_LIBRARY")
        return()
    endif()
    if(DEFINED BOBUI_EXTRA_INTERNAL_TARGET_DEFINES)
        target_compile_definitions("${target}" PRIVATE ${BOBUI_EXTRA_INTERNAL_TARGET_DEFINES})
    endif()
endfunction()
...
# Note: The function works correctly only if bobui_internal_extend_target is used when adding source
# files.
function(bobui_internal_get_target_sources out_var target)
    bobui_internal_get_target_sources_property(sources_property)
...
function(bobui_internal_get_target_sources_property out_var)
    set(${out_var} "SOURCES")
    get_target_property(target_type ${target} TYPE)
    if(CMAKE_VERSION VERSION_LESS "3.19" AND target_type STREQUAL "INTERFACE_LIBRARY")
        set(${out_var} "_bobui_internal_target_sources")
    endif()
    set(${out_var} "${${out_var}}" PARENT_SCOPE)
endfunction()
...
# Usage:
#  bobui_internal_add_custom_properties_to_export(target
...
function(bobui_internal_add_custom_properties_to_export target)
    set(opt_args "")
    set(single_args "")
    set(multi_args
        PROPERTIES
    ...
    if(is_multi_config)
        ...
        # The genex snippet is evaluated to '$<CONFIG:'BobUI config type'>' in the generated cmake
        ...
        # The genex snippet is evaluated to '$<$<OR:$<CONFIG:'BobUI config type'>>:'Property content'>
        # for non-main BobUI configs and to
        # $<$<OR:$<CONFIG:'BobUI config type'>,$<NOT:$<BOOL:$<CONFIG>>>>:'Property content'> for the
        # main BobUI config. This guard is required to choose the correct value of the property for the
        ...
        # file(GENERATE call in the bobui_internal_export_genex_properties function.
    ...
    set(target_name "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${target}")
    ...
    foreach(property_source IN LISTS property_sources)
        if(property_source STREQUAL "PROPERTIES")
            ...
            set(output_property "_bobui_export_custom_properties_content")
        elseif(property_source STREQUAL "PROPERTIES_WITHOUT_GENEXES")
            ...
            set(output_property "_bobui_export_custom_properties_no_genexes_content")
        else()
...
# properties as added by the bobui_internal_add_custom_properties_to_export function.
#
...
#
# Example output:
# set_property(TARGET BobUI6::Foo PROPERTY MY_GENEX_PROP
#                     "$<$<OR:$<CONFIG:RelWithDebInfo>,$<NOT:$<BOOL:$<CONFIG>>>>:OneReleaseVal>")
# set_property(TARGET BobUI6::Foo PROPERTY MY_REGULAR_PROP "SecondValue")
# include("${CMAKE_CURRENT_LIST_DIR}/BobUI6FooExtraProperties-Debug.cmake")
# inside the include
# set_property(TARGET BobUI6::Foo APPEND PROPERTY MY_GENEX_PROP "$<$<OR:$<CONFIG:Debug>>:OneDebugVal>")
#
# Arguments:
# EXPORT_NAME_PREFIX:
...
function(bobui_internal_export_genex_properties)
    set(opt_args "")
    set(single_args
        EXPORT_NAME_PREFIX
        CONFIG_INSTALL_DIR
    ...
    if(NOT arg_EXPORT_NAME_PREFIX)
        message(FATAL_ERROR "bobui_internal_export_genex_properties: "
            "Missing EXPORT_NAME_PREFIX argument.")
    endif()
    ...
    if(targets_count GREATER 1)
        message(AUTHOR_WARNING "bobui_internal_export_genex_properties: "
    ...
    foreach(target IN LISTS arg_TARGETS)
        ...
        bobui_path_join(output_file "${arg_CONFIG_INSTALL_DIR}" "${file_name}")
        ...
        set(property_list
            "$<GENEX_EVAL:$<${t_prop},_bobui_export_custom_properties_content>>")
        ...
        set(property_no_genexes_list
            "$<GENEX_EVAL:$<${t_prop},_bobui_export_custom_properties_no_genexes_content>>")
        ...
        bobui_install(FILES "$<${content_available_condition}:${output_file}>"
            DESTINATION "${arg_CONFIG_INSTALL_DIR}"
            COMPONENT Devel
        )
    endforeach()
...
function(bobui_internal_add_platform_target target)
    _bobui_internal_add_library("${target}" INTERFACE)
    bobui_internal_add_target_aliases("${target}")

    # The platform targets should never be promoted to global via the
    # _bobui_internal_promote_3rd_party_target_to_global command.
    set_property(TARGET "${target}" PROPERTY _bobui_should_skip_3rd_party_global_promotion TRUE)
    set_property(TARGET "${target}" APPEND PROPERTY EXPORT_PROPERTIES
        "_bobui_should_skip_3rd_party_global_promotion")
endfunction()

# A small wrapper for adding the PlatformXInternal and GlobalConfig INTERFACE targets to apply
# common options.
# They can't be added via bobui_internal_add_module, because it automatically links to the
# PlatformInternal targets creating a cyclic dependency.
function(bobui_internal_add_platform_internal_target target)
    bobui_internal_add_platform_target("${target}")
    bobui_internal_mark_as_internal_library("${target}")

    bobui_internal_add_sbom("${target}"
        SBOM_ENTITY_TYPE BOBUI_MODULE
    )
endfunction()

# A small wrapper for passing --dynamic-list to the linker. It will ensure that the symbols will
# be mangled when bobui is compiled in a namespace
function(bobui_internal_apply_dynamic_list_linker_flags target dynlist_template)
    if(NOT BOBUI_FEATURE_reduce_relocations)
        return()
    endif()
    ...
    if(BOBUI_NAMESPACE)
        set(BOBUI_NAMESPACE_PREFIX "${BOBUI_NAMESPACE}::")
        set(BOBUI_NAMESPACE_MANGLE_SUFFIX "_${BOBUI_NAMESPACE}")
    else()
        set(BOBUI_NAMESPACE_PREFIX "")
        set(BOBUI_NAMESPACE_MANGLE_SUFFIX "")
    endif()
...
function(bobui_internal_workaround_static_lib_gcc_lto_issue target)
    get_target_property(target_type "${target}" TYPE)
...
function(bobui_internal_enable_optimized_tools_lto target)
    string(TOLOWER "${PROJECT_NAME}" project_name_lower)

    if(BOBUI_FEATURE_optimized_tools
            AND NOT BOBUI_FORCE_NO_OPTIMIZED_TOOLS_LTO
            AND NOT BOBUI_FORCE_NO_OPTIMIZE_${target}
            AND NOT BOBUI_FORCE_NO_OPTIMIZE_${project_name_lower}
            AND NOT BOBUI_FORCE_NO_LTO_${target}
            AND NOT BOBUI_FORCE_NO_LTO_${project_name_lower}
        )
