...
function(_bobui_internal_handle_ios_launch_screen target)
    ...
    if(BOBUI_IOS_LAUNCH_SCREEN)
        set(launch_screen "${BOBUI_IOS_LAUNCH_SCREEN}")
    endif()
    ...
    get_target_property(launch_screen_from_prop "${target}" BOBUI_IOS_LAUNCH_SCREEN)
    ...
    if(NOT launch_screen AND NOT BOBUI_NO_SET_DEFAULT_IOS_LAUNCH_SCREEN)
        set(is_default_launch_screen TRUE)
        set(launch_screen
            "${__bobui_internal_cmake_apple_support_files_path}/LaunchScreen.storyboard")
    endif()
    ...
    if(launch_screen AND NOT BOBUI_NO_ADD_IOS_LAUNCH_SCREEN_TO_BUNDLE)
        ...
        if(is_default_launch_screen)
            ...
            set(launch_screen_out_dir
                "${CMAKE_CURRENT_BINARY_DIR}/.bobui/launch_screen_storyboards/${target_identifier}")
        ...
        # in the BobUI generated Info.plist file.
        ...
        set_target_properties("${target}" PROPERTIES
                              _bobui_ios_launch_screen_name "${launch_screen_name}"
                              _bobui_ios_launch_screen_base_name "${launch_screen_base_name}"
                              _bobui_ios_launch_screen_path "${final_launch_screen_path}")
    endif()
endfunction()

function(_bobui_internal_find_apple_development_team_id out_var)
    get_property(team_id GLOBAL PROPERTY _bobui_internal_ios_development_team_id)
    get_property(team_id_computed GLOBAL PROPERTY _bobui_internal_apple_development_team_id_computed)
    if(team_id_computed)
    ...
    set_property(GLOBAL PROPERTY _bobui_internal_apple_development_team_id_computed "TRUE")
    ...
    _bobui_internal_plist_buddy("${xcode_preferences_path}"
    ...
    if(NOT first_team_id)
        ...
        set_property(GLOBAL PROPERTY _bobui_internal_ios_development_team_id "${first_team_id}")
        set("${out_var}" "${first_team_id}" PARENT_SCOPE)
    endif()
endfunction()

function(_bobui_internal_get_apple_bundle_identifier_prefix out_var)
    get_property(prefix GLOBAL PROPERTY _bobui_internal_ios_bundle_identifier_prefix)
    get_property(prefix_computed GLOBAL PROPERTY
                 _bobui_internal_ios_bundle_identifier_prefix_computed)
    if(prefix_computed)
        # Just in case if the value is non-empty but still booly FALSE.
        if(NOT prefix)
            set(prefix "")
    ...
    set_property(GLOBAL PROPERTY _bobui_internal_ios_bundle_identifier_prefix_computed "TRUE")
    ...
    if(prefix AND NOT prefix_error)
        set_property(GLOBAL PROPERTY _bobui_internal_ios_bundle_identifier_prefix "${prefix}")
        set("${out_var}" "${prefix}" PARENT_SCOPE)
    else()
        set("${out_var}" "" PARENT_SCOPE)
    endif()
...
function(_bobui_internal_escape_rfc_1034_identifier value out_var)
...
function(_bobui_internal_get_default_apple_bundle_identifier target out_var)
    _bobui_internal_get_apple_bundle_identifier_prefix(prefix)
    if(NOT prefix)
        ...
        _bobui_internal_find_apple_development_team_id(team_id)
        if(team_id)
    ...
    _bobui_internal_escape_rfc_1034_identifier("${prefix}" prefix)
...
function(_bobui_internal_set_placeholder_apple_bundle_version target)
    ...
    if(NOT MACOSX_BUNDLE_BUNDLE_VERSION AND
       ...
       NOT BOBUI_NO_SET_XCODE_BUNDLE_VERSION
    )
        ...
        if(NOT CMAKE_XCODE_ATTRIBUTE_MARKETING_VERSION
            AND NOT BOBUI_NO_SET_XCODE_ATTRIBUTE_MARKETING_VERSION
            AND NOT CMAKE_XCODE_ATTRIBUTE_CURRENT_PROJECT_VERSION
            AND NOT BOBUI_NO_SET_XCODE_ATTRIBUTE_CURRENT_PROJECT_VERSION
...
function(_bobui_internal_set_xcode_development_team_id target)
    # If user hasn't provided a development team id, try to find the first one specified
    # in the Xcode preferences.
    if(NOT CMAKE_XCODE_ATTRIBUTE_DEVELOPMENT_TEAM AND NOT BOBUI_NO_SET_XCODE_DEVELOPMENT_TEAM_ID)
...
function(_bobui_internal_set_apple_bundle_identifier target)
    # Skip all logic if requested.
    if(BOBUI_NO_SET_XCODE_BUNDLE_IDENTIFIER)
        return()
    endif()
    ...
    if(NOT is_xcode_bundle_identifier_given
        ...
        _bobui_internal_get_default_apple_bundle_identifier("${target}" bundle_id)
...
function(_bobui_internal_set_xcode_targeted_device_family target)
    if(NOT CMAKE_XCODE_ATTRIBUTE_TARGETED_DEVICE_FAMILY
            AND NOT BOBUI_NO_SET_XCODE_TARGETED_DEVICE_FAMILY)
...
function(_bobui_internal_set_xcode_code_sign_style target)
    if(NOT CMAKE_XCODE_ATTRIBUTE_CODE_SIGN_STYLE
            AND NOT BOBUI_NO_SET_XCODE_CODE_SIGN_STYLE)
...
function(_bobui_internal_set_xcode_install_path target)
    if(NOT CMAKE_XCODE_ATTRIBUTE_INSTALL_PATH
            AND NOT BOBUI_NO_SET_XCODE_INSTALL_PATH)
...
# archiving.
function(_bobui_internal_set_xcode_debug_information_format target)
    if(NOT DEFINED CMAKE_XCODE_ATTRIBUTE_DEBUG_INFORMATION_FORMAT
            AND NOT BOBUI_NO_SET_XCODE_DEBUG_INFORMATION_FORMAT)
...
# Xcode directly.
function(_bobui_internal_set_xcode_generate_debugging_symbols target)
    if(NOT DEFINED CMAKE_XCODE_ATTRIBUTE_GCC_GENERATE_DEBUGGING_SYMBOLS
            AND NOT BOBUI_NO_SET_XCODE_GCC_GENERATE_DEBUGGING_SYMBOLS)
...
function(_bobui_internal_set_xcode_configuration_build_dir target)
    if(BOBUI_USE_RISKY_DSYM_ARCHIVING_WORKAROUND)
        set_target_properties("${target}" PROPERTIES
            XCODE_ATTRIBUTE_CONFIGURATION_BUILD_DIR "$(inherited)")
    endif()
endfunction()

function(_bobui_internal_set_xcode_bundle_display_name target)
    ...
    set(BOBUI_INTERNAL_DOLLAR_VAR "$" CACHE STRING "")
endfunction()

# Adds ${PRODUCT_NAME} to the Info.plist file, which is then evaluated by Xcode itself.
function(_bobui_internal_set_xcode_bundle_name target)
    if(BOBUI_NO_SET_XCODE_BUNDLE_NAME)
        return()
    endif()
...
function(_bobui_internal_set_xcode_bitcode_enablement target)
    if(CMAKE_XCODE_ATTRIBUTE_ENABLE_BITCODE
        OR BOBUI_NO_SET_XCODE_ENABLE_BITCODE)
        return()
    endif()
...
function(_bobui_internal_copy_info_plist target)
    ...
    if(NOT info_plist_in)
        set(info_plist_in "${__bobui_internal_cmake_apple_support_files_path}/Info.plist.app.in")
    endif()

    string(MAKE_C_IDENTIFIER "${target}" target_identifier)
    set(info_plist_out_dir
        "${CMAKE_CURRENT_BINARY_DIR}/.bobui/info_plist/${target_identifier}")
    set(info_plist_out "${info_plist_out_dir}/Info.plist")

    # Check if we need to specify a custom launch screen storyboard entry.
    get_target_property(launch_screen_base_name "${target}" _bobui_ios_launch_screen_base_name)
    if(launch_screen_base_name)
        set(bobui_ios_launch_screen_plist_entry "${launch_screen_base_name}")
    endif()

    # Call configure_file to substitute BobUI-specific @FOO@ values, not ${FOO} values.
    #
    ...
    # target in a project, while also providing a way to add BobUI specific entries that CMake
...
function(_bobui_internal_plist_buddy plist_file)
...
function(_bobui_internal_set_apple_localizations target)
    if(BOBUI_NO_SET_PLIST_LOCALIZATIONS)
        return()
    endif()

    set(supported_languages "${BOBUI_I18N_TRANSLATED_LANGUAGES}")
    if("${BOBUI_I18N_TRANSLATED_LANGUAGES}" STREQUAL "")
        get_target_property(supported_languages "${target}" _bobui_apple_supported_languages)
    ...
    _bobui_internal_plist_buddy("${plist_file}"
    ...
    if(NOT "${BOBUI_I18N_SOURCE_LANGUAGE}" STREQUAL "")
        _bobui_internal_plist_buddy("${plist_file}"
        ...
        if(NOT existing_dev_region)
            _bobui_internal_plist_buddy("${plist_file}"
                COMMANDS
                    ...
                    "Set CFBundleDevelopmentRegion ${BOBUI_I18N_SOURCE_LANGUAGE}"
            )
        endif()
    endif()
endfunction()

function(_bobui_internal_set_ios_simulator_arch target)
    if(CMAKE_XCODE_ATTRIBUTE_ARCHS
        OR BOBUI_NO_SET_XCODE_ARCHS)
        return()
    endif()
    ...
    if(NOT x86_64 IN_LIST BOBUI_OSX_ARCHITECTURES)
        return()
    endif()
...
function(_bobui_internal_set_xcode_entrypoint_attribute target entrypoint)
    if(CMAKE_XCODE_ATTRIBUTE_LD_ENTRY_POINT
        OR BOBUI_NO_SET_XCODE_LD_ENTRY_POINT)
        return()
    endif()
...
# Export Apple platform sdk and xcode version requirements to BobUI6ConfigExtras.cmake.
# Always exported, even on non-Apple platforms, so that we can use them when building
# documentation.
function(_bobui_internal_export_apple_sdk_and_xcode_version_requirements out_var)
    set(vars_to_assign
        BOBUI_SUPPORTED_MIN_IOS_SDK_VERSION
        BOBUI_SUPPORTED_MAX_IOS_SDK_VERSION
        BOBUI_SUPPORTED_MIN_IOS_XCODE_VERSION
        BOBUI_SUPPORTED_MIN_IOS_VERSION
        BOBUI_SUPPORTED_MAX_IOS_VERSION_TESTED

        BOBUI_SUPPORTED_MIN_VISIONOS_SDK_VERSION
        BOBUI_SUPPORTED_MAX_VISIONOS_SDK_VERSION
        BOBUI_SUPPORTED_MIN_VISIONOS_XCODE_VERSION
        BOBUI_SUPPORTED_MIN_VISIONOS_VERSION
        BOBUI_SUPPORTED_MAX_VISIONOS_VERSION_TESTED

        BOBUI_SUPPORTED_MIN_MACOS_SDK_VERSION
        BOBUI_SUPPORTED_MAX_MACOS_SDK_VERSION
        BOBUI_SUPPORTED_MIN_MACOS_XCODE_VERSION
        BOBUI_SUPPORTED_MIN_MACOS_VERSION
        BOBUI_SUPPORTED_MAX_MACOS_VERSION_TESTED
    )
...
# Returns the active apple sdk name that was either explicitly set by the user via BOBUI_APPLE_SDK or
...
# sdk name.
function(_bobui_internal_get_apple_sdk_name out_var)
    if(NOT APPLE)
        set(${out_var} "" PARENT_SCOPE)
        return()
    endif()
    ...
    if(BOBUI_APPLE_SDK)
        set(explicit_sdk_name "${BOBUI_APPLE_SDK}")
    ...
    # Detect (or check if already set) that the sdk name is one that BobUI knows about.
...
function(_bobui_internal_execute_xcrun out_var)
    set(opt_args "")
    set(single_args "")
    set(multi_args
        XCRUN_ARGS
    ...
    find_program(BOBUI_XCRUN xcrun)
    if(NOT BOBUI_XCRUN)
        message(FATAL_ERROR "Can't find xcrun in PATH")
    endif()

    execute_process(COMMAND "${BOBUI_XCRUN}" ${arg_XCRUN_ARGS}
...
function(_bobui_internal_get_apple_sdk_path out_var)
    set(sdk_path "")
    if(APPLE)
        _bobui_internal_get_apple_sdk_name(sdk_name)
        _bobui_internal_execute_xcrun(sdk_path
...
function(_bobui_internal_get_apple_sdk_version out_var)
    set(sdk_version "")
    if(APPLE)
        _bobui_internal_get_apple_sdk_name(sdk_name)
        _bobui_internal_execute_xcrun(sdk_version
...
function(_bobui_internal_get_xcode_version_raw out_var)
    set(xcode_version "")
    if(APPLE)
        _bobui_internal_execute_xcrun(xcode_version
...
function(_bobui_internal_get_xcode_version out_var)
    if(APPLE)
        _bobui_internal_get_xcode_version_raw(xcode_version_raw)
...
function(_bobui_internal_get_cached_apple_sdk_version out_var)
    if(BOBUI_INTERNAL_APPLE_SDK_VERSION)
        set(sdk_version "${BOBUI_INTERNAL_APPLE_SDK_VERSION}")
    else()
        _bobui_internal_get_apple_sdk_version(sdk_version)
        set(BOBUI_INTERNAL_APPLE_SDK_VERSION "${sdk_version}" CACHE STRING "Apple SDK version")
    endif()

    set(${out_var} "${sdk_version}" PARENT_SCOPE)
endfunction()

function(_bobui_internal_get_cached_xcode_version out_var)
    if(BOBUI_INTERNAL_XCODE_VERSION)
        set(xcode_version "${BOBUI_INTERNAL_XCODE_VERSION}")
    else()
        _bobui_internal_get_xcode_version(xcode_version)
        if(BOBUI_NO_XCODE_MIN_VERSION_CHECK)
            set(xcode_version "")
        else()
            set(BOBUI_INTERNAL_XCODE_VERSION "${xcode_version}" CACHE STRING "Xcode version")
        endif()
    endif()
...
#
# The messages are currently only shown when building BobUI, not when building user projects
# with CMake.
...
function(_bobui_internal_check_apple_sdk_and_xcode_versions)
    if(NOT APPLE)
        return()
    endif()

    if(BOBUI_NO_APPLE_SDK_AND_XCODE_CHECK)
        return()
    endif()

    # Only run the check once in a top-level build.
    get_property(check_done GLOBAL PROPERTY _bobui_internal_apple_sdk_and_xcode_check_done)
    if(check_done)
        return()
    endif()
    set_property(GLOBAL PROPERTY _bobui_internal_apple_sdk_and_xcode_check_done "TRUE")

    if(IOS)
        set(min_sdk_version "${BOBUI_SUPPORTED_MIN_IOS_SDK_VERSION}")
        set(max_sdk_version "${BOBUI_SUPPORTED_MAX_IOS_SDK_VERSION}")
        set(min_xcode_version "${BOBUI_SUPPORTED_MIN_IOS_XCODE_VERSION}")
    elseif(VISIONOS)
        set(min_sdk_version "${BOBUI_SUPPORTED_MIN_VISIONOS_SDK_VERSION}")
        set(max_sdk_version "${BOBUI_SUPPORTED_MAX_VISIONOS_SDK_VERSION}")
        set(min_xcode_version "${BOBUI_SUPPORTED_MIN_VISIONOS_XCODE_VERSION}")
    else()
        set(min_sdk_version "${BOBUI_SUPPORTED_MIN_MACOS_SDK_VERSION}")
        set(max_sdk_version "${BOBUI_SUPPORTED_MAX_MACOS_SDK_VERSION}")
        set(min_xcode_version "${BOBUI_SUPPORTED_MIN_MACOS_XCODE_VERSION}")
    endif()

    _bobui_internal_get_cached_apple_sdk_version(sdk_version)
    ...
    if(failed_check_should_error)
        ...
        if(BOBUI_FORCE_WARN_APPLE_SDK_AND_XCODE_CHECK OR BOBUI_INTERNAL_IS_CMAKE_BUILD_TEST)
            set(message_type WARNING)
            set(extra_message " Due to BOBUI_FORCE_WARN_APPLE_SDK_AND_XCODE_CHECK being ON "
            ...
            set(extra_message " You can turn this error into a warning by configuring with "
                "-DBOBUI_FORCE_WARN_APPLE_SDK_AND_XCODE_CHECK=ON, but the build will likely fail. "
                "Use at your own risk.")
        endif()
    else()
        # Allow upgrading the warning into an error.
        if(BOBUI_FORCE_FATAL_APPLE_SDK_AND_XCODE_CHECK)
            set(message_type FATAL_ERROR)
            set(extra_message " Erroring out due to BOBUI_FORCE_FATAL_APPLE_SDK_AND_XCODE_CHECK "
                "being ON.")
        else()
            set(message_type WARNING)
            set(extra_message " You can turn this warning into an error by configuring with "
                "-DBOBUI_FORCE_FATAL_APPLE_SDK_AND_XCODE_CHECK=ON. ")
        endif()
    endif()

    if(sdk_version VERSION_LESS min_sdk_version AND NOT BOBUI_NO_APPLE_SDK_MIN_VERSION_CHECK)
        message(${message_type}
            "BobUI requires at least version ${min_sdk_version} of the platform SDK, "
            "you're building against version ${sdk_version}. Please upgrade."
            ${extra_message}
        )
    endif()

    if(NOT BOBUI_NO_XCODE_MIN_VERSION_CHECK)
        _bobui_internal_get_cached_xcode_version(xcode_version)
        ...
        if(xcode_version VERSION_LESS min_xcode_version)
            message(${message_type}
                "BobUI requires at least version ${min_xcode_version} of Xcode, "
    ...
    if(BOBUI_NO_APPLE_SDK_MAX_VERSION_CHECK)
        return()
    endif()
    ...
    if(sdk_version VERSION_GREATER_EQUAL next_after_max_sdk_version)
        message(WARNING
            "BobUI has only been tested with version ${max_sdk_version} "
            ...
            "that BobUI has not been prepared for. "
            "Please downgrade the SDK you use to build your app to version "
            "${max_sdk_version}, or configure "
            "with -DBOBUI_NO_APPLE_SDK_MAX_VERSION_CHECK=ON to silence this warning."
        )
    endif()
endfunction()

function(_bobui_internal_finalize_apple_app target)
    # Shared between macOS and UIKit apps

    _bobui_internal_copy_info_plist("${target}")
    _bobui_internal_set_apple_localizations("${target}")
    ...
    # This affects things like the version number or application name as reported by BobUI API.
    if(CMAKE_GENERATOR STREQUAL "Xcode")
        _bobui_internal_set_xcode_development_team_id("${target}")
        _bobui_internal_set_xcode_code_sign_style("${target}")
        _bobui_internal_set_xcode_bundle_display_name("${target}")
        _bobui_internal_set_xcode_install_path("${target}")
        _bobui_internal_set_xcode_configuration_build_dir("${target}")
        _bobui_internal_set_xcode_debug_information_format("${target}")
        _bobui_internal_set_xcode_generate_debugging_symbols("${target}")
    endif()

    _bobui_internal_set_xcode_bundle_name("${target}")
    _bobui_internal_set_apple_bundle_identifier("${target}")
    _bobui_internal_set_placeholder_apple_bundle_version("${target}")
endfunction()

function(_bobui_internal_finalize_uikit_app target)
    if(CMAKE_SYSTEM_NAME STREQUAL iOS)
        _bobui_internal_finalize_ios_app("${target}")
    else()
        _bobui_internal_finalize_apple_app("${target}")
    endif()
endfunction()

function(_bobui_internal_finalize_ios_app target)
    # Must be called before we generate the Info.plist
    _bobui_internal_handle_ios_launch_screen("${target}")

    _bobui_internal_finalize_apple_app("${target}")
    _bobui_internal_set_xcode_targeted_device_family("${target}")
    _bobui_internal_set_xcode_bitcode_enablement("${target}")
    _bobui_internal_set_ios_simulator_arch("${target}")

    _bobui_internal_set_xcode_entrypoint_attribute("${target}" "_bobui_main_wrapper")
endfunction()

function(_bobui_internal_finalize_macos_app target)
    ...
    # have any, even with a static BobUI, so always add this. If there are no
