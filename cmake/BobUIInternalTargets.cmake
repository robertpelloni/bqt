...
function(bobui_internal_set_warnings_are_errors_flags target target_scope)
    # Gate everything by the target property
    set(common_conditions "$<NOT:$<BOOL:$<TARGET_PROPERTY:BOBUI_SKIP_WARNINGS_ARE_ERRORS>>>")
    ...
    set(clang_msvc_frontend_args
        "$<NOT:$<BOOL:$<TARGET_PROPERTY:BobUI6::PlatformCommonInternal,_bobui_internal_clang_msvc_frontend>>>"
    )
    if(GCC OR CLANG OR WIN32)
        ...
        bobui_internal_add_compiler_dependent_flags("${target}" ${target_scope}
        ...
        bobui_internal_add_compiler_dependent_flags("${target}" ${target_scope}
            COMPILERS GNU
                ...
                CONDITIONS VERSION_LESS 15.1
                    AND $<BOOL:${BOBUI_FEATURE_sanitize_thread}>
                    AND $<BOOL:${BUILD_WITH_PCH}>
                    OPTIONS
                    # GCC < 15 raises a TSAN warning from BobUI's own PCHs, despite the warning
                    # being suppressed (BOBUIBUG-134415)
...
#        Possible values:
#            MODULE - set the definition for all BobUI modules
#            PLUGIN - set the definition for all BobUI plugins
#            TOOL - set the definition for all BobUI tools
#            APP - set the definition for all BobUI applications
#        TODO: Add a tests specific platform target and the definition scope for it.
function(bobui_internal_add_global_definition definition)
    set(optional_args "")
...
bobui_internal_add_platform_internal_target(PlatformCommonInternal)
target_link_libraries(PlatformCommonInternal INTERFACE Platform)

bobui_internal_add_platform_internal_target(PlatformModuleInternal)
target_link_libraries(PlatformModuleInternal INTERFACE PlatformCommonInternal)

bobui_internal_add_platform_internal_target(PlatformPluginInternal)
target_link_libraries(PlatformPluginInternal INTERFACE PlatformCommonInternal)

bobui_internal_add_platform_internal_target(PlatformAppInternal)
target_link_libraries(PlatformAppInternal INTERFACE PlatformCommonInternal)

bobui_internal_add_platform_internal_target(PlatformToolInternal)
target_link_libraries(PlatformToolInternal INTERFACE PlatformAppInternal)

bobui_internal_add_platform_internal_target(PlatformExampleInternal)

bobui_internal_add_global_definition(BOBUI_NO_JAVA_STYLE_ITERATORS)
bobui_internal_add_global_definition(BOBUI_NO_QASCONST)
bobui_internal_add_global_definition(BOBUI_NO_QEXCHANGE)
bobui_internal_add_global_definition(BOBUI_NO_QSNPRINTF)
bobui_internal_add_global_definition(BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT)
bobui_internal_add_global_definition(BOBUI_EXPLICIT_QFILE_CONSTRUCTION_FROM_PATH)
bobui_internal_add_global_definition(BOBUI_USE_QSTRINGBUILDER SCOPE PLUGIN TOOL MODULE)
bobui_internal_add_global_definition(BOBUI_NO_FOREACH)
bobui_internal_add_global_definition(BOBUI_NO_STD_FORMAT_SUPPORT SCOPE PLUGIN TOOL MODULE)

bobui_internal_set_warnings_are_errors_flags(PlatformModuleInternal INTERFACE)
bobui_internal_set_warnings_are_errors_flags(PlatformPluginInternal INTERFACE)
bobui_internal_set_warnings_are_errors_flags(PlatformAppInternal INTERFACE)
bobui_internal_set_warnings_are_errors_flags(PlatformExampleInternal INTERFACE)

if(WIN32)
    # Needed for M_PI define. Same as mkspecs/features/bobui_module.prf.
...
target_compile_definitions(PlatformCommonInternal INTERFACE BOBUI_NO_NARROWING_CONVERSIONS_IN_CONNECT)
target_compile_definitions(PlatformCommonInternal INTERFACE $<$<NOT:$<CONFIG:Debug>>:BOBUI_NO_DEBUG>)

if(FEATURE_developer_build)
    # This causes an ABI break on Windows, so we cannot unconditionally
    # enable it. Keep it for developer builds only for now.
    ### BobUI 7: remove the if.
    target_compile_definitions(PlatformCommonInternal INTERFACE BOBUI_STRICT_QLIST_ITERATORS)
endif()

function(bobui_internal_apply_bitcode_flags target)
    ...
    set(is_enabled_genex "$<NOT:$<BOOL:$<TARGET_PROPERTY:BOBUI_NO_BITCODE>>>")
...
# Function guards linker options that are applicable for internal BobUI targets only from propagating
# them to user projects.
function(bobui_internal_platform_link_options target scope)
    set(options ${ARGN})
    set(is_internal_target_genex "$<BOOL:$<TARGET_PROPERTY:_bobui_is_internal_target>>")
...
if ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "AppleClang")
    ...
    set(not_disabled "$<NOT:$<BOOL:$<TARGET_PROPERTY:BOBUI_NO_DISABLE_WARN_DUPLICATE_LIBRARIES>>>")
...
if(WASM AND BOBUI_FEATURE_sse2)
    target_compile_definitions(PlatformCommonInternal INTERFACE BOBUI_COMPILER_SUPPORTS_SSE2)
endif()

# Taken from mkspecs/common/msvc-version.conf and mkspecs/common/msvc-desktop.conf
if (MSVC AND NOT CLANG)
    if (MSVC_VERSION GREATER_EQUAL 1799)
        set_target_properties(PlatformCommonInternal PROPERTIES _bobui_cpp_compiler_frontend_variant
            "${CMAKE_CXX_COMPILER_FRONTEND_VARIANT}")

        # Protect against adding these flags when building bobuibase with MSVC, and bobuiwebengine using
        # clang-cl.
    ...
    bobui_internal_platform_link_options(PlatformCommonInternal INTERFACE
...
bobui_internal_apply_intel_cet_harderning(PlatformCommonInternal)

if(BOBUI_FEATURE_glibc_fortify_source)
    set(is_optimized_build "$<OR:$<NOT:$<CONFIG:Debug>>,$<BOOL:${BOBUI_FEATURE_optimize_debug}>>")
...
if(BOBUI_FEATURE_stack_clash_protection)
    target_compile_options(PlatformCommonInternal INTERFACE -fstack-clash-protection)
endif()

if(BOBUI_FEATURE_libstdcpp_assertions)
    target_compile_definitions(PlatformCommonInternal INTERFACE _GLIBCXX_ASSERTIONS)
endif()

if(BOBUI_FEATURE_libcpp_hardening)
    string(JOIN "" hardening_flags
        "$<$<NOT:$<STREQUAL:"
            "$<TARGET_PROPERTY:"
                "${BOBUI_CMAKE_EXPORT_NAMESPACE}::PlatformCommonInternal,"
                "_bobui_internal_cmake_generator"
            ">,"
            "Xcode"
        ">>:"
...
if(BOBUI_FEATURE_relro_now_linker)
    bobui_internal_platform_link_options(PlatformCommonInternal INTERFACE "-Wl,-z,relro,-z,now")
endif()


if(BOBUI_FEATURE_force_asserts)
    target_compile_definitions(PlatformCommonInternal INTERFACE BOBUI_FORCE_ASSERTS)
endif()

if(DEFINED BOBUI_EXTRA_DEFINES)
    target_compile_definitions(PlatformCommonInternal INTERFACE ${BOBUI_EXTRA_DEFINES})
endif()

if(DEFINED BOBUI_EXTRA_INCLUDEPATHS)
    target_include_directories(PlatformCommonInternal INTERFACE ${BOBUI_EXTRA_INCLUDEPATHS})
endif()

if(DEFINED BOBUI_EXTRA_LIBDIRS)
    target_link_directories(PlatformCommonInternal INTERFACE ${BOBUI_EXTRA_LIBDIRS})
endif()

if(DEFINED BOBUI_EXTRA_FRAMEWORKPATHS AND APPLE)
    list(TRANSFORM BOBUI_EXTRA_FRAMEWORKPATHS PREPEND "-F" OUTPUT_VARIABLE __bobui_fw_flags)
    target_compile_options(PlatformCommonInternal INTERFACE ${__bobui_fw_flags})
    bobui_internal_platform_link_options(PlatformCommonInternal INTERFACE ${__bobui_fw_flags})
    unset(__bobui_fw_flags)
endif()

function(bobui_internal_add_exceptions_flags)
    set(enable_defs "")
    if(MSVC)
    ...
    set(disable_defs "BOBUI_NO_EXCEPTIONS")
    if(MSVC)
    ...
    string(JOIN "" check_default_genex
        "$<OR:"
            "$<STREQUAL:$<TARGET_PROPERTY:_bobui_internal_use_exceptions>,DEFAULT>,"
            "$<STREQUAL:$<TARGET_PROPERTY:_bobui_internal_use_exceptions>,>"
        ">"
    )
    ...
    string(JOIN "" flag_genex
        "$<IF:"
            ...
            "${default_flag},"
            "$<IF:"
                "$<BOOL:$<TARGET_PROPERTY:_bobui_internal_use_exceptions>>,"
                "${enable_flag},"
                "${disable_flag}"
            ">"
        ">"
...
bobui_internal_add_exceptions_flags()

bobui_internal_get_active_linker_flags(__bobui_internal_active_linker_flags)
if(__bobui_internal_active_linker_flags)
    bobui_internal_platform_link_options(PlatformCommonInternal INTERFACE
        "${__bobui_internal_active_linker_flags}")
endif()
unset(__bobui_internal_active_linker_flags)

if(BOBUI_FEATURE_enable_gdb_index)
    bobui_internal_platform_link_options(PlatformCommonInternal INTERFACE "-Wl,--gdb-index")
endif()

if(BOBUI_FEATURE_enable_new_dtags)
    bobui_internal_platform_link_options(PlatformCommonInternal INTERFACE "-Wl,--enable-new-dtags")
endif()

function(bobui_internal_apply_coverage_flags)
    if(BOBUI_FEATURE_coverage_gcov)
        target_compile_options(PlatformCommonInternal INTERFACE
            "$<$<CONFIG:Debug>:--coverage>")
        target_link_options(PlatformCommonInternal INTERFACE "$<$<CONFIG:Debug>:--coverage>")
    endif()
endfunction()
bobui_internal_apply_coverage_flags()

function(bobui_get_implicit_sse2_genex_condition out_var)
    set(is_shared_lib "$<STREQUAL:$<TARGET_PROPERTY:TYPE>,SHARED_LIBRARY>")
    set(is_static_lib "$<STREQUAL:$<TARGET_PROPERTY:TYPE>,STATIC_LIBRARY>")
    set(is_static_bobui_build "$<NOT:$<BOOL:${BOBUI_BUILD_SHARED_LIBS}>>")
    set(is_static_lib_during_static_bobui_build "$<AND:${is_static_bobui_build},${is_static_lib}>")
    set(enable_sse2_condition "$<OR:${is_shared_lib},${is_static_lib_during_static_bobui_build}>")
    set(${out_var} "${enable_sse2_condition}" PARENT_SCOPE)
endfunction()

function(bobui_auto_detect_implicit_sse2)
    # sse2 configuration adjustment in bobui_module.prf
    # If the compiler supports SSE2, enable it unconditionally in all of BobUI shared libraries
    ...
    set(__implicit_sse2_for_bobui_modules_enabled FALSE PARENT_SCOPE)
    if(TEST_subarch_sse2 AND NOT TEST_arch_${TEST_architecture_arch}_subarch_sse2)
        bobui_get_implicit_sse2_genex_condition(enable_sse2_condition)
        set(enable_sse2_genex "$<${enable_sse2_condition}:${BOBUI_CFLAGS_SSE2}>")
        target_compile_options(PlatformModuleInternal INTERFACE ${enable_sse2_genex})
        set(__implicit_sse2_for_bobui_modules_enabled TRUE PARENT_SCOPE)
    endif()
endfunction()
bobui_auto_detect_implicit_sse2()

function(bobui_auto_detect_fpmath)
    # fpmath configuration adjustment in bobui_module.prf
    set(fpmath_supported FALSE)
    if ("${CMAKE_CXX_COMPILER_ID}" MATCHES "Clang|GNU|IntelLLVM")
        set(fpmath_supported TRUE)
    endif()
    if(fpmath_supported AND TEST_architecture_arch STREQUAL "i386" AND __implicit_sse2_for_bobui_modules_enabled)
        bobui_get_implicit_sse2_genex_condition(enable_sse2_condition)
...
bobui_auto_detect_fpmath()

function(bobui_handle_apple_app_extension_api_only)
    if(APPLE)
        # Build BobUI libraries with -fapplication-extension. Needed to avoid linker warnings
        # transformed into errors on darwin platforms.
        set(flags "-fapplication-extension")

        # The flags should only be applied to internal BobUI libraries like modules and plugins.
        ...
        # to internal BobUI libraries that are marked with the property.
        set(not_disabled "$<NOT:$<BOOL:$<TARGET_PROPERTY:BOBUI_NO_APP_EXTENSION_ONLY_API>>>")
        set(is_bobui_internal_library "$<BOOL:$<TARGET_PROPERTY:_bobui_is_internal_library>>")

        set(condition "$<AND:${not_disabled},${is_bobui_internal_library}>")
...
bobui_handle_apple_app_extension_api_only()
