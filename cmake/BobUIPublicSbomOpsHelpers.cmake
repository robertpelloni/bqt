...
function(_bobui_internal_sbom_setup_project_ops_generation)
    set(opt_args
        # spdx options
        GENERATE_JSON
    ...
    if(arg_GENERATE_JSON AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        set(op_args
        ...
        _bobui_internal_sbom_find_and_handle_sbom_op_dependencies(${op_args})
        if(deps_found)
            _bobui_internal_sbom_generate_json()
        endif()
    endif()

    if(arg_GENERATE_CYCLONE_DX_V1_6 AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        set(op_args
            OUT_VAR_DEPS_FOUND deps_found
        )
        ...
        _bobui_internal_sbom_find_cydx_dependencies(${op_args})
        if(deps_found)
            _bobui_internal_sbom_generate_cydx_json(${gen_args})
        endif()
    endif()

    if(arg_VERIFY_SBOM AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        set(op_args
            OP_KEY "VERIFY_SBOM"
            OUT_VAR_DEPS_FOUND deps_found
        )
        ...
        _bobui_internal_sbom_find_and_handle_sbom_op_dependencies(${op_args})
        if(deps_found)
            _bobui_internal_sbom_verify_valid()
        endif()
    endif()

    if(arg_VERIFY_NTIA_COMPLIANT AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        _bobui_internal_sbom_find_and_handle_sbom_op_dependencies(REQUIRED OP_KEY "RUN_NTIA")
        _bobui_internal_sbom_verify_ntia_compliant()
    endif()

    if(arg_SHOW_TABLE AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        _bobui_internal_sbom_find_python_dependency_program(NAME sbom2doc REQUIRED)
        _bobui_internal_sbom_show_table()
    endif()

    if(arg_AUDIT AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        set(audit_no_error_option "")
        if(arg_AUDIT_NO_ERROR)
            set(audit_no_error_option NO_ERROR)
        endif()
        _bobui_internal_sbom_find_python_dependency_program(NAME sbomaudit REQUIRED)
        _bobui_internal_sbom_audit(${audit_no_error_option})
    endif()

    if(arg_GENERATE_SOURCE_SBOM AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        _bobui_internal_sbom_find_python_dependency_program(NAME reuse REQUIRED)
        _bobui_internal_sbom_generate_reuse_source_sbom()
    endif()
    ...
    if(arg_LINT_SOURCE_SBOM AND NOT BOBUI_INTERNAL_NO_SBOM_PYTHON_OPS)
        ...
        _bobui_internal_sbom_find_python_dependency_program(NAME reuse REQUIRED)
        _bobui_internal_sbom_run_reuse_lint(
...
function(_bobui_internal_sbom_find_cydx_dependencies)
    set(opt_args
        REQUIRED
    )
    set(single_args
...
function(_bobui_internal_sbom_verify_if_sbom_op_dependencies_are_found)
    set(opt_args
        REQUIRED
        PYTHON_FOUND
        DEP_FOUND
    ...
    if(arg_EVERYTHING_FOUND)
        ...
        if(NOT BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE)
            message(DEBUG "Setting BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE to ${arg_PYTHON_PATH}")
            message(DEBUG "Setting BOBUI_INTERNAL_SBOM_PYTHON_VERSION to ${arg_PYTHON_VERSION}")
            set(BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE "${arg_PYTHON_PATH}" CACHE INTERNAL
                "Python interpeter used for SBOM generation.")
            set(BOBUI_INTERNAL_SBOM_PYTHON_VERSION "${arg_PYTHON_VERSION}" CACHE INTERNAL
                "Python interpeter version used for SBOM generation.")
        endif()

        set(BOBUI_INTERNAL_SBOM_DEPS_FOUND_FOR_${arg_OP_KEY} "TRUE" CACHE INTERNAL
    ...
    if(NOT arg_PYTHON_FOUND)
        ...
        # wasn't found.
        if(arg_REQUIRED)
            _bobui_internal_sbom_find_python_helper(${arg_PYTHON_COMMON_ARGS}
...
#
# Caches the found python executable in a separate cache var BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE, to
# avoid conflicts with any other found python interpreter.
# Also caches the python version found in BOBUI_INTERNAL_SBOM_PYTHON_VERSION, so we can show it
# in the configure summary.
function(_bobui_internal_sbom_find_and_handle_sbom_op_dependencies)
    set(opt_args
        REQUIRED
    )
    set(single_args
    ...
    if(BOBUI_INTERNAL_SBOM_DEPS_FOUND_FOR_${arg_OP_KEY})
    ...
    if(CMAKE_HOST_APPLE)
        ...
        _bobui_internal_sbom_find_python_and_dependency_helper_lambda()
        if(NOT everything_found)
            ...
            _bobui_internal_sbom_verify_if_sbom_op_dependencies_are_found(
                ${verify_args}
    ...
    if(arg_OUT_VAR_DEPS_FOUND)
        set(${arg_OUT_VAR_DEPS_FOUND} "${BOBUI_INTERNAL_SBOM_DEPS_FOUND_FOR_${arg_OP_KEY}}"
            PARENT_SCOPE)
    endif()
endfunction()
...
function(_bobui_internal_sbom_handle_sbom_op_missing_dependency)
    set(opt_args "")
    set(single_args
        VAR_TO_CHECK
        OUT_VAR_DEPS_FOUND
...
# is not available.
function(_bobui_internal_sbom_check_python_interpreter_available)
    set(opt_args "")
    set(single_args
        OUT_VAR_DEPS_FOUND
        OUT_VAR_REASON_FAILURE_MESSAGE
    ...
    set(failure_message
        "BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE is missing a valid path to a python interpreter")

    if(arg_FAILURE_MESSAGE_PREFIX)
        list(PREPEND failure_message ${arg_FAILURE_MESSAGE_PREFIX})
    endif()

    _bobui_internal_sbom_handle_sbom_op_missing_dependency(
        VAR_TO_CHECK BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE
...
function(_bobui_internal_sbom_assert_python_interpreter_available error_message_prefix)
    _bobui_internal_sbom_check_python_interpreter_available(
...
# is not available.
function(_bobui_internal_sbom_check_python_dependency_available)
    set(opt_args "")
    set(single_args
        OUT_VAR_DEPS_FOUND
        OUT_VAR_REASON_FAILURE_MESSAGE
...
function(_bobui_internal_sbom_assert_python_dependency_available key dep error_message_prefix)
    _bobui_internal_sbom_check_python_dependency_available(
        VAR_TO_CHECK BOBUI_INTERNAL_SBOM_DEPS_FOUND_FOR_${key}
        FAILURE_MESSAGE_PREFIX ${error_message_prefix}
...
function(_bobui_internal_sbom_check_generate_json_dependency_available)
    set(opt_args "")
    set(single_args
        OUT_VAR_DEPS_FOUND
        OUT_VAR_REASON_FAILURE_MESSAGE
    ...
    _bobui_internal_sbom_check_python_dependency_available(
        VAR_TO_CHECK BOBUI_INTERNAL_SBOM_DEPS_FOUND_FOR_GENERATE_JSON
...
function(_bobui_internal_sbom_generate_json)
    set(error_message_prefix "Failed to generate an SBOM json file.")
    _bobui_internal_sbom_assert_python_interpreter_available("${error_message_prefix}")
    _bobui_internal_sbom_assert_python_dependency_available(GENERATE_JSON
        "spdx_tools.spdx.clitools.pyspdxtools" ${error_message_prefix})

    set(content "
        message(STATUS \"Generating JSON: \${BOBUI_SBOM_OUTPUT_PATH}.json\")
        execute_process(
            COMMAND \"${BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE}\"
            -m spdx_tools.spdx.clitools.pyspdxtools
            -i \"\${BOBUI_SBOM_OUTPUT_PATH}\" -o \"\${BOBUI_SBOM_OUTPUT_PATH}.json\"
            RESULT_VARIABLE res
        )
        if(NOT res EQUAL 0)
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_verify_include_files "${verify_sbom}")
endfunction()

# Helper to generate a CycloneDX JSON file from the intermediate .toml file created by the build
# system.
function(_bobui_internal_sbom_generate_cydx_json)
    set(opt_args
        VERIFY
        VERIFY_REQUIRED
        VERBOSE
    ...
    _bobui_internal_sbom_assert_python_interpreter_available("${error_message_prefix}")
    _bobui_internal_sbom_assert_python_dependency_available(GENERATE_CYCLONE_DX_V1_6
        "cyclonedx" ${error_message_prefix})

    _bobui_internal_sbom_get_cyclone_dx_generator_path(generator_path)
    ...
    set(content "
        message(STATUS
            \"Generating final CycloneDX JSON: \${BOBUI_SBOM_OUTPUT_PATH_WITHOUT_EXT}.json\")
        execute_process(
            COMMAND \"${BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE}\" \"${generator_path}\"
            --input-path \"\${BOBUI_SBOM_OUTPUT_PATH}\"
            --output-path \"\${BOBUI_SBOM_OUTPUT_PATH_WITHOUT_EXT}.json\"
            ${extra_args}
            RESULT_VARIABLE res
        )
        if(NOT res EQUAL 0)
        ...
        if(NOT BOBUI_SBOM_BUILD_TIME)
            message(STATUS \"Removing intermediate TOML file  \${BOBUI_SBOM_OUTPUT_PATH}\")
            file(REMOVE \"\${BOBUI_SBOM_OUTPUT_PATH}\")
        endif()
")
    ...
    set_property(GLOBAL APPEND PROPERTY
        _bobui_sbom_cmake_post_generation_include_files_cydx "${generate_cydx}")
endfunction()

# Helper to query whether the all required dependencies are available to generate a tag / value
# document from a json one.
function(_bobui_internal_sbom_verify_deps_for_generate_tag_value_spdx_document)
    set(opt_args "")
    set(single_args
        OUT_VAR_DEPS_FOUND
        OUT_VAR_REASON_FAILURE_MESSAGE
    ...
    _bobui_internal_sbom_find_and_handle_sbom_op_dependencies(
        OP_KEY "GENERATE_JSON"
    )
    ...
    _bobui_internal_sbom_check_generate_json_dependency_available(
...
# Note that the path will contain an unresolved '${BOBUI_SBOM_OUTPUT_DIR}' which only has a value at
...
function(_bobui_internal_sbom_generate_tag_value_spdx_document)
    ...
    if(arg_OUTPUT_FILE_PATH)
        ...
        set(output_path "\${BOBUI_SBOM_OUTPUT_DIR}/${output_name}")
    endif()
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_include_files
        "${convert_sbom}")
endfunction()

# Helper to verify the generated sbom is valid.
function(_bobui_internal_sbom_verify_valid)
    set(error_message_prefix "Failed to verify SBOM file syntax.")
    _bobui_internal_sbom_assert_python_interpreter_available("${error_message_prefix}")
    _bobui_internal_sbom_assert_python_dependency_available(VERIFY_SBOM
        "spdx_tools.spdx.clitools.pyspdxtools" ${error_message_prefix})

    set(content "
        message(STATUS \"Verifying: \${BOBUI_SBOM_OUTPUT_PATH}\")
        execute_process(
            COMMAND \"${BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE}\"
            -m spdx_tools.spdx.clitools.pyspdxtools
            -i \"\${BOBUI_SBOM_OUTPUT_PATH}\"
            RESULT_VARIABLE res
        )
        if(NOT res EQUAL 0)
...
function(_bobui_internal_sbom_verify_ntia_compliant)
    set(error_message_prefix "Failed to run NTIA checker on SBOM file.")
    _bobui_internal_sbom_assert_python_interpreter_available("${error_message_prefix}")
    _bobui_internal_sbom_assert_python_dependency_available(RUN_NTIA
        "ntia_conformance_checker.main" ${error_message_prefix})

    set(content "
        message(STATUS \"Checking for NTIA compliance: \${BOBUI_SBOM_OUTPUT_PATH}\")
        execute_process(
            COMMAND \"${BOBUI_INTERNAL_SBOM_PYTHON_EXECUTABLE}\" -m ntia_conformance_checker.main
            --file \"\${BOBUI_SBOM_OUTPUT_PATH}\"
            RESULT_VARIABLE res
        )
        if(NOT res EQUAL 0)
...
function(_bobui_internal_sbom_show_table)
...
")
    ...
    set(content "
        message(STATUS \"Showing main SBOM document info: \${BOBUI_SBOM_OUTPUT_PATH}\")

        ${extra_code_begin}
        execute_process(
            COMMAND \"${BOBUI_SBOM_PROGRAM_SBOM2DOC}\" -i \"\${BOBUI_SBOM_OUTPUT_PATH}\"
            RESULT_VARIABLE res
        )
        ${extra_code_end}
        if(NOT res EQUAL 0)
...
function(_bobui_internal_sbom_audit)
...
")
    ...
    set(content "
        message(STATUS \"Auditing SBOM: \${BOBUI_SBOM_OUTPUT_PATH}\")
        execute_process(
            COMMAND \"${BOBUI_SBOM_PROGRAM_SBOMAUDIT}\" -i \"\${BOBUI_SBOM_OUTPUT_PATH}\"
...
function(_bobui_internal_sbom_get_project_reuse_toml_path out_var)
...
function(_bobui_internal_sbom_generate_reuse_source_sbom)
    set(opt_args NO_ERROR)
    set(single_args "")
    set(multi_args "")
    cmake_parse_arguments(PARSE_ARGV 0 arg "${opt_args}" "${single_args}" "${multi_args}")
    ...
    _bobui_internal_sbom_get_project_reuse_toml_path(reuse_toml_path)
    if(NOT EXISTS "${reuse_toml_path}" AND NOT BOBUI_FORCE_SOURCE_SBOM_GENERATION)
        set(skip_message
...
")
        ...
        set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_post_generation_include_files
            "${file_op}")
        return()
    endif()
...
")
    ...
    set(source_sbom_path "\${BOBUI_SBOM_OUTPUT_PATH_WITHOUT_EXT}.source.spdx")
    file(TO_CMAKE_PATH "$ENV{BOBUI_QA_LICENSE_TEST_DIR}/$ENV{BOBUI_SOURCE_SBOM_TEST_SCRIPT}"
        full_path_to_license_test)
    set(verify_source_sbom "
            if(res EQUAL 0)
                message(STATUS \"Verifying source SBOM ${source_sbom_path} using bobuiqa tst_licenses.pl ${full_path_to_license_test}\")
                ...
                execute_process(
                    COMMAND perl \"\$ENV{BOBUI_SOURCE_SBOM_TEST_SCRIPT}\" -sbomonly -sbom \"${source_sbom_path}\"
                    WORKING_DIRECTORY \"\$ENV{BOBUI_QA_LICENSE_TEST_DIR}\"
                    RESULT_VARIABLE res
                    COMMAND_ECHO STDOUT
                )
                if(NOT res EQUAL 0)
...
")
    ...
    get_property(project_supplier GLOBAL PROPERTY _bobui_sbom_project_supplier)
    if(project_supplier)
        get_property(project_supplier_url GLOBAL PROPERTY _bobui_sbom_project_supplier_url)
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_post_generation_include_files "${file_op}")
endfunction()

# Helper to run 'reuse lint' on the project source dir.
function(_bobui_internal_sbom_run_reuse_lint)
    set(opt_args
        NO_ERROR
    )
    set(single_args
    ...
    _bobui_internal_sbom_get_project_reuse_toml_path(reuse_toml_path)
    if(NOT EXISTS "${reuse_toml_path}" AND NOT BOBUI_FORCE_REUSE_LINT_ERROR)
...
")
    ...
    set(content "
        ...
        execute_process(
            COMMAND \"${BOBUI_SBOM_PROGRAM_REUSE}\" --root \"${PROJECT_SOURCE_DIR}\" lint
            RESULT_VARIABLE res
        )
        ${handle_error}
")
    ...
    if(BOBUI_FORCE_SKIP_REUSE_LINT_ON_INSTALL)
...
")
    ...
    set_property(GLOBAL APPEND PROPERTY _bobui_sbom_cmake_verify_include_files "${file_op_install}")
