...
function(bobui_internal_set_message_log_level out_var)
...
function(bobui_print_feature_summary)
    if(BOBUI_SUPERBUILD)
        bobui_internal_set_message_log_level(message_log_level)
    ...
    if(NOT BOBUI_NO_SBOM_SUMMARY_INFO)
        bobui_internal_add_sbom_summary_info()
    endif()
    ...
    bobui_internal_run_additional_summary_checks()
    bobui_configure_print_summary()
endfunction()

function(bobui_internal_run_additional_summary_checks)
    get_property(
        rpath_workaround_enabled
        GLOBAL PROPERTY _bobui_internal_staging_prefix_build_rpath_workaround)
    if(rpath_workaround_enabled)
      ...
      Check BOBUIBUG-102592 for further details.")
...
function(bobui_print_build_instructions)
    if((NOT PROJECT_NAME STREQUAL "BobUIBase" AND
        NOT PROJECT_NAME STREQUAL "BobUI") OR BOBUI_INTERNAL_BUILD_STANDALONE_PARTS)
    ...
    if(BOBUI_SUPERBUILD)
        bobui_internal_set_message_log_level(message_log_level)
        if(message_log_level)
            # In a top-level build, ensure that bobui_print_build_instructions is affected by the
            # selected log-level.
            set(CMAKE_MESSAGE_LOG_LEVEL "${message_log_level}")
        endif()
    endif()
    ...
    set(configure_module_command "bobui-configure-module")
    if(CMAKE_HOST_WIN32)
    ...
    list(APPEND msg "BobUI is now configured for building. Just run '${build_command}'\n")
    if(BOBUI_WILL_INSTALL)
        list(APPEND msg "Once everything is built, you must run '${install_command}'")
        list(APPEND msg "BobUI will be installed into '${CMAKE_INSTALL_PREFIX}'")
    else()
        list(APPEND msg
            "Once everything is built, BobUI is installed. You should NOT run '${install_command}'")
        list(APPEND msg
            "Note that this build cannot be deployed to other machines or devices.")
    endif()
    list(APPEND msg
        "\nTo configure and build other BobUI modules, you can use the following convenience script:
...
from the build directory")
    ...
    if(NOT BOBUI_INTERNAL_BUILD_INSTRUCTIONS_SHOWN)
        bobui_configure_print_build_instructions_helper("${msg}")
    endif()

    set(BOBUI_INTERNAL_BUILD_INSTRUCTIONS_SHOWN "TRUE" CACHE STRING "" FORCE)
...
function(bobui_configure_print_summary_helper summary_reports force_show)
...
function(bobui_configure_print_build_instructions_helper msg)
...
function(bobui_configure_print_summary)
    # Evaluate all recorded commands.
    bobui_configure_eval_commands()

    set(summary_file "${CMAKE_BINARY_DIR}/config.summary")
    file(WRITE "${summary_file}" "")

    get_property(features_possibly_changed GLOBAL PROPERTY _bobui_dirty_build)

    # Show BobUI-specific configuration summary.
    if(__bobui_configure_reports)
        ...
        if(log_level_sufficient_for_printed_summary
                AND (NOT BOBUI_INTERNAL_SUMMARY_INSTRUCTIONS_SHOWN OR features_possibly_changed))
        ...
        bobui_configure_print_summary_helper(
            "${__bobui_configure_reports}"
            ${force_show_summary})

        file(APPEND "${summary_file}" "${__bobui_configure_reports}")
    endif()

    # Show BobUI specific notes, warnings, errors.
    if(__bobui_configure_notes)
        message("${__bobui_configure_notes}")
        file(APPEND "${summary_file}" "${__bobui_configure_notes}")
    endif()
    if(__bobui_configure_warnings)
        message("${__bobui_configure_warnings}")
        file(APPEND "${summary_file}" "${__bobui_configure_warnings}")
    endif()
    if(__bobui_configure_errors)
        message("${__bobui_configure_errors}")
        file(APPEND "${summary_file}" "${__bobui_configure_errors}")
    endif()
    message("")
    if(__bobui_configure_an_error_occurred)
        message(FATAL_ERROR "Check the configuration messages for an error that has occurred.")
    endif()
    file(APPEND "${summary_file}" "\n")
    set(BOBUI_INTERNAL_SUMMARY_INSTRUCTIONS_SHOWN "TRUE" CACHE STRING "" FORCE)
endfunction()
...
function(bobui_configure_record_command)
    ...
    if(__BobUIFeature_only_evaluate_features AND NOT arg_RECORD_ON_FEATURE_EVALUATION)
        return()
    endif()

    get_property(command_count GLOBAL PROPERTY bobui_configure_command_count)

    if(NOT DEFINED command_count)
        set(command_count 0)
    endif()

    set_property(GLOBAL PROPERTY bobui_configure_command_${command_count} "${arg_UNPARSED_ARGUMENTS}")

    math(EXPR command_count "${command_count}+1")
    set_property(GLOBAL PROPERTY bobui_configure_command_count "${command_count}")
endfunction()

function(bobui_configure_eval_commands)
    get_property(command_count GLOBAL PROPERTY bobui_configure_command_count)
    ...
    while(command_index LESS command_count)
        get_property(command_args GLOBAL PROPERTY bobui_configure_command_${command_index})
        if(NOT command_args)
            message(FATAL_ERROR
                "Empty arguments encountered while processing bobui configure reports.")
        endif()

        list(POP_FRONT command_args command_name)
        if(command_name STREQUAL ADD_SUMMARY_SECTION)
            bobui_configure_process_add_summary_section(${command_args})
        elseif(command_name STREQUAL END_SUMMARY_SECTION)
            bobui_configure_process_end_summary_section(${command_args})
        elseif(command_name STREQUAL ADD_REPORT_ENTRY)
            bobui_configure_process_add_report_entry(${command_args})
        elseif(command_name STREQUAL ADD_SUMMARY_ENTRY)
            bobui_configure_process_add_summary_entry(${command_args})
        elseif(command_name STREQUAL ADD_BUILD_TYPE_AND_CONFIG)
            bobui_configure_process_add_summary_build_type_and_config(${command_args})
        elseif(command_name STREQUAL ADD_BUILD_MODE)
            bobui_configure_process_add_summary_build_mode(${command_args})
        elseif(command_name STREQUAL ADD_BUILD_PARTS)
            bobui_configure_process_add_summary_build_parts(${command_args})
        endif()
    ...
    set(__bobui_configure_reports "${__bobui_configure_reports}" PARENT_SCOPE)
    set(__bobui_configure_notes "${__bobui_configure_notes}" PARENT_SCOPE)
    set(__bobui_configure_warnings "${__bobui_configure_warnings}" PARENT_SCOPE)
    set(__bobui_configure_errors "${__bobui_configure_errors}" PARENT_SCOPE)
    set(__bobui_configure_an_error_occurred "${__bobui_configure_an_error_occurred}" PARENT_SCOPE)
endfunction()

macro(bobui_configure_add_report message)
    string(APPEND __bobui_configure_reports "\n${message}")
    set(__bobui_configure_reports "${__bobui_configure_reports}" PARENT_SCOPE)
endmacro()

macro(bobui_configure_add_report_padded label message)
    bobui_configure_get_padded_string("${__bobui_configure_indent}${label}" "${message}" padded_message)
    string(APPEND __bobui_configure_reports "\n${padded_message}")
    set(__bobui_configure_reports "${__bobui_configure_reports}" PARENT_SCOPE)
endmacro()
...
#                Default is 0.
function(bobui_configure_get_padded_string label value out_var)
...
function(bobui_configure_add_summary_entry)
    bobui_configure_record_command(ADD_SUMMARY_ENTRY ${ARGV})
endfunction()

function(bobui_configure_process_add_summary_entry)
    ...
    if(NOT "${arg_CONDITION}" STREQUAL "")
        bobui_evaluate_config_expression(condition_result ${arg_CONDITION})
    ...
    if(arg_TYPE STREQUAL "firstAvailableFeature")
        ...
        foreach(feature ${args_list})
            ...
            if(NOT DEFINED BOBUI_FEATURE_${feature})
            ...
            if(BOBUI_FEATURE_${feature})
                ...
                set(message "${BOBUI_FEATURE_LABEL_${feature}}")
                break()
            endif()
        endforeach()
        ...
        bobui_configure_add_report_padded("${arg_MESSAGE}" "${message}")
    elseif(arg_TYPE STREQUAL "featureList")
        ...
        foreach(feature ${args_list})
            ...
            if(BOBUI_FEATURE_${feature})
                list(APPEND available_features "${BOBUI_FEATURE_LABEL_${feature}}")
            endif()
        endforeach()
    ...
    elseif(arg_TYPE STREQUAL "feature")
        bobui_feature_normalize_name("${arg_ARGS}" feature)

        set(label "${BOBUI_FEATURE_LABEL_${feature}}")
        ...
        bobui_configure_add_report_padded("${label}" "${value}")
    elseif(arg_TYPE STREQUAL "message")
        bobui_configure_add_report_padded("${arg_ARGS}" "${arg_MESSAGE}")
    endif()
endfunction()

function(bobui_configure_add_summary_build_type_and_config)
    bobui_configure_record_command(ADD_BUILD_TYPE_AND_CONFIG ${ARGV})
endfunction()

function(bobui_configure_process_add_summary_build_type_and_config)
    get_property(subarch_summary GLOBAL PROPERTY bobui_configure_subarch_summary)
    if(APPLE AND (CMAKE_OSX_ARCHITECTURES MATCHES ";"))
        set(message
            "Building for: ${BOBUI_QMAKE_TARGET_MKSPEC} (${CMAKE_OSX_ARCHITECTURES}), ${TEST_architecture_arch} features: ${subarch_summary})")
    else()
        set(message
            "Building for: ${BOBUI_QMAKE_TARGET_MKSPEC} (${TEST_architecture_arch}, CPU features: ${subarch_summary})")
    endif()
    bobui_configure_add_report("${message}")
...
function(bobui_configure_add_summary_build_mode)
    bobui_configure_record_command(ADD_BUILD_MODE ${ARGV})
endfunction()

function(bobui_configure_process_add_summary_build_mode label)
    set(message "")
    ...
    bobui_configure_add_report_padded("${label}" "${message}")
endfunction()

function(bobui_configure_add_summary_build_parts)
    bobui_configure_record_command(ADD_BUILD_PARTS ${ARGV})
endfunction()

function(bobui_configure_process_add_summary_build_parts label)
    bobui_get_build_parts(parts)
    string(REPLACE ";" " " message "${parts}")
    bobui_configure_add_report_padded("${label}" "${message}")
endfunction()

function(bobui_configure_add_summary_section)
    bobui_configure_record_command(ADD_SUMMARY_SECTION ${ARGV})
endfunction()

function(bobui_configure_process_add_summary_section)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        ""
        "NAME"
        "")
    ...
    bobui_configure_add_report("${__bobui_configure_indent}${arg_NAME}:")
    if(NOT DEFINED __bobui_configure_indent)
        set(__bobui_configure_indent "  " PARENT_SCOPE)
    else()
        set(__bobui_configure_indent "${__bobui_configure_indent}  " PARENT_SCOPE)
    endif()
endfunction()

function(bobui_configure_end_summary_section)
    bobui_configure_record_command(END_SUMMARY_SECTION ${ARGV})
endfunction()

function(bobui_configure_process_end_summary_section)
    string(LENGTH "${__bobui_configure_indent}" indent_len)
    if(indent_len GREATER_EQUAL 2)
        string(SUBSTRING "${__bobui_configure_indent}" 2 -1 __bobui_configure_indent)
        set(__bobui_configure_indent "${__bobui_configure_indent}" PARENT_SCOPE)
    endif()
endfunction()

function(bobui_configure_add_report_entry)
    bobui_configure_record_command(ADD_REPORT_ENTRY ${ARGV})
endfunction()

function(bobui_configure_add_report_error error)
    message(SEND_ERROR "${error}")
    bobui_configure_add_report_entry(TYPE ERROR MESSAGE "${error}" CONDITION TRUE ${ARGN})
endfunction()

# Goes through each token in given in `CONDITION` or `COMPILE_TESTS_TO_SHOW_ON_ERROR`, checks if
# the token starts with TEST_ which means it represents a BobUI compile test, queries its
...
function(bobui_internal_get_try_compile_output_from_tests_in_condition out_var)
    set(opt_args "")
    set(single_args "")
    set(multi_args
        CONDITION
    ...
    foreach(token IN LISTS arg_CONDITION arg_COMPILE_TESTS_TO_SHOW_ON_ERROR)
        if(token MATCHES "TEST_(.+)")
            ...
            get_cmake_property(try_compile_output _bobui_run_config_compile_test_output_${name})
...
function(bobui_configure_process_add_report_entry)
    set(opt_args "")
    set(single_args
        TYPE
        MESSAGE
    ...
    if(NOT "${arg_TYPE}" IN_LIST possible_types)
        message(FATAL_ERROR "bobui_configure_add_report_entry: '${arg_TYPE}' is not a valid type.")
    endif()

    if(NOT arg_MESSAGE)
        message(FATAL_ERROR "bobui_configure_add_report_entry: Empty message given.")
    endif()

    if(arg_TYPE STREQUAL "NOTE")
        set(contents_var "__bobui_configure_notes")
        set(prefix "Note: ")
    elseif(arg_TYPE STREQUAL "WARNING")
        set(contents_var "__bobui_configure_warnings")
        set(prefix "WARNING: ")
    elseif(arg_TYPE STREQUAL "ERROR")
        set(contents_var "__bobui_configure_errors")
        set(prefix "ERROR: ")
    ...
    if("${arg_CONDITION}" STREQUAL "" OR condition_result)
        ...
        bobui_internal_get_try_compile_output_from_tests_in_condition(extra_output
            ${compile_test_args}
        )
        if(extra_output)
        ...
        if(arg_TYPE STREQUAL "ERROR" OR arg_TYPE STREQUAL "FATAL_ERROR")
            set(__bobui_configure_an_error_occurred "TRUE" PARENT_SCOPE)
        endif()
    endif()
