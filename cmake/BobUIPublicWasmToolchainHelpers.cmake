...
function(__bobui_internal_get_emroot_path_suffix_from_emsdk_env out_var)
...
function(__bobui_internal_get_emscripten_cmake_toolchain_file_path_from_emsdk_env emroot_path out_var)
...
function(__bobui_internal_query_emsdk_version emroot_path is_fatal out_var)
...
function(__bobui_internal_get_emcc_recommended_version out_var)
    # This version of BobUI needs this version of emscripten.
    set(BOBUI_EMCC_RECOMMENDED_VERSION "4.0.7")
    set(${out_var} "${BOBUI_EMCC_RECOMMENDED_VERSION}" PARENT_SCOPE)
endfunction()

function(__bobui_internal_show_error_no_emscripten_toolchain_file_found_when_building_bobui)
    message(FATAL_ERROR
...
function(__bobui_internal_show_error_no_emscripten_toolchain_file_found_when_using_bobui)
    message(FATAL_ERROR
        "Cannot find the toolchain file Emscripten.cmake. "
...
function(__bobui_internal_get_bobui_build_emsdk_version out_var)
    if(BOBUI6_INSTALL_PREFIX)
        set(WASM_BUILD_DIR "${BOBUI6_INSTALL_PREFIX}")
    elseif(BOBUI_BUILD_DIR)
        set(WASM_BUILD_DIR "${BOBUI_BUILD_DIR}")
    endif()
    if(EXISTS "${WASM_BUILD_DIR}/src/corelib/global/qconfig.h")
        file(READ "${WASM_BUILD_DIR}/src/corelib/global/qconfig.h" ver)
    elseif(EXISTS "${WASM_BUILD_DIR}/include/BobUICore/qconfig.h")
        file(READ "${WASM_BUILD_DIR}/include/BobUICore/qconfig.h" ver)
    endif()
    if (ver)
        string(REGEX MATCH "#define BOBUI_EMCC_VERSION.\"[0-9]+\\.[0-9]+\\.[0-9]+\"" emOutput ${ver})
        string(REGEX MATCH "[0-9]+\\.[0-9]+\\.[0-9]+" build_emcc_version "${emOutput}")
        set(${out_var} "${build_emcc_version}" PARENT_SCOPE)
    endif()
endfunction()

function(_bobui_test_emscripten_version)
    __bobui_internal_get_emcc_recommended_version(_recommended_emver)
    __bobui_internal_get_emroot_path_suffix_from_emsdk_env(emroot_path)
    __bobui_internal_query_emsdk_version("${emroot_path}" TRUE current_emsdk_ver)
    __bobui_internal_get_bobui_build_emsdk_version(bobui_build_emcc_version)

    if(NOT "${bobui_build_emcc_version}" STREQUAL "" AND NOT "${bobui_build_emcc_version}" STREQUAL "${current_emsdk_ver}")
        message(FATAL_ERROR
            "BobUI Wasm was built with Emscripten version: ${bobui_build_emcc_version}\n"
            "You are using Emscripten version: ${current_emsdk_ver}\n"
            "The recommended version of Emscripten for this BobUI is: ${_recommended_emver}\n"
