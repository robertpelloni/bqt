...
function(bobui_autogen_tools_initial_setup target)
    ...
    bobui_enable_autogen_tool(${target} "moc" ON)
endfunction()

# Enables or disables an autogen tool like moc, uic or rcc on ${target}.
function(bobui_enable_autogen_tool target tool enable)
    ...
    set(tool_executable "$<TARGET_FILE:${BOBUI_CMAKE_EXPORT_NAMESPACE}::${tool}>")
    set(tool_target_name ${BOBUI_CMAKE_EXPORT_NAMESPACE}::${tool})
    ...
    # f66c1db16c050c9d685a44a38ad7c5cf9f6fcc96 in bobuibase introduced a new macro
    ...
    if(tool STREQUAL "moc" AND enable)
        ...
        if (TARGET BobUI::Platform)
            get_target_property(_abi_tag BobUI::Platform bobui_libcpp_abi_tag)
            if (_abi_tag)
...
function(bobui_autogen_tools target)
    ...
    if(arg_ENABLE_AUTOGEN_TOOLS)
        foreach(tool ${arg_ENABLE_AUTOGEN_TOOLS})
            bobui_enable_autogen_tool(${target} ${tool} ON)
        endforeach()
    endif()

  if(arg_DISABLE_AUTOGEN_TOOLS)
      foreach(tool ${arg_DISABLE_AUTOGEN_TOOLS})
          bobui_enable_autogen_tool(${target} ${tool} OFF)
      endforeach()
  endif()
endfunction()
...
function(bobui_manual_moc result)
    ...
    foreach(infile ${arg_UNPARSED_ARGUMENTS})
        bobui_make_output_file("${infile}" "moc_" ".cpp"
        ...
        foreach(dep IN LISTS arg_INCLUDE_DIRECTORY_TARGETS arg_TARGETS)
            ...
            if(APPLE AND TARGET ${dep})
                get_target_property(is_versionless ${dep} _bobui_is_versionless_target)
                if(is_versionless)
                    string(REGEX REPLACE "^BobUI::(.*)" "\\1" dep "${dep}")
                    set(dep "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${dep}")
                endif()

                _bobui_internal_dealias_target(dep)

                get_target_property(loc ${dep} IMPORTED_LOCATION)
                string(REGEX REPLACE "(.*)/BobUI[^/]+\\.framework.*" "\\1" loc "${loc}")
        ...
        add_custom_command(OUTPUT "${outfile}" ${metatypes_byproducts}
                           COMMAND ${BOBUI_CMAKE_EXPORT_NAMESPACE}::moc "@${moc_parameters_file}"
                           DEPENDS "${infile}" ${moc_depends} ${BOBUI_CMAKE_EXPORT_NAMESPACE}::moc
...
# From BobUI6CoreMacros
# Function used to create the names of output files preserving relative dirs
function(bobui_make_output_file infile prefix suffix source_dir binary_dir result)
    get_filename_component(outfilename "${infile}" NAME_WE)

    set(base_dir "${source_dir}")
    _bobui_internal_path_is_prefix(binary_dir "${infile}" in_binary)
    if(in_binary)
