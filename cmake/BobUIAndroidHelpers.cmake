...
#
# Android specific functions/macros/properties required for building BobUI Modules
#

macro(bobui_internal_setup_android_target_properties)
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_MODULE_INSTALL_DIR
        BRIEF_DOCS
            "Recorded install location for a BobUI Module."
        FULL_DOCS
            "Recorded install location for a BobUI Module. Used by bobui_internal_android_dependencies()."
    )

    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_JAR_DEPENDENCIES
        BRIEF_DOCS
            "BobUI Module Jar dependencies list."
        FULL_DOCS
            "BobUI Module Jar dependencies list."
    )
    ...
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_BUNDLED_JAR_DEPENDENCIES
        BRIEF_DOCS
            "BobUI Module Jars that should be bundled with it during packing."
        FULL_DOCS
            "BobUI Module Jars that should be bundled with it during packing."
    )
    ...
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_LIB_DEPENDENCIES
        BRIEF_DOCS
            "BobUI Module C++ libraries that should be bundled with it during packing."
        FULL_DOCS
            "BobUI Module C++ libraries that should be bundled with it during packing."
    )
    ...
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_LIB_DEPENDENCY_REPLACEMENTS
        BRIEF_DOCS
            "BobUI Module C++ libraries that can replace libraries declared with the BOBUI_ANDROID_LIB_DEPENDENCIES property."
        FULL_DOCS
            "BobUI Module C++ libraries that can replace libraries declared with the BOBUI_ANDROID_LIB_DEPENDENCIES property."
    )
    ...
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_BUNDLED_FILES
        BRIEF_DOCS
            "BobUI Module files that need to be bundled during packing."
        FULL_DOCS
            "BobUI Module files that need to be bundled during packing."
    )
    ...
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_PERMISSIONS
        BRIEF_DOCS
            "BobUI Module android permission list."
        FULL_DOCS
            "BobUI Module android permission list."
    )
    ...
    define_property(TARGET
        PROPERTY
            BOBUI_ANDROID_FEATURES
        BRIEF_DOCS
            "BobUI Module android feature list."
        FULL_DOCS
            "BobUI Module android feature list."
    )
...
function(bobui_internal_locate_bobui_android_base_jar out_var)

    set(datadir "${INSTALL_DATADIR}")
    if(NOT DEFINED datadir OR datadir STREQUAL "")
        set(datadir ".")
    endif()
    set(bobui_ns "${BOBUI_CMAKE_EXPORT_NAMESPACE}")
    bobui_path_join(
        jar
        "${BOBUI_TOOLCHAIN_RELOCATABLE_INSTALL_PREFIX}"
        "${datadir}"
        "jar"
        "${bobui_ns}Android.jar"
    )

    # Optional override
    if(DEFINED ENV{BOBUI_ANDROID_JAR_PATH} AND EXISTS "$ENV{BOBUI_ANDROID_JAR_PATH}")
        set(jar "$ENV{BOBUI_ANDROID_JAR_PATH}")
    endif()

    set(${out_var} "${jar}" PARENT_SCOPE)
endfunction()

function(bobui_internal_compute_android_javadoc_classpath out_var)
    ...
    # The target BobUI6Android is already defined = use its build-dir jar.
    # The target may not yet exist in some bobuibase configuration order =
    # still set the jar path in advance.
    set(bobui_ns "${BOBUI_CMAKE_EXPORT_NAMESPACE}")
    if(TARGET ${bobui_ns}Android OR (BOBUI_BUILDING_BOBUI AND PROJECT_NAME STREQUAL "BobUIBase"))
        set(jar "${BOBUI_BUILD_DIR}/jar/${bobui_ns}Android.jar")
        set(${out_var} "${BOBUI_ANDROID_JAR}${sep}${jar}" PARENT_SCOPE)
        return()
    endif()

    # Downstream module: use installed jar from the shared locator
    bobui_internal_locate_bobui_android_base_jar(bobui_classes_jar)
    if(NOT EXISTS "${bobui_classes_jar}")
        message(FATAL_ERROR
            "BobUI Android JAR not found at:\n  ${bobui_classes_jar}")
    endif()

    set(${out_var} "${BOBUI_ANDROID_JAR}${sep}${bobui_classes_jar}" PARENT_SCOPE)
endfunction()

function(bobui_internal_write_android_javadoc_args
                    response_file
                    output_dir
                    source_paths
                    package_names_space_separated)

    bobui_internal_compute_android_javadoc_classpath(class_path)
...
function(bobui_internal_add_android_permission target)
    _bobui_internal_add_android_permission(${ARGV})
endfunction()

function(bobui_internal_android_dependencies_content target file_content_out)
    get_target_property(arg_JAR_DEPENDENCIES ${target} BOBUI_ANDROID_JAR_DEPENDENCIES)
    get_target_property(arg_BUNDLED_JAR_DEPENDENCIES ${target} BOBUI_ANDROID_BUNDLED_JAR_DEPENDENCIES)
    get_target_property(arg_LIB_DEPENDENCIES ${target} BOBUI_ANDROID_LIB_DEPENDENCIES)
    get_target_property(arg_LIB_DEPENDENCY_REPLACEMENTS ${target} BOBUI_ANDROID_LIB_DEPENDENCY_REPLACEMENTS)
    get_target_property(arg_BUNDLED_FILES ${target} BOBUI_ANDROID_BUNDLED_FILES)
    get_target_property(arg_PERMISSIONS ${target} BOBUI_ANDROID_PERMISSIONS)
    get_target_property(arg_FEATURES ${target} BOBUI_ANDROID_FEATURES)
    ...
    # mkspecs/feature/bobui_android_deps.prf
    ...
    # Android Permissions
    _bobui_internal_android_convert_permissions(permissions_string ${target} "DEPENDENCIESXML")
...
# Generate BobUI Module -android-dependencies.xml required by the
# androiddeploytoolbobui to successfully copy all the plugins and other dependent
# items into the APK
function(bobui_internal_android_dependencies target)
    ...
    bobui_internal_android_dependencies_content(${target} file_contents)

    # Get plugins from the module's plugin types and get their dependencies
    foreach(plugin ${BOBUI_KNOWN_PLUGINS})
        get_target_property(iter_known_plugin_type ${plugin} BOBUI_PLUGIN_TYPE)
        foreach(plugin_type ${module_plugin_types})
            if (plugin_type STREQUAL iter_known_plugin_type)
                bobui_internal_android_dependencies_content(${plugin} plugin_file_contents)
    ...
    bobui_path_join(dependency_file "${BOBUI_BUILD_DIR}" "${INSTALL_LIBDIR}" "${target_name}_${CMAKE_ANDROID_ARCH_ABI}-android-dependencies.xml")
    file(WRITE ${dependency_file} ${file_contents})

    get_target_property(target_install_dir ${target} BOBUI_ANDROID_MODULE_INSTALL_DIR)
    if (NOT target_install_dir)
        message(SEND_ERROR "bobui_internal_android_dependencies: Target ${target} is either not a BobUI Module or has no recorded install location")
        return()
    endif()

    # Copy file into install directory, required by the androiddeploybobui tool.
    bobui_install(FILES
        ${dependency_file}
        DESTINATION
            ${target_install_dir}
...
function(bobui_internal_set_up_build_host_java_docs)
    if("${ANDROID_SDK_ROOT}" STREQUAL "")
        message(FATAL_ERROR
            "BOBUI_HOST_DOCUMENT_JAVA_SOURCES=ON requires setting ANDROID_SDK_ROOT."
        )
    endif()
...
# Collect the Java source files that were recorded by bobui_internal_add_jar.
# If we're not building for Android, bobui_internal_add_jar is not called, and we simple collect
# all java files under the current directory.
function(bobui_internal_collect_jar_sources out_var)
    if(NOT ANDROID)
    ...
    get_directory_property(result ${directory_arg} _bobui_jar_sources)
    get_directory_property(subdirs ${directory_arg} SUBDIRECTORIES)
    foreach(subdir IN LISTS subdirs)
        bobui_internal_collect_jar_sources(subdir_result DIRECTORY ${subdir})
...
function(bobui_internal_add_javadoc_target)
    set(no_value_options "")
    ...
    if(TARGET ${arg_MODULE})
        get_target_property(skip ${arg_MODULE} _bobui_skip_javadoc)
        if(skip)
    ...
    foreach(source_dir IN LISTS source_dirs)
        string(REGEX MATCH "/(org/bobuiproject/bobui/android(/.*|$))" package_dir "${source_dir}")
    ...
    list(TRANSFORM source_dirs REPLACE "/org/bobuiproject/bobui/android.*" "")
    list(REMOVE_DUPLICATES source_dirs)
    ...
    # Write the args file
    bobui_internal_write_android_javadoc_args(
        "${response_file}"
        "${javadoc_output_dir}"
        "${source_dirs}"
        "${package_names_space_separated}"
    ...
    set(bobui_ns "${BOBUI_CMAKE_EXPORT_NAMESPACE}")
    if(BOBUI_BUILDING_BOBUI OR PROJECT_NAME STREQUAL "BobUIBase")
        if(TARGET ${bobui_ns}Android)
            add_dependencies(${javadoc_target} ${bobui_ns}Android)
        endif()
    endif()
...
function(bobui_internal_create_source_jar)
    set(no_value_options "")
    ...
    set(include_jars "${BOBUI_ANDROID_JAR}")
    if(EXISTS "${bobui_classes_jar}")
        list(APPEND include_jars "${bobui_classes_jar}")
    endif()
    ...
    if(BOBUI_WILL_INSTALL)
        bobui_path_join(destination "${INSTALL_DATADIR}" "android" "${module}")
...
# The stored INTERFACE_BOBUI_ANDROID_PERMISSIONS is the transitive property.
function(bobui_internal_android_add_interface_permissions target)
    get_target_property(permissions ${target} BOBUI_ANDROID_PERMISSIONS)
    if(NOT permissions)
        return()
    endif()

    bobui_internal_set_module_transitive_properties(${target} TYPE LINK PROPERTIES
        INTERFACE_BOBUI_ANDROID_PERMISSIONS "${permissions}")
endfunction()

# The function stores Android features that are required by the module target.
# The stored INTERFACE_BOBUI_ANDROID_FEATURES is the transitive property.
function(bobui_internal_android_add_interface_features target)
    get_target_property(features ${target} BOBUI_ANDROID_FEATURES)
    if(NOT features)
        return()
    endif()

    bobui_internal_set_module_transitive_properties(${target} TYPE LINK PROPERTIES
        INTERFACE_BOBUI_ANDROID_FEATURES "${features}")
endfunction()
