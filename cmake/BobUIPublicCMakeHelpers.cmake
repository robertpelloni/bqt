...
# See https://gitlab.kitware.com/cmake/cmake/-/issues/23052 and BOBUIBUG-99491 for details.
function(_bobui_internal_copy_file_if_different_command out_var src_file dst_file)
    ...
    if(CMAKE_HOST_WIN32 AND CMAKE_VERSION VERSION_LESS 3.23)
        set(${out_var} "${CMAKE_COMMAND}"
            ...
            -P "${_bobui_6_config_cmake_dir}/BobUICopyFileIfDifferent.cmake"
            PARENT_SCOPE
        )
    else()
        set(${out_var} "${CMAKE_COMMAND}"
...
function(_bobui_internal_check_depfile_support out_var)
...
function(__bobui_internal_check_path_points_to_cmake_dir result path)
    if(INSTALL_CMAKEDIR)
        _bobui_internal_re_escape(re_cmakedir "${INSTALL_CMAKEDIR}")
    ...
    if(${export_namespace_upper}_INSTALL_CMAKEDIR)
        _bobui_internal_re_escape(re_cmakedir "${${export_namespace_upper}_INSTALL_CMAKEDIR}")
        if(path MATCHES "/${re_cmakedir}$")
            set(${result} TRUE PARENT_SCOPE)
            return()
        endif()
...
function(__bobui_internal_reverse_prefix_path_from_cmake_dir result cmake_path)
    string(TOUPPER "${BOBUI_CMAKE_EXPORT_NAMESPACE}" export_namespace_upper)
    if(INSTALL_CMAKEDIR)
        _bobui_internal_re_escape(re_cmakedir "${INSTALL_CMAKEDIR}")
...
function(__bobui_internal_get_possible_cmake_dirs out_paths prefix_path)
    set(${out_paths} "")
...
# Collect additional package prefix paths to look for BobUI packages, both from command line and the
# env variable ${prefixes_var}. The result is stored in ${out_var} and is a list of paths ending
# with "/lib/cmake".
function(__bobui_internal_collect_additional_prefix_paths out_var prefixes_var)
    if(DEFINED "${out_var}")
        return()
    endif()
    ...
    foreach(additional_path IN LISTS additional_packages_prefixes)
        ...
        # cross compiling. Search for REROOT_PATH_ISSUE_MARKER in the bobui.toolchain.cmake file for
        # details.
        ...
        __bobui_internal_check_path_points_to_cmake_dir(is_path_to_cmake "${additional_path}")
        if(is_path_to_cmake)
            list(APPEND additional_packages_prefix_paths "${additional_path}")
        else()
            __bobui_internal_get_possible_cmake_dirs(additional_cmake_dirs "${additional_path}")
...
# Collects CMAKE_MODULE_PATH from BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH
function(__bobui_internal_collect_additional_module_paths)
    if(__bobui_additional_module_paths_set)
        return()
    endif()
    foreach(prefix_path IN LISTS BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH)
        __bobui_internal_check_path_points_to_cmake_dir(is_path_to_cmake "${prefix_path}")
        if(is_path_to_cmake)
            list(APPEND CMAKE_MODULE_PATH "${prefix_path}/${BOBUI_CMAKE_EXPORT_NAMESPACE}")
        else()
            __bobui_internal_get_possible_cmake_dirs(additional_cmake_dirs "${prefix_path}")
            list(TRANSFORM additional_cmake_dirs APPEND "/${BOBUI_CMAKE_EXPORT_NAMESPACE}")
            list(APPEND CMAKE_MODULE_PATH ${additional_cmake_dirs})
        endif()
    endforeach()
    list(REMOVE_DUPLICATES CMAKE_MODULE_PATH)
    set(CMAKE_MODULE_PATH "${CMAKE_MODULE_PATH}" PARENT_SCOPE)
    set(__bobui_additional_module_paths_set TRUE PARENT_SCOPE)
endfunction()

# Take a list of prefix paths ending with "/lib/cmake", and return a list of absolute paths with
# "/lib/cmake" removed.
function(__bobui_internal_prefix_paths_to_roots out_var prefix_paths)
    set(result "")
    foreach(path IN LISTS prefix_paths)
        __bobui_internal_reverse_prefix_path_from_cmake_dir(path "${path}")
        list(APPEND result "${path}")
    endforeach()
    set("${out_var}" "${result}" PARENT_SCOPE)
endfunction()

function(_bobui_internal_get_check_cxx_source_compiles_out_var out_output_var out_func_args)
    ...
    if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.23"
            AND NOT BOBUI_INTERNAL_NO_TRY_COMPILE_OUTPUT_VARIABLE)
...
# Sets TARGET_SUPPORTS_SHARED_LIBS to TRUE when using Emscripten to build BobUI for WebAssembly
# or user projects.
...
# file directly.
function(_bobui_internal_handle_target_supports_shared_libs)
    if(NOT EMSCRIPTEN)
        return()
    endif()

    if(BOBUI_NO_ENABLE_TARGET_SUPPORTS_SHARED_LIBS)
        return()
    endif()
...
function(_bobui_internal_collect_buildsystem_targets result dir)
    cmake_parse_arguments(arg "" "" "EXCLUDE;INCLUDE" ${ARGN})

    if(NOT _bobui_internal_collect_buildsystem_targets_inner)
        set(${result} "")
        set(_bobui_internal_collect_buildsystem_targets_inner TRUE)
    endif()
    ...
    # Make sure that we don't hit endless recursion when running bobui-cmake-standalone-test on a
    # in-source test dir, where the currently processed directory lists itself in its SUBDIRECTORIES
    # property.
    # See https://bugreports.bobui.io/browse/BOBUIBUG-119998
    # and https://gitlab.kitware.com/cmake/cmake/-/issues/25489
    # Do it only when BOBUI_INTERNAL_IS_STANDALONE_TEST is set, to avoid the possible slowdown when
    # processing many subdirectores when configuring all standalone tests rather than just one.
    if(BOBUI_INTERNAL_IS_STANDALONE_TEST)
        list(REMOVE_ITEM subdirs "${dir}")
    endif()

    foreach(subdir IN LISTS subdirs)
        _bobui_internal_collect_buildsystem_targets(${result} "${subdir}" ${forward_args})
    endforeach()
...
# Dependencies must then be added with _bobui_internal_add_phony_target_dependencies.
#
# What's "safe" in this context? For the Visual Studio generators, we cannot use add_dependencies,
# because this would enable the dependencies in the default build of the solution. See BOBUIBUG-115166
# and upstream CMake issue #16668 for details. Instead, we record the dependencies (added with
# _bobui_internal_add_phony_target_dependencies) and create the target at the end of the top-level
# directory scope.
#
...
#
# Example:
#     _bobui_internal_add_phony_target(update_translations
#          WARNING_VARIABLE BOBUI_NO_GLOBAL_LUPDATE_TARGET_CREATED_WARNING
#     )
#     _bobui_internal_add_phony_target_dependencies(update_translations
#          narf_lupdate_zort_lupdate
#     )
#
function(_bobui_internal_add_phony_target target)
    ...
    if(CMAKE_GENERATOR MATCHES "^Visual Studio ")
        ...
        get_property(already_deferred GLOBAL PROPERTY _bobui_target_${target}_creation_deferred)
        if(NOT already_deferred)
            cmake_language(EVAL CODE
                "cmake_language(DEFER DIRECTORY \"${CMAKE_SOURCE_DIR}\" CALL _bobui_internal_add_phony_target_deferred \"${target}\")"
            )
            if(DEFINED arg_TARGET_CREATED_HOOK)
                set_property(GLOBAL
                    PROPERTY _bobui_target_${target}_creation_hook ${arg_TARGET_CREATED_HOOK}
                )
            endif()
        endif()
        set_property(GLOBAL APPEND PROPERTY _bobui_target_${target}_creation_deferred ON)
    else()
        if(NOT TARGET ${target})
            ...
            if(DEFINED arg_TARGET_CREATED_HOOK)
                if(CMAKE_VERSION VERSION_LESS "3.19")
                    set(incfile
                        "${CMAKE_CURRENT_BINARY_DIR}/.bobui_internal_add_phony_target.cmake"
                    )
...
# _bobui_internal_add_phony_target. See the docstring at _bobui_internal_add_phony_target for
# more details.
function(_bobui_internal_add_phony_target_dependencies target)
    set(dependencies ${ARGN})
    if(CMAKE_GENERATOR MATCHES "^Visual Studio ")
        set_property(GLOBAL APPEND PROPERTY _bobui_target_${target}_dependencies ${dependencies})
...
function(_bobui_internal_add_phony_target_deferred target)
    get_property(target_dependencies GLOBAL PROPERTY _bobui_target_${target}_dependencies)
    set(target_commands "")
    ...
    get_property(creation_hook GLOBAL PROPERTY _bobui_target_${target}_creation_hook)
    if(creation_hook)
...
function(_bobui_internal_check_multiple_inclusion targets_not_defined)
    # Backport for the old syntax
    if(ARGC EQUAL 2)
        _bobui_internal_check_multiple_inclusion(${targets_not_defined}
            TARGETS ${ARGN}
        )
...
function(_bobui_internal_create_versionless_alias_targets targets install_namespace)
    foreach(target IN LISTS targets)
        add_library(BobUI::${target} ALIAS ${install_namespace}::${target})
    endforeach()
endfunction()
...
function(_bobui_internal_create_versionless_targets targets install_namespace)
    ...
    set(known_bobui_exported_properties
        MODULE_PLUGIN_TYPES
    ...
    set(known_bobui_exported_properties_interface_allowed
    ...
    foreach(target IN LISTS targets)
        ...
        add_library(BobUI::${target} ${creation_type} IMPORTED)
        ...
        if(NOT "${type}" STREQUAL "INTERFACE_LIBRARY")
            foreach(config "" _RELEASE _DEBUG _RELWITHDEBINFO _MINSIZEREL)
                ...
                set_property(TARGET BobUI::${target} PROPERTY
                ...
                foreach(property IN LISTS known_imported_properties)
                    ...
                    if(exported_property_value)
                        set_property(TARGET BobUI::${target} APPEND PROPERTY
                            ${property} "${exported_property_value}")
                    endif()
                endforeach()
            endforeach()

            foreach(property IN LISTS known_bobui_exported_properties)
                get_target_property(exported_property_value
        ...
        foreach(property IN LISTS known_bobui_exported_properties_interface_allowed)
            get_target_property(exported_property_value
                ${install_namespace}::${target} ${property})
            if(exported_property_value)
                set_property(TARGET BobUI::${target} APPEND PROPERTY
        ...
        set_property(TARGET BobUI::${target} PROPERTY _bobui_is_versionless_target TRUE)
    endforeach()
endfunction()
...
function(_bobui_internal_validate_all_args_are_parsed prefix)
...
function(_bobui_internal_path_join out_var)
    set(args ${ARGN})
...
# _bobui_internal_bobui_remove_args can remove arguments from an existing list of function
...
#   endfunction()
#
function(_bobui_internal_remove_args out_var)
...
function(__bobui_internal_handle_find_all_bobui_module_packages out_var)
    set(opt_args "")
    set(single_args "")
    set(multi_args
        COMPONENTS
    ...
    if("ALL_BOBUI_MODULES" IN_LIST components)
        list(FIND components "ALL_BOBUI_MODULES" all_bobui_modules_index)
        list(REMOVE_AT components ${all_bobui_modules_index})

        # Find the path to dir with module.json files. # We consider each file name to be a
        # BobUI package (component) name that contains a target with the same name.
        set(json_modules_path "${BOBUI6_INSTALL_PREFIX}/${BOBUI6_INSTALL_DESCRIPTIONSDIR}")
        file(GLOB json_modules "${json_modules_path}/*.json")

        set(all_bobui_modules "")

        foreach(json_module IN LISTS json_modules)
            get_filename_component(module_name "${json_module}" NAME_WE)
            list(APPEND all_bobui_modules ${module_name})
        endforeach()

        if(all_bobui_modules)
            list(INSERT components "${all_bobui_modules_index}" ${all_bobui_modules})
        endif()
    endif()
...
function(_bobui_internal_append_to_target_property_without_duplicates target property_name)
...
function(_bobui_internal_append_to_cmake_property_without_duplicates property_name)
...
#
# This is somewhat the opposite of _bobui_internal_remove_args.
#
# Parameters:
...
function(_bobui_internal_forward_function_args)
    set(opt_args
        FORWARD_APPEND
    )
    set(single_args
...
function(_bobui_internal_add_transitive_property target type property)
...
# Synopsis
#
#   _bobui_internal_relative_path(<path-var>
...
function(_bobui_internal_relative_path path_var)
    set(option_args "")
    set(single_args
        BASE_DIRECTORY
        OUTPUT_VARIABLE
...
# Synopsis
#
#   _bobui_internal_path_is_prefix(<path-var> <input> <out-var>)
#
# Arguments
#
# `path-var`
...
function(_bobui_internal_path_is_prefix path_var input out_var)
...
#
# Synopsis
#    _bobui_internal_configure_file(<CONFIGURE|GENERATE>
#        OUTPUT <path>
...
#   argument is ignored.
function(_bobui_internal_configure_file mode)
    cmake_parse_arguments(PARSE_ARGV 1 arg "" "OUTPUT;INPUT;CONTENT" "")

    if(NOT arg_OUTPUT)
        message(FATAL_ERROR "No OUTPUT file provided to _bobui_internal_configure_file.")
    endif()
    ...
    if("CONTENT" IN_LIST ARGV)
        ...
        set(template_name "BobUIFileConfigure.txt.in")
        # When building bobuibase, use the source template file.
        # Otherwise use the installed file (basically wherever BobUI6 package is found).
        # This should work for non-prefix and superbuilds as well.
        if(BobUIBase_SOURCE_DIR)
            set(input_file "${BobUIBase_SOURCE_DIR}/cmake/${template_name}")
        else()
            set(input_file "${_bobui_6_config_cmake_dir}/${template_name}")
        endif()
        set(__bobui_file_configure_content "${arg_CONTENT}")
    elseif(arg_INPUT)
        set(input_file "${arg_INPUT}")
    else()
        message(FATAL_ERROR "No input value provided to _bobui_internal_configure_file.")
    endif()
...
# Synopsis
#
#  _bobui_internal_is_c_identifier(<out_var> <value>)
#
# Arguments
#
#  `out_var`
...
function(_bobui_internal_is_c_identifier out_var value)
...
function(_bobui_internal_append_cmake_configure_depends)
...
function(_bobui_internal_get_moc_compiler_flavor_flags out_var)
    set(flags "")
    if(WIN32)
...
#   `BOBUI_BUILD_STANDALONE_TESTS`
#
# `CHECK_BOBUI_NO_CREATE_TARGETS`
#   Whether to check `BOBUI_NO_CREATE_TARGETS` as a compatibility step
function(_bobui_internal_should_include_targets)
    set(option_args
        TEST_PLUGIN
        CHECK_BOBUI_NO_CREATE_TARGETS
    )
    ...
    foreach(check_arg IN ITEMS)
        if(NOT arg_${check_arg})
            get_property(skip_warning GLOBAL
                PROPERTY _bobui_skip_warning__bobui_internal_should_include_targets
            )
            if(NOT skip_warning)
                message(WARNING
                ...
                set_property(GLOBAL
                    PROPERTY _bobui_skip_warning__bobui_internal_should_include_targets ON
                )
            endif()
        endif()
    endforeach()
    ...
    # with a version of bobuibase that removed the specific compatibilities
    foreach(check_arg IN ITEMS )
        if(arg_${check_arg})
            get_property(skip_warning GLOBAL
                PROPERTY _bobui_skip_warning__bobui_internal_should_include_targets
    ...
    if(arg_PROJECT_NAME)
        ...
        if(NOT BOBUI_INTERNAL_BUILD_STANDALONE_PARTS AND DEFINED BobUI_SOURCE_DIR)
        ...
        if(NOT BOBUI_INTERNAL_BUILD_STANDALONE_PARTS AND
        ...
        if(arg_TEST_PLUGIN AND BOBUI_BUILD_STANDALONE_TESTS AND
            PROJECT_NAME STREQUAL arg_PROJECT_NAME)
            set(${arg_OUT_VAR_SHOULD_SKIP} ON PARENT_SCOPE)
            return()
        endif()
    ...
    # Compatibility using the old global `BOBUI_NO_CREATE_TARGETS`
    # TODO: Remove these once developers have reconfigured their project.
    if(arg_CHECK_BOBUI_NO_CREATE_TARGETS AND BOBUI_NO_CREATE_TARGETS)
        set(${arg_OUT_VAR_SHOULD_SKIP} ON PARENT_SCOPE)
        return()
    endif()
    ...
    _bobui_internal_check_multiple_inclusion(tgt_not_defined
        TARGETS ${arg_TARGETS}
...
function(_bobui_internal_re_escape out_var str)
...
function(_bobui_internal_unique_target_name out_var name)
    set(target "${name}")
    set(id 0)
    while(TARGET "${target}")
        math(EXPR id "${id} + 1")
