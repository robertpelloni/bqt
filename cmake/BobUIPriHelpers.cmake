    ...
    foreach(lib ${BOBUI_QMAKE_LIBS_FOR_${module_name}})
        ...
        while(lib_targets)
            ...
            if(TARGET ${lib_target})
                ...
                get_target_property(skip_include_dir "${lib_target}" _bobui_skip_include_dir_for_pri)
                if(skip_include_dir)
                ...
                __bobui_internal_strip_target_directory_scope_token("${lib_target}" lib_target)
...
# Retrieves the public BobUI module dependencies of the given BobUI module or BobUI Private module.
...
#
function(bobui_get_direct_module_dependencies target out_var)
    ...
    if(arg_PRIVATE)
        ...
        # Remove all BobUI::Foo and BobUI6::Foo from libs that also appear in public_libs.
        set(libs_to_remove "")
        foreach(lib IN LISTS public_libs)
            list(APPEND libs_to_remove "${lib}")
            if(lib MATCHES "^BobUI::(.*)")
                list(APPEND libs_to_remove "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${CMAKE_MATCH_1}")
            elseif(lib MATCHES "^{BOBUI_CMAKE_EXPORT_NAMESPACE}::(.*)")
                list(APPEND libs_to_remove "BobUI::${CMAKE_MATCH_1}")
            endif()
        endforeach()
    ...
    while(libs)
        ...
        get_target_property(lib_config_module_name ${lib} "_bobui_config_module_name")
...
# Also targets with the _bobui_is_nolink_target property are mapped to nolink as well.
function(bobui_internal_map_targets_to_qmake_libs out_var)
    set(result "")
    foreach(target ${ARGN})
        ...
        if(TARGET "${target}")
            get_target_property(marked_as_no_link_target "${target}" _bobui_is_nolink_target)
            if(marked_as_no_link_target)
                set(is_no_link_target TRUE)
            endif()
        endif()

        bobui_internal_map_target_to_qmake_lib(${stripped_target} qmake_lib)
...
#
function(bobui_internal_get_module_run_dependencies out_var target)
    cmake_parse_arguments(arg "PUBLIC" "" "PUBLIC_DEPENDENCIES" ${ARGN})

    # Private dependencies of the module are runtime dependencies.
    bobui_get_direct_module_dependencies(${target} run_dependencies PRIVATE)
    ...
    if(arg_PUBLIC AND TARGET ${target}Private)
        bobui_get_direct_module_dependencies(${target}Private bobui_for_private)

        # FooPrivate depends on Foo, but we must not record this dependency in run_depends.
        get_target_property(config_module_name ${target} _bobui_config_module_name)
        list(REMOVE_ITEM bobui_for_private ${config_module_name})

        list(APPEND run_dependencies ${bobui_for_private})
    endif()
...
function(bobui_generate_module_pri_file target)
    ...
    get_target_property(enabled_features "${target}"
                        "${property_prefix}BOBUI_ENABLED_PUBLIC_FEATURES")
    get_target_property(disabled_features "${target}"
                        "${property_prefix}BOBUI_DISABLED_PUBLIC_FEATURES")
    get_target_property(enabled_private_features "${target}"
                        "${property_prefix}BOBUI_ENABLED_PRIVATE_FEATURES")
    get_target_property(disabled_private_features "${target}"
                        "${property_prefix}BOBUI_DISABLED_PRIVATE_FEATURES")
    bobui_correct_features(enabled_features "${enabled_features}")
    bobui_correct_features(disabled_features "${disabled_features}")
    bobui_correct_features(enabled_private_features "${enabled_private_features}")
    bobui_correct_features(disabled_private_features "${disabled_private_features}")

    get_target_property(module_internal_config "${target}"
                        "${property_prefix}BOBUI_MODULE_INTERNAL_CONFIG")
    get_target_property(module_pri_extra_content "${target}"
                        "${property_prefix}BOBUI_MODULE_PRI_EXTRA_CONTENT")
    get_target_property(module_ldflags "${target}"
                        "${property_prefix}BOBUI_MODULE_LDFLAGS")
    get_target_property(module_depends "${target}"
                        "${property_prefix}BOBUI_MODULE_DEPENDS")
    ...
    get_target_property(config_module_name ${target} _bobui_config_module_name)
    get_target_property(qmake_module_config ${target} ${property_prefix}BOBUI_QMAKE_MODULE_CONFIG)
    if (is_interface_lib)
        list(APPEND qmake_module_config "no_link")
    endif()
    if(qmake_module_config)
        string(REPLACE ";" " " module_build_config "${qmake_module_config}")
        set(module_build_config "\nBOBUI.${config_module_name}.CONFIG = ${module_build_config}")
    else()
        set(module_build_config "")
    endif()
    ...
    if(is_fw)
        ...
        set(framework_base_path "$$BOBUI_MODULE_LIB_BASE")
...
${framework_base_path}/${fw_private_module_header_dir}")
        ...
        set(public_module_includes "$$BOBUI_MODULE_INCLUDE_BASE $$BOBUI_MODULE_INCLUDE_BASE/${module}")
        set(public_module_frameworks "")
        set(private_module_includes "")
        bobui_internal_append_include_directories_with_headers_check(${target}
            private_module_includes PRIVATE
            "$$BOBUI_MODULE_INCLUDE_BASE/${module_versioned_include_dir} \
$$BOBUI_MODULE_INCLUDE_BASE/${module_versioned_inner_include_dir}")
    endif()
    ...
    set(pri_data_cmake_file "bobui_lib_${config_module_name}_private.cmake")
    ...
    bobui_path_join(target_path ${BOBUI_BUILD_DIR} ${INSTALL_MKSPECSDIR}/modules)
    ...
    if(module_plugin_types)
        set(module_plugin_types_assignment
            "\nBOBUI.${config_module_name}.plugin_types = ${module_plugin_types}")
    endif()

    bobui_get_direct_module_dependencies(${target} public_module_dependencies_list)
    ...
    bobui_path_join(pri_file_name "${target_path}" "bobui_lib_${config_module_name}.pri")
    ...
    if(NOT BOBUI_BUILD_SHARED_LIBS AND target STREQUAL Gui)
        set(extra_assignments "\nBOBUI_DEFAULT_QPA_PLUGIN = q${BOBUI_QPA_DEFAULT_PLATFORM}")
    endif()
    ...
    bobui_internal_map_targets_to_qmake_libs(module_uses ${dep_targets})
    list(JOIN module_uses " " joined_module_uses)

    # Retrieve the public module's runtime dependencies.
    bobui_internal_get_module_run_dependencies(public_module_run_dependencies ${target}
        PUBLIC
        PUBLIC_DEPENDENCIES "${public_module_dependencies_list}")

    set(content "BOBUI.${config_module_name}.VERSION = ${PROJECT_VERSION}
BOBUI.${config_module_name}.name = ${module}
BOBUI.${config_module_name}.module = ${module_name_in_pri}
BOBUI.${config_module_name}.libs = $$BOBUI_MODULE_LIB_BASE
BOBUI.${config_module_name}.ldflags = ${module_ldflags}
BOBUI.${config_module_name}.includes = ${public_module_includes}
BOBUI.${config_module_name}.frameworks = ${public_module_frameworks}
BOBUI.${config_module_name}.bins = $$BOBUI_MODULE_BIN_BASE${module_plugin_types_assignment}
BOBUI.${config_module_name}.depends = ${public_module_dependencies}
")
    if(NOT "${public_module_run_dependencies}" STREQUAL "")
        string(APPEND content
            "BOBUI.${config_module_name}.run_depends = ${public_module_run_dependencies}\n")
    endif()
    string(APPEND content
        "BOBUI.${config_module_name}.uses = ${joined_module_uses}
BOBUI.${config_module_name}.module_config = ${joined_module_internal_config}${module_build_config}
BOBUI.${config_module_name}.DEFINES = ${joined_target_defines}
BOBUI.${config_module_name}.enabled_features = ${enabled_features}
BOBUI.${config_module_name}.disabled_features = ${disabled_features}${extra_assignments}
BOBUI_CONFIG += ${enabled_features}
BOBUI_MODULES += ${config_module_name_base}
...
")
    ...
    if (NOT arg_NO_PRIVATE_MODULE AND NOT arg_INTERNAL_MODULE)
        set(pri_data_cmake_file "bobui_lib_${config_module_name}_private.cmake")
        bobui_generate_qmake_libraries_pri_content(${config_module_name} "${CMAKE_CURRENT_BINARY_DIR}"
            ${pri_data_cmake_file})

        set(private_pri_file_name "bobui_lib_${config_module_name}_private.pri")

        set(private_module_dependencies "")
        if(NOT is_interface_lib)
            bobui_get_direct_module_dependencies(${target}Private private_module_dependencies)
        endif()
        ...
        bobui_internal_map_targets_to_qmake_libs(private_module_uses ${dep_targets})
        list(JOIN private_module_uses " " joined_private_module_uses)

        # Retrieve the private module's runtime dependencies.
        bobui_internal_get_module_run_dependencies(private_module_run_dependencies ${target}Private
            PUBLIC_DEPENDENCIES "${dep_targets}")

        # Generate a preliminary bobui_lib_XXX_private.pri file
        set(content
            "BOBUI.${config_module_name}_private.VERSION = ${PROJECT_VERSION}
BOBUI.${config_module_name}_private.name = ${module}
BOBUI.${config_module_name}_private.module =
BOBUI.${config_module_name}_private.libs = $$BOBUI_MODULE_LIB_BASE
BOBUI.${config_module_name}_private.includes = ${private_module_includes}
BOBUI.${config_module_name}_private.frameworks = ${private_module_frameworks}
BOBUI.${config_module_name}_private.depends = ${private_module_dependencies}
")
        if(NOT "${private_module_run_dependencies}" STREQUAL "")
            string(APPEND content
                "BOBUI.${config_module_name}_private.run_depends = ${private_module_run_dependencies}
")
        endif()
        string(APPEND content
            "BOBUI.${config_module_name}_private.uses = ${joined_private_module_uses}
BOBUI.${config_module_name}_private.module_config = ${joined_module_internal_config}
BOBUI.${config_module_name}_private.enabled_features = ${enabled_private_features}
BOBUI.${config_module_name}_private.disabled_features = ${disabled_private_features}")
        file(GENERATE
        ...
        bobui_path_join(private_pri_file_path "${target_path}" "${private_pri_file_name}")
        ...
        add_custom_command(
            ...
            DEPENDS ${inputs}
                    "${BOBUI_CMAKE_DIR}/BobUIGenerateLibPri.cmake"
            ...
            COMMAND ${CMAKE_COMMAND} "-DIN_FILES=${inputs}" "-DOUT_FILE=${private_pri_file_path}"
                    ...
                    -P "${BOBUI_CMAKE_DIR}/BobUIGenerateLibPri.cmake"
            VERBATIM)
    ...
    bobui_install(FILES "${pri_files}" DESTINATION ${INSTALL_MKSPECSDIR}/modules)
endfunction()

# Generates bobui_ext_XXX.pri files for consumption by qmake
function(bobui_generate_3rdparty_lib_pri_file target lib pri_file_var)
    if(NOT lib)
    ...
    file(GENERATE
        OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/$<CONFIG>/bobui_ext_${lib}.cmake"
...
")
    ...
    set(inputs "")
    foreach(cfg ${configs})
        list(APPEND inputs "${CMAKE_CURRENT_BINARY_DIR}/${cfg}/bobui_ext_${lib}.cmake")
    endforeach()

    bobui_path_join(pri_target_path ${BOBUI_BUILD_DIR} ${INSTALL_MKSPECSDIR}/modules)
    bobui_path_join(pri_file "${pri_target_path}" "bobui_ext_${lib}.pri")
    bobui_path_join(bobui_build_libdir ${BOBUI_BUILD_DIR} ${INSTALL_LIBDIR})
    add_custom_command(
        OUTPUT "${pri_file}"
        DEPENDS ${inputs} "${BOBUI_CMAKE_DIR}/BobUIGenerateExtPri.cmake"
        COMMAND ${CMAKE_COMMAND} "-DIN_FILES=${inputs}" "-DOUT_FILE=${pri_file}" -DLIB=${lib}
                "-DCONFIGS=${configs}"
                "-DBOBUI_BUILD_LIBDIR=${bobui_build_libdir}"
                -P "${BOBUI_CMAKE_DIR}/BobUIGenerateExtPri.cmake"
        VERBATIM)
...
# Generates bobui_plugin_XXX.pri files for consumption by qmake
#
# BOBUI_PLUGIN.XXX.EXTENDS is set to "-" for the following plugin types:
#   - generic
#   - platform, if the plugin is not the default QPA plugin
# Otherwise, this variable is empty.
function(bobui_generate_plugin_pri_file target)
    get_target_property(plugin_name ${target} OUTPUT_NAME)
    get_target_property(plugin_type ${target} BOBUI_PLUGIN_TYPE)
    get_target_property(qmake_plugin_type ${target} BOBUI_QMAKE_PLUGIN_TYPE)
    get_target_property(default_plugin ${target} BOBUI_DEFAULT_PLUGIN)
    get_target_property(plugin_class_name ${target} BOBUI_PLUGIN_CLASS_NAME)
    get_target_property(plugin_pri_extra_content ${target} BOBUI_PLUGIN_PRI_EXTRA_CONTENT)
    ...
    foreach(dep ${target_deps})
        ...
        bobui_get_qmake_module_name(dep_name ${dep_name})
    ...
    bobui_path_join(pri_target_path ${BOBUI_BUILD_DIR} ${INSTALL_MKSPECSDIR}/modules)
    bobui_path_join(pri_file "${pri_target_path}" "bobui_plugin_${plugin_name}.pri")

    set(content "BOBUI_PLUGIN.${plugin_name}.TYPE = ${qmake_plugin_type}
BOBUI_PLUGIN.${plugin_name}.EXTENDS = ${plugin_extends}
BOBUI_PLUGIN.${plugin_name}.DEPENDS = ${plugin_deps}
BOBUI_PLUGIN.${plugin_name}.CLASS_NAME = ${plugin_class_name}
BOBUI_PLUGIN.${plugin_name}.module_config = ${module_config}
BOBUI_PLUGINS += ${plugin_name}
...
function(bobui_generate_global_config_pri_file)
    bobui_path_join(qconfig_pri_target_path ${PROJECT_BINARY_DIR} ${INSTALL_MKSPECSDIR})
    bobui_path_join(qconfig_pri_target_path "${qconfig_pri_target_path}" "qconfig.pri")

    get_target_property(enabled_features GlobalConfig INTERFACE_BOBUI_ENABLED_PUBLIC_FEATURES)
    get_target_property(disabled_features GlobalConfig INTERFACE_BOBUI_DISABLED_PUBLIC_FEATURES)

    bobui_correct_features(corrected_enabled_features "${enabled_features}")
    bobui_correct_features(corrected_disabled_features "${disabled_features}")
    ...
    list(APPEND config_entries "${bobui_build_config_type}")
    string (REPLACE ";" " " config_entries "${config_entries}")

    get_target_property(public_config GlobalConfig INTERFACE_BOBUI_QMAKE_PUBLIC_CONFIG)
    get_target_property(bobui_public_config GlobalConfig INTERFACE_BOBUI_QMAKE_PUBLIC_BOBUI_CONFIG)
    bobui_correct_config(corrected_public_config "${public_config}")
    bobui_correct_config(corrected_bobui_public_config "${bobui_public_config}")
    bobui_guess_qmake_build_config(qmake_build_config)
    list(APPEND corrected_bobui_public_config ${qmake_build_config})

    list(JOIN corrected_public_config " " public_config_joined)
    list(JOIN corrected_bobui_public_config " " bobui_public_config_joined)

    set(content "")
    if(GCC OR CLANG AND NOT "${CMAKE_SYSROOT}" STREQUAL "")
        string(APPEND content "!host_build {
    QMAKE_CFLAGS    += --sysroot=\$\$[BOBUI_SYSROOT]
    QMAKE_CXXFLAGS  += --sysroot=\$\$[BOBUI_SYSROOT]
    QMAKE_LFLAGS    += --sysroot=\$\$[BOBUI_SYSROOT]
}
")
    endif()
    ...
    if(CMAKE_CROSSCOMPILING)
        ...
        string(APPEND content "host_build {
    BOBUI_ARCH = ${${host_info_var_prefix}_ARCH}
    BOBUI_BUILDABI = ${${host_info_var_prefix}_BUILDABI}
    BOBUI_TARGET_ARCH = ${TEST_architecture_arch}
    BOBUI_TARGET_BUILDABI = ${TEST_buildAbi}
} else {
    BOBUI_ARCH = ${TEST_architecture_arch}
    BOBUI_BUILDABI = ${TEST_buildAbi}
    BOBUI_LIBCPP_ABI_TAG = ${TEST_libcppabiTag}
}
")
    else()
        string(APPEND content "BOBUI_ARCH = ${TEST_architecture_arch}
BOBUI_BUILDABI = ${TEST_buildAbi}
BOBUI_LIBCPP_ABI_TAG = ${TEST_libcppabiTag}
")
    endif()

    string(APPEND content "BOBUI.global.enabled_features = ${corrected_enabled_features}
BOBUI.global.disabled_features = ${corrected_disabled_features}
BOBUI.global.disabled_features += release build_all
BOBUI_CONFIG += ${bobui_public_config_joined}
CONFIG += ${config_entries} ${public_config_joined}
BOBUI_VERSION = ${PROJECT_VERSION}
BOBUI_MAJOR_VERSION = ${PROJECT_VERSION_MAJOR}
BOBUI_MINOR_VERSION = ${PROJECT_VERSION_MINOR}
BOBUI_PATCH_VERSION = ${PROJECT_VERSION_PATCH}
")

    set(extra_statements "")
    if(BOBUI_NAMESPACE)
        list(APPEND extra_statements "BOBUI_NAMESPACE = ${BOBUI_NAMESPACE}")
    endif()

    if(BOBUI_LIBINFIX)
        list(APPEND extra_statements "BOBUI_LIBINFIX = ${BOBUI_LIBINFIX}")
    endif()

    if(APPLECLANG)
        set(compiler_version_major_var_name "BOBUI_APPLE_CLANG_MAJOR_VERSION")
        set(compiler_version_minor_var_name "BOBUI_APPLE_CLANG_MINOR_VERSION")
        set(compiler_version_patch_var_name "BOBUI_APPLE_CLANG_PATCH_VERSION")
    elseif(IntelLLVM)
        set(compiler_version_major_var_name "BOBUI_INTELLLVM_MAJOR_VERSION")
        set(compiler_version_minor_var_name "BOBUI_INTELLLVM_MINOR_VERSION")
        set(compiler_version_patch_var_name "BOBUI_INTELLLVM_PATCH_VERSION")
    elseif(CLANG)
        set(compiler_version_major_var_name "BOBUI_CLANG_MAJOR_VERSION")
        set(compiler_version_minor_var_name "BOBUI_CLANG_MINOR_VERSION")
        set(compiler_version_patch_var_name "BOBUI_CLANG_PATCH_VERSION")
    elseif(GCC)
        set(compiler_version_major_var_name "BOBUI_GCC_MAJOR_VERSION")
        set(compiler_version_minor_var_name "BOBUI_GCC_MINOR_VERSION")
        set(compiler_version_patch_var_name "BOBUI_GCC_PATCH_VERSION")
    elseif(MSVC)
        set(compiler_version_major_var_name "BOBUI_MSVC_MAJOR_VERSION")
        set(compiler_version_minor_var_name "BOBUI_MSVC_MINOR_VERSION")
        set(compiler_version_patch_var_name "BOBUI_MSVC_PATCH_VERSION")
    endif()

    if(compiler_version_major_var_name)
        list(APPEND extra_statements
            "${compiler_version_major_var_name} = ${BOBUI_COMPILER_VERSION_MAJOR}")
        list(APPEND extra_statements
            "${compiler_version_minor_var_name} = ${BOBUI_COMPILER_VERSION_MINOR}")
        list(APPEND extra_statements
            "${compiler_version_patch_var_name} = ${BOBUI_COMPILER_VERSION_PATCH}")
    endif()

    if(APPLE)
        list(APPEND extra_statements "BOBUI_MAC_SDK_VERSION = ${BOBUI_MAC_SDK_VERSION}")
        if(NOT CMAKE_SYSTEM_NAME OR CMAKE_SYSTEM_NAME STREQUAL "Darwin")
            # macOS
            list(APPEND extra_statements
                "QMAKE_MACOSX_DEPLOYMENT_TARGET = ${CMAKE_OSX_DEPLOYMENT_TARGET}")
            list(APPEND extra_statements
                 "BOBUI_MAC_SDK_VERSION_MIN = ${BOBUI_SUPPORTED_MIN_MACOS_SDK_VERSION}")
            list(APPEND extra_statements
                 "BOBUI_MAC_SDK_VERSION_MAX = ${BOBUI_SUPPORTED_MAX_MACOS_SDK_VERSION}")
        elseif(CMAKE_SYSTEM_NAME STREQUAL iOS)
            list(APPEND extra_statements
                "QMAKE_IOS_DEPLOYMENT_TARGET = ${CMAKE_OSX_DEPLOYMENT_TARGET}")
            list(APPEND extra_statements
                 "BOBUI_MAC_SDK_VERSION_MIN = ${BOBUI_SUPPORTED_MIN_IOS_SDK_VERSION}")
            list(APPEND extra_statements
                 "BOBUI_MAC_SDK_VERSION_MAX = ${BOBUI_SUPPORTED_MAX_IOS_SDK_VERSION}")
         elseif(CMAKE_SYSTEM_NAME STREQUAL visionOS)
            list(APPEND extra_statements
                "QMAKE_VISIONOS_DEPLOYMENT_TARGET = ${CMAKE_OSX_DEPLOYMENT_TARGET}")
            list(APPEND extra_statements
                 "BOBUI_MAC_SDK_VERSION_MIN = ${BOBUI_SUPPORTED_MIN_VISIONOS_SDK_VERSION}")
            list(APPEND extra_statements
                 "BOBUI_MAC_SDK_VERSION_MAX = ${BOBUI_SUPPORTED_MAX_VISIONOS_SDK_VERSION}")
        endif()
        ...
        if (CMAKE_OSX_ARCHITECTURES)
            ...
            set(architectures "$$BOBUI_ARCH")
        endif()
        list(APPEND extra_statements "BOBUI_ARCHS = ${architectures}")
    endif()

    if(WASM)
        list(APPEND extra_statements
            "BOBUI_EMCC_VERSION = ${EMCC_VERSION}")
    endif()
    if(extra_statements)
    ...
    bobui_install(FILES "${qconfig_pri_target_path}" DESTINATION ${INSTALL_MKSPECSDIR})
endfunction()

# Creates mkspecs/qdevice.pri which contains target device information for cross-builds.
# The content of BOBUI_QMAKE_DEVICE_OPTIONS is written verbatim into qdevice.pri.
function(bobui_generate_global_device_pri_file)
    if(NOT CMAKE_CROSSCOMPILING)
        return()
    endif()

    bobui_path_join(qdevice_pri_target_path ${PROJECT_BINARY_DIR} ${INSTALL_MKSPECSDIR} "qdevice.pri")

    set(content "")
    foreach(opt ${BOBUI_QMAKE_DEVICE_OPTIONS})
    ...
    if(ANDROID)
        ...
        # TODO: BOBUIBUG-80943 When we eventually support Android multi-abi, this should be changed.
        string(APPEND content "DEFAULT_ANDROID_ABIS = ${ANDROID_ABI}\n")

        if(BOBUI_ANDROID_JAVAC_SOURCE)
            string(APPEND content "ANDROID_JAVAC_SOURCE_VERSION = ${BOBUI_ANDROID_JAVAC_SOURCE}\n")
        endif()
        if(BOBUI_ANDROID_JAVAC_TARGET)
            string(APPEND content "ANDROID_JAVAC_TARGET_VERSION = ${BOBUI_ANDROID_JAVAC_TARGET}\n")
        endif()
    endif()

    if(BOBUI_APPLE_SDK)
        string(APPEND content "QMAKE_MAC_SDK = ${BOBUI_APPLE_SDK}\n")
    endif()
    ...
    bobui_install(FILES "${qdevice_pri_target_path}" DESTINATION ${INSTALL_MKSPECSDIR})
endfunction()

function(bobui_get_build_parts out_var)
    set(parts "libs")

    if(BOBUI_BUILD_EXAMPLES AND BOBUI_BUILD_EXAMPLES_BY_DEFAULT)
        list(APPEND parts "examples")
    endif()

    if(BOBUI_BUILD_TESTS AND BOBUI_BUILD_TESTS_BY_DEFAULT)
        list(APPEND parts "tests")
    endif()

    if(NOT CMAKE_CROSSCOMPILING OR BOBUI_FORCE_BUILD_TOOLS)
...
function(bobui_generate_global_module_pri_file)
    bobui_path_join(qmodule_pri_target_path ${PROJECT_BINARY_DIR} ${INSTALL_MKSPECSDIR})
    bobui_path_join(qmodule_pri_target_path "${qmodule_pri_target_path}" "qmodule.pri")

    get_target_property(enabled_features GlobalConfig INTERFACE_BOBUI_ENABLED_PRIVATE_FEATURES)
    get_target_property(disabled_features GlobalConfig INTERFACE_BOBUI_DISABLED_PRIVATE_FEATURES)
    ...
    get_target_property(private_config GlobalConfig INTERFACE_BOBUI_QMAKE_PRIVATE_CONFIG)
    bobui_correct_config(corrected_private_config "${private_config}")
    list(JOIN corrected_private_config " " private_config_joined)

    set(content "")
    if(DEFINED BOBUI_EXTRA_DEFINES)
        list(JOIN BOBUI_EXTRA_DEFINES " " value)
        string(APPEND content "EXTRA_DEFINES += ${value}\n")
    endif()
    if(DEFINED BOBUI_EXTRA_INCLUDEPATHS)
        bobui_to_qmake_path_list(value ${BOBUI_EXTRA_INCLUDEPATHS})
        string(APPEND content "EXTRA_INCLUDEPATH += ${value}\n")
    endif()
    if(DEFINED BOBUI_EXTRA_LIBDIRS)
        bobui_to_qmake_path_list(value ${BOBUI_EXTRA_LIBDIRS})
        string(APPEND content "EXTRA_LIBDIR += ${value}\n")
    endif()
    if(DEFINED BOBUI_EXTRA_FRAMEWORKPATHS)
        bobui_to_qmake_path_list(value ${BOBUI_EXTRA_FRAMEWORKPATHS})
    ...
    if(CMAKE_CROSSCOMPILING)
        ...
        string(APPEND content "host_build {
    BOBUI_CPU_FEATURES.${host_arch} = ${host_subarchs}
} else {
    BOBUI_CPU_FEATURES.${arch} = ${subarchs}
}
")
    else()
        string(APPEND content "BOBUI_CPU_FEATURES.${arch} = ${subarchs}\n")
    endif()

    string(APPEND content "BOBUI.global_private.enabled_features = ${corrected_enabled_features}
BOBUI.global_private.disabled_features = ${corrected_disabled_features}
...
")
    ...
    string(APPEND content "BOBUI_COORD_TYPE = ${BOBUI_COORD_TYPE}\n")

    bobui_get_build_parts(build_parts)
    string(REPLACE ";" " " build_parts "${build_parts}")
    string(APPEND content "BOBUI_BUILD_PARTS = ${build_parts}\n")

    if(BOBUI_EXTRA_RPATHS)
        list(JOIN BOBUI_EXTRA_RPATHS " " extra_rpaths)
    ...
    bobui_generate_qmake_libraries_pri_content(global ${preliminary_pri_root} ${pri_data_cmake_file})
    ...
    bobui_install(FILES "${qmodule_pri_target_path}" DESTINATION ${INSTALL_MKSPECSDIR})
endfunction()

function(bobui_correct_features out_var features)
    set(corrected_features "")
    foreach(feature ${features})
        get_property(feature_original_name GLOBAL PROPERTY "BOBUI_FEATURE_ORIGINAL_NAME_${feature}")
...
# a custom name).
function(bobui_correct_config out_var config)
    set(corrected_config "")
    foreach(name ${config})
        # Is the config value a known feature?
        get_property(feature_original_name GLOBAL PROPERTY "BOBUI_FEATURE_ORIGINAL_NAME_${name}")
        ...
        if(name MATCHES "^no_(.*)")
            get_property(feature_original_name GLOBAL PROPERTY
                "BOBUI_FEATURE_ORIGINAL_NAME_${CMAKE_MATCH_1}")
            if(feature_original_name)
