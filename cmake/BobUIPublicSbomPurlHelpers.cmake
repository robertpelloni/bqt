...
macro(_bobui_internal_sbom_parse_purl_entry_options prefix arguments_var_name)
    ...
    cmake_parse_arguments(${prefix} "${purl_opt_args}" "${purl_single_args}" "${purl_multi_args}"
        ${${arguments_var_name}})
    _bobui_internal_validate_all_args_are_parsed(${prefix})
endmacro()
...
macro(_bobui_internal_sbom_forward_purl_handling_options args_var_name)
    if(NOT opt_args)
        message(FATAL_ERROR
            "Expected opt_args to be set by _bobui_internal_get_sbom_purl_handling_options")
    endif()
    if(NOT single_args)
        message(FATAL_ERROR
            "Expected single_args to be set by _bobui_internal_get_sbom_purl_handling_options")
    endif()
    if(NOT multi_args)
        message(FATAL_ERROR
            "Expected multi_args to be set by _bobui_internal_get_sbom_purl_handling_options")
    endif()
...
# Handles purl arguments specified to functions like bobui_internal_add_sbom.
#
# Synopsis
#
...
#             PURL_NAMESPACE "harfbuzz"
#             PURL_NAME "harfbuzz"
#             PURL_QUALIFIERS "vcs_url=https://code.bobui.io/bobui/bobuibase"
#         ....
#     PURL_VALUES
...
#
# If no arguments are specified, for bobui entity types (e.g. libraries built as part of BobUI repos),
...
function(_bobui_internal_sbom_handle_purl_values target)
    ...
    foreach(purl_idx IN LISTS purl_entry_indices)
        ...
        _bobui_internal_sbom_parse_purl_entry_options(arg purl_${purl_idx}_args)
    endforeach()

    # Append bobui specific placeholder entries when handling BobUI entities.
    if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_PURL)
        _bobui_internal_sbom_forward_purl_handling_options(purl_handling_args)
        _bobui_internal_sbom_handle_bobui_entity_purl_entries(${purl_handling_args}
            OUT_VAR_IDS bobui_purl_ids
        )
        if(bobui_purl_ids)
            # Create purl placeholder indices for each bobui purl id.
            foreach(bobui_purl_id IN LISTS bobui_purl_ids)
                math(EXPR purl_idx "${purl_idx}+1")
                list(APPEND purl_entry_indices "${purl_idx}")
                list(APPEND purl_${purl_idx}_args PURL_ID "${bobui_purl_id}")
            endforeach()
        endif()
    endif()

    set(bobui_entity_cydx_purl_values "")
    set(bobui_entity_spdx_purl_ext_refs "")

    # When generating purls for BobUI entities targeting Cyclone DX, we prefer the generic purl first,
    ...
    set(bobui_entity_cydx_purl_for_github_id "")
    set(bobui_entity_cydx_purl_for_generic_id "")
    ...
    foreach(purl_idx IN LISTS purl_entry_indices)
        ...
        set(is_bobui_entity_purl FALSE)
        # BobUI entity types get special treatment to gather the required args.
        if(arg___BOBUI_INTERNAL_HANDLE_BOBUI_ENTITY_TYPE_PURL
                AND arg_PURL_ID
                AND arg_PURL_ID IN_LIST bobui_purl_ids)

            set(is_bobui_entity_purl TRUE)
            _bobui_internal_sbom_handle_bobui_entity_purl("${target}"
                ${purl_handling_args}
                PURL_ID "${arg_PURL_ID}"
                OUT_PURL_ARGS bobui_purl_args
            )
            if(bobui_purl_args)
                list(APPEND purl_args "${bobui_purl_args}")
            endif()
        endif()

        _bobui_internal_sbom_assemble_purl(${target}
            ${purl_args}
            OUT_VAR purl_bare
            OUT_VAR_SPDX_EXT_REF package_manager_external_ref_purl
        )

        if(is_bobui_entity_purl)
            if(arg_PURL_ID STREQUAL "GENERIC")
                set(bobui_entity_cydx_purl_for_generic_id "${purl_bare}")
            elseif(arg_PURL_ID STREQUAL "GITHUB")
                set(bobui_entity_cydx_purl_for_github_id "${purl_bare}")
            else()
    ...
    # Add the custom bobui entity purls at the front in the right order for CycloneDX.
    # If they are empty (for non-BobUI entities), nothing will be prepended.
    set(bobui_entity_cydx_purl_values
        ${bobui_entity_cydx_purl_for_generic_id}
        ${bobui_entity_cydx_purl_for_github_id}
    )
    list(PREPEND purl_values ${bobui_entity_cydx_purl_values})

    foreach(purl_value IN LISTS arg_PURL_VALUES)
        _bobui_internal_sbom_get_purl_value_extref(VALUE "${purl_value}"
        ...
        # important one.
        # For now, I deem that the directly specified ones (probably via a bobui_attribution.json
...
#    PURL_SUBPATH
#    PURL_QUALIFIERS
function(_bobui_internal_sbom_assemble_purl target)
    set(opt_args "")
    set(single_args
        OUT_VAR
    ...
    _bobui_internal_sbom_get_purl_value_extref(VALUE "${purl}" OUT_VAR ext_ref_result)
...
function(_bobui_internal_sbom_get_purl_value_extref)
    set(opt_args "")
    set(single_args
        OUT_VAR
        VALUE
