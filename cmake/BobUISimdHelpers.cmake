...
# SIMD: name of the simd architecture, e.g. sse2, avx, neon. A BobUI feature named BOBUI_FEATURE_foo
#       should exist, as well as BOBUI_CFLAGS_foo assignment in BobUICompilerOptimization.cmake.
...
#
function(bobui_internal_add_simd_part target)
    ...
    if ("x${arg_SIMD}" STREQUAL x)
        message(FATAL_ERROR "bobui_add_simd_part needs a SIMD type to be set.")
    endif()

    set(condition "BOBUI_FEATURE_${arg_SIMD}")
    string(TOUPPER "BOBUI_CFLAGS_${arg_SIMD}" simd_flags_var_name)
    set(simd_flags_expanded "")

    # As per mkspecs/features/simd.prf, the arch_haswell SIMD compiler is enabled when
    # qmake's CONFIG contains "avx2", which maps to CMake's BOBUI_FEATURE_avx2.
    ...
    # If the compiler adds them implicitly, they must be present in qmake's BOBUI_CPU_FEATURES as
    # detected by the architecture test, and thus they are present in TEST_subarch_result.
    if("${arg_SIMD}" STREQUAL arch_haswell)
        set(condition "BOBUI_FEATURE_avx2")
        ...
        if("${simd_flags_var_name}" STREQUAL "")
            set(simd_flags_var_name "BOBUI_CFLAGS_AVX2")
        endif()
    ...
    elseif("${arg_SIMD}" STREQUAL avx512common)
        set(condition "BOBUI_FEATURE_avx512cd")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_ARCH_HASWELL}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512F}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512CD}")
        list(REMOVE_DUPLICATES simd_flags_expanded)
    elseif("${arg_SIMD}" STREQUAL avx512core)
        set(condition "BOBUI_FEATURE_avx512cd AND BOBUI_FEATURE_avx512bw AND BOBUI_FEATURE_avx512dq AND BOBUI_FEATURE_avx512vl")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_ARCH_HASWELL}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512F}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512CD}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512BW}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512DQ}")
        list(APPEND simd_flags_expanded "${BOBUI_CFLAGS_AVX512VL}")
        list(REMOVE_DUPLICATES simd_flags_expanded)
    endif()

    bobui_evaluate_config_expression(result ${condition})
    if(${result})
        if(BOBUI_CMAKE_DEBUG_EXTEND_TARGET)
            message("bobui_add_simd_part(${target} SIMD ${arg_SIMD} ...): Evaluated")
        endif()
        ...
        if(BOBUI_CMAKE_DEBUG_EXTEND_TARGET)
            message("bobui_add_simd_part(${target} SIMD ${arg_SIMD} ...): Skipped")
        endif()
    endif()
endfunction()
