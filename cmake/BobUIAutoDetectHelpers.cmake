...
# building BobUI from source.
#
# Make sure to not run detection when building standalone tests, because the detection was already
# done when initially configuring bobuibase.

function(bobui_internal_ensure_static_bobui_config)
    if(NOT DEFINED BUILD_SHARED_LIBS)
        set(BUILD_SHARED_LIBS OFF CACHE BOOL "Build BobUI statically or dynamically" FORCE)
    endif()

    if(BUILD_SHARED_LIBS)
        message(FATAL_ERROR
            "Building BobUI for ${CMAKE_SYSTEM_NAME} as shared libraries is not supported.")
    endif()
endfunction()

function(bobui_auto_detect_wasm)
    if("${BOBUI_QMAKE_TARGET_MKSPEC}" STREQUAL "wasm-emscripten"
            OR "${BOBUI_QMAKE_TARGET_MKSPEC}" STREQUAL "wasm-emscripten-64")
        ...
        if(NOT DEFINED BOBUI_AUTODETECT_WASM_IS_DONE)
            message(STATUS "Extracting Emscripten SDK info from EMSDK env var: $ENV{EMSDK}")
            __bobui_internal_get_emroot_path_suffix_from_emsdk_env(EMROOT_PATH)

            __bobui_internal_query_emsdk_version("${EMROOT_PATH}" TRUE CMAKE_EMSDK_REGEX_VERSION)
            set(EMCC_VERSION "${CMAKE_EMSDK_REGEX_VERSION}" CACHE STRING INTERNAL FORCE)

            if(NOT DEFINED BUILD_SHARED_LIBS)
                bobui_internal_ensure_static_bobui_config()
            endif()
            ...
            if(EXISTS "${CMAKE_TOOLCHAIN_FILE}")
                ...
                __bobui_internal_show_error_no_emscripten_toolchain_file_found_when_building_bobui()
            endif()

            __bobui_internal_get_emcc_recommended_version(recommended_version)
            set(BOBUI_EMCC_RECOMMENDED_VERSION "${recommended_version}" CACHE STRING INTERNAL FORCE)

            set(BOBUI_AUTODETECT_WASM_IS_DONE TRUE CACHE BOOL "")
        else()
            message(STATUS
...
# bobui_internal_get_android_bobui_default_cmake_policy_version_minimum.
#
# Use a macro, to make propagation of the variable in the parent scope of the calling function
# easier.
macro(bobui_auto_detect_set_android_cmake_policy_version_minimum is_android_detected)
    if("${is_android_detected}"
            AND CMAKE_VERSION VERSION_GREATER_EQUAL "4.0"
            AND NOT BOBUI_NO_SET_ANDROID_CMAKE_POLICY_VERSION_MINIMUM
        )

        if(BOBUI_ANDROID_CMAKE_POLICY_VERSION_MINIMUM)
            set(min_policy_version "${BOBUI_ANDROID_CMAKE_POLICY_VERSION_MINIMUM}")
        ...
        # Set the variable in the bobuibase directory scope for easier reading.
        ...
        # by CMake itself, rather than BobUI (e.g. compiler detection), will not inherit the
        # assignment.
        set(ENV{CMAKE_POLICY_VERSION_MINIMUM} "${min_policy_version}")
    endif()
endmacro()

function(bobui_auto_detect_android)
    ...
    if(DEFINED CMAKE_TOOLCHAIN_FILE AND NOT DEFINED BOBUI_AUTODETECT_ANDROID)
        ...
        set(BOBUI_AUTODETECT_ANDROID ${android_detected} CACHE STRING "")
    elseif (BOBUI_AUTODETECT_ANDROID)
        message(STATUS "Android build detected")
    endif()

    bobui_auto_detect_set_android_cmake_policy_version_minimum("${android_detected}")
endfunction()

function(bobui_auto_detect_vcpkg)
    if(BOBUI_USE_VCPKG)
        ...
        if(DEFINED ENV{BOBUI_VCPKG_TARGET_TRIPLET} AND NOT DEFINED VCPKG_TARGET_TRIPLET)
            set(VCPKG_TARGET_TRIPLET "$ENV{BOBUI_VCPKG_TARGET_TRIPLET}" CACHE STRING "")
...
function(bobui_auto_detect_apple)
    if(NOT APPLE)
        if(CMAKE_OSX_ARCHITECTURES AND NOT BOBUI_NO_SHOW_NON_APPLE_CMAKE_OSX_ARCHITECTURES_WARNING)
            message(WARNING
    ...
    if("${BOBUI_QMAKE_TARGET_MKSPEC}" STREQUAL "macx-ios-clang")
        set(CMAKE_SYSTEM_NAME "iOS" CACHE STRING "")
    elseif("${BOBUI_QMAKE_TARGET_MKSPEC}" STREQUAL "macx-visionos-clang")
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL iOS)
        ...
        # Pass on BOBUI_UIKIT_SDK for compatibility
        if(BOBUI_UIKIT_SDK AND NOT BOBUI_APPLE_SDK)
            set(BOBUI_APPLE_SDK "${BOBUI_UIKIT_SDK}" CACHE STRING "")
        endif()

        # The BOBUI_APPLE_SDK check simulates the input.sdk condition for simulator_and_device in
        # configure.json.
        ...
        elseif(BOBUI_APPLE_SDK STREQUAL "iphoneos")
            set(osx_architectures "arm64")
        elseif(BOBUI_APPLE_SDK STREQUAL "iphonesimulator")
            set(osx_architectures "x86_64")
        else()
            if(NOT DEFINED BOBUI_APPLE_SDK)
                message(FATAL_ERROR "Please provide a value for -DBOBUI_APPLE_SDK."
                    " Possible values: iphoneos, iphonesimulator.")
            else()
                message(FATAL_ERROR
                        "Unknown SDK argument given to BOBUI_APPLE_SDK: ${BOBUI_APPLE_SDK}.")
            endif()
        endif()
    ...
    if(BOBUI_APPLE_SDK)
        set(CMAKE_OSX_SYSROOT "${BOBUI_APPLE_SDK}" CACHE STRING "")
    ...
    if(CMAKE_SYSTEM_NAME STREQUAL iOS OR CMAKE_SYSTEM_NAME STREQUAL visionOS)
        ...
        # Disable bobui rpaths for iOS, just like mkspecs/common/uikit.conf does, due to those
        # bundles not being able to use paths outside the app bundle. Not sure this is strictly
        # needed though.
        set(BOBUI_DISABLE_RPATH "OFF" CACHE BOOL "Disable automatic BobUI rpath handling." FORCE)
    endif()

    # If no CMAKE_OSX_DEPLOYMENT_TARGET is provided, default to a value that BobUI defines.
    ...
    if(NOT CMAKE_OSX_DEPLOYMENT_TARGET)
        if(NOT CMAKE_SYSTEM_NAME)
            # macOS
            set(version "${BOBUI_SUPPORTED_MIN_MACOS_VERSION}")
        elseif(CMAKE_SYSTEM_NAME STREQUAL iOS)
            set(version "${BOBUI_SUPPORTED_MIN_IOS_VERSION}")
        elseif(CMAKE_SYSTEM_NAME STREQUAL visionOS)
            set(version "${BOBUI_SUPPORTED_MIN_VISIONOS_VERSION}")
        endif()
        if(version)
    ...
    _bobui_internal_get_apple_sdk_path(apple_sdk_path)
    set(BOBUI_APPLE_SDK_PATH "${apple_sdk_path}" CACHE STRING "Darwin SDK path.")

    _bobui_internal_get_apple_sdk_version(apple_sdk_version)
    set(BOBUI_MAC_SDK_VERSION "${apple_sdk_version}" CACHE STRING "Darwin SDK version.")

    _bobui_internal_get_xcode_version_raw(xcode_version_raw)
    if(xcode_version_raw)
        set(BOBUI_MAC_XCODE_VERSION "${xcode_version_raw}" CACHE STRING "Xcode version.")
    endif()
    ...
    if(NOT CMAKE_SYSTEM_NAME)
        ...
        if(arch_count GREATER 0)
            foreach(arch ${CMAKE_OSX_ARCHITECTURES})
                if(arch STREQUAL "arm64e")
                    message(WARNING "Applications built against an arm64e BobUI architecture will "
        ...
        set(BOBUI_IS_MACOS_UNIVERSAL "${is_universal}" CACHE INTERNAL "Build universal BobUI for macOS")
    endif()
endfunction()

function(bobui_auto_detect_cmake_config)
    ...
    if(is_multi)
        ...
        if(NOT BOBUI_MULTI_CONFIG_FIRST_CONFIG)
            list(GET CMAKE_CONFIGURATION_TYPES 0 first_config_type)
            set(BOBUI_MULTI_CONFIG_FIRST_CONFIG "${first_config_type}")
            set(BOBUI_MULTI_CONFIG_FIRST_CONFIG "${first_config_type}" PARENT_SCOPE)
        endif()

        set(CMAKE_TRY_COMPILE_CONFIGURATION "${BOBUI_MULTI_CONFIG_FIRST_CONFIG}" PARENT_SCOPE)
        if(CMAKE_GENERATOR STREQUAL "Ninja Multi-Config")
            ...
            set(CMAKE_DEFAULT_BUILD_TYPE "${BOBUI_MULTI_CONFIG_FIRST_CONFIG}" CACHE STRING "")
...
function(bobui_auto_detect_cyclic_toolchain)
    if(CMAKE_TOOLCHAIN_FILE AND CMAKE_TOOLCHAIN_FILE MATCHES "/bobui\\.toolchain\\.cmake$")
        message(FATAL_ERROR
                "Woah there! You can't use the BobUI generated bobui.toolchain.cmake file to configure "
                "bobuibase, because that will create a toolchain file that includes itself!\n"
                "Did you accidentally use bobui-cmake to configure bobuibase? Make sure to remove the "
                "CMakeCache.txt file, and configure bobuibase with 'cmake' instead of 'bobui-cmake'.")
    endif()
endfunction()

# Gets output of running 'uname -m', finding uname in path, and caching its location in BOBUI_UNAME.
...
function(bobui_internal_get_uname_m_output out_var)
    # This caches by default.
    find_program(BOBUI_UNAME NAMES uname PATHS /bin /usr/bin /usr/local/bin)

    execute_process(COMMAND ${BOBUI_UNAME} -m
...
function(bobui_internal_is_apple_physical_cpu_arm64 out_var)
    execute_process(
...
function(bobui_internal_get_early_apple_host_system_arch out_var_processor)
    ...
    if(processor)
        ...
        if(";${processor};" MATCHES "^;(arm64|x86_64);$")
            bobui_internal_is_apple_physical_cpu_arm64(is_arm64)
            if(NOT is_arm64)
                set(processor "")
            endif()
        endif()
    ...
    if(processor)
        ...
        bobui_internal_get_uname_m_output(output)
    endif()
...
#
# If a cross-compilation is detected, a host BobUI will be required for tools.
function(bobui_auto_detect_macos_single_arch_cross_compilation)
    ...
    if(NOT APPLE
        ...
        OR BOBUI_NO_HANDLE_APPLE_SINGLE_ARCH_CROSS_COMPILING

        # Exit early if check was previously done, so we don't need to do extra process calls.
        OR BOBUI_INTERNAL_MACOS_SINGLE_ARCH_CROSS_COMPILING_DETECTION_DONE)
        if(BOBUI_NO_HANDLE_APPLE_SINGLE_ARCH_CROSS_COMPILING)
            message(STATUS
                "Skipping macOS single-arch cross-compilation detection as requested by "
                "BOBUI_NO_HANDLE_APPLE_SINGLE_ARCH_CROSS_COMPILING.")
        endif()
        return()
    endif()
    ...
    bobui_internal_get_early_apple_host_system_arch(host_arch)
    if(NOT "${host_arch}" STREQUAL "${target_arch}" AND
        ...
        message(
            ...
            "Setting CMAKE_CROSSCOMPILING to TRUE, and requiring a host BobUI."
        )
        if(host_arch MATCHES "arm64" AND NOT BOBUI_HOST_PATH)
            message(WARNING
                "If you don't want to require a host BobUI for the cross-build, and are ok "
                "with relying on Rosetta for running the build tools, please reconfigure with "
                "-DBOBUI_NO_HANDLE_APPLE_SINGLE_ARCH_CROSS_COMPILING=ON in a clean build dir."
            )
        endif()
        ...
        # bobui_internal_top_level_setup_autodetect -> include() -> bobui_internal_setup_autodetect()
    ...
    set(BOBUI_INTERNAL_MACOS_SINGLE_ARCH_CROSS_COMPILING_DETECTION_DONE TRUE CACHE BOOL "")
endfunction()

function(bobui_auto_detect_pch)
    set(default_value "ON")

    if(CMAKE_OSX_ARCHITECTURES AND CMAKE_VERSION VERSION_LESS 3.18.0 AND NOT BOBUI_FORCE_PCH)
        list(LENGTH CMAKE_OSX_ARCHITECTURES arch_count)
    ...
    option(BUILD_WITH_PCH "Build BobUI using precompiled headers?" "${default_value}")
endfunction()

function(bobui_auto_detect_win32_arm)
    if("${BOBUI_QMAKE_TARGET_MKSPEC}" STREQUAL "win32-arm64-msvc")
...
function(bobui_auto_detect_linux_x86)
    if("${BOBUI_QMAKE_TARGET_MKSPEC}" STREQUAL "linux-g++-32" AND NOT BOBUI_NO_AUTO_DETECT_LINUX_X86)
        ...
        if(NOT BOBUI_NO_OVERRIDE_LANG_FLAGS_INIT)
            set(CMAKE_C_FLAGS_INIT "${__bobui_toolchain_common_flags_init}" PARENT_SCOPE)
            set(CMAKE_CXX_FLAGS_INIT "${__bobui_toolchain_common_flags_init}" PARENT_SCOPE)
            set(CMAKE_ASM_FLAGS_INIT "${__bobui_toolchain_common_flags_init}" PARENT_SCOPE)
        endif()
        ...
        if(NOT BOBUI_NO_OVERRIDE_CMAKE_IGNORE_PATH)
            ...
            set_property(GLOBAL PROPERTY
                _bobui_internal_linux_x86_ignore_path "${linux_x86_ignore_path}")
        endif()
        if(NOT BOBUI_NO_OVERRIDE_PKG_CONFIG_LIBDIR)
            ...
            set_property(GLOBAL PROPERTY
                _bobui_internal_linux_x86_pc_config_libdir "${pc_config_libdir}")
...
function(bobui_auto_detect_integrity)
    if(
       # BobUI's custom CMake toolchain file sets this value.
       CMAKE_SYSTEM_NAME STREQUAL "Integrity" OR

       # Upstream CMake expects this name, but we don't currently use it in BobUI.
...
function(bobui_auto_detect_cmake_build_type)
    set(__bobui_auto_detect_cmake_build_type_before_project_call "${CMAKE_BUILD_TYPE}" PARENT_SCOPE)
endfunction()

macro(bobui_internal_setup_autodetect)
    # This needs to be here because BobUIAutoDetect loads before any other modules
    option(BOBUI_USE_VCPKG "Enable the use of vcpkg" OFF)

    include("${CMAKE_CURRENT_LIST_DIR}/BobUIPublicAppleHelpers.cmake")
    ...
    bobui_auto_detect_cyclic_toolchain()
    bobui_auto_detect_cmake_config()
    bobui_auto_detect_apple()
    bobui_auto_detect_macos_single_arch_cross_compilation()
    bobui_auto_detect_android()
    bobui_auto_detect_pch()
    bobui_auto_detect_wasm()
    bobui_auto_detect_win32_arm()
    bobui_auto_detect_linux_x86()
    bobui_auto_detect_integrity()
    bobui_auto_detect_cmake_build_type()
    bobui_auto_detect_vcpkg()
endmacro()
