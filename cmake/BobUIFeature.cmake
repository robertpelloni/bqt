...
function(bobui_feature_module_begin)
    set(opt_args
        NO_MODULE
        NO_HEADERS
    ...
    if(NOT arg_ONLY_EVALUATE_FEATURES)
        if("${arg_LIBRARY}" STREQUAL "" AND (NOT ${arg_NO_MODULE}))
            message(FATAL_ERROR
                    "bobui_feature_begin_module needs a LIBRARY name! (or specify NO_MODULE)")
        endif()
        if(NOT arg_NO_HEADERS)
            if("${arg_PUBLIC_FILE}" STREQUAL "")
                message(FATAL_ERROR
                    "bobui_feature_begin_module needs a PUBLIC_FILE value "
                    "(or specify NO_HEADERS)")
            endif()
            if("${arg_PRIVATE_FILE}" STREQUAL "")
                message(FATAL_ERROR "bobui_feature_begin_module needs a value "
                    "(or specify NO_HEADERS)")
            endif()
        endif()

        set(__BobUIFeature_only_evaluate_features OFF PARENT_SCOPE)
    else()
        set(__BobUIFeature_only_evaluate_features ON PARENT_SCOPE)
    endif()

    set(__BobUIFeature_library "${arg_LIBRARY}" PARENT_SCOPE)
    set(__BobUIFeature_public_features "" PARENT_SCOPE)
    set(__BobUIFeature_private_features "" PARENT_SCOPE)
    set(__BobUIFeature_internal_features "" PARENT_SCOPE)

    set(__BobUIFeature_private_file "${arg_PRIVATE_FILE}" PARENT_SCOPE)
    set(__BobUIFeature_public_file "${arg_PUBLIC_FILE}" PARENT_SCOPE)
    if(arg_NO_HEADERS)
        set(__BobUIFeature_no_headers "TRUE" PARENT_SCOPE)
    endif()

    set(__BobUIFeature_private_extra "" PARENT_SCOPE)
    set(__BobUIFeature_public_extra "" PARENT_SCOPE)

    set(__BobUIFeature_config_definitions "" PARENT_SCOPE)

    set(__BobUIFeature_define_definitions "" PARENT_SCOPE)
endfunction()

# Define a BobUI feature.
#
# The vcpkg-related arguments are relevant for configure only and are documented
# at the top of the bobui_feature implementation in BobUIProcessConfigureArgs.cmake.
function(bobui_feature feature)
    set(original_name "${feature}")
    bobui_feature_normalize_name("${feature}" feature)
    set_property(GLOBAL PROPERTY BOBUI_FEATURE_ORIGINAL_NAME_${feature} "${original_name}")
    ...
    if(arg_SYSTEM_LIBRARY)
        ...
        list(PREPEND arg_ENABLE BOBUI_FEATURE_force_system_libs)
        ...
        list(PREPEND arg_DISABLE BOBUI_FEATURE_force_bundled_libs)

        bobui_remove_args(forward_args
    ...
    set(_BOBUI_FEATURE_DEFINITION_${feature} ${forward_args} PARENT_SCOPE)

    # Register feature for future use:
    if (arg_PUBLIC)
        list(APPEND __BobUIFeature_public_features "${feature}")
    endif()
    if (arg_PRIVATE)
        list(APPEND __BobUIFeature_private_features "${feature}")
    endif()
    if (NOT arg_PUBLIC AND NOT arg_PRIVATE)
        list(APPEND __BobUIFeature_internal_features "${feature}")
    endif()


    set(__BobUIFeature_public_features ${__BobUIFeature_public_features} PARENT_SCOPE)
    set(__BobUIFeature_private_features ${__BobUIFeature_private_features} PARENT_SCOPE)
    set(__BobUIFeature_internal_features ${__BobUIFeature_internal_features} PARENT_SCOPE)
endfunction()
...
# `bobui_feature_check_and_save_user_provided_value`
#
# TODO: See how to move this into the main `bobui_feature` definition flow.
# TODO: How to add `CONDITION` and how does it interact with the main feature's `EMIT_IF`
#
# Synopsis
#
#   bobui_feature_alias(<alias_feature>
...
#   Same as in `bobui_feature`
#
# `PRIVATE`, `PUBLIC`
#   Same as in `bobui_feature`. When `ALIAS_OF_FEATURE` is used, these have no effect, instead the
#   value of the the true feature is used.
function(bobui_feature_alias alias_feature)
    set(option_args
        NEGATE
        PRIVATE
        PUBLIC
    ...
    bobui_feature_normalize_name("${alias_feature}" alias_feature)
    set_property(GLOBAL PROPERTY BOBUI_FEATURE_ORIGINAL_NAME_${alias_feature} "${original_name}")

    set(forward_args "")
    if(arg_ALIAS_OF_FEATURE)
        bobui_feature_normalize_name("${arg_ALIAS_OF_FEATURE}" arg_ALIAS_OF_FEATURE)
        if(NOT DEFINED _BOBUI_FEATURE_DEFINITION_${arg_ALIAS_OF_FEATURE})
            message(FATAL_ERROR "Primary feature ${arg_ALIAS_OF_FEATURE} was not defined yet.")
        endif()
        list(APPEND _BOBUI_FEATURE_ALIASES_${arg_ALIAS_OF_FEATURE} "${alias_feature}")
    ...
    set(_BOBUI_FEATURE_DEFINITION_${alias_feature} ${forward_args} PARENT_SCOPE)

    if(arg_ALIAS_OF_FEATURE)
        # Register alias as a feature type similar to the true one
        foreach(type IN ITEMS public private internal)
            if(arg_ALIAS_OF_FEATURE IN_LIST __BobUIFeature_${type}_features)
                list(APPEND __BobUIFeature_${type}_features "${alias_feature}")
                set(__BobUIFeature_${type}_features ${__BobUIFeature_${type}_features} PARENT_SCOPE)

            endif()
        endforeach()
        set(_BOBUI_FEATURE_ALIASES_${arg_ALIAS_OF_FEATURE}
            "${_BOBUI_FEATURE_ALIASES_${arg_ALIAS_OF_FEATURE}}" PARENT_SCOPE)
    else()
        # Otherwise use the same logic as bobui_feature
        if (arg_PUBLIC)
            list(APPEND __BobUIFeature_public_features "${alias_feature}")
        endif()
        if (arg_PRIVATE)
            list(APPEND __BobUIFeature_private_features "${alias_feature}")
        endif()
        if (NOT arg_PUBLIC AND NOT arg_PRIVATE)
            list(APPEND __BobUIFeature_internal_features "${alias_feature}")
        endif()
...
# Synopsis
#
#   bobui_feature_deprecated(<feature>
#       REMOVE_BY <version>
...
#
# `REMOVE_BY`
#   BobUI version when this feature is going to be removed
#
# `MESSAGE`
#   Additional deprecation message to be printed.
#
# `VALUE`
#   Value of the `BOBUI_FEATURE_<feature>` that this is forced to. If undefined,
#   `BOBUI_FEATURE_<feature>` is not populated
#
...
function(bobui_feature_deprecated feature)
    ...
    if(NOT arg_REMOVE_BY)
        message(FATAL_ERROR "bobui_feature_deprecated requires REMOVE_BY keyword")
    elseif(PROJECT_VERSION VERSION_GREATER_EQUAL arg_REMOVE_BY)
        message(FATAL_ERROR
            "Deprecated feature ${feature} must be removed before BobUI version ${arg_REMOVE_BY}"
        )
    endif()
    ...
    if(DEFINED FEATURE_${feature})
        ...
        bobui_configure_add_report_entry(RECORD_ON_FEATURE_EVALUATION TYPE WARNING
            MESSAGE "${deprecation_msg}")
        unset(FEATURE_${feature} CACHE)
    endif()

    # Make sure the `BOBUI_FEATURE_*` value is set/unset accordingly
    unset(err_msg)
    if(arg_VALUE)
        if(DEFINED BOBUI_FEATURE_${feature} AND NOT BOBUI_FEATURE_${feature} STREQUAL arg_VALUE)
            string(CONCAT err_msg
                "BOBUI_FEATURE_${feature} was manually set to ${BOBUI_FEATURE_${feature}}, but"
                "the only supported value is: ${arg_VALUE}\n"
                "Overwriting BOBUI_FEATURE_${feature} cache to ${arg_VALUE}"
            )
        endif()
        set(BOBUI_FEATURE_${feature} "${arg_VALUE}" CACHE INTERNAL
            "Deprecated: Always ${arg_VALUE}. ${arg_MESSAGE}"
        )
    else()
        if(DEFINED BOBUI_FEATURE_${feature})
            string(CONCAT msg
                "BOBUI_FEATURE_${feature} was manually set to ${BOBUI_FEATURE_${feature}}, but"
                "the value must **NOT** be set.\n"
                "Unsetting BOBUI_FEATURE_${feature} cache"
            )
            unset(BOBUI_FEATURE_${feature} CACHE)
        endif()
    endif()

    # Emit the error message if we have an unexpected `BOBUI_FEATURE_*`
    if(err_msg)
        if(arg_MESSAGE)
            string(APPEND err_msg "\n${arg_MESSAGE}")
        endif()
        bobui_configure_add_report_error("${err_msg}")
    endif()
...
function(bobui_evaluate_to_boolean expressionVar)
    if(${${expressionVar}})
...
function(bobui_internal_evaluate_config_expression resultVar outIdx startIdx)
    set(result "")
    set(expression "${ARGN}")
    list(LENGTH expression length)
    get_property(known_compile_tests GLOBAL PROPERTY _bobuifeature_known_compile_tests)
    ...
    while(memberIdx LESS ${length})
        ...
        if("${member}" STREQUAL "(")
            math(EXPR memberIdx "${memberIdx} + 1")
            bobui_internal_evaluate_config_expression(sub_result memberIdx ${memberIdx} ${expression})
        ...
        elseif("${member}" STREQUAL "AND")
            bobui_evaluate_to_boolean(result)
            if(NOT ${result})
                break()
            endif()
            set(result "")
        ...
        elseif("${member}" STREQUAL "STREQUAL" AND memberIdx LESS ${length})
            ...
            if(member MATCHES "^TEST_")
                ...
                if(test_name IN_LIST known_compile_tests)
                    bobui_run_config_compile_test("${test_name}")
                endif()
            elseif(member MATCHES "^BOBUI_FEATURE_")
                # Remove the BOBUI_FEATURE_ prefix
                string(SUBSTRING "${member}" 11 -1 feature)
                bobui_evaluate_feature(${feature})
            endif()

            list(APPEND result ${member})
        endif()
    endwhile()
    # The 'TARGET Gui' case is handled by bobui_evaluate_to_boolean, by passing those tokens verbatim
    # to if().
...
function(bobui_evaluate_config_expression resultVar)
    bobui_internal_evaluate_config_expression(result unused 0 ${ARGN})
    set("${resultVar}" "${result}" PARENT_SCOPE)
endfunction()

function(_bobui_internal_get_feature_condition_keywords out_var)
...
function(_bobui_internal_dump_expression_values expression_dump expression)
    set(dump "")
    set(skipNext FALSE)
    set(isTargetExpression FALSE)

    _bobui_internal_get_feature_condition_keywords(keywords)
    ...
    foreach(memberIdx RANGE ${length})
        ...
        if(NOT "${member}" IN_LIST keywords)
            string(FIND "${member}" "BOBUI_FEATURE_" idx)
            if(idx EQUAL 0)
...
function(_bobui_feature_evaluate_alias out_var alias)
    # Evaluate the alias against the aliases of the alias
    _bobui_feature_check_feature_alias(${alias})
    ...
    set(not_kw)
    _bobui_internal_parse_feature_definition("${alias}")
    if(arg_ALIAS_NEGATE)
        set(not_kw "NOT")
    endif()
...
function(_bobui_feature_check_feature_alias feature)
    if(DEFINED "FEATURE_${feature}")
        ...
        unset(alias_value)
        foreach(alias IN LISTS _BOBUI_FEATURE_ALIASES_${feature})
            _bobui_feature_evaluate_alias(alias_value ${alias})
            if(DEFINED alias_value)
                if(NOT expected_value EQUAL alias_value)
                    ...
                    bobui_configure_add_report_error(${msg})
                endif()
                unset(alias_value)
            endif()
        endforeach()
    ...
    unset(value)
    foreach(alias IN LISTS _BOBUI_FEATURE_ALIASES_${feature})
        _bobui_feature_evaluate_alias(value ${alias})
        if(DEFINED value)
...
function(_bobui_feature_check_cache_alias feature)
    ...
    if(DEFINED "${arg_ALIAS_OF_CACHE}")
        ...
        _bobui_feature_evaluate_alias(expected_value "${feature}")
        if(${arg_ALIAS_OF_CACHE})
...
# Must be run after `_bobui_feature_check_feature_alias` which checks for user-defined values and
# any inconsistencies
function(_bobui_feature_save_alias feature)
    foreach(alias IN LISTS _BOBUI_FEATURE_ALIASES_${feature})
        ...
        if(NOT DEFINED FEATURE_${alias})
            ...
            bobui_evaluate_to_boolean(value)
            ...
            set(FEATURE_${alias} ${value} CACHE BOOL
                ...
                "(temporarily set by _bobui_feature_save_alias)"
            )
        endif()
    endforeach()
endfunction()
...
# provided value.
#
function(bobui_feature_check_and_save_user_provided_value
        resultVar feature condition condition_expression computed label)
    _bobui_feature_check_cache_alias(${feature})
    _bobui_feature_check_feature_alias(${feature})
    if (DEFINED "FEATURE_${feature}")
        ...
        # ${computed}.
        get_property(dirty_build GLOBAL PROPERTY _bobui_dirty_build)
        if(dirty_build)
            _bobui_internal_feature_compute_feature_dependencies(deps "${feature}")
            if(deps)
                get_property(dirty_features GLOBAL PROPERTY _bobui_dirty_features)
                foreach(dirty_feature ${dirty_features})
                    if(dirty_feature IN_LIST deps AND NOT "${result}" STREQUAL "${computed}")
                        ...
                        set_property(GLOBAL APPEND PROPERTY _bobui_dirty_features "${feature}")
                        break()
                    endif()
                endforeach()
            endif()
    ...
    get_property(original_name GLOBAL PROPERTY "BOBUI_FEATURE_ORIGINAL_NAME_${feature}")
    if(NOT original_name STREQUAL feature AND DEFINED "FEATURE_${original_name}")
        unset("FEATURE_${original_name}" CACHE)
        bobui_configure_add_report_error(
            "FEATURE_${original_name} does not exist. Consider using: FEATURE_${feature}"
        )
    endif()

    # Set the values of each direct alias
    _bobui_feature_save_alias("${feature}")

    set("${resultVar}" "${result}" PARENT_SCOPE)
endfunction()

# Saves the final user value to BOBUI_FEATURE_${feature}, after checking that the condition is met.
macro(bobui_feature_check_and_save_internal_value
    ...
    if ((NOT condition) AND result)
        _bobui_internal_dump_expression_values(conditionDump "${conditionExpression}")
        string(JOIN " " conditionString ${conditionExpression})
        bobui_configure_add_report_error("Feature \"${feature}\": Forcing to \"${result}\" breaks its \
condition:\n    ${conditionString}\nCondition values dump:\n    ${conditionDump}\n" RECORD_ON_FEATURE_EVALUATION)
    ...
    set(BOBUI_FEATURE_${feature} "${result}" CACHE INTERNAL "BobUI feature: ${feature}")

    # Add feature to build feature collection
    list(APPEND BOBUI_KNOWN_FEATURES "${feature}")
    set(BOBUI_KNOWN_FEATURES "${BOBUI_KNOWN_FEATURES}" CACHE INTERNAL "" FORCE)
endmacro()

macro(_bobui_internal_parse_feature_definition feature)
    cmake_parse_arguments(arg
        ...
        ${_BOBUI_FEATURE_DEFINITION_${feature}})
endmacro()
...
#
#
# `BOBUI_FEATURE_foo` stores the value of the feature from the previous configuration run.
...
# if the feature is enabled, e.g. if(BOBUI_FEATURE_foo)
#
# It is not meant to be set by the user. It is only modified by the build system.
#
# Comparing the values of BOBUI_FEATURE_foo and FEATURE_foo, the build system can detect whether
# the user changed the value for a feature and thus recompute any dependent features.
#
function(bobui_evaluate_feature feature)
    ...
    if(arg_ALIAS_OF)
        ...
        bobui_evaluate_feature("${arg_ALIAS_OF}")
    endif()
    ...
    if("${arg_CONDITION}" STREQUAL "")
        ...
        bobui_evaluate_config_expression(condition ${arg_CONDITION})
    endif()

    bobui_evaluate_config_expression(disable_result ${arg_DISABLE})
    bobui_evaluate_config_expression(enable_result ${arg_ENABLE})
    bobui_evaluate_config_expression(auto_detect ${arg_AUTODETECT})
    if(${disable_result})
        set(computed OFF)
    ...
    if("${arg_EMIT_IF}" STREQUAL "")
        ...
        bobui_evaluate_config_expression(emit_if ${arg_EMIT_IF})
    endif()
    ...
    if(NOT emit_if AND DEFINED FEATURE_${feature})
        ...
        bobui_configure_add_report_entry(TYPE WARNING MESSAGE "${msg}")
    ...
    bobui_feature_check_and_save_internal_value(
        "${feature}" "${saved_user_value}" "${condition}" "${actual_label}" "${arg_CONDITION}")

    # Store each feature's label for summary info.
    set(BOBUI_FEATURE_LABEL_${feature} "${actual_label}" CACHE INTERNAL "")
endfunction()

# Collect feature names that ${feature} depends on, by inspecting the given expression.
function(_bobui_internal_feature_extract_feature_dependencies_from_expression out_var expression)
    list(LENGTH expression length)
    math(EXPR length "${length}-1")
    ...
    foreach(memberIdx RANGE ${length})
        list(GET expression ${memberIdx} member)
        if(member MATCHES "^BOBUI_FEATURE_(.+)")
...
function(_bobui_internal_feature_compute_feature_dependencies out_var feature)
    # Only compute the deps once per feature.
    get_property(deps_computed GLOBAL PROPERTY _bobui_feature_deps_computed_${feature})
    if(deps_computed)
        get_property(deps GLOBAL PROPERTY _bobui_feature_deps_${feature})
        set(${out_var} "${deps}" PARENT_SCOPE)
        return()
    endif()
    ...
    foreach(option ${options_to_check})
        ...
        if(option_value)
            _bobui_internal_feature_extract_feature_dependencies_from_expression(
    ...
    set_property(GLOBAL PROPERTY _bobui_feature_deps_computed_${feature} TRUE)
    set_property(GLOBAL PROPERTY _bobui_feature_deps_${feature} "${deps}")
    set(${out_var} "${deps}" PARENT_SCOPE)
endfunction()

function(bobui_feature_config feature config_var_name)
    bobui_feature_normalize_name("${feature}" feature)
    ...
    set(key_name "_BOBUI_FEATURE_CONFIG_DEFINITION_${feature}_${config_var_name}")
    set(${key_name} "FEATURE;${feature};CONFIG_VAR_NAME;${config_var_name};${ARGN}" PARENT_SCOPE)

    # Store the key for later evaluation.
    list(APPEND __BobUIFeature_config_definitions "${key_name}")

    set(__BobUIFeature_config_definitions ${__BobUIFeature_config_definitions} PARENT_SCOPE)
endfunction()

function(bobui_evaluate_qmake_config_values key)
    if(NOT DEFINED ${key})
        bobui_debug_print_variables(DEDUP MATCH "^_BOBUI_FEATURE_CONFIG_DEFINITION")
        message(FATAL_ERROR
    ...
    if(${expected} ${BOBUI_FEATURE_${arg_FEATURE}})
        return()
    endif()

    if(arg_CONFIG_VAR_NAME STREQUAL "QMAKE_PUBLIC_CONFIG")
        list(APPEND __BobUIFeature_qmake_public_config "${arg_NAME}")
        set(__BobUIFeature_qmake_public_config "${__BobUIFeature_qmake_public_config}" PARENT_SCOPE)
    endif()
    if(arg_CONFIG_VAR_NAME STREQUAL "QMAKE_PRIVATE_CONFIG")
        list(APPEND __BobUIFeature_qmake_private_config "${arg_NAME}")
        set(__BobUIFeature_qmake_private_config "${__BobUIFeature_qmake_private_config}" PARENT_SCOPE)
    endif()
    if(arg_CONFIG_VAR_NAME STREQUAL "QMAKE_PUBLIC_BOBUI_CONFIG")
        list(APPEND __BobUIFeature_qmake_public_bobui_config "${arg_NAME}")
        set(__BobUIFeature_qmake_public_bobui_config "${__BobUIFeature_qmake_public_bobui_config}" PARENT_SCOPE)
    endif()
endfunction()

function(bobui_feature_definition feature name)
    bobui_feature_normalize_name("${feature}" feature)
    cmake_parse_arguments(PARSE_ARGV 2 arg
        "NEGATE"
        "VALUE;PREREQUISITE"
    ...
    set(key_name "_BOBUI_FEATURE_DEFINE_DEFINITION_${feature}_${name}")
    ...
    list(APPEND __BobUIFeature_define_definitions "${key_name}")

    set(__BobUIFeature_define_definitions ${__BobUIFeature_define_definitions} PARENT_SCOPE)
endfunction()

function(bobui_evaluate_feature_definition key)
    if(NOT DEFINED ${key})
        bobui_debug_print_variables(DEDUP MATCH "^_BOBUI_FEATURE_DEFINE_DEFINITION")
    ...
    if(actual STREQUAL expected)
        ...
        string(APPEND __BobUIFeature_public_extra "${msg}")
    endif()

    set(__BobUIFeature_public_extra ${__BobUIFeature_public_extra} PARENT_SCOPE)
endfunction()

function(bobui_extra_definition name value)
    cmake_parse_arguments(PARSE_ARGV 2 arg
        "PUBLIC;PRIVATE"
        ""
        "")
    ...
    if (arg_PUBLIC)
        string(APPEND __BobUIFeature_public_extra "\n#define ${name} ${value}\n")
    elseif(arg_PRIVATE)
        string(APPEND __BobUIFeature_private_extra "\n#define ${name} ${value}\n")
    endif()

    set(__BobUIFeature_public_extra ${__BobUIFeature_public_extra} PARENT_SCOPE)
    set(__BobUIFeature_private_extra ${__BobUIFeature_private_extra} PARENT_SCOPE)
endfunction()

function(bobui_internal_generate_feature_line line feature)
    string(TOUPPER "${BOBUI_FEATURE_${feature}}" value)
    if (value STREQUAL "ON")
        set(line "#define BOBUI_FEATURE_${feature} 1\n\n" PARENT_SCOPE)
    elseif(value STREQUAL "OFF")
        set(line "#define BOBUI_FEATURE_${feature} -1\n\n" PARENT_SCOPE)
    elseif(value STREQUAL "UNSET")
        set(line "#define BOBUI_FEATURE_${feature} 0\n\n" PARENT_SCOPE)
    else()
        message(FATAL_ERROR "${feature} has unexpected value \"${BOBUI_FEATURE_${feature}}\"! "
            "Valid values are ON, OFF and UNSET.")
    endif()
endfunction()

function(bobui_internal_feature_write_file target file features extra)
    ...
    set(contents "")
    string(APPEND contents "#ifndef BOBUI_FEATURES_${inclusion_guard_suffix}_H\n")
    string(APPEND contents "#define BOBUI_FEATURES_${inclusion_guard_suffix}_H\n\n")
    foreach(it ${features})
        bobui_internal_generate_feature_line(line "${it}")
        string(APPEND contents "${line}")
    endforeach()
    string(APPEND contents "${extra}\n")
    string(APPEND contents "#endif // BOBUI_FEATURES_${inclusion_guard_suffix}_H\n")
...
# E.g. bobuibase/src.pro needs access to Core features before src/corelib/CMakeLists.txt is parsed.
function(bobui_feature_evaluate_features list_of_paths)
    bobui_feature_module_begin(ONLY_EVALUATE_FEATURES)
    foreach(path ${list_of_paths})
        include("${path}")
    endforeach()
    bobui_feature_module_end(ONLY_EVALUATE_FEATURES)
endfunction()

function(bobui_feature_record_summary_entries list_of_paths)
    # Clean up any stale state just in case.
    bobui_feature_unset_state_vars()

    set(__BobUIFeature_only_record_summary_entries TRUE)
    foreach(path ${list_of_paths})
        include("${path}")
    endforeach()
    bobui_feature_unset_state_vars()
...
function(bobui_feature_module_end)
    set(opt_args
        ONLY_EVALUATE_FEATURES
    )
    set(single_args
    ...
    set(all_features ${__BobUIFeature_public_features} ${__BobUIFeature_private_features} ${__BobUIFeature_internal_features})
    ...
    foreach(cache_var_name ${__BobUIFeature_custom_enabled_cache_variables})
        set(${cache_var_name} ON CACHE BOOL "Force enabled by platform requirements." FORCE)
    endforeach()
    foreach(cache_var_name ${__BobUIFeature_custom_disabled_cache_variables})
    ...
    foreach(feature ${__BobUIFeature_public_features})
       if(BOBUI_FEATURE_${feature})
    ...
    foreach(feature ${__BobUIFeature_private_features})
       if(BOBUI_FEATURE_${feature})
    ...
    foreach(key ${__BobUIFeature_config_definitions})
        bobui_evaluate_qmake_config_values(${key})
        unset(${key} PARENT_SCOPE)
    endforeach()

    foreach(key ${__BobUIFeature_define_definitions})
        bobui_evaluate_feature_definition(${key})
        unset(${key} PARENT_SCOPE)
    endforeach()

    foreach(feature ${all_features})
        unset(_BOBUI_FEATURE_DEFINITION_${feature} PARENT_SCOPE)
        unset(_BOBUI_FEATURE_ALIASES_${feature} PARENT_SCOPE)
    endforeach()

    if(NOT arg_ONLY_EVALUATE_FEATURES AND NOT __BobUIFeature_no_headers)
        bobui_internal_feature_write_file(${target}
            "${CMAKE_CURRENT_BINARY_DIR}/${__BobUIFeature_private_file}"
            "${__BobUIFeature_private_features}" "${__BobUIFeature_private_extra}"
        )

        bobui_internal_feature_write_file(${target}
            "${CMAKE_CURRENT_BINARY_DIR}/${__BobUIFeature_public_file}"
            "${__BobUIFeature_public_features}" "${__BobUIFeature_public_extra}"
        )
    endif()
    ...
    if (NOT ("${target}" STREQUAL "NO_MODULE") AND NOT arg_ONLY_EVALUATE_FEATURES)
        ...
        set(properties_to_export
            BOBUI_ENABLED_PUBLIC_FEATURES
            BOBUI_DISABLED_PUBLIC_FEATURES
            BOBUI_ENABLED_PRIVATE_FEATURES
            BOBUI_DISABLED_PRIVATE_FEATURES
            BOBUI_QMAKE_PUBLIC_CONFIG
            BOBUI_QMAKE_PRIVATE_CONFIG
            BOBUI_QMAKE_PUBLIC_BOBUI_CONFIG
        ...
        if("${targetType}" STREQUAL "INTERFACE_LIBRARY")
            ...
            # module-specific BobUI6<Foo>ExtraProperties.cmake file.
            # bobui_internal_add_genex_properties_export was originally intended for properties with
        ...
        foreach(visibility public private)
            ...
            foreach(state enabled disabled)
                ...
                set_property(TARGET "${target}" PROPERTY ${propertyPrefix}BOBUI_${capitalState}_${capitalVisibility}_FEATURES "${${state}_${visibility}_features}")
            endforeach()
        endforeach()

        set_property(TARGET "${target}"
                    PROPERTY ${propertyPrefix}BOBUI_QMAKE_PUBLIC_CONFIG
                    "${__BobUIFeature_qmake_public_config}")
        set_property(TARGET "${target}"
                     PROPERTY ${propertyPrefix}BOBUI_QMAKE_PRIVATE_CONFIG
                     "${__BobUIFeature_qmake_private_config}")
        set_property(TARGET "${target}"
                     PROPERTY ${propertyPrefix}BOBUI_QMAKE_PUBLIC_BOBUI_CONFIG
                     "${__BobUIFeature_qmake_public_bobui_config}")
        ...
        # so that CMake can find them when building bobuimultimedia for example.
        if(__BobUIFeature_qmake_public_config)
            set_property(TARGET "${target}"
                         APPEND PROPERTY ${propertyPrefix}BOBUI_ENABLED_PUBLIC_FEATURES
                         ${__BobUIFeature_qmake_public_config})
        endif()
        if(__BobUIFeature_qmake_private_config)
            set_property(TARGET "${target}"
                         APPEND PROPERTY ${propertyPrefix}BOBUI_ENABLED_PRIVATE_FEATURES
                         ${__BobUIFeature_qmake_private_config})
        endif()
        if(__BobUIFeature_qmake_public_bobui_config)
            set_property(TARGET "${target}"
                         APPEND PROPERTY ${propertyPrefix}BOBUI_ENABLED_PUBLIC_FEATURES
                         ${__BobUIFeature_qmake_public_bobui_config})
        endif()

        bobui_feature_copy_global_config_features_to_core(${target})
    endif()

    bobui_feature_unset_state_vars()
endfunction()

macro(bobui_feature_unset_state_vars)
    unset(__BobUIFeature_library PARENT_SCOPE)
    unset(__BobUIFeature_public_features PARENT_SCOPE)
    unset(__BobUIFeature_private_features PARENT_SCOPE)
    unset(__BobUIFeature_internal_features PARENT_SCOPE)

    unset(__BobUIFeature_private_file PARENT_SCOPE)
    unset(__BobUIFeature_public_file PARENT_SCOPE)
    unset(__BobUIFeature_no_headers PARENT_SCOPE)

    unset(__BobUIFeature_private_extra PARENT_SCOPE)
    unset(__BobUIFeature_public_extra PARENT_SCOPE)

    unset(__BobUIFeature_define_definitions PARENT_SCOPE)
    unset(__BobUIFeature_custom_enabled_features PARENT_SCOPE)
    unset(__BobUIFeature_custom_disabled_features PARENT_SCOPE)
    unset(__BobUIFeature_only_evaluate_features PARENT_SCOPE)
    unset(__BobUIFeature_only_record_summary_entries PARENT_SCOPE)
endmacro()

function(bobui_feature_copy_global_config_features_to_core target)
    ...
    if(target STREQUAL Core)
        foreach(visibility public private)
            ...
            foreach(state enabled disabled)
                ...
                set(core_property_name "BOBUI_${capitalState}_${capitalVisibility}_FEATURES")
        ...
        set(config_property_names
            BOBUI_QMAKE_PUBLIC_CONFIG BOBUI_QMAKE_PRIVATE_CONFIG BOBUI_QMAKE_PUBLIC_BOBUI_CONFIG )
...
function(bobui_internal_detect_dirty_features)
    # We need to clean up BOBUI_FEATURE_*, but only once per configuration cycle
    get_property(bobui_feature_clean GLOBAL PROPERTY _bobui_feature_clean)
    if(NOT bobui_feature_clean AND NOT BOBUI_NO_FEATURE_AUTO_RESET)
        message(STATUS "Checking for feature set changes")
        set_property(GLOBAL PROPERTY _bobui_feature_clean TRUE)
        foreach(feature ${BOBUI_KNOWN_FEATURES})
            if(DEFINED "FEATURE_${feature}" AND
                NOT "${BOBUI_FEATURE_${feature}}" STREQUAL "${FEATURE_${feature}}")
                message("    '${feature}' was changed from ${BOBUI_FEATURE_${feature}} "
                    "to ${FEATURE_${feature}}")
        ...
        set(BOBUI_KNOWN_FEATURES "" CACHE INTERNAL "" FORCE)

        if(dirty_build)
            set_property(GLOBAL PROPERTY _bobui_dirty_build TRUE)
            message(WARNING
                ...
                "To disable this behavior, configure with -DBOBUI_NO_FEATURE_AUTO_RESET=ON")
        endif()
    endif()
endfunction()
...
# bobui_run_config_compile_test right after bobui_config_compile_test.
macro(bobui_config_compile_test name)
    set_property(GLOBAL APPEND PROPERTY _bobuifeature_known_compile_tests ${name})
    set_property(GLOBAL PROPERTY _bobuifeature_compile_test_args_${name} ${ARGN})
    if(BOBUI_RUN_COMPILE_TESTS_IMMEDIATELY)
        bobui_run_config_compile_test(${name})
    endif()
endmacro()

# Runs a compile test that was defined with bobui_config_compile_test.
function(bobui_run_config_compile_test name)
    if(DEFINED "TEST_${name}")
        return()
    endif()

    get_property(test_args GLOBAL PROPERTY _bobuifeature_compile_test_args_${name})
    if("${test_args}" STREQUAL "")
        message(FATAL_ERROR
            "Can't find definition for compile test '${name}'. "
            "The test probably wasn't defined with bobui_config_compile_test."
        )
    endif()
    ...
    if(arg_PROJECT_PATH)
        ...
        if(arg_PACKAGES)
            ...
            list(APPEND flags "-DBOBUI_CONFIG_COMPILE_TEST_PACKAGES:STRING=${packages_list}")

            # Inside the project, the value of BOBUI_CONFIG_COMPILE_TEST_PACKAGES is used in a foreach
        ...
        if(arg_LIBRARIES)
            ...
            endforeach()
            if(link_flags)
                list(APPEND flags "-DBOBUI_CONFIG_COMPILE_TEST_LIBRARIES:STRING=${link_flags}")
            endif()
            if(library_targets)
                list(APPEND flags
                     "-DBOBUI_CONFIG_COMPILE_TEST_LIBRARY_TARGETS:STRING=${library_targets}")
            endif()
        endif()

        # Pass override values for CMAKE_SYSTEM_{PREFIX|FRAMEWORK}_PATH.
        if(DEFINED BOBUI_CMAKE_SYSTEM_PREFIX_PATH_BACKUP)
            set(path_list ${CMAKE_SYSTEM_PREFIX_PATH})
            string(REPLACE ";" "\\;" path_list "${path_list}")
            list(APPEND flags "-DBOBUI_CONFIG_COMPILE_TEST_CMAKE_SYSTEM_PREFIX_PATH=${path_list}")
        endif()
        if(DEFINED BOBUI_CMAKE_SYSTEM_FRAMEWORK_PATH_BACKUP)
            set(path_list ${CMAKE_SYSTEM_FRAMEWORK_PATH})
            string(REPLACE ";" "\\;" path_list "${path_list}")
            list(APPEND flags "-DBOBUI_CONFIG_COMPILE_TEST_CMAKE_SYSTEM_FRAMEWORK_PATH=${path_list}")
        endif()
        ...
        # CI passes the project dir of the BobUI repository as absolute path without drive letter:
        #   \Users\bobui\work\bobui\bobuibase
        # Ensure that arg_PROJECT_PATH is an absolute path with drive letter:
        #   C:/Users/bobui/work/bobui/bobuibase
        ...
        if(NOT DEFINED HAVE_${name})
            ...
            _bobui_internal_get_check_cxx_source_compiles_out_var(try_compile_output extra_args)
    ...
    set_property(GLOBAL PROPERTY _bobui_run_config_compile_test_output_${name} "${try_compile_output}")
...
function(bobui_get_platform_try_compile_vars out_var)
    ...
    # Let CMake load our custom platform modules.
    if(NOT BOBUI_AVOID_CUSTOM_PLATFORM_MODULES)
        list(APPEND flags_cmd_line "-DCMAKE_MODULE_PATH:STRING=${BOBUI_CMAKE_DIR}/platforms")
    endif()
    ...
    if(UIKIT)
        ...
        if(BOBUI_APPLE_SDK)
            list(APPEND flags_cmd_line "-DCMAKE_OSX_SYSROOT:STRING=${BOBUI_APPLE_SDK}")
        endif()
    endif()
...
function(bobui_internal_get_first_osx_arch out_var)
    set(value "")
...
function(bobui_config_compile_test_x86simd extension label)
...
function(bobui_config_compile_test_armintrin extension label)
...
function(bobui_config_compile_test_loongarchsimd extension label)
...
function(bobui_config_compile_test_machine_tuple label)
...
function(bobui_config_compiler_supports_flag_test name)
    if(DEFINED "TEST_${name}")
        return()
    endif()
...
# TODO: In the gcc < 12.1 case, the bobui_internal_check_if_linker_is_available(mold) check will
...
#       Moving such a BobUI to a different machine and trying to build another repo won't
#       work because the recorded path will be invalid. This is not a problem with
#       the gcc >= 12.1 case
function(bobui_internal_get_mold_linker_flags out_var)
    cmake_parse_arguments(PARSE_ARGV 1 arg "ERROR_IF_EMPTY" "" "")

    find_program(BOBUI_INTERNAL_LINKER_MOLD mold)

    set(flag "")
    if(BOBUI_INTERNAL_LINKER_MOLD)
        if(GCC)
            if(CMAKE_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "12.1")
                set(flag "-fuse-ld=mold")
            else()
                set(mold_linker_dir "${CMAKE_CURRENT_BINARY_DIR}/.bobui_linker")
...
function(bobui_internal_get_active_linker_flags out_var)
    set(flags "")
    if(GCC OR CLANG)
        if(BOBUI_FEATURE_use_gold_linker)
            list(APPEND flags "-fuse-ld=gold")
        elseif(BOBUI_FEATURE_use_bfd_linker)
            list(APPEND flags "-fuse-ld=bfd")
        elseif(BOBUI_FEATURE_use_lld_linker)
            list(APPEND flags "-fuse-ld=lld")
        elseif(BOBUI_FEATURE_use_mold_linker)
            bobui_internal_get_mold_linker_flags(mold_flags ERROR_IF_EMPTY)
...
function(bobui_internal_check_if_linker_is_available name)
    if(DEFINED "TEST_${name}")
        return()
    endif()
...
function(bobui_config_linker_supports_flag_test name)
    if(DEFINED "TEST_${name}")
        return()
    endif()
...
function(bobui_make_features_available target)
    if(NOT "${target}" MATCHES "^${BOBUI_CMAKE_EXPORT_NAMESPACE}::[a-zA-Z0-9_-]*$")
        message(FATAL_ERROR "${target} does not match ${BOBUI_CMAKE_EXPORT_NAMESPACE}::[a-zA-Z0-9_-]*. INVALID NAME.")
    endif()
    ...
    foreach(visibility IN ITEMS PUBLIC PRIVATE)
        ...
        foreach(state IN ITEMS ENABLED DISABLED)
            get_target_property(features "${target}" ${property_prefix}BOBUI_${state}_${visibility}_FEATURES)
            if("${features}" STREQUAL "features-NOTFOUND")
                continue()
            endif()
            foreach(feature IN ITEMS ${features})
                if (DEFINED "BOBUI_FEATURE_${feature}" AND NOT "${BOBUI_FEATURE_${feature}}" STREQUAL "${value}")
                    message(WARNING
                        "This project was initially configured with the BobUI feature \"${feature}\" "
                        "set to \"${BOBUI_FEATURE_${feature}}\". While loading the "
                ...
                set(BOBUI_FEATURE_${feature} "${value}" CACHE INTERNAL "BobUI feature: ${feature} (from target ${target})")
            endforeach()
            set(value OFF)
        endforeach()
    endforeach()
