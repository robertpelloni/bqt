...
function(bobui_install)
    ...
    # in export() commands.
    if(BOBUI_WILL_INSTALL OR is_install_targets)
        install(${ARGV})
    endif()
    ...
    if(NOT is_install_targets AND arg_EXPORT)
        ...
        # ExternalProjects as part of the BobUI build.
        # In a top-level build the exported config files are placed under bobuibase/lib/cmake.
        # In a per-repo build, they will be placed in each repo's build dir/lib/cmake.
        if(BOBUI_WILL_INSTALL)
            bobui_path_join(arg_DESTINATION "${BOBUI_BUILD_DIR}" "${arg_DESTINATION}")
        endif()
...
function(bobui_non_prefix_copy)
...
function(bobui_get_install_executable_permissions out_var)
...
function(bobui_copy_or_install)
    ...
    elseif(arg_PROGRAMS)
        ...
        bobui_get_install_executable_permissions(executable_permissions)
    ...
    bobui_install(${install_option} ${argv_copy})
    bobui_non_prefix_copy(COPY ${argv_copy} ${copy_arguments})
endfunction()
...
function(bobui_internal_install_versioned_link)
    if(NOT BOBUI_WILL_INSTALL)
        return()
    endif()

    if(NOT BOBUI_CREATE_VERSIONED_HARD_LINK)
        return()
    endif()
    ...
    if(arg_TARGETS)
        foreach(target "${arg_TARGETS}")
            _bobui_internal_create_versioned_link_or_copy("${arg_WORKING_DIRECTORY}"
...
function(_bobui_internal_create_versioned_link_or_copy install_dir base_name suffix)
    bobui_path_join(install_base_file_path "$\{bobui_full_install_prefix}"
    ...
    set(code "set(bobui_full_install_prefix \"$\{CMAKE_INSTALL_PREFIX}\")"
        "  if(NOT \"$ENV\{DESTDIR}\" STREQUAL \"\")"
        )

    if(CMAKE_HOST_WIN32)
        list(APPEND code
            "    if(bobui_full_install_prefix MATCHES \"^[a-zA-Z]:\")"
            "        string(SUBSTRING \"$\{bobui_full_install_prefix}\" 2 -1 bobui_full_install_prefix)"
            "    endif()"
            )
    endif()
    list(APPEND code
        "    string(PREPEND bobui_full_install_prefix \"$ENV\{DESTDIR}\")"
        "  endif()"
    ...
    if(BOBUI_GENERATOR_IS_MULTI_CONFIG)
        ...
        bobui_create_case_insensitive_regex(main_config_regex ${BOBUI_MULTI_CONFIG_FIRST_CONFIG})
...
function(bobui_internal_copy_at_build_time)
    set(flags)
    ...
    get_property(count GLOBAL PROPERTY _bobui_internal_copy_at_build_time_count)
    if(NOT count)
        set(count 0)
    endif()

    add_custom_target(bobui_internal_copy_at_build_time_${count} DEPENDS ${outputs})
    if(arg_TARGET)
        add_dependencies(${arg_TARGET} bobui_internal_copy_at_build_time_${count})
    endif()

    math(EXPR count "${count} + 1")
    set_property(GLOBAL PROPERTY _bobui_internal_copy_at_build_time_count ${count})
endfunction()
