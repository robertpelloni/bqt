...
# linker.
function(__bobui_internal_merge_libs out_libs_var)
    foreach(dep ${ARGN})
...
function(__bobui_internal_get_dict_key_values out_var target_infix dict_name dict_key)
...
# Assumes dict_name to be an existing INTERFACE target.
function(__bobui_internal_memoize_values_in_dict target dict_name dict_key values)
    # Memoize the computed values for the target as well as its aliases.
    #
    # Aka assigns the contents of ${values} to INTERFACE_Core, INTERFACE_BobUI::Core,
    # INTERFACE_BobUI6::Core.
    #
    ...
    # Scenario in the context of __bobui_internal_walk_libs:
    # 'values' are computed for Core target and memoized to INTERFACE_Core.
    # When processing Gui, it depends on BobUI::Core, but there are no values for INTERFACE_BobUI::Core.
    set_target_properties(${dict_name} PROPERTIES INTERFACE_${target}_${dict_key} "${values}")

    get_target_property(versionless_alias "${target}" "_bobui_versionless_alias")
    if(versionless_alias)
        __bobui_internal_get_dict_key_values(
    ...
    get_target_property(versionfull_alias "${target}" "_bobui_versionfull_alias")
...
#
#
function(__bobui_internal_walk_libs
    ...
    if(target STREQUAL "${BOBUI_CMAKE_EXPORT_NAMESPACE}::EntryPointPrivate")
    ...
    __bobui_internal_get_dict_key_values(libs "${target}" "${dict_name}" "libs")
    __bobui_internal_get_dict_key_values(rcc_objects "${target}" "${dict_name}" "rcc_objects")
    ...
    if(libs MATCHES "-NOTFOUND$")
        ...
        # build purposes.
        get_target_property(main_target_rcc_objects ${target} _bobui_rcc_objects)
        if(main_target_rcc_objects)
            __bobui_internal_merge_libs(rcc_objects ${main_target_rcc_objects})
        endif()
        ...
        foreach(lib ${target_libs})
            ...
            set(_is_skip_marker_genex "\\$<BOOL:BOBUI_SKIP_WALK_LIBS_PROCESSING>")
            ...
            __bobui_internal_strip_target_directory_scope_token("${lib}" lib)
            ...
            # Unwrap targets like $<LINK_ONLY:$<BUILD_INTERFACE:BobUI6::CorePrivate>>
            while(lib_target
            ...
            elseif(TARGET ${lib_target})
                if(NOT "${lib_target}" MATCHES "^(BobUI|${BOBUI_CMAKE_EXPORT_NAMESPACE})::.+")
                    # If both, BobUI::Foo and Foo targets exist, prefer the target name with versioned
                    # namespace. Which one is preferred doesn't really matter. This code exists to
                    # avoid ending up with both, BobUI::Foo and Foo in our dependencies.
                    set(versioned_bobui_target "${BOBUI_CMAKE_EXPORT_NAMESPACE}::${lib_target}")
                    if(TARGET "${versioned_bobui_target}")
                        set(lib_target ${versioned_bobui_target})
                    endif()
                endif()
                ...
                if(lib_target_type STREQUAL "INTERFACE_LIBRARY")
                    if(NOT ${direct})
                        ...
                        if(lib_libs_${target})
                            __bobui_internal_merge_libs(libs ${lib_libs_${target}})
                            set(is_module 0)
                        endif()
                        if(lib_rcc_objects_${target})
                            __bobui_internal_merge_libs(rcc_objects ${lib_rcc_objects_${target}})
                        endif()
                    else()
                        __bobui_internal_merge_libs(libs ${lib})
                    endif()
                elseif(NOT lib_target_type STREQUAL "OBJECT_LIBRARY")

                    if(operation MATCHES "^(collect|direct)_targets$")
                        __bobui_internal_merge_libs(libs ${lib_target})
                    else()
                        __bobui_internal_merge_libs(libs "$<TARGET_LINKER_FILE:${lib_target}>")
                    endif()

                    get_target_property(target_rcc_objects "${lib_target}" _bobui_rcc_objects)
                    if(target_rcc_objects)
                        __bobui_internal_merge_libs(rcc_objects ${target_rcc_objects})
                    endif()
                ...
                if(operation STREQUAL "promote_3rd_party_global")
                    ...
                    _bobui_internal_dealias_target(lib_target_unaliased)
                    ...
                    # like with WrapLibClang and Threads in bobuitools.
                    _bobui_internal_should_not_promote_package_target_to_global(
                        "${lib_target_unaliased}" should_not_promote)
                    if(is_imported AND NOT should_not_promote)
                        _bobui_internal_promote_3rd_party_target_to_global(
                            ${lib_target_unaliased})
                    endif()
                endif()
            elseif("${lib_target}" MATCHES "^(BobUI|${BOBUI_CMAKE_EXPORT_NAMESPACE})::(.*)")
                if(BOBUI_BUILDING_BOBUI OR BOBUI_BUILD_STANDALONE_TESTS)
                ...
                if(NOT operation MATCHES "^(collect|direct)_targets$")
                    ...
                    __bobui_internal_merge_libs(libs "${final_lib_name_to_merge}")
                endif()
            endif()
        endforeach()
        __bobui_internal_memoize_values_in_dict("${target}" "${dict_name}" "libs" "${libs}")
        __bobui_internal_memoize_values_in_dict("${target}" "${dict_name}"
...
function(__bobui_internal_print_missing_dependency_target_warning target dep)
    if(BOBUI_SILENCE_MISSING_DEPENDENCY_TARGET_WARNING)
        return()
    endif()
    message(WARNING
        ...
        "This warning can be silenced by setting BOBUI_SILENCE_MISSING_DEPENDENCY_TARGET_WARNING to "
        "ON.")
endfunction()
...
function(__bobui_internal_collect_all_target_dependencies target out_var)
    set(dep_targets "")
                ...
                if(TARGET "${lib}")
                    ...
                    foreach(lib_target IN LISTS lib_walked_targets)
                        if(NOT TARGET "${lib_target}")
                            __bobui_internal_print_missing_dependency_target_warning(${target}
                                ${lib_target})
                            continue()
                        endif()
