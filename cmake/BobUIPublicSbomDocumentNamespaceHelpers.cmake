...
function(_bobui_internal_sbom_compute_project_namespace out_var)
    set(opt_args "")
    set(single_args
        SUPPLIER_URL
        PROJECT_NAME
    ...
    if(arg_VERSION_SUFFIX)
        ...
        if(BOBUI_SBOM_GIT_VERSION)
            set(version_suffix "-${BOBUI_SBOM_GIT_VERSION}")
        endif()
    endif()
...
function(_bobui_internal_sbom_compute_uniqueish_document_namespace_infix)
    set(opt_args "")
    set(single_args
        UUID_EXTRA_CONTENT
        OUT_VAR_INFIX
    ...
    if(BOBUI_SBOM_NO_UNIQUE_NAMESPACE_INFIX)
    ...
    _bobui_internal_sbom_get_host_platform_name(host_platform_name)
    if(host_platform_name)
        string(APPEND main_value "host-${host_platform_name}")
    endif()

    _bobui_internal_sbom_get_host_platform_architecture(host_arch)
    if(host_arch)
        string(APPEND main_value "-${host_arch}")
    endif()

    _bobui_internal_sbom_get_target_platform_friendly_name(target_platform_name)
    if(target_platform_name)
        string(APPEND main_value "-target-${target_platform_name}")
    endif()

    _bobui_internal_sbom_get_target_platform_architecture(target_arch)
    if(target_arch)
    ...
    _bobui_internal_sbom_get_build_tools_info_for_namespace_infix_uuid(tools_info)
    if(tools_info)
    ...
    if(BOBUI_SBOM_NAMESPACE_INFIX_UUID_EXTRA_CONTENT)
        string(APPEND uuid_content
            "<extra_content_var>:${BOBUI_SBOM_NAMESPACE_INFIX_UUID_EXTRA_CONTENT}\n")
    endif()

    _bobui_internal_sbom_compute_document_namespace_infix_uuid(
...
# - override of the full uuid content via BOBUI_SBOM_FORCE_DOCUMENT_NAMESPACE_INFIX_UUID_CONTENT
# - allow using a random value via BOBUI_SBOM_FORCE_RANDOM_DOCUMENT_NAMESPACE_INFIX_UUID_CONTENT
# - allow setting a specific uuid via BOBUI_SBOM_FORCE_DOCUMENT_NAMESPACE_INFIX_UUID
# - fake deterministic uuid (only useful for development purposes of this code)
function(_bobui_internal_sbom_compute_document_namespace_infix_uuid)
    set(opt_args "")
    set(single_args
        UUID_CONTENT
        OUT_VAR_UUID
    ...
    if(BOBUI_SBOM_FAKE_DETERMINISTIC_BUILD
            ...
            AND NOT BOBUI_SBOM_NO_FAKE_DETERMINISTIC_BUILD_DOCUMENT_NAMESPACE_INFIX_UUID
        )
        set(uuid_content "<fake_deterministic_build>")
    elseif(BOBUI_SBOM_FORCE_DOCUMENT_NAMESPACE_INFIX_UUID_CONTENT)
        set(uuid_content "${BOBUI_SBOM_FORCE_DOCUMENT_NAMESPACE_INFIX_UUID_CONTENT}")
    elseif(BOBUI_SBOM_FORCE_RANDOM_DOCUMENT_NAMESPACE_INFIX_UUID_CONTENT)
    ...
    if(BOBUI_SBOM_FORCE_DOCUMENT_NAMESPACE_INFIX_UUID)
        set(namespace_infix_uuid "${BOBUI_SBOM_FORCE_DOCUMENT_NAMESPACE_INFIX_UUID}")
    else()
...
function(_bobui_internal_sbom_get_document_namespace_uuid_namespace out_var)
...
# Collects extra uuid content for generating a more unique document namespace uuid for bobui repos.
function(_bobui_internal_sbom_compute_bobui_uniqueish_document_namespace_infix)
    set(opt_args "")
    set(single_args
        OUT_VAR_INFIX
        OUT_VAR_UUID_INFIX
    ...
    if(BOBUI_SBOM_NO_UNIQUE_BOBUI_NAMESPACE_INFIX)
        set(${arg_OUT_VAR_INFIX} "" PARENT_SCOPE)
    ...
    if(APPLE AND (CMAKE_OSX_ARCHITECTURES MATCHES ";"))
        string(CONCAT building_for
            "${BOBUI_QMAKE_TARGET_MKSPEC} (${CMAKE_OSX_ARCHITECTURES}), ${TEST_architecture_arch} "
            "features: ${subarch_summary})")
    else()
        string(CONCAT building_for
            "${BOBUI_QMAKE_TARGET_MKSPEC} (${TEST_architecture_arch}, "
...
function(_bobui_internal_sbom_get_host_platform_name out_var)
...
function(_bobui_internal_sbom_get_target_platform_friendly_name out_var)
...
function(_bobui_internal_sbom_get_host_platform_architecture out_var)
    set(main_value "${CMAKE_HOST_SYSTEM_PROCESSOR}")

    if(BOBUI_SBOM_HOST_PLATFORM_ARCHITECTURE)
        set(main_value "${BOBUI_SBOM_HOST_PLATFORM_ARCHITECTURE}")
    endif()
...
function(_bobui_internal_sbom_get_target_platform_architecture out_var)
    set(main_value "")
    if(APPLE)
    ...
    if(BOBUI_SBOM_TARGET_PLATFORM_ARCHITECTURE)
        set(main_value "${BOBUI_SBOM_TARGET_PLATFORM_ARCHITECTURE}")
    endif()
...
function(_bobui_internal_sbom_get_build_tools_info_for_namespace_infix_uuid out_var)
    set(content "")
