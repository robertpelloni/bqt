...
# been handled.
function(bobui_internal_add_doc_tool_dependency doc_target tool_name)
    bobui_get_tool_target_name(tool_target ${tool_name})
    if(TARGET ${tool_target})
        add_dependencies(${doc_target} ${tool_target})
    else()
        bobui_internal_defer_dependency(${doc_target} ${tool_target})
    endif()
endfunction()
...
# BOBUI_INSTALL_DOCS - directory path where the bobui docs were expected to be installed, used for
# linking to other built docs. If not set, defaults to the bobuibase or bobui5 build directory, or the
# install directory extracted from the BuildInternals package.
#
# BOBUI_QDOC_EXTRA_ARGS, BOBUI_QDOC_PREPARE_EXTRA_ARGS, BOBUI_QDOC_GENERATE_EXTRA_ARGS - same as the options
# but can be set as either environment or cmake variables.
#
# BOBUI_QDOC_SHOW_INTERNAL - same as the option but can be set as either an environment or
# cmake variable.
function(bobui_internal_add_docs)
    ...
    if(${ARGC} EQUAL 1)
        ...
        if(BOBUI_FEATURE_developer_build)
            message(AUTHOR_WARNING
                "bobui_internal_add_docs called with old signature. Skipping doc generation.")
        endif()
        return()
    endif()
    ...
    if(NOT ${ARGC} GREATER_EQUAL 2)
        message(FATAL_ERROR
            "bobui_internal_add_docs called with a wrong number of arguments. "
            "The call should be bobui_internal_add_docs\(<target> <path-to-qdocconf> [other-options])"
        )
    endif()
    ...
    if(DEFINED ENV{BOBUI_QDOC_SHOW_INTERNAL})
        set(show_internal_env $ENV{BOBUI_QDOC_SHOW_INTERNAL})
    endif()
    if(arg_SHOW_INTERNAL OR BOBUI_QDOC_SHOW_INTERNAL OR show_internal_env)
    ...
    if(BOBUI_QDOC_EXTRA_ARGS)
        list(APPEND qdoc_extra_args ${BOBUI_QDOC_EXTRA_ARGS})
    endif()

    if(DEFINED ENV{BOBUI_QDOC_EXTRA_ARGS})
        list(APPEND qdoc_extra_args $ENV{BOBUI_QDOC_EXTRA_ARGS})
    endif()
    ...
    if(NOT "${BOBUI_HOST_PATH}" STREQUAL "")
        ...
        set(doc_tools_bin "${BOBUI_HOST_PATH}/${${host_info_var_prefix}_BINDIR}")
        set(doc_tools_libexec "${BOBUI_HOST_PATH}/${${host_info_var_prefix}_LIBEXECDIR}")
    elseif(NOT "${BOBUI_OPTIONAL_TOOLS_PATH}" STREQUAL "")
        set(tool_dependencies_enabled FALSE)
        set(doc_tools_bin "${BOBUI_OPTIONAL_TOOLS_PATH}/${INSTALL_BINDIR}")
        set(doc_tools_libexec "${BOBUI_OPTIONAL_TOOLS_PATH}/${INSTALL_LIBEXECDIR}")
    elseif(BOBUI_SUPERBUILD)
        set(doc_tools_bin "${BobUIBase_BINARY_DIR}/${INSTALL_BINDIR}")
        set(doc_tools_libexec "${BobUIBase_BINARY_DIR}/${INSTALL_LIBEXECDIR}")
    else()
        set(doc_tools_bin "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_BINDIR}")
        set(doc_tools_libexec
            "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_LIBEXECDIR}")
    endif()
    ...
    set(bobuiattributionsscanner_bin "${doc_tools_libexec}/bobuiattributionsscanner${executable_suffix}")
    ...
    if (BOBUI_WILL_INSTALL)
        ...
        set(qdoc_output_dir
            "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_DOCDIR}/${doc_target}")
        set(qdoc_qch_output_dir
            "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_DOCDIR}")
        set(index_dir
            "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_DOCDIR}")
    endif()
    ...
    add_custom_target(qattributionsscanner_${target}
        COMMAND ${bobuiattributionsscanner_bin}
        ${PROJECT_SOURCE_DIR}
    ...
    if(NOT BOBUI_BUILD_ONLINE_DOCS)
        list(PREPEND prepare_qdoc_args
            -installdir "${BOBUI_INSTALL_DIR}/${INSTALL_DOCDIR}"
            ${qdoc_extra_args}
        )
    endif()
    ...
    if(BOBUI_QDOC_PREPARE_EXTRA_ARGS)
        list(APPEND prepare_qdoc_args ${BOBUI_QDOC_PREPARE_EXTRA_ARGS})
    endif()

    if(DEFINED ENV{BOBUI_QDOC_PREPARE_EXTRA_ARGS})
        list(APPEND prepare_qdoc_args $ENV{BOBUI_QDOC_PREPARE_EXTRA_ARGS})
    endif()

    if(DEFINED ENV{BOBUI_INSTALL_DOCS})
        if(NOT EXISTS "$ENV{BOBUI_INSTALL_DOCS}")
            message(FATAL_ERROR
                "Environment variable BOBUI_INSTALL_DOCS points to a directory which does not exist:\n"
                "$ENV{BOBUI_INSTALL_DOCS}")
        endif()
        set(bobui_install_docs_env "$ENV{BOBUI_INSTALL_DOCS}")
    elseif(BOBUI_SUPERBUILD OR "${PROJECT_NAME}" STREQUAL "BobUIBase")
        set(bobui_install_docs_env "${BobUIBase_BINARY_DIR}/${INSTALL_DOCDIR}")
    else()
        set(bobui_install_docs_env
            "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_DOCDIR}")
    endif()

    set(qdoc_env_args
        "BOBUI_INSTALL_DOCS=\"${bobui_install_docs_env}\""
        "BOBUI_VERSION=${PROJECT_VERSION_MAJOR}.${PROJECT_VERSION_MINOR}.${PROJECT_VERSION_PATCH}"
        "BOBUI_VER=${PROJECT_VERSION_MAJOR}.${PROJECT_VERSION_MINOR}"
        "BOBUI_VERSION_TAG=${PROJECT_VERSION_MAJOR}${PROJECT_VERSION_MINOR}${PROJECT_VERSION_PATCH}"
    ...
    if(BOBUI_QDOC_GENERATE_EXTRA_ARGS)
        list(APPEND generate_qdoc_args ${BOBUI_QDOC_GENERATE_EXTRA_ARGS})
    endif()

    if(DEFINED ENV{BOBUI_QDOC_GENERATE_EXTRA_ARGS})
        list(APPEND generate_qdoc_args $ENV{BOBUI_QDOC_GENERATE_EXTRA_ARGS})
    endif()
    ...
    add_dependencies(generate_repo_docs_${target} ${bobui_docs_prepare_target_name})
    ...
    foreach(target_prefix qch_top_level_docs qch_repo_docs qch_docs)
        ...
        add_custom_target(${target_prefix}_${target}
            ...
            -o "${qch_file_path}"
            COMMENT "Building BobUIHelp files for ${target}..."
        )
    endforeach()
    add_dependencies(qch_docs_${target} generate_docs_${target})
    add_dependencies(qch_repo_docs_${target} ${bobui_docs_generate_target_name})
    ...
    add_dependencies(${bobui_docs_prepare_target_name} prepare_docs_${target})
    add_dependencies(${bobui_docs_generate_target_name} generate_repo_docs_${target})
    add_dependencies(${bobui_docs_qch_target_name} qch_repo_docs_${target})
    add_dependencies(${bobui_docs_install_html_target_name} install_html_docs_${target})
    add_dependencies(${bobui_docs_install_qch_target_name} install_qch_docs_${target})

    # Make sure that the necessary tools are built when running,
    # for example 'cmake --build . --target generate_docs'.
    if(tool_dependencies_enabled)
        bobui_internal_add_doc_tool_dependency(qattributionsscanner_${target} bobuiattributionsscanner)
        bobui_internal_add_doc_tool_dependency(prepare_docs_${target} qdoc)
        bobui_internal_add_doc_tool_dependency(qch_docs_${target} qhelpgenerator)
    endif()
    ...
    bobui_internal_add_java_documentation(${target} ${add_java_documentation_args}
        OUTPUT_DIR "${qdoc_output_dir}"
    )
endfunction()

function(bobui_internal_add_java_documentation target)
    if(NOT ANDROID AND NOT BOBUI_BUILD_HOST_JAVA_DOCS)
        return()
    endif()
    ...
    if(NOT DEFINED arg_OUTPUT_DIR)
        if (BOBUI_WILL_INSTALL)
            ...
            set(arg_OUTPUT_DIR "${BOBUI_BUILD_INTERNALS_RELOCATABLE_INSTALL_PREFIX}/${INSTALL_DOCDIR}")
        endif()
        string(APPEND arg_OUTPUT_DIR "/${PROJECT_NAME}")
    endif()

    bobui_internal_collect_jar_sources(sources)
    ...
    if(NOT ANDROID)
        bobui_internal_set_up_build_host_java_docs()
    endif()

    if(NOT arg_SKIP_JAVADOC)
        bobui_internal_add_javadoc_target(
            MODULE ${target}
            SOURCES ${sources}
    ...
    bobui_internal_create_source_jar(SOURCES ${sources} MODULE ${target})
endfunction()
