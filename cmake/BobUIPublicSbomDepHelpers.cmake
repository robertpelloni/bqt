...
# Currently handles various BobUI targets and system libraries.
...
function(_bobui_internal_sbom_handle_target_dependencies target)
    set(opt_args "")
    set(single_args
        SPDX_ID
    ...
    get_target_property(extend_libraries "${target}" _bobui_extend_target_libraries)
    if(extend_libraries)
    ...
    get_target_property(extend_public_libraries "${target}" _bobui_extend_target_public_libraries)
    ...
    get_target_property(extend_sbom_dependencies "${target}" _bobui_extend_target_sbom_dependencies)
    ...
    foreach(direct_lib IN LISTS all_direct_libraries)
        ...
        # Check for BobUI-specific system library targets. These are marked via bobui_find_package calls.
        get_target_property(is_system_library "${direct_lib}" _bobui_internal_sbom_is_system_library)
        if(is_system_library)
            ...
            if(lib_walked_targets)
                foreach(lib_walked_target IN LISTS lib_walked_targets)
                    get_target_property(is_3rdparty_bundled_lib
                        "${lib_walked_target}" _bobui_module_is_3rdparty_library)
                    _bobui_internal_sbom_get_spdx_id_for_target("${lib_walked_target}" lib_spdx_id)
            ...
            if(NOT bundled_targets_found)
                ...
                _bobui_internal_append_to_cmake_property_without_duplicates(
                    _bobui_internal_sbom_consumed_system_library_targets
                    "${direct_lib}"
                )
            endif()
        endif()
        ...
        _bobui_internal_sbom_get_spdx_id_for_target("${direct_lib}" lib_spdx_id)
        if(NOT lib_spdx_id)
        ...
        _bobui_internal_sbom_is_external_target_dependency("${direct_lib}"
        ...
        if(NOT is_dependency_in_external_document)
            ...
            _bobui_internal_sbom_add_external_target_dependency("${direct_lib}"
...
function(_bobui_internal_sbom_is_external_target_dependency target)
    set(opt_args
        SYSTEM_LIBRARY
    )
    set(single_args
    ...
    get_target_property(is_custom_sbom_target "${target}" _bobui_sbom_is_custom_sbom_target)

    _bobui_internal_sbom_get_root_project_name_lower_case(current_repo_project_name)
    get_property(target_repo_project_name TARGET ${target}
        PROPERTY _bobui_sbom_spdx_repo_project_name_lowercase)
...
function(_bobui_internal_sbom_add_external_target_dependency target)
    set(opt_args "")
    set(single_args
        OUT_CYDX_DEPENDENCIES
    ...
    _bobui_internal_sbom_get_spdx_id_for_target("${target}" dep_spdx_id)
    ...
    if(relative_installed_repo_document_path AND project_name_lowercase)
        ...
        if(NOT known_external_document)
            ...
            _bobui_internal_sbom_generate_add_external_reference(
            ...
            set_property(GLOBAL PROPERTY
                _bobui_known_external_documents_${external_document_ref} TRUE)
            set_property(GLOBAL APPEND PROPERTY
                _bobui_known_external_documents "${external_document_ref}")
        endif()
    else()
