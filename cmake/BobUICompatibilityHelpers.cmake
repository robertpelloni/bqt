...
function(bobui_collect_forward_find_package_args out_var package_prefix)
    ...
    if(DEFINED ${package_prefix}_FIND_VERSION)
        if(NOT "${${package_prefix}_FIND_VERSION}" STREQUAL "")
            list(APPEND _bobui_args "${${package_prefix}_FIND_VERSION}")
        endif()
    endif()

    if(DEFINED ${package_prefix}_FIND_VERSION_EXACT)
        if(${${package_prefix}_FIND_VERSION_EXACT})
            list(APPEND _bobui_args EXACT)
        endif()
    endif()

    if(DEFINED ${package_prefix}_FIND_QUIETLY)
        if(${${package_prefix}_FIND_QUIETLY})
            list(APPEND _bobui_args QUIET)
        endif()
    endif()

    if(DEFINED ${package_prefix}_FIND_REQUIRED)
        if(${${package_prefix}_FIND_REQUIRED})
            list(APPEND _bobui_args REQUIRED)
        endif()
    endif()

    if(DEFINED ${package_prefix}_FIND_COMPONENTS)
        if(NOT "${${package_prefix}_FIND_COMPONENTS}" STREQUAL "")
