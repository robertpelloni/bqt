...
macro(bobui_examples_build_begin)
    ...
    bobui_internal_sbom_disable_sbom_for_examples_subdir()
    ...
    # by unsetting the BOBUI_INTERNAL_SKIP_DEPLOYMENT variable before its bobui_internal_add_example call.
    # This will be used by our CI.
    if(NOT DEFINED BOBUI_INTERNAL_SKIP_DEPLOYMENT AND BOBUI_DEPLOY_MINIMAL_EXAMPLES)
        set(BOBUI_INTERNAL_SKIP_DEPLOYMENT TRUE)
    endif()

    # Use by bobui_internal_add_example.
    set(BOBUI_EXAMPLE_BASE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")

    if(BOBUI_BUILD_STANDALONE_EXAMPLES)
        # Find all bobui packages, so that the various if(BOBUI_FEATURE_foo) add_subdirectory()
    ...
    if(arg_EXTERNAL_BUILD AND BOBUI_BUILD_EXAMPLES_AS_EXTERNAL)
        ...
        set(BOBUI_EXAMPLE_DEPENDENCIES ${bobui_repo_plugins_recursive} ${arg_DEPENDS})

        if(TARGET ${bobui_repo_targets_name}_src)
            list(APPEND BOBUI_EXAMPLE_DEPENDENCIES ${bobui_repo_targets_name}_src_for_examples)
        endif()

        if(TARGET ${bobui_repo_targets_name}_tools)
            list(APPEND BOBUI_EXAMPLE_DEPENDENCIES ${bobui_repo_targets_name}_tools)
        endif()

        set(BOBUI_IS_EXTERNAL_EXAMPLES_BUILD TRUE)

        if(NOT TARGET examples)
            if(BOBUI_BUILD_EXAMPLES_BY_DEFAULT)
        ...
        if(NOT BOBUI_BUILD_EXAMPLES_BY_DEFAULT)
            set_directory_properties(PROPERTIES EXCLUDE_FROM_ALL TRUE)
        endif()

        # Add active linker flags to all targets created below this subdirectory.
        bobui_internal_get_active_linker_flags(active_linker_flags)
        if(active_linker_flags)
            add_link_options(${active_linker_flags})
        endif()

        if(BOBUI_EXTRA_EXAMPLE_TARGET_DEFINES)
            add_compile_definitions(${BOBUI_EXTRA_EXAMPLE_TARGET_DEFINES})
        endif()

        # Marker for warnings_as_errors.
        set(BOBUI_INTERNAL_IS_EXAMPLE_IN_TREE_BUILD ON)
    endif()

    # TODO: Change this to TRUE when all examples in all repos are ported to use
    # bobui_internal_add_example.
    # We shouldn't need to call bobui_internal_set_up_build_dir_package_paths when
    # BOBUI_IS_EXTERNAL_EXAMPLES_BUILD is TRUE.
    # Due to not all examples being ported, if we don't
    # call bobui_internal_set_up_build_dir_package_paths -> set(BOBUI_NO_CREATE_TARGETS TRUE) we'll get
    # CMake configuration errors saying we redefine BobUI targets because we both build them and find
    # them as part of find_package.
    set(__bobui_all_examples_ported_to_external_projects FALSE)

    # Examples that are built as part of the BobUI build need to use the CMake config files from the
    # build dir, because they are not installed yet in a prefix build.
    # Prepending to CMAKE_PREFIX_PATH helps find the initial BobUI6Config.cmake.
    ...
    if(NOT BOBUI_IS_EXTERNAL_EXAMPLES_BUILD OR NOT __bobui_all_examples_ported_to_external_projects)
        bobui_internal_set_up_build_dir_package_paths()
...
# and restore it after all examples are processed so that BobUIFooToolsAdditionalTargetInfo.cmake
# files are installed into the original install prefix.
set(_bobui_internal_examples_cmake_install_prefix_backup \"\${CMAKE_INSTALL_PREFIX}\")
")

    # Backup the DESTDIR and unset it, so that example installation is not affected by DESTDIR.
    # This is activated by our CI when BOBUI_INTERNAL_EXAMPLES_INSTALL_PREFIX is set.
    if(BOBUI_INTERNAL_EXAMPLES_INSTALL_PREFIX)
        set(_bobui_examples_should_unset_destdir TRUE)
    else()
        set(_bobui_examples_should_unset_destdir FALSE)
    endif()

    set_property(GLOBAL PROPERTY
        _bobui_examples_should_unset_destdir_${project_name_lower}
        "${_bobui_examples_should_unset_destdir}")
    if(_bobui_examples_should_unset_destdir)
        install(CODE "
    # Temporarily unset DESTDIR while examples are being installed.
    set(_bobui_internal_examples_destdir_backup \"\$ENV{DESTDIR}\")
    unset(ENV{DESTDIR})
")
    endif()

    unset(_bobui_examples_should_unset_destdir)
    unset(project_name_lower)
endmacro()

macro(bobui_examples_build_end)
    ...
    # sure we do not fail on a fresh BobUI build (e.g. the moc binary won't exist
    ...
    foreach(target ${targets})
        # Skip re-enabling AUTMOC for object libraries created by _bobui_internal_add_rcc_pass2,
        # to avoid build errors.
        get_target_property(is_rcc_pass2_obj_lib "${target}" _bobui_internal_is_rcc_pass2_obj_lib)
...
set(CMAKE_INSTALL_PREFIX \"\${_bobui_internal_examples_cmake_install_prefix_backup}\")
")

    set(CMAKE_UNITY_BUILD ${BOBUI_UNITY_BUILD})

    string(TOLOWER ${PROJECT_NAME} project_name_lower)
    get_property(_bobui_examples_should_unset_destdir
        GLOBAL PROPERTY _bobui_examples_should_unset_destdir_${project_name_lower})

    if(_bobui_examples_should_unset_destdir)
        install(CODE "
    # Restore the DESTDIR env var after examples have been installed.
    set(ENV{DESTDIR} \"\${_bobui_internal_examples_destdir_backup}\")
    unset(_bobui_internal_examples_destdir_backup)
")
    endif()
...
# Allows building an example either as an ExternalProject or in-tree with the BobUI build.
# Also allows installing the example sources.
function(bobui_internal_add_example subdir)
    # Don't show warnings for examples that were added via bobui_internal_add_example.
    ...
    if(BOBUI_FEATURE_developer_build)
        set(BOBUI_LINT_EXAMPLES ON)
        if(NOT BOBUI_NO_WARN_ABOUT_EXAMPLE_ADD_SUBDIRECTORY_WARNING)
            set(BOBUI_WARN_ABOUT_EXAMPLE_ADD_SUBDIRECTORY FALSE)
        endif()
    endif()

    # Pre-compute unique example name based on the subdir, in case of target name clashes.
    bobui_internal_get_example_unique_name(unique_example_name "${subdir}")

    # BOBUI_INTERNAL_NO_CONFIGURE_EXAMPLES is not meant to be used by BobUI builders, it's here for faster
    # testing of the source installation code path for build system engineers.
    if(NOT BOBUI_INTERNAL_NO_CONFIGURE_EXAMPLES)
        if(NOT BOBUI_IS_EXTERNAL_EXAMPLES_BUILD)
            bobui_internal_add_example_in_tree("${subdir}")
        else()
            bobui_internal_add_example_external_project("${subdir}"
    ...
    if(BOBUI_INSTALL_EXAMPLES_SOURCES)
        ...
        bobui_internal_install_example_sources("${subdir}"
            NAME "${unique_example_name}"
...
function(bobui_internal_get_example_install_prefix out_var)
    # Allow customizing the installation path of the examples. Will be used in CI.
    if(BOBUI_INTERNAL_EXAMPLES_INSTALL_PREFIX)
        set(bobui_example_install_prefix "${BOBUI_INTERNAL_EXAMPLES_INSTALL_PREFIX}")
    elseif(BOBUI_BUILD_STANDALONE_EXAMPLES AND NOT BOBUI_NO_FAKE_STANDALONE_EXAMPLE_INSTALL_PREFIX)
        ...
        bobui_internal_get_fake_standalone_install_prefix(bobui_example_install_prefix)
    else()
        set(bobui_example_install_prefix "${CMAKE_INSTALL_PREFIX}/${INSTALL_EXAMPLESDIR}")
    endif()
    file(TO_CMAKE_PATH "${bobui_example_install_prefix}" bobui_example_install_prefix)
    set(${out_var} "${bobui_example_install_prefix}" PARENT_SCOPE)
endfunction()

# Gets the install prefix where an example's sources should be installed.
# Used for computing the final installation path.
function(bobui_internal_get_examples_sources_install_prefix out_var)
    # Allow customizing the installation path of the examples source specifically.
    if(BOBUI_INTERNAL_EXAMPLES_SOURCES_INSTALL_PREFIX)
        set(bobui_example_install_prefix "${BOBUI_INTERNAL_EXAMPLES_SOURCES_INSTALL_PREFIX}")
    else()
        bobui_internal_get_example_install_prefix(bobui_example_install_prefix)
    endif()
    file(TO_CMAKE_PATH "${bobui_example_install_prefix}" bobui_example_install_prefix)
    set(${out_var} "${bobui_example_install_prefix}" PARENT_SCOPE)
endfunction()
...
# BOBUI_EXAMPLE_BASE_DIR is meant to be already set in a parent scope.
function(bobui_internal_get_example_rel_path out_var subdir)
    file(RELATIVE_PATH example_rel_path
         "${BOBUI_EXAMPLE_BASE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}/${subdir}")
    set(${out_var} "${example_rel_path}" PARENT_SCOPE)
endfunction()

# Gets the install path where an example should be installed.
function(bobui_internal_get_example_install_path out_var subdir)
    bobui_internal_get_example_install_prefix(bobui_example_install_prefix)
    bobui_internal_get_example_rel_path(example_rel_path "${subdir}")
    set(example_install_path "${bobui_example_install_prefix}/${example_rel_path}")

    set(${out_var} "${example_install_path}" PARENT_SCOPE)
endfunction()

# Gets the install path where an example's sources should be installed.
function(bobui_internal_get_examples_sources_install_path out_var subdir)
    bobui_internal_get_examples_sources_install_prefix(bobui_example_install_prefix)
    bobui_internal_get_example_rel_path(example_rel_path "${subdir}")
    set(example_install_path "${bobui_example_install_prefix}/${example_rel_path}")
...
function(bobui_internal_get_example_unique_name out_var subdir)
    bobui_internal_get_example_rel_path(example_rel_path "${subdir}")

    set(name "${subdir}")

    # bobuideclarative has calls like bobui_internal_add_example(imagine/automotive)
...
# Use old non-ExternalProject approach, aka build in-tree with the BobUI build.
function(bobui_internal_add_example_in_tree subdir)
...
")
    ...
    bobui_internal_get_example_install_path(example_install_path "${subdir}")
    ...
    set(BOBUI_INTERNAL_SET_EXAMPLE_INSTALL_DIR_TO_DOT ON)

    add_subdirectory(${subdir})
endfunction()

function(bobui_internal_add_example_external_project subdir)
    set(options "")
    set(singleOpts NAME)
    set(multiOpts "")
    ...
    _bobui_internal_get_build_vars_for_external_projects(
        CMAKE_DIR_VAR bobui_cmake_dir
        PREFIXES_VAR bobui_prefixes
        ADDITIONAL_PACKAGES_PREFIXES_VAR bobui_additional_packages_prefixes
    )

    list(APPEND BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH "${bobui_additional_packages_prefixes}")

    set(vars_to_pass_if_defined)
    set(var_defs)
    if(BOBUI_HOST_PATH OR CMAKE_CROSSCOMPILING)
        list(APPEND var_defs
            -DCMAKE_TOOLCHAIN_FILE:FILEPATH=${bobui_cmake_dir}/bobui.toolchain.cmake
        )
        if(BOBUI_LINT_EXAMPLES)
            list(APPEND var_defs -DBOBUI_LINT_EXAMPLES=ON)
        endif()

    else()
        list(PREPEND CMAKE_PREFIX_PATH ${bobui_prefixes})
    ...
    set(module_paths "${bobui_prefixes}")
    list(TRANSFORM module_paths APPEND "/${INSTALL_CMAKEDIR}/${BOBUI_CMAKE_EXPORT_NAMESPACE}")
    list(APPEND CMAKE_MODULE_PATH ${module_paths})

    # Pass additional paths where qml plugin config files should be included by BobUI6QmlPlugins.cmake.
    # This is needed in prefix builds, where the cmake files are not installed yet.
    set(glob_prefixes "${bobui_prefixes}")
    list(TRANSFORM glob_prefixes APPEND "/${INSTALL_CMAKEDIR}/${BOBUI_CMAKE_EXPORT_NAMESPACE}Qml")
    ...
    if(qml_plugin_cmake_config_file_glob_prefixes)
        set(BOBUI_ADDITIONAL_QML_PLUGIN_GLOB_PREFIXES ${qml_plugin_cmake_config_file_glob_prefixes})
    endif()
    ...
    # BobUIFooConfig.cmake files would error out saying moc is not found.
    # Make sure to build examples only with the main config.
    # When users build an example against an installed BobUI they won't have this problem because
    # the generated non-main BobUIFooTargets-$<CONFIG>.cmake file is empty and doesn't advertise
    # a tool that is not there.
    if(BOBUI_GENERATOR_IS_MULTI_CONFIG)
        set(CMAKE_CONFIGURATION_TYPES "${BOBUI_MULTI_CONFIG_FIRST_CONFIG}")
    endif()
    ...
    # See bobui_internal_set_up_config_optimizations_like_in_qmake, "/Zi" "/Z7".
    ...
    list(APPEND vars_to_pass_if_defined
        ...
        BOBUI_BUILD_CMAKE_PREFIX_PATH:STRING
        BOBUI_ADDITIONAL_PACKAGES_PREFIX_PATH:STRING
        BOBUI_ADDITIONAL_QML_PLUGIN_GLOB_PREFIXES:STRING
        BOBUI_INTERNAL_SKIP_DEPLOYMENT:BOOL
        BOBUI_REPO_EXAMPLES_WARNINGS_CLEAN:BOOL
    ...
    # BOBUI_EXAMPLE_CMAKE_VARS_TO_PASS can be set by specific repos to pass any additional required
    # CMake cache variables.
    # One use case is passing locations of 3rd party package locations like Protobuf via _ROOT
    # variables.
    set(extra_vars_var_name "")
    if(BOBUI_EXAMPLE_CMAKE_VARS_TO_PASS)
        set(extra_vars_var_name "BOBUI_EXAMPLE_CMAKE_VARS_TO_PASS")
    endif()
    ...
    if(BOBUI_INTERNAL_VERBOSE_EXAMPLES)
    ...
    if(BOBUI_EXTRA_EXAMPLE_TARGET_DEFINES)
        unset(extra_args)
        foreach(def IN LISTS BOBUI_EXTRA_EXAMPLE_TARGET_DEFINES)
    ...
    set(deps "")
    list(REMOVE_DUPLICATES BOBUI_EXAMPLE_DEPENDENCIES)
    foreach(dep IN LISTS BOBUI_EXAMPLE_DEPENDENCIES)
        if(TARGET ${dep})
    ...
    if(CMAKE_GENERATOR MATCHES "Ninja")
        option(BOBUI_BUILD_EXAMPLES_WITH_CPU_OVERCOMMIT
            "Allow CPU over-commitment when building examples (Ninja only)"
        )
        if(BOBUI_BUILD_EXAMPLES_WITH_CPU_OVERCOMMIT)
            set(terminal_args)
        endif()
    endif()

    # BOBUI_EXAMPLE_INSTALL_MARKER
    ...
    # E.g. ensure bobuibase/examples/widgets/widgets/wiggly is installed to
    # $bobui_example_install_prefix/examples/widgets/widgets/wiggly/wiggly.exe
    # $bobui_example_install_prefix defaults to ${CMAKE_INSTALL_PREFIX}/${INSTALL_EXAMPLEDIR}
    # but can also be set to a custom location.
    # This needs to work both:
    #  - when using ExternalProject to build examples
    #  - when examples are built in-tree as part of BobUI (no ExternalProject).
    # The reason we want to support the latter is for nicer IDE integration: a can developer can
    # work with a BobUI repo and its examples using the same build dir.
    #
    # In both case we have to ensure examples are not accidentally installed to $bobui_prefix/bin or
    # similar.
    #
    # Example projects installation matrix.
    # 1) ExternalProject + unclean example install rules (INSTALL_EXAMPLEDIR is set) =>
    #    use _bobui_internal_override_example_install_dir_to_dot + ExternalProject_Add's INSTALL_DIR
    #    using relative_dir from BOBUI_EXAMPLE_BASE_DIR to example_source_dir
    #
    # 2) ExternalProject + clean example install rules =>
    #    use ExternalProject_Add's INSTALL_DIR using relative_dir from BOBUI_EXAMPLE_BASE_DIR to
    #    example_source_dir, _bobui_internal_override_example_install_dir_to_dot would be a no-op
    #
    ...
    #    CMAKE_INSTALL_PREFIX using relative_dir from BOBUI_EXAMPLE_BASE_DIR to
    #    example_source_dir, use _bobui_internal_override_example_install_dir_to_dot to ensure
    ...
    set(build_command "")
    if(BOBUI_INTERNAL_VERBOSE_EXAMPLES AND CMAKE_GENERATOR MATCHES "Ninja")
    ...
    ExternalProject_Add(${arg_NAME}
        ...
        CMAKE_CACHE_ARGS ${var_defs}
                         -DBOBUI_INTERNAL_IS_EXAMPLE_EP_BUILD:BOOL=TRUE
                         -DCMAKE_INSTALL_PREFIX:STRING=<INSTALL_DIR>
                         -DBOBUI_INTERNAL_SET_EXAMPLE_INSTALL_DIR_TO_DOT:BOOL=TRUE
        ${terminal_args}
    )
...
function(bobui_internal_install_example_sources subdir)
    set(options "")
    ...
    bobui_internal_get_examples_sources_install_path(example_install_path "${subdir}")
    ...
    if(BOBUI_INSTALL_EXAMPLES_SOURCES_BY_DEFAULT)
