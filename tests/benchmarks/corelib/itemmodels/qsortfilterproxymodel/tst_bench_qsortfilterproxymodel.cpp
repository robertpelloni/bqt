...
void tst_QSortFilterProxyModel::clearFilter_data()
{
    ...
    BOBUIest::addColumn<int>("filteredRowCount");
    ...
    for (int thousandItemCount : { 10, 25, 50, 100, 250, 500, 1000, 2000 }) {
        ...
        BOBUIest::addRow("no match in %dK", thousandItemCount) << itemCount << "-" << 0;
        BOBUIest::addRow("match all in %dK", thousandItemCount) << itemCount << "\\d+" << itemCount;

        BOBUIest::addRow("match first in %dK", thousandItemCount)
                << itemCount << matchSingleItem(1) << 1;
        BOBUIest::addRow("match 1000th in %dK", thousandItemCount)
                << itemCount << matchSingleItem(1000) << 1;
        BOBUIest::addRow("match middle in %dK", thousandItemCount)
                << itemCount << matchSingleItem(itemCount / 2) << 1;
        BOBUIest::addRow("match 1000th from end in %dK", thousandItemCount)
                << itemCount << matchSingleItem(itemCount - 999) << 1;
        BOBUIest::addRow("match last in %dK", thousandItemCount)
                << itemCount << matchSingleItem(itemCount) << 1;

        BOBUIest::addRow("match each 10'000th in %dK", thousandItemCount)
                << itemCount << "0000$" << thousandItemCount / 10;
        BOBUIest::addRow("match each 100'000th in %dK", thousandItemCount)
