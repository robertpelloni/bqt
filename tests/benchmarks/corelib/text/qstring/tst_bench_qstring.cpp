...
void tst_QString::section_data_impl(bool includeRegExOnly)
{
    ...
    BOBUIest::addColumn<bool>("isRegExp");

    BOBUIest::newRow("IPv4") << QStringLiteral("192.168.0.1") << QStringLiteral(".") << false;
    BOBUIest::newRow("IPv6") << QStringLiteral("2001:0db8:85a3:0000:0000:8a2e:0370:7334") << QStringLiteral(":") << false;
    if (includeRegExOnly) {
        BOBUIest::newRow("IPv6-reversed-roles") << QStringLiteral("2001:0db8:85a3:0000:0000:8a2e:0370:7334") << QStringLiteral("\\d+") << true;
        BOBUIest::newRow("IPv6-complex") << QStringLiteral("2001:0db8:85a3:0000:0000:8a2e:0370:7334") << QStringLiteral("(\\d+):\\1") << true;
    }
}
...
void tst_QString::toUpper_data()
{
    ...
    BOBUIest::newRow("600<a>") << (lowerLatin1 + lowerLatin1);
    BOBUIest::newRow("600<A>") << (upperLatin1 + upperLatin1);

    BOBUIest::newRow("300<a>+300<A>") << (lowerLatin1 + upperLatin1);
    BOBUIest::newRow("300<A>+300<a>") << (upperLatin1 + lowerLatin1);

    BOBUIest::newRow("300<10428>") << (lowerDeseret + lowerDeseret);
    BOBUIest::newRow("300<10400>") << (upperDeseret + upperDeseret);

    BOBUIest::newRow("150<10428>+150<10400>") << (lowerDeseret + upperDeseret);
    BOBUIest::newRow("150<10400>+150<10428>") << (upperDeseret + lowerDeseret);

    BOBUIest::newRow("300a+150<10400>") << (lowerLatin1 + upperDeseret);
    BOBUIest::newRow("300a+150<10428>") << (lowerLatin1 + lowerDeseret);
    BOBUIest::newRow("300A+150<10400>") << (upperLatin1 + upperDeseret);
    BOBUIest::newRow("300A+150<10428>") << (upperLatin1 + lowerDeseret);

    BOBUIest::newRow("600<FB03> (ligature)") << lowerLigature;

    QString fullRange;
    for (char32_t i = 0; i <= QChar::LastValidCodePoint; ++i)
        fullRange.append(QChar::fromUcs4(i));

    BOBUIest::newRow("full-range") << fullRange;
}
...
{
    BOBUIest::addColumn<Integer>("number");
    BOBUIest::addColumn<int>("base");
    BOBUIest::addColumn<QString>("expected");

    BOBUIest::newRow("0") << Integer(0ull) << 10 << QStringLiteral("0");
    BOBUIest::newRow("1234") << Integer(1234ull) << 10 << QStringLiteral("1234");
    BOBUIest::newRow("123456789") << Integer(123456789ull) << 10 << QStringLiteral("123456789");
    BOBUIest::newRow("bad1dea, base 16") << Integer(0xBAD1DEAull) << 16 << QStringLiteral("bad1dea");
    BOBUIest::newRow("242, base 8") << Integer(0242ull) << 8 << QStringLiteral("242");
    BOBUIest::newRow("101101, base 2") << Integer(0b101101ull) << 2 << QStringLiteral("101101");
    BOBUIest::newRow("ad, base 30") << Integer(313ull) << 30 << QStringLiteral("ad");
}
...
void tst_QString::number_qlonglong_data()
{
    ...
    BOBUIest::newRow("-1234") << -1234ll << 10 << QStringLiteral("-1234");
    BOBUIest::newRow("-123456789") << -123456789ll << 10 << QStringLiteral("-123456789");
    BOBUIest::newRow("-bad1dea, base 16") << -0xBAD1DEAll << 16 << QStringLiteral("-bad1dea");
    BOBUIest::newRow("-242, base 8") << -0242ll << 8 << QStringLiteral("-242");
    BOBUIest::newRow("-101101, base 2") << -0b101101ll << 2 << QStringLiteral("-101101");
    BOBUIest::newRow("-ad, base 30") << -313ll << 30 << QStringLiteral("-ad");

    BOBUIest::newRow("qlonglong-max")
            << std::numeric_limits<qlonglong>::max() << 10 << QStringLiteral("9223372036854775807");
    BOBUIest::newRow("qlonglong-min")
            << std::numeric_limits<qlonglong>::min() << 10
            << QStringLiteral("-9223372036854775808");
    BOBUIest::newRow("qlonglong-max, base 2")
            << std::numeric_limits<qlonglong>::max() << 2 << QString(63, u'1');
    BOBUIest::newRow("qlonglong-min, base 2") << std::numeric_limits<qlonglong>::min() << 2
                                           << (QStringLiteral("-1") + QString(63, u'0'));
    BOBUIest::newRow("qlonglong-max, base 16")
            << std::numeric_limits<qlonglong>::max() << 16 << (QChar(u'7') + QString(15, u'f'));
    BOBUIest::newRow("qlonglong-min, base 16") << std::numeric_limits<qlonglong>::min() << 16
...
void tst_QString::number_double_data()
{
    ...
    for (auto &datum : data) {
        BOBUIest::addRow("%s, format '%c', precision %d", qPrintable(datum.expected), datum.f,
                      datum.p)
...
void toWholeCommon_data()
{
    ...
    BOBUIest::newRow("empty") << u""_s << 10 << false << Integer(0ull);
    BOBUIest::newRow("0") << u"0"_s << 10 << true << Integer(0ull);
    BOBUIest::newRow("1234") << u"1234"_s << 10 << true << Integer(1234ull);
    BOBUIest::newRow("1,234") << u"1,234"_s << 10 << false << Integer(0ull);
    BOBUIest::newRow("123456789")
            << u"123456789"_s << 10 << true << Integer(123456789ull);
    BOBUIest::newRow("bad1dea, base 16")
            << u"bad1dea"_s << 16 << true << Integer(0xBAD1DEAull);
    BOBUIest::newRow("bad1dea, base 10") << u"bad1dea"_s << 10 << false << Integer(0ull);
    BOBUIest::newRow("42, base 13") << u"42"_s << 13 << true << Integer(6ull * 9ull);
    BOBUIest::newRow("242, base 8") << u"242"_s << 8 << true << Integer(0242ull);
    BOBUIest::newRow("495, base 8") << u"495"_s << 8 << false << Integer(0ull);
    BOBUIest::newRow("101101, base 2")
            << u"101101"_s << 2 << true << Integer(0b101101ull);
    BOBUIest::newRow("ad, base 30") << u"ad"_s << 30 << true << Integer(313ull);
}
...
void tst_QString::toLongLong_data()
{
    ...
    BOBUIest::newRow("-1234") << u"-1234"_s << 10 << true << -1234ll;
    BOBUIest::newRow("-123456789") << u"-123456789"_s << 10 << true << -123456789ll;
    BOBUIest::newRow("-bad1dea, base 16") << u"-bad1dea"_s << 16 << true << -0xBAD1DEAll;
    BOBUIest::newRow("-242, base 8") << u"-242"_s << 8 << true << -0242ll;
    BOBUIest::newRow("-101101, base 2") << u"-101101"_s << 2 << true << -0b101101ll;
    BOBUIest::newRow("-ad, base 30") << u"-ad"_s << 30 << true << -313ll;
    ...
    BOBUIest::newRow("qlonglong-min, base 2")
            << (u"-1"_s + QString(63, u'0')) << 2 << true
            << std::numeric_limits<qlonglong>::min();
    BOBUIest::newRow("qlonglong-max, base 16")
            << (QChar(u'7') + QString(15, u'f')) << 16 << true
            << std::numeric_limits<qlonglong>::max();
    BOBUIest::newRow("qlonglong-min, base 16")
...
void tst_QString::toDouble_data()
{
    ...
    BOBUIest::newRow("empty") << u""_s << false << 0.0;
    BOBUIest::newRow("0") << u"0"_s << true << 0.0;
    BOBUIest::newRow("0.12340") << u"0.12340"_s << true << 0.12340;
    BOBUIest::newRow("-0.12340") << u"-0.12340"_s << true << -0.12340;
    BOBUIest::newRow("epsilon")
        << u"2.220446049e-16"_s << true << std::numeric_limits<double>::epsilon();
    BOBUIest::newRow("1.0e-4") << u"1.0e-4"_s << true << 1.0e-4;
    BOBUIest::newRow("1.0e+4") << u"1.0e+4"_s << true << 1.0e+4;
    BOBUIest::newRow("10.e+3") << u"10.e+3"_s << true << 1.0e+4;
    BOBUIest::newRow("10e+3.") << u"10e+3."_s << false << 0.0;
    BOBUIest::newRow("1e4") << u"1e4"_s << true << 1.0e+4;
    BOBUIest::newRow("1.0e-8") << u"1.0e-8"_s << true << 1.0e-8;
    BOBUIest::newRow("1.0e+8") << u"1.0e+8"_s << true << 1.0e+8;

    // NaN and infinity:
    BOBUIest::newRow("nan") << u"nan"_s << true << qQNaN();
    BOBUIest::newRow("NaN") << u"NaN"_s << true << qQNaN();
    BOBUIest::newRow("-nan") << u"-nan"_s << false << 0.0;
    BOBUIest::newRow("+nan") << u"+nan"_s << false << 0.0;
    BOBUIest::newRow("inf") << u"inf"_s << true << qInf();
    BOBUIest::newRow("Inf") << u"Inf"_s << true << qInf();
    BOBUIest::newRow("+inf") << u"+inf"_s << true << qInf();
    BOBUIest::newRow("-inf") << u"-inf"_s << true << -qInf();
}
...
BOBUIEST_APPLESS_MAIN(tst_QString)
