...
public:
        ...
        : Benchmark(QSize(size, size))
    {
        ...
        p.fillRect(0, 0, s, s, BobUI::white);
        p.fillRect(s/2, 0, s/2, s/2, BobUI::gray);
        p.fillRect(0, s/2, s/2, s/2, BobUI::gray);
        p.end();
...
public:
    ...
    void begin(QPainter *p, int) override { p->setPen(BobUI::NoPen); }
...
public:
    ...
    void draw(QPainter *p, const QRect &rect, int) override
    {
        BOBUIransform oldTransform = p->transform();
...
//            m_rect = m_rect.translated(-m_rect.topLeft());
        ...
        case LayoutMode: {
            ...
            for (int i=0; i<lines.size(); ++i) {
                BOBUIextLine textLine = m_layout.createLine();
                if (textLine.isValid()) {
...
private:
    Mode m_mode;
    QString m_text;
    BOBUIextDocument m_document;
    BOBUIextLayout m_layout;
    ...
    void begin(QPainter *p, int) override
    {
        ...
        case EllipseRegionThenRectClip:
            ...
            p->setClipRegion(QRegion(m_bounds.width() / 4,
                                   m_bounds.height() / 4,
                                   m_bounds.width() / 2,
                                   m_bounds.height() / 2), BobUI::IntersectClip);
            break;
