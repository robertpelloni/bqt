...
class tst_BobUIBench : public QObject
{
    Q_OBJECT
...
void tst_BobUIBench::bobuiBench_data()
{
    BOBUIest::addColumn<void *>("benchmark");
    ...
    for (Benchmark *benchmark : std::as_const(benchmarks))
        BOBUIest::newRow(qPrintable(benchmark->name())) << reinterpret_cast<void *>(benchmark);
}

void tst_BobUIBench::bobuiBench()
{
    ...
    BOBUIest::setBenchmarkResult(widget.result(), BOBUIest::WalltimeMilliseconds);
}

BOBUIEST_MAIN(tst_BobUIBench)
#include "tst_bobuibench.moc"
