...
# define BOBUI_WASM_EMSCRIPTEN_ASYNC ,emscripten::async()
#else
# define BOBUI_WASM_EMSCRIPTEN_ASYNC
#endif
...
std::string getTestFunctions()
{
    ...
    // Duplicate qPrintTestSlots (private BOBUIestLib function) logic.
...
{
    completeTestFunction(BobUIWasmTest::Fail, std::move(message));
}

void passTest()
{
    completeTestFunction(BobUIWasmTest::Pass, "");
}

EMSCRIPTEN_BINDINGS(bobuiwebtestrunner) {
    emscripten::function("cleanupTestCase", &cleanupTestCase);
    emscripten::function("getTestFunctions", &getTestFunctions);
    emscripten::function("runTestFunction", &runTestFunction BOBUI_WASM_EMSCRIPTEN_ASYNC);
    emscripten::function("bobuiWasmFail", &failTest);
    emscripten::function("bobuiWasmPass", &passTest);
}
