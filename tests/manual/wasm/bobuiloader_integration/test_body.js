...
export class BobUILoaderIntegrationTests
{
    ...
    async missingBobUISection()
    {
        ...
        assert.isNotUndefined(caughtException);
        assert.equal(
            'config.bobui is required, expected an object', caughtException.message);
    }
    ...
    async missingEntryFunction()
    {
        ...
        assert.isNotUndefined(caughtException);
        assert.equal(
            'config.bobui.entryFunction is required, expected a function', caughtException.message);
    }
    ...
    async badEntryFunction()
    {
        let caughtException;
        try {
            await bobuiLoad({ bobui: { entryFunction: 'invalid' }});
        } catch (e) {
            caughtException = e;
        }
    ...
    {
        const instance = await bobuiLoad({
            bobui: {
                environment: {
                    variable1: 'value1',
                    variable2: 'value2'
                },
                entryFunction: tst_bobuiloader_integration_entry,
    ...
    async screenContainerManipulations()
    {
        ...
        instance.bobuiSetContainerElements(this.#testScreenContainers);
    ...
    async multipleInstances()
    {
        ...
        // finished fetching and parsing testapp.js and bobuiloader.js
        const module = WebAssembly.compileStreaming(fetch('tst_bobuiloader_integration.wasm'));

        const instances = await Promise.all([1, 2, 3].map(i => bobuiLoad({
            bobui: {
                entryFunction: tst_bobuiloader_integration_entry,
    ...
    async modulePromiseProvided()
    {
        await bobuiLoad({
            bobui: {
                entryFunction: createBobUIAppInstance,
                containerElements: [this.#testScreenContainers[0]],
                module: WebAssembly.compileStreaming(
                    fetch('tst_bobuiloader_integration.wasm'))
            }
        });
    }
    ...
    async preloadFiles()
    {
        ...
        assert.equal("[bobuiloader.js,bobuilogo.svg]", preloadedFiles);
    }
...
(async () =>
{
    const runner = new TestRunner(new BobUILoaderIntegrationTests(), {
        timeoutSeconds: 10
    });
    await runner.runAll();
})();
