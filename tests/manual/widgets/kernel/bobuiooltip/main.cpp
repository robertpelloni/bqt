...
class BOBUIoolTipTest : public QProxyStyle
{
    Q_OBJECT
public:
    BOBUIoolTipTest() : QProxyStyle()
    {
...
class TestDialog : public QDialog
{
    Q_OBJECT
public:
    TestDialog(BOBUIoolTipTest *s);
    BOBUIoolTipTest *style;
protected slots:
...
void TestDialog::showSomeToolTips()
{
    ...
    for (int u = 1; u < 20; u += 5) {
        ...
        BOBUIoolTip::showText(p, s, 0, QRect(), 1000 * u);
        BOBUIest::qWait((u + 1) * 1000);
    }

    BOBUIoolTip::showText(p, tr("Seconds: 2"), 0, QRect(), 2000);
    BOBUIest::qWait(3000);

    BOBUIoolTip::showText(p, tr("Standard label"), 0, QRect());
    BOBUIest::qWait(12000);
}

TestDialog::TestDialog(BOBUIoolTipTest *s) : style(s)
{
...
int main(int argc, char *argv[])
{
    ...
    BOBUIoolTipTest *style = new BOBUIoolTipTest();
