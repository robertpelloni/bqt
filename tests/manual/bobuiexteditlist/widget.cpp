    ...
    ui(new Ui::Widget)
{
    ...
    // Changing font size and indent values to verify fix for BOBUIBUG-5111.
    ...
    textCursor = new BOBUIextCursor(ui->textEdit->document());
    ...
    textCursor->insertText("\ndecimal two");
    // a., b.
    listFormat.setStyle(BOBUIextListFormat::ListLowerAlpha);
    textCursor->insertList(listFormat);
    textCursor->insertText("lower alpha one");
    textCursor->insertText("\nlower alpha two");
    // A., B.
    listFormat.setStyle(BOBUIextListFormat::ListUpperAlpha);
    textCursor->insertList(listFormat);
