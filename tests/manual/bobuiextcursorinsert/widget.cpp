...
void Widget::on_insertMarkdownButton_clicked()
{
    if (ui->newBlockBeforeCB->isChecked()) {
        if (ui->moveToBeginningRB->isChecked())
            ui->textEdit->moveCursor(BOBUIextCursor::StartOfBlock);
        if (ui->moveToEndRB->isChecked())
            ui->textEdit->moveCursor(BOBUIextCursor::EndOfBlock);
        if (ui->defaultBlockFormatCB->isChecked())
            ui->textEdit->textCursor().insertBlock(BOBUIextBlockFormat());
        else
...
void Widget::on_richTextCB_activated(int index)
{
    if (index < m_texts.size()) {
        auto it = m_texts.constBegin();
        std::advance(it, index);
        if (it.key().contains(u"markdown", BobUI::CaseInsensitive))
            ui->textEdit->setMarkdown(it.value());
        else if (it.key().contains(u"html", BobUI::CaseInsensitive))
...
{
    BOBUIextBlock block = ui->textEdit->textCursor().block();
    ...
    if (list) {
        ...
        ui->listDesc->setText(u"index %1: \"%2\" is in list\nwith style %3 and %4 items starting with %5: %6 %7"_s
                              ...
                              .arg(list->format().style() > BOBUIextListFormat::ListDecimal ? u""_s : list->itemText(first))
                              .arg(first.text()) );
    } else {
