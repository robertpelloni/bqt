...
TabletWidget::TabletWidget(bool mouseToo) : mMouseToo(mouseToo), mWheelEventCount(0), mQuitShortcut(QKeySequence::Quit, this)
{
    ...
    newPalette.setColor(QPalette::WindowText, BobUI::black);
...
bool TabletWidget::eventFilter(QObject *, QEvent *ev)
{
    ...
    case QEvent::TabletRelease:
        {
            BOBUIabletEvent *event = static_cast<BOBUIabletEvent*>(ev);
...
const char *TabletWidget::buttonToString(BobUI::MouseButton b)
{
...
QString TabletWidget::buttonsToString(BobUI::MouseButtons bs)
{
    QStringList ret;
    for (int i = 0; (uint)(1 << i) <= BobUI::MaxMouseButton; ++i) {
        BobUI::MouseButton b = static_cast<BobUI::MouseButton>(1 << i);
        if (bs.testFlag(b))
...
QString TabletWidget::modifiersToString(BobUI::KeyboardModifiers m)
{
    QStringList ret;
    if (m & BobUI::ShiftModifier)
        ret << QLatin1String("Shift");
    if (m & BobUI::ControlModifier)
        ret << QLatin1String("Control");
    if (m & BobUI::AltModifier)
        ret << QLatin1String("Alt");
    if (m & BobUI::MetaModifier)
        ret << QLatin1String("Meta");
    if (m & BobUI::KeypadModifier)
        ret << QLatin1String("Keypad");
    if (m & BobUI::GroupSwitchModifier)
        ret << QLatin1String("GroupSwitch");
    return ret.join(QLatin1Char('|'));
}

void TabletWidget::tabletEvent(BOBUIabletEvent *event)
{
    event->accept();
}
