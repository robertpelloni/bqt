...
struct TabletPoint
{
    TabletPoint(const QPointF &p = QPointF(), TabletPointType t = TabletMove, BobUI::MouseButton b = BobUI::LeftButton,
...
protected:
    ...
    void tabletEvent(BOBUIabletEvent *) override;
...
private:
    ...
    BobUI::MouseButton m_lastButton = BobUI::NoButton;
    ...
    for (const TabletPoint &t : std::as_const(m_points)) {
        if (geom.contains(t.pos)) {
              ...
              case TabletButtonPress:
                  p.fillPath(pp, BobUI::darkGreen);
                  if (t.button != BobUI::NoButton)
                    p.drawText(pointBounds, BobUI::AlignCenter, QString::number(t.button));
                  break;
              case TabletButtonRelease:
                  p.fillPath(pp, BobUI::red);
                  if (t.button != BobUI::NoButton)
                    p.drawText(pointBounds, BobUI::AlignCenter, QString::number(t.button));
                  break;
              case TabletMove:
                  if (t.pressure > 0.0) {
                      p.setPen(t.ptype == QPointingDevice::PointerType::Eraser ? BobUI::red : BobUI::black);
                      if (t.angle != 0.0) {
                          p.save();
                          p.translate(t.pos);
                      ...
                      p.fillRect(t.pos.x() - 2, t.pos.y() - 2, 4, 4, BobUI::black);
                  }
                  break;
              }
          }
...
void EventReportWidget::tabletEvent(BOBUIabletEvent *event)
{
    ...
    if (!(isMove && m_lastIsTabletMove)) {
        ...
        if (event->button() != BobUI::NoButton)
...
void EventReportWidget::timerEvent(BOBUIimerEvent *)
{
...
{
    setWindowTitle(QString::fromLatin1("Tablet Test %1").arg(BOBUI_VERSION_STR));
    ...
    showAction->setShortcut(BobUI::CTRL | BobUI::Key_D);
...
void MainWindow::showDevices()
{
    if (m_devicesDialog.isNull()) {
        ...
        m_devicesDialog->setAttribute(BobUI::WA_DeleteOnClose);
    }
