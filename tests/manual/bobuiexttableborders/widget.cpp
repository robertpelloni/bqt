...
// This manual test allows checking the BOBUIextTable border logic (BOBUIBUG-36152)
    ...
    ui(new Ui::Widget)
{
    ...
    connect(ui->sourceEdit, &BOBUIextEdit::textChanged, this,
            [this]() {
...
void Widget::onPreview()
{
    ...
    connect(&preview, &QPrintPreviewDialog::paintRequested, ui->htmlEdit, &BOBUIextEdit::print);
    preview.exec();
#endif
}
...
#ifndef BOBUI_NO_DESKTOPSERVICES
    auto tf = new BOBUIemporaryFile(QDir::tempPath() + "/XXXXXX.html", this);
    if (tf->open()) {
