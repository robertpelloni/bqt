...
BOBUIestData &newRow(const char *dataTag, quint16 checksum = 0);
...
do {\
    ...
    bool _err = false;\
    if (!QBaselineTest::checkImage((image), (name), (checksum), &_msg, &_err)) {\
        BOBUIest::qFail(_msg.constData(), __FILE__, __LINE__);\
    } else if (_err) {\
        QSKIP(_msg.constData());\
    }\
} while (0)
...
#define QBASELINETEST_MAIN(TestObject) BOBUIEST_MAIN_WRAPPER(TestObject, \
