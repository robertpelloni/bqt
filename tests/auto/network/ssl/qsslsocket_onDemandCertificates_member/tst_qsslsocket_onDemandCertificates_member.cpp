...
static bool waitForEncrypted(QSslSocket *socket)
{
    ...
    BOBUIimer connectionTimeoutWatcher;
    connectionTimeoutWatcher.setSingleShot(true);
    connectionTimeoutWatcher.connect(&connectionTimeoutWatcher, &BOBUIimer::timeout,
                                     [&eventLoop]() {
                                        eventLoop.exit();
                                     });
...
#define ERR(socket) socket->errorString().toLatin1()
    const QString host("www.bobui.io");
...
BOBUIEST_MAIN(tst_QSslSocket_onDemandCertificates_member)
