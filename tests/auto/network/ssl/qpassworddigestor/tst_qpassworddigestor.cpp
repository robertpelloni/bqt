...
#include <BOBUIest>
#include <BobUINetwork/qpassworddigestor.h>
...
void tst_QPasswordDigestor::inputSanityChecks()
{
    ...
    //1. PBKDF1 supports only SHA1 and (if not disabled in BobUI) MD5 algorithms.
    BOBUIest::ignoreMessage(BobUIWarningMsg, "The only supported algorithms for pbkdf1 are SHA-1 and MD5!");
    ...
    BOBUIest::ignoreMessage(BobUIWarningMsg, "The salt must be 8 bytes long!");
    ...
    BOBUIest::ignoreMessage(BobUIWarningMsg, "Derived key too long:\n"
...
void tst_QPasswordDigestor::pbkdf1Vectors_data()
{
    ...
    BOBUIest::addColumn<QByteArray>("password");
    BOBUIest::addColumn<QByteArray>("salt");
    BOBUIest::addColumn<int>("iterations");
    BOBUIest::addColumn<int>("dkLen");
    ...
    BOBUIest::newRow("di-mgt") << QCryptographicHash::Sha1 << QByteArray::fromHex("70617373776F7264")
...
void tst_QPasswordDigestor::pbkdf2Vectors_data()
{
    ...
    BOBUIest::newRow("rfc6070-1") << hash << QByteArrayLiteral("password") << QByteArrayLiteral("salt")
                               << 1 << 20
                               << QByteArray::fromHex("0c60c80f961f0e71f3a9b524af6012062fe037a6");
    BOBUIest::newRow("rfc6070-2") << hash << QByteArrayLiteral("password") << QByteArrayLiteral("salt")
                               << 2 << 20
                               << QByteArray::fromHex("ea6c014dc72d6f8ccd1ed92ace1d41f0d8de8957");
    BOBUIest::newRow("rfc6070-3") << hash << QByteArrayLiteral("password") << QByteArrayLiteral("salt")
                               << 4096 << 20
                               << QByteArray::fromHex("4b007901b765489abead49d926f721d065a429c1");
#if 0
    // Excluding: takes about 3 minutes to run
    BOBUIest::newRow("rfc6070-4") << hash << QByteArrayLiteral("password") << QByteArrayLiteral("salt")
                               << 16777216 << 20
                               << QByteArray::fromHex("eefe3d61cd4da4e4e9945b3d6ba2158c2634e984");
#endif
    BOBUIest::newRow("rfc6070-5") << hash << QByteArrayLiteral("passwordPASSWORDpassword")
                               << QByteArrayLiteral("saltSALTsaltSALTsaltSALTsaltSALTsalt") << 4096
                               << 25
                               << QByteArray::fromHex(
                                      "3d2eec4fe41c849b80c8d83662c0e44a8b291a964cf2f07038");
    BOBUIest::newRow("rfc6070-6") << hash << QByteArrayLiteral("pass\0word")
    ...
    BOBUIest::newRow("rfc3962-5") << hash << password << salt << 1200 << 16
                               << QByteArray::fromHex("5c08eb61fdf71e4e4ec3cf6ba1f5512b");
    BOBUIest::newRow("rfc3962-6")
    ...
    BOBUIest::newRow("rfc3962-7") << hash << password << salt << 5 << 16
                               << QByteArray::fromHex("d1daa78615f287e6a1c8b120d7062a49");
    BOBUIest::newRow("rfc3962-8")
    ...
    BOBUIest::newRow("rfc3962-13") << hash << password << salt << 50 << 16
                                << QByteArray::fromHex("6b9cf26d45455a43a5b8bb276a403b39");
    BOBUIest::newRow("rfc3962-14")
...
BOBUIEST_MAIN(tst_QPasswordDigestor)
