...
#include <private/bobuiexturefilereader_p.h>
#include <BOBUIest>

class tst_bobuiexturefilereader : public QObject
{
    Q_OBJECT
...
void tst_bobuiexturefilereader::checkHandlers_data()
{
    BOBUIest::addColumn<QString>("fileName");
    BOBUIest::addColumn<QSize>("size");
    BOBUIest::addColumn<quint32>("glFormat");
    BOBUIest::addColumn<quint32>("glInternalFormat");
    BOBUIest::addColumn<quint32>("glBaseInternalFormat");
    BOBUIest::addColumn<int>("levels");
    BOBUIest::addColumn<int>("faces");
    BOBUIest::addColumn<QList<int>>("dataOffsets");
    BOBUIest::addColumn<QList<int>>("dataLengths");

    BOBUIest::addRow("pattern.pkm")
    ...
    BOBUIest::addRow("cubemap_float32_rgba.ktx")
    ...
    BOBUIest::addRow("newlogo_srgb.astc")
...
void tst_bobuiexturefilereader::checkHandlers()
{
    ...
    BOBUIextureFileReader r(&f, fileName);
    QVERIFY(r.canRead());

    BOBUIextureFileData tex = r.read();
    QVERIFY(!tex.isNull());
    QVERIFY(tex.isValid());
...
void tst_bobuiexturefilereader::checkMetadata()
{
    QFile f(":/texturefiles/cubemap_metadata.ktx");
    QVERIFY(f.open(QIODevice::ReadOnly));
    BOBUIextureFileReader r(&f);
    BOBUIextureFileData d = r.read();
...
void tst_bobuiexturefilereader::checkInvalid()
{
...
BOBUIEST_MAIN(tst_bobuiexturefilereader)

#include "tst_bobuiexturefilereader.moc"
