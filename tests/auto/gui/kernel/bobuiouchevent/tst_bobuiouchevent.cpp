...
#include <BobUIWidgets/QGraphicsWidget>
...
class tst_BOBUIouchEventWidget : public QWidget
{
public:
    ...
    explicit tst_BOBUIouchEventWidget(QWidget *parent = nullptr) : QWidget(parent)
    {
        reset();
    }
    ...
    void paintEvent(QPaintEvent *) override
    {
        ...
        painter.setPen(BobUI::darkGray);
        painter.drawText(rect(), BobUI::AlignHCenter | BobUI::AlignCenter, objectName());
        ...
        painter.setPen(BobUI::darkYellow);
        for (const auto &pt : std::as_const(touchUpdatePoints))
            painter.drawText(pt.position(), pointFormat.arg(pt.position().toPoint().x()).arg(pt.position().toPoint().y()));
        painter.setPen(BobUI::darkRed);
...
class tst_BOBUIouchEventGraphicsItem : public QGraphicsItem
{
public:
    QList<QEventPoint> touchBeginPoints, touchUpdatePoints, touchEndPoints;
    bool seenTouchBegin, seenTouchUpdate, seenTouchEnd;
    ...
    bool acceptTouchBegin, acceptTouchUpdate, acceptTouchEnd;
    bool deleteInTouchBegin, deleteInTouchUpdate, deleteInTouchEnd;
    tst_BOBUIouchEventGraphicsItem **weakpointer;

    explicit tst_BOBUIouchEventGraphicsItem(QGraphicsItem *parent = nullptr)
        : QGraphicsItem(parent), weakpointer(0)
    {
        reset();
    }

    ~tst_BOBUIouchEventGraphicsItem()
    {
        if (weakpointer)
            *weakpointer = 0;
    }
    ...
    {
        painter->fillRect(QRectF(QPointF(0, 0), boundingRect().size()), BobUI::yellow);
    }
    ...
    bool sceneEvent(QEvent *event) override
    {
        ...
        case QEvent::TouchBegin:
            ...
            seenTouchBegin = !seenTouchBegin && !seenTouchUpdate && !seenTouchEnd;
            ++touchBeginCounter;
            touchBeginPoints = static_cast<BOBUIouchEvent *>(event)->points();
            event->setAccepted(acceptTouchBegin);
            if (deleteInTouchBegin)
                delete this;
            break;
        case QEvent::TouchUpdate:
            ...
            seenTouchUpdate = seenTouchBegin && !seenTouchEnd;
            ++touchUpdateCounter;
            touchUpdatePoints = static_cast<BOBUIouchEvent *>(event)->points();
            event->setAccepted(acceptTouchUpdate);
            if (deleteInTouchUpdate)
                delete this;
            break;
        case QEvent::TouchEnd:
            ...
            seenTouchEnd = seenTouchBegin && !seenTouchEnd;
            ++touchEndCounter;
            touchEndPoints = static_cast<BOBUIouchEvent *>(event)->points();
            event->setAccepted(acceptTouchEnd);
            if (deleteInTouchEnd)
                delete this;
            break;
...
struct GrabberWindow : public QWindow
{
    ...
    void touchEvent(BOBUIouchEvent *ev) override {
        qCDebug(lcTests) << ev;
...
class tst_BOBUIouchEvent : public QObject
{
    Q_OBJECT
public:
    tst_BOBUIouchEvent();
...
tst_BOBUIouchEvent::tst_BOBUIouchEvent()
  : touchScreenDevice(BOBUIest::createTouchDevice())
  , secondaryTouchScreenDevice(BOBUIest::createTouchDevice())
  , touchPadDevice(BOBUIest::createTouchDevice(QInputDevice::DeviceType::TouchPad))
{
...
void tst_BOBUIouchEvent::cleanup()
{
    QVERIFY(QGuiApplication::topLevelWindows().isEmpty());
}

void tst_BOBUIouchEvent::qPointerUniqueId()
{
...
void tst_BOBUIouchEvent::state()
{
    BOBUIouchEvent touchEvent(QEvent::TouchBegin, touchScreenDevice,
                           BobUI::NoModifier, QList<QEventPoint>() <<
    ...
    BOBUIouchEvent touchEvent2 = BOBUIouchEvent(QEvent::TouchBegin, touchScreenDevice,
                              BobUI::NoModifier, QList<QEventPoint>() <<
    ...
    BOBUIouchEvent touchEvent3 = BOBUIouchEvent(QEvent::TouchBegin, touchScreenDevice,
                              BobUI::NoModifier, QList<QEventPoint>() <<
                              QEventPoint(0, QEventPoint::State::Updated, {}, {}) <<
    ...
    BOBUI_WARNING_PUSH BOBUI_WARNING_DISABLE_DEPRECATED // test BobUI 5 compatibility wrappers
    QCOMPARE(touchEvent3.touchPoints(), touchEvent3.points());
    BOBUI_WARNING_POP
#endif
}

void tst_BOBUIouchEvent::touchDisabledByDefault()
{
    // QWidget
    {
        // the widget attribute is not enabled by default
        QWidget widget;
        QVERIFY(!widget.testAttribute(BobUI::WA_AcceptTouchEvents));

        // events should not be accepted since they are not enabled
        QList<QEventPoint> touchPoints;
        touchPoints.append(QEventPoint(0));
        BOBUIouchEvent touchEvent(QEvent::TouchBegin,
                               touchScreenDevice,
                               BobUI::NoModifier,
                               touchPoints);
    ...
    // QGraphicsView
    {
        ...
        tst_BOBUIouchEventGraphicsItem item;
...
void tst_BOBUIouchEvent::touchEventAcceptedByDefault()
{
    // QWidget
    {
        ...
        BOBUIouchEvent touchEvent(QEvent::TouchBegin,
                               ...
                               touchPoints);
        QVERIFY(QApplication::sendEvent(&widget, &touchEvent));
        QVERIFY(!touchEvent.isAccepted()); // BobUI 5.X ignores touch events.

        // tst_BOBUIouchEventWidget does handle, sending succeeds
        tst_BOBUIouchEventWidget touchWidget;
        touchWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
        touchEvent.ignore();
    ...
    // QGraphicsView
    {
        ...
        QVERIFY(view.viewport()->testAttribute(BobUI::WA_AcceptTouchEvents));
...
void tst_BOBUIouchEvent::touchBeginPropagatesWhenIgnored()
{
    // QWidget
    {
        tst_BOBUIouchEventWidget window, child, grandchild;
        ...
        child.setAttribute(BobUI::WA_AcceptTouchEvents);
        grandchild.setAttribute(BobUI::WA_AcceptTouchEvents);
        ...
        child.reset();
        window.reset();
        grandchild.setAttribute(BobUI::WA_AcceptTouchEvents, false);
    ...
    {
        QGraphicsScene scene;
        tst_BOBUIouchEventGraphicsItem root, child, grandchild;
        QGraphicsView view(&scene);
        scene.addItem(&root);
        root.setPos(100, 100);
...
void tst_BOBUIouchEvent::touchUpdateAndEndNeverPropagate()
{
    // QWidget
    {
        tst_BOBUIouchEventWidget window, child;
        child.setParent(&window);
        ...
        QList<QEventPoint> touchPoints;
        touchPoints.append(QEventPoint(0));
        BOBUIouchEvent touchBeginEvent(QEvent::TouchBegin,
                                    touchScreenDevice,
                                    BobUI::NoModifier,
                                    touchPoints);
        ...
        BOBUIouchEvent touchUpdateEvent(QEvent::TouchUpdate,
                                     touchScreenDevice,
                                     BobUI::NoModifier,
                                     touchPoints);
        ...
        BOBUIouchEvent touchEndEvent(QEvent::TouchEnd,
                                  touchScreenDevice,
                                  BobUI::NoModifier,
                                  touchPoints);
...
void tst_BOBUIouchEvent::basicRawEventTranslation()
{
    tst_BOBUIouchEventWidget touchWidget;
    touchWidget.setWindowTitle(BOBUIest::currentTestFunction());
    touchWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
    touchWidget.setGeometry(100, 100, 400, 300);
    touchWidget.show();
    QVERIFY(BOBUIest::qWaitForWindowExposed(&touchWidget));
...
void tst_BOBUIouchEvent::multiPointRawEventTranslationOnTouchScreen()
{
    tst_BOBUIouchEventWidget touchWidget;
    ...
    tst_BOBUIouchEventWidget leftWidget(&touchWidget);
    leftWidget.setObjectName("leftWidget");
    leftWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
    leftWidget.setGeometry(0, 100, 100, 100);

    tst_BOBUIouchEventWidget rightWidget(&touchWidget);
    rightWidget.setObjectName("rightWidget");
    rightWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
    ...
    auto touchSequence = BOBUIest::touchEvent(touchWidget.windowHandle(), touchScreenDevice);
...
void tst_BOBUIouchEvent::touchOnMultipleTouchscreens()
{
    tst_BOBUIouchEventWidget touchWidget;
    touchWidget.setWindowTitle(BOBUIest::currentTestFunction());
    touchWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
    ...
    touchWidget.setAttribute(BobUI::WA_WState_AcceptedTouchBeginEvent, true);
...
void tst_BOBUIouchEvent::multiPointRawEventTranslationOnTouchPad()
{
#ifdef Q_OS_MACOS
#if BOBUI_CONFIG(cursor)
    QCursor::setPos(0, 0); // move mouse out of the way
    if (!BOBUIest::qWaitFor([]{ return QCursor::pos() == QPoint(0, 0); }))
#endif
    ...
    touchWidget.show();
    QVERIFY(BOBUIest::qWaitForWindowActive(&touchWidget));
...
void tst_BOBUIouchEvent::basicRawEventTranslationOfIds()
{
    tst_BOBUIouchEventWidget touchWidget;
    touchWidget.setWindowTitle(BOBUIest::currentTestFunction());
    touchWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
...
void tst_BOBUIouchEvent::deleteInEventHandler()
{
    // QWidget
    {
        QWidget window;
        QPointer<tst_BOBUIouchEventWidget> child1 = new tst_BOBUIouchEventWidget(&window);
        QPointer<tst_BOBUIouchEventWidget> child2 = new tst_BOBUIouchEventWidget(&window);
        QPointer<tst_BOBUIouchEventWidget> child3 = new tst_BOBUIouchEventWidget(&window);
        child1->setAttribute(BobUI::WA_AcceptTouchEvents);
        child2->setAttribute(BobUI::WA_AcceptTouchEvents);
        child3->setAttribute(BobUI::WA_AcceptTouchEvents);
    ...
    // QGraphicsView
    {
        QGraphicsScene scene;
        QGraphicsView view(&scene);
        QScopedPointer<tst_BOBUIouchEventGraphicsItem> root(new tst_BOBUIouchEventGraphicsItem);
        tst_BOBUIouchEventGraphicsItem *child1 = new tst_BOBUIouchEventGraphicsItem(root.data());
        tst_BOBUIouchEventGraphicsItem *child2 = new tst_BOBUIouchEventGraphicsItem(root.data());
        tst_BOBUIouchEventGraphicsItem *child3 = new tst_BOBUIouchEventGraphicsItem(root.data());
        child1->setZValue(1.);
        child2->setZValue(0.);
        child3->setZValue(-1.);
        ...
        touchUpdateEvent.ignore();
        QVERIFY(QApplication::sendEvent(view.viewport(), &touchUpdateEvent));
        QVERIFY(!touchUpdateEvent.isAccepted()); // BobUI 5.X ignores touch events.
        QVERIFY(!child1);
        touchEndEvent.ignore();
        QVERIFY(QApplication::sendEvent(view.viewport(), &touchEndEvent));
        QVERIFY(!touchUpdateEvent.isAccepted());
...
void tst_BOBUIouchEvent::deleteInRawEventTranslation()
{
    tst_BOBUIouchEventWidget touchWidget;
    touchWidget.setWindowTitle(BOBUIest::currentTestFunction());
    touchWidget.setAttribute(BobUI::WA_AcceptTouchEvents);
    ...
    QPointer<tst_BOBUIouchEventWidget> leftWidget = new tst_BOBUIouchEventWidget(&touchWidget);
    leftWidget->setAttribute(BobUI::WA_AcceptTouchEvents);
    leftWidget->setGeometry(0, 100, 100, 100);
    leftWidget->deleteInTouchBegin = true;

    QPointer<tst_BOBUIouchEventWidget> centerWidget = new tst_BOBUIouchEventWidget(&touchWidget);
    centerWidget->setAttribute(BobUI::WA_AcceptTouchEvents);
    centerWidget->setGeometry(100, 100, 100, 100);
    centerWidget->deleteInTouchUpdate = true;

    QPointer<tst_BOBUIouchEventWidget> rightWidget = new tst_BOBUIouchEventWidget(&touchWidget);
    rightWidget->setAttribute(BobUI::WA_AcceptTouchEvents);
...
void tst_BOBUIouchEvent::crashInQGraphicsSceneAfterNotHandlingTouchBegin()
{
    ...
    // Touch the button
    BOBUIest::touchEvent(view.viewport(), touchScreenDevice).press(0, centerPos, nullptr);
    BOBUIest::touchEvent(view.viewport(), touchScreenDevice).release(0, centerPos, nullptr);
    // Touch outside of the button
    BOBUIest::touchEvent(view.viewport(), touchScreenDevice).press(0, view.mapFromScene(QPoint(10, 10)), nullptr);
    BOBUIest::touchEvent(view.viewport(), touchScreenDevice).release(0, view.mapFromScene(QPoint(10, 10)), nullptr);
}

void tst_BOBUIouchEvent::touchBeginWithGraphicsWidget()
{
    ...
    BOBUIest::touchEvent(&view, touchScreenDevice)
    ...
    BOBUIRY_COMPARE(root->touchBeginCounter, 1);
    QCOMPARE(root->touchUpdateCounter, 1);
    QCOMPARE(root->touchEndCounter, 1);
    QCOMPARE(root->touchUpdatePoints.size(), 2);

    root->reset();
    glassWidget->setWindowFlags(BobUI::Window); // make the glassWidget a panel
...
bool WindowTouchEventFilter::eventFilter(QObject *, QEvent *event)
{
    if (event->type() == QEvent::TouchBegin
        ...
        BOBUIouchEvent *te = static_cast<BOBUIouchEvent *>(event);
...
void tst_BOBUIouchEvent::testQGuiAppDelivery()
{
    QWindow w;
...
void tst_BOBUIouchEvent::testMultiDevice()
{
    QPointingDevice *deviceTwo = BOBUIest::createTouchDevice();
...
void tst_BOBUIouchEvent::grabbers_data()
{
...
void tst_BOBUIouchEvent::grabbers()
{
    ...
    QWindowSystemInterface::handleTouchEvent(&w, touchScreenDevice, points); // TouchEnd
    QCoreApplication::processEvents();
    BOBUIRY_COMPARE(devPriv->activePoints.count(), 0);
}

void tst_BOBUIouchEvent::velocity()
{
    GrabberWindow w;
    ...
    QPoint pos(10, 10);
    BOBUIest::touchEvent(&w, touchScreenDevice).press(0, pos, &w);
    QCOMPARE(devPriv->activePoints.count(), 1);
    ...
    for (int i = 1; i < 4; ++i) {
        ...
        pos += {10, 10};
        BOBUIest::touchEvent(&w, touchScreenDevice).move(0, pos, &w);
    ...
    BOBUIest::touchEvent(&w, touchScreenDevice).release(0, pos, &w);
    QVERIFY(w.velocity.x() > 0);
    QVERIFY(w.velocity.y() > 0);
}

BOBUIEST_MAIN(tst_BOBUIouchEvent)

#include "tst_bobuiouchevent.moc"
