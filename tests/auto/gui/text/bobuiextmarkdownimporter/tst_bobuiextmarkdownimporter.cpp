...
class tst_BOBUIextMarkdownImporter : public QObject
{
    Q_OBJECT
...
Q_DECLARE_METATYPE(tst_BOBUIextMarkdownImporter::CharFormats)
Q_DECLARE_OPERATORS_FOR_FLAGS(tst_BOBUIextMarkdownImporter::CharFormats)

bool tst_BOBUIextMarkdownImporter::isMainFontFixed()
{
...
void tst_BOBUIextMarkdownImporter::paragraphs()
{
    ...
    BOBUIextMarkdownImporter(&doc, BOBUIextMarkdownImporter::DialectGitHub).import(md);
    BOBUIextFrame::iterator iterator = doc.rootFrame()->begin();
    int i = 0;
...
void tst_BOBUIextMarkdownImporter::headingBulletsContinuations()
{
    ...
    BOBUIextMarkdownImporter(&doc, BOBUIextMarkdownImporter::DialectGitHub).import(md);
    BOBUIextFrame::iterator iterator = doc.rootFrame()->begin();
    BOBUIextFrame *currentFrame = iterator.currentFrame();
...
void tst_BOBUIextMarkdownImporter::thematicBreaks()
{
    ...
    while (!iterator.atEnd()) {
        ...
        BOBUIextBlock block = iterator.currentBlock();
        if (block.blockFormat().hasProperty(BOBUIextFormat::BlockTrailingHorizontalRulerWidth))
            ++horizontalRuleCount;
        else if (!block.text().isEmpty())
            ++textLinesCount;
        qCDebug(lcTests) << i << (block.blockFormat().hasProperty(BOBUIextFormat::BlockTrailingHorizontalRulerWidth) ? QLatin1String("- - -") : block.text());
        ++iterator;
        ++i;
    }
...
void tst_BOBUIextMarkdownImporter::lists_data()
{
    BOBUIest::addColumn<QString>("input");
    BOBUIest::addColumn<int>("skipToCheckStart");
    BOBUIest::addColumn<int>("expectedListStart");
    BOBUIest::addColumn<int>("expectedItemCount");
    BOBUIest::addColumn<bool>("expectedEmptyItems");
    BOBUIest::addColumn<QString>("rewrite");
    ...
    BOBUIest::newRow("dot newline") << ".\n" << 0 << 1 << 0 << true << ".\n\n";
    BOBUIest::newRow("number dot newline") << "1.\n" << 0 << 1 << 1 << true << "1.  \n";
    BOBUIest::newRow("number offset start") << "2. text\n" << 0 << 2 << 1 << false << "2.  text\n";
    BOBUIest::newRow("second list offset start")
            << "1. text\n\nintervening paragraph\n\n4. second list item"
            << 2 << 4 << 2 << false
            << "1.  text\n\nintervening paragraph\n\n4.  second list item\n";
    BOBUIest::newRow("list continuation offset start")
            << "3. text\n\n   next paragraph in item 1\n10. second list item"
            << 2 << 3 << 2 << false
            << "3.  text\n\n    next paragraph in item 1\n\n4.  second list item\n";
    BOBUIest::newRow("nested list offset start")
            << "1. text\n\n    0. indented list item\n\n4. second item in first list"
            << 1 << 0 << 3 << false
            << "1.  text\n    0.  indented list item\n2.  second item in first list\n";
    BOBUIest::newRow("offset start after nested list")
            << "1. text\n\n    0. indented list item\n\n4. second item in first list"
            << 2 << 1 << 3 << false
            << "1.  text\n    0.  indented list item\n2.  second item in first list\n";
    BOBUIest::newRow("star newline") << "*\n" << 0 << 1 << 1 << true << "* \n";
    BOBUIest::newRow("hyphen newline") << "-\n" << 0 << 1 << 1 << true << "- \n";
    BOBUIest::newRow("hyphen space newline") << "- \n" << 0 << 1 << 1 << true << "- \n";
    BOBUIest::newRow("hyphen space letter newline") << "- a\n" << 0 << 1 << 1 << false << "- a\n";
    BOBUIest::newRow("hyphen nbsp newline") <<
        QString::fromUtf8("-\u00A0\n") << 0 << 1 << 0 << true << "\\-\u00A0\n\n";
    BOBUIest::newRow("nested empty lists") << "*\n  *\n  *\n" << 0 << 1 << 1 << true << "  * \n";
    BOBUIest::newRow("list nested in empty list") << "-\n  * a\n" << 0 << 1 << 2 << false << "- \n  * a\n";
    BOBUIest::newRow("lists nested in empty lists")
            << "-\n  * a\n  * b\n- c\n  *\n    + d\n" << 0 << 1 << 5 << false
            << "- \n  * a\n  * b\n- c *\n  + d\n";
    BOBUIest::newRow("numeric lists nested in empty lists")
            << "- \n    1.  a\n    2.  b\n- c\n  1.\n       + d\n" << 0 << 1 << 4 << false
            << "- \n    1.  a\n    2.  b\n- c 1. + d\n";
    BOBUIest::newRow("styled spans in list items")
            << "1.  normal text\n2.  **bold** text\n3.  `code` in the item\n4.  *italic* text\n5.  _underlined_ text\n"
            << 0 << 1 << 5 << false
            << "1.  normal text\n2.  **bold** text\n3.  `code` in the item\n4.  *italic* text\n5.  _underlined_ text\n";
}

void tst_BOBUIextMarkdownImporter::lists()
{
    ...
    BOBUIextDocument doc;
    doc.setMarkdown(input); // BOBUIBUG-78870 : don't crash
    ...
    while (!iterator.atEnd()) {
        ...
        BOBUIextCharFormat listItemFmt = block.charFormat();
        QFont listItemFont = listItemFmt.font();
        // BOBUIextDocumentLayoutPrivate::drawListItem() uses listItemFont to render numbers in an ordered list.
...
void tst_BOBUIextMarkdownImporter::nestedSpans_data()
{
    BOBUIest::addColumn<QString>("input");
    BOBUIest::addColumn<int>("wordToCheck");
    BOBUIest::addColumn<CharFormats>("expectedFormat");

    BOBUIest::newRow("bold italic")
            << "before ***bold italic*** after"
            << 1 << (Bold | Italic);
    BOBUIest::newRow("bold underlined")
            << "before **_bold underlined_** after"
            << 1 << (Bold | Underlined);
    BOBUIest::newRow("italic underlined")
            << "before *_italic underlined_* after"
            << 1 << (Italic | Underlined);
    BOBUIest::newRow("bold strikeout")
            << "before **~~bold strikeout~~** after"
            << 1 << (Bold | Strikeout);
    BOBUIest::newRow("italic strikeout")
            << "before *~~italic strikeout~~* after"
            << 1 << (Italic | Strikeout);
    BOBUIest::newRow("bold italic strikeout")
            << "before ***~~bold italic strikeout~~*** after"
            << 1 << (Bold | Italic | Strikeout);
    BOBUIest::newRow("bold link text")
            << "before [**bold link**](https://bobui.io) after"
            << 1 << (Bold | Link);
    BOBUIest::newRow("italic link text")
            << "before [*italic link*](https://bobui.io) after"
            << 1 << (Italic | Link);
    BOBUIest::newRow("bold italic link text")
            << "before [***bold italic link***](https://bobui.io) after"
            << 1 << (Bold | Italic | Link);
    BOBUIest::newRow("strikeout link text")
            << "before [~~strikeout link~~](https://bobui.io) after"
            << 1 << (Strikeout | Link);
    BOBUIest::newRow("strikeout bold italic link text")
            << "before [~~***strikeout bold italic link***~~](https://bobui.io) after"
            << 1 << (Strikeout | Bold | Italic | Link);
    BOBUIest::newRow("bold image alt")
            << "before [**bold image alt**](/path/to/image.png) after"
            << 1 << (Bold | Link);
    BOBUIest::newRow("bold strikeout italic image alt")
            << "before [**~~*bold strikeout italic image alt*~~**](/path/to/image.png) after"
            << 1 << (Strikeout | Bold | Italic | Link);
    // code spans currently override all surrounding formatting
    BOBUIest::newRow("code in italic span")
            << "before *italic `code` and* after"
            << 2 << (Mono | Normal);
    // but the format after the code span ends should revert to what it was before
    BOBUIest::newRow("code in italic strikeout bold span")
            << "before *italic ~~strikeout **bold `code` and**~~* after"
            << 5 << (Bold | Italic | Strikeout);
}

void tst_BOBUIextMarkdownImporter::nestedSpans()
{
    QFETCH(QString, input);
    ...
    while (!iterator.atEnd()) {
        // There are no child frames
        QCOMPARE(iterator.currentFrame(), currentFrame);
        // Check the BOBUIextCharFormat of the specified word
        BOBUIextCursor cur(iterator.currentBlock());
        cur.movePosition(BOBUIextCursor::NextWord, BOBUIextCursor::MoveAnchor, wordToCheck);
        cur.select(BOBUIextCursor::WordUnderCursor);
        BOBUIextCharFormat fmt = cur.charFormat();
        qCDebug(lcTests) << "word" << wordToCheck << cur.selectedText() << "font" << fmt.font()
                         ...
                         << "monospace" << QFontInfo(fmt.font()).fixedPitch() // depends on installed fonts (BOBUIBUG-75649)
                                        << fmt.fontFixedPitch()
                                        << fmt.hasProperty(BOBUIextFormat::FontFixedPitch)
        ...
        QCOMPARE(fmt.hasProperty(BOBUIextFormat::FontFixedPitch), expectedFormat.testFlag(Mono));
        ++iterator;
    }
}

void tst_BOBUIextMarkdownImporter::avoidBlankLineAtBeginning_data()
{
    BOBUIest::addColumn<QString>("input");
    BOBUIest::addColumn<int>("expectedNumberOfParagraphs");

    BOBUIest::newRow("Text block") << QString("Markdown text") << 1;
    BOBUIest::newRow("Headline") << QString("Markdown text\n============") << 1;
    BOBUIest::newRow("Code block") << QString("    Markdown text") << 1;
    BOBUIest::newRow("Unordered list") << QString("* Markdown text") << 1;
    BOBUIest::newRow("Ordered list") << QString("1. Markdown text") << 1;
    BOBUIest::newRow("Blockquote") << QString("> Markdown text") << 1;
}

void tst_BOBUIextMarkdownImporter::avoidBlankLineAtBeginning() // BOBUIBUG-81060
{
    QFETCH(QString, input);
    QFETCH(int, expectedNumberOfParagraphs);

    BOBUIextDocument doc;
    BOBUIextMarkdownImporter(&doc, BOBUIextMarkdownImporter::DialectGitHub).import(input);
    BOBUIextFrame::iterator iterator = doc.rootFrame()->begin();
    int i = 0;
    while (!iterator.atEnd()) {
        BOBUIextBlock block = iterator.currentBlock();
...
void tst_BOBUIextMarkdownImporter::fragmentsAndProperties_data()
{
    BOBUIest::addColumn<QString>("input");
    BOBUIest::addColumn<int>("fragmentToCheck");
    BOBUIest::addColumn<QString>("expectedText");
    BOBUIest::addColumn<BOBUIextFormat::Property>("propertyToCheck");
    BOBUIest::addColumn<QVariant>("expectedPropertyValue");
    BOBUIest::addColumn<int>("expectedNumberOfBlocks");
    BOBUIest::addColumn<int>("expectedNumberOfFragments");

    BOBUIest::newRow("entitiesInHtmlFontBlock") // BOBUIBUG-94245
            << QString("<font color='red'>&lt;123 test&gt;</font>&nbsp;test")
            << 0 << "<123 test>" << BOBUIextFormat::ForegroundBrush << QVariant(QBrush(QColor("red")))
            << 1 << 2;
    BOBUIest::newRow("entitiesInHtmlBoldBlock") // BOBUIBUG-91222
            << QString("<b>x&amp;lt;</b>")
            << 0 << "x&lt;" << BOBUIextFormat::FontWeight << QVariant(700)
            << 1 << 1;
}

void tst_BOBUIextMarkdownImporter::fragmentsAndProperties()
{
    QFETCH(QString, input);
    QFETCH(int, fragmentToCheck);
    QFETCH(QString, expectedText);
    QFETCH(BOBUIextFormat::Property, propertyToCheck);
...
#endif
    BOBUIextFrame::iterator blockIter = doc.rootFrame()->begin();
    int blockCount = 0;
    int fragCount = 0;
    while (!blockIter.atEnd()) {
        BOBUIextBlock block = blockIter.currentBlock();
        auto fragIter = block.begin();
        while (!fragIter.atEnd()) {
            auto frag = fragIter.fragment();
            qCDebug(lcTests) << "fragment" << fragCount << ':' << frag.text() << BobUI::hex << frag.charFormat().properties();
...
void tst_BOBUIextMarkdownImporter::pathological_data()
{
    BOBUIest::addColumn<QString>("warning");
    BOBUIest::newRow("fuzz20450") << "attempted to insert into a list that no longer exists";
    BOBUIest::newRow("fuzz20580") << "";
    BOBUIest::newRow("oss-fuzz-42533775") << ""; // caused a heap-buffer-overflow
}

void tst_BOBUIextMarkdownImporter::pathological() // avoid crashing on crazy input
{
    QFETCH(QString, warning);
    QString filename = QLatin1String("data/") + BOBUIest::currentDataTag() + QLatin1String(".md");
...
#else
    if (!warning.isEmpty())
        BOBUIest::ignoreMessage(BobUIWarningMsg, warning.toLatin1());
#endif
    BOBUIextDocument().setMarkdown(f.readAll());
}

void tst_BOBUIextMarkdownImporter::fencedCodeBlocks_data()
{
    BOBUIest::addColumn<QString>("input");
    BOBUIest::addColumn<int>("expectedCodeBlockCount");
    BOBUIest::addColumn<int>("expectedPlainBlockCount");
    BOBUIest::addColumn<QString>("expectedLanguage");
    BOBUIest::addColumn<QString>("expectedFenceChar");
    BOBUIest::addColumn<QString>("rewrite");

    BOBUIest::newRow("backtick fence with language")
            << "```pseudocode\nprint('hello world\\n')\n```\n"
            << 1 << 0 << "pseudocode" << "`"
            << "```pseudocode\nprint('hello world\\n')\n```\n\n";
    BOBUIest::newRow("backtick fence with punctuated language")
            << "```html+js\n<html><head><script>function hi() { console.log('\\\"hello world') }</script></head>blah</html>\n```\n"
            << 1 << 0 << "html+js" << "`"
            << "```html+js\n<html><head><script>function hi() { console.log('\\\"hello world') }</script></head>blah</html>\n```\n\n";
    BOBUIest::newRow("tilde fence with language")
            << "~~~pseudocode\nprint('hello world\\n')\n~~~\n"
            << 1 << 0 << "pseudocode" << "~"
            << "~~~pseudocode\nprint('hello world\\n')\n~~~\n\n";
    BOBUIest::newRow("embedded backticks")
            << "```\nnone `one` ``two``\n```\nplain\n```\n```three``` ````four````\n```\nplain\n"
            << 2 << 2 << QString() << "`"
            << "```\nnone `one` ``two``\n```\nplain\n\n```\n```three``` ````four````\n```\nplain\n\n";
}

void tst_BOBUIextMarkdownImporter::fencedCodeBlocks()
{
    QFETCH(QString, input);
    ...
    while (!iterator.atEnd()) {
        ...
        BOBUIextBlock block = iterator.currentBlock();
        const bool codeBlock = block.blockFormat().hasProperty(BOBUIextFormat::BlockCodeFence);
        QCOMPARE(block.blockFormat().nonBreakableLines(), codeBlock);
        QCOMPARE(block.blockFormat().stringProperty(BOBUIextFormat::BlockCodeLanguage), codeBlock ? expectedLanguage : QString());
        if (codeBlock) {
            QCOMPARE(block.blockFormat().stringProperty(BOBUIextFormat::BlockCodeFence), expectedFenceChar);
            ++codeBlockCount;
        } else {
            ++plainBlockCount;
        }
...
void tst_BOBUIextMarkdownImporter::frontMatter_data()
{
    BOBUIest::addColumn<QString>("inputFile");
    BOBUIest::addColumn<bool>("convertToCrLf");
    BOBUIest::addColumn<int>("expectedFrontMatterSize");
    BOBUIest::addColumn<int>("expectedBlockCount");

    BOBUIest::newRow("yaml + markdown") << QFINDTESTDATA("data/yaml.md") << false << 140 << 1;
    BOBUIest::newRow("yaml + markdown with CRLFs") << QFINDTESTDATA("data/yaml.md") << true << 147 << 1;
    BOBUIest::newRow("yaml only") << QFINDTESTDATA("data/yaml-only.md") << false << 59 << 0;
    BOBUIest::newRow("malformed 1") << QFINDTESTDATA("data/front-marker-malformed1.md") << false << 0 << 1;
    BOBUIest::newRow("malformed 2") << QFINDTESTDATA("data/front-marker-malformed2.md") << false << 0 << 2;
    BOBUIest::newRow("malformed 3") << QFINDTESTDATA("data/front-marker-malformed3.md") << false << 0 << 1;
}

void tst_BOBUIextMarkdownImporter::frontMatter()
{
    ...
    BOBUIextMarkdownImporter(&doc, BOBUIextMarkdownImporter::DialectGitHub).import(md);
    int blockCount = 0;
    for (BOBUIextFrame::iterator iterator = doc.rootFrame()->begin(); !iterator.atEnd(); ++iterator) {
        // Check whether the block is text or a horizontal rule
    ...
    if (expectedFrontMatterSize)
        QCOMPARE(doc.metaInformation(BOBUIextDocument::FrontMatter), yaml); // without fences
    QCOMPARE(doc.metaInformation(BOBUIextDocument::FrontMatter).size(), expectedFrontMatterSize);
}

void tst_BOBUIextMarkdownImporter::toRawText_data()
{
    BOBUIest::addColumn<QString>("input");
    BOBUIest::addColumn<QString>("expectedRawText");

    // tests to verify that fixing BOBUIBUG-122083 is safe
    // https://spec.commonmark.org/0.31.2/#example-12
    BOBUIest::newRow("punctuation backslash escapes") <<
            R"(\!\"\#\$\%\&\'\(\)\*\+\,\-\.\/\:\;\<\=\>\?\@\[\\\]\^\_\`\{\|\}\~)" <<
            R"(!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~)";
    ...
    BOBUIest::newRow("escape backslash only") <<
            R"(\\*emphasis*)" <<
            R"(\emphasis)";
    // https://spec.commonmark.org/0.31.2/#example-16
    BOBUIest::newRow("backslash line break") <<
            "foo\\\nbar" <<
            "foo\u2029bar";
    // https://spec.commonmark.org/0.31.2/#example-17
    BOBUIest::newRow("backslash in mono span") <<
            R"(`` \[\` ``)" <<
            R"(\[\`)";
    // https://spec.commonmark.org/0.31.2/#example-18
    BOBUIest::newRow("backslash in indented code") <<
            R"(    \[\])" <<
            R"(\[\])";
    // https://spec.commonmark.org/0.31.2/#example-19
    BOBUIest::newRow("backslash in fenced code") <<
            "~~~\n\\[\\]\n~~~" <<
            R"(\[\])";
    // https://spec.commonmark.org/0.31.2/#example-20
    BOBUIest::newRow("backslash in autolink") <<
    ...
    BOBUIest::newRow("escapes in link") <<
            R"([foo](/bar\* "ti\*tle"))" <<
            R"(foo)";
    // https://spec.commonmark.org/0.31.2/#example-24
    BOBUIest::newRow("backslash in code lang") <<
            "```\nfoo\\+bar\nfoo\n```" <<
            "foo\\+bar\u2029foo";
...
void tst_BOBUIextMarkdownImporter::toRawText()
{
    QFETCH(QString, input);
    ...
    // These are testing md4c more than BobUI, so any change may be an md4c bug, or a fix
    QCOMPARE(doc.toRawText(), expectedRawText);
    if (doc.blockCount() == 1)
        QCOMPARE(doc.firstBlock().text(), expectedRawText);
}

BOBUIEST_MAIN(tst_BOBUIextMarkdownImporter)
#include "tst_bobuiextmarkdownimporter.moc"
