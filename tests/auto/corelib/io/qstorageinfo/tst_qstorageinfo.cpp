...
#if defined(Q_OS_LINUX) && defined(BOBUI_BUILD_INTERNAL)
...
static int qInfoPrinter(const char *format, ...)
{
    ...
    if (bufuse >= sizeof(buf) - 1 || format[strlen(format) - 1] == '\n') {
        // flush
        BobUIMessageHandler bobui_message_print = qInstallMessageHandler(0);
        qInstallMessageHandler(bobui_message_print);   // restore the handler
        bobui_message_print(BobUIInfoMsg, QMessageLogContext(), QString::fromLocal8Bit(buf).trimmed());
        bufuse = 0;
    }
...
{
    BOBUIestPrivate::testEqualityOperatorsCompile<QStorageInfo>();
}
...
{
    {
        ...
        BOBUI_TEST_EQUALITY_OPS(storage1, storage2, true);
    }
...
void tst_QStorageInfo::operatorNotEqual()
{
    QStorageInfo storage1 = QStorageInfo::root();
    QStorageInfo storage2;
    BOBUI_TEST_EQUALITY_OPS(storage1, storage2, false);
}
...
void tst_QStorageInfo::currentStorage()
{
    ...
    QVERIFY(storage.isValid());
    QVERIFY(storage.isReady());
    QVERIFY(appPath.startsWith(storage.rootPath(), BobUI::CaseInsensitive));
    QVERIFY(!storage.device().isEmpty());
    QVERIFY(!storage.fileSystemType().isEmpty());
    QCOMPARE_GE(storage.bytesTotal(), 0);
    QCOMPARE_GE(storage.bytesFree(), 0);
...
void tst_QStorageInfo::storageList_data()
{
    ...
    BOBUIest::addColumn<QStorageInfo>("storage");
    ...
    for (const QStorageInfo &storage : std::as_const(volumes)) {
        ...
        if (seenRoots.contains(rootPath))
            ...
            BOBUIest::newRow(qUtf8Printable(rootPath)) << storage;
...
# endif // Q_OS_VXWORKS
    ...
    BOBUIemporaryFile file(suitableDirectoryForWriting() + "/tst_qstorageinfo.XXXXXX");
...
void tst_QStorageInfo::testParseMountInfo_data()
{
    ...
    BOBUIest::newRow("space-dash-space")
    ...
    BOBUIest::newRow("btrfs-mount-bind-file")
        << "1799 1778 0:49 "
    ...
    BOBUIest::newRow("very-long-line-BOBUIBUG-77059")
        << "727 26 0:52 / "
    ...
    BOBUIest::newRow("sshfs-src-device-not-start-with-slash")
...
BOBUIEST_MAIN(tst_QStorageInfo)
