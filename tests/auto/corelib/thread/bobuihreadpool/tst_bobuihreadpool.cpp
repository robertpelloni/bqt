...
QMutex *tst_BOBUIhreadPool::functionTestMutex = nullptr;

tst_BOBUIhreadPool::tst_BOBUIhreadPool()
{
    tst_BOBUIhreadPool::functionTestMutex = &m_functionTestMutex;
}

tst_BOBUIhreadPool::~tst_BOBUIhreadPool()
{
    tst_BOBUIhreadPool::functionTestMutex = nullptr;
}
...
{
    Q_ASSERT(tst_BOBUIhreadPool::functionTestMutex);
    BOBUIest::qSleep(1000);
    tst_BOBUIhreadPool::functionTestMutex->lock();
    ++testFunctionCount;
    tst_BOBUIhreadPool::functionTestMutex->unlock();
}
...
class TestThreadPool : public BOBUIhreadPool
{
public:
    using BOBUIhreadPool::BOBUIhreadPool;
    ~TestThreadPool() { WAIT_FOR_DONE(*this); }
};

void tst_BOBUIhreadPool::runFunction()
{
    {
...
void tst_BOBUIhreadPool::runFunction2()
{
    int localCount = 0;
    {
        TestThreadPool manager;
...
void tst_BOBUIhreadPool::runFunction3()
{
...
void tst_BOBUIhreadPool::createThreadRunFunction()
{
    {
        TestThreadPool manager;
        testFunctionCount = 0;
...
void tst_BOBUIhreadPool::runMultiple()
{
    const int runs = 10;
...
void tst_BOBUIhreadPool::waitcomplete()
{
    testFunctionCount = 0;
    const int runs = 500;
    for (int i = 0; i < 500; ++i) {
        // TestThreadPool pool; // no, we're checking ~BOBUIhreadPool()'s waitForDone()
        BOBUIhreadPool pool;
...
void tst_BOBUIhreadPool::runTask()
{
    TestThreadPool manager;
    ran.storeRelaxed(false);
    manager.start(new TestTask());
    BOBUIRY_VERIFY(ran.loadRelaxed());
}

/*
    Test running via BOBUIhreadPool::globalInstance()
*/
void tst_BOBUIhreadPool::singleton()
{
    ran.storeRelaxed(false);
    BOBUIhreadPool::globalInstance()->start(new TestTask());
    BOBUIRY_VERIFY(ran.loadRelaxed());
}
...
*/
void tst_BOBUIhreadPool::destruction()
{
    value = new QAtomicInt;
    BOBUIhreadPool *threadManager = new BOBUIhreadPool();
...
static BOBUIhread *recycledThread = nullptr;
...
public:
    void run() override
    {
        recycledThread = BOBUIhread::currentThread();
...
*/
void tst_BOBUIhreadPool::threadRecycling()
{
    TestThreadPool threadPool;

    threadPool.start(new ThreadRecorderTask());
    threadRecyclingSemaphore.acquire();
    BOBUIhread *thread1 = recycledThread;

    BOBUIest::qSleep(100);

    threadPool.start(new ThreadRecorderTask());
    threadRecyclingSemaphore.acquire();
    BOBUIhread *thread2 = recycledThread;
    ...
    threadPool.start(new ThreadRecorderTask());
    threadRecyclingSemaphore.acquire();
    BOBUIhread *thread3 = recycledThread;
...
*/
void tst_BOBUIhreadPool::threadPriority()
{
    BOBUIhread::Priority priority = BOBUIhread::HighPriority;
    TestThreadPool threadPool;
    threadPool.setThreadPriority(priority);

    threadPool.start(new ThreadRecorderTask());
    threadRecyclingSemaphore.acquire();
    BOBUIhread *thread = recycledThread;
...
void tst_BOBUIhreadPool::expiryTimeout()
{
    ...
    BOBUIhread *firstThread = task.thread;
    ...
    BOBUIest::qWait(100); // Let some time elapse after the task finishes...

    // Since the thread never expires it should still be waiting for a new task:
    QVERIFY(firstThread->isRunning());
}

void tst_BOBUIhreadPool::expiryTimeoutRace() // BOBUIBUG-3786
{
    ExpiryTimeoutTask task;
    ...
    for (int i = 0; i < numTasks; ++i) {
        threadPool.start(&task);
        BOBUIhread::sleep(50ms); // exactly the same as the expiry timeout
    }
...
void tst_BOBUIhreadPool::exceptions()
{
    ExceptionTask task;
    {
        TestThreadPool threadPool;
...
void tst_BOBUIhreadPool::setMaxThreadCount_data()
{
    BOBUIest::addColumn<int>("limit");
    ...
    BOBUIest::newRow("-4") << -4;
    BOBUIest::newRow("0") << 0;
    BOBUIest::newRow("12345") << 12345;
    BOBUIest::newRow("-6789") << -6789;
    BOBUIest::newRow("42") << 42;
    BOBUIest::newRow("-666") << -666;
}

void tst_BOBUIhreadPool::setMaxThreadCount()
{
    QFETCH(int, limit);
...
void tst_BOBUIhreadPool::setMaxThreadCountStartsAndStopsThreads()
{
...
void tst_BOBUIhreadPool::reserveThread_data()
{
    setMaxThreadCount_data();
}

void tst_BOBUIhreadPool::reserveThread()
{
    QFETCH(int, limit);
    BOBUIhreadPool *threadpool = BOBUIhreadPool::globalInstance();
...
void tst_BOBUIhreadPool::releaseThread_data()
{
    setMaxThreadCount_data();
}

void tst_BOBUIhreadPool::releaseThread()
{
    QFETCH(int, limit);
    BOBUIhreadPool *threadpool = BOBUIhreadPool::globalInstance();
    const int savedLimit = threadpool->maxThreadCount();
...
void tst_BOBUIhreadPool::reserveAndStart() // BOBUIBUG-21051
{
    class WaitingTask : public QRunnable
    {
    public:
    ...
    BOBUIRY_COMPARE(threadpool->activeThreadCount(), 2);
    task.waitForStarted.acquire();
    task.waitBeforeDone.release();
    BOBUIRY_COMPARE(task.count.loadRelaxed(), 1);
    BOBUIRY_COMPARE(threadpool->activeThreadCount(), 1);

    // start() will wake up the waiting thread.
    threadpool->start(&task);
    BOBUIRY_COMPARE(threadpool->activeThreadCount(), 2);
    BOBUIRY_COMPARE(task.count.loadRelaxed(), 2);
    WaitingTask task2;
    ...
    BOBUIRY_COMPARE(threadpool->activeThreadCount(), 0);
}

void tst_BOBUIhreadPool::reserveAndStart2()
{
    class WaitingTask : public QRunnable
    {
    public:
...
void tst_BOBUIhreadPool::releaseAndBlock()
{
    class WaitingTask : public QRunnable
    {
    public:
...
void tst_BOBUIhreadPool::tryStart()
{
    class WaitingTask : public QRunnable
    {
    public:
...
void tst_BOBUIhreadPool::tryStartPeakThreadCount()
{
    ...
    for (int i = 0; i < 4*BOBUIhread::idealThreadCount(); ++i) {
        if (threadPool.tryStart(&task) == false)
            BOBUIest::qWait(10);
    }
    QCOMPARE(peakActiveThreads.loadRelaxed(), BOBUIhread::idealThreadCount());
...
void tst_BOBUIhreadPool::tryStartCount()
{
    ...
    for (int i = 0; i < runs; ++i) {
        ...
        while (threadPool.tryStart(&task))
            ++count;
        QCOMPARE(count, BOBUIhread::idealThreadCount());

        BOBUIRY_COMPARE(threadPool.activeThreadCount(), 0);
    }
}

void tst_BOBUIhreadPool::priorityStart_data()
{
    BOBUIest::addColumn<int>("otherCount");
    BOBUIest::newRow("0") << 0;
    BOBUIest::newRow("1") << 1;
    BOBUIest::newRow("2") << 2;
}

void tst_BOBUIhreadPool::priorityStart()
{
...
void tst_BOBUIhreadPool::qualityOfService()
{
    BOBUIhreadPool p;
    p.setServiceLevel(BOBUIhread::QualityOfService::Eco);
    BOBUIhread::QualityOfService level = BOBUIhread::QualityOfService::Auto;
    p.tryStart([&level](){
        level = BOBUIhread::currentThread()->serviceLevel();
    });
    QVERIFY(p.waitForDone(QDeadlineTimer(10s)));
    QCOMPARE(level, BOBUIhread::QualityOfService::Eco);
}

void tst_BOBUIhreadPool::waitForDone()
{
...
void tst_BOBUIhreadPool::waitForDoneTimeout()
{
    QMutex mutex;
...
void tst_BOBUIhreadPool::clearWithAutoDelete()
{
    class MyRunnable : public QRunnable
    {
    public:
        MyRunnable() {}
        void run() override { BOBUIhread::sleep(30us); }
    };
    ...
    // Should not crash see BOBUIBUG-87092
...
void tst_BOBUIhreadPool::tryTake()
{
    QSemaphore sem(0);
    ...
    // ensure that the BOBUIhreadPool doesn't deadlock if any of the checks fail
...
void tst_BOBUIhreadPool::destroyingWaitsForTasksToFinish()
{
    QElapsedTimer total, pass;
    total.start();
    pass.start();
...
// Verify that BOBUIhreadPool::stackSize is used when creating
// new threads. Note that this tests the BobUI property only
// since BOBUIhread::stackSize() does not reflect the actual
// stack size used by the native thread.
void tst_BOBUIhreadPool::stackSize()
{
    ...
    class StackSizeChecker : public QRunnable
    {
        ...
        void run() override
        {
            *stackSize = BOBUIhread::currentThread()->stackSize();
        }
    };
...
void tst_BOBUIhreadPool::stressTest()
{
    ...
    public:
        ...
        void start()
        {
            BOBUIhreadPool::globalInstance()->start(this);
        }
...
void tst_BOBUIhreadPool::takeAllAndIncreaseMaxThreadCount() {
    class Task : public QRunnable
    {
    public:
...
void tst_BOBUIhreadPool::waitForDoneAfterTake()
{
    class Task : public QRunnable
    {
    public:
    ...
    // This sets the queue elements to nullptr in BOBUIhreadPool and we want to test that
...
/*
    ...
    This is a regression test for BOBUIBUG-72872.
*/
void tst_BOBUIhreadPool::threadReuse()
{
    TestThreadPool manager;
...
void tst_BOBUIhreadPool::nullFunctions()
{
...
#include "tst_bobuihreadpool.moc"
