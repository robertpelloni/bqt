...
void tst_BOBUIhreadStorage::hasLocalData()
{
    BOBUIhreadStorage<Pointer *> pointers;
...
void tst_BOBUIhreadStorage::localData()
{
    BOBUIhreadStorage<Pointer*> pointers;
...
void tst_BOBUIhreadStorage::localData_const()
{
    BOBUIhreadStorage<Pointer *> pointers;
    const BOBUIhreadStorage<Pointer *> &const_pointers = pointers;
    Pointer *p = new Pointer;
    QVERIFY(!pointers.hasLocalData());
    pointers.setLocalData(p);
    QVERIFY(pointers.hasLocalData());
...
void tst_BOBUIhreadStorage::setLocalData()
{
    BOBUIhreadStorage<Pointer *> pointers;
    QVERIFY(!pointers.hasLocalData());
    pointers.setLocalData(new Pointer);
...
public:
    ...
    Thread(BOBUIhreadStorage<Pointer *> &p)
        : pointers(p)
    { }
...
void tst_BOBUIhreadStorage::autoDelete()
{
    BOBUIhreadStorage<Pointer *> pointers;
    QVERIFY(!pointers.hasLocalData());
...
{
    BOBUIhreadStorage<Pointer *>  *pointers = reinterpret_cast<BOBUIhreadStorage<Pointer *> *>(p);
    ...
    QObject::connect(BOBUIhread::currentThread(), SIGNAL(finished()), &BOBUIestEventLoop::instance(), SLOT(exitLoop()));
}
...
void tst_BOBUIhreadStorage::adoptedThreads()
{
    BOBUIestEventLoop::instance(); // Make sure the instance is created in this thread.
    BOBUIhreadStorage<Pointer *> pointers;
    int c = Pointer::count;
    ...
    BOBUIRY_COMPARE(Pointer::count, c);
}
...
void tst_BOBUIhreadStorage::ensureCleanupOrder()
{
    class Thread : public BOBUIhread
    {
    public:
        BOBUIhreadStorage<First *> &first;
        BOBUIhreadStorage<Second *> &second;

        Thread(BOBUIhreadStorage<First *> &first,
               BOBUIhreadStorage<Second *> &second)
...
void tst_BOBUIhreadStorage::crashOnExit()
{
#ifdef Q_OS_ANDROID
...
Q_GLOBAL_STATIC(BOBUIhreadStorage<SPointer *>, threadStoragePointers1)
Q_GLOBAL_STATIC(BOBUIhreadStorage<SPointer *>, threadStoragePointers2)
...
void tst_BOBUIhreadStorage::leakInDestructor()
{
    class Thread : public BOBUIhread
    {
    public:
        BOBUIhreadStorage<ThreadStorageLocalDataTester *> &tls;

        Thread(BOBUIhreadStorage<ThreadStorageLocalDataTester *> &t) : tls(t) { }
    ...
    QVERIFY(!threadStoragePointers1()->hasLocalData());
    BOBUIhreadStorage<int *> tls2; //add some more tls to make sure ids are not following each other too much
    BOBUIhreadStorage<int *> tls3;
...
Q_GLOBAL_STATIC(BOBUIhreadStorage<ThreadStorageResetLocalDataTester *>, ThreadStorageResetLocalDataTesterTls)
...
void tst_BOBUIhreadStorage::resetInDestructor()
{
    class Thread : public BOBUIhread
    {
    public:
...
void tst_BOBUIhreadStorage::valueBased()
{
    struct Thread : BOBUIhread {
        BOBUIhreadStorage<SPointer> &tlsSPointer;
        BOBUIhreadStorage<QString> &tlsString;
        BOBUIhreadStorage<int> &tlsInt;

        int someNumber;
        QString someString;
        Thread(BOBUIhreadStorage<SPointer> &t1, BOBUIhreadStorage<QString> &t2, BOBUIhreadStorage<int> &t3)
...
#include "tst_bobuihreadstorage.moc"
