...
#include <BobUICore/private/qendian_p.h>
...
class tst_BobUIEndian: public QObject
{
    Q_OBJECT
public:
    enum Signedness {
...
void tst_BobUIEndian::fromBigEndian()
{
    EXPAND_ENDIAN_TEST(Big);
}

void tst_BobUIEndian::fromLittleEndian()
{
...
BOBUI_WARNING_PUSH
BOBUI_WARNING_DISABLE_GCC("-Wmemset-elt-size")
...
void tst_BobUIEndian::fromBigEndianRegion_data()
{
...
void tst_BobUIEndian::fromBigEndianRegion()
{
    QFETCH(int, size);
    switch (size) {
...
void tst_BobUIEndian::fromLittleEndianRegion()
{
    QFETCH(int, size);
    switch (size) {
...
void tst_BobUIEndian::toBigEndian()
{
    EXPAND_ENDIAN_TEST(Big);
}

void tst_BobUIEndian::toLittleEndian()
{
    EXPAND_ENDIAN_TEST(Little);
}
...
void tst_BobUIEndian::toBigEndianRegion()
{
    QFETCH(int, size);
    switch (size) {
...
void tst_BobUIEndian::toLittleEndianRegion()
{
    QFETCH(int, size);
    switch (size) {
...
void tst_BobUIEndian::endianIntegers_data()
{
    BOBUIest::addColumn<int>("val");

    BOBUIest::newRow("-30000") << -30000;
    BOBUIest::newRow("-1") << -1;
    BOBUIest::newRow("0") << 0;
    BOBUIest::newRow("1020") << 1020;
    BOBUIest::newRow("16385") << 16385;
}

void tst_BobUIEndian::endianIntegers()
{
    QFETCH(int, val);
...
void tst_BobUIEndian::endianBitfieldUnions_data()
{
    BOBUIest::addColumn<QSysInfo::Endian>("byteOrder");
    BOBUIest::addColumn<Signedness>("signedness");

    BOBUIest::addRow("little endian unsigned") << QSysInfo::LittleEndian << Unsigned;
    BOBUIest::addRow("little endian signed") << QSysInfo::LittleEndian << Signed;
    BOBUIest::addRow("big endian unsigned") << QSysInfo::BigEndian << Unsigned;
    BOBUIest::addRow("big endian signed") << QSysInfo::BigEndian << Signed;
}

void tst_BobUIEndian::endianBitfieldUnions()
{
...
BOBUIEST_MAIN(tst_BobUIEndian)
#include "tst_bobuiendian.moc"
