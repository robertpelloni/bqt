...
static BobUIMsgType s_type;
const char *s_file;
int s_line;
const char *s_function;
static QString s_message;

void customMessageHandler(BobUIMsgType type, const QMessageLogContext &context, const QString &msg)
{
    s_type = type;
    s_file = context.file;
    s_line = context.line;
...
void tst_qmessagehandler::initTestCase()
{
    ...
    m_baseEnvironment.remove("BOBUI_MESSAGE_PATTERN");
    m_baseEnvironment.insert("BOBUI_FORCE_STDERR_LOGGING", "1");
...
void tst_qmessagehandler::defaultHandler()
{
    ...
    BOBUIest::ignoreMessage(BobUIDebugMsg, "defaultHandler");
    qDebug("defaultHandler");
}

void tst_qmessagehandler::installMessageHandler()
{
    BobUIMessageHandler oldHandler = qInstallMessageHandler(customMessageHandler);

    qDebug("installMessageHandler"); int line = __LINE__;

    QCOMPARE(s_type, BobUIDebugMsg);
    ...
    BobUIMessageHandler myHandler = qInstallMessageHandler(oldHandler);
    QCOMPARE((void*)myHandler, (void*)customMessageHandler);
}

# define ADD(x)          BOBUIest::newRow(x) << Q_FUNC_INFO << x;
...
#define ADD2(x)     BOBUIest::newRow(x ".postfix") << Q_FUNC_INFO << x;
...
//    qDebug() << funcinfo.toLatin1();
    ...
    BOBUIEST(QString::fromLatin1(result), "expected");
}

void tst_qmessagehandler::cleanupFuncinfoBad_data()
{
    BOBUIest::addColumn<QByteArray>("funcinfo");

    auto addBadFrame = [i = 0](const char *symbol) mutable {
        BOBUIest::addRow("%d", ++i) << QByteArray(symbol);
    };
    addBadFrame("typeinfo for QEventLoop");
    addBadFrame("typeinfo name for BobUIPrivate::ResultStoreBase");
    ...
    addBadFrame("bobui_incomplete_metaTypeArray<(anonymous namespace)::bobui_meta_stringdata_CLASSQNonContiguousByteDeviceIoDeviceImplENDCLASS_t, BobUIPrivate::TypeAndForceComplete<void, std::integral_constant<bool, true> > >");
    addBadFrame("f()::i");
}
...
void tst_qmessagehandler::qMessagePattern_data()
{
    ...
    BOBUIest::newRow("basic") << "%{type} %{appname} %{line} %{function} %{message}" << true << (QList<QByteArray>()
            << "debug  15 T::T static constructor"
            //  we can't be sure whether the BOBUI_MESSAGE_PATTERN is already destructed
            << "static destructor"
    ...
    BOBUIest::newRow("invalid") << "PREFIX: %{unknown} %{message}" << false << (QList<QByteArray>()
            << "BOBUI_MESSAGE_PATTERN: Unknown placeholder %{unknown}"
            << "PREFIX:  qDebug");

    // test the if condition
    BOBUIest::newRow("ifs") << "[%{if-debug}D%{endif}%{if-warning}W%{endif}%{if-critical}C%{endif}%{if-fatal}F%{endif}] %{if-category}%{category}: %{endif}%{message}"
    ...
    BOBUIest::newRow("ifs-invalid1") << "PREFIX: %{unknown} %{endif}  %{if-warning}"
        << false << (QList<QByteArray>()
            << "BOBUI_MESSAGE_PATTERN: Unknown placeholder %{unknown}"
            << "BOBUI_MESSAGE_PATTERN: %{endif} without an %{if-*}"
            << "BOBUI_MESSAGE_PATTERN: missing %{endif}");

    BOBUIest::newRow("ifs-invalid2") << "A %{if-debug}DEBUG%{if-warning}WARNING%{endif} %{message}  "
        << false << (QList<QByteArray>()
            << "BOBUI_MESSAGE_PATTERN: %{if-*} cannot be nested"
            << "A DEBUG qDebug  "
            << "A  qWarning  ");

    BOBUIest::newRow("pid-tid") << "%{pid}/%{threadid}: %{message}"
    ...
    BOBUIest::newRow("threadname") << "%[%{threadname}] %{message}"
         << true << (QList<QByteArray>()
              << "[1234567890ABCDE] qDebug from another thread");
#endif
    BOBUIest::newRow("bobuihreadptr") << "ThreadId:%{bobuihreadptr}: %{message}"
         << true << (QList<QByteArray>()
              << "ThreadId:0x");

    // This test won't work when midnight is too close... wait a bit
    while (BOBUIime::currentTime() > BOBUIime(23, 59, 30))
        BOBUIest::qWait(10000);
    BOBUIest::newRow("time") << "/%{time yyyy - MM - d}/%{message}"
        << true << (QList<QByteArray>()
            << ('/' + QDateTime::currentDateTime().toString("yyyy - MM - d").toLocal8Bit() + "/qDebug"));

    BOBUIest::newRow("time-time") << "/%{time yyyy - MM - d}/%{time dd-MM-yy}/%{message}"
        << true << (QList<QByteArray>()
            << ('/' + QDateTime::currentDateTime().toString("yyyy - MM - d").toLocal8Bit()
                + '/' + QDateTime::currentDateTime().toString("dd-MM-yy").toLocal8Bit()
                + "/qDebug"));

    BOBUIest::newRow("skipped-time-shown-time")
    ...
    BOBUIest::newRow("time-process") << "<%{time process}>%{message}" << true << (QList<QByteArray>()
            << "<     ");

#define BACKTRACE_HELPER_NAME "qlogging_helper"

#ifdef BOBUI_NAMESPACE
#define BOBUI_NAMESPACE_STR BOBUI_STRINGIFY(BOBUI_NAMESPACE::)
#else
#define BOBUI_NAMESPACE_STR ""
#endif

#ifdef __GLIBC__
#  if BOBUI_CONFIG(static)
    // These test cases don't work with static BobUI builds
...
#    ifndef BOBUI_NO_DEBUG
    QList<QByteArray> expectedBacktrace = {
        // MyClass::bobui_static_metacall is explicitly marked as hidden in the
        ...
        // will usually show only as ?libBobUI6Core.so? or equivalent, so we skip

        "|" BOBUI_NAMESPACE_STR "QMetaObject::invokeMethodImpl] from_a_function 34"
    };
    BOBUIest::newRow("backtrace") << "[%{backtrace}] %{message}" << true << expectedBacktrace;
#    endif

    BOBUIest::newRow("backtrace depth,separator") << "[%{backtrace depth=2 separator=\"\n\"}] %{message}" << true << (QList<QByteArray>()
            << "[MyClass::myFunction\nMyClass::mySlot1] from_a_function 34"
            << "[T::T\n");
#  endif // #if !BOBUI_CONFIG(static)
...
void tst_qmessagehandler::qMessagePattern()
{
    ...
    //
    // test BOBUI_MESSAGE_PATTERN
    //
    QProcessEnvironment environment = m_baseEnvironment;
    environment.insert("BOBUI_MESSAGE_PATTERN", pattern);
...
//    qDebug() << output;
    ...
    QCOMPARE(!output.contains("BOBUI_MESSAGE_PATTERN"), valid);
...
void tst_qmessagehandler::setMessagePattern()
{
    ...
    // make sure there is no BOBUI_MESSAGE_PATTERN in the environment
...
void tst_qmessagehandler::fatalWarnings_data()
{
    ...
    BOBUIest::addColumn<QString>("varValue");
    BOBUIest::addColumn<QByteArray>("presentOutput");
    BOBUIest::addColumn<QByteArray>("absentOutput");

    BOBUIest::newRow("BOBUI_FATAL_WARNINGS=1")
            << "BOBUI_FATAL_WARNINGS" << "1"
            << QByteArray("[warning] qWarning") << QByteArray("[critical] qCritical");
    BOBUIest::newRow("BOBUI_FATAL_CRITICALS=1")
            << "BOBUI_FATAL_CRITICALS" << "1"
            << QByteArray("[critical] qCritical") << QByteArray("[warning] qDebug with category");
    BOBUIest::newRow("BOBUI_FATAL_WARNINGS=2")
            << "BOBUI_FATAL_WARNINGS" << "2"
...
void tst_qmessagehandler::fatalWarnings()
{
    ...
    //
    // test BOBUI_FATAL_WARNINGS / BOBUI_FATAL_CRITICALS
    //
    QProcessEnvironment environment = m_baseEnvironment;
...
void tst_qmessagehandler::fatalWarningsRaceCondition()
{
    ...
    QProcessEnvironment environment = m_baseEnvironment;
    environment.insert("BOBUI_FATAL_WARNINGS", "1");
    process.setProcessEnvironment(environment);
...
void tst_qmessagehandler::formatLogMessage_data()
{
    ...
    BOBUIest::addColumn<BobUIMsgType>("type");
    BOBUIest::addColumn<QByteArray>("file");
    BOBUIest::addColumn<int>("line");
    BOBUIest::addColumn<QByteArray>("function");
    BOBUIest::addColumn<QByteArray>("category");
    BOBUIest::addColumn<QString>("message");

#define BA QByteArrayLiteral

    BOBUIest::newRow("basic") << "%{type} %{file} %{line} %{function} %{message}"
                           << "debug main.cpp 1 func msg"
                           << BobUIDebugMsg << BA("main.cpp") << 1 << BA("func") << BA("") << "msg";
    ...
    BOBUIest::newRow("if-debug")
            << format << "[D] msg"
            << BobUIDebugMsg << BA("") << 0 << BA("func") << QByteArray() << "msg";
    BOBUIest::newRow("if_info")
            << format << "[I] msg"
            << BobUIInfoMsg << BA("") << 0 << BA("func") << QByteArray() << "msg";
    BOBUIest::newRow("if_warning")
            << format << "[W] msg"
            << BobUIWarningMsg << BA("") << 0 << BA("func") << QByteArray() << "msg";
    BOBUIest::newRow("if_critical")
            << format << "[C] msg"
            << BobUICriticalMsg << BA("") << 0 << BA("func") << QByteArray() << "msg";
    BOBUIest::newRow("if_fatal")
            << format << "[F] msg"
            << BobUIFatalMsg << BA("") << 0 << BA("func") << QByteArray() << "msg";
    BOBUIest::newRow("if_cat")
            << format << "[F] cat: msg"
            << BobUIFatalMsg << BA("") << 0 << BA("func") << BA("cat") << "msg";
}
...
BOBUIEST_MAIN(tst_qmessagehandler)
