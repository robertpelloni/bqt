...
from option_management import need_separate_output_sequence, bobui_quirk_case
...
def OutputSequence(toptions):
    ...
    # quirk of bobui: not a QFuture<Sequence> is returned
    if bobui_quirk_case(toptions):
...
def function_template_args(options):
    ...
    if options["reduce"] and options["reductionfunction"] == "lambda":
        ...
        if not bobui_quirk_case(options):
            args.append(out_s)
...
def generate_testcase(function_options, testcase_options):
    ...
    if options["pool"]:
        pool_initialization = """BOBUIhreadPool pool;
    ...
    if options["reduce"]:
        ...
        if options["reductionoptions"] == "UnorderedReduce":
            arguments.append("BobUIConcurrent::UnorderedReduce")
        elif options["reductionoptions"] == "OrderedReduce":
            arguments.append("BobUIConcurrent::OrderedReduce")
        elif options["reductionoptions"] == "SequentialReduce":
            arguments.append("BobUIConcurrent::SequentialReduce")
        else:
    ...
    final_string = f"""
        void tst_BobUIConcurrentFilterMapGenerated::{testcase_name}()
        {{
            /* test for
{function_signature}
...
{option_description}
            */
            ...
            {result_accepting}BobUIConcurrent::{function_name}{template_args}({arguments_passing});
