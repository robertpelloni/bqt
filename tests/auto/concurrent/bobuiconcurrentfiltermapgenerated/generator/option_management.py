...
def skip_function_description(options):
    ...
    # the following things are skipped because BobUI does not support them
    if options["filter"] and options["map"]:
        return "Not supported by BobUI: both map and filter operation"

    if not options["filter"] and not options["map"]:
        return "Not supported by BobUI: no map and no filter operation"

    if options["inplace"] and options["iterators"] and options["filter"]:
        return "Not supported by BobUI: filter operation in-place with iterators"

    return False


def bobui_quirk_case(options):
