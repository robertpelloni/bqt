...
class tst_BobUIConcurrentIterateKernel: public QObject
{
    Q_OBJECT
private slots:
    // "for" iteration tests:
...
{
public:
    PrintFor(TestIterator begin, TestIterator end) : IterateKernel<TestIterator, void>(BOBUIhreadPool::globalInstance(), begin, end) { iterations.storeRelaxed(0); }
    bool runIterations(TestIterator/*beginIterator*/, int begin, int end, void *) override
    {
        iterations.fetchAndAddRelaxed(end - begin);
#ifdef PRINT
        qDebug() << BOBUIhread::currentThread() << "iteration" << begin <<  "to" << end << "(exclusive)";
#endif
        return false;
    }
...
{
public:
    SleepPrintFor(TestIterator begin, TestIterator end) : IterateKernel<TestIterator, void>(BOBUIhreadPool::globalInstance(), begin, end) { iterations.storeRelaxed(0); }
...
void tst_BobUIConcurrentIterateKernel::instantiate()
{
...
void tst_BobUIConcurrentIterateKernel::cancel()
{
    {
        ...
        QVERIFY2(iterations.loadRelaxed() <= BOBUIhread::idealThreadCount(),
                 (QByteArray::number(iterations.loadRelaxed()) + ' ' + QByteArray::number(BOBUIhread::idealThreadCount())));
    }
}
...
{
public:
    CountFor(TestIterator begin, TestIterator end) : IterateKernel<TestIterator, void>(BOBUIhreadPool::globalInstance(), begin, end) { iterations.storeRelaxed(0); }
    inline bool runIterations(TestIterator/*beginIterator*/, int begin, int end, void *) override
    {
...
void tst_BobUIConcurrentIterateKernel::stresstest()
{
...
void tst_BobUIConcurrentIterateKernel::noIterations()
{
    const int times = 20000;
    for (int i = 0; i < times; ++i) {
        auto future = startThreadEngine(new IterateKernel<TestIterator, void>(
                                                BOBUIhreadPool::globalInstance(), 0, 0))
                              .startAsynchronously();
        future.waitForFinished();
    }
}
...
public:
    ...
    ThrottleFor(TestIterator begin, TestIterator end) : IterateKernel<TestIterator, void>(BOBUIhreadPool::globalInstance(), begin, end) { iterations.storeRelaxed(0); throttling = false; }
    inline bool runIterations(TestIterator/*beginIterator*/, int begin, int end, void *) override
    {
        ...
        BOBUIhread *thread = BOBUIhread::currentThread();
...
void tst_BobUIConcurrentIterateKernel::throttling()
{
...
{
public:
    MultipleResultsFor(TestIterator begin, TestIterator end) : IterateKernel<TestIterator, int>(BOBUIhreadPool::globalInstance(), begin, end) { }
...
void tst_BobUIConcurrentIterateKernel::multipleResults()
{
...
BOBUIEST_MAIN(tst_BobUIConcurrentIterateKernel)

#include "tst_bobuiconcurrentiteratekernel.moc"
