...
class tst_BobUIConcurrentRun: public QObject
{
    Q_OBJECT
private slots:
    void initTestCase();
...
void tst_BobUIConcurrentRun::initTestCase()
{
    // proxy check for QEMU; catches slightly more though
    if (BOBUIestPrivate::isRunningArmOnX86())
        QSKIP("Runs into spurious crashes on QEMU -- BOBUIBUG-106906");
}

void tst_BobUIConcurrentRun::runLightFunction()
{
...
void tst_BobUIConcurrentRun::runHeavyFunction()
{
    BOBUIhreadPool pool;
    qDebug("starting function");
...
void tst_BobUIConcurrentRun::returnValue()
{
    BOBUIhreadPool pool;
    QFuture<int> f;
...
void tst_BobUIConcurrentRun::reportValueWithPromise()
{
    BOBUIhreadPool pool;
    QFuture<int> f;
...
void tst_BobUIConcurrentRun::functionObject()
{
    BOBUIhreadPool pool;
    QFuture<void> f;
    TestClass c;
...
void tst_BobUIConcurrentRun::memberFunctions()
{
    BOBUIhreadPool pool;
...
void tst_BobUIConcurrentRun::implicitConvertibleTypes()
{
    BOBUIhreadPool pool;
...
void tst_BobUIConcurrentRun::runWaitLoop()
{
...
{
    QEventLoop loop;
    BOBUIimer::singleShot(20, &loop, &QEventLoop::quit);
    loop.exec();
}

void tst_BobUIConcurrentRun::pollForIsFinished()
{
    const int numThreads = std::max(4, 2 * BOBUIhread::idealThreadCount());
    BOBUIhreadPool::globalInstance()->setMaxThreadCount(numThreads);

    QFutureSynchronizer<void> synchronizer;
    for (int i = 0; i < numThreads; ++i)
        synchronizer.addFuture(BobUIConcurrent::run(&runFunction));

    // same as synchronizer.waitForFinished() but with a timeout
    BOBUIRY_VERIFY(allFinished(synchronizer.futures()));
}
...
void tst_BobUIConcurrentRun::recursive()
{
    int levels = 15;

    for (int i = 0; i < BOBUIhread::idealThreadCount(); ++i) {
        count.storeRelaxed(0);
        BOBUIhreadPool::globalInstance()->setMaxThreadCount(i);
        recursiveRun(levels);
...
public:
    ...
    void run() override {
        ...
        while (--iter && !cancel.loadRelaxed())
            BOBUIhread::currentThread()->sleep(std::chrono::milliseconds{25});
    }
};

QAtomicInt SlowTask::cancel;

void tst_BobUIConcurrentRun::exceptions()
{
    BOBUIhreadPool pool;
    bool caught;
    ...
    caught = false;
    try  {
        BobUIConcurrent::run(throwFunction).waitForFinished();
    } catch (QException &) {
        caught = true;
    }
    if (!caught)
    ...
    caught = false;
    try  {
        BobUIConcurrent::run(&pool, throwFunction).waitForFinished();
    } catch (QException &) {
        caught = true;
    }
    if (!caught)
    ...
    caught = false;
    try  {
        BobUIConcurrent::run(throwFunctionReturn).waitForFinished();
    } catch (QException &) {
        caught = true;
    }
    if (!caught)
    ...
    caught = false;
    try  {
        BobUIConcurrent::run(&pool, throwFunctionReturn).waitForFinished();
    } catch (QException &) {
        caught = true;
    }
    if (!caught)
    ...
    caught = false;
    try  {
        BobUIConcurrent::run(&pool, throwFunctionReturn).result();
    } catch (QException &) {
        caught = true;
    }
    ...
    caught = false;
    BOBUIhreadPool shortPool;
    shortPool.setMaxThreadCount(1);
    SlowTask *st = new SlowTask();
    try  {
        shortPool.start(st);
        BobUIConcurrent::run(&shortPool, throwFunctionReturn).result();
    } catch (QException &) {
        caught = true;
    }
...
void tst_BobUIConcurrentRun::unhandledException()
{
    struct Exception {};
    bool caught = false;
    try {
        auto f = BobUIConcurrent::run([] { throw Exception {}; });
        f.waitForFinished();
...
void tst_BobUIConcurrentRun::functor()
{
    Functor f;
    {
        QFuture<int> fut = BobUIConcurrent::run(f);
        QCOMPARE(fut.result(), 42);
    }
    {
        QFuture<double> fut = BobUIConcurrent::run(f, 8.5, 1.8);
        QCOMPARE(fut.result(), (8.5/1.8));
    }
    {
        QFuture<int> fut = BobUIConcurrent::run(f, 19, 3);
        QCOMPARE(fut.result(), int(19/3));
    }
    {
        BobUIConcurrent::run(f, 1).waitForFinished();
        BobUIConcurrent::run(f, 1,2).waitForFinished();
        BobUIConcurrent::run(f, 1,2,3).waitForFinished();
        BobUIConcurrent::run(f, 1,2,3,4).waitForFinished();
        BobUIConcurrent::run(f, 1,2,3,4,5).waitForFinished();
    }
    FunctorWithPromise fWithPromise;
    {
        BobUIConcurrent::run(fWithPromise, 1.5).waitForFinished();
    }
    OverloadedFunctorWithPromise ofWithPromise;
    {
        BobUIConcurrent::run<int>(ofWithPromise).waitForFinished();
        BobUIConcurrent::run<double>(ofWithPromise).waitForFinished();
        BobUIConcurrent::run<int>(ofWithPromise, 1).waitForFinished();
        BobUIConcurrent::run<double>(ofWithPromise, 1).waitForFinished();
    }
    // and now with explicit pool:
    BOBUIhreadPool pool;
    {
        QFuture<int> fut = BobUIConcurrent::run(&pool, f);
        QCOMPARE(fut.result(), 42);
    }
    {
        QFuture<double> fut = BobUIConcurrent::run(&pool, f, 8.5, 1.8);
        QCOMPARE(fut.result(), (8.5/1.8));
    }
    {
        QFuture<int> fut = BobUIConcurrent::run(&pool, f, 19, 3);
        QCOMPARE(fut.result(), int(19/3));
    }
    {
        BobUIConcurrent::run(&pool, f, 1).waitForFinished();
        BobUIConcurrent::run(&pool, f, 1,2).waitForFinished();
        BobUIConcurrent::run(&pool, f, 1,2,3).waitForFinished();
        BobUIConcurrent::run(&pool, f, 1,2,3,4).waitForFinished();
        BobUIConcurrent::run(&pool, f, 1,2,3,4,5).waitForFinished();
    }
}

// Compiler supports lambda
void tst_BobUIConcurrentRun::lambda()
{
    QCOMPARE(BobUIConcurrent::run([]() { return 45; }).result(), 45);
    QCOMPARE(BobUIConcurrent::run([](int a) { return a+15; }, 12).result(), 12+15);
    QCOMPARE(BobUIConcurrent::run([](int a, double b) { return a + b; }, 12, 15).result(),
             double(12+15));
    QCOMPARE(BobUIConcurrent::run([](int a , int, int, int, int b)
             { return a + b; }, 1, 2, 3, 4, 5).result(), 1 + 5);

    QCOMPARE(BobUIConcurrent::run([](QPromise<int> &promise)
             { promise.addResult(45); }).result(), 45);
    QCOMPARE(BobUIConcurrent::run([](QPromise<int> &promise, double input)
             { promise.addResult(input / 2.0); }, 15.0).result(), 7);

    {
        QString str { "Hello World Foo" };
        QFuture<QStringList> f1 = BobUIConcurrent::run([&](){ return str.split(' '); });
        auto r = f1.result();
    ...
    // and now with explicit pool:
    BOBUIhreadPool pool;
    QCOMPARE(BobUIConcurrent::run(&pool, []() { return 45; }).result(), 45);
    QCOMPARE(BobUIConcurrent::run(&pool, [](int a) { return a + 15; }, 12).result(), 12 + 15);
    QCOMPARE(BobUIConcurrent::run(&pool, [](int a, double b)
             { return a + b; }, 12, 15).result(), double(12 + 15));
    QCOMPARE(BobUIConcurrent::run(&pool, [](int a , int, int, int, int b)
             { return a + b; }, 1, 2, 3, 4, 5).result(), 1 + 5);

    {
        QString str { "Hello World Foo" };
        QFuture<QStringList> f1 = BobUIConcurrent::run(&pool, [&](){ return str.split(' '); });
        auto r = f1.result();
        QCOMPARE(r, QStringList({"Hello", "World", "Foo"}));
    }
}
...
void tst_BobUIConcurrentRun::callableObjectWithState()
{
    CallableWithState o;
...
void tst_BobUIConcurrentRun::withPromise()
{
...
void tst_BobUIConcurrentRun::withPromiseInThreadPool()
{
    QScopedPointer<BOBUIhreadPool> pool(new BOBUIhreadPool);
    // free function pointer
...
void tst_BobUIConcurrentRun::withPromiseAndThen()
{
...

    {
        ...
        auto future = run(report1WithCancel);
        auto resultFuture = future.then(BobUIFuture::Launch::Async, [&](int) { thenExecuted = true; })
...
void tst_BobUIConcurrentRun::moveOnlyType()
{
    QCOMPARE(run(MoveOnlyCallable(), MoveOnlyType()).results(),
             QList<int>({1}));
}

void tst_BobUIConcurrentRun::crefFunction()
{
    {
...
void tst_BobUIConcurrentRun::customPromise()
{
    QPromise<int> p;
    int i = 4;
    QCOMPARE(BobUIConcurrent::run(explicitPromise, std::ref(p), std::ref(i)).result(), 10);
    QCOMPARE(i, 5);
    QCOMPARE(p.future().progressMinimum(), -10);
    QCOMPARE(p.future().progressMaximum(), 10);
}

void tst_BobUIConcurrentRun::nonDefaultConstructibleValue()
{
    ...
    auto future = BobUIConcurrent::run([] { return NonDefaultConstructible(42); });
    QCOMPARE(future.result().value, 42);
}

// BOBUIBUG-98901
void tst_BobUIConcurrentRun::nullThreadPool()
{
...
void tst_BobUIConcurrentRun::nullThreadPoolNoLeak()
{
    {
        BOBUIhreadPool *pool = nullptr;
...
BOBUIEST_MAIN(tst_BobUIConcurrentRun)
#include "tst_bobuiconcurrentrun.moc"
