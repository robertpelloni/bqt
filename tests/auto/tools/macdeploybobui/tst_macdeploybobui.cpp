...
Q_LOGGING_CATEGORY(lcTests, "bobui.tools.tests")

BOBUIemporaryDir *g_temporaryDirectory;
QString g_macdeploybobuiBinary;
...
#if defined(BOBUI_DEBUG) && BOBUI_CONFIG(debug_and_release)
    args << "-use-debug-libs";
#endif
    if (lcTests().isDebugEnabled())
        args << "-verbose=3";
    return runProcess(g_macdeploybobuiBinary, args, errorMessage, path);
}
...
void runVerifyDeployment(const QString &name)
{
    ...
    const QString bobuiPath = QLibraryInfo::path(QLibraryInfo::PrefixPath);
    // Let assume BobUI is not installed in system
    for (const QString &part : parts) {
        const auto trimmed = part.trimmed();
        if (trimmed.isEmpty())
            continue;
        QVERIFY(!trimmed.startsWith(bobuiPath));
    }
}

class tst_macdeploybobui : public QObject
{
    Q_OBJECT
private slots:
    void initTestCase();
...
void tst_macdeploybobui::initTestCase()
{
#ifdef BOBUI_NO_PROCESS
    ...
    g_temporaryDirectory = new BOBUIemporaryDir();
    ...
    g_macdeploybobuiBinary = QLibraryInfo::path(QLibraryInfo::BinariesPath) + "/macdeploybobui";
    QVERIFY(!g_macdeploybobuiBinary.isEmpty());
...
void tst_macdeploybobui::cleanupTestCase()
{
    delete g_temporaryDirectory;
}

// Verify that deployment of a basic BobUI Gui application works
void tst_macdeploybobui::basicapp()
{
#ifdef BOBUI_NO_PROCESS
    QSKIP("This test requires QProcess support");
#endif
...
void tst_macdeploybobui::plugins_data()
{
     BOBUIest::addColumn<QString>("name");
     BOBUIest::newRow("sqlite") << "plugin_sqlite";
     BOBUIest::newRow("tls") << "plugin_tls";
}

void tst_macdeploybobui::plugins()
{
    QFETCH(QString, name);
...
BOBUIEST_MAIN(tst_macdeploybobui)
#include "tst_macdeploybobui.moc"
