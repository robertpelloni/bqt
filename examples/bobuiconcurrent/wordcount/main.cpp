...
WordCount singleThreadedWordCount(const QStringList &files)
{
    ...
    for (const QString &file : files) {
        ...
        BOBUIextStream textStream(&f);
        while (!textStream.atEnd()) {
            const auto words =  textStream.readLine().split(' ', BobUI::SkipEmptyParts);
...
int main(int argc, char** argv)
{
    ...
    qint64 mapReduceTime;
    {
        ...
        timer.start();
    //! [2]
        WordCount total = BobUIConcurrent::mappedReduced(files, countWords, reduce).result();
    //! [2]
...
QStringList findFiles(const QString &startDir)
{
    ...
    for (const QString &file : files) {
        ...
        for (const auto &i : mimeTypesForFile) {
            if (i.contains("text", BobUI::CaseInsensitive)
                || mime.comment().contains("text", BobUI::CaseInsensitive)) {
