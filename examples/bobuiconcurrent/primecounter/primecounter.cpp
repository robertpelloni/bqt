    ...
    if (ui->pushButton->isChecked()) {
        ...
        watcher.setFuture(
            BobUIConcurrent::filteredReduced(
                ...
                filterFunction,
                reduceFunction,
                currentReduceOpt | BobUIConcurrent::SequentialReduce));
//! [2]
    } else {
        watcher.cancel();
...
Ui::PrimeCounter* PrimeCounter::setupUi()
{
    ...
    setupUI->comboBox->insertItem(0, tr("Unordered Reduce"), BobUIConcurrent::UnorderedReduce);
    setupUI->comboBox->insertItem(1, tr("Ordered Reduce"), BobUIConcurrent::OrderedReduce);

    auto comboBoxChange = [this, setupUI](int pos) {
        currentReduceOpt = setupUI->comboBox->itemData(pos).value<BobUIConcurrent::ReduceOptions>();
