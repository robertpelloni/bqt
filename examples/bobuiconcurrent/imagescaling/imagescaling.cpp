...
//! [3]
        ...
        downloadFuture
                .then([this](auto) {
                    ...
                    //! [16]
                    scalingWatcher.setFuture(BobUIConcurrent::run(Images::scaled,
...
//! [9]
    ...
    //! [11]
        BobUIFuture::connect(reply.get(), &QNetworkReply::finished).then([=] {
...
Images::OptionalImages Images::scaled(const QList<QByteArray> &data)
{
    ...
    for (const auto &imgData : data) {
        ...
        scaled.push_back(image.scaled(100, 100, BobUI::KeepAspectRatio));
    }
...
void Images::showImages(const QList<QImage> &images)
{
    for (int i = 0; i < images.size(); ++i) {
        labels[i]->setAlignment(BobUI::AlignCenter);
